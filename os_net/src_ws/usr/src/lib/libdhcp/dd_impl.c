...
#pragma	ident	"@(#)dd_impl.c	1.14	96/07/18 SMI"
...
_set_nis_path(const char *pathp)
{
	...
	sysinfo((int)SI_SRPC_DOMAIN, domain, (long)(DOM_NM_LN));
	...
	if (getenv(NIS_GROUP_ENV_STR) == NULL) {
		(void) sprintf(ngroup, "%s=admin.%s.", 
					NIS_GROUP_ENV_STR, domain);
		(void) putenv(ngroup);
	}
	return (0);
}
...
fns_get_initial_context()
{
	...
	mutex_lock(&initial_context_lock);
	if (initial_context == NULL) {
		FN_status_t	*status = fn_status_create();
		if (status == NULL) {
			mutex_unlock(&initial_context_lock);
			return (NULL);
		}
		initial_context =
