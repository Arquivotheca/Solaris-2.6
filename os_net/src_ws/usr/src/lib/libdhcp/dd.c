...
#pragma	ident	"@(#)dd.c	1.20	96/06/24 SMI"
	...
	if (tbl_st != NULL) {
		...
		if (tbl_st->ns == TBL_NS_NISPLUS) {
			if (tbl_st->perm.nis.owner_user != NULL)
				free(tbl_st->perm.nis.owner_user);
			if (tbl_st->perm.nis.owner_group != NULL)
				free(tbl_st->perm.nis.owner_group);
		}
		free(tbl_st);
	}
}
...
dd_ns(int *tbl_err, char **pathpp)
{
	...
	if ((fp = fopen(TBL_NS_FILE, "r")) != NULL) {
		...
		case TBL_NS_NISPLUS:
			if (*path == '\0') {
				sysinfo((int)SI_SRPC_DOMAIN, path,
				    (long)MAXPATHLEN);
			} else {
...
check_dd_access(Tbl_stat *tsp, int *errp)
{
	...
	case TBL_NS_NISPLUS:
		...
		/* build nis net name */
		if (uid == (uid_t)0) {
			sysinfo((int)SI_HOSTNAME, hostname,
		...
		sysinfo((int)SI_SRPC_DOMAIN, dombuff, (long)DOM_NM_LN);
