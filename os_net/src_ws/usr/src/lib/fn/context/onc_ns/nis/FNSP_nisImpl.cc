...
#pragma ident	"@(#)FNSP_nisImpl.cc	1.20	96/09/20 SMI"
...
FNSP_nis_orgname_of(FN_string &name, unsigned &status)
{
	...
	status = split_internal_name(table, &map, &domain);
	delete map;
	return (domain);
}
    ...
    unsigned &status)
{
	...
	FN_string *map, *domain;
	status = split_internal_name(parent.get_table_name(), &map, &domain);
...
class FN_nis_namelist_hu : public FN_namelist {
	// Domain and map information
	char domain[FNS_NIS_INDEX];
	char map[FNS_NIS_INDEX];
	// Pointer to the next in the map
	char *inkey;
	int inkeylen;
	// The next name from the the map
	char *next_in_line;
	char *next_ref;
	unsigned int next_status;
	int name_from_keyvalue(char *value, int valuelen);
public:
	FN_nis_namelist_hu(const char *domain, const char *table_name);
...
int
FN_nis_namelist_hu::name_from_keyvalue(char *value, int vallen)
{
	char host[FNS_NIS_INDEX];
	if ((strncmp(value, FNSP_nis_hu_reference,
	    strlen(FNSP_nis_hu_reference)) == 0) ||
	    (strncmp(value, FNSP_nis_hu_context,
	    strlen(FNSP_nis_hu_context)) == 0)) {
		// Found an entry
		strncpy(host, inkey,
		    (inkeylen - strlen(FNSP_bind_suffix)));
		host[inkeylen - strlen(FNSP_bind_suffix)] = '\0';
		next_in_line = (char *) malloc(strlen(host) + 1);
		strcpy(next_in_line, host);
		// Copy the reference value
		next_ref = (char *) malloc(vallen + 1);
		strncpy(next_ref, value, vallen);
		next_ref[vallen] = '\0';
...
FN_nis_namelist_hu::FN_nis_namelist_hu(const char *indomain,
    const char *table_name)
{
	...
	strcpy(domain, indomain);
	strcpy(map, table_name);

	char *outval, *outkey;
	int outvallen, outkeylen, yp_ret;
	if ((yp_ret = yp_first(domain, map, &inkey, &inkeylen,
	    &outval, &outvallen)) == 0) {
		if (!name_from_keyvalue(outval, outvallen))
			while ((yp_ret = yp_next(domain, map, inkey, inkeylen,
	...
	if (yp_ret != 0) {
		if (yp_ret == YPERR_NOMORE) {
			...
			if ((yp_ret = yp_first(domain, map, &inkey, &inkeylen,
			    &outval, &outvallen)) == 0) {
				if (!name_from_keyvalue(outval, outvallen))
					while ((yp_ret = yp_next(domain,
					    map, inkey, inkeylen,
					    &outkey, &outkeylen, &outval,
					    &outvallen)) == 0) {
						inkey = outkey;
						inkeylen = outkeylen;
						if (name_from_keyvalue(outval,
						    outvallen))
							break;
					}
			}
...
FNSP_nisImpl::list_names_hu(unsigned &status)
{
	...
	FN_string *map;
	FN_string *domain;
	status = split_internal_name(my_address->get_table_name(),
	    &map, &domain);
	if (status != FN_SUCCESS)
		return (0);
...
public:
	FN_nis_bindinglist_hu(FNSP_nisImpl *impl);
...
FN_nis_bindinglist_hu::FN_nis_bindinglist_hu(FNSP_nisImpl *impl)
{
	...
	status = split_internal_name(impl->my_address->get_table_name(),
	    &map, &domain);
	if (status == FN_SUCCESS) {
...
FNSP_nisImpl::list_bindings(unsigned &status)
{
	...
	FN_nameset *ns = list_names(status);
	if (status != FN_SUCCESS)
		return (0);
	...
	FN_ref *ref;
	for (aname = ns->first(ip); aname != NULL; aname = ns->next(ip)) {
    ...
    int /* find_legal_name */)
{
	...
	status = split_internal_name(parent_table, &map, &domain);
	if (status != FN_SUCCESS)
		return (0);
    ...
    FN_string **map, FN_string **domain)
{
	...
	status = split_internal_name(parent.get_table_name(),
	    map, domain);
	if (status != FN_SUCCESS)
		return (status);
    ...
    unsigned &status)
{
	...
	char *pw_entry;
	int pw_entrylen;
	int yperr = yp_match((char *)dom, (char *)passwd_map, (char *)user,
	    username.charcount(), &pw_entry, &pw_entrylen);
    ...
    const FN_string &atomic_name, unsigned &status)
{
	...
	if ((yperr = yp_match((char *) domain->str(),
	    (char *) map->str(), map_index, strlen(map_index),
	    &mapentry, &maplen)) != 0)
		status = FN_E_NO_SUCH_ATTRIBUTE;
	delete domain;
...
FNSP_nisImpl::is_org_context()
{
	...
	FN_string *map, *domain;
	if (split_internal_name(my_address->get_table_name(),
