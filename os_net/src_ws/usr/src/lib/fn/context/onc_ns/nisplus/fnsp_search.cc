...
#pragma ident	"@(#)fnsp_search.cc	1.4	96/08/29 SMI"
	...
	unsigned int &status)
{
	...
	if (status != FN_SUCCESS)
		return (0);
	if ((strncmp(table_name, FNSP_org_map,
	    strlen(FNSP_org_map)) == 0) &&
	    (match_attrs)) {
...
FNSP_nisplusImpl::search_attrset(const FN_attrset *match_attrs,
	...
	unsigned int &status)
{
	return (FNSP_search_attrset(*my_address, match_attrs, return_ref,
