...
#pragma ident	"@(#)fnsp_internal.cc	1.28	96/09/24 SMI"
...
{
	return (FNSP_list_names(*my_address, status, children_only));
}
...
{
	return (FNSP_list_bindings(*my_address, status));
}
    ...
    const FN_string &owner)
{
	...
	FNSP_nisplus_address parent_addr(parent);
