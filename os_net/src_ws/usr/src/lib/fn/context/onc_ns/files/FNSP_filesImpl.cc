...
#pragma ident	"@(#)FNSP_filesImpl.cc	1.21	96/09/20 SMI"
...
#include "../nis/fnsp_internal_common.hh"
...
public:
	FN_files_namelist_hu(char *table_name);
    ...
    datum &dbm_value)
{
	...
	if ((strncmp(dbm_value.dptr, FNSP_nis_hu_reference,
		...
		// Found an entry
		strncpy(host, dbm_key.dptr,
		    (dbm_key.dsize - strlen(FNSP_bind_suffix)));
		host[dbm_key.dsize - strlen(FNSP_bind_suffix)] = '\0';
...
FN_files_namelist_hu::FN_files_namelist_hu(char *table_name)
{
	...
	next_ref = 0;
	next_status = FN_SUCCESS;
	if ((db = dbm_open(table_name, O_RDONLY, 0444)) == 0) {
...
FN_nameset *
FNSP_filesImpl::list_names(unsigned &status, int /* children_only */)
{
	FN_nameset *ns = 0;
	const FN_string map = my_address->get_table_name();
...
public:
	FN_files_bindinglist_hu(char *);
	virtual ~FN_files_bindinglist_hu();

	FN_string *next(FN_ref **, FN_status &);
};

FN_files_bindinglist_hu::FN_files_bindinglist_hu(char *mapname)
{
	namelist = new FN_files_namelist_hu(mapname);
}
...
FNSP_filesImpl::destroy_context(const FN_string * /* dirname */)
{
	...
	if (status != FN_SUCCESS) {
		// Remove binding entry, if any
		FNSP_files_update_map(map, map_index, 0, FNSP_map_delete);
		if (status == FN_E_NAME_NOT_FOUND)
			return (FN_SUCCESS);
		else
			return (status);
...
static unsigned
FNSP_files_lookup_user_entry(const char *map_index,
    ...
    char **mapentry, int *maplen)
{
	...
	// Open the file
	if ((file = fopen(hosts_file, "r")) == NULL)
		return (FN_E_INSUFFICIENT_RESOURCES);
    ...
    unsigned &status)
{
	...
	char *pw_entry;
	int pw_entrylen;
	status = FNSP_files_lookup_user_entry(user, &pw_entry, &pw_entrylen);
    ...
    const FN_string &atomic_name, unsigned &status)
{
	...
	if (FNSP_is_hostuser_ctx_type(parent, atomic_name) == 1) {
		user_ctx = 1;
		status = FNSP_files_lookup_user_entry(map_index,
		    &mapentry, &maplen);
	} else
