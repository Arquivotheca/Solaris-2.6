...
#pragma ident	"@(#)FNSP_HUContext.cc	1.11	96/04/02 SMI"
    ...
    FN_status_csvc &cstat)
{
	if (name.is_empty()) {
		...
		if (match_attrs == NULL) {
			matches = FNSP_slist_names(*(ns_impl->my_address),
			    status);
		} else {
...
FN_ext_searchlist *
FNSP_HUContext::c_attr_ext_search(
    const FN_string &name,
