...
#pragma ident	"@(#)fnsp_internal_common.cc	1.11	96/09/20 SMI"
...
static char *
FNSP_encode_xdr_ref(const void *refbuf, int len, unsigned &status)
{
	char *in_buf, *out_buf, *optr, *iptr;
	status = FN_E_INSUFFICIENT_RESOURCES;

	out_buf = (char *) malloc(sizeof (char)*(135*len/100 + 4));
	optr = out_buf;
	in_buf = (char *) malloc(sizeof (char)*(len + 1));
	iptr = in_buf;
	if ((optr == 0) || (iptr == 0))
		return (0);
	else
	...
	memset((void *) iptr, 0, sizeof (char)*(len + 1));
...
static char *
FNSP_decode_xdr_ref(const char *data, int len, unsigned &status)
{
	char *ipb, *opb, ch;
	int octets;
	char *buf = (char *) malloc(sizeof (char)*(135*len/100 + 4));
	if (buf == 0) {
		status = FN_E_INSUFFICIENT_RESOURCES;
		return (0);
	}
	memset((void *) buf, 0, sizeof (char)*(135*len/100 + 4));
    ...
    FNSP_binding_type btype, unsigned &status)
{
	...
	data = FNSP_encode_xdr_ref(refbuf, len, status);
	free(refbuf);
    ...
    FNSP_binding_type &btype, unsigned &status)
{
	...
	data += strlen(data) + strlen(FNSP_sub_context_sept);
	xdr_data = FNSP_decode_xdr_ref(data, len, status);
	if (status != FN_SUCCESS)
		return (0);
	ref = FN_ref_xdr_deserialize(xdr_data, len, status);
	free(xdr_data);
	if (status != FN_SUCCESS)
		return (0);
	return (ref);
...
unsigned split_internal_name(const FN_string &wholename,
    ...
    unsigned &status)
{
	...
	data = FNSP_encode_xdr_ref(attrbuf, len, status);
	free(attrbuf);
	if (status != FN_SUCCESS) {
		return (0);
	}
    ...
    unsigned &status)
{
	...
	if (status != FN_SUCCESS)
		return (0);
	FN_attrset *attrset = FN_attr_xdr_deserialize(xdr_data,
	    len, status);
	free(xdr_data);
	if (status != FN_SUCCESS)
		return (0);
    ...
    char *next_index)
{
	...
	if ((strlen(index) + length) < DBM_LIMIT) {
