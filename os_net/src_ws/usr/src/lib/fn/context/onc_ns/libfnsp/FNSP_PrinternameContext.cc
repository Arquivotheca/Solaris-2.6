...
#pragma ident	"@(#)FNSP_PrinternameContext.cc	1.9	96/10/09 SMI"
    ...
    FN_status_csvc& cstat)
{
	...
	// Get the "onc_fn_printer_" address type from ref_ns
    ...
    FN_status_csvc &cstat)
{
	...
	if (name.is_empty()) {
		...
		unsigned status;
		FN_nameset *ns_source = ns_impl->list_names(status);
		...
		if ((status == FN_SUCCESS) && ns_source) {
			...
			for (name = ns_source->first(ip); name;
			    name = ns_source->next(ip))
				ns_files->add(*name);
			delete ns_source;
			cstat.set_success();
		}
    ...
    FN_status_csvc &cstat)
{
	...
	if (name.is_empty()) {
		...
		if ((status == FN_SUCCESS) && bs_ns) {
			...
			const FN_string *name;
			const FN_ref *ref;
			for (name = bs_ns->first(ip, ref); name;
			    name = bs_ns->next(ip, ref)) {
    ...
    FN_status_csvc &cstat)
{
	if (name.is_empty()) {
		unsigned int status;
		FN_searchset *matches = ns_impl->search_attrset(
...
{
	// %%% needs work
	astat.set_error(FN_E_OPERATION_NOT_SUPPORTED, *my_reference, name);
	return (0);
}
