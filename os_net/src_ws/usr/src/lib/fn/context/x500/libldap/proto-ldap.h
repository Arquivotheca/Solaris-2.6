...
#pragma ident	"@(#)proto-ldap.h	1.1	96/03/31 SMI"
 ...
 * in cache.c
 */
LIBIMPORT int ldap_enable_cache(LDAP *ld, long timeout, long maxmem);
LIBIMPORT void ldap_disable_cache(LDAP *ld);
LIBIMPORT void ldap_set_cache_options(LDAP *ld, unsigned long opts);
...
LIBIMPORT LDAPFiltDesc *ldap_init_getfilter_buf(char *buf, long buflen);
...
LIBIMPORT void ldap_setfilteraffixes(LDAPFiltDesc *lfdp, char *prefix,
	char *suffix);
LIBIMPORT void ldap_build_filter(char *buf, unsigned long buflen,
...
LIBIMPORT void cldap_setretryinfo(LDAP *ld, int tries, int timeout);
...
LIBIMPORT int ldap_t61_to_8859(char **bufp, unsigned long *buflenp,
	int free_input);
LIBIMPORT int ldap_8859_to_t61(char **bufp, unsigned long *buflenp,
	int free_input);
