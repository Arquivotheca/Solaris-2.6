...
#pragma ident  "@(#)xtd_po.c 1.17     96/06/05 SMI"

/*
* MODULE_STD_PO______________________________________________________
*
*  Description:
	...
	td_thrhandle_t *th_p)
{
	...
	if (db_return != PS_OK) {
		...
		th_p->th_unique = (paddr_t)gregset[R_R2];

		/*
		 * Check that the thread struct pointer is not NULL.
		 */
		if (th_p->th_unique == NULL) {
			return_val = TD_NOTHR;
		} else {
			/*
			 * Check that the lwpid matches.
			 */
