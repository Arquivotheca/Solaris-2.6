#ident "@(#)thread_db.c 1.13 95/05/26 SMI"
/*
...
thread_db_from_vcpu(VCpu vcpu, thread_t *tidp)
{
	...
	if (gregp->regs[2] == 0) {
		*tidp = 0;
	} else if (gregp->regs[2] < 0x2000) {
		terror ("small %%r2 (%#x)", gregp->regs[2]);
		return (TDB_ERR);
	} else {
		*tidp = (thread_t)(gregp->regs[2]);
	}
	return (TDB_OK);
#else
#error Unknown architecture!
...
thread_db_get_regs(thread_t tid, struct RegGSet_t *rp, Proc proc)
{
	...
	if (IS_ONPROC(t) || IS_BOUND(t)) {
		...
		*rp = * VCpu_get_regs(vcpu);
		...
		rp->regs[2] = (int)t;
		rp->pc = thread_pc(t);
		rp->regs[1]  = dreadl((Address)&(t->t_sp));
		rp->regs[13] = dreadl((Address)&(t->t_r13));
		rp->regs[14] = dreadl((Address)&(t->t_r14));
		rp->regs[15] = dreadl((Address)&(t->t_r15));
		rp->regs[16] = dreadl((Address)&(t->t_r16));
		rp->regs[17] = dreadl((Address)&(t->t_r17));
		rp->regs[18] = dreadl((Address)&(t->t_r18));
		rp->regs[19] = dreadl((Address)&(t->t_r19));
		rp->regs[20] = dreadl((Address)&(t->t_r20));
		rp->regs[21] = dreadl((Address)&(t->t_r21));
		rp->regs[22] = dreadl((Address)&(t->t_r22));
		rp->regs[23] = dreadl((Address)&(t->t_r23));
		rp->regs[24] = dreadl((Address)&(t->t_r24));
		rp->regs[25] = dreadl((Address)&(t->t_r25));
		rp->regs[26] = dreadl((Address)&(t->t_r26));
		rp->regs[27] = dreadl((Address)&(t->t_r27));
		rp->regs[28] = dreadl((Address)&(t->t_r28));
		rp->regs[29] = dreadl((Address)&(t->t_r29));
		rp->regs[30] = dreadl((Address)&(t->t_r30));
		rp->regs[31] = dreadl((Address)&(t->t_r31));
		rp->cr = dreadl((Address)&(t->t_cr));
#else
#error Unknown architecture!
#endif /* sparc */
	}
