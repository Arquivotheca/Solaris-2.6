/*
*
*ident	"@(#)td_event.c	1.53	96/07/24 SMI"
*
	...
	td_notify_t *notify_p)
{
	...
	notify_p->u.bptaddr = (paddr_t) ta_p->tdb_invar.tdb_events[event];
...
__td_thr_event_getmsg(const td_thrhandle_t *th_p, td_event_msg_t *msg)
{
	...
	if (__td_read_thr_struct(th_p->th_ta_p,	th_p->th_unique, &thr_struct)
	    != TD_OK) {
		(void) ps_pcontinue(ph_p);
		rw_unlock(&(th_p->th_ta_p->rwlock));
		return (TD_ERR);
	}
	if (thr_struct.t_td_evbuf == NULL) {
		(void) ps_pcontinue(ph_p);
		rw_unlock(&(th_p->th_ta_p->rwlock));
		return (TD_NOEVENT);
	}
	if (ps_pdread(ph_p, (paddr_t) thr_struct.t_td_evbuf,
		...
		if (evbuf.eventnum == TD_EVENT_NONE)
			...
			msg->th_p = th_p;
			msg->msg.data = (int) evbuf.eventdata;
			/* "Consume" the message */
			evbuf.eventnum = TD_EVENT_NONE;
			if (ps_pdwrite(ph_p, (paddr_t) thr_struct.t_td_evbuf,
			    (char *) &evbuf, sizeof (evbuf)) != PS_OK)
				td_return = TD_DBERR;
		}
	} else {
...

{
	...
	if (return_val == TD_OK && thr_struct.t_td_evbuf == NULL) {
		/*
		...
		return_val = __td_agent_send(th_p->th_ta_p, &agent_msg);
		if (return_val == TD_OK)
			return_val = agent_msg.result;
	}
	if (return_val == TD_OK && ps_pdread(ph_p,
	    (paddr_t) thr_struct.t_td_evbuf,
	    (char *) &evbuf, sizeof (evbuf)) != PS_OK)
		return_val = TD_DBERR;
	if (return_val == TD_OK) {
		if (onoff)
...

{
	...
	if ((ps_ret = ps_pdread(ta_p->ph_p,
	    ta_p->tdb_invar.tdb_eventmask_addr,
	    (char *) &targ_eventset, sizeof (targ_eventset))) == PS_OK) {
		if (onoff)
