...
#pragma	ident	"@(#)td_to.c	1.77	96/10/21	SMI"
	...
	struct tsd_common * tsd_common_p, void **tsd_array_ptr)
{
	...
	return_val = __td_read_thr_struct(ta_p, th_p->th_unique, &thr_struct);
	if (return_val == TD_OK) {
		...
		*thr_tsd_p = NULL_TSD_T;
		...
		if (thr_struct.t_tls == NULL)
			return_val = TD_NOTSD;
		else if (ps_pdread(ph_p, (paddr_t) thr_struct.t_tls,
		    (char *) thr_tsd_p, sizeof (*thr_tsd_p)) != PS_OK)
			return_val = TD_DBERR;
		else if ((array_addr = (paddr_t) thr_tsd_p->array) != NULL) {
			/*
			...
			*tsd_array_ptr = (void *) malloc(tsd_size);
			...
			/* Get tsd_common to check number of keys used. */
			    ...
			    ta_p->tdb_invar.tsd_common_addr,
			    (char *) tsd_common_p,
	...
	int tab_size)
{
	...
	symbol_addr = ta_p->tdb_invar.allthreads_addr;
...
td_thr_sleepinfo(const td_thrhandle_t *th_p, td_synchandle_t *sh_p)
{
	...
	if (ps_pstop(th_p->th_ta_p->ph_p) == PS_OK) {
		...
		sh_p->sh_unique = (paddr_t) thr_struct.t_wchan;
		/*
		 * Continue process.
		 */
...

{
	...
	if (ps_pdread(sh_p->sh_ta_p->ph_p, (paddr_t) sh_p->sh_unique,
