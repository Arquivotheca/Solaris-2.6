    ...
    td_thragent_t **ta_pp)
{
	 ...
	 */
	if (ps_kill_p != NULL) {
		if (return_val == TD_OK && ps_pdwrite(ph_p,
		    ta_p->tdb_invar.tdb_agent_stat_addr,
		    (char *) &start_agent, sizeof (start_agent)) != PS_OK)
			is_core_file = 1;
...

{
	...
	ps_err_e (*lroll_p)(const struct ps_prochandle *,
	    lwpid_t, paddr_t, paddr_t);
	...
	if (ta_p->tdb_agent_data.agent_lwpid == 0) {
		...
		if (ps_pdread(ta_p->ph_p,
		    ta_p->tdb_invar.tdb_agent_data_addr,
		    (char *) &ta_p->tdb_agent_data,
		    sizeof (ta_p->tdb_agent_data)) != TD_OK)
			return (TD_DBERR);
		if (ta_p->tdb_agent_data.agent_lwpid == 0) {
			/*
			 * XXX Probably should have invented a more
			 * apposite error code; this really means
			 * "Agent not running yet, try again."
			 */
			return (TD_NOLWP);
		}
	}
	...
	if (ps_pdwrite(ta_p->ph_p,
		...
		return (TD_DBERR);
	if (ps_lrolltoaddr(ta_p->ph_p, ta_p->tdb_agent_data.agent_lwpid,
	    ta_p->tdb_agent_data.agent_go_addr,
	    ta_p->tdb_agent_data.agent_stop_addr) != PS_OK)
		return (TD_DBERR);
	if (ps_pdread(ta_p->ph_p,
	    ta_p->tdb_invar.tdb_agent_ctl_addr,
	    (char *) agent_msg, sizeof (*agent_msg)) != PS_OK)
