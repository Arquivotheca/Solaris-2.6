/*
*
*ident	"@(#)td_so.c	1.48	96/10/21 SMI"
*
...
*/
td_err_e
__td_ta_map_addr2sync(const td_thragent_t *ta_p, paddr_t addr,
...

{
	...
	wcb->errcode = __td_read_thr_struct(th_p->th_ta_p, th_p->th_unique,
	    &thr_struct);
...

{
	...
	case MUTEX_MAGIC:
		...
		memcpy(si_p->si_flags, generic_so.lock.flags.flag,
		    sizeof (generic_so.lock.flags.flag));
	...
	case SEMA_MAGIC:
		...
		si_p->si_data = (paddr_t) generic_so.semaphore.count;
		break;
	case RWL_MAGIC:
		...
		si_p->si_data = (paddr_t) generic_so.rwlock.readers;
		break;
	default:
		ret_val = TD_BADSH;
		break;
