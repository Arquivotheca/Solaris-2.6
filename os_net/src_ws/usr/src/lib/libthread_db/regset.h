...
#pragma	ident	"@(#)regset.h	1.5	94/11/15 SMI"
...
typedef struct RegGSet_t {
    unsigned long       regs[32];       /* general registers */
    unsigned long	lr;		/* link register */
    unsigned long	cr;		/* condition register */
    unsigned long	pc;		/* program counter */
    unsigned long	xer;		/* exception register */
...
typedef struct RegFSet_t {
    double		d[32];          /* double (precision) view */
    unsigned long       fsr;            /* FPU status register */
    Boolean             valid;
} RegFSet_t;
