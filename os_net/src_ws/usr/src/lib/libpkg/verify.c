...
/*LINTLIBRARY*/
#ident	"@(#)verify.c	1.23	96/01/11 SMI"	/* SVr4.0  1.21.1.1	*/
...
cverify(int fix, char *ftype, char *path, struct cinfo *cinfo)
{
	...
	if (setval || (cinfo->modtime == BADCONT))
		...
		if (fix > 0) {
			...
			reperr(gettext(ERR_MTIME), tbuf1, tbuf2);
			retcode = VE_CONT;
		}
#if 0
		else
	...
	if (setval || (cinfo->size == BADCONT))
		...
		if (!retcode /* || (retcode == VE_TIME)*/)
			retcode = VE_CONT;
		reperr(gettext(ERR_SIZE), cinfo->size, status.st_size);
	}
...
/* checksum disable swicth */
	...
	if (setval || (cinfo->cksum == BADCONT))
		...
		if (!retcode /* || (retcode == VE_TIME)*/)
			retcode = VE_CONT;
		if (!cksumerr)
			reperr(gettext(ERR_CKSUM), cinfo->cksum, mycksum);
	}
...
averify(int fix, char *ftype, char *path, struct ainfo *ainfo)
{
	...
	if (*ftype == 'l') {
		...
		if (getcwd(cwd, MAXPATHLEN) == NULL) {
			reperr(gettext(ERR_GETWD), ainfo->local);
			return (VE_FAIL);
		}
		...
		if (retcode || (status.st_nlink < 2) ||
			...
			if (fix) {
				...
				if (chdir(cwd) != 0)
					reperr(gettext(ERR_CHDIR), cwd);

				reperr(gettext(ERR_LINK), ainfo->local);
				return (VE_CONT);
			}
		}
	...
	if (setval)
		*ftype = myftype;
	    ...
	    ((myftype != 'd') || (*ftype != 'x'))) {
		reperr(gettext(ERR_FTYPE), *ftype, myftype);
		retcode = VE_FTYPE;
	}
	...
	if (!retcode && (*ftype == 's')) {
		...
		if (n < 0) {
			reperr(gettext(ERR_SLINK), ainfo->local);
			retcode = VE_CONT;
		} else {
			buf[n] = '\0';
	...
	if (retcode) {
		...
		if (fix) {
			...
			if (strchr("dx", *ftype)) {
				...
				if (stat(path, &status) < 0) {
					reperr(gettext(ERR_DIRFAIL));
					return (VE_FAIL);
				}
			} else if (*ftype == 's') {
				if (symlink(ainfo->local, path)) {
					reperr(gettext(ERR_SLINKFAIL),
					    ainfo->local);
					return (VE_FAIL);
				}
...
#endif
				    (stat(path, &status) < 0)) {
					reperr(gettext(ERR_BDEVFAIL));
					return (VE_FAIL);
				}
			} else if (*ftype == 'p') {
				if (mknod(path, ainfo->mode | S_IFIFO, NULL) ||
				    (stat(path, &status) < 0)) {
					reperr(gettext(ERR_PIPEFAIL));
					return (VE_FAIL);
				}
			} else
				return (retcode);
	...
	if (strchr("cb", myftype)) {
		...
		if (status.st_rdev != makedev(ainfo->xmajor, ainfo->xminor)) {
			reperr(gettext(ERR_MAJMIN), ainfo->xmajor,
			    ainfo->xminor,
		...
		if (status.st_rdev != makedev(ainfo->major, ainfo->minor)) {
			reperr(gettext(ERR_MAJMIN), ainfo->major, ainfo->minor,
	...
	if (setval || (ainfo->mode == BADMODE))
		...
		if (fix) {
			...
			reperr(gettext(ERR_PERM), ainfo->mode,
	...
	if (setval || strcmp(ainfo->group, BADGROUP) == 0) {
		...
		if (grp)
			...
			if (!retcode)
				retcode = VE_ATTR;
			reperr(gettext(ERR_BADGRPID), status.st_gid);
		}
		gid = status.st_gid;
	} else if ((grp = cgrnam(ainfo->group)) == NULL) {
		reperr(gettext(ERR_BADGRPNM), ainfo->group);
		if (!retcode)
			retcode = VE_ATTR;
		...
		if (fix) {
			...
			if ((grp = cgrgid((int)status.st_gid)) ==
			    (struct group *)NULL) {
				reperr(gettext(ERR_GROUP), ainfo->group,
				    "(null)");
			} else {
				reperr(gettext(ERR_GROUP), ainfo->group,
				    grp->gr_name);
	...
	if (setval || strcmp(ainfo->owner, BADOWNER) == 0) {
		...
		if (pwd)
			...
			if (!retcode)
				retcode = VE_ATTR;
			reperr(gettext(ERR_BADUSRID), status.st_uid);
		}
		uid = status.st_uid;
	} else if ((pwd = cpwnam(ainfo->owner)) == NULL) {
		/* UID does not exist in password file */
		reperr(gettext(ERR_BADUSRNM), ainfo->owner);
		if (!retcode)
			retcode = VE_ATTR;
		...
		if (fix) {
			...
			pwd = cpwuid((int)status.st_uid);
			if (pwd == NULL)
				reperr(gettext(ERR_BADUSRID),
				    (int)status.st_uid);
			else
				reperr(gettext(ERR_OWNER), ainfo->owner,
				    pwd->pw_name);
	...
	if ((retval = averify(fix, ftype, path, ainfo)) == 0) {
		if (*ftype == 'f' || *ftype == 'i') {
			if (cinfo->size != status.st_size) {
				logerr(gettext(WRN_QV_SIZE), path);
				retval = VE_CONT;
			}
			if (cinfo->modtime != status.st_mtime) {
				logerr(gettext(WRN_QV_MTIME), path);
				retval = 0;
			}
		}
	}
