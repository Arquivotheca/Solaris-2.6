...
/*LINTLIBRARY*/
#ident	"@(#)pkgmount.c	1.9	96/04/05 SMI"	/* SVr4.0  1.10.3.1	*/
...
pkgmount(struct pkgdev *devp, char *pkg, int part, int nparts, int getvolflg)
{
	...
	if (getuid()) {
		progerr(gettext(ERR_NOTROOT));
		return (99);
	}

	if (part && nparts) {
		if (pkg) {
			(void) sprintf(prompt, gettext(LABEL0), part,
			    nparts, pkg);
		} else {
			(void) sprintf(prompt, gettext(LABEL1), part,
			    nparts);
		}
	} else if (pkg)
		(void) sprintf(prompt, gettext(LABEL2), pkg);
	else
		(void) sprintf(prompt, gettext(LABEL3));
	...
	for (;;) {
		...
		if (getvolflg && (n = getvol(devp->bdevice, NULL,
			...
			if (n == 2)
				...
				progerr(
				    gettext("unable to obtain package volume"));
			return (99);
		}
		...
		if (devp->fstyp == NULL) {
			...
			if ((pp = epopen(cmd, "r")) == NULL) {
				rpterr();
				logerr(gettext(ERR_FSTYP), devp->bdevice);
				n = -1;
				continue;
			}
			cmd[0] = '\0';
		...
		if (n) {
			progerr(gettext("mount of %s failed"), devp->bdevice);
			continue;
		}
		devp->mntflg++;
		break;
...
pkgumount(struct pkgdev *devp)
{
	...
	while (n != 0 && retry-- > 0) {
		n = pkgexecl(NULL, NULL, NULL, NULL, UMOUNT, devp->bdevice,
		    NULL);
		if (n != 0) {
			progerr(gettext("retrying umount of %s"),
			    devp->bdevice);
			sleep(5);
		}
	}
