...
/*LINTLIBRARY*/
#pragma ident	"@(#)gpkgmap.c	1.14	94/11/22 SMI"	/* SVr4.0  1.5.1.1 */
	...
	if (strchr("cbdxpfve", ept->ftype)) {
		...
		if (mapmode != MAPNONE) {
			if (mapvar(mapmode, ept->ainfo.owner)) {
				sprintf(errbuf, gettext(ERR_NOVAR),
				...
				*d = BADMODE;	/* may be a problem */
				...
				while (tempmode[n] && !isspace(tempmode[n])) {
					if (!isdigit(tempmode[n])) {
						errstr = gettext(ERR_MODEALPHA);
						return (2);
					}

					if (strchr("89abcdefABCDEF",
					    tempmode[n])) {
						errstr = gettext(ERR_BASEINVAL);
						return (2);
					}
					n++;
				}
