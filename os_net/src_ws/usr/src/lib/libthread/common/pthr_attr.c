...
#pragma	" @(#)pthr_attr.c 1.8 95/08/24 "
...
{
	if (attr == NULL || attr->pthread_attrp == NULL ||
			_free_attr(attr->pthread_attrp) < 0)
		return (EINVAL);
	attr->pthread_attrp = NULL;
	return (0);
}
...
_pthread_attr_setstacksize(pthread_attr_t *attr, size_t stacksize)
{
	...
	if (attr != NULL && (ap = attr->pthread_attrp) != NULL &&
