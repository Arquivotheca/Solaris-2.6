...
#pragma ident	"@(#)util.c	1.29	96/07/22 SMI"
...
find_trace_file_info(tnfctl_handle_t *hndl)
{
	...
	int		miscstat;
	char		*preexisting;
	paddr_t		name_addr, size_addr, min_addr;
	size_t		outsize, minoutsize;
 ...
 */
int
_tnfctl_read_targ(void *proc_p, paddr_t addr, void *buf, size_t size)
{
	return (prb_proc_read(proc_p, addr, buf, size));
}

int
_tnfctl_write_targ(void *proc_p, paddr_t addr, void *buf, size_t size)
{
...
tnfctl_errcode_t
_tnfctl_readstr_targ(tnfctl_handle_t *hndl, paddr_t addr, char **outstr_pp)
{
	int		retstat;
	int		bufsz = BUFSZ;
	char		buffer[BUFSZ + 1];
