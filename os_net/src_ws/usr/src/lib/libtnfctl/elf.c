...
#pragma ident	"@(#)elf.c	1.6	96/07/22 SMI"
...
static tnfctl_errcode_t dynsec_num(tnfctl_handle_t *hndl, paddr_t baseaddr,
 ...
 */
tnfctl_errcode_t
_tnfctl_elf_dbgent(tnfctl_handle_t *hndl, paddr_t * entaddr_p)
{
...
dyn_findtag(Elf32_Dyn * start,	/* start of dynam table read in */
		Elf32_Sword tag,	/* tag to search for */
		paddr_t dynam_addr,	/* base address of _DYNAMIC in target */
		int limit, /* number of entries in table */
		paddr_t * dentry_address)
{				/* return value */
	Elf32_Dyn	  *dp;
...
/*ARGSUSED*/
static tnfctl_errcode_t
dynsec_num(tnfctl_handle_t *hndl, paddr_t baseaddr,
	int objfd, int *num_dyn)
{
	int		num_ent = 0;
