...
#pragma ident	"@(#)kernel_int.c	1.17	96/07/22 SMI"
...
/* Dummy "end" function */
#define	PRBK_DUMMY_END ((paddr_t) 16)

/* Dummy "alloc" function */
#define	PRBK_DUMMY_ALLOC ((paddr_t) 20)
 ...
 */
void
_tnfctl_prbk_get_other_funcs(paddr_t *allocp, paddr_t *commitp,
	paddr_t *rollbackp, paddr_t *endp)
{
	*allocp = PRBK_DUMMY_ALLOC;
	*commitp = (paddr_t) PRBK_DUMMY_COMMIT;
	*rollbackp = (paddr_t) PRBK_DUMMY_ROLLBACK;
	*endp = PRBK_DUMMY_END;
}
 ...
 */
void
_tnfctl_prbk_test_func(paddr_t *outp)
{
	*outp = (paddr_t) PRBK_DUMMY_TEST;
}
