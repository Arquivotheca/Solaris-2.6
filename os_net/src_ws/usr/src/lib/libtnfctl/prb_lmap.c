...
#pragma ident	"@(#)prb_lmap.c	1.12	96/05/15 SMI"
...
prb_loadobj_iter(prb_proc_ctl_t *proc_p, prb_loadobj_f *obj_func, void *cd)
{
	...
	for (lmapaddr = (paddr_t) r_dbg.r_map; lmapaddr;
		lmapaddr = (paddr_t) lmap.l_next) {
		...
		(void) prb_proc_readstr(proc_p, (paddr_t) lmap.l_name,
						&loadobj.objname);
 ...
 */
prb_status_t
prb_mainobj_get(prb_proc_ctl_t *proc_p, int *objfd, paddr_t *baseaddr)
{
	int		procfd;
	int		retfd;
