...
#pragma ident	"@(#)tnfctl.h	1.30	96/08/27 SMI"
...
typedef struct tnfctl_probe_state {
	...
	const paddr_t 	*func_addrs;		/* array of func addresses */
...
typedef struct tnfctl_ind_obj_info {
	...
	paddr_t	text_base;	/* address where text of loadobj was mapped */
	paddr_t	data_base;	/* address where data of loadobj was mapped */
...
typedef struct tnfctl_ind_config {
	int (*p_read)(void *, paddr_t, void *, size_t);
	int (*p_write)(void *, paddr_t, void *, size_t);
	pid_t (*p_getpid)(void *);
