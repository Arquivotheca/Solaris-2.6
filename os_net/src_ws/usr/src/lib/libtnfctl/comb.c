...
#pragma ident  "@(#)comb.c 1.5 96/07/22 SMI"
...
{
		(paddr_t) prb_chain_entry,
		(paddr_t) prb_chain_down,
		(paddr_t) prb_chain_next,
		(paddr_t) prb_chain_end}
};
...
static tnfctl_errcode_t decode(tnfctl_handle_t *hndl, paddr_t addr,
	char ***func_names, paddr_t **func_addrs);
static boolean_t find(tnfctl_handle_t *hndl, comb_op_t op, paddr_t down,
	paddr_t next, paddr_t * comb_p);
static tnfctl_errcode_t build(tnfctl_handle_t *hndl, comb_op_t op,
	paddr_t down, paddr_t next, paddr_t * comb_p);
static tnfctl_errcode_t add(tnfctl_handle_t *hndl, comb_op_t op, paddr_t down,
	paddr_t next, paddr_t comb);
static int comb_compare(const void *a, const void *b);
static int decode_compare(const void *v0p, const void *v1p);
static tnfctl_errcode_t iscomb(tnfctl_handle_t *hndl, paddr_t addr,
	paddr_t *down_p, paddr_t *next_p, boolean_t *ret_val);
static tnfctl_errcode_t findname(tnfctl_handle_t *hndl, paddr_t addr,
	char **ret_name);
 ...
 */
tnfctl_errcode_t
_tnfctl_comb_decode(tnfctl_handle_t *hndl, paddr_t addr, char ***func_names,
	paddr_t **func_addrs)
{
	tnfctl_errcode_t prexstat;
...
decode(tnfctl_handle_t *hndl, paddr_t addr, char ***func_names,
	paddr_t **func_addrs)
{
 ...
 */
static tnfctl_errcode_t
iscomb(tnfctl_handle_t *hndl, paddr_t addr, paddr_t *down_p, paddr_t *next_p,
	boolean_t *ret_val)
{
	int		type;
	...
	for (type = 0; type < PRB_COMB_COUNT; type++) {
		...
		paddr_t		nextaddr;
		int		num_bits = 0;
		long		tmp_bits = prb_callinfo.mask;
		...
		/* LINTED pointer cast may result in improper alignment */
			 ...
			 */
			if ((paddr_t) tptr == calltmpl[type].down +
				prb_callinfo.offset) {
				downbits = *uptr;
				...
				ptr += 3;
				tptr += 3;
			} else if ((paddr_t) tptr == calltmpl[type].next +
				prb_callinfo.offset) {
				nextbits = *uptr;
 ...
 */
static tnfctl_errcode_t
findname(tnfctl_handle_t *hndl, paddr_t addr, char **ret_name)
{
	char		*symname;
	tnfctl_errcode_t prexstat;
	...
	if ((prexstat == TNFCTL_ERR_NONE) && (symname != NULL)) {
		...
		/* no name found, use the address - could use %p format ? */
 ...
 */
static boolean_t
find(tnfctl_handle_t *hndl, comb_op_t op, paddr_t down, paddr_t next,
	paddr_t * comb_p)
{
	comb_key_t	key;
	comb_key_t	**find_pp;
 ...
 */
static tnfctl_errcode_t
add(tnfctl_handle_t *hndl, comb_op_t op, paddr_t down, paddr_t next,
	paddr_t comb)
{
	comb_key_t	 *new_p;
...
decode_compare(const void *v0p, const void *v1p)
{
	...
	return ((unsigned) k1p->addr - (unsigned) k0p->addr);
}				/* end decode_compare */
 ...
 */
static tnfctl_errcode_t
build(tnfctl_handle_t *hndl, comb_op_t op, paddr_t down, paddr_t next,
	paddr_t *comb_p)
{
	size_t		size;
	paddr_t		addr;
	...
	*comb_p = NULL;
	...
	/*LINTED pointer cast may result in improper alignment*/
	word_p = (paddr_t *) (buffer_p + offset + prb_callinfo.offset);
