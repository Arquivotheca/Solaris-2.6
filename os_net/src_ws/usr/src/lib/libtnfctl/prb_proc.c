...
#pragma ident	"@(#)prb_proc.c	1.14	96/08/20 SMI"
 ...
 * process.  Returns a pointer to an opaque structure that contains the
 * fd needed for /proc control.
 */
 ...
 */
prb_status_t
prb_proc_read(prb_proc_ctl_t *proc_p, paddr_t addr, void *buf, size_t size)
{
	int		procfd;
	ssize_t		sz;
	off_t		offset;
 ...
 */
prb_status_t
prb_proc_write(prb_proc_ctl_t *proc_p, paddr_t addr, void *buf, size_t size)
{
	int		procfd;
	ssize_t		sz;
	off_t		offset;
...
prb_status_t
prb_proc_readstr(prb_proc_ctl_t *proc_p, paddr_t addr, const char **outstr_pp)
{
...
again:
	 ...
	 *  unfortunately, ppc uses R_R3 and R_R4 for return values
	 ...
	 */
#if defined(__ppc)
	*r0 = prstatus.pr_reg[R_R3];
	*r1 = prstatus.pr_reg[R_R4];
#else
