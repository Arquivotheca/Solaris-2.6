...
struct prbctlref {
	paddr_t			addr;		/* probe address in target */
...
struct objlist {
	...
	paddr_t		baseaddr;
	int		objfd;
	ulong_t		min_probe_num;	/* first probe id in object */
	ulong_t		probecnt;	/* number of probes in object */
...
struct tnfctl_handle {
	...
	paddr_t		commitfunc;
	paddr_t		endfunc;
	paddr_t		rollbackfunc;
	paddr_t		probelist_head;
	paddr_t		probelist_valid;
	paddr_t		trace_error;
	paddr_t		memseg_p;
	paddr_t		nonthread_test;
	paddr_t		thread_test;
	paddr_t		thread_sync;
	boolean_t	mt_target;
	ulong_t		num_probes;	/* number of probes in target */
	...
	int (*p_read)(void *prochandle, paddr_t addr, void *buf, size_t size);
	int (*p_write)(void *prochandle, paddr_t addr, void *buf, size_t size);
...
(*tnfctl_traverse_section_func_t) (Elf * elf, char *strs, Elf_Scn * scn,
	Elf32_Shdr * shdr, Elf_Data * data, paddr_t baseaddr,
	tnfctl_elf_search_t * search_info);

/* prototype for callback for traversing records in an elf section */
typedef tnfctl_errcode_t
(*tnfctl_record_func_t) (char *name, paddr_t addr, void *entry,
	tnfctl_elf_search_t * search_info);
...
tnfctl_errcode_t _tnfctl_traverse_object(int objfd, paddr_t addr,
...
tnfctl_errcode_t _tnfctl_traverse_dynsym(Elf * elf, char *elfstrs,
	Elf_Scn * scn, Elf32_Shdr * shdr, Elf_Data * data, paddr_t baseaddr,
	tnfctl_elf_search_t * search_info_p);
...
tnfctl_errcode_t _tnfctl_create_tracefile(tnfctl_handle_t *hndl,
		const char *trace_file_name, size_t trace_file_size);
...
tnfctl_errcode_t _tnfctl_comb_build(tnfctl_handle_t *hndl, comb_op_t op,
	paddr_t down, paddr_t next, paddr_t *comb_p);
tnfctl_errcode_t _tnfctl_comb_decode(tnfctl_handle_t *hndl, paddr_t addr,
	char ***func_names, paddr_t **func_addrs);
...
int _tnfctl_read_targ(void *proc_p, paddr_t addr, void *buf, size_t size);
int _tnfctl_write_targ(void *proc_p, paddr_t addr, void *buf, size_t size);
...
tnfctl_errcode_t _tnfctl_readstr_targ(tnfctl_handle_t *hndl, paddr_t addr,
				char **outstr_pp);

/* symbol searching interfaces */
tnfctl_errcode_t _tnfctl_sym_find_in_obj(int objfd, paddr_t baseaddr,
		const char *symname, paddr_t *symaddr);
tnfctl_errcode_t _tnfctl_sym_obj_find(tnfctl_handle_t *hndl,
	const char *lib_base_name, const char *symname, paddr_t *symaddr);
tnfctl_errcode_t _tnfctl_sym_find(tnfctl_handle_t *hndl, const char *symname,
			paddr_t *symaddr);
tnfctl_errcode_t _tnfctl_sym_findname(tnfctl_handle_t *hndl, paddr_t symaddr,
	char **symname);
tnfctl_errcode_t _tnfctl_elf_dbgent(tnfctl_handle_t *hndl, paddr_t * entaddr_p);
