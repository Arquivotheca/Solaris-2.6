...
static int inprocess_read(void *ignore, paddr_t addr, void *buf, size_t size);
static int inprocess_write(void *ignore, paddr_t addr, void *buf, size_t size);
static pid_t inprocess_getpid(void *ignore);
static tnfctl_errcode_t inprocess_get_dtdebug(void *hndl, paddr_t *ret_val);
 ...
 */
static int
inprocess_read(void *ignore, paddr_t addr, void *buf, size_t size)
{
 ...
 */
static int
inprocess_write(void *ignore, paddr_t addr, void *buf, size_t size)
{
...
inprocess_get_dtdebug(void *hndl, paddr_t *ret_val)
{
	Elf32_Dyn 	*dyn = &_DYNAMIC;
	Elf32_Dyn	*dp;
