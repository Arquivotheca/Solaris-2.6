...
#ident	"@(#)devfsinfo.c	1.6	96/07/16 SMI"
...
#define	OBP_OF			0x4	/* versions OBP 3.x and PPC */
 ...
 * For ppc and x86: we append fdisk partition 0 (:q).  Note that
 * for ppc, this call could go out and find the first active
 * fdisk partition which is what the PPC 1275 Binding specifies
 * if the fdisk partition number is not explicit.  For now, because
 * consumers of this call do not want/need this functionality,
 * we append fdisk partition #0 (which represents the entire disk device.
 */
static int
