...
#pragma	" @(#)pthread.c 1.10 95/12/05 SMI"
#include <pthread.h>
...
#pragma init(initialize_libc_interface)

#endif /* __STDC__ */

static void
initialize_libc_interface()
{
	/*
	 * The following call to thr_main() will cause the opening
	 * of libthread, which in turn will initialize the libthread/libc
	 * interface.  By default a filter library (libpthread) will not
	 * load it's filtee until a function is called.
	 */
	(void) _thr_main();
}
