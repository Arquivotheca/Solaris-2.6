...
#pragma ident	"@(#)local_cache.cc	1.14	96/05/22 SMI"
...
NisLocalCache::NisLocalCache(nis_error &status, u_long *exp_time)
{
	rwlock_init(&lock, USYNC_THREAD, NULL);
	head = NULL;
	tail = NULL;
...
NisLocalCache::addBinding(nis_bound_directory *binding)
{
	...
	if (nis_dir_cmp(new_entry->name, nis_local_directory()) == SAME_NAME)
		is_coldstart = 1;
