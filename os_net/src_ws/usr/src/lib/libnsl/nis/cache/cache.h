...
#pragma ident	"@(#)cache.h	1.3	96/10/15 SMI"
...
class NisCache {
	...
	virtual bool_t readServerColdStart(u_long *exp_time);
	virtual void print();
	...
	virtual u_long loadPreferredServers();
	virtual u_long refreshCache();
	...
	nis_error pingServers(nis_server *srv, int nsrv);
	u_long expireTime(u_long ttl);
	int nextGeneration();
	void rerankServers();
