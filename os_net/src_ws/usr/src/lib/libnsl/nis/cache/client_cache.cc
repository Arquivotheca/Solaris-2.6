...
#ident	"@(#)client_cache.cc	1.21	96/07/15 SMI"
...
NisClientCache::clientHandle()
{
	...
	clnt_control(mgr_clnt, CLSET_FD_CLOSE, (char *)NULL);
	tv.tv_sec = 60 * 60;	/* 1 hour (approximately infinite) */
	tv.tv_usec = 0;
	clnt_control(mgr_clnt, CLSET_TIMEOUT, (char *)&tv);
	if (clnt_control(mgr_clnt, CLGET_FD, (char *)&curFd))
		fcntl(curFd, F_SETFD, 1);	/* close on exec */
...
NisClientCache::checkClntState()
{
	...
	if (curRdev != -1) {
		if (_FSTAT(curFd, &stbuf) == -1) {
			...
			clnt_control(mgr_clnt, CLSET_FD_NCLOSE, (char *)NULL);
			return (0);
		}
