...
#pragma ident	"@(#)nislib.c	1.41	96/10/25 SMI"
...
u_long  __nis_force_hard_lookups = HARD_LOOKUP;
...
static nis_result *__nis_path_list(nis_object *, int, nis_result *,
				...
				void *);
static nis_result * nis_ibops(ib_request *, u_long);
static nis_result * nis_nameops(nis_name, nis_object *, u_long);
 ...
 */
nis_error
nis_bind_dir(dname, parent_first, binding, flags)
	char *dname;
	int parent_first;
	...
	void		*cbdata;	/* Callback private data */
{
	...
	__nis_CacheStart();
	if (__nis_debug_calls) {
		fprintf(__nis_debug_file, "nis_list(%s, 0x%x, 0x%x, 0x%x)\n",
			name, flags, cback, cbdata);
	}
 ...
 */
nis_error
nis_call(nis_call_state *state, u_long func,
	...
	nis_object	*obj;
{
	...
	__nis_CacheStart();
	if (__nis_debug_calls) {
		fprintf(__nis_debug_file, "nis_add(%s, 0x%x\n",
			name?name:"(nil)", obj);
	}
	...
	nis_object	*obj;
{
	...
	__nis_CacheStart();
	if (__nis_debug_calls) {
		fprintf(__nis_debug_file, "nis_remove(%s, 0x%x)\n",
			name?name:"(nil)", obj);
	}
	...
	nis_object	*obj;
{
	...
	__nis_CacheStart();
	if (__nis_debug_calls) {
		fprintf(__nis_debug_file, "nis_modify(%s, 0x%x)\n",
			name?name:"(nil)", obj);
	}
...
cookie_to_name(netobj *cookie)
{
	...
	memmove(cookie->n_bytes, cookie->n_bytes + offset, len);
	cookie->n_len = len;
...
name_to_cookie(char *name, nis_result *res)
{
	...
	strcpy(p, name);
	memmove(p+offset, res->cookie.n_bytes, res->cookie.n_len);
	free(res->cookie.n_bytes);
	res->cookie.n_bytes = p;
	res->cookie.n_len = len;
}
	...
	u_long		flags;		/* Semantic modification flags	*/
{
	...
	__nis_CacheStart();
	if (__nis_debug_calls) {
		fprintf(__nis_debug_file, "nis_add_entry(%s, 0x%x, 0x%x\n",
			name?name:"(nil)", obj, flags);
	}
	...
	u_long		flags;		/* semantic modification flags	*/
{
	...
	__nis_CacheStart();
	if (__nis_debug_calls) {
		fprintf(__nis_debug_file, "nis_remove_entry(%s, 0x%x, 0x%x)\n",
			name?name:"(nil)", obj, flags);
	}
	stat = nis_get_request(name, obj, NULL, &req);
	if (stat != NIS_SUCCESS) {
	...
	u_long		flags;		/* Semantic modification flags	*/
{
	...
	__nis_CacheStart();
	if (__nis_debug_calls) {
		fprintf(__nis_debug_file, "nis_modify_entry(%s, 0x%x, 0x%x)\n",
			name?name:"(nil)", obj, flags);
	}
	stat = nis_get_request(name, obj, NULL, &req);
	if (stat != NIS_SUCCESS) {
	...
	netobj		*cookie;	/* First/Next Cookie	*/
{
	...
	__nis_CacheStart();
	if (__nis_debug_calls) {
		fprintf(__nis_debug_file, "nis_next_entry(%s, x0%x)\n",
			table, cookie);
	}
	...
	nis_server	*srv;
{
	...
	__nis_CacheStart();
	if (__nis_debug_calls) {
		fprintf(__nis_debug_file, "nis_mkdir(%s, %s)\n",
			name, srv->name);
	}
	__nis_init_call_state(&state);
	state.srv = srv;
	...
	if (call_err != NIS_SUCCESS)
		err = call_err;
	if (__nis_debug_calls) {
		fprintf(__nis_debug_file, "status=%s\n", nis_sperrno(err));
	}
	return (err);
}
	...
	nis_server	*srv;
{
	...
	__nis_CacheStart();
	if (__nis_debug_calls) {
		fprintf(__nis_debug_file, "nis_rmdir(%s, %s)\n",
			name, srv->name);
	}
	__nis_init_call_state(&state);
	state.srv = srv;
...
__nis_send_msg(nis_server *srv, int proc, xdrproc_t out, char *msg)
{
	...
	__nis_CacheStart();
	if (__nis_debug_calls) {
		fprintf(__nis_debug_file, "nis_send_msg(%s, %d)\n",
			srv->name, proc);
	}
	__nis_init_call_state(&state);
	state.srv = srv;
