...
#pragma ident	"@(#)nis_rpc.c	1.43	96/10/21 SMI"
...
CLIENT * nis_make_rpchandle_uaddr(nis_server *, int, u_long, u_long, u_long, int
, int, char *);
...
static
CLIENT *
nis_handle(nis_bound_directory *binding, int n, u_long flags, int create)
{
	int fd;
	...
	for (scan = srv_tobefree; scan; scan = next_srv) {
		...
		if (scan->ref_cnt == 0) {
			...
			free(scan->mach_name);
			memset((char *)scan, 0, sizeof (server));
	...
	if (cur) {
		if (__nis_debug_bind) {
			fprintf(__nis_debug_file, "using %s at %s (%s)\n",
				cur->mach_name, cur->rpcb_uaddr, cur->netid);
		}
		if (__nis_debuglevel) {
			syslog(LOG_INFO,
			"NIS+: __bind_rpc: reusing handle to %s (%s, %s(%d))",
...
CLIENT *
nis_client_handle(nis_bound_directory *binding, int n, u_long flags)
{
	return (nis_handle(binding, n, flags, 1));
}

CLIENT *
nis_cached_handle(nis_bound_directory *binding, int n, u_long flags)
{
...
bind_message(nis_bound_directory *binding, int n, char *msg)
{
	...
	fprintf(__nis_debug_file, "%s:  server %s at %s (%s)\n",
...
{
	memset((char *)state, 0, sizeof (nis_call_state));
	state->state = S_INIT;
...
__nis_get_server(nis_call_state *state)
{
	...
	for (;;) {
		...
		"__nis_get_server:  can't find server %s in directory %s",
		    ...
		    case S_BINDFAIL:
			if (__nis_debug_bind) {
				fprintf(__nis_debug_file, "binding failure\n");
			}
			state->aticks += __stop_clock(CLOCK_CACHE);
			return (NULL);
			break;
