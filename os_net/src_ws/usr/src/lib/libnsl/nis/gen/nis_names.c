...
#pragma ident	"@(#)	nis_names.c	1.17	96/07/09	SMI"
#include <stdlib.h>
#include <string.h>
...
dots_in_name(name)
	register char	*name;
{
	register int i, ndots, len = strlen(name);
 ...
 */
static int
parse_default(name, local, result, rmax)
	...
	int		rmax;		/* max array value	*/
{
	...
	register nis_name dom, tmpdom;	/* temp domain names	*/
	register nis_name tmpnam;	/* temp domain names	*/
	nis_name mkr = NULL;		/* name in domain flag	*/
	register int comps = 0;		/* # of new components	*/
	...
	if (!(mkr = strstr(dom, name))) {
		...
		while (tmpnam < (name + strlen(name))) {
			...
			/* save first label */
			nis_leaf_of_r(tmpnam, tmpbuf, NIS_MAXSRCHLEN);
			strcat(buf, tmpbuf);
			strcat(buf, ".");
	...
	int		rmax;
{
	...
	for (i = 0, cur = 0; (i < comps) && (cur < rmax); i++) {
		...
		/* otherwise it's something like org_dir.$ or groups_dir.$ */
			...
			if (*(list[i] + (len - 1)) == '$') {
				*(list[i] + (len - 1)) = '\0';
				sprintf(buf, "%s.%s%s", name, list[i], local);
			} else
...
*/
static void
check_dup_dnames(namelist, count)
	nis_name *namelist;
	int	count;
{
	register char	*p1, *p2;
