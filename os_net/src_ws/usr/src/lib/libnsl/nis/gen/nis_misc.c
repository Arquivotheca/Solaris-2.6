...
#pragma ident	"@(#)nis_misc.c	1.23	96/07/17 SMI"
...
__nis_server_dup(nis_server *src, nis_server *dst)
{
	...
	memset((char *)dst, 0, sizeof (nis_server));
	return ((nis_server *)
...
__nis_print_call(CLIENT *clnt, int proc)
{
	...
	fprintf(__nis_debug_file, "calling server %s for %s\n", name, pname);
}

void
__nis_print_rpc_result(enum clnt_stat status)
{
	fprintf(__nis_debug_file, "result:  %s\n", clnt_sperrno(status));
}
...
__nis_print_req(ib_request *req)
{
	...
	for (i = 0; i < nattr; i++) {
		if (i != 0)
			fprintf(__nis_debug_file, ",");
		fprintf(__nis_debug_file, "%s=%s",
			attr[i].zattr_ndx,
			attr[i].zattr_val.zattr_val_val);
	}
	fprintf(__nis_debug_file, "],%s\n", req->ibr_name);
}
...
{
	fprintf(__nis_debug_file, "%s (from %s)\n",
