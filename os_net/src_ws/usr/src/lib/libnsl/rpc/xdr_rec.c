...
#pragma ident	"@(#)xdr_rec.c	1.22	95/01/27 SMI"

#if !defined(lint) && defined(SCCSIDS)
static char sccsid[] = "@(#)xdr_rec.c	1.22	95/01/27 SMI";
#endif
	...
	int len;
{
	...
	case XDR_DECODE:
		if ((len <= rstrm->fbtbc) &&
			 ...
			 * if not, align it.
			 */
			if (((int)rstrm->in_finger) & (sizeof (long) - 1))
				align_instream(rstrm);
...
static bool_t
fill_input_buf(rstrm, do_align)
	register RECSTREAM *rstrm;
	register bool_t do_align;
{
...
/* knows nothing about records!  Only about input buffers */
static bool_t
get_input_bytes(rstrm, addr, len, do_align)
	register RECSTREAM *rstrm;
	register caddr_t addr;
	register int len;
	register bool_t do_align;
