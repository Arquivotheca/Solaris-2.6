...
#ident	"@(#)clnt_door.c	1.3	96/06/21 SMI"
...
/* LINTED pointer alignment */
	(*(u_long *)cu->cu_header)++;	/* increment XID */
	...
	if (params.rbuf == NULL || params.rsize == 0) {
		return (cu->cu_error.re_status = RPC_FAILED);
	}
	need_to_unmap = (params.rbuf != outbuf_ref);

/* LINTED pointer alignment */
	if (*(u_long *)params.rbuf != *(u_long *)cu->cu_header) {
...
/* LINTED pointer alignment */
	...
	case CLGET_XID:
		 ...
		 */
/* LINTED pointer alignment */
		*(u_long *)info = ntohl(*(u_long *)cu->cu_header);
		break;

	case CLSET_XID:
		/* This will set the xid of the NEXT call */
/* LINTED pointer alignment */
		*(u_long *)cu->cu_header =  htonl(*(u_long *)info - 1);
...
/* LINTED pointer alignment */
	...
	case CLSET_VERS:
/* LINTED pointer alignment */
		*(u_long *)(cu->cu_header + 4 * BYTES_PER_XDR_UNIT) =
/* LINTED pointer alignment */
			htonl(*(u_long *)info);
		break;
...
/* LINTED pointer alignment */
	...
	case CLSET_PROG:
/* LINTED pointer alignment */
		*(u_long *)(cu->cu_header + 3 * BYTES_PER_XDR_UNIT) =
/* LINTED pointer alignment */
			htonl(*(u_long *)info);
		break;
