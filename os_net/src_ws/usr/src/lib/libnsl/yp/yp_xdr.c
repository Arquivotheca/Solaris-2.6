	...
	datum * pdatum;
{
	...
	dummy = xdr_bytes(xdrs, (char **)&(pdatum->dptr),
			(u_int *)&(pdatum->dsize), YPMAXRECORD);
	...
	struct ypresp_val *ps;
{
	...
	dummy = xdr_u_long(xdrs, &ps->status) &&
	...
	struct yppushresp_xfr *ps;
{
	...
	dummy = xdr_u_long(xdrs, &ps->transid) &&
