#ident	"@(#)yp_all.c	1.11	95/09/26 SMI"
	...
	struct ypall_callback *callback;
{
	...
	strcpy(server_name, pdomb->dom_binding->ypbind_servername);
	mutex_unlock(&pdomb->server_name_lock);
...
- transport level create failure"));
	...
	if (s != RPC_SUCCESS) {
		(void) syslog(LOG_ERR, clnt_sperror(allc,
	...
	struct ypall_callback *callback;
{
	...
	__yp_rel_binding(pdomb);
	if ((allc = __yp_clnt_create_rsvdport(server_name,
					 YPPROG, YPVERS,
					 "tcp", 0, 0)) == (CLIENT *) NULL) {
		    (void) syslog(LOG_ERR,
				  clnt_spcreateerror("yp_all - transport "
