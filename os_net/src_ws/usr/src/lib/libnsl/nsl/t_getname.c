...
#ident	"@(#)t_getname.c	1.18	96/10/14 SMI"
		/* SVr4.0 1.1.1.1	*/
...
int
_tx_getname(fd, name, type, api_semantics)
int fd;
struct netbuf *name;
int type;
int api_semantics;
...
static int
__tx_getname_locked(fd, name, type)
int fd;
struct netbuf *name;
int type;
{
...
int
_tx_getprotaddr(fd, boundaddr, peeraddr, api_semantics)
int fd;
...
int api_semantics;
{
	...
	do {
		retval = _t_do_ioctl(fd, (caddr_t) ctlbuf.buf,
			sizeof (struct T_addr_req), TI_GETADDRS, &retlen);
	} while (retval < 0 && errno == EINTR);
	...
	if (retlen < sizeof (struct T_addr_ack)) {
		t_errno = TSYSERR;
		trace2(TR_t_getprotaddr, 1, fd);
		errno = EIO;
		return (-1);
	...
	if (boundaddr != NULL && boundaddr->addr.maxlen > 0) {
		...
		memcpy(boundaddr->addr.buf,
			(char *) (ctlbuf.buf + addrackp->LOCADDR_offset),
			(int) addrackp->LOCADDR_length);
	}
	...
	if (tiptr->ti_state != T_OUTCON &&
		...
		memcpy(peeraddr->addr.buf,
			(char *) (ctlbuf.buf + addrackp->REMADDR_offset),
			(int) addrackp->REMADDR_length);
	}
...
static int
__tx_tlitpi_getprotaddr_locked(tiptr, boundaddr, peeraddr)
