...
#ident	"@(#)t_unbind.c	1.19	96/10/14 SMI"	/* SVr4.0 1.3.4.1	*/
...
int
_tx_unbind_locked(fd, tiptr, ctlbufp)
int fd;
...
struct strbuf *ctlbufp;
{
	...
	if (_t_do_ioctl(fd, (char *) unbind_reqp, sizeof (struct T_unbind_req),
			TI_UNBIND, &retlen) < 0) {
		goto err_out;
	}
