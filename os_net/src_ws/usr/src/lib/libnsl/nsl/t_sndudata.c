...
#ident	"@(#)t_sndudata.c	1.19	96/10/14 SMI"	/* SVr4.0 1.5	*/
...
int
_tx_sndudata(fd, unitdata, api_semantics)
int fd;
...
int api_semantics;
{
	...
	if ((((int)unitdata->udata.len == 0) &&
	    ...
	    /* preserve compat: only do second case for XTI */
	    (api_semantics == TX_XTI_API &&
		unitdata->udata.len > unitdata->udata.maxlen)) {
		t_errno = TBADDATA;
		MUTEX_UNLOCK_THRMASK(&tiptr->ti_lock, mask);
		trace2(TR_t_sndudata, 1, fd);
		return (-1);
