...
#ident	"@(#)t_optmgmt.c	1.19	96/10/14 SMI"
		/* SVr4.0 1.3.4.1	*/
...
/*
 * The following is likely to change to alignment along t_scalar_t or
 * some such type which should get derived to be a 32-bit type. Should
 * change when XTI standard changes.
 */
#define	ALIGN_XTI_opthdr_size	(sizeof (u_long))
...
int
_tx_optmgmt(fd, req, ret, api_semantics)
int fd;
...
int api_semantics;
{
	...
	if ((api_semantics == TX_XTI_API) && (optlen > 0)) {
		...
		for (opt = opt_start; opt < opt_end; opt = next_opt) {
			...
			if (opt->name == T_ALLOPT) {
				if (next_opt < opt_end) {
					/*
					 * there are options folowing, ignore
					 * them and truncate input
					 */
					optlen = ((u_char *) next_opt
					    - (u_char *) opt_start);
					opt_end = next_opt;
				}
			}
	...
	if (retlen < sizeof (struct T_optmgmt_ack)) {
		t_errno = TSYSERR;
		errno = EIO;
		goto err_out;
	}
