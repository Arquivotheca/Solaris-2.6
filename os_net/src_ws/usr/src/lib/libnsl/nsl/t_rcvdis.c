...
#ident	"@(#)t_rcvdis.c	1.21	96/10/14 SMI"	/* SVr4.0 1.10	*/
...
int
_tx_rcvdis(fd, discon, api_semantics)
int fd;
...
int api_semantics;
{
	...
	if ((tiptr->ti_lookcnt > 0) &&
	    (*((long *)tiptr->ti_lookbufs.tl_lookcbuf) == T_DISCON_IND)) {
		/*
		...
		if ((retval = _t_look_locked(fd, tiptr, api_semantics)) < 0) {
			sv_errno = errno;
			MUTEX_UNLOCK_THRMASK(&tiptr->ti_lock, mask);
			trace2(TR_t_rcvdis, 1, fd);
			errno = sv_errno;
