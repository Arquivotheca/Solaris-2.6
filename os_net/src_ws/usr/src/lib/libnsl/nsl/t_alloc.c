...
#ident	"@(#)t_alloc.c	1.20	96/10/14 SMI"	/* SVr4.0 1.4.1.2	*/
 ...
 * Function protoypes
 */
static int _alloc_buf(struct netbuf *buf, int n, int fields,
    int api_semantics);

char *
_tx_alloc(fd, struct_type, fields, api_semantics)
int fd;
int struct_type;
int fields;
int api_semantics;
	...
	case T_CALL:
		...
		if (fields & T_OPT) {
			if (info.OPT_size >= 0 && api_semantics == TX_XTI_API)
				...
				optsize = info.OPT_size;
			if (_alloc_buf(&p.call->opt, (int)optsize,
			    fields, api_semantics) < 0)
				goto errout;
		}
		if (fields & T_UDATA) {
			dsize = _T_MAX((int)info.CDATA_size,
					(int)info.DDATA_size);
			if (_alloc_buf(&p.call->udata, (int) dsize,
					fields, api_semantics) < 0)
				goto errout;
		}
		MUTEX_UNLOCK_THRMASK(&tiptr->ti_lock, mask);
	...
	case T_OPTMGMT:
		...
		if (fields & T_OPT) {
			if (info.OPT_size >= 0 && api_semantics == TX_XTI_API)
				...
				optsize = info.OPT_size;
			if (_alloc_buf(&p.opt->opt, (int)optsize,
				fields, api_semantics) < 0)
				goto errout;
		}
		MUTEX_UNLOCK_THRMASK(&tiptr->ti_lock, mask);
	...
	case T_UNITDATA:
		...
		if (fields & T_OPT) {
			if (info.OPT_size >= 0 && api_semantics == TX_XTI_API)
				...
				optsize = info.OPT_size;
			if (_alloc_buf(&p.udata->opt, (int)optsize,
				fields, api_semantics) < 0)
				goto errout;
		}
		if (fields & T_UDATA) {
	...
	case T_UDERROR:
		...
		if (fields & T_OPT) {
			if (info.OPT_size >= 0 && api_semantics == TX_XTI_API)
				...
				optsize = info.OPT_size;
			if (_alloc_buf(&p.uderr->opt, (int)optsize,
				fields, api_semantics) < 0)
				goto errout;
		}
		MUTEX_UNLOCK_THRMASK(&tiptr->ti_lock, mask);
...
static int
_alloc_buf(buf, n, fields, api_semantics)
struct netbuf *buf;
int n;
int fields;
int api_semantics;
