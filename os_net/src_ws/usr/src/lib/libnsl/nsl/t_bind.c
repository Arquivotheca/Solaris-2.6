...
#ident	"@(#)t_bind.c	1.18	96/10/14 SMI"	/* SVr4.0 1.3.4.1	*/
...
int
_tx_bind(fd, req, ret, api_semantics)
int fd;
register struct t_bind *req;
register struct t_bind *ret;
int api_semantics;
{
	register struct T_bind_req *bind_reqp;
	...
	if (retlen < sizeof (struct T_bind_ack)) {
		t_errno = TSYSERR;
		errno = EIO;
		goto err_out;
	}
	...
	if ((req != NULL) && req->addr.len != 0 &&
		...
		if ((req->addr.len != bind_ackp->ADDR_length) ||
		    (memcmp((caddr_t) req->addr.buf,
			    (caddr_t) ctlbuf.buf + bind_ackp->ADDR_offset,
			    req->addr.len) != 0)) {
	...
	if (ret != NULL) {
		if (api_semantics == TX_TLI_API ||
			...
			(void) memcpy(ret->addr.buf,
				(char *)(ctlbuf.buf + bind_ackp->ADDR_offset),
				(int)bind_ackp->ADDR_length);
