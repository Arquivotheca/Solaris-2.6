...
#ident	"@(#)_conn_util.c	1.18	96/10/14 SMI"
		/* SVr4.0 1.6.3.2	*/
...
/*
 * The routines in this file are shared by both TLI and XTI entry points.
 * It uses the new TPI version to get access to the enhanced TPI.
 * It still uses only the TLI header <tiuser.h> (and not <xti.h>) to get
 * access to data structures.
 * Since the XTI header redefines entry point names too
 * (e.g. t_open --> _xti_open) and it is not clear if that is desirable in
 * shared utility routines.
 * This would however not give it access to XTI versions of data structures
 * that have changed (e.g. struct t_info). Currently that is not a problem.
 * It could be reconsidered if it ever becomes necessary to access something
 * from <xti.h> which is not visible therough combination of <tiuser.h> and
 * the enhanced TPI (with full <sys/tpicommon.h> space exposed)
 */
 ...
 */
int
_t_snd_conn_req(tiptr, call, ctlbufp)
...
struct strbuf *ctlbufp;
{
	...
	creq->OPT_offset = 0;
	size = sizeof (struct T_conn_req); /* size without any buffers */
 ...
 */
int
_t_rcv_conn_con(tiptr, call, maskp, ctlbufp, api_semantics)
struct _ti_user *tiptr;
struct t_call *call;
sigset_t *maskp;
struct strbuf *ctlbufp;
int api_semantics;
{
	 ...
	 */
	if (ctlbufp->len < (int)sizeof (long)) {
		t_errno = TSYSERR;
		errno = EPROTO;
		goto err_out;
	}
