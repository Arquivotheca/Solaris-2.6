...
#ident	"@(#)t_listen.c	1.19	96/10/14 SMI"	/* SVr4.0 1.5	*/
...
int
_tx_listen(fd, call, api_semantics)
int fd;
struct t_call *call;
int api_semantics;
{
	...
	case T_CONN_IND:
		...
		tiptr->ti_ocnt++;
		call->sequence = (int) pptr->conn_ind.SEQ_number;
		...
		if (api_semantics == TX_TLI_API || call->opt.maxlen > 0) {
			...
			(void) memcpy(call->opt.buf, ctlbuf.buf +
				pptr->conn_ind.OPT_offset,
				(unsigned int)pptr->conn_ind.OPT_length);
