...
#ident	"@(#)getrpcent_r.c	1.7	94/10/04	SMI"
	...
	int	buflen;
{
	...
	memcpy(buffer, namestart, namelen);
	buffer[namelen] = '\0';
	rpc->r_name = buffer;
	...
	numbuf[numlen] = '\0';
	rpc->r_number = strtol(numbuf, &numend, 10);
	if (*numend != '\0') {
		trace3(TR_str2rpcent, 1, lenstr, buflen);
		return NSS_STR_PARSE_PARSE;
	}
	...
	rpc->r_aliases = _nss_netdb_aliases(p, lenstr - (p - instr),
