#ifndef lint
static char sccsid[] = "@(#)au_open.c 1.9 94/05/16 SMI";
#endif
...
#endif
{
	...
	long byte_count;	/* bytes in the record */
	int   v;
	 ...
	 */
	for (byte_count = 18, record = dchain;
	    record != (token_t *)0; record = record->tt_next) {
	 ...
	 */
	v = audit((caddr_t) buffer, (int) byte_count);
	free(buffer);
	_mutex_unlock(&mutex_au_d);
	return (v);
}
