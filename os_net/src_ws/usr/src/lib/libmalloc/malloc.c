...
#ident	"@(#)malloc.c	1.10	96/09/26 SMI"		/* SVr4.0 1.12  */
...
/* moved before include of assert.h */
#ifndef debug
#define	NDEBUG
#endif
#include <stdlib.h>
 ...
 *  To guarentee freeptr follows arena in memory define arena[2-3] as freeptr
...
static unsigned int maxfast = MAXFAST - MINHEAD;
...
{
	struct header *blk;
	unsigned nb;	/* size of entire block we need */
	...
	if (nbytes <= maxfast) {
		...
		/* previous code assumed MINHEAD was a multiple of grain */
		/* align */
		nb = (nbytes + MINHEAD + grain - 1) / grain * grain;
		...
		if ((holdblk != HGROUND) && (holdblk->lfreeq != LGROUND)) {
			...
			if (lblk < holdblk->unused) {
				...
				holdhead[nb/grain] = holdblk->nexthblk;
			}
		...
		{
			struct holdblk * psh =
			    (struct holdblk *)CLRALL(lblk->header.holder);
			assert(psh->blksz >= nbytes);
			assert(psh->blksz < nbytes + grain);
		}
		...
		if (blk == &(freeptr[1])) {
			...
			int nget;	/* number of words to get */
			...
			if ((newblk = (struct header *)_sbrk(0)) !=
			    (struct header *)((char *)arenaend + MINHEAD)) {
				/* case 1 */
#ifdef debug
				if (case1count++ > 0)
					...
					"brk or sbrk?\n", 41);
#endif
				/* note nb = ROUNDUP(nget + minhead, ALIGNSZ) */
				/* get size to fetch */
				nget = nb+ ALIGNSZ;
				/* round up to a block */
				nget = (nget + (BLOCKSZ - 1)) /
				...
				/* add to arena */
				newend = (struct header *)((char *)newblk +
				    nget - MINHEAD);
				newend->nextblk = SETBUSY(&(arena[1]));
				newblk = (struct header *)((char *)newblk +
				    (ALIGNSZ - MINHEAD));
...
free_unlocked(void *ptr)
{
	...
	struct holdblk *oldhead;	/* former head of the hold */
	...
	if (TESTSMAL(((struct header *)((char *)ptr - MINHEAD))->nextblk)) {
		...
		int		offset;	/* choice of header lists */
		...
		if (!TESTBUSY(holdblk = lblk->header.holder))
			return;
		...
		offset = (holdblk->blksz + MINHEAD)/grain;
...
realloc_unlocked(void *ptr, size_t size)
{
	...
	unsigned trusize;	/* block size as allocater sees it */
	char *newptr;			/* pointer to user's new block */
	unsigned cpysize;	/* amount to copy */
