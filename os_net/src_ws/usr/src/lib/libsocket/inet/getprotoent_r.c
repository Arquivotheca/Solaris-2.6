...
#ident	"@(#)getprotoent_r.c	1.6	93/09/30	SMI"
...
struct protoent *
getprotobyname_r(name, result, buffer, buflen)
	const char	*name;
...
struct protoent *
getprotobynumber_r(proto, result, buffer, buflen)
	int proto;
	struct protoent	*result;
	char		*buffer;
	int		buflen;
...
struct protoent *
getprotoent_r(result, buffer, buflen)
	struct protoent	*result;
	char		*buffer;
	int		buflen;
{
 ...
 */
static int
str2protoent(instr, lenstr, ent, buffer, buflen)
	const char	*instr;
	int		lenstr;
	...
	int	buflen;
{
	...
	numbuf[numlen] = '\0';
	proto->p_proto = strtol(numbuf, &numend, 10);
	if (*numend != '\0') {
	...
	proto->p_aliases = _nss_netdb_aliases(p, lenstr - (p - instr),
