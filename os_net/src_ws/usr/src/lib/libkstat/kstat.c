...
#pragma	ident	"@(#)kstat.c	1.3	93/06/30 SMI"
...
static void
kstat_zalloc(void **ptr, int size, int free_first)
{
	if (free_first)
		free(*ptr);
...
kstat_read(kstat_ctl_t *kc, kstat_t *ksp, void *data)
{
	...
	while ((kcid = (kid_t) ioctl(kc->kc_kd, KSTAT_IOC_READ, ksp)) == -1) {
		if (errno == EAGAIN) {
			poll(NULL, 0, 100);	/* back off a moment */
	...
	if (data != NULL)
		memcpy(data, ksp->ks_data, ksp->ks_data_size);
	return (kcid);
}
...
kstat_write(kstat_ctl_t *kc, kstat_t *ksp, void *data)
{
	...
	if (data != NULL)
		memcpy(ksp->ks_data, data, ksp->ks_data_size);
...
kstat_chain_update(kstat_ctl_t *kc)
{
	...
	k0.ks_data = NULL;
	k0.ks_data_size = 0;
	strcpy(k0.ks_name, "kstat_headers");
...
kstat_lookup(kstat_ctl_t *kc, char *ks_module, int ks_instance, char *ks_name)
{
	...
	for (ksp = kc->kc_chain; ksp; ksp = ksp->ks_next) {
		if ((ks_module == NULL || !strcmp(ksp->ks_module, ks_module)) &&
		    (ks_instance == -1 || ksp->ks_instance == ks_instance) &&
		    (ks_name == NULL || !strcmp(ksp->ks_name, ks_name)))
			return (ksp);
	}
	return (NULL);
}
