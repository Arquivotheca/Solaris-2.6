...
#pragma ident	"@(#)volmgt_fsidbi.c	1.6	96/10/10 SMI"
...
#define	READ_FORMAT	"%d||%d||%[^\n]"
#define	WRITE_FORMAT	"%d||%d||%s\n"
...
vol_db_insert(vol_dbid_t dbid, vol_db_entry_t *ep)
{
	...
	denter("volmgt_db_insert(%d, [%d(%d.%d), %d, ...]): entering\n", dbid,
	    ep->dev, major(ep->dev), minor(ep->dev), ep->pid);
#endif
	...
	if (fprintf(fp, WRITE_FORMAT, ep->dev, ep->pid, ep->id_tag) < 0) {
#ifdef	DEBUG
...
vol_db_remove(vol_dbid_t dbid, dev_t dev)
{
	...
	dev_t		fdev;		/* dev read from database */
	...
	while (flen > 0) {
		...
		nitems = sscanf(tiobp, READ_FORMAT, &fdev, &fpid, buf);
		...
		if (fdev == dev) {
			...
			if (fprintf(fp, WRITE_FORMAT, fdev, fpid, buf) < 0) {
				goto dun;
			}
		}
...
{
	FILE			*fp;
	dev_t			fdev;	/* device read from database */
	 ...
	 */
	while ((nitems = fscanf(fp, READ_FORMAT, &fdev, &fpid, buf)) > 0) {
...
dun:
#ifdef	DEBUG
	dexit("volmgt_db_find: returning %#x\n", (char *)retval);
#endif
	return (retval);
}
