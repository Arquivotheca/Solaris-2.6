...
#pragma ident	"@(#)volname.c	1.28	96/09/19 SMI"
...
media_findname_work(char *start)
{
	...
	if (IS_ABS_PATH(start)) {
		...
		rv = getfullrawname(nameptr);
		if ((rv == NULL) || (*rv == NULLC)) {
#ifdef	DEBUG
			dprintf(
			    "media_findname_work: getfullrawname(%s) failed\n",
			    nameptr);
#endif
			goto dun;
		}
	 ...
	 */
	(void) sprintf(pathbuf, "%s/%s", vold_alias_dir, start);
	...
	while (dp = readdir64(dirp)) {
		 ...
		 */
		(void) sprintf(pathbuf, "%s/%s", vold_alias_dir, dp->d_name);
...
try_hack:
	...
	for (i = 0; i < HACKNAME_MAX; i++) {
		...
		(void) sprintf(pathbuf, "%s/%s%d", vold_alias_dir, start, i);
...
_media_printaliases(void)
{
	...
	(void) sprintf(pathbuf, "%s/%s", vold_root, ALIAS_DIR);
