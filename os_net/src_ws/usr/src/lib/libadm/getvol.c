...
#ident	"@(#)getvol.c	1.5	93/02/09 SMI"       /* SVr4.0 1.4 */
...
void	doremovecmd(char *device, int echo);
	...
	for(;;) {
		...
		if(*strval == 'f') {
			 if (options & DM_FORMFS)
				...
				doformat(voltxt, label, fmtcmd, NULL);
			continue;
		} else if (*strval == 'm') {
			doformat(voltxt, label, NULL, mkfscmd);
			continue;
