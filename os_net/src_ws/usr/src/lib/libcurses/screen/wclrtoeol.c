...
#ident	"@(#)wclrtoeol.c	1.7	93/06/08 SMI"	/* SVr4.0 1.4	*/
...
#ifdef	DEBUG
	if (outf)
	    fprintf(outf, "CLRTOEOL: line %d begx = %d, maxx = %d, lastch = %d, next firstch %d\n", y, win->_begx, win->_firstch[y], win->_lastch[y], win->_firstch[y+1]);
#endif	/* DEBUG */
