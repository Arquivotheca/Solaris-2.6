...
#ident	"@(#)typeahead.c	1.6	96/03/27 SMI"	/* SVr4.0 1.5	*/
...
#include	<fcntl.h>
    ...
    /*
    cur_term->_check_fd = (tty = ttyname(fd)) ? open(tty, O_RDONLY | O_NDELAY) :
						-1;
    */
    if (isatty(fd)) {
        if ((cur_term->_check_fd = open("/dev/tty", O_RDONLY | O_NDELAY)) >= 0)
		fcntl(cur_term->_check_fd, F_SETFD, 1);
    }
    else
