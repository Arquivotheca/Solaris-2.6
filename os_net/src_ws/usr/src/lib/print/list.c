...
#pragma ident "@(#)list.c	1.7	96/04/22 SMI"
...
list_append(void **list, void *item)
{
	...
	if (list == NULL) {
		...
		memset(list, NULL, (_list_increment * sizeof (void *)));
		list[0] = item;
	} else {
		int	count;
		...
		if ((count + 1) % _list_increment == 0) { /* increase size */
			...
			new_list = (void **)calloc(new_size,
						sizeof (void *));
			memset(new_list, NULL, (new_size * sizeof (void *)));
			for (count = 0; list[count] != NULL; count++)
...
list_concatenate(void **list1, void **list2)
{
	...
	if ((list = (void **)calloc((new_size), sizeof (void *)))
		...
		memset(list, NULL, (new_size * sizeof (void *)));
