...
#pragma ident "@(#)misc.c	1.7	96/04/22 SMI"
...
#ifdef SUNOS_4
char *
cftime(char *buf, char *fmt, time_t *curr)
{
...
char *
strtok_r(char *s, char *t, char **last)
{
	return (strtok(s, t));
}
#endif
...
start_daemon(int do_fork)
{
	...
	if ((jobs = job_list_append(NULL, NULL, SPOOL_DIR)) == NULL)
		return;

#ifdef SUNOS_4
	list_iterate((void **)jobs, job_free);
#else
	...
	if (lock < 0)
		return;
	if (do_fork == 0) {
		execl("/usr/bin/lp", MASTER_NAME, NULL);
