...
#ident	"@(#)_Qfdiv.c	1.3	92/07/14 SMI"
	...
	*pz = *px;
	...
	case fp_normal:
		...
		case fp_zero:	/* number/0 */
			__fpu_set_exception(fp_division,cexc);
	...
	while(q<0x10000) {	/* generate quo[0] */
		...
		if(__fpu_cmpli(r,y,4)>=0) {
			...
			c  = 0;
			c = __fpu_sub3wc(&r[3],r[3],y[3],c);
			c = __fpu_sub3wc(&r[2],r[2],y[2],c);
			c = __fpu_sub3wc(&r[1],r[1],y[1],c);
			c = __fpu_sub3wc(&r[0],r[0],y[0],c);
		}
	...
	if((r[0]|r[1]|r[2]|r[3])==0) pz->sticky = pz->rounded = 0;
		...
		if(__fpu_cmpli(r,y,4)>=0) pz->rounded = 1;
	}
}
...
{				/* *pz gets sqrt(*px) */
	...
	*pz = *px;
	...
	while(r!=0) {			/* compute sqrt[1] */
		t[1] = s[1]+r;	/* no carry */
		t[0] = s[0];
		if(__fpu_cmpli(t,x,2)<=0) {
			c = 0;
			c = __fpu_add3wc(&s[1],t[1],r,c);
			c = __fpu_add3wc(&s[0],t[0],0,c);
			c = 0;
			c = __fpu_sub3wc(&x[1],x[1],t[1],c);
			c = __fpu_sub3wc(&x[0],x[0],t[0],c);
			q    += r;
		}
		x[0] = (x[0]<<1)|((x[1]&0x80000000)>>31);	/* x<<1 */
		x[1] = (x[1]<<1)|((x[2]&0x80000000)>>31);
	...
	while(r!=0) {			/* compute sqrt[2] */
		...
		t[1] = s[1];
		t[0] = s[0];
		if(__fpu_cmpli(t,x,3)<=0) {
			c = 0;
			c = __fpu_add3wc(&s[2],t[2],r,c);
			c = __fpu_add3wc(&s[1],t[1],0,c);
			c = __fpu_add3wc(&s[0],t[0],0,c);
			c = 0;
			c = __fpu_sub3wc(&x[2],x[2],t[2],c);
			c = __fpu_sub3wc(&x[1],x[1],t[1],c);
			c = __fpu_sub3wc(&x[0],x[0],t[0],c);
			q    += r;
		}
	...
	while(r!=0) {			/* compute sqrt[3] */
		...
		t[1] = s[1];
		t[0] = s[0];
		if(__fpu_cmpli(t,x,4)<=0) {
			c = 0;
			c = __fpu_add3wc(&s[3],t[3],r,c);
			c = __fpu_add3wc(&s[2],t[2],0,c);
			c = __fpu_add3wc(&s[1],t[1],0,c);
			c = __fpu_add3wc(&s[0],t[0],0,c);
			c = 0;
			c = __fpu_sub3wc(&x[3],x[3],t[3],c);
			c = __fpu_sub3wc(&x[2],x[2],t[2],c);
			c = __fpu_sub3wc(&x[1],x[1],t[1],c);
			c = __fpu_sub3wc(&x[0],x[0],t[0],c);
			q    += r;
	...
	if((x[0]|x[1]|x[2]|x[3])==0) {
		...
		pz->sticky = 1;
		if(__fpu_cmpli(s,x,4)<0) pz->rounded=1; else pz->rounded = 0;
	}
}
