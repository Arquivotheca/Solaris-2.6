#ident "@(#)_lwp_mutex_unlock.s 1.8 95/08/08"
 ...
 */
	...
	swap 	[%o0+MUTEX_LOCK_WORD], %o5	! clear lock/get waiter field
						! atomically
	set 	WAITER_MASK, %o4 		! mask to extract waiter field
						! from word
	btst 	%o5, %o4			! check for waiters
	beq,a 	.ret				! if no waiters
	clr 	%o0				! 	return 0
						! else (note that %o0 is still
						!       &mutex)
	SYSTRAP(lwp_mutex_unlock)		! call kernel to wakeup waiter
	SYSLWPERR
.ret: 	retl
	nop
