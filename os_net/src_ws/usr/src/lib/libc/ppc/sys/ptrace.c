...
#pragma ident "@(#)ptrace.c 1.9	96/08/15 SMI"
...
again:
	...
	case 6:		/* PTRACE_POKEUSER */
		...
		if ((int)xaddr >= U_REG && xaddr < U_REG+sizeof (gregset_t)) {
			...
			if (rx == R_CR)
				data = cp->user.u_reg[R_CR];
			else if (rx == R_R1 || rx == R_PC)
				data &= ~03;
	...
	case 9:		/* PTRACE_SINGLESTEP */
		...
		if (cp->flags & CS_SETREGS) {
			...
			ps->pr_lwp.pr_reg[R_MSR] = cp->user.u_reg[R_MSR];
			ps->pr_lwp.pr_reg[R_PC]  = cp->user.u_reg[R_PC];
			ps->pr_lwp.pr_reg[R_CR]  = cp->user.u_reg[R_CR];
			ps->pr_lwp.pr_reg[R_R0]  = cp->user.u_reg[R_R0];
			ps->pr_lwp.pr_reg[R_R1]  = cp->user.u_reg[R_R1];
			ps->pr_lwp.pr_reg[R_R2]  = cp->user.u_reg[R_R2];
			ps->pr_lwp.pr_reg[R_R3]  = cp->user.u_reg[R_R3];
			ps->pr_lwp.pr_reg[R_R4]  = cp->user.u_reg[R_R4];
			ps->pr_lwp.pr_reg[R_R5]  = cp->user.u_reg[R_R5];
			ps->pr_lwp.pr_reg[R_R6]  = cp->user.u_reg[R_R6];
			ps->pr_lwp.pr_reg[R_R7]  = cp->user.u_reg[R_R7];
			ps->pr_lwp.pr_reg[R_R8]  = cp->user.u_reg[R_R8];
			ps->pr_lwp.pr_reg[R_R9]  = cp->user.u_reg[R_R9];
			ps->pr_lwp.pr_reg[R_R10] = cp->user.u_reg[R_R10];
			ps->pr_lwp.pr_reg[R_R11] = cp->user.u_reg[R_R11];
			ps->pr_lwp.pr_reg[R_R12] = cp->user.u_reg[R_R12];
			ps->pr_lwp.pr_reg[R_R13] = cp->user.u_reg[R_R13];
			ps->pr_lwp.pr_reg[R_R14] = cp->user.u_reg[R_R14];
			ps->pr_lwp.pr_reg[R_R15] = cp->user.u_reg[R_R15];
			ps->pr_lwp.pr_reg[R_R16] = cp->user.u_reg[R_R16];
			ps->pr_lwp.pr_reg[R_R17] = cp->user.u_reg[R_R17];
			ps->pr_lwp.pr_reg[R_R18] = cp->user.u_reg[R_R18];
			ps->pr_lwp.pr_reg[R_R19] = cp->user.u_reg[R_R19];
			ps->pr_lwp.pr_reg[R_R20] = cp->user.u_reg[R_R20];
			ps->pr_lwp.pr_reg[R_R21] = cp->user.u_reg[R_R21];
			ps->pr_lwp.pr_reg[R_R22] = cp->user.u_reg[R_R22];
			ps->pr_lwp.pr_reg[R_R23] = cp->user.u_reg[R_R23];
			ps->pr_lwp.pr_reg[R_R24] = cp->user.u_reg[R_R24];
			ps->pr_lwp.pr_reg[R_R25] = cp->user.u_reg[R_R25];
			ps->pr_lwp.pr_reg[R_R26] = cp->user.u_reg[R_R26];
			ps->pr_lwp.pr_reg[R_R27] = cp->user.u_reg[R_R27];
			ps->pr_lwp.pr_reg[R_R28] = cp->user.u_reg[R_R28];
			ps->pr_lwp.pr_reg[R_R29] = cp->user.u_reg[R_R29];
			ps->pr_lwp.pr_reg[R_R30] = cp->user.u_reg[R_R30];
			ps->pr_lwp.pr_reg[R_R31] = cp->user.u_reg[R_R31];
			cmd = PCSREG;
	...
	register cstatus_t *cp;
{
	...
	cp->user.u_reg[R_MSR] = ps->pr_lwp.pr_reg[R_MSR];
	cp->user.u_reg[R_PC]  = ps->pr_lwp.pr_reg[R_PC];
	cp->user.u_reg[R_CR]  = ps->pr_lwp.pr_reg[R_CR];
	cp->user.u_reg[R_R0]  = ps->pr_lwp.pr_reg[R_R0];
	cp->user.u_reg[R_R1]  = ps->pr_lwp.pr_reg[R_R1];
	cp->user.u_reg[R_R2]  = ps->pr_lwp.pr_reg[R_R2];
	cp->user.u_reg[R_R3]  = ps->pr_lwp.pr_reg[R_R3];
	cp->user.u_reg[R_R4]  = ps->pr_lwp.pr_reg[R_R4];
	cp->user.u_reg[R_R5]  = ps->pr_lwp.pr_reg[R_R5];
	cp->user.u_reg[R_R6]  = ps->pr_lwp.pr_reg[R_R6];
	cp->user.u_reg[R_R7]  = ps->pr_lwp.pr_reg[R_R7];
	cp->user.u_reg[R_R8]  = ps->pr_lwp.pr_reg[R_R8];
	cp->user.u_reg[R_R9]  = ps->pr_lwp.pr_reg[R_R9];
	cp->user.u_reg[R_R10] = ps->pr_lwp.pr_reg[R_R10];
	cp->user.u_reg[R_R11] = ps->pr_lwp.pr_reg[R_R11];
	cp->user.u_reg[R_R12] = ps->pr_lwp.pr_reg[R_R12];
	cp->user.u_reg[R_R13] = ps->pr_lwp.pr_reg[R_R13];
	cp->user.u_reg[R_R14] = ps->pr_lwp.pr_reg[R_R14];
	cp->user.u_reg[R_R15] = ps->pr_lwp.pr_reg[R_R15];
	cp->user.u_reg[R_R16] = ps->pr_lwp.pr_reg[R_R16];
	cp->user.u_reg[R_R17] = ps->pr_lwp.pr_reg[R_R17];
	cp->user.u_reg[R_R18] = ps->pr_lwp.pr_reg[R_R18];
	cp->user.u_reg[R_R19] = ps->pr_lwp.pr_reg[R_R19];
	cp->user.u_reg[R_R20] = ps->pr_lwp.pr_reg[R_R20];
	cp->user.u_reg[R_R21] = ps->pr_lwp.pr_reg[R_R21];
	cp->user.u_reg[R_R22] = ps->pr_lwp.pr_reg[R_R22];
	cp->user.u_reg[R_R23] = ps->pr_lwp.pr_reg[R_R23];
	cp->user.u_reg[R_R24] = ps->pr_lwp.pr_reg[R_R24];
	cp->user.u_reg[R_R25] = ps->pr_lwp.pr_reg[R_R25];
	cp->user.u_reg[R_R26] = ps->pr_lwp.pr_reg[R_R26];
	cp->user.u_reg[R_R27] = ps->pr_lwp.pr_reg[R_R27];
	cp->user.u_reg[R_R28] = ps->pr_lwp.pr_reg[R_R28];
	cp->user.u_reg[R_R29] = ps->pr_lwp.pr_reg[R_R29];
	cp->user.u_reg[R_R30] = ps->pr_lwp.pr_reg[R_R30];
	cp->user.u_reg[R_R31] = ps->pr_lwp.pr_reg[R_R31];
