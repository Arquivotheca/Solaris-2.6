...
#pragma ident "@(#)double_decim.c 1.2	96/11/25 SMI"
...
__binary_to_decimal_integer(pfint, nsig, ds, nzeros, ndigs)
	...
	unsigned	*nzeros;	/* Output number of implicit */
					/* trailing zeros produced. */
	unsigned	*ndigs;		/* Output number of explicit */
					/* digits produced in ds. */
...
__binary_to_decimal_fraction(pfrac, nsig, nfrac, ds, nzeros, ndigs)
	...
	char		ds[];	/* Output decimal integer string output - */
				/* must be large enough. */
	int		*nzeros;	/* Output number of implicit leading */
					/* zeros produced. */
	int		*ndigs;		/* Output number of explicit digits */
					/* produced in ds. */
...

{
	...
	*nzeros = 0;
	...
	d.bsize = _BIG_FLOAT_SIZE;
	tenpower = nsig + (int) (((17 - pfrac->bexponent - 16 * pfrac->blength)
...
#ifdef DEBUG
	(void) printf(" binary to decimal fraction nsig %d nfrac %d tenpower \
	...
	if (pb->bexponent < 0) {
		__right_shift_base_two(pb, (short unsigned) -pb->bexponent,
						&stickyshift);
		if (stickyshift != 0)
...
finish_ds:
	...
	*ndigs = is;
	...
	assert(tenpower >= is);
#endif
	*nzeros = tenpower - is;	/* There were supposed to be tenpower */
					/* leading digits, and is were found. */
...
_unpacked_to_decimal_two(pfint, pfrac, pm, pd, ps)
	...
	fp_exception_field_type *ps;
{
	int		i, intdigs, fracdigs, fraczeros, fracsigs, ids = 0;
	int		idsbound, lzbound, fracnonzero;
...
overflow:
	...
	if (pfint->blength > 0) {	/* Compute integer part of result. */
		if (pm->df == floating_form)
			nsig = pm->ndigits + 1;
			/* Significant digits wanted for E format, plus one */
			/* for rounding. */
		else
			nsig = DECIMAL_STRING_LENGTH - 1;
			/* Significant digits wanted for F format */
			/* == all. */
	...
	if (((pm->df == fixed_form) && (pm->ndigits >= 0)) ||
		...
		__binary_to_decimal_fraction(pfrac, nsig, nfrac, fs,
					&fraczeros, &fracsigs);
	...
	if (pm->df == floating_form) {
			...
			idsbound = pm->ndigits;
		{
			int	ncopy = idsbound;
			if (idsbound > 0) {
				(void) memcpy((char *) (pd->ds), (char *) is,
							(int) ncopy);
				ids = ncopy;
			}
		}
		...
		} else {	/* Need more significant digits from */
				/* fraction part. */
		...
		if (pm->ndigits >= 0) {	/* Normal F format. */
			...
			/* Some integer digits do not get rounded away. */
				{
...

{
	...
	if ((pfint->bexponent < 0) && (pfint->blength > 0)) {
		__right_shift_base_two(pfint, (short unsigned)
				-pfint->bexponent, &sticky); /* right shift */
...
tryidsexp:
		...
		if (__tbl_tens[pm->ndigits - 1] >= prod) {
...
#ifdef DEBUG
				if (retry > 0)
					(void) printf("idsexp too small second \
						try x %X %X nd %d idsexp %d \
						prod %30.1f \n", dds,
						pm->ndigits, idsexp, prod);
				else
					(void) printf("idsexp too small first \
						try x %X %X nd %d idsexp %d \
						prod %30.1f \n", dds,
						pm->ndigits, idsexp, prod);
#endif DEBUG
			...
			/* prod too big - wrong power of ten or */
			/* other failure. */
#ifdef DEBUG
			if (retry > 0)
				(void) printf("idsexp too big second try x \
					%X %X nd %d idsexp %d prod %30.1f \n",
					dds, pm->ndigits, idsexp, prod);
			else
				(void) printf("idsexp too big first  try x \
					%X %X nd %d idsexp %d prod %30.1f \n",
					dds, pm->ndigits, idsexp, prod);
#endif
			if (retry > 0)
