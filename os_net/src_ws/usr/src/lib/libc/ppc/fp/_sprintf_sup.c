...
#ident	"@(#)_sprintf_sup.c	1.5	94/09/06 SMI"
...

{
	...
	pd->bsignificand[1] = __quorem10000((unsigned long)
	...
	for (i = pb->blength - 2; i >= 0; i--) { /* Multiply by 2**16 and */
		...
		__mul_65536short((unsigned long) pb->bsignificand[i],
			(&pd->bsignificand[0]), &(pd->blength));
	}
	for (i = 0; i <= (pb->bexponent - 16); i += 16) { /* Multiply by  */
							/* 2**16 for each */
							/* trailing zero. */
		__mul_65536short((unsigned long) 0, (&pd->bsignificand[0]),
				&(pd->blength));
	}
	if (pb->bexponent > i)
		__left_shift_base_ten(pd, (short unsigned)
				(pb->bexponent - i));
	pd->bexponent = 0;
