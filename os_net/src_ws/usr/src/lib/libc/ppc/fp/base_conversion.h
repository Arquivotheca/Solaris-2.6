...
#pragma ident "@(#)base_conversion.h 1.5	94/09/10 SMI"
...
extern void	__infnanstring();
		/* Utility to copy inf infinity or nan strings. */

/* Whether to use floating-point or integer arithmetic varies */
/* among implementations; floating point seems to have a slight */
/* edge on Sunrise+FPU2 but not on Sirius+FPA or Sun-386+80387 */

#if 0	/* Tried this for SPARC, causes FP exceptions in wrong places */
#define	QUOREM10000(A, B) \
/* A = A/10000, B = A%10000, for int A > 0 and short unsigned B */ \
...
#define	LONGQUOREM10000(A, B) \
/* A = A/10000, B = A%10000, for unsigned A and short unsigned B */ \
	{ register int ___iq; register int ___ir; \
	___ir = (A); \
	___iq = ___ir * 1.0e-4; \
	___ir -= 1.0e4 * ___iq; \
...
#define	_rshift_b65536(x, n, c) ((((unsigned long) (x)) << (16-(n))) + \
							((c)<<16))
...
#define	__carry_out_b10000(coz, coc) \
...
/* function to multiply a big_float times a positive power of two or ten. */
		...
		int		mult,	/* if mult is two, x is base 10**4; */
					/* if mult is ten, x is base 2**16 */
		int		n,
		int		precision,	/* Number of bits of */
						/* precision ultimately */
						/* required (mult=10) or */
						/* number of digits of */
						/* precision ultimately */
						/* required (mult=2). Extra */
						/* bits are allowed */
						/* internally to permit */
						/* correct rounding. */
		_big_float    **pnewbf	/* Return result *pnewbf is set to: */
					/* pbf uneventful: *pbf holds the */
					/* product;  BIG_FLOAT_TIMES_TOOBIG */
					/* if n is bigger than the tables */
					/* permit; BIG_FLOAT_TIMES_NOMEM */
					/* if pbf->blength was insufficient */
					/* to hold the product, and malloc */
					/* failed to produce a new block; */
					/* &newbf if pbf->blength was */
					/* insufficient to hold the product, */
					/* and a new _big_float was allocated */
					/* by malloc. newbf holds the */
					/* product. It's the caller's */
 ...
 *
 * The powers are stored consecutively in the tables, with the start index
 * table at the end.
 *
...
extern const unsigned short __tbl_10_small_digits[],
	...
	__tbl_10_big_start[], __tbl_10_huge_digits[], __tbl_10_huge_start[];

#define	__TBL_2_SMALL_SIZE	176
#define	__TBL_2_BIG_SIZE	16
#define	__TBL_2_HUGE_SIZE	6
extern const unsigned short __tbl_2_small_digits[], __tbl_2_small_start[],
...
extern const char __four_digits_quick_table[];  /* Table to translate short */
						/* unsigned < 10000 to four */
						/* ASCII digits. */
...
#define	__TBL_TENS_EXACT	22	/* The max n such that 10**n fits */
					/* exactly in IEEE double. */
#define	__TBL_TENS_MAX		49	/* The max n such that 10**n is in */
					/* the table. */
...
typedef struct {
	int status, mode;
}__ieee_flags_type;	/* Struct for storing IEEE mode/status. */

extern void __get_ieee_flags();	/* Returns IEEE mode/status and sets up */
