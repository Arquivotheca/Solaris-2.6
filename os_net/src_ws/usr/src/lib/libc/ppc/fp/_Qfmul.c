...
#ident	"@(#)_Qfmul.c	1.4	94/09/06 SMI"
...

{
	...
	*pz = *px;
	...
	s = r = acc[0] = acc[1] = acc[2] = acc[3] = 0;	/* intialize acc */
							/* to zero */
	...
	if (y != 0) {
		...
		if (j & y) {		/* bit i of y != 0, add x to acc */
			c = 0;
			c = __fpu_add3wc(&acc[3], acc[3], x[3], c);
			c = __fpu_add3wc(&acc[2], acc[2], x[2], c);
			c = __fpu_add3wc(&acc[1], acc[1], x[1], c);
			c = __fpu_add3wc(&acc[0], acc[0], x[0], c);
		}
		j += j;
	    } while (j != 0);
	}
