...
#ident	"@(#)_base_sup.c	1.5	94/09/06 SMI"
...
#define	IDIGIT(i) ((i < 0) ? 0 : ((i < ndigs) ? (ds[i] - '0') : 0))
	...
	pd->bsignificand[id] = 1000 * IDIGIT(ids) + 100 * IDIGIT(ids + 1) +
	...
	for (; ids < (int) (ndigs + taken - 4); ids += 4) {
					...
					/* be found. Main loop. */
		id--;
		pd->bsignificand[id] = 1000 * ds[ids] + 100 * ds[ids + 1] +
...
#endif
	if (id != 0)
		pd->bsignificand[0] = 1000 * IDIGIT(ids) + 100 *
		IDIGIT(ids + 1) + 10 * IDIGIT(ids + 2) + IDIGIT(ids + 3);
...
#define	FDIGIT(i) ((i < nzin) ? 0 : ((i < (nzin+ndigs)) ? (ds[i-nzin] - '0') :\
						0))
	...
	for (; ids < (int) (nzin + ndigs - 4); ids += 4) {
		...
		pbf->bsignificand[ibf] = 1000 * ds[ids - nzin] + 100 *
					ds[ids + 1 - nzin] + 10 * ds[ids + 2 -
					nzin] + ds[ids + 3 - nzin] - 1111 * '0';
	}
...

{
	...
	if ((id == 0) && (pd->bexponent < 0)) {
		...
		idbound = (pd->bexponent < 0) ? 1 : 0; /* How far to carry */
						/* next for loop depends on */
						/* whether last digit */
						/* requires special */
						/* treatment. */
		for (; id >= idbound; id--) {
			__mul_10000short(pb, (long unsigned)
			pd->bsignificand[id]);
		}
	}
...
#ifdef DEBUG
			(void) sprintf(bcastring,
			" __big_decimal_to_big_binary exponent %d ",
			pd->bexponent);
#endif
...

{
	...
	iu = 0;
	for (ib = pb->blength - 1; ((ib - 1) >= 0) && (iu < UNPACKED_SIZE);
			ib -= 2) {
	...
	if (iu < UNPACKED_SIZE) { /* The big float fits in the unpacked */
		...
		while (pb->bsignificand[ib] == 0)
			ib--;
		if (ib >= 0)
			pu->significand[UNPACKED_SIZE - 1] |= 1; /* Stick lsb */
								/* if nonzero */
								/* found. */
	}
 ...
 */
{
	...
	if (n >= 19) {
		...
		*pe = __fbe_none;
		...
		t = __add_set(t, (double) (acc -
			0x3DE43550 /* nine '0' worth */), pe);
	}
	return (t);
}
