...
#ident	"@(#)_Qfpack.c	1.8	94/09/06 SMI"
...
infinity:
	...
	case fp_signaling:
		px->exponent = 0x7fff;
		px->significand = 0x8000 | pu->significand[0];	/* Insure */
								/* quiet nan. */
		*py = pu->significand[1];
		*pz = pu->significand[2];
		*pw = pu->significand[3];
	...
	case fp_op_extended:
		{
			...
			packextended(pu, &x, (unsigned*) &n[i1],
				(unsigned*) &n[i2], (unsigned*) &n[i3], cexc);
			n[i0] = *(int*)&x;
			}
