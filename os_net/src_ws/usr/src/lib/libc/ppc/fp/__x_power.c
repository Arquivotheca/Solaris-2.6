...
#ident	"@(#)__x_power.c	1.7	94/09/06 SMI"
...
{
	short unsigned	base;
	unsigned short	productsize, trailing_zeros_to_delete,
		needed_precision, *pp, *table[3], *start[3], tablepower[3];
	int		i, j, itlast;
...
compute_product_size:
	...
	if (productsize <= pbf->bsize) {
		*pnewbf = pbf;	/* Work with *pnewbf from now on. */
		...
		mallocarg = sizeof (_big_float) + sizeof (_BIG_FLOAT_DIGIT) *
		...
		*pnewbf = (_big_float *) mallocresult;
		__copy_big_float_digits((*pnewbf)->bsignificand,
	...
	for (i = 0; i <= itlast; i++)
		if (tablepower[i] != 0) {	/* Step through each of the */
...
#ifdef DEBUG
			{
				...
				(void) printf(" step %d p operand length %d\
						power %d \n", i, lengthp,
						tablepower[i]);
			...
			if (lengthp == 1) {	/* Special case - multiply by */
						/* <= 10**4 or 2**13 */
				if (base == 10) {
					__multiply_base_ten_by_two(pbf,
							tablepower[i]);
				} else {
					__multiply_base_two(pbf,
					(_BIG_FLOAT_DIGIT) ((table[i])
					[tablepower[i]]), (unsigned long) 0);
				}
#ifdef DEBUG
				...
				for (j = lengthx + lengthp - 2; j >= 0; j--) {
					...
					if (pbf->bsignificand
						[excess_check - 1] == 0)
						excess_check--;
					excess_check -= (int) needed_precision
								+ 4;
					if ((j <= excess_check) &&
						...
						excess = j + 2;	/* Can disca- */
								/* rd j+1, j, */
								/* ... 0 */
#ifdef DEBUG
						(void) printf(" decided to quit\
			early at j %d since s[j+1] is %d <= %d \n", j,
			pbf->bsignificand[j + 1], canquit);
						(void) printf(" s[j+2..j] are \
			%d %d %d \n", pbf->bsignificand[j + 2],
			pbf->bsignificand[j + 1], pbf->bsignificand[j]);
						(void) printf(" requested \
			precision %d needed_precision %d big digits out of %d \
			\n", precision, needed_precision, lengthx + lengthp);
#endif
						pbf->bsignificand[excess] |= 1;
/* Sticky bit on. */
#ifdef DEBUG
						(void) printf(" discard %d \
					digits - last gets %d \n", excess,
					pbf->bsignificand[excess]);
#endif
...
#ifdef DEBUG
					/*
					 * else { (void) printf(" early
					 *		termination
					 * rejected at j %d since s[j+1] =
					 * %d, canquit = %d \n", j,
					 * pbf->bsignificand[j + 1],
					 * canquit); (void) printf(" s[j+2..j]
					 * are
					 * %d %d %d \n", pbf->bsignificand[j
					 * + 2], pbf->bsignificand[j + 1],
					 * pbf->bsignificand[j]); (void)
					 * printf("
		...
		donegeneral:
				...
				if (base == 10) {
					...
					if (deletelimit < 0)
						deletelimit = 0;
					if ((int)trailing_zeros_to_delete >
						deletelimit) {
                                                /*
...
#ifdef DEBUG
					printf("\n __x_power trailing \
					zeros delete count lowered from %d to \
					%d \n",
...
#ifdef DEBUG
					(void) printf(" %d trailing zeros \
					deleted \n", trailing_zeros_to_delete);
#endif
					__copy_big_float_digits(
						pbf->bsignificand,
						&(pbf->bsignificand
						[trailing_zeros_to_delete]),
						pbf->blength -
