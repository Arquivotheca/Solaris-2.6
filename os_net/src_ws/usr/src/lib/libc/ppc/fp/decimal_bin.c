...
#ident	"@(#)decimal_bin.c	1.6	94/09/10 SMI"
...
#ifdef DEBUG
			(void) printf(" __decimal_to_binary_integer: decimal \
				exponent %d too large for tables \n", nzout);
#endif
			pb->bexponent = 0x7fff;
...
/* Ifdef'ed out diagnostic message to keep tsort happy. */
			...
			if (pbout != pb)
				...
				large decimal exponent %d needs heap buffer \
				\n", nzout);
			...
			if (pbout != pb) {	/* We don't really need such */
				...
				pb->bexponent = pbout->bexponent + 16 *
					(pbout->blength - allweneed);
#ifdef DEBUG
				printf(" __decimal_to_binary_integer: removed \
				%d excess digits from product \n",
				pbout->blength - allweneed);
				__display_big_float(pb, 2);
#endif
...

{
	...
	if (bf.blength != 0) {	/* Combine integer and fraction bits. */
		int	expdiff = bi.bexponent -
					(bf.bexponent + 16 * (bf.blength - 1));
				/* Exponent difference. */
		...
		assert(bi.blength != 0);
		assert(bf.blength != 0);
		assert(bi.bsignificand[bi.blength - 1] != 0);
						/* Normalized bi. */
...

{
	...
	__get_ieee_flags(&fb);
	if ((pd->exponent <= __TBL_TENS_EXACT) &&
		(pd->exponent >= 1)) {	/* small positive exponent */
		...
		esum += (int) eround;
			...
			(-pd->exponent >= 1)) {	/* small negative exponent */
		dds = __digits_to_double(pd->ds, nds, &eround);
		if (eround != __fbe_none)
			NOTQUICKX('U');
		if (pd->sign == 1)
...

{
	...
	/* small positive exponent */
	else if ((pd->exponent <= -1) && (pd->exponent >= -__TBL_TENS_EXACT)) {
			/* small negative exponent */
		dds = __digits_to_double(pd->ds, nds, &eround);
		if (eround != __fbe_none)
			NOTQUICKX('Y');
