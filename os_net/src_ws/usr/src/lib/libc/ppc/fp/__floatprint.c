...
#ident	"@(#)__floatprint.c	1.6	96/03/21 SMI"
...
min_normal(void)
{
	...
	fp_limit.l[0] = 0;
	fp_limit.l[1] = 0x00100000;
#else
	fp_limit.l[0] = 0x00100000;
	fp_limit.l[1] = 0;
#endif
	return fp_limit.d;
}
...
#ifdef __LITTLE_ENDIAN
	fp_limit.l[0] = 1;
	fp_limit.l[1] = 0x7ff00000;
#else
	fp_limit.l[0] = 0x7ff00000;
	fp_limit.l[1] = 1;
#endif
	return fp_limit.d;
}
...
__arint_set_n(x, nrx, pe)
	...
	enum __fbe_type	*pe;
     ...
     /* 
      * Converts double to integral value rounding to nearest, 
      * returns result and exceptions.  nrx is the number of rounding errors
      * x already contains. *pe will get __fbe_many if the rounded value of x, 
      * rx,  is too close to the ambiguous case: 
      * nrx > 0 & nrx * ulp(rx) >= 0.5 - |x-rx| 
      */

{
	...
	case 1:
		*pe = (__abs_ulp(rx) < (0.5 - rmx)) ? __fbe_one : __fbe_many;
		break;
	default:
		*pe = ((2.0 * __abs_ulp(rx)) < (0.5 - rmx)) ? __fbe_one :
								__fbe_many;
		break;
	}
	return (rx);
