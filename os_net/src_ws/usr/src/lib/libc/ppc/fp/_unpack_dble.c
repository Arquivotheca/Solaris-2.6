...
#ident	"@(#)_unpack_dble.c	1.5	94/09/06 SMI"
	...
	if ((*pu).fpclass == fp_normal) {
		...
		if (nlzwords > 0) {
			for (i = 0; i < UNPACKED_SIZE - (int)nlzwords; i++)
				pu->significand[i] = pu->significand[i +
							nlzwords];
...
#define	SHIFTMACRO(n) if (t <= (((unsigned long) 0xffffffff) >> n)) \
			{ t = t << n; nlzbits += n; }
		SHIFTMACRO(16);
		SHIFTMACRO(8);
		SHIFTMACRO(4);
	...
	double		*px;	/* packed double */
{
	...
	if (x.f.msw.exponent == 0) {	/* zero or sub */
		if ((x.f.msw.significand == 0) &&
			(x.f.significand2 == 0)) {	/* zero */
			pu->fpclass = fp_zero;
			return;
		} else {	/* subnormal */
