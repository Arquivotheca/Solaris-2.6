...
static int
add_char(char *compilep,
    wchar_t wchar);

static int
add_single_char_expr(char *compilep,
    wchar_t wchar);
...
_get_vars_storage(thread_key_t *keyp)
{
    ...
    if (_thr_getspecific(*keyp, &varsp) != 0) {
...
regcmp(const char *regexp, ...)
{
     ...
     * two extra single-byte characters for each character in the expression,
     * so allocating three times the number of bytes in all the strings
     * that comprise the regular expression will ensure that regcmp()
     * won't overwrite the end of the allocated block when compiling the
     * expression.
     */
    ...
    for (;;) {
        ...
        switch (current_char) {
            ...
            /*
             * No fall-through.  Each case ends with either a break or an
             * error exit.  Each case starts with compilep addressing the
             * next location to be written in the compiled regular expression,
             * and with regexp addressing the next character to be read from
             * the regular expression being compiled.  Each case that
             * doesn't return increments regexp to address the next character
             * to be read from the regular expression and increments compilep
             * to address the next location to be written in the compiled
             * regular expression.
             *
             ...
             *       regular expression by the case.  Each single character
             ...
             *       single characters written to the compiled regular
        ...
        case DOLLAR_SIGN: /* end of string marker or simple dollar sign */

            /* compiles to <END_OF_STRING_MARK> or <ASCII_CHAR><'$'> */
        ...
        case BACKSLASH: /* escaped character */
            ...
            if (char_size <= 0) {
                ...
                regexp += char_size;
                can_repeat = B_TRUE;
                expr_length = add_single_char_expr(compilep, current_char);
                regex_typep = compilep;
                compilep += expr_length;
            }
            break; /* end case '\\' */

        case LEFT_SQUARE_BRACKET: /* start of a character class expression */
            ...
            /*
             * NOTE: This algorithm checks the value of the "multibyte"
             *       macro in <euc.h> (included in <widec.h> which is
             *       included in <wctype.h>) to find out if regcmp()
             *       is compiling the regular expression in a multibyte
             *       locale.
             */
            char_size = get_wchar(&current_char, regexp);
            if (char_size <= 0) {
                ...
                if (char_size <= 0) {
                    ...
                    regexp += char_size;
                    if (!multibyte) {
                        *compilep = (unsigned char)NOT_IN_ASCII_CHAR_CLASS;
                    } else {
                        *compilep = (unsigned char)NOT_IN_MULTIBYTE_CHAR_CLASS;
                    }
                    compilep += 2; /* leave space for <class_length> */
                }
            } else {
                regexp += char_size;
                if (!multibyte) {
                    *compilep = (unsigned char)IN_ASCII_CHAR_CLASS;
                } else {
                    *compilep = (unsigned char)IN_MULTIBYTE_CHAR_CLASS;
                }
                compilep += 2; /* leave space for <class_length> */
            }
            ...
            /* check for a leading right square bracket, which is allowed */

            if (current_char == RIGHT_SQUARE_BRACKET) {
                /*
                 * the leading RIGHT_SQUARE_BRACKET may be part
                 * of a character range expression like "[]-\]"
                 */
                ...
                /*
                 * decode the character in the following while loop
                 * and decide then if it can be the first character
            ...
            while (current_char != RIGHT_SQUARE_BRACKET) {
                if (current_char != DASH) {
                    /*
                     * if a DASH follows current_char, current_char,
                     * the DASH and the character that follows the DASH
                     * may form a character range expression
                     */
                    dash_indicates_range = B_TRUE;
                    first_char_in_range = current_char;
                    expr_length = add_char(compilep, current_char);
                    compilep += expr_length;

                } else if /* (current_char == DASH) && */
                    (dash_indicates_range == B_FALSE) {
                    /*
                     * current_char is a DASH, but either begins the entire
                     * character class or follows a character that's already
                     * part of a character range expression, so it simply
                     * represents the DASH character itself
                     */
                    *compilep = DASH;
                    compilep ++;
                    /*
                     * if another DASH follows this one, this DASH is part
                     * of a character range expression like "[--\]"
                     */
                    dash_indicates_range = B_TRUE;
                    first_char_in_range = current_char;
                           ...
                           (dash_indicates_range == B_TRUE)) */ {
                    /*
                     * the DASH appears after a single character that isn't
                     * already part of a character range expression, so it
                     * and the characters preceding and following it can
                     * form a character range expression like "[a-z]"
                     */
                    char_size = get_wchar(&current_char, regexp);
                    if (char_size <= 0) {
                    ...
                    } else if (current_char == RIGHT_SQUARE_BRACKET) {
                        /*
                         * the preceding DASH is the last character in the
                         * class and represents the DASH character itself
                         */
                        *compilep = DASH;
                        compilep++;

                    } else if (valid_range(first_char_in_range, current_char)
                               == B_FALSE) {
                        ...
                        /*
                         * the DASH is part of a character range expression;
                         * encode the rest of the expression
                         */
                        regexp += char_size;
                        *compilep = (unsigned char)THRU;
                        compilep++;
                        expr_length = add_char(compilep, current_char);
                        compilep += expr_length;
                        /*
                         * if a DASH follows this character range expression,
                         * it represents the DASH character itself
                         */
                        dash_indicates_range = B_FALSE;
                    }
                }
            ...
            /* INSERT THE LENGTH OF THE CLASS INTO THE COMPILED EXPRESSION */

            class_length = (unsigned int)(compilep - regex_typep - 1);
            if ((class_length < 2) || (class_length > MAX_SINGLE_BYTE_INT)) {
                ERROR_EXIT(&regcmp_lock, arg_listp, compile_startp);
            } else {
                *(regex_typep + 1) = (unsigned char)class_length;
            }
        ...
        case LEFT_PAREN:
            ...
            /*
             * start of a parenthesized group of regular expressions
             *
             * compiles to <'\0'><'\0'>, leaving space in the compiled
            ...
            if (push_compilep(compilep) == (char *)0) {
               /*
                * groups can contain groups, so group start pointers
        ...
        case RIGHT_PAREN: /* end of a marked group of regular expressions */
             ...
             * (<regex>)* compiles to
             *    <ZERO_OR_MORE_GROUP|ADDED_LENGTH_BITS><group_length>\
             *    <compiled_regex...><END_GROUP|ZERO_OR_MORE><groupn>
             * (<regex>)+ compiles to
             *    <ONE_OR_MORE_GROUP|ADDED_LENGTH_BITS><group_length>\
             *    <compiled_regex...><END_GROUP|ONE_OR_MORE><groupn>
             * (<regex>){...} compiles to
             ...
             *    <SIMPLE_GROUP><blank><compiled_regex...><END_GROUP><groupn>
             *
             * NOTE:
             *
             * group_length + (256 * ADDED_LENGTH_BITS) ==
             *     length_of(<compiled_regex...><END_GROUP|...><groupn>)
             * which also ==
             *     length_of(<group_type|ADDED_LENGTH_BITS><group_length>\
             *         <compiled_regex...>)
             *
             *       groupn no longer seems to be used, but the code
             *       still computes it to preserve backward compatibility
            ...
            if (char_size < 0) {
                ...
                *regex_typep = (char)substringn;
                ...
                if ((return_arg_number < 0) || (substringn >= NSUBSTRINGS)) {
                    ERROR_EXIT(&regcmp_lock, arg_listp, compile_startp);
		}
                regexp++;
                ...
                *compilep = (unsigned char)return_arg_number;
                ...
                if (*regex_typep != SIMPLE_GROUP) {
                    group_length = (unsigned int)(compilep - regex_typep);
                    if (group_length >= 1024) {
                        ERROR_EXIT(&regcmp_lock, arg_listp, compile_startp);
		    }
                    high_bits = group_length >> TIMES_256_SHIFT;
                    low_bits = group_length & SINGLE_BYTE_MASK;
                    *regex_typep =
                        (unsigned char)((unsigned int)*regex_typep | high_bits);
                    regex_typep++;
                    *regex_typep = (unsigned char)low_bits;
		}
                can_repeat = B_TRUE;
                *compilep = (unsigned char)END_GROUP;
                regex_typep = compilep;
        ...
        case STAR: /* zero or more repetitions of the preceding expression */

            /*
             * <regex...>* compiles to <regex_type|ZERO_OR_MORE>\
             *                         <compiled_regex...>
             *
             * (<regex...>)* compiles to <ZERO_OR_MORE_GROUP|ADDED_LENGTH_BITS>\
            ...
            if (can_repeat == B_FALSE) {
                ...
                can_repeat = B_FALSE;
                *regex_typep =
                    (unsigned char)((unsigned int)*regex_typep | ZERO_OR_MORE);
            }
            break; /* end case '*' */

        case PLUS: /* one or more repetitions of the preceding expression */

            /*
             * <regex...>+ compiles to <regex_type|ONE_OR_MORE>\
             *                         <compiled_regex...>
             * (<regex...>)+ compiles to <ONE_OR_MORE_GROUP|ADDED_LENGTH_BITS>\
             *                           <group_length><compiled_regex...>\
            ...
            if (can_repeat == B_FALSE) {
                ...
                can_repeat = B_FALSE;
                *regex_typep =
                    (unsigned char)((unsigned int)*regex_typep | ONE_OR_MORE);
            }
            break; /* end case '+' */
        ...
        case LEFT_CURLY_BRACE:
            ...
            /*
             * repeat the preceding regular expression at least min_count times
             * and at most max_count times
             *
             * <regex...>{min_count} compiles to
             *     <regex type|COUNT><compiled_regex...><min_count><min_count>
             *
             * <regex...>{min_count,} compiles to
             *     <regex type|COUNT><compiled_regex...><min_count><UNLIMITED>
             *
             * <regex...>{min_count,max_count} compiles to
             *     <regex type>|COUNT><compiled_regex...><min_count><max_count>
             *
             * (<regex...>){min_count,max_count} compiles to
             *     <COUNTED_GROUP|ADDED_LENGTH_BITS><group_length>\
             *     <compiled_regex...><END_GROUP|COUNT><groupn>\
            ...
            can_repeat = B_FALSE;
            *regex_typep = (unsigned char)((unsigned int)*regex_typep | COUNT);
            ...
            if (*regexp == RIGHT_CURLY_BRACE) {      /* {min_count} */
                ...
                if (*regexp == RIGHT_CURLY_BRACE) {  /* {min_count,}   */
                    ...
                    count_length = get_count(&max_count, regexp);
                    if (count_length <= 0) {
                        ERROR_EXIT(&regcmp_lock, arg_listp, compile_startp);
		    }
                    regexp += count_length;
            ...
            if ((min_count > MAX_SINGLE_BYTE_INT) ||
                ((max_count != UNLIMITED) && (min_count > max_count))) {
                ERROR_EXIT(&regcmp_lock, arg_listp, compile_startp);
            } else {
        ...
        if (char_size < 0) {
            ...
            if (pop_compilep() != (char *)0) {    /* unmatched parentheses */
                ERROR_EXIT(&regcmp_lock, arg_listp, compile_startp);
            }
            *compilep = (unsigned char)END_REGEX;
            compilep++;
    ...
    wchar_t upper_char)
{
    return (((lower_char <= 0x7f) && (upper_char <= 0x7f) &&
              !iscntrl(lower_char) && !iscntrl(upper_char) &&
              (lower_char < upper_char))
              ||
             (((lower_char & WCHAR_CSMASK) == (upper_char & WCHAR_CSMASK)) &&
              (lower_char < upper_char)));
}
