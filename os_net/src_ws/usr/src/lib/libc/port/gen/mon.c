...
#ident	"@(#)mon.c	1.13	96/01/30 SMI"	/* SVr4.0 2.24	*/
/*	3.0 SID #	1.2	*/
/*LINTLIBRARY*/
/*
 ...
 */
void
monitor(int (*alowpc)(), int (*ahighpc)(), WORD *buffer, int bufsize, int nfunc)
{
	int scale;
	long text;
	register char *s;
	...
	ssiz = (sizeof (struct hdr) + nfunc *
	    sizeof (struct cnt)) / sizeof (WORD);
