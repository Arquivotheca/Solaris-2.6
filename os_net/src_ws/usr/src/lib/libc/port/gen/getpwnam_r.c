...
#pragma	ident	"@(#)getpwnam_r.c	1.16	96/09/20 SMI"
    ...
    size_t bufsize, struct passwd **result)
{
	...
	errno = 0;
	if ((*result = _getpwuid_r(uid, pwd, buffer, (int)bufsize)) == NULL) {
		if (errno == 0)
			nerrno = EINVAL;
		else
			nerrno = errno;
    ...
    size_t bufsize, struct passwd **result)
{
	...
	errno = 0;
	if ((*result = _getpwnam_r(name, pwd, buffer, (int)bufsize)) == NULL) {
		if (errno == 0)
			nerrno = EINVAL;
		else
			nerrno = errno;
