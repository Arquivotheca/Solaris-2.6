...
#ident	"@(#)getspent_r.c	1.4	92/10/09	SMI"
...
struct spwd *
getspnam_r(name, result, buffer, buflen)
	const char	*name;
	struct spwd	*result;
	char		*buffer;
	int		buflen;
{
	...
	arg.key.name = name;
	nss_search(&db_root, _nss_initf_shadow, NSS_DBOP_SHADOW_BYNAME, &arg);
...
struct spwd *
getspent_r(result, buffer, buflen)
	struct spwd	*result;
	char		*buffer;
	int		buflen;
{
	...
	do {
		NSS_XbyY_INIT(&arg, result, buffer, buflen, str2spwd);
		/* No key to fill in */
		nss_getent(&db_root, _nss_initf_shadow, &context, &arg);
...
struct spwd *
fgetspent_r(f, result, buffer, buflen)
	FILE		*f;
	struct spwd	*result;
	char		*buffer;
	int		buflen;
