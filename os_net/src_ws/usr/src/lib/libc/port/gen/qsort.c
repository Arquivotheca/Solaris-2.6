...
#ident	"@(#)qsort.c	1.10	95/10/16 SMI"	/* SVr4.0 1.9	*/
...
static void qst(char *, char *, int, int (*)(), int, int);
...
qsort(void *base, size_t n, size_t qsz, int (*qcmp)())
{
	...
	if (j != base) {
		/* swap j into place */
		for (i = (char *)base, hi = (char *)base + qsz; i < hi; /* CSTYLED */) {
			c = *j;
			*j++ = *i;
			*i++ = c;
		}
...
static void
qst(char *base, char *max, int qsz, int (*qcmp)(), int thresh, int mthresh)
{
	char c, *i, *j, *jj;
	int ii;
	char *mid, *tmp;
	int lo, hi;
	int aligned = ((int)base & (sizeof(int) - 1)) == 0 &&
				((qsz & (sizeof(int) - 1)) == 0);
