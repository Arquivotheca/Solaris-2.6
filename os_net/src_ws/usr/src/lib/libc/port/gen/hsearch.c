...
#ident	"@(#)hsearch.c	1.11	93/10/01 SMI"	/* SVr4.0 1.11	*/
...
#define RETURN(n)    { _mutex_unlock(&table_lock); return (n); }
#else
...
size_t size;		/* Minimum size for hash table */
{
    unsigned int unsize;	/* Holds the shifted size */

    _mutex_lock(&table_lock);
    if(size <= 0){
        _mutex_unlock(&table_lock);
	return(FALSE);
    }
