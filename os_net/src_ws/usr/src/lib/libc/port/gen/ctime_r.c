...
#ident	"@(#)ctime_r.c	1.2	95/08/24 SMI"	/* SVr4.0 1.14	*/
 ...
 */
char *
_ctime_r(const time_t *t, char *buffer, int buflen)
{
	struct tm res;

	if (_localtime_r(t, &res) == NULL)
		return (NULL);

	if (_asctime_r(&res, buffer, buflen) == NULL)
		return (NULL);
