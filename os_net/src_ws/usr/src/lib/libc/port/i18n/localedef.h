...
#pragma ident	"@(#)localedef.h	1.17	96/08/21 SMI"
...
**
*/
#define	_COLL_WEIGHTS_INLINE	(sizeof (wchar_t *)/sizeof (wchar_t))

typedef union {
	wchar_t	n[_COLL_WEIGHTS_INLINE];
		/* This type is used if the number of */
		/* orders can fit into this space */

	_const wchar_t	*p;
		/* This type is used if the number of */
		/* collation orders won't fit */
} _LC_weight_t;
...
typedef struct {
	...
	_const _LC_collel_t	*ct_collel;
					/* Pointer to collation symbol array */
} _LC_coltbl_t;
...
struct _LC_ctype_t {
	...
	_const wchar_t	**transtabs; 	/* pointer to transtabs array */
