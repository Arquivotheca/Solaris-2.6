...
#pragma ident "@(#)getnetgrent.c	1.11	93/12/16 SMI"
	...
	nss_status_t		*statusp;
{
	...
	while (nname > 1) {
		...
		if (easy_way(be, ia, &just_one, try_lc, statusp) &&
		    ia->status == NSS_NETGR_FOUND) {
			return (1);
		}
		++pname;
...
/* ====> diddle this to try fast path for domains.argc == 0 too */
	...
	ia->status = NSS_NETGR_NO;
	res = top_down(be, ia->groups.argv, ia->groups.argc, match_triple, ia);
	return (res);
}
