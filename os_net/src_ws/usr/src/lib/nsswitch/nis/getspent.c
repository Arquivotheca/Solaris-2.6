...
#pragma ident "@(#)getspent.c	1.10	93/09/03	SMI"
	...
	void			*a;
{
	...
	nss_status_t		res;
	struct spwd 		*spwd;
	char			key_name[NSS_BUFLEN_SHADOW];
				/*
				 * this is of size NSS_BUFLEN_SHADOW
				 * just to make sure that it can
				 * hold the the longest username
				 * allowed by the result buffer.
				 */
	...
	if ((res == NSS_SUCCESS) && (spwd->sp_pwdp) &&
	    (*(spwd->sp_pwdp) == '#') && (*(spwd->sp_pwdp + 1) == '#'))
			/* get password from passwd.adjunct.byname */
		(void) strcpy(key_name, spwd->sp_namp);
		res = _nss_nis_lookup_rsvdport(be, argp, 0,
					"passwd.adjunct.byname",
					key_name, 0);
	argp->str2ent	= save_c2e;
