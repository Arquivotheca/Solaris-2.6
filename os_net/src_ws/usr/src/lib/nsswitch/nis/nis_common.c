...
#pragma ident	"@(#)nis_common.c	1.11	93/09/03 SMI"
	...
	int			netdb;
{
	...
	if ((res = switch_err(ypstatus)) != NSS_SUCCESS) {
		return (res);
	}
	...
	if (netdb) {
		massage_netdb(&outval, &outvallen);
	}
