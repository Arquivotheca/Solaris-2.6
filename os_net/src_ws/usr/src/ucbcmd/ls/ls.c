...
#pragma ident	"@(#)ls.c	1.12	96/08/27 SMI"
...
/* Copyright (c) 1984,1985,1986,1987,1988,1989,1996, by Sun Microsystems, Inc.
 * All Rights Reserved.
 */
...
gstat(fp, file, statarg, pnb)
	...
	char *file;
	int statarg;
	off_t *pnb;	/* (int *)0 if file is ISARG */
{
	...
	register struct afile *p;
{
	...
	(void) sprintf(inumbuf, "%6llu ", p->fnum);
	return (inumbuf);
}
