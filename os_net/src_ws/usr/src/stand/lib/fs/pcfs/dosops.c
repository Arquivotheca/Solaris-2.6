...
#pragma ident "@(#)dosops.c 1.22       96/09/19 SMI"
...
#endif	/* DEBUG */
static char *vollst(char *srcvol);
...
extern int  isfloppy(), isfloppy0(), isfloppy1();
...
arch_prom_open(int v, char *path)
{
	if ((_device_fd[v] = prom_open(path)) == 0) {
		DPrint(DBG_ERR, ("Can't open device\n"));
		return (-1);
	} else
		return (0);
...
arch_fix_spt(int v, _fdisk_p fp)
{
	...
	if (prom_read(_device_fd[v], (caddr_t)pi[v]->f_sector,
		SECSIZ, 0, 0) != SECSIZ) {
		return (-1);
	}

	flop_spt = ltohs(pi[v]->f_bpb.bs_sectors_per_track);
	flop_spc = flop_spt * ltohs(pi[v]->f_bpb.bs_heads);
	flop_endblk = ltohs(pi[v]->f_bpb.bs_siv) ?
	    ltohs(pi[v]->f_bpb.bs_siv) : ltohl(pi[v]->f_bpb.bs_lsiv);
	diskette_ivc();
...
fat_init(int volidx, char *path, int type)
{
	...
	pi[volidx]->f_adjust = ltohl(fd.fd_start_sec);
	...
	pi[volidx]->f_ncluster =
		...
		(u_long)ltohs(pi[volidx]->f_bpb.bs_siv) :
		(u_long)ltohl(pi[volidx]->f_bpb.bs_lsiv)) -
		pi[volidx]->f_filesec) /
...
vollst(char *srcvol)
{
	...
	if (!volname) {
		...
		for (ct = 0; ct < VOLLABELSIZE && srcvol[ct] != ':'; ct++)
			volname[ct] = srcvol[ct];
		for (; ct < VOLLABELSIZE; ct++)
			volname[ct] = ' ';
		volname[VOLLABELSIZE] = ':';
		volname[VOLLABELSIZE+1] = '\0';
...
int
boot_pcfs_read(int fd, caddr_t b, int c)
{
	u_long		sector;
...
/*
 *  Extract info from DOS directory entry:
 *
 *  This routine is used by boot_pcfs_getdents to convert a DOS directory entry
 *  into a generic "dirent" struct.  Arguments are the DOS file "name"
 *  and "ext"ension, and a pointer to a "dents_info" struct that tells
 *  us where to put the reformatted directory entry.
 *
 *  Returns 1 if it works (to keep "searchdir" happy), -1 if there's no
 *  more room in the output buffer.
 */
static int
dirExtract(void *p, char *name, char *ext)
{
	char *np;
	int j, k, n;
	struct dents_info *dip = (struct dents_info *)p;
#define	SLOP (sizeof (struct dirent) - (int)&((struct dirent *)0)->d_name[1])

	/*
	 * Calculate name length.  "j" register holds length of the name field,
	 * "k" register length of the extension, and "n" the length of the con-
	 * catenated pair (which includes the dot).  The total length of the
	 * output "dirent" struct is then calculted by adding the length of the
	 * fixed overhead to "n" and rounding to an appropriate alignment.
	 *
	 * NOTE: "SLOP" is the amount of space at the end of the "dirent"
	 * struct's "d_name" field that was inserted by the compiler
	 * to preserve alignment.
	 */

	for (j = 0; (name[j] != ' ') && (j < NAMESIZ); j++);
	for (k = 0; (ext[k] != ' ') && (k < EXTSIZ); k++);
	n = j + k + (k > 0);
	...
	if (n <= dip->size) {
		/*
		 * There's room in the output buffer for the next reformatted
		 * directory entry.  The values stored in the "d_ino" and
		 * "d_off" fields are somewhat arbitrary, the only requirement
		 * being that they uniquely identify the directory and entry,
		 * respectively. We use the address of the directory's first
		 * sector for the former, the entry number for the latter.
		 */

		dip->cnt = (dip->cnt < 0) ? 1 : (dip->cnt + 1);
		dip->dep->d_ino = dip->inum;
		dip->dep->d_off = dip->cnt;
		dip->dep->d_reclen = (u_short)n;
		np = dip->dep->d_name;
		dip->size -= n;

		while (j-- > 0) { *np++ = tolower(*name++); }
		if (k) {
			*np++ = '.';
			while (k--)
				*np++ = tolower(*ext++);
		...
		*np = '\0';
		...
		dip->dep = (struct dirent *)((char *)dip->dep + n);
		return (1);
	}
...
{
	_file_desc_p fp;
	struct dents_info di;
	int volidx;

	DPrint(DBG_VOL, ("dosGetdents:"));
	...
	fp->f_volidx = volidx;
	searchdir(fp->f_volidx, di.inum, dirExtract, (void *)&di);
	return (di.cnt);
}
...
static void
ftruncate(_file_desc_p f)
{
	int	c;		/* ---- current cluster being freed */
	int	cn;		/* ---- next cluster in chain */

	DPrint(DBG_GEN, ("(ftruncate len %d, clust %d)",
...
loadVolume(char *path, char **volnm, char **newfn, int volhint)
{
	...
	DPrint(DBG_VOL, ("loadVolume:[%s]:", path ? path : "Empty Path"));

	if (path2VolnPath(path, &needvol, &needfn) < 0)
		return (-1);

	DPrint(DBG_VOL, ("Need volume %s containing %s.\n",
	    needvol ? needvol : "NONE?",
	    needfn ? needfn : "NONE?"));
	...
	if (!*needvol || (bootvolume && VOLEQ(needvol, bootvolume)) ||
