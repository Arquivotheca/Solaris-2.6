...
#pragma ident "@(#)machdep.c	1.17	96/10/25 SMI"
...
/*
 * Called from mlsetup() in the kernel to handle kadb -d.  Also, provide
 * a way for cpr to access the words which kadb defines to the prom and
 * to arm the prom to use the words.
 */
void
scbsync(int cmd, void *arg)
{
	switch (cmd) {
	case KADB_CALLB_TRAP:
		if (scbstop) {
			/*
		...
		break;
	case KADB_CALLB_FORMAT:
		*(caddr_t *)arg = format_prom_callback();
		break;
	case KADB_CALLB_ARM:
		arm_prom_callback();
		break;
...
idle_other_cpus(void)
{
	...
	for (i = 0; i < NCPU; i++) {
		if (i != cpuid && cpu_nodeid[i] != -1) {
			...
			kadb_send_mondo(i, (func_t)&save_cpu_state,
...
resume_other_cpus(void)
{
	...
	for (i = 0; i < NCPU; i++) {
		if (i != cpuid && cpu_nodeid[i] != -1) {
			...
			kadb_send_mondo(i, (func_t)&restore_cpu_state,
			    (u_int)&cpusave[i], (u_int)&mpfpregs[i],
			    (u_int)0, (u_int)0);
		}
	}
...
switch_cpu(int to)
{
	...
	kadb_send_mondo(to, (func_t)&start_cmd_loop,
...
set_brkpt(caddr_t kva, int val)
{
	...
	if (prom_translate_virt(addr, &valid, &physaddr,
	    &mode) != 0 || valid != -1) {
	...
	if (prom_map_phys(mode, vac_size, bkptbuf, physaddr) != 0) {
		prom_panic("do_bkpt: translate_virt() failure");
	}
	*(u_int *)(bkptbuf + (int)((u_int)kva % vac_size)) = val;

	sf_iflush(bkptbuf + (int)((int)kva % vac_size));

	prom_unmap_virt(vac_size, bkptbuf);
}
