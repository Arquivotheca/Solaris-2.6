...
#pragma ident	"@(#)readfile_ppc.c	1.10	96/10/15 SMI"
	...
	char *name;
{
	...
	for (i = 0; i < nphdrs; i++) {
		...
		if (phdr->p_type == PT_LOAD) {
			...
			if (phdr->p_flags & PF_X)
				sync_instruction_memory(phdr->p_vaddr,
				    phdr->p_memsz);
	...
	if (interp) {
		 ...
		 * RB_DEBUG should mimic RB_KRTLD which will
	...
	auxv_t **avp)
{
	...
	for (i = 1; i < ehdr->e_shnum; i++) {
		...
		if (sp->sh_flags & SHF_EXECINSTR)
			sync_instruction_memory(sp->sh_addr, sp->sh_size);
	}
