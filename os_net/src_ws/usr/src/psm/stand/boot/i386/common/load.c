...
#ident "@(#)load.c	1.23	96/04/08 SMI"
...
extern int open(), close(), read(), fstat();
extern long lseek();
...
static  int	load_reloc(int fd, struct dos_exehdr *hdrp,
    caddr_t *reclobufp, int *reclobufsizep);
...
loadnrun_elf(char *elfpath, int argc, char **argv)
{
	...
	/* Create delayed-write property for environment */
	saveenv();
