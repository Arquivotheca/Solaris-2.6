...
#ident "@(#)memory.c	1.12	96/04/08 SMI"
...
extern void use_2lb_pgtbls();
extern void restore_pgtbls();
...
rm_check()
{
	 ...
	 */
	static int rm_check_warn = 1;	/* XXX turn off for production! */
...
void
rm_free(caddr_t virt, u_int size)
{
	/*
...
caddr_t
rm_malloc(u_int size, u_int align, caddr_t virt)
{
	/* BEGIN CSTYLED */
	/*
	...
	for (hp = (struct freenode *)&free_realmem;
		 ...
		 *  list is maintained physical address sequence.
		 */
...
printf("rm_alloc: allocating from the top!!\n");
...
int *freeseg;
{
...
#ifdef BOOT_DEBUG
	printf("find_mem: size 0x%x align 0x%x freeseg addr 0x%x\n",
		size, align, freeseg);
#endif
	...
	for (availseg = 0; availseg < bip->memavailcnt; availseg++) {
		...
		if (size > bip->memavail[availseg].extent)
			continue;
		loadaddr = bip->memavail[availseg].base;
		break;
	}
