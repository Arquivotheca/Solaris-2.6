...
getprop_cmd(int argc, char **argv)
{
	...
	if (argc >= 2) {
		 ...
		 */
		int length = bgetproplen(bop, argv[1], active_node);
...
setbin_cmd(int argc, char **argv)
{
	...
	if (argc == 3) {
		...
		if ((n = get_bin_prop(argv[2], &buf, "setbinprop")) > 0) {
			 ...
			 */
			(void) bsetprop(bop, argv[1], buf, n, active_node);
			bkmem_free(buf, n);
		}
	} else {
		/*
...
props_cmd(int argc, char **argv)
{
	...
	while (bnextprop(bop, buf, name_buf, active_node) > 0) {
		...
		if ((len > 0) && (buf = bkmem_alloc(len))) {
			 ...
			 * appropriate value.
			 */
			(void) bgetprop(bop, name_buf, buf, len, active_node);
			print_prop(buf, len);
			bkmem_free(buf, len);
		}
