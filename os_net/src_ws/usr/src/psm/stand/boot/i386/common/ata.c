...
#pragma ident	"@(#)ata.c	1.16	96/08/15 SMI"
...
static void ata_not_present(ushort ioaddr);
...
#define	EXP	(ATS_DRDY|ATS_DSC)
	...
	(void) inb(ioaddr+0x8);
	drive = inb(ioaddr+0x8);
	if (drive != 0xff && drive != 0x0 && inb(ioaddr+0x8) == drive) {
		ata_not_present(ioaddr);
		return;
	}
...
static void
ata_not_present(ushort ioaddr)
{
	char	configname[35];

#ifdef BOOT_DEBUG
	if (btep->db_flag & BOOTTALK)
		printf("SUNW-ata-not-present.\n");
#endif
	(void) sprintf(configname, "SUNW-ata-not-present-%x", ioaddr);
	(void) bsetprop(bop, configname, "1", 0, 0);
}
