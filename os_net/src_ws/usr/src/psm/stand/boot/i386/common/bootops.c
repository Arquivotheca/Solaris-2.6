...
#ident "@(#)bootops.c	1.30	96/06/28 SMI"
...
extern void rm_free(caddr_t virt, u_int size);
...
struct bootcode {
	...
	{'a',   RB_ASKNAME},	{'s',   RB_SINGLE},
	{'i',   RB_INITNAME},	{'h',   RB_HALT},
	{'b',   RB_NOBOOTRC},	{'d',   RB_DEBUG},
	{'w',   RB_WRITABLE},	{'G',	RB_GDB},
	{'c',	 RB_CONFIG},	{'r',	RB_RECONFIG},
	{'v',	 RB_VERBOSE},	{'f',	RB_FLUSHCACHE},
	{0, 0}
};
...
bkern_read(struct bootops *bop, int fd, caddr_t buf, u_int size)
{
	...
	extern int kern_read(int, caddr_t, u_int);
...
bkern_lseek(struct bootops *bop, int fd, off_t hi, off_t lo)
{
	...
	extern off_t kern_lseek(int, off_t, off_t);
...
/*ARGSUSED*/
static int
bkern_getdents(struct bootops *bop, int fd, struct dirent *buf, u_int size)
{
...
bkern_fstat(struct bootops *bop, int fd, struct stat *buf)
{
	...
	extern int kern_fstat(int, struct stat *);
 ...
 */
static caddr_t
bkern_resalloc(struct bootops *bop, caddr_t virt, u_int size, int align,
    int flags);

static caddr_t
bkern_oldalloc(struct bootops *bop, caddr_t virt, u_int size, int align)
{
...
bootflags(register char *cp)
{
	...
	char *save_cp = cp;
	static char tmp_impl_arch[MAX1275NAME];
