#ident	"@(#)heap_kmem.c	1.28	96/03/22 SMI" /* From SunOS 4.1 1.10 */
...
caddr_t		kmem_alloc(u_int);
void		kmem_free(caddr_t ptr, u_int nbytes);
Freehdr		getfreehdr(void);
bool		morecore(u_int);
void		insert(Dblk p, u_int len, Freehdr *tree);
void		freehdr(Freehdr p);
void		delete(Freehdr *p);
static void	check_need_to_free(void);
extern caddr_t	resalloc(enum RESOURCES, unsigned int, caddr_t, int);
...
void
insert(register Dblk p,		/* Ptr to the block to insert */
...
caddr_t
kmem_alloc(register u_int nbytes)
{
 ...
 */
void
kmem_free(caddr_t ptr, register u_int nbytes)
{
