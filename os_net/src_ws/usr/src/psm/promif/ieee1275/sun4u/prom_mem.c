...
#pragma ident	"@(#)prom_mem.c	1.4	95/10/25 SMI"
 ...
 */
int
prom_allocate_phys(u_int size, u_int align, unsigned long long *physaddr)
{
	cell_t ci[10];
	int rv;
 ...
 */
int
prom_claim_phys(u_int size, unsigned long long physaddr)
{
	cell_t ci[10];
	int rv;
	ihandle_t imemory = prom_memory_ihandle();
 ...
 */
void
prom_free_phys(u_int size, unsigned long long physaddr)
{
	cell_t ci[8];
	ihandle_t imemory = prom_memory_ihandle();
