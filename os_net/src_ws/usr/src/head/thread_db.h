...
#pragma ident	"@(#)thread_db.h	1.15	96/10/15 SMI"
...
/* Event set manipulation macros -- assumes 32-bit u_longs. */
#define	__td_eventmask(n)	((unsigned int)1 << (((n) - 1) & (32 - 1)))
...
typedef struct td_thrinfo {
	...
	paddr_t		ti_startfunc;	/* startfunc passed to thr_create() */
	paddr_t		ti_stkbase;	/* base of thread's stack */
	int		ti_stksize;	/* size of thread's stack */
	paddr_t		ti_ro_area;	/* address of uthread_t struct */
	...
	int		ti_pc;		/* resume PC when sleeping */
	int		ti_sp;		/* resume SP when sleeping */
...
typedef struct td_syncinfo {
	...
	paddr_t		si_sv_addr;	/* address of sync. object */
 ...
 */
td_err_e
td_ta_new(const struct ps_prochandle *, td_thragent_t **);
 ...
 */
td_err_e
td_ta_map_addr2thr(const td_thragent_t *, paddr_t, td_thrhandle_t *);
 ...
 */
td_err_e
td_ta_map_addr2sync(const td_thragent_t *, paddr_t, td_synchandle_t *);
 ...
 */
td_err_e
td_thr_getxregs(const td_thrhandle_t *th_p, prxregset_t *xregs);

/*
 * Set a thread's extra state register set.
 */
td_err_e
td_thr_setxregs(const td_thrhandle_t *th_p, const prxregset_t *xregs);
#endif /* sparc */
