...
#pragma ident  "@(#)dsr.c 1.2     96/06/11 SMI"
...
static void	make_an_entry(char *, const char *, nms_t **);
...
rummage_dev_dsk(void)
{
	...
	if (dskp != (DIR *) 0) {
		...
		while ((bpt = readdir(dskp)) != NULL) {
			...
			(void) sprintf(dnmbuf, "/dev/dsk/%s", bpt->d_name);
...
static void
make_an_entry(char *lname, const char *shortnm, nms_t **list)
{
	nms_t	*entry;
...
{
	while (list) {
		insert_dlist_ent(list->dsk, list->dnum, list->dty, hd);
		list = list->next;
	}
}
/*
...
lookup_ks_name(char *dev_nm, void *val)
{
	...
	*tmpnm = '\0';
	...
	strncpy(cmpbuf, device, len);
	cmpbuf[len] = '\0';

#ifdef i386
	if (strcmp(cmpbuf, "sd") == 0)
		strcpy(cmpbuf, "cmdk");
#endif
