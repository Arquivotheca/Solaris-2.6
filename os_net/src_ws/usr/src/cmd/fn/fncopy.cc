...
#pragma ident	"@(#)fncopy.cc	1.9	96/04/15 SMI"
 ...
 *
 * FNSP_change_binding_ownership()
 * FNSP_change_context_ownership()
 * FNSP_orgname_of()
 * FNSP_nis_orgname_of()
 *
 */
...
create_fnsp_contexts(FN_ctx *ctx, FNSP_ctx_info *ctx_info)
{
	...
	FN_ref *ref = 0;
	FN_ctx *sub_ctx;
	FN_string *path_str, *owner;
	FN_string *domainname, *hu_name;
	FN_ref *parent_ref;
	void *ip;
	...
	if (ctx_info->type == context) {
		...
		// If user context or host context change permissions
		if ((ctx_info->ctx_type = FNSP_host_context ||
		    ctx_info->ctx_type == FNSP_user_context) &&
		    (destination_ns == FNSP_nisplus_ns)) {
			domainname = get_domain_name_from_ref(ref);
			hu_name = ctx_info->name->string();
			owner = new FN_string(0, hu_name, &NISPLUS_separator,
			    domainname, 0);
			parent_ref = ctx->get_ref(status);
			FNSP_change_context_ownership(*ref, *owner);
			...
			delete hu_name;
			delete owner;
		} else if ((ctx_info->ctx_type == FNSP_user_context) &&
		    (destination_ns == FNSP_files_ns)) {
			hu_name = ctx_info->name->string();
			FNSP_files_change_user_ownership(
			    (char *) hu_name->str());
			delete hu_name;
		}
