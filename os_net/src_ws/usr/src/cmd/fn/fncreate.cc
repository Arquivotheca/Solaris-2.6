...
#pragma ident	"@(#)fncreate.cc	1.27	96/10/24 SMI"
...
FNSP_change_user_ownership(char *username)
{
	...
	if (owner_passwd == NULL) {
		...
		fprintf(stderr, gettext("Permissions for the user context "
		    "%s not changed\n"), username);
		fprintf(stderr, gettext("Use chmod to change the ownership "
		    ...
		    int &how_many)
{
	...
	// No need to change ownership for NIS and files
	if (name_service != FNSP_nisplus_ns)
		return (host_ref);
	...
	if (can_hostname.compare(alias_hostname,
		...
		if (status.is_success()) {
			if (fnsbind(ctx, alias_fullname, alias_hostname_nsid,
				...
				// canonical host, only for NISPLUS
				if (name_service == FNSP_nisplus_ns) {
	...
	int &how_many)
{
	...
	if (user_ref && (how_many_before < how_many)) {
		if (name_service == FNSP_files_ns) {
			...
			FN_string owner(0, &user_name, &NISPLUS_separator,
			    &domain_name, 0);

			// note: service subcontext need not be
			// changed explicitly
			// update as side effect of updating nsid context
