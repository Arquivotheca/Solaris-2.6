...
#pragma ident	"@(#)fncreate_printer.cc	1.13	96/06/27 SMI"
    ...
    const FN_ref *default_printer)
{
	...
	if (!status.is_success())
		...
		// Try to unbind, if success create the printer context
		if ((service_ctx->unbind(printer_comp, status)) ||
		    (status.is_success()))
			create_printer_context = 1;
	}
    ...
    FN_ref *printer_ref)
{
	...
	if ((answer) && (supercede)) {
		// Delete the current printer context and create
		// the new one
		if (verbose)
			fprintf(stdout, gettext("Printer: %s already"
			    ...
			    printer_name->str());
		ctx->destroy_subcontext(*printer_name, status);
		if (status.is_success())
			answer = 0;
	} else if (verbose)
		fprintf(stdout,
	...
	if (!answer) {
		...
		if (!supercede)
			fprintf(stdout,
			    gettext("Printer: %s already exits\n"),
			    printer_name->str());
		if (verbose)
			fprintf(stdout,
		...
		while (prt_address) {
			...
			if (!ref_address) {
				...
				fprintf(stdout, gettext("New printer"
				    " address appended to the printer name\n"));
			}
    ...
    FN_ref *printer_object_ref)
{
	...
	while (prt_comp_string) {
		if (prt_comp_string_next) {
			...
			delete printer_ctx;
			printer_ctx =
			    FN_ctx::from_ref(*prt_ref, AUTHORITATIVE, status);
			check_error(status,
			    gettext("Unable to obtain printer context"));
			delete prt_ref;
		} else {
    ...
    FN_ref *printer_object_ref)
{
	...
	prt_comp_string = printer_comp.first(ip);
	if (prt_comp_string) {
		printer_ctx = FN_ctx::from_ref(*answer, AUTHORITATIVE, status);
		check_error(status,
		    gettext("Unable to obtain printer name context"));
		delete answer;
	}
				...
				if (!printer_ctx) {
					...
					printer_ctx = FN_ctx::from_ref(
					    *printer_ref,
					    AUTHORITATIVE, status);
					check_error(status,
					    gettext("No printer name context"));
					delete printer_ref;
				}
				name_str = new
