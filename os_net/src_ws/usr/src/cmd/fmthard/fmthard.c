...
#pragma ident	"@(#)fmthard.c	1.9	96/06/13 SMI"
 ...
 *
 * Features supported on x86 and PowerPC:
 *
...
#elif defined(__ppc)
static char	*uboot = "boot";
int		vt_inval;	/* no vtoc on disk */
static short	usilent;	/* be quiet about work being done */
...
#elif defined(__ppc)
	while ((c = getopt(argc, argv, "d:u:iSn:qs:I:")) != EOF)
#else
#error No platform defined.
#endif
		switch (c) {
...
#elif defined(__ppc)
"Usage:	fmthard [ -i ] [ -S ] [-I geom_file] [ -d arguments] \
-n volumename | -s datafile  raw-device\n");
