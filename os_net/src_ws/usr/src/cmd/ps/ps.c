...
#pragma	ident	"@(#)ps.c	1.35	96/08/27 SMI"
...
#define	MAXUNAME 10	/* max number of chars in a user name or printed uid */
#define	MAXGNAME 10	/* max number of chars in a group name or printed gid */
...
struct gdata {
	...
	char	name[MAXGNAME];	/* group name, may not be null terminated */
};
...
uconv(struct udata *utbl, int *pn)
{
	...
	for (i = 0; i < n; i++) {
		...
		if (pwd != NULL) {
			...
			(void) strncpy(utbl[i].name, pwd->pw_name, MAXUNAME);
		} else {
			rval++;
			(void) fprintf(stderr,
...
gconv(struct gdata *gtbl, int *pn)
{
	...
	for (i = 0; i < n; i++) {
		...
		if (grp != NULL) {
			...
			(void) strncpy(gtbl[i].name, grp->gr_name, MAXGNAME);
		} else {
			rval++;
			(void) fprintf(stderr,
