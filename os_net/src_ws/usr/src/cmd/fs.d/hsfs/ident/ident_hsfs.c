...
#pragma	ident	"@(#)ident_hsfs.c	1.5	96/04/17 SMI"
 ...
 *
 * NOTE:
 *	arguments "rawpath" and "verbose" not currently used (but required
...
{
	static bool_t	findcdivol(int, char *, bool_t);
	static bool_t	findhsvol(int, char *, bool_t);
	static bool_t	findisovol(int, char *, bool_t);
	...
	*clean = TRUE;
	...
	if (findcdivol(fd, volp_buf, verbose)) {
		return (TRUE);
	}
	if (findhsvol(fd, volp_buf, verbose)) {
		return (TRUE);
	}
	if (findisovol(fd, volp_buf, verbose)) {
		return (TRUE);
	}
...
/*ARGSUSED*/
static bool_t
findcdivol(int fd, char *buf, bool_t verbose)
{
	static bool_t	getsector(int, char *, int);
	int		secno = ISO_VOLDESC_SEC;
	int		i;
	bool_t		found = FALSE;
#ifdef	DEBUG
...
/*ARGSUSED*/
static bool_t
findhsvol(int fd, char *buf, bool_t verbose)
{
	static bool_t	getsector(int, char *, int);
	int		secno = HS_VOLDESC_SEC;
	int		i;
	bool_t		found = FALSE;
#ifdef	DEBUG
...
/*ARGSUSED*/
static bool_t
findisovol(int fd, char *buf, bool_t verbose)
{
	static bool_t	getsector(int, char *, int);
	int		secno = ISO_VOLDESC_SEC;
	int		i;
