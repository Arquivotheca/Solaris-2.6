...
#ident   "@(#)cfsd_logelem.c 1.3	 96/03/26 SMI"
...
#define	TIMECHANGE(A, B) (memcmp(&A, &B, sizeof (timestruc_t)) != 0)
...
void
logelem_format_fid(cfsd_logelem_object_t *logelem_object_p, fid_t *fidp)
{
	ulong_t val;
	int index;
	char format[10];
...
logelem_roll_rename(cfsd_logelem_object_t *logelem_object_p, u_long *seqp)
{
	int xx;
	fid_t *fp;
	fid_t odirfid, ndirfid;
	struct cfs_dlog_mapping_space map, dirmap, delmap;
	cfs_dlog_tm_t *tmp;
	vattr_t va;
	timestruc_t mtime, ctime;
	timestruc_t delmtime, delctime;
	timestruc_t *delctimep = NULL;
	int time_log;
