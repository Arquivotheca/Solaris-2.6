...
#pragma ident "@(#)main.c   1.5     96/03/18 SMI"
 ...
 */
void
bld_pack_list(FILE *fd, char *fnam)
{
	static char last_base[MAXPATHLEN] = {" "};
	...
	if (*fnam == '/') {	/* absolute pathname */
		...
		if (strncmp(last_base, fnam, last_base_sz) != 0) {
			fprintf(fd, "BASE %.*s\n", sz, fnam);
			last_base_sz = sz;
