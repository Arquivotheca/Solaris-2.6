...
#ident	"@(#)df.c	1.30	96/04/18 SMI"	/* SVr4.0 1.11 */
	...
	struct statvfs64	*fs;
{
	...
	(void) printf("%8d%8lld%8lld",
	    fs->f_frsize,
	    fs->f_blocks,
	    fs->f_bavail);
	(void) printf("%8lld%8lld%8d\n",
	    fs->f_files,
	    fs->f_ffree,
	    fs->f_fsid);
	(void) printf("0x%x ",
...
print_totals()
{
	 ...
	 * columns as the original items.
	 */
	(void) printf(gettext("Totals              %8lld%8lld%8lld"),
