...
#pragma	ident	"@(#)nfs_cast.c 1.14	96/04/11 SMI"
...
static int comp_sm(const void *, const void *b);
	...
	struct transp *trans;
{
...
#ifdef DEBUG
	if (trace > 3) {
		trace_prt(1, "  sort_responses: before sort:\n");
		for (i = 0; i < size; i++)
	...
	qsort((void *) buffer, size, sizeof (struct sm), comp_sm);
...
/*
 * Comparison routine called by qsort(3).
 */
static int comp_sm(const void *a, const void *b)
{
