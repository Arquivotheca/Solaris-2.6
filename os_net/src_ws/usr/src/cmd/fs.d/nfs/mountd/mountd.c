...
#ident	"@(#)mountd.c	1.42	96/07/08 SMI"	/* SVr4.0 1.11  */
	...
	char **argv;
{
	...
	while ((c = getopt(argc, argv, "vrm")) != EOF) {
		...
		case 'r':
			...
			break;
		/*
		 * XXX Remove following option when bug 1247652
		 * is fixed.
		 */
		case 'm':
			multithread = 1;
...
#define	TEXT_DOMAIN "SYS_TEST"
	...
	/*
	 * XXX Turn on multithreading only if "-m" flag
	 * is set.  This must be removed when bug 1247652
	 * is fixed!  The mountd should be MT by default.
	 */
	if (multithread) {
		/*
...
checkrootmount(sh, rpath)
	...
	char *rpath;
{
	if (getshareopt(sh->sh_opts, SHOPT_NOSUB) != NULL) {
	...
	struct svc_req *rqstp;
{
	...
	if (flavor_count == 0) {
		error = EACCES;
		audit_mountd_mount(host, path, 0);
		goto reply;
	}
...
reply:
	...
	case MOUNTVERS_POSIX:
		...
		if (!svc_sendreply(transp, xdr_fhstatus, (char *)&fhs))
			log_cant_reply(transp);
		if (!fhs.fhs_status)
			audit_mountd_mount(host, path, 1); /* BSM */
		break;
	...
	for (shp = share_list; shp; shp = shp->shl_next) {
		...
		if ((*p1 == '\0' && *p2 == '/') ||
			...
			if (stat(sh->sh_path, &st1) < 0) {
				syslog(LOG_ERR, "%s: %m", p1);
				shp = NULL;
				goto done;
			}
			if (stat(path, &st2) < 0) {
				syslog(LOG_ERR, "%s: %m", p2);
				shp = NULL;
				goto done;
			}
...
netmatch(struct netbuf *nb, char *name)
{
	...
	/*
	 * Check if it's an IP addr
	 */
	if (nb->len != 4)
		return (0);

	memcpy(&claddr, nb->buf, 4);
	...
	if (isdigit(*name)) {
		 ...
		 * an IP address.
		 */
		np = getnetbyname_r(name, &n, buf, sizeof (buf));
		if (np == NULL) {
	...
	struct svc_req *rqstp;
{
	...
	mntlist_delete(host, path);	/* remove from mount list */
