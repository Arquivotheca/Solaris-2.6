...
# All rights reserved.
#
#pragma ident   "@(#)sysidpm.sh 1.1     96/09/13 SMI"
...
#
get_behavior() {
	grep -s "autoshutdown" $PWR_CONF > /dev/null
	if [ $? = 0 ]; then
		set - `grep "autoshutdown" $PWR_CONF`
		CURRENT_IDLE_TIME=$2
...
set_behavior() {
	...
	grep -v Auto-Shutdown $PWR_CONF | grep -vw autoshutdown > $TMP
...
print_estar_guidelines() {
        echo
	echo "\t****************************************************************"
	echo "\tThis system is compliant with Version 2 of the EPA's"
	echo "\tEnergy Star guidelines. This means that it is configured by"
	echo "\tdefault to save the state of the system to disk and then"
	echo "\tpower off after 30 minutes of idleness."
	echo "\t"
	echo "\tA system that has been suspended in this way can be restored"
	echo "\tto exactly where it was by pressing the power key."
	echo "\tThe definition of inactivity and the various times are user"
	echo "\tconfigurable."
	echo "\tSee the dtpower(1M) man page for more information."
        echo "\t****************************************************************"
}
