...
#ident	"@(#)sh.func.c	1.19	96/08/26 SMI"	/* SVr4.0 1.5	*/
	...
	register tchar **v;
{
	...
	limit = getval(lp, v+1);
	if (setlim(lp, hard, (rlim_t)limit) < 0)
		error(NOSTR);
}
	...
	tchar **v;
{
	...
	if (*cp == 0) {
		if (*v == 0)
			return ((longlong_t)(f+0.5) * lp->limdiv);
		cp = *v;
	}
	switch (*cp) {
