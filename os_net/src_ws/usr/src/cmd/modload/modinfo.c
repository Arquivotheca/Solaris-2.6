#ident	"@(#)modinfo.c 1.10 95/01/12 SMI"
	...
	char *argv[];
{
	...
	while ((opt = getopt(argc, argv, "i:w:c")) != EOF) {
		switch (opt) {
		case 'i':
...
print_info(struct modinfo *mi)
{
	...
	for (n = 0; n < MODMAXLINK; n++) {
		...
		mi->mi_msinfo[n].msi_linkinfo[MODMAXNAMELEN - 1]
		    = '\0';
		...
		if (wide) {
			...
			(void) sprintf(&namebuf[strlen(namebuf)], "%.*s (%.*s)",
			    255, mi->mi_name,
			    252 - (strlen(mi->mi_msinfo[n].msi_linkinfo)),
			    mi->mi_msinfo[n].msi_linkinfo);
			(void) printf("%.42s\n", namebuf);
		}
	}
...
usage()
{
	fatal("usage:  modinfo [-w] [-i <id_number>]\n");
}
