...
{	struct fileinfo *ip;
	...
	if (opt_debug & DBG_STAT)
		fprintf(stderr,
			"STAT: list=%d, file=%s, mod=0x%08lx, nacl=%d\n",
			which, fp->f_name, ip->f_modtime, ip->f_numacls);
}
...
update_info(struct file *fp, side_t which)
{
	...
	if (opt_debug & DBG_STAT)
		fprintf(stderr, "STAT: UPDATE from=%d, file=%s, mod=0x%08lx\n",
