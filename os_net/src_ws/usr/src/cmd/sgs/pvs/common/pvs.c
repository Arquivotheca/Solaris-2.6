 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)pvs.c	1.16	96/04/10 SMI"
 ...
 */
static void
sym_local(Cache * cache, Cache * sym, const char * file, int oflag)
{
	 ...
	 */
	syms++;
	if (ELF_ST_TYPE(syms->st_info) != STT_FILE) {
	 ...
	 */
	for (_symn = 1, syms++; _symn < symn; _symn++, syms++) {
		char *	name;

		if (ELF_ST_TYPE(syms->st_info) == STT_SECTION)
			continue;
		if (ELF_ST_TYPE(syms->st_info) == STT_FILE)
			break;
...
main(int argc, char ** argv)
{
	...
	for (; optind < argc; optind++) {
		...
		if ((elf = elf_begin(var, ELF_C_READ, NULL)) == NULL) {
			(void) fprintf(stderr, MSG_INTL(MSG_ELF_BEGIN), cname,
		...
		if ((ehdr = elf_getehdr(elf)) == NULL) {
			(void) fprintf(stderr, MSG_INTL(MSG_ELF_GETEHDR), cname,
			    file, elf_errmsg(elf_errno()));
			error = 1;
			(void) close(var);
			(void) elf_end(elf);
		...
		if ((scn = elf_getscn(elf, ehdr->e_shstrndx)) == NULL) {
			(void) fprintf(stderr, MSG_INTL(MSG_ELF_GETSCN), cname,
			    file, elf_errmsg(elf_errno()));
			error = 1;
			(void) close(var);
			(void) elf_end(elf);
		...
		if ((data = elf_getdata(scn, NULL)) == NULL) {
			(void) fprintf(stderr, MSG_INTL(MSG_ELF_GETDATA), cname,
			    file, elf_errmsg(elf_errno()));
			error = 1;
			(void) close(var);
			(void) elf_end(elf);
		...
		for (scn = NULL; scn = elf_nextscn(elf, scn); _cache++) {
			if ((shdr = elf_getshdr(scn)) == NULL) {
				(void) fprintf(stderr,
				    MSG_INTL(MSG_ELF_GETSHDR), cname, file,
			...
			if ((_cache->c_data = elf_getdata(scn, NULL)) ==
			    NULL) {
				(void) fprintf(stderr,
				    MSG_INTL(MSG_ELF_GETDATA), cname, file,
				    elf_errmsg(elf_errno()));
				error = 1;
				continue;
			}
