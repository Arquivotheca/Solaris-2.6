 ...
 *	All rights reserved
 */
#pragma ident	"@(#)entry.c	1.9	96/02/28 SMI"
...
sym_muldef_title()
{
	(void) printf(MSG_INTL(MSG_ENT_MUL_TITLE));
	symbol_title = FALSE;
}
...
ldmap_out(Ofl_desc * ofl)
{
	...
	(void) printf(MSG_INTL(MSG_ENT_MAP_TITLE_1));
	if (ofl->ofl_flags & FLG_OF_RELOBJ)
		(void) printf(MSG_INTL(MSG_ENT_MAP_TITLE_2));
	else
		(void) printf(MSG_INTL(MSG_ENT_MAP_TITLE_3));
