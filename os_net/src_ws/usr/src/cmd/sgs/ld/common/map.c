 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)map.c	1.42	96/10/03 SMI"
...
map_equal(const char * mapfile, Sg_desc * sgp, Ofl_desc * ofl)
{
	 ...
	 * they must have an associated address and size and are only
	 * meaningful for LOAD segments (the last failure we can correct, hence
...
map_version(const char * mapfile, char * name, Ofl_desc * ofl)
{
	...
	while ((tok = gettoken(mapfile)) != TK_RIGHTBKT) {
		...
		case TK_COLON:
			 ...
			 */
			if (strcmp(MSG_ORIG(MSG_MAP_LOCAL), _name) == 0)
...
map_parse(const char * mapfile, Ofl_desc * ofl)
{
	...
	while ((tok = gettoken(mapfile)) != TK_EOF) {
		...
		if ((strcmp(sgp1->sg_name, MSG_ORIG(MSG_STR_INTERP))  == 0) ||
		    (strcmp(sgp1->sg_name, MSG_ORIG(MSG_STR_DYNAMIC)) == 0)) {
