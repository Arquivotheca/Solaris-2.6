...
main(int argc, char ** argv)
{
	...
	Elf *	elf;
	int	cflag = 0, dflag = 0, fflag = 0, lflag = 0, rflag = 0;
	int	sflag = 0, vflag = 0;
...
#ifdef	ENABLE_CACHE
	while ((var = getopt(argc, argv, "cdflrsv")) != EOF) {
		switch (var) {
		case 'c' :			/* enable cache search */
			cflag = 1;
			break;
#else
	while ((var = getopt(argc, argv, "dflrsv")) != EOF) {
		switch (var) {
#endif
