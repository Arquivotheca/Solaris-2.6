...
#pragma ident	"@(#)ar.c	1.12	96/03/04 SMI" 	/* SVr4.0 1.6	*/
...
_elf_armem(Elf * elf, char * file, size_t fsz)
{
	...
	m->m_hdr.ar_date = _elf_number(f->ar_date,
		&f->ar_date[ARSZ(ar_date)], 10);
	m->m_hdr.ar_uid = _elf_number(f->ar_uid,
		&f->ar_uid[ARSZ(ar_uid)], 10);
	m->m_hdr.ar_gid = _elf_number(f->ar_gid,
		&f->ar_gid[ARSZ(ar_gid)], 10);
	m->m_hdr.ar_mode = _elf_number(f->ar_mode,
		&f->ar_mode[ARSZ(ar_mode)], 8);
	m->m_hdr.ar_size = _elf_number(f->ar_size,
