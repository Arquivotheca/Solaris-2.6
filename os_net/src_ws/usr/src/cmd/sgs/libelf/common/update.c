...
#pragma ident	"@(#)update.c	1.12	96/03/04 SMI" 	/* SVr4.0 1.17	*/
...
_elf_upd_lib(Elf * elf)
{
	...
	for (; s != 0; s = s->s_next) {
		...
		for (d = s->s_hdnode; d != 0; d = d->db_next) {
			if ((fsz = elf32_fsize(d->db_data.d_type, 1, ver)) ==
			    0)
				return (0);
...
size_t
wrt(Elf * elf, size_t outsz, unsigned fill, int update_cmd)
{
	NOTE(ASSUMING_PROTECTED(*elf))
	Elf_Data	dst, src;
	unsigned	flag;
...
elf_update(Elf * elf, Elf_Cmd cmd)
{
	...
	if ((u = elf->ed_ehdr->e_ident[EI_DATA]) == ELFDATANONE) {
		...
		elf->ed_ehdr->e_ident[EI_DATA] = (Byte)encode;
	}
