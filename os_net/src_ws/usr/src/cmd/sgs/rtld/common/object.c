 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)object.c	1.26	96/09/30 SMI"
...
elf_obj_init(Lm_list * lml, const char * name)
{
	...
	/*
	 * Load the link-editor library.  Note we switch off tracing during
	 * this loading so that an ldd(1) of an object preload only shows the
	 * user the objects that they care about.
	 */
	tracing = 0;
