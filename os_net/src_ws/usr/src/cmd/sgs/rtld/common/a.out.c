...
aout_map_so(Lm_list * lml, const char * pname, const char * profile)
{
	 ...
	 */
	/* LINTED */
	ld = (struct link_dynamic *)
		/* LINTED */
		&addr[((struct nlist *)
		&addr[N_SYMOFF(*exec)])->n_value];
	...
	if (!(lmp = aout_new_lm(lml, pname, profile, ld, addr, size)))
		goto error;
		...
		if (dlp->dl_magic == DL_DLOPEN_0) {
			for (tlmp = LIST(clmp)->lm_head; tlmp;
				...
				if (!(MODE(tlmp) & RTLD_GLOBAL))
					continue;
				if (sym = LM_LOOKUP_SYM(clmp)(_name, 0, 0, tlmp,
