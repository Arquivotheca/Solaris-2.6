 ...
 *	All Rights Reserved
 */
#pragma ident	"@(#)_setup.c	1.44	96/10/11 SMI"
...
_setup(Boot * ebp, Dyn * ld_dyn)
{
	 ...
	 */
	{
		...
		while (reladdr < relend) {
			...
			if (rtype == R_SPARC_RELATIVE) {
				/*
				 * Because we no longer store values in the
				 * got (besides GOT[0]) there is no reason for
				 * us to add in the location from memory.
				 */
	 ...
	 */
	_environ = (char **)((int)_auxv - sizeof (char *));
	_init();
	_environ = _envp;
	_GET_TIME(interval1);
	...
	FLAGS(lmp) |= (FLG_RT_ANALYZED | FLG_RT_INITDONE);
	 ...
	 */
	if (!readenv((const char **)_envp, aoutflag))
		exit(1);
	...
	if (fd != -1) {
		...
		if (aoutflag) {
			...
			/*
			 * Disable any cache (BCP apps bring in ld.so and sbcp,
			 * so the cache won't be mapped at the appropriate
			 * place).
			 */
