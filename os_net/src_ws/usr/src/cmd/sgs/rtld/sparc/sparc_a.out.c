 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)sparc_a.out.c	1.27	96/10/07 SMI"
	...
	for (k = 0; k < nr; k++, rp++) {
		...
		case RELOC_DISP8:
			...
			if (!S_INRANGE(value, 8))
				...
				NAME(lmp), (name ? name :
				MSG_INTL(MSG_STR_UNKNOWN)), value, 8, ra);
			*ra = value;
			break;
		case RELOC_LO10:
		case RELOC_BASE10:
		...
		case RELOC_DISP16:
			...
			if (!S_INRANGE(value, 16))
			    eprintf(ERR_FATAL, MSG_INTL(MSG_REL_OVERFLOW),
				NAME(lmp), (name ? name :
				MSG_INTL(MSG_STR_UNKNOWN)), value, 16, ra);
			*(short *)ra = value;
			break;
		case RELOC_22:
		case RELOC_BASE22:
			...
			if (!S_INRANGE(value, 22))
			    eprintf(ERR_FATAL, MSG_INTL(MSG_REL_OVERFLOW),
				NAME(lmp), (name ? name :
				MSG_INTL(MSG_STR_UNKNOWN)), value, 22, ra);
			*(long *)ra = (*(long *)ra & ~S_MASK(22)) |
				(value & S_MASK(22));
			break;
		case RELOC_HI22:
