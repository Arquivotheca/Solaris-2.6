 ...
 *	All Rights Reserved
 */
#pragma ident	"@(#)_setup.c	1.31	96/10/11 SMI"
...
_setup(Boot * ebp, Dyn * ld_dyn)
{
	 ...
	 */
	{
		...
		while (reladdr < relend) {
			 ...
			 * (R_386_RELATIVE).  For these relocations we
			 ...
			 *
			 * Also note that (unlike SPARC) we have to process
			 * any jump relocations to set the base address of the
			 * corresponding got entry.
			 */
			if ((rtype == R_386_RELATIVE) ||
			    (rtype == R_386_JMP_SLOT))
				*(unsigned long *)roff += ld_base;
			else
				error++;
