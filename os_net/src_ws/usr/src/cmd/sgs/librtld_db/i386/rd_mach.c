 ...
 * All rights reserved.
 */
#pragma	ident	"@(#)rd_mach.c	1.5	96/09/09 SMI"
	...
	psaddr_t pltbase, rd_plt_info_t * rpi)
{
	...
	if (ps_pread(rap->rd_psp, pc + 2, (char *)&addr,
		...
		LOG(ps_plog(MSG_ORIG(MSG_DB_READFAIL_2), pc + 2));
		RDAGUNLOCK(rap);
		return (RD_ERR);
	}
	/*
	...
	if (ps_pread(rap->rd_psp, pc + 1, (char *)&ebx,
	    sizeof (unsigned)) != PS_OK) {
		LOG(ps_plog(MSG_ORIG(MSG_DB_READFAIL_2), pc + 1));
		RDAGUNLOCK(rap);
		return (RD_ERR);
	}
	...
	if (ps_pread(rap->rd_psp, addr, (char *)&addr,
	    sizeof (unsigned)) != PS_OK) {
		LOG(ps_plog(MSG_ORIG(MSG_DB_READFAIL_2), addr));
		RDAGUNLOCK(rap);
		return (RD_ERR);
	}
	if (addr == (pc + 6)) {
