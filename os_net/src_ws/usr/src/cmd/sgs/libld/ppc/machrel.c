 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)machrel.c	1.23	96/03/11 SMI"
 ...
 */
/* LINTED */
static Word negative_got_offset = -1;
...
/*
 * Checks the ABSOLUTE value of the address -- the assumption is that
 * the caller will mask the value to fit the field appropriately.
 * Returns zero if address does not fit size (bits), one if address does.
 */
static int
Fits(long addr, int size)
{

	if (size == 32)
		return (1);
	return (((S_ABS(addr)) & ~(S_MASK(size))) == 0);
}
...
plt_entry(Ofl_desc * ofl, Word pltndx)
{
	...
	/*
	 * PLT[0]: li	%r11, pltndx * M_PLT_INSSIZE
	 */
	*(Word *)pltent = M_LIR11 | pltndx * M_PLT_INSSIZE;
	...
	pltoff = -pltoff;
	*(Word *)pltent = M_B | (pltoff & S_MASK(26));
}
#endif
...
perform_outreloc(Rel_desc * orsp, Ofl_desc * ofl)
{
	...
	if (orsp->rel_flags & FLG_REL_GOT) {
		...
		/*
		 * this must be a R_PPC_COPY - for those
		 ...
		 * R_PPC_COPY relocation.  We will null
	...
	if ((rep->re_flags & FLG_RE_UNALIGN) == 0) {
		if (((rep->re_fsize == 2) && ((Word)roffset & 0x1)) ||
			...
			eprintf(ERR_FATAL, MSG_INTL(MSG_REL_NONALIGN),
			    conv_reloc_PPC_type_str(orsp->rel_rtype),
	...
	} else if (orsp->rel_rtype == R_PPC_RELATIVE)
		ndx = STN_UNDEF;
	else
		ndx = sdp->sd_symndx;
	...
	for (LIST_TRAVERSE(&ofl->ofl_actrels, lnp, rcp)) {
		for (arsp = (Rel_desc *)(rcp + 1);
			...
			if (arsp->rel_flags & FLG_REL_GOT) {
				...
				Word	R2addr;
				/*
				 * Clear the GOT table entry, on PPC
				...
				*(Word *)R2addr = value;
				...
				if (arsp->rel_rtype == R_PPC_GOT16)
					if (Fits((long)value, 16))
						value &= S_MASK(16);
					else {
						eprintf(ERR_FATAL,
						    MSG_INTL(MSG_REL_GOTOVER),
						    arsp->rel_sname, value);
						return (S_ERROR);
					}
			} else if (IS_SECT_RELATIVE(arsp->rel_rtype)) {
			...
			if (arsp->rel_isdesc->is_indata->d_buf == 0) {
				eprintf(ERR_FATAL, MSG_INTL(MSG_REL_EMPTYSEC),
				    conv_reloc_PPC_type_str(arsp->rel_rtype),
...
reloc_local(Rel_desc * rsp, Rel * reloc, Ofl_desc * ofl)
{
	...
	if (!(rsp->rel_flags & FLG_REL_LOAD) &&
		...
		(void) eprintf(ERR_WARNING, MSG_INTL(MSG_REL_EXTERNSYM),
		    conv_reloc_PPC_type_str(rsp->rel_rtype), rsp->rel_fname,
 ...
 * a processor like PPC which has different models for addressing the
 * GOT.  PPC has two: small and large, small uses a signed 16-bit offset
...
{
	if (ofl->ofl_pltcnt >= M_PLT_MAXSMALL)
		ofl->ofl_pltcnt++;
...
allocate_got(Ofl_desc * ofl)
{
	...
	negative_got_offset = countSmallGOT > (M_GOT_MAXSMALL / 2) ?
	    -(countSmallGOT - (M_GOT_MAXSMALL / 2)) : -1;

	/*
	 * Initialize the large and small got offsets (used in assign_got()).
	 */
	small_index = (int)negative_got_offset == -1 ?
	    M_GOT_XNumber - 1 : negative_got_offset;
...
/*
 * Initializes .got[0] with _DYNAMIC.
 * Initializes .got[-1] with a "blrl" instruction.
 */
void
fillin_gotplt1(Ofl_desc * ofl)
{
	...
	if (ofl->ofl_osgot) {
		...
		genptr = ((unsigned char *)ofl->ofl_osgot->os_outdata->d_buf
			- ((negative_got_offset + 1) * M_GOT_ENTSIZE));
		/* LINTED */
		*((Word *)genptr) = M_BLRL;
	}
}
