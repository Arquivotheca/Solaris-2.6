 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)syms.c	1.55	96/10/23 SMI"
...
sym_undef_title()
{
	eprintf(ERR_NONE, MSG_INTL(MSG_SYM_UNDEF));
...
sym_validate(Ofl_desc * ofl)
{
	...
	Sym *		sym;
	u_longlong_t	flags = ofl->ofl_flags;
	u_longlong_t	undef = 0, needed = 0, verdesc = 0;
	Half		align = 0;
	size_t		size = 0;
	Ifl_desc *	ifl;
...
sym_process(Is_desc * isc, Ifl_desc * ifl, Ofl_desc * ofl)
{
	 ...
	 */
	if ((ifl->ifl_oldndx = libld_malloc(total * sizeof (Sym_desc *))) == 0)
		return (S_ERROR);
