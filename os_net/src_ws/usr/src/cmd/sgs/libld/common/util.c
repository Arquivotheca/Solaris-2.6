 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)util.c	1.29	96/10/03 SMI"
...
libld_malloc(size_t size)
{
	...
	if ((ldhp == 0) ||
		...
		size_t	alloc_size = S_ROUND((asize + ldhpsz), HEAPALIGN);
		...
		new_ldhp->lh_free = vptr = (void *)((unsigned long)new_ldhp +
			ldhpsz);
		new_ldhp->lh_end = (void *)((int)new_ldhp + alloc_size);
...
ofl_cleanup(Ofl_desc * ofl)
{
	...
	for (cur = ld_heap, prev = 0; cur; cur = cur->lh_next) {
		if (prev)
			(void) munmap((caddr_t)prev, (int)prev->lh_end -
				(int)prev);
		prev = cur;
	}
	if (prev)
