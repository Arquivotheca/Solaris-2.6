 ...
 *	All rights reserved.
 */
#pragma ident	"@(#)files.c	1.52	96/09/18 SMI"
	...
	Ofl_desc * ofl)
{
	...
	if (ehdr->e_machine != M_MACH) {
		if ((ehdr->e_machine == M_MACHPLUS) &&
			...
			(void) elf_errno();
			eprintf(ERR_ELF, MSG_INTL(MSG_ELF_INVALTYPE), name);
	...
	if (ehdr->e_version > ofl->ofl_libver) {
		(void) elf_errno();
		eprintf(ERR_ELF, MSG_INTL(MSG_ELF_VERSHIGH), name);
		ofl->ofl_flags |= FLG_OF_FATAL;
		return (0);
	}
	if (ehdr->e_ident[EI_CLASS] != M_CLASS) {
		(void) elf_errno();
		eprintf(ERR_ELF, MSG_INTL(MSG_ELF_INVALCLASS), name);
		ofl->ofl_flags |= FLG_OF_FATAL;
		return (0);
	}
	if (ehdr->e_ident[EI_DATA] != M_DATA) {
		(void) elf_errno();
		eprintf(ERR_ELF, MSG_INTL(MSG_ELF_INVALBYTE), name);
		ofl->ofl_flags |= FLG_OF_FATAL;
		return (0);
	}
 ...
 *
 * Note: That the .dynamic section is only considered interesting
 *	 when dlopen()ing a relocatable object (which is how
 *	 FLG_OF_RELDYN gets set).
 */
int
	...
	Elf_Scn *scn, Word ndx, int ident, Ofl_desc * ofl)
{
	 ...
	 */
	if ((ofl->ofl_flags & FLG_OF_RELDYN) == 0)
		return (1);
...
rel_process(Is_desc * isc, Ifl_desc * ifl, Ofl_desc * ofl)
{
	...
	if (shdr->sh_type != M_REL_SHT_TYPE) {
		eprintf(ERR_FATAL, MSG_INTL(MSG_FIL_INVALSEC), ifl->ifl_name,
		    isc->is_name, conv_sectyp_str(shdr->sh_type));
		ofl->ofl_flags |= FLG_OF_FATAL;
		return (0);
	}
	...
	Half flags, Ofl_desc * ofl)
{
	...
	case ELF_K_ELF:
		...
		if ((ehdr = elf_getehdr(elf)) == NULL) {
			eprintf(ERR_ELF, MSG_INTL(MSG_ELF_GETEHDR), name);
			ofl->ofl_flags |= FLG_OF_FATAL;
			return (0);
		}
		 ...
		 */
		ifl = ifl_setup(name, ehdr, elf, flags, ofl);
