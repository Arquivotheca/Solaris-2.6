 ...
 */
#pragma ident	"@(#)dldump.c	1.19	96/05/15 SMI"
...
rt_dldump(Rt_map * lmp, const char * opath, int pfd, int flags, Addr addr)
{
	...
	Word		rel_null_no = 0, rel_data_no = 0, rel_func_no = 0;
	...
	if ((ielf = elf_begin(fd, ELF_C_READ, NULL)) == NULL) {
		eprintf(ERR_ELF, MSG_INTL(MSG_ELF_BEGIN), ipath);
	...
	if ((oelf = elf_begin(fd, ELF_C_WRITE, NULL)) == NULL) {
		eprintf(ERR_ELF, MSG_INTL(MSG_ELF_BEGIN), opath);
		cleanup(ielf, oelf, melf, icache, mcache, fd);
		return (1);
	}
	...
	if ((iphdr = elf_getphdr(ielf)) == NULL) {
		eprintf(ERR_ELF, MSG_INTL(MSG_ELF_GETPHDR), ipath);
		cleanup(ielf, oelf, melf, icache, mcache, fd);
		return (1);
	}
	...
	if ((scn = elf_getscn(ielf, iehdr->e_shstrndx)) == NULL) {
		eprintf(ERR_ELF, MSG_INTL(MSG_ELF_GETSCN), ipath);
		cleanup(ielf, oelf, melf, icache, mcache, fd);
		return (1);
	}
	if ((data = elf_getdata(scn, NULL)) == NULL) {
		eprintf(ERR_ELF, MSG_INTL(MSG_ELF_GETDATA), ipath);
		cleanup(ielf, oelf, melf, icache, mcache, fd);
		return (1);
	}
	...
	for (scn = 0; scn = elf_nextscn(ielf, scn); _icache++) {
		...
		if ((_icache->c_shdr = shdr = elf_getshdr(scn)) == NULL) {
			eprintf(ERR_ELF, MSG_INTL(MSG_ELF_GETSHDR), ipath);
			cleanup(ielf, oelf, melf, icache, mcache, fd);
			return (1);
		}
	...
	if (endx) {
		endx = elf_ndxscn(data_cache->c_scn);
		if (status)
			endx++;
	}
	...
	if ((oehdr = elf_newehdr(oelf)) == NULL) {
		eprintf(ERR_ELF, MSG_INTL(MSG_ELF_NEWEHDR), opath);
		cleanup(ielf, oelf, melf, icache, mcache, fd);
		return (1);
	}
	...
	if ((ophdr = elf_newphdr(oelf, iehdr->e_phnum)) == NULL) {
		eprintf(ERR_ELF, MSG_INTL(MSG_ELF_NEWPHDR), opath);
		cleanup(ielf, oelf, melf, icache, mcache, fd);
		return (1);
	}
	...
	*_shstr++ = '\0';
	...
	for (_icache++; _icache->c_flags != FLG_C_END; _icache++) {
		...
		if ((scn = elf_newscn(oelf)) == NULL) {
			eprintf(ERR_ELF, MSG_INTL(MSG_ELF_NEWSCN), opath);
			cleanup(ielf, oelf, melf, icache, mcache, fd);
			return (1);
		}
		if ((shdr = elf_getshdr(scn)) == NULL) {
			eprintf(ERR_ELF, MSG_INTL(MSG_ELF_NEWSHDR), opath);
			cleanup(ielf, oelf, melf, icache, mcache, fd);
			return (1);
		}
		...
		if ((data = elf_newdata(scn)) == NULL) {
			eprintf(ERR_ELF, MSG_INTL(MSG_ELF_NEWDATA), opath);
			cleanup(ielf, oelf, melf, icache, mcache, fd);
			return (1);
		}
		...
		if (_icache->c_flags == FLG_C_SHSTR) {
			...
			*data = *_icache->c_data;
			...
			oehdr->e_shstrndx = elf_ndxscn(scn);
	...
	if (elf_update(oelf, ELF_C_WRIMAGE) == -1) {
		eprintf(ERR_ELF, MSG_INTL(MSG_ELF_UPDATE), opath);
		cleanup(ielf, oelf, melf, icache, mcache, fd);
		return (1);
	}
	...
	if ((mehdr = elf_getehdr(melf)) == NULL) {
		eprintf(ERR_ELF, MSG_INTL(MSG_ELF_GETEHDR), opath);
		cleanup(ielf, oelf, melf, icache, mcache, fd);
		return (1);
	}
	...
	for (scn = 0; scn = elf_nextscn(melf, scn); _mcache++) {
		...
		if ((_mcache->c_shdr = elf_getshdr(scn)) == NULL) {
			eprintf(ERR_ELF, MSG_INTL(MSG_ELF_GETSHDR), opath);
			cleanup(ielf, oelf, melf, icache, mcache, fd);
			return (1);
		}

		if ((_mcache->c_data = elf_getdata(scn, NULL)) == NULL) {
			eprintf(ERR_ELF, MSG_INTL(MSG_ELF_GETDATA), opath);
			cleanup(ielf, oelf, melf, icache, mcache, fd);
			return (1);
		}
	}
	...
	for (_icache = &icache[1]; _icache->c_flags != FLG_C_END; _icache++) {
		...
		if (shdr->sh_type == M_REL_SHT_TYPE) {
			if (rel_base == (Rel *)0) {
				...
				rel_null = rel_base;
				rel_data = (Rel *)((Word)rel_null +
				    (rel_null_no * sizeof (Rel)));
				rel_func = (Rel *)((Word)rel_data +
	...
	if (dyn_cache) {
		Word	off = (Word)rel_base - (Word)mehdr;
