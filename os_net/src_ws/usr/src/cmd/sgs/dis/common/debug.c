 ...
 */
#pragma ident	"@(#)debug.c	1.5	94/03/06 SMI"
...
static		void	print_record(Elf32_Off, size_t),
			not_interp(short),
			user_def_type(),
			mod_fund_type(),
			location(),
...
get_debug_info()
{
	...
	"%dis: %s: failed to get section .debug; limited functionality\n",
		...
		debug = 0;
		return;
	} else if ((shdr = elf32_getshdr(scn)) != 0) {
		debug_data = 0;
...
static void
print_record(Elf32_Off offset, size_t size)
{
