 ...
 */
#pragma ident	"@(#)utls.c	1.7	94/03/06 SMI"
 ...
 */
int
get_rel_section(Elf32_Word sec_no)
{
	extern	Elf	*elf;
	Elf_Scn		*scn,  *scn1;
			...
			if (shdr->sh_type == SHT_REL &&
				...
				if ((scn1 = elf_getscn(elf, shdr->sh_link))
					...
					if ((data1 = elf_getdata(scn1, data1))
						...
						return (FAILURE);
					rel_sym_data =
					    (Elf32_Sym *)data1->d_buf;
					no_sym_entries =
					    data1->d_size/sizeof (Elf32_Sym);
				}
				else
					return (FAILURE);
...
void
search_rel_data(Elf32_Addr in_offset, long val, char **pos)
{
 ...
 */
void
locsympr(long val, int regno, char **pos)
{
	extern	int	oflag,
			trace;
#if defined(i386)
 ...
 */
void
build_labels(unsigned char *name, long location)
{
	extern int	trace;
 ...
 */
int
dfpconv(long fpword1, long fpword2, double *fpdoub, short *fpshort)
{
