 ...
 *		option allows an alternative name to be specified.
 *
 * msg.c	a data array of message strings.  The msg.h definitions are
 *		offsets into this array.  The -d option allows an alternative
 *		name to be specified.
 *
 * messages	a message file suitable for catgets(3c) or gettext(3i) use.  The
 *		-m option selects this output and specifies the filename to be
 *		used.
 *
 ...
 */
#pragma ident	"@(#)sgsmsg.c	1.7	96/08/22 SMI"
 ...
 */
static const char
	* Errmsg_malt =	"file %s: line %d: malformed input at line\n",
	* Errmsg_nmem =	"file %s: memory allocation failed: %s\n",
	* Errmsg_opne =	"file %s: open: %s\n",
	* Errmsg_wrte =	"file %s: write to file %s: %s\n",
	* Errmsg_use =	"usage: %s [-cl] [-d mesgdata] [-h mesgdefs] "
				"[-m messages] [-n name] [-i mesgident] file\n";

/*
 * Define any default file names.
 */
static const char
	* fldefs =	"msg.h",
	* fldata =	"msg.c",
 ...
 */
static int		cflag = 0, lflag = 0, prtmsgs = 0, line;
...
getmesgid(char * id)
{
	 ...
	 */
	if (flmids == 0) {
		(void) fprintf(stderr, "file %s: line %d: mesgid %s: unable to "
	...
	if ((buffer = (char *)malloc(LINE_MAX)) == 0) {
		(void) fprintf(stderr, Errmsg_nmem, fldesc, strerror(errno));
		return (1);
	}
	...
	if (mesgid) {
		if (cflag == 1) {
			 ...
			 * the catgets(3c) call.
			 */
			(void) fprintf(stderr, "file %s: line %d: setid %s: "
			    "warning: multiple mesgids encountered\n\t"
	...
	if (flmsgs && (prtmsgs == 1)) {
		if (cflag == 1) {
			if (fprintf(fdmsgs, "$quote \"\n$set %s\n",
			    setid) < 0) {
				(void) fprintf(stderr, Errmsg_wrte, fldesc,
	 ...
	 */
	if (cflag == 1) {
		if (fprintf(fddefs, "#define\t%s\t%s\n\n", mesgid, setid) < 0) {
			(void) fprintf(stderr, Errmsg_wrte, fldesc, fldefs,
			    strerror(errno));
			return (1);
		}
	}
...
init_defs(void)
{
	 ...
	 */
	if (fprintf(fddefs, "extern\tconst char *\t_%s(int);\n\n",
	    interface) < 0) {
		(void) fprintf(stderr, Errmsg_wrte, fldesc, fldefs,
		    strerror(errno));
		return (1);
...
init_data(void)
{
	...
	if (fprintf(fddata, "%s char __%s[] = { 0x00,", str, interface) < 0) {
		(void) fprintf(stderr, Errmsg_wrte, fldesc, fldata,
		    strerror(errno));
		return (1);
	}
...
main(int argc, char ** argv)
{
	...
	int		ptr = 1, column = 1, escape = 0, msgid = 0;
	...
	/*
	 * Validate the we have been give one input file.
	 */
	if ((argc - optind) != 1) {
		(void) fprintf(stderr, Errmsg_use, argv[0]);
		exit(1);
	...
	/*
	 * Open the input file and all the required output files.
	 */
	fldesc = argv[optind];

	if ((fddesc = fopen(fldesc, "r")) == NULL) {
		(void) fprintf(stderr, Errmsg_opne, fldesc, strerror(errno));
		return (1);
	}
	if ((fddefs = fopen(fldefs, "w+")) == NULL) {
		(void) fprintf(stderr, Errmsg_opne, fldefs, strerror(errno));
		return (1);
	}

	/*
	 * It's possible to assign the message definitions and message data to
	 * the same file.
	 */
	if (strcmp(fldata, fldefs) == 0)
		fddata = fddefs;
	else {
		if ((fddata = fopen(fldata, "w+")) == NULL) {
...
message:
			...
			while (*token) {
				...
				if (fprintf(fddata, str, _token) < 0) {
					(void) fprintf(stderr, Errmsg_wrte,
					    fldesc, fldata, strerror(errno));
					return (1);
				}
				ptr++, token++, len++;
				escape = 0;
			...
			if (escape == 0) {
				...
				if (fprintf(fddefs, "%s#define\t%s_SIZE\t%d\n",
				    str, defn, (len - 1)) < 0) {
					(void) fprintf(stderr, Errmsg_wrte,
					    fldesc, fldefs, strerror(errno));
					return (1);
				}
			}
			break;
	...
	if (msgid) {
		msgid = 0;
		if (fprintf(fdmsgs, "msgstr\t\"\"\n") < 0) {
			(void) fprintf(stderr, Errmsg_wrte, fldesc, flmsgs,
			    strerror(errno));
			return (1);
		}
	}
	...
	/*
	 * Finish off the generated data file.
	 */
	if (fini_data())
		return (1);

	/*
	 * Finish off the generated header file.
	 */
	if (fini_defs())
		return (1);
