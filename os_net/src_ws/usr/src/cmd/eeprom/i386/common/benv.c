...
#pragma ident   "@(#)benv.c 1.7 96/06/21 SMI"
 ...
 */
static void
add_cmd(benv_des_t *bd, char *base, char *last, char **next, int *line)
{
	char *cmd, *name, *val;
...
parse_benv(benv_des_t *bd)
{
	...
	for (tok = tnext = pbase; tnext < pend; tok = tnext)
		if (*tok == COMM)
			...
			add_cmd(bd, tnext, pend, &tnext, &line);
}
...
init_eeprom(void)
{
	if ((truncate(bootargs, 0) == -1) && errno != ENOENT)
		exit(_error(PERROR, "cannot truncate %s", bootargs));
