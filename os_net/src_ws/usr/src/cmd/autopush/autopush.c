...
#ident	"@(#)autopush.c	1.10	92/07/14 SMI"	/* SVr4.0 1.8	*/
...
#define	Digiterr	gettext("%s: ERROR: argument to %s option must be numeric\n")
	...
	char *namep;		/* autopush configuration filename */
{
	...
	while (fgets(buf, sizeof(buf), fp) != NULL) {
		...
		if (ioctl(sadfd, SAD_SAP, &push) < 0) {
			...
			retcode = 1;
			(void) fprintf(stderr, gettext("%s: ERROR: File %s: could not configure autopush for line %d\n"), Cmdp, namep, line);
			switch (error) {
			case EPERM:
				(void) fprintf(stderr, gettext("%s: ERROR: You don't have permission to set autopush information\n"), Cmdp);
				break;

			case EINVAL:
				(void) fprintf(stderr, gettext("%s: ERROR: Invalid major device number or\n\t\tinvalid module name or too many modules\n"), Cmdp);
				break;

			case ENOSTR:
				(void) fprintf(stderr, gettext("%s: ERROR: Major device is not a STREAMS driver\n"), Cmdp);
				break;

			case EEXIST:
				(void) fprintf(stderr, gettext("%s: ERROR: Major/minor already configured\n"), Cmdp);
				break;

			case ENOSR:
				(void) fprintf(stderr, gettext("%s: ERROR: Ran out of autopush structures\n"), Cmdp);
				break;

			case ERANGE:
				(void) fprintf(stderr, gettext("%s: ERROR: lastminor must be greater than minor\n"), Cmdp);
				break;
	...
	minor_t min;
{
	...
	if (ioctl(sadfd, SAD_SAP, &push) < 0) {
		...
		retcode = 1;
		(void) fprintf(stderr, gettext("%s: ERROR: Could not remove autopush information\n"), Cmdp);
		switch (error) {
		case EPERM:
			(void) fprintf(stderr, gettext("%s: ERROR: You don't have permission to remove autopush information\n"), Cmdp);
			break;

		case EINVAL:
			if ((min != 0) && (ioctl(sadfd, SAD_GAP, &push) == 0) &&
			    (push.sap_cmd == SAP_ALL))
				(void) fprintf(stderr, gettext("%s: ERROR: When removing an entry for ALL minors, minor must be set to 0\n"), Cmdp);
			else
				(void) fprintf(stderr, gettext("%s: ERROR: Invalid major device number\n"), Cmdp);
			break;

		case ENODEV:
			(void) fprintf(stderr, gettext("%s: ERROR: Major/minor not configured for autopush\n"), Cmdp);
			break;

		case ERANGE:
			(void) fprintf(stderr, gettext("%s: ERROR: minor must be set to begining of range when clearing\n"), Cmdp);
			break;
	...
	minor_t min;
{
	...
	if (ioctl(sadfd, SAD_GAP, &push) < 0) {
		...
		(void) fprintf(stderr, gettext("%s: ERROR: Could not get autopush information\n"), Cmdp);
		switch (error) {
		case EINVAL:
			(void) fprintf(stderr, gettext("%s: ERROR: Invalid major device number\n"), Cmdp);
			break;
	...
	default:
		(void) fprintf(stderr, gettext("%s: ERROR: Unknown configuration type\n"), Cmdp);
		return (1);
	}
	if (push.sap_npush > 1)
	...
	*cp = c;
	...
	while ((npush < MAXAPUSH) && (*cp)) {
		...
		if ((midx >= FMNAMESZ) && (*cp) && !isspace((int)*cp)) {
			(void) fprintf(stderr, gettext("%s: ERROR: File %s: module name too long, line %d ignored\n"), Cmdp, namep, lineno);
			return (1);
		}
		if (midx > 0) {
	...
	while ((*cp) && isspace((int)*cp))
		cp++;
	if (*cp) {
		(void) fprintf(stderr, gettext("%s: ERROR: File %s: too many modules, line %d ignored\n"), Cmdp, namep, lineno);
		return (1);
	}
	return (0);
}
