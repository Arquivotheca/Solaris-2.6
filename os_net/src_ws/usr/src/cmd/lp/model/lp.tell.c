...
#ident	"@(#)lp.tell.c	1.8	93/10/18 SMI"	/* SVr4.0 1.6	*/
	...
	while (fgets(buf, BUFSIZ, stdin)) {
		if ( useLaserWriterMessages) {
			...
			if ( strncmp(prefixString,bufPtr,pLen) == 0 ) {
				...
				if (strncmp(printerErrorString,bufPtr,peLen) == 0) {
					bufPtr += peLen;
					while ( *bufPtr == ' ' )  {
						bufPtr++;
						}
			...
			/* non NeWSprint style processing, backward compatible */
		...
		if ( mesgRetType ) {
			...
			/* temporarily check for R_CLEAR_FAULT here and 3 lines below
				until lpsched patch supports the message S_CLEAR_FAULT */

			if ((mtype != mesgRetType) && (mesgRetType != R_CLEAR_FAULT))
				done (93);
