...
#pragma ident   "@(#)netpr.c 1.20     96/04/26 SMI"
...
#define	TEXT_DOMAIN "SYS_TEST"  /* Use this only if it weren't */
	...
	while ((opt = getopt(argc, argv, "I:p:d:T:P:t:U:c:b")) != EOF)
		switch (opt) {
		case 'I': /* foo-49 */
	...
	if ((pa = mmap((caddr_t)0, filesize, PROT_READ, MAP_PRIVATE,
	...
	if (job_data->protocol == BSD) {
		if ((sockfd =  net_open(destination, 20)) == -1) {
			(void) fprintf(stderr,
...
usage_exit()
{
	...
	(void) fprintf(stderr,
	gettext("\t\t-U username [ -T title ] [ -P protocol ]\n"));
	(void) fprintf(stderr,
