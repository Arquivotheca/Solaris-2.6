...
#pragma ident	"@(#)submit_job.c	1.7	96/10/04 SMI"
...
mail(REQUEST *request, char *req_file, char *fmt, ...)
{
	...
	sprintf(buf, "/bin/mail %s", request->user);
...
parse_cf(char *cf)
{
	...
	for (entry = strtok(entry, "\n"); entry != NULL;
		...
		case 'p':	/* RFC-1179 PR file (print) */
			sprintf(buf, "prtitle='%s'", (prtitle ? prtitle : ""));
	...
	sprintf(buf, "%s%s%s", (host ? host : ""), (host ? "!" : ""),
		(user ? user : "nobody"));
	request.user = strdup(buf);

	if (banner != 0) {
		sprintf(buf, "%s",
			(job_name ? job_name : (class ? class : "")));
			...
			char **df_list)
{
	...
	if ((request = parse_cf(cf)) == NULL) {
		syslog(LOG_ERR|LOG_DEBUG,
			"Error parsing control file, Contents:\n%s\n", cf);
		return (-1);
	}

	if ((df_list != NULL) && (*df_list != NULL) &&
	    (is_postscript(*df_list) == 1))
