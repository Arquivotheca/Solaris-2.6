...
#pragma ident	"@(#)show_queue.c	1.6	96/10/04 SMI"
...
status_string(const char *printer, int *bad_status)
{
	...
	*bad_status = 1; /* assume it's bad news */
	...
	if (isprinter((char *)printer)) {
		...
		sprintf(mesg, gettext("%s: not a printer or class"), printer);
		return (mesg);
	}

	switch (status) {
	case MNODEST:
		sprintf(mesg, gettext("unknown destination: %s"), printer);
		break;
	case MNOINFO:
		sprintf(mesg, gettext("unknown status: %s"), printer);
		break;
	case MTRANSMITERR:
	...
	case MOK:
		...
		break;
	default:
		sprintf(mesg, gettext("bad status: %s, 0x%x"), printer, status);
		break;
	}
