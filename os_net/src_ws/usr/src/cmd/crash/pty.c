#ident	"@(#)pty.c	1.7	94/06/10 SMI"		/* SVr4.0 1.2 */
...
getpty()
{
	...
	if ((strcmp("", type) == 0) && !args[optind]) {
		addr = getaddr("ptms");
		count = getcount("pt");
		readmem(addr, 1, -1, (char *)&addr, sizeof (long),
							"ptms_pty address");
		print_header = 0;
...
char *heading;
{
	...
	if ((phys || !Virtmode) && (slot == -1))
		readmem(addr, 0, -1, (char *)&pt_tty, sizeof (pt_tty),
			"ptms_tty structure");
	else if (slot == -1)
		readmem((long)addr, 1, -1, (char *)&pt_tty, sizeof (pt_tty),
			"ptms_tty structure");
	else
		readmem((long)(addr + slot * sizeof (pt_tty)), 1, -1,
			(char *) &pt_tty, sizeof (pt_tty),
			"ptms_tty structure");
	...
	fprintf(fp, "%9x%9x%9x%8hd",	pt_tty.ptm_rdq,
					pt_tty.pts_rdq,
					pt_tty.pt_bufp,
					pt_tty.tty);
...
long addr;
{
	...
	readmem((long)(q.q_ptr), 1, -1, (char *)&ptem,
						sizeof (struct ptem), "");

	if (print) {
		fprintf(fp, "%s", heading);
		fprintf(fp, "\t%-9s%6d%9x%9x", mname, minfo.mi_idnum,
