...
#pragma	ident	"@(#)util.c	1.19	96/09/08 SMI"	/* SVr4.0 1.11.6.1 */
...
{
	uintptr_t u1 = *(uintptr_t *)a1;
	uintptr_t u2 = *(uintptr_t *)a2;
	...
	while (sp != &cp->cache_nullslab) {
		...
		for (i = 0; i < chunks; i++) {
			...
			if ((btp->bt_bxstat ^ KMEM_BUFTAG_ALLOC) !=
			    (int)btp->bt_bufctl)
				continue;
			if (kvm_read(kd, (u_long)btp->bt_bufctl, (void *)&bc,
...
struct allocowner *ownerhash = NULL;
int ownersize = 0;	/* Total number of entries */
int ownercnt = 0;	/* Number of entries in use */
...
ownerhash_grow(void)
{
	...
	for (ao = ownerhash, aoend = ownerhash + ownercnt; ao < aoend; ao++) {
		int bucket = ao->signature & (ownersize - 1);
...
ownercmp(const void *a1, const void *a2)
{
	...
	return ((int)ao2->total_size - (int)ao1->total_size);
}
...
print_owner(char *itemstr, int mem_threshold, int cnt_threshold)
{
	...
	for (ao = ownerhash, aoend = ownerhash + ownercnt; ao < aoend; ao++) {
		if (ao->total_size < mem_threshold &&
			...
			continue;
		fprintf(fp, "%d bytes for %d %s with data size %d:\n",
...
{
	Elf32_Sym* sym = symsrch(sym_name);
	if (!sym)
		return (NULL);
...
{
	Elf32_Sym *sym = findsym((u_long)addr);
	if (!sym)
		return (NULL);
...
/*
 * Routine that complains about being out of memory and does longjmp out.
 * For commands that use the gc stuff, we establish this as the memory error
 * handler (using the interposed malloc, etc. in mem.c), and then there is no
 * need to check for malloc, etc. failures.
 */
int
