#ident	"@(#)main.c	1.23	96/07/28 SMI"		/* SVr4.0 1.18.12.1 */
...
Elf32_Sym	*V, *Panic, *Start, *cpu_sym;	/* namelist symbol pointers */

int		active;		/* Set if crash is examining an active system */

gc_handle_t	temp;		/* pool of memory allocations freed after */
				/* each command, automatically (see gc.c) */
...
extern int	getas(), getbufhdr(), getbuffer(), getcallout(), getdis(),
		...
		getprnode(), getsnode(), getsocket(), getsoconfig(),
	...
	char	**argv;
{
	...
	/*
	 * We are about to go into a loop processing commands.  Our strategy
	 * for handling errors is to do a setjmp here and to longjmp back
	 * from the error() routine.  This includes running out of memory:
	 * we are linked with routines (mem.c) that interpose between us
	 * and the normal malloc/realloc/calloc, calling out_of_memory() if
	 * the allocation fails.  Since out_of_memory() calls error(), we
	 * end up back here.
	 *
	 * In order not to leak memory if a command aborts for some reason,
	 * we use a primitive garbage-collecting allocator that keeps track
	 * of the things we've allocated.  We free it all at once here.
	 *
	 * All of this simplifies memory allocation in the commands: you
	 * don't have to check for errors, and you don't have to free it,
	 * SO LONG AS YOU USE THE GC_MALLOC, ETC. ROUTINES!
	 *
	 * Note that most of the commands were written before the gc stuff
	 * and the interposed malloc, etc. were added.  Therefore we set
	 * the out-of-memory handler to NULL here, and let the smart
	 * commands set it to out_of_memory.
	 */
	temp = gc_create();  /* create pool of memory to free automatically */
	...
	for (;;) {
		...
		/*
		 * By default, don't call error() on allocation failures.
		 * Commands coded to handle this (any new commands should be!)
		 * should override this; see disp.c for an example.
		 */
		set_alloc_err_func(NULL);

		/*
		 * Free any temporary memory allocated.  Note that we do this
		 * here rather than at the end of the loop because an early
		 * abort through error() will mean that we never get to the
		 * end of the loop.
		 */
		gc_free_all(temp);
