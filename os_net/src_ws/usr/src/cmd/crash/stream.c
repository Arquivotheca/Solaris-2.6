#ident	"@(#)stream.c	1.20	96/07/28 SMI"		/* SVr4.0 1.15.16.1 */
...
/* ARGSUSED */
static void
prmblk(int addr, int phys)
{
	mblk_t *mp, mb;
	dblk_t *dp, db;

	readbuf((unsigned)addr, 0, phys, -1, (char *)&mb, sizeof (mb), "mblk");
	dp = mb.b_datap;
	readbuf((unsigned)dp, 0, phys, -1, (char *)&db, sizeof (db), "dblk");
	mp = &mb;
	dp = &db;
	if (strfull)
		fprintf(fp, "%s", mblkheading);
...
prmblkuser(void *kaddr, void *buf, u_int size, kmem_bufctl_audit_t *bcp)
{
	...
	mp = buf;
	readbuf((u_int)mp->b_datap, 0, 0, -1, (char *)&db, sizeof (db), "dblk");
	dp = &db;
