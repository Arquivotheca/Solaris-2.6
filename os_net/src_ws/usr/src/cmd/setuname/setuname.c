...
#pragma ident	"@(#)setuname.c	1.7	93/12/20 SMI"       /* SVr4.0 1.3 */
	...
	char   *nodename;	/* Network node-name */
{
	...
	if (kvm_nlist(kd, nl) != 0)
		...
		utsname_addr = (unsigned int)nl[0].n_value;
#else
	if (nlist("/unix", nl) != 0)
		rtncd = -1;
#endif
	 ...
	 */
#ifdef sun
	if (kvm_read(kd, utsname_addr, (char *)&utsname, sizeof (utsname)) ==
	    sizeof (utsname)) {
		if (sysname)
		...
		(void) kvm_write(kd, utsname_addr, (char *)&utsname,
		    sizeof (utsname));
		kvm_close(kd);
	} else
		return (-1);
