 ...
 *
 * The following changes were made to the elsie code:
 ...
 */
#ident	"@(#)zdump.c 1.7	96/07/15 SMI"
...
int	v;
{
	...
	(void) printf(gettext("%-*s  "), longest, zone);
	if (v)
		(void) printf(gettext("%.24s GMT = "), asctime(gmtime(&t)));
	tmp = localtime(&t);
...
#ifdef TM_GMTOFF
		(void) printf(gettext(" gmtoff=%ld"), tmp->TM_GMTOFF);
