...
#ident	"@(#)checknr.c	1.6	93/04/21 SMI"	/* SVr4.0 1.1	*/
	...
	for (lineno = 1; fgets(line, sizeof line, f); lineno++) {
		if (line[0] == '.') {
			...
			if (isspace(mac[0])) {
				...
				pe(lineno);
				printf(gettext("Command too long\n"));
			}
...
prop(i)
{
	...
	case FT:
		...
		break;
	default:
		printf(gettext("Bug: stk[%d].opno = %d = .%s, .%s"),
	...
	for (j=stktop; j>=0; j--)
		if (eq(mac,br[stk[j].opno].clbr)) {
			...
			if (j == stktop-2) {
				...
				pe(stk[j+1].lno);
				prop(j+1);
				printf(gettext(" does not match %d: "),
					stk[j+2].lno);
				prop(j+2);
				printf("\n");
...
char *line;
{
	...
	if (*mac == 0) {
		pe(lineno);
		printf(gettext("illegal define: %s\n"), line);
		return;
	}
	mac[2] = 0;
	if (isspace(mac[1]) || mac[1] == '\\')
		mac[1] = 0;
	if (ncmds >= MAXCMDS) {
		printf(gettext("Only %d known commands allowed\n"), MAXCMDS);
		exit(1);
	}
	addmac(mac);
}
