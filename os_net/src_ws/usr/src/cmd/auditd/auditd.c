#ifndef lint
static char sccsid[] = "@(#)auditd.c 94/03/08 SMI";
#endif 
...
#define TEXT_DOMAIN "SUNW_BSM_AUDITD"
#endif
...
#ifdef SunOS_CMW
	priv_t permitted_privs[SEC_SPRIVVEC_SIZE];
	bclear_t clearance;
	bclabel_t cmwlabel;

	/* get process's permitted privileges */
	if (getprocpriv(sizeof (priv_set_t),
	    permitted_privs, PRIV_PERMITTED) == -1) {
		perror("auditd: error getting process privileges");
		exit(1);
	}

	if (!PRIV_ISASSERT(permitted_privs, PRIV_SYS_TRUSTED_PATH)) {
		perror("auditd: not called from trusted path");
		exit(1);
	}

	bclearhigh(&clearance);

	if (setclearance(&clearance) == -1) {
		perror("auditd: error setting clearance");
		exit(1);
	}

	bclhigh(&cmwlabel);

	if (setcmwplabel(&cmwlabel, SETCL_ALL) == -1) {
		perror("auditd: could not set process labels");
		exit(1);
	}
...
#ifdef SunOS_CMW
	/*
	 * We cannot write-down to the console, so audit_warn needs to run at
	 * system_low.
	 */
	bcllow(&cmwlabel);

	if (setcmwplabel(&cmwlabel, SETCL_ALL) == -1) {
		(void) strcpy(warnstring,
			"could not reset process labels for audit_warn");
		(void) openlog("auditd", LOG_PID | LOG_ODELAY | LOG_CONS,
			LOG_AUTH);
		(void) syslog(LOG_ALERT, warnstring);
		(void) closelog();
		exit(1);
	}
#else
...
#ifdef SunOS_CMW
static void
dprintf(arg0, arg1, arg2, arg3, arg4, arg5, arg6)
	long	arg0, arg1, arg2, arg3, arg4, arg5, arg6;
{
	(void) bcllow(&debuglabel);
	(void) setcmwplabel(&debuglabel, SETCL_ALL);
	(void) fprintf(arg0, arg1, arg2, arg3, arg4, arg5, arg6);
	(void) bclhigh(&debuglabel);
	(void) setcmwplabel(&debuglabel, SETCL_ALL);
}
