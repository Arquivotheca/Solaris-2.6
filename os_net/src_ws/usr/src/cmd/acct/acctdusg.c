...
#ident	"@(#)acctdusg.c	1.6	94/07/18 SMI"	/* SVr4.0 1.8	*/
/*
...
int 	argc;
{
	...
	while ((pw = getpw(pwf)) != NULL) max_users++;
...
hashinit() {
	...
	if ((usglist = (struct disk *) (calloc(max_users, sizeof(struct disk))))
		== NULL) {
	   	fprintf(stderr,
			"acctdusg:  cannot allocate memory for %d users\n", 
			max_users);
	    	exit(1);
	}
...
uid_t j;
{
	...
	circle = start = (unsigned)j % max_users;
	do
        {
         	if ( usglist[circle].dsk_uid == j
		     || usglist[circle].dsk_uid == UNUSED )
  			return (circle);
		circle = (circle + 1) % max_users;
	} while ( circle != start);
	return (FAIL);
}
...
register struct passwd	*p;
{
	...
	if ((index = hash(p->pw_uid)) == FAIL) {
		fprintf(stderr, "acctdusg:  hash fails\n");
		return;
	}
	if (usglist[index].dsk_uid == UNUSED) {
