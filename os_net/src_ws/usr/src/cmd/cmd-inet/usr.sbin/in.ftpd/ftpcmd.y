...
%{
#ident	"@(#)ftpcmd.y	1.4	92/07/14 SMI"	/* SVr4.0 1.5	*/
...
/* tmpline may contain saved command from urgent mode interruption */
	for (c = 0; tmpline[c] != '\0' && --n > 0; ++c) {
		*cs++ = tmpline[c];
		if (tmpline[c] == '\n') {
			*cs++ = '\0';
			if (debug) {
				/* don't log the passwd 
				   from a PASSWD command */
				if (strncasecmp(s, "PASS", 4))
					syslog(LOG_DEBUG, "FTPD: command: %s", s);
				else
					syslog(LOG_DEBUG, "FTPD: command: PASS <password>");
			}
			tmpline[0] = '\0';
			return(s);
		}
	...
	*cs++ = '\0';
	if (debug) {
		/* don't log passwd from PASS cmd */
		if (strncasecmp(s, "PASS", 4))
			syslog(LOG_DEBUG, "FTPD: command: %s", s);
		else
