...
#ident		"@(#)snoop_rpcsec.c 1.3     96/06/07 SMI"
...
{
	switch (proc) {
	case RPCSEC_GSS_NULL:	return "RPCSEC_GSS_NULL"; break;
...
rpcsec_gss_control_proc(int type, int flags, int xid)
{
	...
	if (x->xid_gss_proc != RPCSEC_GSS_NULL) {
		if (flags & F_SUM) {
			if (type == CALL) {
	...
	struct cache_struct *x;
{
	...
	(void) sprintf(get_line(pos, getxdr_pos()),
		"   quality of protection (qop) = %d", qop);

	(void) sprintf(get_line(pos, getxdr_pos()),
		"   service = %d (%s)", service,
		rpcsec_gss_service_to_string(service));
