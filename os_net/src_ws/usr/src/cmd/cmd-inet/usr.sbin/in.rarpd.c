...
#ident	"@(#)in.rarpd.c	1.15	96/07/02 SMI"
...
struct	rarpdev {
	uchar_t	device[IFNAMSIZ];
	uchar_t	ldevice[IFNAMSIZ];
	int	unit;
	int	lunit;
	int	fd;
	int	d_fd;			/* delay pipe file descriptor */
	struct ether_addr etheraddr;
	ulong_t	ipaddr;			/* network order */
	ulong_t	if_netmask;		/* host order */
	ulong_t	if_ipaddr;		/* host order */
	ulong_t	if_netnum;		/* host order, with subnet */
	struct rarpdev *next;
};
...
static void	getdevice(char *, struct rarpdev *);
static void	getunit(char *, struct rarpdev *);
...
static void	arp_request(struct rarpdev *, struct ether_arp *,
							struct ether_addr *);
					/* ARP request handler */
static int	rarp_open(uchar_t *, int, ushort_t, struct ether_addr *);
...
static int	mightboot(ulong_t);
static void	get_ifdata(uchar_t *, int, ulong_t *, ulong_t *);
static int	get_ipaddr(struct rarpdev *, struct ether_addr, uchar_t *);
...
main(int argc, char *argv[])
{
	...
	if (aflag) {
		...
		(void) strcat(buf, argv[optind + 1]);
		getdevice(buf, rarpdev_head);
		getunit(buf, rarpdev_head);
	}
	/*
	 ...
	 */
	for (rdev = rarpdev_head; rdev != NULL; rdev = rdev->next) {
		thr_create(NULL, NULL, (void *(*)(void *))do_rarp, (void *)rdev,
							THR_NEW_LWP, NULL);
	}
...
getintf(void)
{
	...
	if (ioctl(fd, SIOCGIFCONF, (char *)&ifconf) < 0)
		syserr("SIOCGIFCONF");
#ifdef notdef
	if (strioctl(fd, SIOCGIFCONF, -1, sizeof (reqbuf), (char *)reqbuf) < 0)
		syserr("SIOCGIFCONF");
#endif
	...
	for (ifr = ifconf.ifc_req; ifconf.ifc_len > 0;
		...
		rdev = (struct rarpdev *)calloc(1, sizeof (struct rarpdev));
		if (rdev == NULL) {
			error("out of memory");
		}

		getdevice(ifr->ifr_name, rdev);
		getunit(ifr->ifr_name, rdev);
 ...
 */
static void
getdevice(char *s, struct rarpdev *rdev)
{
	char *cp;
	 ...
	 */
	(void) strcpy((char *)rdev->ldevice, s);
	if ((cp = strrchr((char *)rdev->ldevice, ':')) != NULL) {
		*++cp = '\0';
	} else {
		rdev->ldevice[0] = '\0';
	}
 ...
 */
static void
getunit(char *s, struct rarpdev *rdev)
{
	char	intbuf[128];
	char	*p = intbuf;
	char	*cp;
...
init_rarpdev(struct rarpdev *rdev)
{
	...
	/*
	 * Get the IP address and netmask from directory service.
	 */
	if (rdev->lunit != -1) {
		dev = rdev->ldevice;
		unit = rdev->lunit;
	...
	get_ifdata(dev, unit, &rdev->if_ipaddr, &rdev->if_netmask);

	/*
	 * Use IP address of the interface.
	 */
	rdev->if_netnum = rdev->if_ipaddr & rdev->if_netmask;
	rdev->ipaddr = htonl(rdev->if_ipaddr);
}
...
do_rarp(void *buf)
{
	...
	if (dflag) {
		(void) memcpy(&addr.s_addr, &rdev->ipaddr, IPADDRL);
		if (rdev->lunit != -1)
			debug("starting rarp service on device %s%d address %s",
				rdev->ldevice, rdev->lunit, inet_ntoa(addr));
		else
			debug("starting rarp service on device %s%d address %s",
				rdev->device, rdev->unit, inet_ntoa(addr));
	}
						...
						struct ether_addr *shostp)
{
	...
	if (memcmp((char *)&rdev->ipaddr, (char *)rp->arp_tpa, IPADDRL))
		return;
 ...
 */
static int
rarp_open(uchar_t *device, int unit, ushort_t type, struct ether_addr *etherp)
{
	register int fd;
	char	path[MAXPATHL];
	union DL_primitives *dlp;
...
mightboot(ulong_t ipa)
{
	...
	(void) sprintf(path, "%s/%08lX", BOOTDIR, ipa);
 ...
 */
static void
get_ifdata(uchar_t *dev, int unit, ulong_t *ipp, ulong_t *maskp)
{
	int	fd;
	struct	ifreq	ifr;
	struct	sockaddr_in	*sin;
 ...
 */
static int
get_ipaddr(struct rarpdev *rdev, struct ether_addr e, uchar_t *ipp)
{
	char host[MAXHOSTL];
	...
	for (p = hp->h_addr_list; *p; p++) {
		...
		if ((ntohl(addr.s_addr) & rdev->if_netmask) == rdev->if_netnum)
			break;
	}
	if ((ntohl(addr.s_addr) & rdev->if_netmask) != rdev->if_netnum) {
		if (dflag)
