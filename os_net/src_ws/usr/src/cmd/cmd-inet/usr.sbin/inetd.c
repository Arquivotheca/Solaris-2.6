...
#ident	"@(#)inetd.c	1.36	96/10/22 SMI"	/* SVr4.0 1.21	*/
	...
	while (argc > 0 && *argv[0] == '-') {
		...
		case 'r':
			...
			if (check_int(argv[1])) {
				...
				fprintf(stderr,
					"inetd: The argument to -r option <%s> is not an integer\n", argv[1]);
				exit(1);
			}
			if (check_int(argv[1])) {
		...
		case 'l':
			...
			if (check_int(argv[1])) {
				...
				fprintf(stderr,
					"inetd: The argument to -l option <%s> is not an integer\n", argv[1]);
				exit(1);
			}
	...
	register struct servtab *sep;
{
	...
	if (sep->se_socktype == SOCK_TX) {
		...
		if (nconf && (strcmp(nconf->nc_protofmly, NC_LOOPBACK) == 0)) {
			ns.h_host = HOST_SELF;
			ns.h_serv = sep->se_service;
			/*
			 * Map the machine name and service name in
			 * the nd_hostserv to a collection of addresses.
			 * Note:  Don't care about services that don't
			 * require this.  Thus, error is ignored.
			 */
			if (!netdir_getbyname(nconf, &ns, &nas)) {
				/* Copy the address */
				int tlen = addr_req.len = nas->n_addrs->len;
				addr_req.maxlen = nas->n_addrs->maxlen;
				addr_req.buf = (char *)malloc(tlen);
				memcpy(addr_req.buf, nas->n_addrs->buf,
					tlen);
				netdir_free((char *)nas, ND_ADDRLIST);
			} else if (debug) {
				fprintf(debugfp,
					"%s/%s: netdir_getbyname: %s\n",
					sep->se_service, sep->se_proto,
					netdir_sperror());
			}
		}
	...
	if (nconf) {
		...
		syslog(LOG_ERR, "could not get transport information");
...
{
	fprintf(stderr,
		"usage: %s -s [-d] [-t] [-r <count> <interval>] [-l <listen_backlog>] [<config_file>]\n",
		cmdname);
}
