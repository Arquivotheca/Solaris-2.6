#ident	"@(#)per_network.c	1.33	96/06/15 SMI"
...
ascii_to_pnrec(PER_NET_DB *pndbp, enum pn_type type, void *vp, PN_REC *pnp)
{
	...
	register u_char		*ucp;
	register Row		*rowp;
	register PN_ASCII_REC	*pn_asp;
	register char		*save_cp;
...
pnrec_to_ascii(PER_NET_DB *pndbp, PN_REC *pnp, PN_ASCII_REC *pn_asp)
{
	...
	/* CLIENTIP */
	(void) strcpy(pn_asp->as_clientip, inet_ntoa(pnp->clientip));

	/* SERVERIP */
	(void) strcpy(pn_asp->as_serverip, inet_ntoa(pnp->serverip));

	/* LEASE */
	(void) sprintf(pn_asp->as_lease, "%ld", ntohl(pnp->lease));
...
{
	register int		records = 0;
	int			tbl_err;
	char			buffer[DHCP_SCRATCH];
	char			sipbuf[sizeof (PN_NAME_TEMPLATE) + 2];
	register char		*fscp;
	int 			blen;
	...
	if (sip != (struct in_addr *)NULL) {
		(void) strcpy(sipbuf, inet_ntoa(*sip));
		fscp = sipbuf;
	} else
		fscp = (char *)NULL;
...
%1$s database failed: %d\n",
	...
	case PN_CLIENT_IP:
		...
		if (list_dd(TBL_DHCPIP, TBL_NS_DFLT, pndbp->name, NULL,
		    &tbl_err, &pndbp->tbl, NULL, inet_ntoa(*ip), fscp,
		    NULL, NULL) != TBL_SUCCESS) {
			if (tbl_err != TBL_NO_ENTRY) {
...
address in %1$s database failed: %d\n",
	...
	case PN_SERVER_IP:
		...
		if (list_dd(TBL_DHCPIP, TBL_NS_DFLT, pndbp->name, NULL,
		    &tbl_err, &pndbp->tbl, NULL, NULL, inet_ntoa(*ip), NULL,
		    NULL) != TBL_SUCCESS) {
			if (tbl_err != TBL_NO_ENTRY) {
