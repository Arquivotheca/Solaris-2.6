#ident	"@(#)test_per_net.c	1.7	96/07/08 SMI"
...
main()
{
	struct in_addr five_net = {129, 148, 5, 0};
	struct in_addr five_mask = {129, 148, 5, 255};
	struct in_addr serverid = {129, 148, 5, 138};
	...
	printf("Test 2: START ******************************************\n");
	/*
	 * Locate client ip 129.148.5.221.
	 */
	scratch.s_addr = 0x819405dd;
	if (open_per_net(&pndb, &five_net, &five_mask) != 0) {
		printf("didn't work.\n");
		return (1);
	} else {
