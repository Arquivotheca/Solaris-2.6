#ident	"@(#)dhcptab.c	1.57	96/09/08 SMI"
...
readtab(void)
{
	...
	if (verbose)
		dhcpmsg(LOG_INFO,
		    "Read %d entries from DHCP macro database\n", nentries);
...
check_includes(int first, char *mname)
{
	...
	register int	m, err = 0;
	register Row	*current_rowp = (Row *)NULL;
	char		*cp;
...
{
	register int		index, optype, i, j, err = 0;
	register SYM		*sp;
	register ENCODE		*tmp;
	register VNDLIST	**mpp, **ipp;
	register MACRO		*ic;
	register u_int		hc;
	u_char			ilen;
	char			t_sym[DHCP_SYMBOL_SIZE + 1];
...
{
	register int		i, j, class_exists, copy;
	register VNDLIST	**tmp;
...
process_data(ENCODE *ec, SYM *sym, char **pos)
{
	...
	register u_char *tmp;
	u_long scratch[(MAXUCHARLEN + 1) / sizeof (u_long)];
	...
	case NUMBER:
		...
		for (i = 0; i < items; i++) {
			...
			switch (sym->gran) {
			case sizeof (short):
				*(u_short *)tmp = htons(*(u_short *)tmp);
				break;
			case sizeof (long):
				*(u_long *)tmp = htonl(*(u_long *)tmp);
				break;
			case sizeof (longlong_t):
				*(u_long *)&tmp[0] =
				    htonl(*(u_long *)&tmp[0]);
				*(u_long *)&tmp[sizeof (long)] =
				    htonl(*(u_long *)&tmp[sizeof (long)]);
				break;
			}
...
{
	register int n = 0, len, quoteflag;
...
prs_inetaddr(char **src, uint32_t *result)
{
	...
	for (;;) {
		value = 0L;
		if (get_number(src, (void *)&value, sizeof (u_long)) != 0)
			value = 0L;
		if (**src == '.') {
			/*
			 * Internet format:
