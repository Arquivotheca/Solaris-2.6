...
#pragma ident	"@(#)myregexpr.c	1.5	94/02/03 SMI"
		/* SVr4.0 1.4	*/
#include <string.h>
#include "myregexpr.h"
int regerrno;
...
extern int _Sadvance(const char *lp, const char *ep);
extern int _Sstep(const char *p1, const char *p2);
extern char *_Scompile(char *instring, char *ep, const char *endbuf, int seof);
#else
static void getrnge();
...
char *
#ifdef __STDC__
compile(char *instring, char *ep, const char *endbuf)
#else
compile(instring, ep, endbuf)
register char *ep;
...
#endif
{
	...
	if (ep)
		...
		/* too large for the buffer or the buffer cannot be */
		/* allocated. */
		regerrno = 50;
		ret = 0;
