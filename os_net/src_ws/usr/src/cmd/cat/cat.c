...
#ident	"@(#)cat.c	1.24	96/04/18 SMI"	/* SVr4.0 1.17	*/
/*
**	Concatenate files.
*/
	...
	char *filenm;
{
	...
	if (S_ISREG(statp->st_mode) && (lseek(fi_desc, (off_t)0, SEEK_CUR)
		 ...
		 * Mmap time!
		 */
		bufferp = mmap((caddr_t)NULL, (size_t)mapsize, PROT_READ, MAP_SHARED,
		    fi_desc, (off_t)0);
	...
	if (mapsize != 0) {
		 ...
		 */
		while ((nitems = read(fi_desc, bufferp, (size_t)buffsize)) > 0) {
			offset = 0;
			/*
			 ...
			 */
			do {
				nwritten = write(1, bufferp+offset, (size_t)nitems);
				if (nwritten < 0) {
					if (!silent) {
						if (nwritten == -1)
							nwritten = 0l;
	...
	FILE *fi;
{
	...
	for (;;) {
		...
		if (isascii(c)) {
			...
			/*
			 * For non-printable ascii charactors.
			 */

			if (iscntrl(c)) {
				/* For cntrl charactors. */
				if ((c == '\t') || (c == '\f')) {
					/*
