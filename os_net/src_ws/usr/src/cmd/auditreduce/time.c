#ifndef lint
static char	*sccsid = "@(#)time.c 1.4 95/02/22 SMI;";
...
static char	*invalid = "invalid date/time format -";
...
struct tm *tme;
{
	...
	if (tme->tm_sec < 0 || tme->tm_sec > 59) {
		(void) sprintf(errbuf,
		    "seconds out of range (%d)", tme->tm_sec);
		error_str = errbuf;
	} else if (tme->tm_min < 0 || tme->tm_min > 59) {
		(void) sprintf(errbuf,
		    "minutes out of range (%d)", tme->tm_min);
		error_str = errbuf;
	} else if (tme->tm_hour < 0 || tme->tm_hour > 23) {
		(void) sprintf(errbuf, "hours out of range (%d)", tme->tm_hour);
		error_str = errbuf;
	} else if (tme->tm_mon < 0 || tme->tm_mon > 11) {
		(void) sprintf(errbuf, "months out of range (%d)", tme->tm_mon);
		error_str = errbuf;
	} else if (tme->tm_year < 0) {
		(void) sprintf(errbuf, "years out of range (%d)", tme->tm_year);
		error_str = errbuf;
		...
		if (!(days_in_year(tme->tm_year) == 366 && tme->tm_mon == 1 &&
			...
			(void) sprintf(errbuf,
				"days out of range (%d)", tme->tm_mday);
			error_str = errbuf;
		}
	} else if (tme->tm_wday < 0 || tme->tm_wday > 6) {
		(void) sprintf(errbuf,
		    "weekday out of range (%d)", tme->tm_wday);
		error_str = errbuf;
	} else if (tme->tm_yday < 0 || tme->tm_yday > 365) {
		(void) sprintf(errbuf,
		    "day of year out of range (%d)", tme->tm_yday);
		error_str = errbuf;
	}
...
int	opt;
{
	...
	if (len < 6) {
		if (*str++ != '+') {
			(void) sprintf(errbuf,
			    "%s needs '+' (%s)", invalid, str);
			error_str = errbuf;
			return (-1);
		}
		if (opt != 'b') {
			(void) sprintf(errbuf,
			    "%s only allowed with 'b' option (%s)",
			    invalid, str);
			error_str = errbuf;
			return (-1);
		}
		if (m_after == 0) {
			(void) sprintf(errbuf,
			    "must have -a to use -b +nx form (%s)", str);
			error_str = errbuf;
			return (-1);
		}
		/*
		...
		if (strlen(str) != strspn(str, "0123456789")) {
			(void) sprintf(errbuf,
				"%s non-numeric offset (%s)", invalid, str);
			error_str = errbuf;
			return (-1);
		}
...
struct tm *tme;
{
	...
	if (len != strspn(str, digits)) {
		(void) sprintf(errbuf, "%s not all digits (%s)", invalid, str);
		error_str = errbuf;
		return (-1);
	}
	if (len % 2) {
		(void) sprintf(errbuf,
		    "%s odd number of digits (%s)", invalid, str);
		error_str = errbuf;
		return (-1);
	}
	/*
	...
	(void) memcpy((char *) & nowtime, (char *)gmtime(&time_now),
		sizeof (nowtime));
	/*
	 * If the '19' or '20' is missing in the year add it in.
	 */
	if (nowtime.tm_year < 100) {	/* century = 1900 */
		if (str[0] != '1' && str[1] != '9') {
			(void) strcpy(&strxx[2], str);
			strxx[0] = '1';
			strxx[1] = '9';
			str = strxx;
		}
	...
	if (len < 8 || len > 14) {
		(void) sprintf(errbuf, "invalid date/time length (%s)", str);
		error_str = errbuf;
		return (-1);
	}
