#ifndef lint
static char sccsid[] = "@(#)regex2.c 1.3 95/02/22 SMI";
#endif
...
char *fexp = NULL;	/* full list of regular expressions */
int nexp = 1;		/* number of regular expressions in fexp */
struct exp *p_exp = NULL; /* list of individual expressions */
	...
	char *s;
{
	...
	for (i = 0, p = fexp; *p != '\0'; i++) {
		...
		if ((ret = regcmp(p_exp[i].s, NULL)) == NULL)
			return (er);
	}
	return (NULL);
}
