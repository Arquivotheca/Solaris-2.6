#ifndef lint
static char	*sccsid = "@(#)option.c 1.10 95/02/22 SMI;";
...
#ifdef SunOS_CMW
static int	proc_slabel();
static int	proc_ilabel();
#endif
...
#ifdef SunOS_CMW
	/* only CMW has 'i', and 's'/'z' options */
	static char	*options = "ACD:M:QR:S:VO:a:b:c:d:e:g:i:j:m:o:r:s:u:z:";
#else
...
start_over:
	while ((opt = getopt(argc, argv, options)) != EOF) {
...
#ifdef DUMP
		/*
		 * This option is so old and so unsupported, it didn't even
		 * compile when def'd during the integration into the source.
		 * Therefore if this code is ever to be used again,
		 * it needs to be properly debugged.
		 */
		case 'T':
			if (proc_token(optarg))
				error = TRUE;
			break;
#endif
		...
		case 'o':		/* object type */
			...
			break;
#ifdef SunOS_CMW
		case 'i':		/* information label range */
			if (proc_ilabel(optarg))
				error = TRUE;
			break;
		case 's':		/* sensitivity label range */
		case 'z':		/* keep 'z' to maintain compatibility */
			if (proc_slabel(optarg, opt))
				error = TRUE;
			break;
#endif
		...
		default:
			...
			if (toptind == optind)
				...
				offset = optind - 1;
			(void) sprintf(errbuf, "unknown option (%s)",
...
#ifdef SunOS_CMW
/*
 * process information label range
 */
static int
proc_ilabel(optstr)
char	*optstr;
{
	...
	if (flags & M_ILABEL) {
		error_str = "'i' option specified multiple times";
		return (-1);
	}
	flags |= M_ILABEL;
	p = strchr(optstr, ':');
	if (p == NULL) {
		/* match this exact label */
		if (!stobil(optstr, NEW_LABEL, &ilow, &error)) {
			(void) sprintf(errbuf,
			    "invalid information label (%s)", optstr);
			error_str = errbuf;
			return (-1);
		}
		exactilab = TRUE;
	...
	if (p == optstr) {
		/* lower bound is not specified .. default is system_low */
		billow(&ilow);
		if (!stobil(p + 1, NEW_LABEL, &ihigh, error)) {
			(void) sprintf(errbuf,
			    "invalid information label (%s)", p + 1);
			error_str = errbuf;
			return (-1);
		}
		return (0);
	...
	*p++ = '\0';
	...
	if (*p == '\0')
		/* upper bound is not specified .. default is system_high */
		bslhigh(&ihigh);
	else {
		if (!stobil(p, NEW_LABEL, &ihigh, &error)) {
			(void) sprintf(errbuf,
			    "invalid information label (%s)", optstr);
			error_str = errbuf;
			return (-1);
	...
	if (!bildominates(&ihigh, &ilow)) {
		(void) sprintf(errbuf,
		    "invalid information label range (%s)", optstr);
		error_str = errbuf;
		return (-1);
	}
	return (0);
...
/*
 * process sensitivity label range
 */
static int
proc_slabel(optstr, opt)
char	*optstr;
int	opt;
{
	char	*p;
	...
	if (flags & M_SLABEL) {
		sprintf(errbuf, "%c option specified multiple times", opt);
		error_str = errbuf;
		return (-1);
	}
	flags |= M_SLABEL;
	...
	if (p == NULL) {
		/* match this exact label */
		if (!stobsl(optstr, &slow, NEW_LABEL, &error)) {
			(void) sprintf(errbuf,
			    "invalid sensitivity  label (%s)", optstr);
			error_str = errbuf;
			return (-1);
		}
		exactslab = TRUE;
	...
	if (p == optstr) {
		/* lower bound is not specified .. default is system_low */
		bsllow(&slow);
		if (!stobsl(p + 1, &shigh, NEW_LABEL, &error)) {
			(void) sprintf(errbuf,
			    "invalid sensitivity label (%s)", p + 1);
			error_str = errbuf;
			return (-1);
		}
		return (0);
	...
	*p++ = '\0';
	...
	if (*p == '\0')
		...
		if (!stobsl(p, &shigh, NEW_LABEL, &error)) {
			(void) sprintf(errbuf,
			    "invalid sensitivity label (%s)", optstr);
			error_str = errbuf;
			return (-1);
	...
	/* make sure that upper bound dominates the lower bound */
	if (!bldominates(&shigh, &slow)) {
		(void) sprintf(errbuf,
		    "invalid sensitivity label range (%s)", optstr);
		error_str = errbuf;
		return (-1);
	}
	return (0);
...
int	do_check;
{
	...
	if (strcmp(&namep[15], "not_terminated") == 0) {
		...
		(void) strcat(errb, "ending time-stamp invalid - ");
		(void) strcat(errb, error_str);
		(void) strcpy(errbuf, errb);
		error_str = errbuf;
		return (ret);
