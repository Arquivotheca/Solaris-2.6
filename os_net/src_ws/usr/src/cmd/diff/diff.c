...
#ident	"@(#)diff.c	1.29	96/08/13 SMI"	/* SVr4.0 1.17	*/
...
#define IS_EOF(x)  (x == EOF && !errno)
#define IS_ECHAR(x) (x == WEOF && errno == EILSEQ)
	...
	for (i = 1; i <= len[0]; i++) {
		...
		if (bflag || wflag || iflag) {
			for (;;) {
				...
				c = getwc(input[0]);
				d = getwc(input[1]);
				/* need to take care of invalid char here */
...
skipline(f)
{
	...
	for (i = 1; c = getwc(input[f]); ) {
		...
		else if ((clen = wctomb(mbdummy, c)) < 0)
			error(BAD_MB_ERR);
		i += clen;
                errno = 0;
	}
...
char *s;
{
	...
	if (opt == D_IFDEF && oldfile) {
		...
		for (i = 0; i < nc; i += mlen) {
			for (j = 0; j < (unsigned int)MB_CUR_MAX; j++) {
				if ((ch = getc(lb)) != EOF)
					mbuf[j] = ch;
				else
					break;
			...
			for (j--; j >= mlen; j--)
				(void) ungetc((unsigned char)mbuf[j], lb);
			}
