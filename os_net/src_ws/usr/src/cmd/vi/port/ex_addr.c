...
#ident	"@(#)ex_addr.c	1.8	95/09/01 SMI"	/* SVr4.0 1.7	*/
#include "ex.h"
#include "ex_re.h"
...
error(value(vi_TERSE) ?
	...
	gettext("Address search hit TOP without matching pattern"));
		...
		case '\'':
			...
			addr = getmark(c);
			if (addr == 0)
				error(value(vi_TERSE) ? gettext("Undefined mark") :
		...
		default:
			...
			addr += lastsign;
			if (addr < zero)
				error(value(vi_TERSE) ? gettext("Negative address") :
				gettext("Negative address - first buffer line is 1"));
			if (addr > dol)
				error(value(vi_TERSE) ? gettext("Not that many lines") :
