...
#ident	"@(#)ex_temp.c	1.13	95/09/22 SMI"	/* SVr4.0 1.19	*/
#include "ex.h"
#include "ex_temp.h"
#include "ex_vis.h"
#include "ex_tty.h"
...
synctmp()
{
	...
	if (xtflag)
		...
		H.encrypted = 0;
	*zero = (line) H.Time;
	for (a = zero, bp = blocks; a <= dol; a += BUFSIZE / sizeof *a, bp++) {
		if (bp >= &H.Blocks[LBLKS-1])
			error(gettext("file too large to recover with -r option"));
		if (*bp < 0) {
	...
	int (*iofcn)();
{
	...
	if (rfile == -1) {
		CP(rfname, tfname);
		/* Replace the 'E' after the last '/' with 'R' */
		*tailpath(rfname) = 'R';
