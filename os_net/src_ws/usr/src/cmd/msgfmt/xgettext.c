 ...
 */
#pragma ident   "@(#)xgettext.c 1.8     94/12/19 SMI"
...
char	*pathname = NULL;
int	sflg = FALSE;
int	tflg = FALSE;	/* Undocumented option to extract dcgettext */
int	xflg = FALSE;
...
int	in_dcgettext		= FALSE;
int	in_textdomain		= FALSE;
int	in_str			= FALSE;
int	in_quote		= FALSE;
int	is_last_comment_line	= FALSE;
int	is_first_comma_found	= FALSE;
int	num_nested_open_paren	= 0;
	...
	struct domain_st	*head;
{
	...
	struct element_st	*p;
	struct element_st	*existing_po_list, *save_list;
	...
	if (jflg == TRUE) {
		...
		head->gettext_head = append_list
			(existing_po_list, head->gettext_head);
#ifdef DEBUG
		if (head->dname != NULL) {
