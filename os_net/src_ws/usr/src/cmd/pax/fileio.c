...
#ident	"@(#)fileio.c	1.3	96/10/14 SMI"
...
int openout(char *new_name, char *name, Stat *asb, Link *linkp, int ispass)

{
	...
	if (link(linkp->l_name, new_name) != 0) {
	    if (errno == ENOENT) {
		if (f_dir_create) {
		    if (dirneed(new_name) != 0 ||
			    link(linkp->l_name, new_name) != 0) {
	...
	if (mknod(new_name, (int) asb->sb_mode, (int) asb->sb_rdev) < 0) {
	    if (errno == ENOENT) {
		if (f_dir_create) {
		    if (dirneed(new_name) < 0 ||
	...
	if (exists) {
	    ...
	    if (dirmake(new_name, asb) < 0 && dirneed(new_name) < 0) {
		warn(new_name, strerror(errno));
		return (-1);
	    }
	} else {
	    warn(new_name, MSGSTR(FIO_DIR,
	...
	if (((dp = (Dirlist *) mem_get(sizeof(Dirlist))) != (Dirlist *)NULL) &&
	    ((dp->name = mem_str(new_name)) != (char *)NULL)) {
		dp->perm = asb->sb_mode;
	...
	if (exists) {
		...
		if (f_dir_create) {
		    if (dirneed(new_name) < 0
		       || mknod(new_name, (int) asb->sb_mode, 0) < 0) {
			warn(new_name, strerror(errno));
			return (-1);
		    }
		} else {
	...
	if (symlink(asb->sb_link, new_name) < 0) {
	    if (errno == ENOENT) {
		if (f_dir_create) {
		    if (dirneed(new_name) < 0 ||
			    symlink(asb->sb_link, new_name) < 0) {
			warn(new_name, strerror(errno));
			return (-1);
		    }
		} else {
	...
	if ((fd = creat(new_name, perm)) < 0) {
	    if (errno == ENOENT) {
		if (f_dir_create) {
		    if (dirneed(new_name) < 0 || 
			    (fd = creat(new_name, perm)) < 0) {
			warn(new_name, strerror(errno));
			return (-1);
		    }
		} else {
