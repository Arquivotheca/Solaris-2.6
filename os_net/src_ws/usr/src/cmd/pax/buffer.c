...
#pragma	ident	"@(#)buffer.c	1.5	96/10/14 SMI"
 ...
 */
int
inentry(char *new_name, char *name, Stat *asb)
{
	Link		*linkp;
	int		ifd;
	int		ofd;
	...
	if ((ofd = openout(new_name, name, asb,
		linkp = linkfrom(name, asb), 0)) > 0) {
		if (asb->sb_size ||
	...
	if (f_mode && chmod(name, asb->sb_mode & S_IPERM) == -1)
		warn(name, gettext("unable to preserve file mode"));

	if (f_owner) {
		if (chown(name, asb->sb_uid, asb->sb_gid) < 0) 
			warn(name, gettext("unable to preserve owner/group"));
		else if (f_mode &&
	    	    chmod(name, asb->sb_mode & (S_IPERM|S_ISUID|S_ISGID)) == -1)
			warn(name, gettext("unable to preserve file mode"));
	}
...

{
	...
	if ((pad = (total + M_STRLEN + H_STRLEN + TRAILZ) % BLOCKSIZE) != 0) {
	    pad = BLOCKSIZE - pad;
	}
