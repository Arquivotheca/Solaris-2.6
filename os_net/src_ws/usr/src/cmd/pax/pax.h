...
#pragma	ident	"@(#)pax.h	1.11	96/04/18 SMI"
...
#define	TAR_OFFSET_MAX	077777777777	/* largest we can archive */
/*
 ...
 */
typedef struct {
    struct stat64   sb_stat;
    char            sb_link[PATH_MAX + 1];
    char 	    *linkname;		/* for tar links */
} Stat;

#define	STAT(name, asb)		stat64(name, &(asb)->sb_stat)
#define	FSTAT(fd, asb)		fstat64(fd, &(asb)->sb_stat)
...
#ifdef	S_IFLNK
#	define	LSTAT(name, asb)	lstat64(name, &(asb)->sb_stat)
 ...
 */
#	define	LSTAT(name, asb)	stat64(name, &(asb)->sb_stat)
#endif				/* S_IFLNK */
