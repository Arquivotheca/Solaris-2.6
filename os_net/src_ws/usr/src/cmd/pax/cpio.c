...
#define CPIO_MODE	"%s: you must specify only one of the o,i or p options\n"
#define	CPIO_USAGE1	"Usage: %s -o[Bacv]\n"
#define	CPIO_USAGE2	"       %s -i[Bcdmrtuvf] [pattern...]\n"
#define CPIO_USAGE3	"       %s -p[adlmruv] directory\n"
...
do_cpio(void)
{
	...
	(void) fprintf(stderr, gettext(CPIO_MODE), myname);
	usage();
    }
    if (!f_pass) {
