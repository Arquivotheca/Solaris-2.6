...
#ident	"@(#)namelist.c	1.4	95/03/01 SMI"
...
#define	NL_MEM		"Unable to allocate enough memory for the filename list."
#define	NL_DIR		"An error occurred attempting to open the directory."
#define	NL_FOUND	"%1$s: %2$s was not found in the archive.\n"
#define	NL_LONG		"The name is too long."
...
name_match(char *p, int isdir)
{
	...
	if (nlp->regexp) {
		...
		if (f_no_depth) {
			...
			return(0);	/* subdir of root dir */
		    if (isdir) {	/* match top level dir */
			/* remember we matched a directory in namelist */
...
names_notfound(void)
{
    ...
    for (nlp = namelist; nlp != NULL; nlp = nlp->next, free(nlp)) {
	if (!nlp->found) {
