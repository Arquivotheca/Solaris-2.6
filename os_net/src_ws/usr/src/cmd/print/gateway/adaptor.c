/*
 * Copyright (c) 1995, 1996 by Sun Microsystems, In.c
 * All Rights Reserved
 */

#pragma ident	"@(#)adaptor.c	1.11	96/10/04 SMI"
...
lpsched_adaptor_available(const char *printer)
{
	...
	sprintf(buf, "/etc/lp/interfaces/%s", printer);
...
adaptor_available(const char *printer)
{
	...
	for (dir = strtok(path, ":,"); dir != NULL; dir = strtok(NULL, ":,")) {
		...
		if (paradigm_version < 0)
			sprintf(object, "%s/bsd_%s.so", dir, paradigm_name);
		else
			sprintf(object, "%s/bsd_%s.so.%d", dir, paradigm_name,
				paradigm_version);
...
adaptor_function(const char *paradigm, const char *function)
{
	...
	sprintf(name, "%s_%s", paradigm, function);
