...
#pragma ident	"@(#)dmesg.c	1.5	93/12/20 SMI"
	  /* from UCB 5.4 2/20/86 */
...
char **argv;
{
	...
	if (argc > 1 && argv[1][0] == '-') {
        ...
        if (kvm_read(kd, nl[NL_MSGBUF].n_value, (char *)&msgbuf_hd,
	...
	if (msgbufp == NULL)
        ...
        if (kvm_read(kd, nl[NL_MSGBUF].n_value, (char *)msgbufp,
	...
	if (sflg) {
		...
		omsgbufp = (struct msgbuf *) calloc(msgbufsize, 1);
		if (omsgbufp == NULL)
			done("Can't allocate memory\n");
