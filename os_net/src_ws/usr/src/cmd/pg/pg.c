...
char *file_name;
{
	...
	for (;;) {
		...
		if (find(0, new_ss.last_line + 1) != EOF)
			...
			new_ss.first_line = new_ss.last_line - (off_t)window + 1;
		}
...
char *filename;
{
	...
	for (;;) {
		...
		case 'z':
			...
			new_ss.last_line = new_ss.first_line + (off_t)window - 1;
			inwait = 0;
			return (0);
		case '\004': /* ^D */
		case 'd':
			...
			if (sign == 0)
				sign = 1;
			new_ss.last_line = old_ss.last_line + (off_t)sign*window/2;
			new_ss.first_line = new_ss.last_line - (off_t)window + 1;
			inwait = 0;
			return (0);
		case 's':
		...
		while (find(1, direction) != END_COND) {
			...
			if (step(Line, expbuf))
				if ((n -= direction) == 0) {
					switch (leave_search) {
					case 't':
						new_ss.first_line = find(1,(off_t)0);
						new_ss.last_line =
						    new_ss.first_line + (off_t)window
						    - 1;
						break;
					case 'b':
						new_ss.last_line = find(1,(off_t)0);
						new_ss.first_line =
						    new_ss.last_line - (off_t)window
						    + 1;
						break;
					case 'm':
						new_ss.first_line =
						    find(1, (off_t)0) - ((off_t)window - 1)/2;
						new_ss.last_line =
						    new_ss.first_line + (off_t)window
						    - 1;
						break;
