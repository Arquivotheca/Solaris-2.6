...
#ident "@(#)xstat.c	1.12	96/04/18 SMI"	/* SVr4.0 1.3	*/
...
void
show_xstat(process_t *Pr, long offset)
{
	struct xstat statb;

	if (offset != NULL &&
	    Pread(Pr, offset, (char *)&statb, sizeof (statb))
	    == sizeof (statb)) {
		(void) printf(
...
void
show_stat(process_t *Pr, long offset)
{
	struct stat statb;
...
void
show_stat64(process_t *Pr, long offset)
{
	struct stat64 statb;
	...
	if (offset != NULL &&
	    ...
	    == sizeof (statb)) {
		(void) printf(
	"%s    d=0x%.8lX i=%-5llu m=0%.6lo l=%-2lu u=%-5lu g=%-5lu",
			pname,
			statb.st_dev,
			statb.st_ino,
			statb.st_mode,
