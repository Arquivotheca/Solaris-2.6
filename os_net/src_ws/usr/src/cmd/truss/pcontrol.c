...
#pragma ident	"@(#)pcontrol.c	1.27	96/10/19 SMI"	/* SVr4.0 1.3	*/
...
void
msleep(int msec)	/* milliseconds to sleep */
{
...
again:	/* Come back here if we lose it in the Window of Vulnerability */
	...
	/* set run-on-last-close so process runs even if we die from SIGKILL */
	if (Psetflags(P, PR_RLC) != 0) {
		if (errno == EAGAIN)	/* WoV */
			goto again;
		if (errno != ENOENT)	/* Don't complain about zombies */
	...
	if (pid == getpid()) {
		/* write a magic number, read it through "as" file */
 ...
 * We choose the representative lwp because that
 * is all that truss needs, and it always exists.
 */
int
Pchoose_lwp(process_t *P)
{
	int fd;
	char pathname[100];
	char *fname;

	Punchoose_lwp(P);	/* Punchoose_lwp() calls Psync() */

	(void) sprintf(pathname, "%s/%ld/lwp/%ld/", procdir,
	    P->pid, P->why.pr_lwp.pr_lwpid);
...
Prelease(process_t *P)
{
	...
	if (P->state == PS_STOP) {
		...
		if (P->sethold) {
			...
			iov[1].iov_base = (caddr_t)&P->why.pr_lwp.pr_lwphold;
			iov[1].iov_len = sizeof (P->why.pr_lwp.pr_lwphold);
			if (writev(P->ctlfd, iov, 2) < 0)
				perror("Prelease set lwphold");
		}
		if (P->setregs) {
			cmd[0] = PCSREG;
			iov[0].iov_base = (caddr_t)&cmd[0];
			iov[0].iov_len = sizeof (long);
			iov[1].iov_base = (caddr_t)&P->REG[0];
			iov[1].iov_len = sizeof (P->REG);
			if (writev(P->ctlfd, iov, 2) < 0)
				perror("Prelease set registers");
		}
	}
...
Phang(process_t *P)
{
	...
	if (P->state == PS_STOP) {
		...
		if (Psync(P) != 0)
			perror("Phang set lwphold/registers");

		/* no run-on-last-close or kill-on-last-close */
		if (Presetflags(P, PR_RLC|PR_KLC|PR_ASYNC|PR_PTRACE) != 0)
			perror("Phang PCUNSET");

		/* inherit-on-fork */
		if (Psetflags(P, PR_FORK) != 0)
			perror("Phang PCSET");
...
int
Pstatus(process_t *P,	/* program table entry */
	int request,	/* PCSTATUS, PCSTOP, PCWSTOP */
	unsigned sec)	/* if non-zero, alarm timeout in seconds */
{
	...
	if (P->lwpstatfd < 0) {
		if (pread(P->statfd, &P->why, sizeof (P->why), (off_t)0)
		    != sizeof (P->why)) {
			err = errno;
			(void) memset(&P->why, 0, sizeof (P->why));
		}
	} else {
		if (pread(P->lwpstatfd, &P->why.pr_lwp, sizeof (P->why.pr_lwp),
		    (off_t)0) != sizeof (P->why.pr_lwp)) {
			err = errno;
			(void) memset(&P->why, 0, sizeof (P->why));
		}
...
int
Pgetsubcode(process_t *P)
{
	register int syscall = P->why.pr_lwp.pr_syscall;
	register int nsysarg = P->why.pr_lwp.pr_nsysarg;
	register int subcode = -1;

	if (syscall > 0 && nsysarg > 0) {
		subcode = P->why.pr_lwp.pr_sysarg[0];
		switch (syscall) {
		case SYS_utssys:
			if (nsysarg > 2)
				subcode = P->why.pr_lwp.pr_sysarg[2];
			break;
		case SYS_open:
		case SYS_open64:
			if (nsysarg > 1)
				subcode = P->why.pr_lwp.pr_sysarg[1];
			break;
		case SYS_door:
			if (nsysarg > 5)
				subcode = P->why.pr_lwp.pr_sysarg[5];
			break;
		}
	}
	return (subcode);
...
deadcheck(process_t *P)
{
	if (P->statfd < 0)
		...
		while (pread(P->statfd, &P->why, sizeof (P->why), (off_t)0)
		    != sizeof (P->why)) {
			switch (errno) {
			default:
				/* process is dead */
	...
	int sec)	/* timeout for PRWAIT, seconds */
{
	...
	*ctlp++ = (flags & ~PRWAIT);
	if (flags & PRWAIT) {
		*ctlp++ = PCTWSTOP;
		*ctlp++ = MILLISEC*sec;		/* timeout */
	}
	size = (char *)ctlp - (char *)ctl;

	ctlfd = (P->lwpctlfd >= 0)? P->lwpctlfd : P->ctlfd;
	if (write(ctlfd, ctl, size) != size) {
		if (errno == EINTR || errno == ENOENT || errno == EAGAIN)
			goto ok;
...
#define	ARGOFF	0
#endif
#if __ppc	/* stack grows down, 16 byte-aligned */
#define	ALIGN(sp)	((sp) & ~((sizeof (int)<<2) - 1))
#define	ARGOFF	0
#endif
	...
	struct argdes *argp)	/* argument descriptor array */
{
	...
	if (P->why.pr_nlwp > 1 && P->lwpctlfd < 0) {
		/* save P->why to restore on exit in case Pchoose_lwp() fails */
		save_pstatus = P->why;
		if (Pchoose_lwp(P) != 0)
			goto bad8;
		waschosen = TRUE;
...
#endif
#if __ppc
	sp -= (nargs > 8)? sizeof (int)*(nargs-8) : 0;
	sp -= sizeof (struct minframe);
	sp = ALIGN(sp);
	ap = sp+sizeof (struct minframe);	/* address of arg dump area */
#endif
...
int
is_empty(const uint32_t *sp,	/* pointer to set (array of longs) */
	unsigned n)		/* number of longs in set */
{
	if (n) {
		do {
			if (*sp++)
