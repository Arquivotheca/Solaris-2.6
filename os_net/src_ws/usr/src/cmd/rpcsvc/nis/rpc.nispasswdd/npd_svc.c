...
#pragma ident	"@(#)npd_svc.c	1.7	96/10/19 SMI"
...
passwd update; maybe out-of-sync with YP map -- verify by hand");
	...
	if (error == NPD_SUCCESS) {
	    ...
	    (void) __npd_upd_cred(entry->ul_user, entry->ul_domain,
		"DES", entry->ul_oldpass, pass, &error);
	    if (error == NPD_KEYSUPDATED && verbose == TRUE)
		syslog(LOG_DEBUG, "Generated new key-pair for %s in %s",
		    entry->ul_user, entry->ul_domain);
	    if (error == NPD_KEYNOTREENC && verbose == TRUE)
		syslog(LOG_DEBUG, "Cannot encrypt secret key for %s in %s",
		    entry->ul_user, entry->ul_domain);
