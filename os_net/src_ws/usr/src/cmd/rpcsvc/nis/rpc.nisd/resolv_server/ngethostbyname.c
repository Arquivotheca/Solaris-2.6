...
#pragma ident	"@(#)ngethostbyname.c	1.5	96/05/21 SMI"

/* Taken from 4.1.3 ypserv resolver code. */

#ident	"@(#)ngethostbyname.c	1.5	96/05/21 SMI"
	...
	struct nres    *temp;
{
	...
	if (nres_search(temp) >= 0) {
		...
		temp->question_len = res_mkquery(QUERY, temp->search_name,
					C_IN, type, (u_char *) NULL, 0, NULL,
					(u_char *)temp->question, MAXPACKET);
	...
	struct rpc_as  *as;
{
	...
	if (status > 0) {
		...
		if (verbose && verbose_out) p_query((u_char *)temp->answer);
	} else if (status < 0) {
