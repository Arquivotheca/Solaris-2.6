...
#ident	"@(#)nisprefadm.c	1.5	96/10/25 SMI"
...
main(int argc, char **argv)
{
	...
	if (op == OP_FLUSH) {
		...
		if (geteuid() != (uid_t)0) {
			fprintf(stderr, "%s: -F option must be run as root.\n",
				argv[0]);
			exit(1);
		}
		...
		if (system(cmd) != 0) {
			...
			fprintf(stderr,
			"%s: Unable to flush the cache manager.", argv[0]);
			fprintf(stderr,
...
get_servers(server_list *servers, char *domain, char *client, int create)
{
	...
	sprintf(name, "[client=%s],%s.org_dir.%s", client, TABLE, domain);
	...
	if (!parse_info(servers, info, &hosts, &options)) {
		printf("Error found while parsing the table information\n");
...
update_servers(server_list *servers, char *domain)
{
	...
	cols[1].ec_flags = 0;
	cols[1].ec_value.ec_value_val = buf;
	cols[1].ec_value.ec_value_len = strlen(buf) + 1;
