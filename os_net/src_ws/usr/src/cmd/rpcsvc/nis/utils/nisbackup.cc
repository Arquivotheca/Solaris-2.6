...
#pragma ident	"@(#)nisbackup.cc	1.6	96/09/08 SMI"
...
static int 		flags = 0;
static char 		*nis_local_dir = nis_local_directory();
...
_mv_dir(char * name, int arg)
{
	...
	switch (child = vfork()) {
		case -1: /* error  */
			...
			return (1);
		case 0:  /* child  */
			execlp("/usr/bin/mv", "mv", src, dst, NULL);
			exit(0); /* Since this is the child proc, we'll exit */
