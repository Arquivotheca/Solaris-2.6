...
#pragma ident	"@(#)nisgrep.c	1.16	96/05/13 SMI"
...
usage()
{
	fprintf(stderr, "usage: nisgrep [-AMchvo] [-s sep] keypat tablename\n");
	fprintf(stderr,
	    "       nisgrep [-AMchvo] [-s sep] colname=keypat ... tablename\n");
	exit(EXIT_ERROR);
}
	...
	char *argv[];
{
	...
	pld.ta_sep = '\0';
	while ((c = getopt(argc, argv, "AMchvos:")) != -1) {
		switch (c) {
		case 'A':
			allres = ALL_RESULTS;
			break;
