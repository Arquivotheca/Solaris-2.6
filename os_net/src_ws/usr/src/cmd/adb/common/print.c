...
#ident	"@(#)print.c	1.57	96/07/28 SMI"
...
print_dis(int regnum, int pc)
{
	...
	if (!(Prstatus.pr_flags & PR_PCINVAL)) {
		 ...
		 */
		if (Prstatus.pr_lwp.pr_instr == bpt && (bp = bkptlookup(dot)))
			Prstatus.pr_lwp.pr_instr = (long) bp->ins;
		if (bp && (bp->flag == BKPT_TEMP))
			bp->flag = 0;
		printins('i', ISP,
		    ((regnum == Reg_PC && Prstatus.pr_lwp.pr_instr) ?
		    Prstatus.pr_lwp.pr_instr : t_srcinstr(chkget(dot, ISP))));
#if !defined(__ppc)
		printc('\n');
#endif
		return;
