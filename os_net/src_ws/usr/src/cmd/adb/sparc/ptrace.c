...
#pragma ident "@(#)ptrace.c 1.16	96/10/08 SMI"
...
int
ptrace(int request, int pid, int addr, int data, caddr_t addr2)
{
	...
	db_printf(5, "ptrace: request=%s, pid=%D, addr=%X, data=%X, addr2=%X",
...
OpenProc(pid_t pid)
{
	 ...
	 */
	omode = (asfd > 0)? O_RDWR : (O_RDWR|O_EXCL);
...
FetchAuxv()
{
	static auxv_t auxv[NUM_AUX_VECTORS+4];
	char procname[64];	/* /proc/<pid>/auxv */
	int fd;

	(void) memset((char *)auxv, 0, sizeof (auxv));
	MakeProcName(procname, process_id);
	(void) strcat(procname, "/auxv");
	if ((fd = open(procname, O_RDONLY, 0)) >= 0) {
		(void) read(fd, (char *)auxv, sizeof (auxv));
		(void) close(fd);
	}
