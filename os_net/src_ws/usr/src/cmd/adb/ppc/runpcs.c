...
#pragma ident "@(#)runpcs.c 1.7	96/07/28 SMI"
	...
	register struct bkpt *bp;
{
	if (bp->type == BPACCESS) {
		...
		db_printf(2, "setbrkpt: setting @ bp->loc 0x%x\n", bp->loc);
...
rewait:
	...
	if ((stat & WSTOPFLG) != WSTOPFLG) {
		...
		if (signo != SIGTRAP) {
			 ...
			 */
			if (Prstatus.pr_lwp.pr_info.si_code == TRAP_BRKPT) {
				db_printf(2, "bpwait: BREAKPOINT");
				pc_adjust = 0;	/* in case of PPC */
			}
...
bpwait ( int mode )		/* at386 version */
{
	...
	if (istrap) {
		...
		ishtrap = 0;
		/* XXXPPC setdr6(0); - hardware breakpoints */
	}
#ifdef DEBUG
	if (debugging >= 1) {
...
setup()
{
...
#endif
	{
		...
		error("try again");
	/* Wait for child to stop and set signal tracing flags. */
	prfillset(&signals);
	if ((fd = openproc(pid, O_RDWR)) < 0 || ioctl(fd, PIOCWSTOP, 0) != 0 ||
	                              ioctl(fd, PIOCSTRACE, (int) &signals) != 0)
		error("unable to trace child");
	(void) close(fd);
#endif
