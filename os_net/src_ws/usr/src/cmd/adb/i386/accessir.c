...
#ident "@(#)accessir.c	1.6	96/06/18 SMI"
	...
	register struct stackpos *sp;
{
	...
	db_printf(1, "stacktop: XXX why only 14 registers?");
	return findentry(sp);
}
...
#	define	BYTEMASK	0xff
	...
	else if ((get(addr - 3, ISP) & BYTEMASK) == CALLEDI &&
		 (get(addr - 2, ISP) & CALLRMASK) == CALLDISP8 ) {
		if (findsym(sp->k_pc, ISYM))
			sp->k_entry = cursym->s_value;
		else
			sp->k_entry = MAXINT;
	} else if ((get(addr - 6, ISP) & BYTEMASK) == CALLEDI &&
	...
	if ((instruc & BYTEMASK) == ADDIMMBYTE)
		...
		sp->k_nargs = 1;
	else if (!findsym(sp->k_entry, ISYM) && !strcmp(cursym->s_name, "main"))
		sp->k_nargs = 3;
	else
		sp->k_nargs = 0;
#ifndef KADB
	...
	register struct stackpos *sp;
{
	...
	calladdr = (caddr_t) sp->k_caller;
	addr     = (caddr_t) sp->k_entry;
	db_printf(2, "nextframe: calladdr=%X, addr=%X", calladdr, addr);
