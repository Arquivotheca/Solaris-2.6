...
#pragma ident	"@(#)volrmmount.c	1.10	96/10/10 SMI"
...
main(int argc, char **argv)
{
	...
	static bool_t	work(action_t, char *, char *, bool_t);
...
#define	TEXT_DOMAIN	"SYS_TEST"
	...
	if (argc == optind) {
		...
		if (!work(user_act, vol, rmm_config, do_debug)) {
			ret_val = 1;
		}
	} else {
		/* multiple thingys to handle */
		for (; optind < argc; optind++) {
			if (!work(user_act, argv[optind], rmm_config,
			    do_debug)) {
				ret_val = 1;
			}
...
usage(char *prog_name)
{
	(void) fprintf(stderr,
	    gettext(
	    "\nusage: %s [-D] [-c CONFIG] [-i | -e] [NAME | NICKNAME]\n"),
	    prog_name);
	(void) fprintf(stderr,
	    gettext("or:    %s -d\n"), prog_name);
	(void) fprintf(stderr,
 ...
 */
static bool_t
work(action_t user_act, char *vol, char *rmm_config, bool_t do_debug)
{
	static bool_t	access_ok(char *);
	static bool_t	setup_env(action_t, char *);
	static bool_t	call_rmmount(char *, bool_t);
	char		*path;
	bool_t		res = FALSE;		/* return value */
	...
	/* handle the request */
	if (!call_rmmount(rmm_config, do_debug)) {
		goto dun;
	}
...
static bool_t
call_rmmount(char *rmm_config, bool_t do_debug)
{
	pid_t	fork_pid;
