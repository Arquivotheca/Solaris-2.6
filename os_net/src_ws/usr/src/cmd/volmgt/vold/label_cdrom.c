...
#pragma ident	"@(#)label_cdrom.c	1.35	96/10/10 SMI"
...
{
	static bool_t	findcdivol(int, u_char *);
	static bool_t	findhsvol(int, u_char *);
	static bool_t	findisovol(int, u_char *);
	char		*cd_nm_ptr = NULL;
	size_t		cd_nm_sz;
	char		*nm;


	if (findcdivol(fd, pvd)) {
		cd_nm_ptr = (char *)ISO_vol_id(pvd);
		cd_nm_sz = ISO_VOL_ID_STRLEN;
	} else if (findhsvol(fd, pvd)) {
		cd_nm_ptr = (char *)HSV_vol_id(pvd);
		cd_nm_sz = HSV_VOL_ID_STRLEN;
	} else if (findisovol(fd, pvd)) {
		cd_nm_ptr = (char *)ISO_vol_id(pvd);
		cd_nm_sz = ISO_VOL_ID_STRLEN;
	}
