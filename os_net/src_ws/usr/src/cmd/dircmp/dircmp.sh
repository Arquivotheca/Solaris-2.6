...
#	All rights reserved.
#ident	"@(#)dircmp.sh	1.10	96/10/07 SMI"	/* SVr4.0 1.13	*/
PATH=/usr/bin
...
rm -f /usr/tmp/dc$$c
ls -ld `cat /usr/tmp/dc$$f` > /usr/tmp/dc$$i 2>/dev/null
cd $D0
cd $D1
ls -ld `cat /usr/tmp/dc$$f` > /usr/tmp/dc$$h 2>/dev/null
cd $D0
> /usr/tmp/dc$$g
#
# Process the results of the 'ls -ld' to obtain file size info and identify a large
# file's existence.
#
while read -u3 tmp tmp tmp tmp fsize1 tmp tmp tmp a &&
      read -u4 tmp tmp tmp tmp fsize2 tmp tmp tmp b; do
	#
	# A window of opportunity exists where the ls -ld above could produce different
	...
	while [[ $a < $b ]]; do
		...
		read -u3 tmp tmp tmp tmp fsize1 tmp tmp tmp a
	done
	while [[ $a > $b ]]; do
		if (( Sflag != 1 ))
		...
		read -u4 tmp tmp tmp tmp fsize2 tmp tmp tmp b
	done
	fi
	cmpdiff=0
	sizediff=0
