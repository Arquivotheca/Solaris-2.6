...
#pragma ident	"@(#)tnfxtract.c	1.6	95/03/22 SMI"
...
static unsigned long dump_bufaddr;
static unsigned long tnf_bufsize;
...
static void
read_tnf_fwzone(long *dest, int start, int slots)

{
	...
	if (dumpfile != NULL) {
		offset = tnf_header->block_size + start * sizeof (long);
		len = slots * sizeof (long);
...

{
	...
	magicp = (unsigned long *) local_buf;
	/*LINTED pointer cast may result in improper alignment*/
	 ...
	 */
	/*LINTED pointer cast may result in improper alignment*/
	fwzone = (long *)(local_buf + tnf_header->block_size);
	read_tnf_fwzone(fwzone, 0, fwzonesize / sizeof (fwzone[0]));
	fwtries = 0;
