/*
 *	llc1 - an LLC Class 1 MUX compatible with SunConnect LLC2
 *	uses DLPI interface.
 *
...
#pragma ident "@(#)llc1.h	1.1	92/10/15 SMI"
...
struct llc_stats {
   ...
   long llcs_brdcstxmt;
   long llcs_brdcstrcv;
   long llcs_blocked;	/* discard due to upstream being flow controlled */
   long llcs_pktxmt;
   long llcs_pktrcv;
   long llcs_bytexmt;
   long llcs_bytercv;
...
typedef struct llc1_multicast_addr {
   int		 llcm_refcnt;	/* number of streams referring to entry */
...
typedef struct llc1 {
   ...
   long		llc_multicnt;	/* number of multicast addresses for stream */
   llc_mcast_t **llc_mcast;	/* multicast table if multicast is enabled */
   queue_t     *llc_qptr;
   kmutex_t	llc_lock;
...
typedef struct llc1device {
   ...
   int		llc1_minors;
   int		llc1_multisize;
   llc_mac_info_t *llc1_mac_next,
   	          *llc1_mac_prev; /* the various mac layers */
...
#define LLC1_ATTACHED	0x0001	/* board is attached so mutexes are initialized */
...
#define LLC1_LOWATER	2048	/* low water mark for flow control */
