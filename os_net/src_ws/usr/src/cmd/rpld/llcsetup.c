#pragma ident "@(#)llcsetup.c 1.19	94/04/27 SMI"
...
dl_info_t	if_info;
	...
	if (need_llc) {
		 ...
		 * So first this will attempt to open llc2 if this fails
		 * it will attempt to open llc1.  If that fails bail out
		 * and error.  This insures that llc2 driver will be 
		 * favored over llc1 if it is present.
		 */

		llcdev = "/dev/llc2";
		if (debugLevel >= MSG_INFO_2) {
			sprintf(debugmsg, "Using llc device %s.\n", llcdev);
			senddebug(MSG_INFO_2);
		}
