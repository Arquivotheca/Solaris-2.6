...
#pragma	ident	"@(#)visinstr.c	1.5	96/08/19 SMI"
 ...
 */
enum ftt_type
vis_fpu_simulator(pfpsd, pinst, pregs, pwindow, fp)
	...
	kfpu_t		*fp;	/* Need to fp to access gsr reg */
{
	...
	nrd = pinst.rd;
	i.pinst = pinst;
	if ((i.inst.opf & 1) == 0) {		/* double precision */
	...
	case alignaddr:
	case alignaddrl:
		ftt = vis_alignaddr(pfpsd, i.inst, pregs, pwindow, fp);
		break;
	case fcmple16:
	case fcmpne16:
	case fcmpgt16:
	...
	case fcmpgt32:
	case fcmpeq32:
		ftt = vis_fcmp(pfpsd, i.inst, pregs, pwindow);
		break;
	case fmul8x16:
	case fmul8x16au:
	case fmul8x16al:
	...
	case fexpand:
	case fpmerge:
		ftt = vis_fpixel(pfpsd, i.inst, fp);
		break;
	case pdist:
...
vis_alignaddr(pfpsd, inst, pregs, pwindow, fp)
	fp_simd_type	*pfpsd;	/* FPU simulator data. */
	...
	struct v9_fpu	*fp;	/* Need to fp to access gsr reg */
{
	...
	if (inst.opf == alignaddrl) {
		...
		if (s < 0)
			r = (u_longlong_t)((s + 1) & 0x7);
		else
			r = (u_longlong_t)(s & 0x7);
	}
	g |= r;
	set_gsr((caddr_t)&g, fp);
 ...
 */
static enum ftt_type
vis_fcmp(pfpsd, inst, pregs, pwindow)
	fp_simd_type	*pfpsd;	/* FPU simulator data. */
	vis_inst_type	inst;	/* FPU instruction to simulate. */
	struct regs	*pregs;	/* Pointer to PCB image of registers. */
	struct rwindow	*pwindow; /* Pointer to locals and ins. */
{
	...
	ftt = write_iureg(pfpsd, nrd, pregs, pwindow, &krd.ll);
 ...
 */
enum ftt_type
vis_fldst(pfpsd, pinst, pregs, pwindow, pfpu, asi)
	fp_simd_type	*pfpsd;	/* FPU simulator data. */
	fp_inst_type	pinst;	/* FPU instruction to simulate. */
	struct regs	*pregs;	/* Pointer to PCB image of registers. */
	struct rwindow	*pwindow; /* Pointer to locals and ins. */
 ...
 */
static enum ftt_type
vis_prtl_fst(pfpsd, inst, pregs, pwindow, pfpu, asi)
	fp_simd_type	*pfpsd;	/* FPU simulator data. */
	...
	unsigned	asi;	/* asi to emulate! */
{
	...
	case ASI_PST8_P:
	case ASI_PST8_S:
		ftt = _fp_read_extword((u_longlong_t *) ea, &l.ll, pfpsd);
		if (ftt != ftt_none)
			return (ftt);
		...
		ftt = _fp_write_extword((u_longlong_t *) ea, res.ea, pfpsd);
		if (ftt != ftt_none)
			return (ftt);
		break;
 ...
 */
static enum ftt_type
vis_short_fls(pfpsd, inst, pregs, pwindow, pfpu, asi)
	fp_simd_type	*pfpsd;	/* FPU simulator data. */
	vis_inst_type	inst;	/* ISE instruction to simulate. */
	struct regs	*pregs;	/* Pointer to PCB image of registers. */
	struct rwindow	*pwindow; /* Pointer to locals and ins. */
	...
	unsigned	asi;	/* asi to emulate! */
{
	...
	u_short us;
	u_char uc;
	extern int subyte(), susword();
	extern int fubyte(), fusword();
 ...
 */
static enum ftt_type
vis_blk_fldst(pfpsd, inst, pregs, pwindow, pfpu, asi)
	fp_simd_type	*pfpsd;	/* FPU simulator data. */
	vis_inst_type	inst;	/* ISE instruction to simulate. */
	struct regs	*pregs;	/* Pointer to PCB image of registers. */
	struct rwindow	*pwindow; /* Pointer to locals and ins. */
	...
	unsigned	asi;	/* asi to emulate! */
{
	...
	if ((nrd & 1) == 1) 		/* fix register encoding */
		nrd = (nrd & 0x1e) | 0x20;
	if ((nrd & 0x7) != 0)		/* check the 8 register boundary */
		return (ftt_unimplemented);
	opf = inst.opf;
	if ((opf & 0x100) == 0) { 	/* effective address = rs1 + rs2 */
		ftt = read_iureg(pfpsd, nrs1, pregs, pwindow, &k.ea);
 ...
 */
enum ftt_type
vis_rdgsr(pfpsd, pinst, pregs, pwindow, fp)
	fp_simd_type	*pfpsd;	/* FPU simulator data. */
	fp_inst_type	pinst;	/* FPU instruction to simulate. */
	struct regs	*pregs;	/* Pointer to PCB image of registers. */
	struct rwindow	*pwindow; /* Pointer to locals and ins. */
	struct v9_fpu	*fp;	/* Need to fp to access gsr reg */
{
	...
	nrd = pinst.rd;
	get_gsr(fp, (caddr_t)&r);
	if (r > 0x7f)
		cmn_err(CE_CONT, "vis_rdgsr: gsr 0x%x\n", (u_int)r);
	ftt = write_iureg(pfpsd, nrd, pregs, pwindow, &r);
	pregs->r_pc = pregs->r_npc;	/* Do not retry emulated instruction. */
	pregs->r_npc += 4;
	return (ftt);
 ...
 */
enum ftt_type
vis_wrgsr(pfpsd, pinst, pregs, pwindow, fp)
	fp_simd_type	*pfpsd;	/* FPU simulator data. */
	fp_inst_type	pinst;	/* FPU instruction to simulate. */
	struct regs	*pregs;	/* Pointer to PCB image of registers. */
	struct rwindow	*pwindow; /* Pointer to locals and ins. */
	struct v9_fpu	*fp;	/* Need to fp to access gsr reg */
{
	...
	r.l = r1.l = r2.l = 0;
	nrs1 = pinst.rs1;
	ftt = read_iureg(pfpsd, nrs1, pregs, pwindow, &r1.l);
	if (ftt != ftt_none)
		return (ftt);
	if (pinst.ibit == 0) {	/* copy the value in r[rs2] */
		...
		nrs2 = pinst.rs2;
		ftt = read_iureg(pfpsd, nrs2, pregs, pwindow, &r2.l);
		if (ftt != ftt_none)
			return (ftt);
		...
		fk.fi = pinst;		/* Extract simm13 field */
		r2.i[1] = (fk.i << 19) >> 19;
	}
	r.i[1] = r1.i[1] ^ r2.i[1];
	r.i[1] &= 0x7f;
	set_gsr((caddr_t)&r, fp);
	pregs->r_pc = pregs->r_npc;	/* Do not retry emulated instruction. */
	pregs->r_npc += 4;
	return (ftt);
}
