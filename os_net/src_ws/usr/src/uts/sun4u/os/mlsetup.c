...
#pragma ident	"@(#)mlsetup.c	1.47	96/05/20 SMI"
...
{
	struct machpcb *mpcb;
	unsigned long long phys_msgbuf;
	u_int msgbufsz;
	...
	if (prom_retain("msgbuf", msgbufsz, MMU_PAGESIZE, &phys_msgbuf) != 0)
	...
	if ((boothowto & RB_DEBUG) != 0) {
		if (dvec == NULL || ddi_peeks((dev_info_t *)0,
			 ...
			 * to work correctly.
			 */
			(*dvec->dv_scbsync)(KADB_CALLB_TRAP, (void *)0);
		}
	}
#endif
