...
#pragma ident	"@(#)sysiosbus.h	1.51	96/09/24 SMI"
...
struct sbus_soft_state {
	...
	volatile u_ll_t *iommu_flush_reg; /* IOMMU regs */
	volatile u_ll_t *iommu_ctrl_reg;
	volatile u_ll_t *tsb_base_addr;	  /* Hardware reg for phys TSB base */
	volatile u_ll_t *soft_tsb_base_addr; /* virtual address of TSB base */
	volatile u_ll_t *iommu_tlb_tag;
	volatile u_ll_t *iommu_tlb_data;
	u_long iommu_dvma_size;
	u_long iommu_dvma_base;
	u_long iommu_dvma_pagebase;

	volatile u_ll_t *sysio_ctrl_reg;	/* sysio regs */
	volatile u_ll_t *sbus_ctrl_reg;	/* Reg also used to flush store bufs */
	volatile u_ll_t *sbus_slot_config_reg;
	u_long sbus_slave_burstsizes[MAX_SBUS_SLOTS];

	volatile u_ll_t *intr_mapping_reg;	/* Interrupt regs */
	volatile u_ll_t *clr_intr_reg;
	volatile u_ll_t *intr_retry_reg;
	volatile u_ll_t *sbus_intr_state;
	volatile u_ll_t *obio_intr_state;
	u_char intr_hndlr_cnt[MAX_SBUS_SLOT_ADDR];
	u_char spurious_cntrs[MAX_PIL + 1]; /* Spurious intr counter */

	volatile u_ll_t *sysio_ecc_reg;		/* sysio ecc control reg */
	volatile u_ll_t *sysio_ue_reg;		/* sysio ue ecc error regs */
	volatile u_ll_t *sysio_ce_reg;		/* sysio ce ecc error regs */
	volatile u_ll_t *sbus_err_reg;		/* sbus async error regs */

	volatile u_ll_t *str_buf_ctrl_reg;	/* streaming buffer regs */
	volatile u_ll_t *str_buf_flush_reg;
	volatile u_ll_t *str_buf_sync_reg;
	volatile u_ll_t *str_buf_pg_tag_diag;
	int stream_buf_off;
