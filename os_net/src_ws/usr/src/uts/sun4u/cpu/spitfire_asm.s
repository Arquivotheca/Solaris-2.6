...
#pragma ident	"@(#)spitfire_asm.s	1.28	96/09/12 SMI"
...
/* ARGSUSED */
void
vtag_flushpage_tl1(caddr_t vaddr, u_int ctxnum)
{
}

/* ARGSUSED */
void
vtag_flushctx_tl1(u_int ctxnum)
{
}

/* ARGSUSED */
void
vac_flushpage(u_int pfnum, int vcolor)
{
}

/* ARGSUSED */
void
vac_flushpage_tl1(u_int pfnum, int vcolor)
{
}
...
/* ARGSUSED */
void
flush_instr_mem(caddr_t vaddr, u_int len)
{
}

/* ARGSUSED */
void
flush_ecache(u_longlong_t physaddr, u_int size)
{
}

void
enable_internal_caches(void)
{
}
	...
	/*
	 * flush page from the tlb and tsb
	 *
	 * %o0 = vaddr
	 * %o1 = ctxnum
	 */
...
3:
	...
	/*
	 * First blow out the tsb
	 * no locking is required since only the cpu who owns the tsb
	 * is allowed to update it.
	 */
	or	%o0, %o1, %o4			/* build tagaccess reg */
	GET_TSB_POINTER(%o4, %g1, %g2, %g3, %g4)	/* g1 = tsbp */
	ldxa	[%g1] ASI_MEM, %g3			/* g3 = tsb tag */
	BUILD_TSB_TAG(%o0, %o1, %g2, %o2)		/* g2 = tag target */
	cmp	%g3, %g2
	bne,pt	%xcc, 1f
	add	%g1, TSBE_TAG + TSBTAG_INTHI, %g1
	sethi	%hi(TSBTAG_INVALID), %o3
	sta	%o3, [%g1] ASI_MEM			/* inv tsbe */
1:
	membar	#StoreStore
...
5:
	...
	sllx	%g1, MMU_PAGESHIFT, %g1			/* g1 = vaddr */
	or	%g1, %g2, %g3		/* build tagaccess reg */
	GET_TSB_POINTER(%g3, %g4, %g5, %g6, %g7)	/* g4 = tsbp */
	ldxa	[%g4] ASI_MEM, %g5			/* g5 = tsb tag */
	BUILD_TSB_TAG(%g1, %g2, %g3, %g6)		/* g3 = tag target */
	cmp	%g5, %g3
	bne,pt	%xcc, 1f
	 sethi	%hi(TSBTAG_INVALID), %g6
	sta	%g6, [%g4] ASI_MEM			/* inv tsb entry */
1:
	/*
	 *
 ...
 */
	ENTRY(flush_ecache)
#ifdef __sparcv9
	srl	%o1, 0, %g1		! store ecache size to flush in %g1
	mov	%o0, %g2		! and 64 bit physaddr in %g2
#else
	srl	%o2, 0, %g1		! store ecache size to flush in %g1

	sllx	%o0, 32, %o0		! shift upper 32 bits
	srl	%o1, 0, %o1		! clear upper 32 bits
	or	%o0, %o1, %g2		! form 64 bit physaddr in %g2
					! using (%o0, %o1)
#endif
	...
	rdpr	%pstate, %g4
	andn	%g4, PSTATE_IE | PSTATE_AM, %g5
	wrpr	%g0, %g5, %pstate	! clear AM to access 64 bit physaddr
	b	2f
	nop
1:
	ldxa	[%g2+%g1]ASI_MEM, %g0	! start reading from physaddr + size
2:
	subcc	%g1, %g3, %g1
	bcc,a	1b
	nop
...
/*
 * enable_internal_caches: re-enable the i$ and d$
 */
	ENTRY(enable_internal_caches)
	ldxa	[%g0]ASI_LSU, %g1	! read LSU
	or	%g1, LSU_IC | LSU_DC, %g1	! Turn On IC and DC bits
	stxa	%g1, [%g0]ASI_LSU	! Store to LSU
	membar	#Sync
	retl
	nop
	SET_SIZE(enable_internal_caches)
 ...
 * %g2: [ 61:56 tt=[0xA|0x32] | 55:53 tl=0 |
 *	  52:43 UDB lower | 42:33 UDB upper |32:0 afsr ] - arg #3/arg #1
 * %g3: [ 40:4 afar ] - sys_trap->have_win: arg #4/arg #2
 */
void
async_err(void)
...
2:
	...
	subcc	%g6, 0xf, %g0
	movneg	%icc, 0xe, %g4		! run at pil 14 unless already at 15
	sethi	%hi(sys_trap), %g5
	jmp	%g5 + %lo(sys_trap)	! goto sys_trap
	movge	%icc, 0xf, %g4		! already at pil 15
	SET_SIZE(ce_err)
