...
#pragma ident	"@(#)copy.s	1.33	96/06/18 SMI"
...
{ return (0); }
	...
	swap	[THREAD_REG + T_LOFAULT], %l6	! save t_lofault, no tramp
...
{ return (0); }
	...
	save	%sp, -424, %sp		! get another window
	set	.copyioerr, %l6		! .copyioerr is lofault value
	swap	[THREAD_REG + T_LOFAULT], %l6	! set/save t_lofault, no tramp
...
{}
	...
	save	%sp, -424, %sp	! get another window
	ld	[THREAD_REG + T_LOFAULT], %l6
	brz,pn	%l6, .do_copyin
	  nop
	set	.copyioerr, %o2
	st	%o2, [THREAD_REG + T_LOFAULT]	! set/save t_lofault
	b	.do_copyin
	  or	%l6, 2, %l6		! error should trampoline
