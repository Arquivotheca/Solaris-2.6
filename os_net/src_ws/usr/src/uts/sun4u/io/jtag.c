...
#pragma ident	"@(#)jtag.c 1.8	96/01/30 SMI"
...
#ifdef notyet
static int jtag_check_board(volatile u_int *, int, enum board_type, u_char *);
#endif	/* notyet */

/* Verify the structure of a ring */
static int verify_jtag_ring(volatile u_int *, jtag_ring);
...
static int jtag_single_IR_DR(volatile u_int *, jtag_phys_comp *,
	jtag_instruction, u_char *, int, u_char *);
static int jtag_single_check_ID(volatile u_int *, jtag_phys_comp *);
...
static int jtag_init_dc(volatile u_int *, int, int);
static int jtag_init_ac(volatile u_int *, int, int);

#ifdef notyet
static int jtag_init_fhc(volatile u_int *, int);
#endif	/* notyet */
...
#define	XIR_LEN (SIO_LEN+SIO_LEN+HM_LEN+NDP_LEN+SOC_LEN)
...
static u_char ac_init_pdesc[] = {
	JTIN_INSERT|0, 159, 161, 7,	/* BOARD ADDR 40:38 */
	...
	JTIN_INSERT|0, 88, 98, 0,	/* Error Masks */
	JTIN_INSERT|0, 0, 7, 0x55,
...
jtag_phys_comp io1_sysbd_ring_components[] = {
...
jtag_phys_comp io2_sysbd_ring_components[] = {
...
jtag_phys_comp dsk_sysbd_ring_components[] = {
 ...
 */
jtag_phys_comp ffb_sngl_ring_components[] = {
...
jtag_phys_comp ffb_dbl_ring_components[] = {
...
get_ring_descriptor(int ring, int ring_len)
{
	...
	default:
		return ((jtag_ring_desc *)0);
	}
}

enum board_type
jtag_get_board_type(volatile u_int *jreg, int board)
{
	int len;
	int ring;
	int result;
...
/*
 * This function is meant as the front end for all JTAG diagnostics
 * performed automatically during hotplug testing.
 *
 * XXX - Extensive board testing will be initiated from this function.
 */
int
jtag_integrity_scan(volatile u_int *jreg, int board)
{
	if (verify_jtag_ring(jreg, board << 4) != JTAG_OK) {
		return (-1);
	} else {
		return (0);
	}
...
int
jtag_get_board_info(volatile u_int *jreg, struct bd_info *info)
{
	jtag_ring_desc *rd;
	jtag_phys_comp *rc;
	int status;
	...
	rd = get_ring_descriptor(ring, jtag_ring_length(jreg, ring));
	...
	if (rc != NULL) {
		info->ac_compid = jtag_get_comp_id(jreg, rc);
	}

	/* get the FHC component ID */
	rc = find_chip(rd, &chip_fhc, 0);
	if (rc != NULL) {
		info->fhc_compid = jtag_get_comp_id(jreg, rc);
	}
	...
	case CPU_BOARD:
		...
		for (i = 0, cpu = &info->bd.cpu[i]; i < 2; i++, cpu++) {
	...
	case IO_2SBUS_BOARD:
		...
		if (rc != NULL) {
			info->bd.io1.sio0_compid =
				jtag_get_comp_id(jreg, rc);
		}

		rc = find_chip(rd, &chip_sio, 1);
		if (rc != NULL) {
			info->bd.io1.sio1_compid =
				jtag_get_comp_id(jreg, rc);
		}

		rc = find_chip(rd, &chip_hm, 0);
		if (rc != NULL) {
			info->bd.io1.hme_compid = jtag_get_comp_id(jreg, rc);
		}

		rc = find_chip(rd, &chip_soc, 0);
		if (rc != NULL) {
			info->bd.io1.soc_compid = jtag_get_comp_id(jreg, rc);
		}
	...
	case IO_SBUS_FFB_BOARD:
		rc = find_chip(rd, &chip_sio, 0);
		if (rc != NULL) {
			info->bd.io2.sio1_compid = jtag_get_comp_id(jreg, rc);
		}

		rc = find_chip(rd, &chip_hm, 0);
		if (rc != NULL) {
			info->bd.io2.hme_compid = jtag_get_comp_id(jreg, rc);
		}

		rc = find_chip(rd, &chip_soc, 0);
		if (rc != NULL) {
			info->bd.io2.soc_compid = jtag_get_comp_id(jreg, rc);
		}
		...
		case FFB_SNG_LEN:
			rd = &ffb_sngl_ring;
			info->bd.io2.ffb_size = FFB_SINGLE;
			break;

		case FFB_DBL_LEN:
			rd = &ffb_dbl_ring;
			info->bd.io2.ffb_size = FFB_DOUBLE;
			break;

		case RING_BROKEN:
			rd = NULL;
			info->bd.io2.ffb_size = FFB_NOT_FOUND;
			break;

		default:
			rd = NULL;
			info->bd.io2.ffb_size = FFB_FAILED;
			break;
		}
		...
		if (rc != NULL) {
			info->bd.io2.fbc_compid = jtag_get_comp_id(jreg, rc);
		}
		break;
	...
	case DISK_BOARD:
		...
		if ((bct_data.disk0_pres && 0x1) == 0) {
			info->bd.dsk.disk_pres[0] = 1;
			info->bd.dsk.disk_id[0] = 0xf & ~bct_data.disk0_id;
		} else {
			info->bd.dsk.disk_pres[0] = 0;
		}

		if ((bct_data.disk1_pres && 0x1) == 0) {
			info->bd.dsk.disk_pres[1] = 1;
			info->bd.dsk.disk_id[1] = 0xf & ~bct_data.disk1_id;
		} else {
			info->bd.dsk.disk_pres[1] = 0;
		}
...
/*
 *    Ring Verification routines
 */

/*
 * Verify that the target ring contains what is advertised.
 * This should be called to check which rings are populated,
 * and it would not hurt either to call it to check rings
 * which are normally populated. This avoids checking the
 * component ID each time the ring is selected!
 */
static int
verify_jtag_ring(
	volatile u_int *jreg,
	jtag_ring ring)
{
	...
	rd = get_ring_descriptor(ring, len);
	if (rd == NULL) {
		(void) select_ring(jreg, ring, 1);
		return (BAD_ARGS);
	}


	/* Verify that the length agrees with each component */
	for (i = rd->size, rc = rd->components; i != 0; rc++, i--) {
		if (len != (rc->by_after + rc->by_before+1)) {
			(void) select_ring(jreg, ring, 1);
	...
	for (i = rd->size, rc = rd->components; i != 0; rc++, i--) {
		status = jtag_single_check_ID(jreg, rc);
	...
	return (JTAG_OK);
}	/* end of verify_jtag_ring() */

/*
 * Each board has an AC asic freeze/unfreeze.
 */
static int
jtag_init_ac(volatile u_int *jreg, int bid, int freeze)
{
	int rc = JTAG_OK;
	int ring = (bid<<4);
	ac_options ac_opt;
	...
	ac_opt.frozen = freeze;
	ac_opt.reset_a = 0;
	ac_opt.reset_b = 0;
	ac_opt.board_id = bid;
	ac_opt.mask_hwerr = 0;

	if ((get_board_type(bid) == CPU_BOARD) ||
	    (get_board_type(bid) == MEM_BOARD)) {
		rc = jtag_init_chip(jreg, ring, &cpu_sysbd_ring_components[0],
			(jtag_opt)&ac_opt, scan_out);
	} else if (get_board_type(bid) == IO_2SBUS_BOARD) {
		rc = jtag_init_chip(jreg, ring, &io1_sysbd_ring_components[0],
			(jtag_opt)&ac_opt, scan_out);
	} else if (get_board_type(bid) == IO_SBUS_FFB_BOARD) {
		rc = jtag_init_chip(jreg, ring, &io2_sysbd_ring_components[0],
			(jtag_opt)&ac_opt, scan_out);
	} else {
		cmn_err(CE_NOTE, " Unsupported Board type %2X\n",
			get_board_type(bid));
	}
...
/*
 * Each board has 8 DC asics freeze/unfreeze them one at a time.
 * TODO - Optimize the switch better in this function. York
 */
static int
jtag_init_dc(volatile u_int *jreg, int bid, int freeze)
{
	int i;
	u_int rscan = 0;
	int rc;
	...
	if ((get_board_type(bid) == CPU_BOARD) ||
		...
		for (i = 1; i <= 8; i++) {
			do {
				if ((rc = jtag_init_chip(jreg, ring,
				    &cpu_sysbd_ring_components[i],
				    (jtag_opt)&dc_opt, &scan_out[0])) < 0) {
					result = rc;
				}

				if ((rc = jtag_scanout_chip(jreg, ring,
				    &cpu_sysbd_ring_components[i],
				    &rscan)) < 0) {
					result = rc;
				}
			} while (rscan != dc_opt.frozen);
		}
	} else if (get_board_type(bid) == IO_2SBUS_BOARD) {
		for (i = 1; i <= 8; i++) {
			do {
				if ((rc = jtag_init_chip(jreg, ring,
				    &io1_sysbd_ring_components[i],
				    (jtag_opt)&dc_opt, &scan_out[0])) < 0) {
					result = rc;
				}
...
/*
 * TODO - Fill in the comment header.
 *
...
#ifdef notyet
static int
jtag_init_fhc(volatile u_int *jreg, int board)
{
	int rc = JTAG_OK;
	int ring = (board << 4);
	fhc_options fhc_opt;
...
/*
 * TODO - Fill in the commect header.
 */
int
	...
	u_int *fhc_csr, u_int *fhc_bsr)
{
	...
	fhc_opt.csr_hi = 0;
	fhc_opt.csr_mid = MOD_OFF | ACDC_OFF | EPDA_OFF | EPDB_OFF |
		NOT_BRD_PRESENT;
 ...
 */
int
jtag_init_disk_board(volatile u_int *jreg, int board, int hotplug,
	u_int *fhc_csr, u_int *fhc_bsr)
{
	int rc = JTAG_OK;
	fhc_options fhc_opt;
	...
	if (hotplug) {
		...
		fhc_opt.csr_midlo = BRD_LED_RIGHT;
	}
 ...
 */
static int
jtag_check_board(volatile u_int *jreg, int board, enum board_type type,
	u_char *scan_out)
{
	int rc = JTAG_OK;
	fhc_options fhc_opt;
...
/*
 * jtag_enable_firetruck()
 *
 * This routine freezes or unfreezes the AC and DC ASICs on a Sunfire
 * board. This is done in order to bring a board onto the Firetruck
 * backplane or to remove one from the Firetruck backplane. This is
 * to support hotplug/hot unplug features. Special care must be taken
 * not to corrupt backplane arbitration, data or parity while a board
 * is transitioning off of or onto the backplane.
 */
int
jtag_enable_firetruck(volatile u_int *jreg, int board, int freeze)
{
	int result;

	/* Do not attempt to init the DCs if the AC init fails. */
	if ((result = jtag_init_ac(jreg, board, freeze)) == -1) {
		cmn_err(CE_WARN, "JTAG init of Board %d AC failed", board);
		return (result);
	}

	if ((result = jtag_init_dc(jreg, board, freeze)) == -1) {
		cmn_err(CE_WARN, "JTAG init of Board %d DCs failed", board);
	}
...
/*
 * Get the component ID for the specified component & check
 * that it matches the list of allowed component IDs.
 * Notice that the revision number (4 msb) is checked to be
 * at least what is in the tables, but is accepted if larger
 * to allow future revisions of the chip.
 */
int
jtag_single_check_ID(volatile u_int *jreg, jtag_phys_comp *comp)
{
	u_char b[4];
	int status;
	u_int id, *allowed;

	allowed = comp->chip->id;
	if (allowed == NULL) {
		return (JTAG_OK); /* no ID */
	}

	status = jtag_single_IR_DR(jreg, comp, comp->chip->id_code, b, 32, b);
	if (status < 0) {
		return (status);
	}
	...
	/*
	 * Search for an ID which matches in the low 28 bits, and
	 * is smaller or equal in the high 4 bits (revision number.
	 */
	for (; *allowed != 0; allowed++) {
		if (((*allowed & 0x0FFFFFFF) == (id & 0x0FFFFFFF)) &&
		    ((*allowed >> 28) <= (id >> 28))) {
			return (JTAG_OK);
		}
	}
