...
#pragma ident	"@(#)consplat.c	1.3	95/09/01 SMI"
...
extern	int	prom_getproplen(dnode_t, caddr_t);
extern	int	prom_getprop(dnode_t, caddr_t, caddr_t);
...
i_kbdpath(void)
{
	...
	/*
	 * We only need the rest of this on legacy OBP systems
	 * [...]
	 * This is sun4u code. OBP _must_ be > 2.4, so lack of keyboard
	 * alias indicates something really busted. Give hint of failure.
	 */
	...
	struct termios *termiosp;
{
	...
	for (i = 0; i < ('z'-'a'); i++) {
		...
		prom_printf("Alias for %s is %s, dev_t is %X\n",
			    name, path, tdev);
#endif
		if (tdev == dev) break;	/* Exit loop if found */
	}
	...
	strcpy(name, "ttya-mode"); /* name of option we want */
	...
	switch (buf[ppos+1]) {
		...
		{
			cmn_err(CE_WARN,
				"i_setmodes: Illegal bits-per-char %s", buf);
			return (1);
		}
	}
	...
	switch (buf[ppos+3]) {
		...
		{
			cmn_err(CE_WARN,
				"i_setmodes: Illegal parity %s", buf);
			return (1);
		}
	}
	...
	switch (buf[ppos+5]) {
		...
		{
			cmn_err(CE_WARN,
				"i_setmodes: Illegal stop bits %s", buf);
			return (1);
		}
	}
