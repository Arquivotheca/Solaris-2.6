...
#pragma ident	"@(#)mp_startup.c	1.35	96/10/17 SMI"
...
#include <vm/hat_i86.h>	/* needed for contexts below */
#include <vm/hat.h>
#include <vm/as.h>
...
init_cpu_info(struct cpu *cp)
{
	...
	strcpy(pi->pi_processor_type, "i386");
	if (fpu_exists)
		strcpy(pi->pi_fputypes, "i387 compatible");
}
...
mp_startup_init(register int cpun)
{
	...
	if ((addr = rmalloc(kernelmap, (long)(2 * CLSIZE))) == NULL) {
		cmn_err(CE_PANIC,
		"mp_startup_init: Can't virtual address for cpu_addr "
		"for cpu: %d", cpun);
		/*NOTREACHED*/
	}
	cp->cpu_caddr1 = Sysbase + mmu_ptob(addr);
	cp->cpu_caddr2 = cp->cpu_caddr1 + PAGESIZE;
	cp->cpu_caddr1pte =
		(u_int *)&Sysmap1[mmu_btop(cp->cpu_caddr1 - Sysbase)];
	cp->cpu_caddr2pte =
		(u_int *)&Sysmap1[mmu_btop(cp->cpu_caddr2 - Sysbase)];
	 ...
	 * page aligned
	 */
	tablesp = (struct cpu_tables *)kmem_zalloc(sizeof (struct cpu_tables),
	    KM_NOSLEEP);
	...
	/*
	 * Do we really need the entries for gdt
	...
	dscrp->a_base0015 = (u_int)(&cpus[cpun]) & 0xffff;
	dscrp->a_base1623 = ((u_int)(&cpus[cpun])>>16) & 0xff;
	dscrp->a_base2431 = ((u_int)(&cpus[cpun])>>24) & 0xff;
	...
	cp->cpu_pagedir = &tablesp->ct_pagedir[0];
	bcopy((caddr_t)kernel_only_pagedir, (caddr_t)cp->cpu_pagedir, ptob(1));
	cp->cpu_cr3 = ptob(hat_getkpfnum((caddr_t)cp->cpu_pagedir));
...
mp_cpu_start(struct cpu *cp)
{
	...
	if (cp->cpu_id == getbootcpuid())
		return (EINVAL); 	/* Cannot start boot CPU */
	atomic_orl(&cpu_online_set, cp->cpu_mask);
	return (0);
}
...
mp_cpu_stop(struct cpu *cp)
{
	ASSERT(MUTEX_HELD(&cpu_lock));
	if (cp->cpu_id == getbootcpuid())
		return (EINVAL); 	/* Cannot stop boot CPU */
