...
#pragma	ident	"@(#)isa.c	1.23	96/09/27 SMI"
...
isa_initchild(dev_info_t *child)
{
	...
	if (pnpisa) {
		if (func != 0)
			sprintf(name, "pnp%s,%04x,%x,%x", vendor, device,
				serial, func);
		else
			sprintf(name, "pnp%s,%04x,%x", vendor, device, serial);
	} else {
		sprintf(name, "%x,%x", bustype, base);
	}
