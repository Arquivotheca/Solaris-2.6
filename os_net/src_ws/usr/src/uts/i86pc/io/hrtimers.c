...
#ident "@(#)hrtimers.c	1.5	94/05/09 SMI"
...
extern int xgetitimer(register struct gtimera *uap, rval_t *rvp, int iskaddr);
...
extern int xsetitimer(register struct stimera *uap, rval_t *rvp, int iskaddr);
	...
	rval_t	*rvp;
{
	...
	for (cnt = 0; cnt < uap->cmds; cnt++, hrcmdp++) {
		...
		case HRT_BSD:		/* one-shot timer */
		{
			...
			switch (cp->hrtc_clk) {
			case CLK_STD:
				ua.which = ITIMER_REAL;
				break;
			case CLK_USERVIRT:
				ua.which = ITIMER_VIRTUAL;
				break;
			case CLK_PROCVIRT:
				ua.which = ITIMER_PROF;
				break;
			default:
				error = EINVAL;
				goto bad;
