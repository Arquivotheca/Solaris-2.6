...
#pragma ident	"@(#)seg_vpix.h	1.4	94/07/12 SMI"
/* from SVR4:seg_vpix.h	1.3.1.1 */
 ...
 * This is kept along with some other per-page information in a vpix_page
 * structure.
 */

typedef struct vpix_page {	/* one for every virtual page */
	...
	u_short		rp_errata10:1;	/* some vpage in 1000 - F000 range */
...
/*
 * (Semi) private data maintained by the seg_vpix driver per segment mapping
 *
 * The read/write segment lock protects all of segvpix_data including the
 * vpage array. The "read" version of the address space and the segment
 * locks are held. The "write" version of the segment lock, however, is
 * required in order to update the following fields:
 *
 *	vpage
 */
struct	segvpix_data {
	krwlock_t lock;		/* protect segvpix_data and vpage list */
	...
	int	hatid;		/* unique id used in hat page data collection */
};

#ifdef _KERNEL

/*
 * Macros for segvpix segment driver locking.  These macros are defined
 * to be the same as address space locking macros since the segvn lock
 * also happens to be a readers/writer lock.
 */
#define	SEGVPIX_LOCK_ENTER	AS_LOCK_ENTER
#define	SEGVPIX_LOCK_EXIT	AS_LOCK_EXIT
#define	SEGVPIX_LOCK_DESTROY	AS_LOCK_DESTROY

#define	SEGVPIX_LOCK_HELD	AS_LOCK_HELD
#define	SEGVPIX_READ_HELD	AS_READ_HELD
#define	SEGVPIX_WRITE_HELD	AS_WRITE_HELD
