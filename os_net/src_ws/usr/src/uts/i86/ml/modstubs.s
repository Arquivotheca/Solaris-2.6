...
#pragma ident	"@(#)modstubs.s	1.53	96/10/23 SMI"
 ...
 * Stubs for vc_dptbl
 */
#ifndef VC_DPTBL_MODULE
	MODULE(VC_DPTBL,sched);
	STUB(VC_DPTBL, vc_getdptbl,		0);
	STUB(VC_DPTBL, vc_getkmdpris,		0);
	STUB(VC_DPTBL, vc_getmaxumdpri,	0);
	END_MODULE(VC_DPTBL);
#endif

/*
 * Stubs for 'vpix' module (VPIX support)
 */
#ifndef VPIX_MODULE
	MODULE(vpix,misc);
	NO_UNLOAD_STUB(vpix, v86init,		nomod_zero);
	NO_UNLOAD_WSTUB(vpix, v86memfunc,	nomod_zero);
	NO_UNLOAD_WSTUB(vpix, v86stash,		nomod_zero);
	NO_UNLOAD_WSTUB(vpix, v86unstash,	nomod_zero);
	NO_UNLOAD_WSTUB(vpix, v86setpseudo,	nomod_zero);
	NO_UNLOAD_WSTUB(vpix, v86sdeliver,	nomod_zero);
	NO_UNLOAD_WSTUB(vpix, v86deliver,	nomod_zero);
	NO_UNLOAD_WSTUB(vpix, v86sleep,		nomod_zero);
	END_MODULE(vpix);
#endif

#ifdef MERGE386
/*
 * Stubs for 'merge' module (MERGE386 support)
 */
#ifndef MERGE_MODULE
	MODULE(merge,misc);
	NO_UNLOAD_STUB(merge, vm86,			nomod_zero);
	NO_UNLOAD_WSTUB(merge, vm86_portalloc,		nomod_one);
	NO_UNLOAD_WSTUB(merge, vm86_portfree,		nomod_one);
	NO_UNLOAD_WSTUB(merge, vm86_asy_is_assigned,	nomod_zero);
	NO_UNLOAD_WSTUB(merge, vm86_com_ppiioctl,	nomod_zero);
	NO_UNLOAD_WSTUB(merge, vm86_floppy_free,	nomod_one);
	NO_UNLOAD_WSTUB(merge, vm86_flp_for_dos,	nomod_zero);
	NO_UNLOAD_WSTUB(merge, vm86_grab_floppy,	nomod_zero);
	NO_UNLOAD_WSTUB(merge, vm86_floppy_is_free,	nomod_one);
	NO_UNLOAD_WSTUB(merge, vm86_deassign_floppy,	nomod_zero);
	NO_UNLOAD_WSTUB(merge, vm86_get_dma_df,		nomod_zero);
	NO_UNLOAD_WSTUB(merge, vm86_dma_icheck,		nomod_zero);
	NO_UNLOAD_WSTUB(merge, vm86_kdppi_ioctl,	nomod_zero);
	NO_UNLOAD_WSTUB(merge, vm86_mrg_in_use,		nomod_zero);
	NO_UNLOAD_WSTUB(merge, vm86_trap,		nomod_zero);
	END_MODULE(merge);
#endif
#endif /* MERGE386 */
