...
#pragma ident	"@(#)door_support.c 1.7	96/05/20 SMI"
...
/*
 * This structure is passed to libc (door_return) with the arguments
 * in place for the server function
 */
struct door_results {
	void		*cookie;
	char		*data_ptr;
...
door_server_dispatch(door_data_t *caller_t, door_node_t *dp)
{
	...
	if (caller_t == NULL) {		/* no caller, so no data */
		...
		if ((ndid = dr.desc_num) == 0)
			door_size = 0;	/* Avoid multiplication */
		else
	...
	lwp_setsp(ttolwp(curthread), data_ptr - SA(sizeof (dr)));
	return (0);
}
