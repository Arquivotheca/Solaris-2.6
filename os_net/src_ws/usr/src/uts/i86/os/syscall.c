 ...
 * All rights reserved.
 */
#pragma ident "@(#)syscall.c	1.35	96/10/17 SMI"
 ...
 */
int
copyin_args(int *sp, int *ap, uint_t nargs)
{
	int mapped = 0;
	int rc;
	...
	rc = _copyin((caddr_t)sp, (caddr_t)ap, nargs);
	if (mapped)
 ...
 */
llfcn_t
syscall_entry(register int *argp)
{
...
void
syscall_exit(int rval1, int rval2)
{
	register kthread_id_t t = curthread;
	register klwp_t *lwp = ttolwp(t);
	register struct regs *rp = lwptoregs(lwp);
