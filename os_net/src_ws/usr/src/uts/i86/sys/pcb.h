...
#pragma ident	"@(#)pcb.h	1.10	96/06/18 SMI"
...
typedef struct pcb {
	...
	int		pcb_gpfault;	/* Flag to catch GP faults caused by */
					/* user modifying/corrupting the */
					/* saved state on his stack during */
					/* signal processing or thru debugger */
					/* (using /proc) */
} pcb_t;

#define	lwp_gpfault	lwp_pcb.pcb_gpfault
