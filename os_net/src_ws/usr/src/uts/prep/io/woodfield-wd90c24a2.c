...
#pragma ident "@(#)woodfield-wd90c24a2.c	1.30	96/09/24 SMI"
#pragma	weak	setintrenable

/*
 * Driver for Western Digital WD90C24A2
 */
...
#define	WD90C24A2_IO_RNUMBER	1
#define	WD90C24A2_FB_RNUMBER	2

#define	CMAP_ENTRIES	VGA8_CMAP_ENTRIES

#define	WD90C24A2DELAY(c, n)    \
{ \
	register int N = n; \
	while (--N > 0) { \
	    if (c) \
...
#if DRIVER_DEBUG
int	wd90c24a2_debug = 0;

#define	DEBUGF(level, args) \
		{ if (wd90c24a2_debug >= (level)) cmn_err args; }
#define	DUMP_SEGS(level, s, c) \
		{ if (wd90c24a2_debug >= (level)) dump_segs(s, c); }
#else
...
static int	wd90c24a2_open(dev_t *, int, int, cred_t *);
static int	wd90c24a2_close(dev_t, int, int, cred_t *);
static int	wd90c24a2_ioctl(dev_t, int, intptr_t, int, cred_t *, int *);
static int	wd90c24a2_mmap(dev_t, off_t, int);
static int	wd90c24a2_segmap(dev_t, off_t,
			struct as *, caddr_t *, off_t, u_int,
			u_int, u_int, cred_t *);

static struct vis_identifier wd90c24a2_ident = { "SUNWwd90c24a2" };

static struct cb_ops wd90c24a2_cb_ops = {
	wd90c24a2_open,		/* open */
	wd90c24a2_close,		/* close */
	nodev,			/* strategy */
	nodev,			/* print */
	nodev,			/* dump */
	nodev,			/* read */
	nodev,			/* write */
	wd90c24a2_ioctl,	/* ioctl */
	nodev,			/* devmap */
	wd90c24a2_mmap,		/* mmap */
	wd90c24a2_segmap,		/* segmap */
	nochpoll,		/* poll */
...
static int wd90c24a2_info(dev_info_t *dip, ddi_info_cmd_t infocmd,
		void *arg, void **result);
static int wd90c24a2_attach(dev_info_t *, ddi_attach_cmd_t);
static int wd90c24a2_detach(dev_info_t *, ddi_detach_cmd_t);
static int wd90c24a2_power(dev_info_t *, int, int);
static int wd90c24a2_probe(dev_info_t *);

struct dev_ops wd90c24a2_ops = {
	DEVO_REV,		/* devo_rev, */
	0,			/* refcnt  */
	wd90c24a2_info,		/* info */
	nulldev,		/* identify */
	wd90c24a2_probe,		/* probe */
	wd90c24a2_attach,		/* attach */
	wd90c24a2_detach,		/* detach */
	nodev,			/* reset */
	&wd90c24a2_cb_ops,		/* driver operations */
 ...
 */
static
struct wd90c24a2map_pvt {
	struct	wd90c24a2_softc *softc;
	ddi_mapdev_handle_t handle;	/* handle of mapdev object	*/
	u_int   offset;			/* starting offset of this map	*/
	u_int   len;			/* length of this map		*/
	struct wd90c24a2_cntxt *context; /* associated context		*/
	struct wd90c24a2map_pvt *next;	/* List of associated pvt's for	*/
					/* this context			*/
};

/* how much to map */
#define	WD90C24A2MAPSIZE	MMAPSIZE(0)

/* vertical retrace counter page */
#ifndef	WD90C24A2_VRT_SZ
#define	WD90C24A2_VRT_SZ	8192
#endif
 ...
 */
struct wd90c24a2_cntxt {
	struct wd90c24a2_cntxt *link; /* link to next (private) ctx if any */
	struct wd90c24a2map_pvt *pvt; /* List of associated pvt's */
					/* for this context */
...
#define	_linebytes	prd.mpr.md_linebytes
	...
	caddr_t	regs;		/* control registers */
...
#define	WD90C24A2VRTIOCTL	1	/* FBIOVERTICAL in effect */
#define	WD90C24A2VRTCTR	2	/* OWGX vertical retrace counter */
	...
	struct wd90c24a2_info adpinfo;	/* info about this adapter */
	struct mon_info moninfo;	/* info about this monitor */
	struct wd90c24a2_cntxt *curctx;	/* context switching */
	struct wd90c24a2_cntxt shared_ctx;	/* shared context */
	struct wd90c24a2_cntxt *pvt_ctx; /* list of non-shared contexts */
	int		chiprev;	/* fbc chip revision # */
	int		emulation;	/* emulation type, normally wd90c24a2 */
	...
	int		wd90c24a2_suspended; /* true if driver is suspended */
	struct wd90c24a2_console_private
...
static int	wd90c24a2_devinit(dev_t dev, struct vis_devinit *);
static void	wd90c24a2_cons_free(struct wd90c24a2_console_private *);
static void	wd90c24a2_cons_copy(struct wd90c24a2_console_private *,
			struct vis_conscopy *);
static void	wd90c24a2_cons_display(struct wd90c24a2_console_private *,
			struct vis_consdisplay *);
static void	wd90c24a2_cons_cursor(struct wd90c24a2_console_private *,
			struct vis_conscursor *);

static int wd90c24a2map_access(ddi_mapdev_handle_t, void *, off_t);
static void wd90c24a2map_free(ddi_mapdev_handle_t, void *);
static int wd90c24a2map_dup(ddi_mapdev_handle_t, void *,
		ddi_mapdev_handle_t, void **);

static
struct ddi_mapdev_ctl wd90c24a2map_ops = {
	MAPDEV_REV,	/* mapdev_ops version number	*/
	wd90c24a2map_access,	/* mapdev access routine	*/
	wd90c24a2map_free,	/* mapdev free routine		*/
	wd90c24a2map_dup	/* mapdev dup routine		*/
};

static u_int	pagesize;
static void	*wd90c24a2_softc_head;

/* default structure for FBIOGATTR ioctl */
static struct fbgattr wd90c24a2_attr = {
/*	real_type	 owner */
	FBTYPE_SUNFAST_COLOR, 0,
/* fbtype: type		 h  w  depth    cms  size */
	{FBTYPE_SUNFAST_COLOR, 0, 0, VGA8_DEPTH, CMAP_ENTRIES, 0},
...
#define	getsoftc(instance)	\
	((struct wd90c24a2_softc *)ddi_get_soft_state(wd90c24a2_softc_head, \
	    (instance)))
...
#define	S_TEC(softc)	((struct tec *)((softc)->fbctec + WD90C24A2_TEC_POFF))
#define	S_FHC(softc)	((u_int *)(softc)->fhcthc)
#define	S_THC(softc)	((struct thc *)((softc)->fhcthc + WD90C24A2_TEC_POFF))
#endif	/* XXXppc */
#define	S_CMAP(softc)	((struct wd90c24a2_cmap *)(softc)->cmap)

#ifdef	XXXppc
#define	wd90c24a2_set_video(softc, on)	thc_set_video(S_THC(softc), (on))
#define	wd90c24a2_get_video(softc)		thc_get_video(S_THC(softc))

#define	wd90c24a2_set_sync(softc, on) \
	    S_THC(softc)->l_thc_hcmisc = \
	    (S_THC(softc)->l_thc_hcmisc & ~THC_HCMISC_SYNCEN | \
	    ((on) ? THC_HCMISC_SYNCEN : 0))


#define	wd90c24a2_int_enable(softc) \
	{\
	    thc_int_enable(S_THC(softc)); \
	    if ((softc)->p4reg) \
		(void) setintrenable(1); }

#define	wd90c24a2_int_disable_intr(softc) \
	{\
	    if ((softc)->p4reg) \
		(void) setintrenable(0); \
	    thc_int_disable(S_THC(softc)); }

#define	wd90c24a2_int_disable(softc) \
	{\
	    mutex_enter(&(softc)->interlock); \
	    wd90c24a2_int_disable_intr(softc);    \
	    mutex_exit(&(softc)->interlock); }

#define	wd90c24a2_int_pending(softc)		thc_int_pending(S_THC(softc))
#else
#define	wd90c24a2_set_video(softc, on)	/* NYI */
#define	wd90c24a2_set_sync(softc, on)	/* NYI */
#endif	/* XXXppc */

/* check if color map update is pending */
#define	wd90c24a2_update_pending(softc) \
 ...
 */
#ifdef	XXXppc
static u_int	wd90c24a2_intr(caddr_t);
#endif
static void	wd90c24a2_reset_cmap(volatile u_char *, u_int);
static void	wd90c24a2_update_cmap(struct wd90c24a2_softc *, u_int, u_int);
#ifdef	XXXppc
static void	wd90c24a2_cmap_bcopy(u_char *, u_char *, u_int);
#endif
static void	wd90c24a2_cons_put_cmap(struct wd90c24a2_softc *, int,
			unsigned char, unsigned char, unsigned char);
static void	wd90c24a2_cons_get_cmap(struct wd90c24a2_softc *, int,
			unsigned char *, unsigned char *, unsigned char *);

static void	wd90c24a2_setcurpos(struct wd90c24a2_softc *);
static void	wd90c24a2_setcurshape(struct wd90c24a2_softc *);
static void	wd90c24a2_reset(struct wd90c24a2_softc *);
#ifdef notdef
static void	wd90c24a2_set_indexed(struct wd90c24a2_softc *,
			caddr_t indexreg, caddr_t datareg, int index, int val);
#endif
static int	wd90c24a2_get_indexed(struct wd90c24a2_softc *,
			caddr_t indexreg, caddr_t datareg, int index);
static void	wd90c24a2_smart_bcopy(char *, char *, int);
#if	DRIVER_DEBUG
static void	wd90c24a2_register_dump(struct wd90c24a2_softc *softc);
#endif
#ifdef	XXXppc
static int	wd90c24a2_cntxsave(volatile struct fbc *, volatile struct tec *,
			struct wd90c24a2_cntxt *);
static int	wd90c24a2_cntxrestore(volatile struct fbc *,
			volatile struct tec *, struct wd90c24a2_cntxt *);
#endif	/* XXXppc */
static struct wd90c24a2_cntxt *ctx_map_insert(struct wd90c24a2_softc *, int);
static int	getpid(void);

/*
 * SunWindows specific stuff
 */
static  wd90c24a2_rop(), wd90c24a2_putcolormap();

/* kernel pixrect ops vector */
static struct pixrectops wd90c24a2_pr_ops = {
	wd90c24a2_rop,
	wd90c24a2_putcolormap,
	mem_putattributes
};
...
static struct modldrv modldrv = {
	...
	"wd90c24a2 driver v1.30",	/* Name of the module. */
	&wd90c24a2_ops,		/* driver ops */
};
...
_init(void)
{
	...
	DEBUGF(1,
	    (CE_CONT, "wd90c24a2: compiled %s, %s\n", __TIME__, __DATE__));

	if ((e = ddi_soft_state_init(&wd90c24a2_softc_head,
		    sizeof (struct wd90c24a2_softc), 1)) != 0) {
	...
	if (e) {
	    ddi_soft_state_fini(&wd90c24a2_softc_head);
	    DEBUGF(1, (CE_CONT, "done\n"));
	}
	DEBUGF(1, (CE_CONT, "wd90c24a2: _init done rtn=%d\n", e));
	return (e);
}
...
#ifdef	XXXppc
	DEBUGF(1, (CE_CONT, "wd90c24a2: _fini, mem used=%d\n", total_memory));
#else
	DEBUGF(1, (CE_CONT, "wd90c24a2: _fini\n"));
#endif
...
static  wd90c24a2_rop_wait = 50;	/* milliseconds */

static
wd90c24a2_rop(Pixrect *dpr,
	int dx,
	int dy,
	int dw,
	int dh,
	...
	int sy)
{
	...
	(void) unit;
#endif
	if (spr && spr->pr_ops == &wd90c24a2_pr_ops) {
	...
	if (wd90c24a2_rop_wait) {
#ifdef	XXXppc
	    volatile u_int *statp = &fbc->l_fbc_status;
	    CDELAY(!(*statp & L_FBC_BUSY), wd90c24a2_rop_wait << 10);
#else
	    /*EMPTY*/
	    /* NYI:  wait for idle */
#endif
...
/*ARGSUSED*/
static
wd90c24a2_putcolormap(Pixrect *pr,
		int index,
		int count,
		unsigned char *red,
		unsigned char *green,
...
{
#ifdef	XXXppc
	register struct wd90c24a2_softc *softc =
	    getsoftc(mpr_d(pr)->md_primary);
	register u_int rindex = (u_int) index;
	register u_int rcount = (u_int) count;
	register u_char *map;
	register u_int entries;

	DEBUGF(5, (CE_CONT, "wd90c24a2_putcolormap unit=%d index=%d count=%d\n",
	...
	if (wd90c24a2_update_pending(softc))
	    wd90c24a2_int_disable(softc);
#endif

	map += rindex * 3;
	wd90c24a2_cmap_bcopy(red, map, rcount);
	wd90c24a2_cmap_bcopy(green, map + 1, rcount);
	wd90c24a2_cmap_bcopy(blue, map + 2, rcount);

	wd90c24a2_update_cmap(softc, rindex, rcount);

#ifdef	XXXppc
	/* enable interrupt so we can load the hardware colormap */
	wd90c24a2_int_enable(softc);
#endif
...
static int
wd90c24a2_probe(dev_info_t *devi)
{
	...
	DEBUGF(1, (CE_CONT, "wd90c24a2_probe (%s) unit=%d\n",
	 ...
	 */
	if (ddi_map_regs(devi, WD90C24A2_REG_RNUMBER, (caddr_t *)&fbc,
		    WD90C24A2_ADDR_P4REG, (off_t)sizeof (long)) != 0) {
	...
	if (ddi_peekl(devi, fbc, &id) == DDI_FAILURE) {
		...
		DEBUGF(2, (CE_CONT, " not a wd90c24a2 - returning failure\n"));
		error = DDI_FAILURE;
	    }
	}

	ddi_unmap_regs(devi, WD90C24A2_REG_RNUMBER, (caddr_t *)&fbc,
		WD90C24A2_ADDR_P4REG, (off_t)sizeof (long));
#endif
	return (error);
}

#define	GET_CRTC(s, i) \
	wd90c24a2_get_indexed(s, s->regs+VGA_CRTC_ADR, s->regs+VGA_CRTC_DATA, i)
#ifdef notdef
#define	SET_CRTC(s, i, v) \
	wd90c24a2_set_indexed(s, \
	    s->regs+VGA_CRTC_ADR, s->regs+VGA_CRTC_DATA, i, v)
#endif

static int
wd90c24a2_attach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	register struct wd90c24a2_softc *softc;
	int		w, h, bytes;
	off_t		bytes_to_map;
	...
	DEBUGF(1,
	    (CE_CONT, "wd90c24a2_attach unit=%d cmd=%d\n", unit, (int)cmd));
	...
	case DDI_RESUME:
	    if (!(softc =
		(struct wd90c24a2_softc *)ddi_get_driver_private(devi)))
		    return (DDI_FAILURE);
	    if (!softc->wd90c24a2_suspended)
		    return (DDI_SUCCESS);
	    mutex_enter(&softc->mutex);
	    wd90c24a2_reset(softc);
	    if (softc->curctx) {
#ifdef	XXXppc
		    /* Restore non display RAM */
		    if (ddi_map_regs(devi, WD90C24A2_REG_RNUMBER,
			(caddr_t *)&fb_ndvram,
			WD90C24A2_ADDR_COLOR + softc->_w * softc->_h,
			softc->ndvramsz) == -1) {
			    mutex_exit(&softc->mutex);
			    return (DDI_FAILURE);
		    }
		    bcopy(softc->ndvram, fb_ndvram, softc->ndvramsz);
		    ddi_unmap_regs(devi, WD90C24A2_REG_RNUMBER,
			(caddr_t *)&fb_ndvram,
			WD90C24A2_ADDR_COLOR + softc->_w * softc->_h,
			softc->ndvramsz);
		    ...
		    (void) wd90c24a2_cntxrestore(S_FBC(softc), S_TEC(softc),
			softc->curctx);
		    wd90c24a2_setcurpos(softc);
		    wd90c24a2_setcurshape(softc);
		    wd90c24a2_update_cmap(softc, (u_int)_ZERO_, CMAP_ENTRIES);
		    wd90c24a2_int_enable(softc); /* Schedule the update */
#else
		/*EMPTY*/
		/* NYI:  parts of DDI_RESUME not implemented */
#endif	/* XXXppc */
	    }
	    softc->wd90c24a2_suspended = 0;
	    mutex_exit(&softc->mutex);
	    /* Restore brightness level */
	    if ((power = pm_get_normal_power(devi, 1)) == DDI_FAILURE) {
		cmn_err(CE_WARN, "wd90c24a2_attach(DDI_RESUME) can't get "
			"normal power");
		return (DDI_FAILURE);
	    }
	    if (wd90c24a2_power(devi, 1, power) != DDI_SUCCESS) {
		cmn_err(CE_WARN, "wd90c24a2_attach: wd90c24a2_power failed");
			return (DDI_FAILURE);
	    }
	    return (DDI_SUCCESS);
	...
	DEBUGF(1, (CE_CONT, "wd90c24a2_attach unit=%d\n", unit));

	pagesize = ddi_ptob(devi, 1);

	/* Allocate softc struct */
	if (ddi_soft_state_zalloc(wd90c24a2_softc_head, unit) != 0) {
		return (DDI_FAILURE);
	}
	...
	softc->devi = devi;
	DEBUGF(1,
	    (CE_CONT, "wd90c24a2_attach devi=0x%x unit=%d\n", devi, unit));
	...
	if (ddi_dev_is_sid(devi) != DDI_SUCCESS) {
	    if (ddi_map_regs(devi, WD90C24A2_REG_RNUMBER,
			(caddr_t *)&softc->p4reg,
			(off_t)WD90C24A2_ADDR_P4REG,
			sizeof (*softc->p4reg)) != 0) {
		DEBUGF(2, (CE_CONT,
			"wd90c24a2_probe: ddi_map_regs (p4reg) FAILED\n"));
		return (DDI_FAILURE);
	    }
	}
#endif
	...
	if (ddi_regs_map_setup(devi, WD90C24A2_IO_RNUMBER,
		(caddr_t *)&softc->regs,
		(off_t)WD90C24A2_VGAREG_OFFSET, (off_t)WD90C24A2_VGAREG_SIZE,
		&reg_access_mode, &softc->regs_handle) == -1) {
	    wd90c24a2_detach(devi, DDI_DETACH);
	    return (DDI_FAILURE);
	}
	softc->regs_mapped = B_TRUE;

#if	DRIVER_DEBUG
	if (wd90c24a2_debug > 0)
	    wd90c24a2_register_dump(softc);
#endif
...
#undef	GET_INTERLACED	/* NO_WD90C24A2_DOCS:  assume no interlace support */
#define	GET_VERT_X2(c)	\
	(GET_CRTC(c, VGA_CRTC_CRT_MD) & VGA_CRTC_CRT_MD_VT_X2)

	w = (GET_HORIZ_END(softc)+1)*8;
	h = GET_VERT_END(softc)+1;
#ifdef	GET_INTERLACED
	if (GET_INTERLACED(softc)) h *= 2;
#endif
	if (GET_VERT_X2(softc)) h *= 2;
...
#else
#ifdef	NO_WD90C24A2_DOCS	/* NEEDSWORK */
	softc->adpinfo.vmsize = WD90C24A2_CFG1_MEMSIZE(
	    WD90C24A2_GET_CRTC(softc, WD90C24A2_CONFG_REG1));
	softc->fbmappable =
	    softc->adpinfo.vmsize * WD90C24A2_CFG1_MEMSIZE_SCALE;
#else
	/* NYI:  constant vmsize */
	softc->adpinfo.vmsize = 1;
	softc->fbmappable = softc->adpinfo.vmsize * 1024*1024;
#endif
	...
	softc->adpinfo.accessible_height = (u_int)
	    (softc->adpinfo.vmsize * 1024 * 1024) /* NEEDSWORK: vmsize units */
...
#ifdef	XXXppc
#ifdef DEBUG
	softc->adpinfo.pad1 = WD90C24A2_VADDR_COLOR + WD90C24A2_FB_SZ;
#endif
#endif	/* XXXppc */
	...
	if (ddi_regs_map_setup(devi, WD90C24A2_FB_RNUMBER,
		(caddr_t *)&softc->_fb,
		(off_t)WD90C24A2_FB_OFFSET, bytes_to_map,
		&fb_access_mode, &softc->fb_handle) == -1) {
	    wd90c24a2_detach(devi, DDI_DETACH);
	    return (DDI_FAILURE);
	}
	softc->fb_mapped = B_TRUE;

	softc->fbpfnum = hat_getkpfnum((caddr_t)softc->_fb);

#ifdef	NO_WD90C24A2_DOCS	/* NEEDSWORK */
	softc->chiprev = WD90C24A2_GET_CRTC(softc, WD90C24A2_CHIP_ID_REV);
#else
	/* NYI:  unknown chip rev */
	softc->chiprev = 0;
#endif
	...
	if (ddi_add_intr(devi, 0, &softc->iblock_cookie, 0,
	    ...
	    DEBUGF(2, (CE_CONT, "wd90c24a2_attach%d add_intr failed\n", unit));
	    (void) wd90c24a2_detach(devi, DDI_DETACH);
	    return (DDI_FAILURE);
	}

	mutex_init(&softc->interlock, "wd90c24a2_interlock", MUTEX_DRIVER,
	    softc->iblock_cookie);
	mutex_init(&softc->mutex,
		    "wd90c24a2_softc_mtx", MUTEX_DRIVER, softc->iblock_cookie);
	cv_init(&softc->vrtsleep,
		    "wd90c24a2_vrt_wait", CV_DRIVER, softc->iblock_cookie);

	ddi_remove_intr(devi, 0, softc->iblock_cookie);

	if (ddi_add_intr(devi, 0, &softc->iblock_cookie, 0,
		    wd90c24a2_intr, (caddr_t)softc) != DDI_SUCCESS) {
	    DEBUGF(2, (CE_CONT,
		"wd90c24a2_attach%d add_intr failed\n", unit));
	    (void) wd90c24a2_detach(devi, DDI_DETACH);
	    return (DDI_FAILURE);
	}
#else
	 ...
	 */
	wd90c24a2_reset_cmap(softc->cmap_rgb, CMAP_ENTRIES);
	wd90c24a2_reset_cmap(softc->omap_rgb, 2);
	wd90c24a2_update_cmap(softc, (u_int) _ZERO_, CMAP_ENTRIES);
	wd90c24a2_update_cmap(softc, (u_int) _ZERO_, (u_int) _ZERO_);

	DEBUGF(2, (CE_CONT,
	    "wd90c24a2_attach%d just before create_minor node\n", unit));
	sprintf(name, "wd90c24a2_%d", unit);
	if (ddi_create_minor_node(devi, name, S_IFCHR,
			    unit, DDI_NT_DISPLAY, NULL) == DDI_FAILURE) {
	    ddi_remove_minor_node(devi, NULL);
	    DEBUGF(2, (CE_CONT,
		"wd90c24a2_attach%d create_minor node failed\n", unit));
	    return (DDI_FAILURE);
	}
	ddi_report_dev(devi);

#ifdef	XXXPPC
	/* NYI:  None of this info is real, so... */
	cmn_err(CE_CONT,
		"?wd90c24a2%d: screen %dx%d, %s buffered, "
		"%ld%sM mappable, rev %u.%u\n",
	    unit, w, h, softc->adpinfo.hdb_capable ? "double" : "single",
	    (long)softc->adpinfo.vmsize *
		WD90C24A2_CFG1_MEMSIZE_SCALE / 1024 / 1024,
	    ((long)softc->adpinfo.vmsize *
		WD90C24A2_CFG1_MEMSIZE_SCALE/512/1024) % 2
	    ? ".5" : "", softc->chiprev>>4, softc->chiprev & 0x0f);
#endif
...
static int
wd90c24a2_detach(dev_info_t *devi, ddi_detach_cmd_t cmd)
{
	int instance = ddi_get_instance(devi);
	register struct wd90c24a2_softc *softc = getsoftc(instance);
#ifdef	XXXppc
	caddr_t fb_ndvram;
#endif

	DEBUGF(1,
	    (CE_CONT, "wd90c24a2_detach softc=%x, devi=0x%x\n", softc, devi));
	...
	case DDI_SUSPEND:
	    ...
	    if (softc->curctx) {
		    ...
		    if (ddi_map_regs(devi, WD90C24A2_REG_RNUMBER, &fb_ndvram,
			WD90C24A2_ADDR_COLOR + softc->_w * softc->_h,
			softc->ndvramsz) == -1) {
			    kmem_free(softc->ndvram, softc->ndvramsz);
			    mutex_exit(&softc->mutex);
		    ...
		    ddi_unmap_regs(devi, WD90C24A2_REG_RNUMBER, &fb_ndvram,
			WD90C24A2_ADDR_COLOR + softc->_w * softc->_h,
			softc->ndvramsz);

		    /* Save other frame buffer state */
		    (void) wd90c24a2_cntxsave(S_FBC(softc), S_TEC(softc),
			softc->curctx);
	    }
	    softc->wd90c24a2_suspended = 1;
	    mutex_exit(&softc->mutex);
#else
	...
	if (softc->fbctec)
	    ddi_unmap_regs(devi, WD90C24A2_REG_RNUMBER,
		&softc->fbctec, WD90C24A2_ADDR_FBC, WD90C24A2_FBCTEC_SZ);
	if (softc->cmap)
	    ddi_unmap_regs(devi, WD90C24A2_REG_RNUMBER,
		&softc->cmap, WD90C24A2_ADDR_CMAP, Bt485_CMAP_SZ);
	if (softc->fhcthc)
	    ddi_unmap_regs(devi, WD90C24A2_REG_RNUMBER,
		&softc->fhcthc, WD90C24A2_ADDR_FHC, WD90C24A2_FHCTHC_SZ);
	if (softc->rom)
	    ddi_unmap_regs(devi, WD90C24A2_REG_RNUMBER,
		&softc->rom, softc->addr_rom, WD90C24A2_ROM_SZ);
	if (softc->dhc)
	    ddi_unmap_regs(devi, WD90C24A2_REG_RNUMBER,
		&softc->dhc, WD90C24A2_ADDR_DHC, WD90C24A2_DHC_SZ);
	if (softc->alt)
	    ddi_unmap_regs(devi, WD90C24A2_REG_RNUMBER,
		&softc->alt, WD90C24A2_ADDR_ALT, WD90C24A2_ALT_SZ);
	/* TODO: uart (future) */
#else
	...
	/* free softc struct */
	(void) ddi_soft_state_free(wd90c24a2_softc_head, instance);
...
static int
wd90c24a2_power(dev_info_t *dip, int cmpt, int level)
{
	register struct wd90c24a2_softc *softc;

	if (cmpt != 1 || 0 > level || level > 255 ||
	    !(softc = (struct wd90c24a2_softc *)ddi_get_driver_private(dip)))
		return (DDI_FAILURE);
	...
	if (level) {
		/*EMPTY*/
		wd90c24a2_set_sync(softc, FBVIDEO_ON);
		wd90c24a2_set_video(softc, FBVIDEO_ON);
	} else {
		/*EMPTY*/
		wd90c24a2_set_video(softc, FBVIDEO_OFF);
		wd90c24a2_set_sync(softc, FBVIDEO_OFF);
	}
...
/* ARGSUSED */
static int
wd90c24a2_info(dev_info_t *dip, ddi_info_cmd_t infocmd,
    void *arg, void **result)
{
...
/*ARGSUSED*/
static
wd90c24a2_open(dev_t *devp, int flag, int otyp, cred_t *cred)
{
	int	unit = getminor(*devp);
	struct	wd90c24a2_softc *softc = getsoftc(unit);
	int	error = 0;

#ifdef	XXXppc
	DEBUGF(2,
	    (CE_CONT, "wd90c24a2_open(%d), mem used=%d\n", unit, total_memory));
#else
	DEBUGF(2, (CE_CONT, "wd90c24a2_open(%d)\n", unit));
#endif
...
static
int
wd90c24a2_close(dev_t dev, int flag, int otyp, cred_t *cred)
{
...
#ifdef	XXXppc
	DEBUGF(2, (CE_CONT, "wd90c24a2_close(%d, %d, %d), mem used=%d\n",
	    unit, flag, otyp, total_memory));
#else
	DEBUGF(2, (CE_CONT, "wd90c24a2_close(%d, %d, %d)\n",
	    unit, flag, otyp));
#endif
...
/*ARGSUSED*/
static
wd90c24a2_mmap(dev_t dev, register off_t off, int prot)
{
	register struct wd90c24a2_softc *softc = getsoftc(getminor(dev));
	register int rval;

	DEBUGF(off ? 5 : 1, (CE_CONT, "wd90c24a2_mmap(%d, 0x%x)\n",
	...
	DEBUGF(5, (CE_CONT, "wd90c24a2_mmap returning 0x%x\n", rval));
...
/*ARGSUSED*/
static
wd90c24a2_ioctl(dev_t dev, int cmd, intptr_t data, int mode, cred_t *cred,
    int *rval)
{
	register struct wd90c24a2_softc *softc = getsoftc(getminor(dev));
	int    cursor_cmap;
	...
	static char not_init[] =
		"wd90c24a2_ioctl: Console has not been initialized";
	static char kernel_only[] =
		"wd90c24a2_ioctl: %s is a kernel only ioctl";
	static char kernel_only2[] =
		"wd90c24a2_ioctl: %s and %s are kernel only ioctls";

	DEBUGF(3, (CE_CONT, "wd90c24a2_ioctl(%d, 0x%x)\n", getminor(dev), cmd));
	...
	case VIS_GETIDENTIFIER:
	    ...
	    if (ddi_copyout((caddr_t)&wd90c24a2_ident,
			    (caddr_t)data,
	...
	case VIS_DEVINIT:
	    ...
	    if ((err = wd90c24a2_devinit(dev,
		(struct vis_devinit *)data)) != 0) {
		cmn_err(CE_CONT,
		    "wd90c24a2ioctl: Could not initialize console");
		return (err);
	    }
	    break;
	...
	case VIS_DEVFINI:
	    ...
	    wd90c24a2_cons_free(softc->conspriv);
	    softc->conspriv = NULL;
	...
	case VIS_STAND_CONSCOPY:	/* copy */
	    ...
	    wd90c24a2_cons_copy(softc->conspriv, (struct vis_conscopy *)data);
	    break;
	...
	case VIS_STAND_CONSDISPLAY:	/* display with no system services */
	    ...
	    wd90c24a2_cons_display(softc->conspriv,
	...
	case VIS_STAND_CONSCURSOR:	/* cursor */
	    ...
	    wd90c24a2_cons_cursor(softc->conspriv,
	...
	cmap_ioctl:
		...
		if (cmd == FBIOPUTCMAP || cmd == VIS_PUTCMAP) {
			...
			wd90c24a2_cmap_bcopy(iobuf_cmap_red, map++, count);
			wd90c24a2_cmap_bcopy(iobuf_cmap_green, map++, count);
			wd90c24a2_cmap_bcopy(iobuf_cmap_blue, map, count);

			/* cursor colormap update */
			if (entries < CMAP_ENTRIES)
			    count = 0;
			wd90c24a2_update_cmap(softc, index, count);
#ifdef	XXXppc
			wd90c24a2_int_enable(softc);
#endif
#else
			for (i = 0; i < count; i++) {
				wd90c24a2_cons_put_cmap(softc,
					index+i,
					iobuf_cmap_red[i],
					iobuf_cmap_green[i],
					iobuf_cmap_blue[i]);
	...
	case FBIOGATTR: {
		...
		DEBUGF(3, (CE_CONT, "FBIOGATTR, emu_type=%d\n",
			softc->emulation));
		bcopy((caddr_t)&wd90c24a2_attr, (caddr_t)&attr, sizeof (attr));
		mutex_enter(&softc->mutex);
	 ...
	 * support older software which was staticly-linked before wd90c24a2 was
	...
	case FBIOGTYPE: {
		...
		bcopy((caddr_t)&wd90c24a2_attr.fbtype,
			(caddr_t)&fb,
	...
	case FBIOGPIXRECT: {
		...
		softc->pr.pr_ops = &wd90c24a2_pr_ops;
		...
		/* enable video */
		wd90c24a2_set_video(softc, _ONE_);
	...
	case FBIOSVIDEO:
		...
		if (ddi_copyin((caddr_t)data,
			...
			return (EFAULT);
		mutex_enter(&softc->mutex);
		wd90c24a2_set_video(softc, i & FBVIDEO_ON);
		mutex_exit(&softc->mutex);
		break;
	...
	case FBIOGVIDEO:
		...
		mutex_enter(&softc->mutex);
#ifdef	XXXppc
		i = wd90c24a2_get_video(softc) ? FBVIDEO_ON : FBVIDEO_OFF;
#else
	...
	case FBIOVERTICAL:
		...
		mutex_enter(&softc->mutex);
		softc->vrtflag |= WD90C24A2VRTIOCTL;
#ifdef	XXXppc
		wd90c24a2_int_enable(softc);
#endif
...
#if	DRIVER_DEBUG
	case 255:
		wd90c24a2_debug = (int)data;
		if (wd90c24a2_debug == -1)
		    wd90c24a2_debug = DRIVER_DEBUG;
		cmn_err(CE_CONT,
		    "wd90c24a2_debug is now %d\n", wd90c24a2_debug);
		break;
#endif
...
#ifdef	XXXppc
static  u_int
wd90c24a2_intr(caddr_t arg)
{
	register struct wd90c24a2_softc *softc =
	    (struct wd90c24a2_softc *) arg;
	volatile u_long *in;
	volatile u_long *out;
	volatile u_long  tmp;

	DEBUGF(7, (CE_CONT,
		"wd90c24a2_intr: softc=%x, vrtflag=%x\n",
		softc, softc->vrtflag));

	if (!wd90c24a2_int_pending(softc)) {
	    return (DDI_INTR_UNCLAIMED);	/* nope, not mine */
	}
	mutex_enter(&softc->mutex);
	mutex_enter(&softc->interlock);

	if (!(wd90c24a2_update_pending(softc) || (softc)->vrtflag)) {
	...
	if (softc->vrtflag & WD90C24A2VRTCTR) {
	    if (softc->vrtmaps == 0) {
		softc->vrtflag &= ~WD90C24A2VRTCTR;
	    } else
		*softc->vrtpage += 1;
	}
	if (softc->vrtflag & WD90C24A2VRTIOCTL) {
	    softc->vrtflag &= ~WD90C24A2VRTIOCTL;
	    cv_broadcast(&softc->vrtsleep);
	}
	if (wd90c24a2_update_pending(softc)) {
	    volatile struct wd90c24a2_cmap *cmap = S_CMAP(softc);
...
static int
wd90c24a2_devinit(dev_t dev, struct vis_devinit *data)
{
	struct wd90c24a2_softc		*softc;
	struct wd90c24a2_console_private	*private;
	int				unit;
	...
	private = (struct wd90c24a2_console_private *)
	    kmem_alloc(sizeof (struct wd90c24a2_console_private), KM_SLEEP);
	...
	private->softc = softc;
	mutex_init(&private->pmutex,
	    "wd90c24a2 console lock", MUTEX_DRIVER, NULL);
	private->viscursor = 0;
...
static
void
clip(struct wd90c24a2_softc *softc, struct wd90c24a2_rect *rp)
{
...
wd90c24a2_cons_put_cmap(
	...
	unsigned char b)
{
	ddi_io_putb(softc->regs_handle,
		(int)softc->regs + VGA_DAC_WR_AD, index);
	ddi_io_putb(softc->regs_handle,
		(int)softc->regs + VGA_DAC_DATA, r >> 2);
	ddi_io_putb(softc->regs_handle,
		(int)softc->regs + VGA_DAC_DATA, g >> 2);
	ddi_io_putb(softc->regs_handle,
		(int)softc->regs + VGA_DAC_DATA, b >> 2);
}

static void
wd90c24a2_cons_get_cmap(
	struct wd90c24a2_softc *softc,
	int index,
	unsigned char *r,
	unsigned char *g,
...
{
	ddi_io_putb(softc->regs_handle,
		(int)softc->regs + VGA_DAC_RD_AD, index);
	*r = ddi_io_getb(softc->regs_handle,
		(int)softc->regs + VGA_DAC_DATA) << 2;
	*g = ddi_io_getb(softc->regs_handle,
		(int)softc->regs + VGA_DAC_DATA) << 2;
	*b = ddi_io_getb(softc->regs_handle,
		(int)softc->regs + VGA_DAC_DATA) << 2;
}

static
void
wd90c24a2_cons_free(struct wd90c24a2_console_private *cp)
{
	mutex_destroy(&cp->pmutex);

	kmem_free(cp, sizeof (struct wd90c24a2_console_private));
}

static void
wd90c24a2_cons_copy(struct wd90c24a2_console_private *cp,
...
#else
		    wd90c24a2_smart_bcopy((char *)srcp + pma->s_col,
			(char *)dstp + pma->t_col, r.w);
#endif
		    dstp += cp->softc->_w;
		    srcp += cp->softc->_w;
...
static void
wd90c24a2_cons_display(struct wd90c24a2_console_private *cp,
...
#else
		wd90c24a2_smart_bcopy((char *)imgcp, (char *)scrcp + r.x, r.w);
#endif
		imgcp += r.w;
		scrcp += cp->softc->_w;
	}
...
static void
wd90c24a2_cons_cursor(struct wd90c24a2_console_private *cp,
 ...
 */
static void
wd90c24a2_reset_cmap(volatile u_char *cmap, register u_int entries)
{
 ...
 */
static void
wd90c24a2_update_cmap(struct wd90c24a2_softc *softc, u_int index, u_int count)
{
	u_int   high, low;
 ...
 */
static void
wd90c24a2_cmap_bcopy(register u_char *bufp, register u_char *rgb, u_int count)
{
...
/*ARGSUSED*/
static void
wd90c24a2_setcurpos(struct wd90c24a2_softc *softc)
{
	/* NEEDSWORK:  hw cursor */
}
...
/*ARGSUSED*/
static void
wd90c24a2_setcurshape(struct wd90c24a2_softc *softc)
{
#ifdef	XXXppc
	u_long  tmp, edge = 0;
...
static void
wd90c24a2_reset(struct wd90c24a2_softc *softc)
{
#ifdef	XXXppc
	volatile struct thc *thc = S_THC(softc);

	/* disable HW cursor */
	thc->l_thc_cursor = WD90C24A2_CURSOR_OFFPOS;
	 ...
	 * comments.
	 *
	 * wd90c24a2_init(softc)
	 *	struct wd90c24a2_softc *softc;
	 *{
	 *	// Initialize DAC
	 *	{
	 *	    register struct wd90c24a2_cmap *cmap = S_CMAP(softc);
	 *	    register char *p;
	 *
	 *	    static char dacval[] = {
	 *		4, 0xff,
	 ...
	 *
	 *	    rev = *fhc >> FHC_CONFIG_REV_SHIFT & FHC_CONFIG_REV_MASK;
	 *	    DEBUGF(1, (CE_CONT, "wd90c24a2_init: FBC rev %d\n", rev));
	 *
	 *	//
...
#define	WD90C24A2MAP_SHARED	0x02	/* shared context */
#define	WD90C24A2MAP_VRT	0x04	/* vrt page */
#define	WD90C24A2MAP_FBCTEC	0X08	/* mapping includes fbc and/or tec */
#define	WD90C24A2MAP_FB	0X10	/* mapping includes framebuffer */

/* needs context */
#define	WD90C24A2MAP_CTX	(WD90C24A2MAP_FBCTEC | WD90C24A2MAP_FB)

static struct wd90c24a2map_pvt *
wd90c24a2_pvt_alloc(struct wd90c24a2_cntxt *ctx,
		u_int off,
		u_int len,
		struct wd90c24a2_softc *softc)
{
	struct wd90c24a2map_pvt *pvt;

	/*
	 * create the private data portion of the mapdev object
	 */
	pvt = (struct wd90c24a2map_pvt *)kmem_zalloc(
	    sizeof (struct wd90c24a2map_pvt), KM_SLEEP);
	pvt->offset  = off;
	pvt->len = len;
	pvt->context = ctx;
	pvt->softc = softc;
 ...
 * the creation of lego (wd90c24a2) segments.
 */
/*ARGSUSED*/
static int
wd90c24a2_segmap(dev_t	dev,
	    off_t	off,
	    struct as	*as,
	    caddr_t	*addrp,
	    off_t	len,
	    ...
	    cred_t	*cred)
{
	...
	struct wd90c24a2_cntxt *ctx = (struct wd90c24a2_cntxt *)NULL;
	struct wd90c24a2_cntxt *shared_ctx = &softc->shared_ctx;
	struct wd90c24a2map_pvt *pvt;
	u_int	maptype = 0;
	u_int	ctxmap;
	int	error;
	...
	*/
	...
	if (off + len > 0 &&
	    ...
	    maptype |= WD90C24A2MAP_FB;

#ifdef	XXXppc
	/* decide if this segment is part of a context. */

	ctxmap = (softc->chiprev == 5) ?
		    (WD90C24A2MAP_FBCTEC|WD90C24A2MAP_FB) : WD90C24A2MAP_FBCTEC;
#else
	ctxmap = 0;
#endif
	 ...
	 */
	    ...
	    if (flags & MAP_SHARED) {	/* shared mapping */
		    ctx = shared_ctx;
		    ctx->flag = WD90C24A2MAP_CTX;
	    } else {
		    ctx = ctx_map_insert(softc, maptype);
		    ctx->flag |= maptype;
	    }

	    pvt = wd90c24a2_pvt_alloc(ctx, 0, len, softc);
...
/* ARGSUSED */
static int
wd90c24a2map_access(ddi_mapdev_handle_t handle, void *pvt, off_t offset)
{
#ifdef	XXXppc
	struct wd90c24a2map_pvt *p   = (struct wd90c24a2map_pvt *)pvt;
	struct wd90c24a2map_pvt *pvts;
	struct wd90c24a2_softc *softc = p->softc;
	...
	if (softc->curctx != p->context) {
		 ...
		 */
		if (softc->curctx != (struct wd90c24a2_cntxt *)NULL) {
			/*
			...
			if (wd90c24a2_cntxsave(fbc, S_TEC(softc),
					softc->curctx) == 0) {
				DEBUGF(1, (CE_CONT,
				    "wd90c24a2: context save failed\n"));
				/*
		 ...
		 * Idle the chips
		 */
		WD90C24A2DELAY(!(fbc->l_fbc_status & L_FBC_BUSY),
		    WD90C24A2_FBC_WAIT);
		if (fbc->l_fbc_status & L_FBC_BUSY) {
			DEBUGF(1, (CE_CONT,
			    "wd90c24a2: idle_wd90c24a2: status = %x\n",
			    fbc->l_fbc_status));
			/*
			 * At this point we have no current context.
			 */
		...
		if (p->context->flag & WD90C24A2MAP_FBCTEC)
			if (wd90c24a2_cntxrestore(fbc, S_TEC(softc),
						p->context) == 0) {
				DEBUGF(1, (CE_CONT,
				    "wd90c24a2: context restore failed\n"));
				/*
				 * At this point we have no current context.
				 */
				softc->curctx = NULL;
...
/* ARGSUSED */
static void
wd90c24a2map_free(ddi_mapdev_handle_t handle, void *pvt)
{
	struct wd90c24a2map_pvt *p   = (struct wd90c24a2map_pvt *)pvt;
	struct wd90c24a2map_pvt *pvts;
	struct wd90c24a2map_pvt *ppvts;
	struct wd90c24a2_cntxt *ctx   = p->context;
	struct wd90c24a2_softc *softc = p->softc;
	struct wd90c24a2_cntxt *shared_ctx	= &softc->shared_ctx;
	...
	for (pvts = ctx->pvt; pvts != NULL; pvts = pvts->next) {
		if (pvts == pvt) {
			...
			kmem_free(pvt, sizeof (struct wd90c24a2map_pvt));
			break;
		}
		ppvts = pvts;
	}
	...
	if ((ctx != shared_ctx) && (ctx->pvt == NULL)) {
		register struct wd90c24a2_cntxt *ctxptr;
		...
		if (softc->pvt_ctx == ctx) {
			...
			kmem_free(ctx, sizeof (struct wd90c24a2_cntxt));
		} else {
...
/* ARGSUSED */
static int
wd90c24a2map_dup(ddi_mapdev_handle_t old_handle,
	    void *oldpvt,
	    ddi_mapdev_handle_t new_handle,
	    void **newpvt)
{
	struct wd90c24a2map_pvt *p   = (struct wd90c24a2map_pvt *)oldpvt;
	struct wd90c24a2_softc *softc = p->softc;
	struct wd90c24a2map_pvt *pvt;
	struct wd90c24a2_cntxt *ctx;

	mutex_enter(&softc->mutex);
	if (p->context != &softc->shared_ctx) {
		ctx = (struct wd90c24a2_cntxt *)
		    kmem_zalloc(sizeof (struct wd90c24a2_cntxt), KM_SLEEP);
		*ctx = *p->context;
		ctx->pvt = NULL;
	} else
		ctx = &softc->shared_ctx;

	pvt = wd90c24a2_pvt_alloc(ctx, 0, p->len, softc);

	pvt->handle = new_handle;
	*newpvt = pvt;

	if (p->context && (p->context->flag & WD90C24A2MAP_VRT)) {
	    softc->vrtflag |= WD90C24A2VRTCTR;
#ifdef	XXXppc
	    if (softc->vrtmaps == 0)
		wd90c24a2_int_enable(softc);
...
static int
wd90c24a2_cntxsave(fbc, tec, saved)
	volatile struct fbc *fbc;
	volatile struct tec *tec;
	struct wd90c24a2_cntxt *saved;
{
	...
	CDELAY(!(fbc->l_fbc_status & L_FBC_BUSY), WD90C24A2_FBC_WAIT);
	if (fbc->l_fbc_status & L_FBC_BUSY) {
	    DEBUGF(1, (CE_CONT, "wd90c24a2: idle_wd90c24a2: status = %x\n",
			fbc->l_fbc_status));
	    return (0);
	}
...
static int
wd90c24a2_cntxrestore(fbc, tec, saved)
	volatile struct fbc *fbc;
	volatile struct tec *tec;
	struct wd90c24a2_cntxt *saved;
{
...
static
struct wd90c24a2_cntxt *
ctx_map_insert(struct wd90c24a2_softc *softc, int maptype)
{
	register struct wd90c24a2_cntxt *ctx;
	int curpid = getpid();
	...
	for (ctx = softc->pvt_ctx; ctx != NULL; ctx = ctx->link) {
		if (ctx->pid == curpid &&
		    (maptype & ctx->flag &
		    (WD90C24A2MAP_FBCTEC|WD90C24A2MAP_FB)) == 0)
			break;
	}
...
int val)
{
	ddi_io_putb(softc->regs_handle, (int)indexreg, index);
	ddi_io_putb(softc->regs_handle, (int)datareg, val);
}
#endif

int
wd90c24a2_get_indexed(
struct wd90c24a2_softc *softc,
caddr_t indexreg,
caddr_t datareg,
int index)
{
	...
	ddi_io_putb(softc->regs_handle, (int)indexreg, index);
	i = ddi_io_getb(softc->regs_handle, (int)datareg);
	return (i);
}

static void
wd90c24a2_smart_bcopy(char *from, char *to, int count)
{
	unsigned long *to4;
	unsigned long *from4;
...
#if	DRIVER_DEBUG
static void
wd90c24a2_register_dump(struct wd90c24a2_softc *softc)
{
	int i, j;
