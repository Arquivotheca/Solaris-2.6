...
#pragma ident	"@(#)uppc.c	1.7	96/10/21 SMI"

#include <sys/psm.h>
#include <sys/pit.h>

#define	DAKOTA_IACK_PADDR	0xBFFFFFF0
...
uppc_softinit(void)
{
	...
	/* map in the interrupt acknowledgement register */
	uppc_iack = psm_map((paddr_t)DAKOTA_IACK_PADDR, 1, PSM_PROT_READ);
}
...
uppc_clkinit(int hertz)
{
	...
	outb(PITCTR0_PORT, (u_char)PIT_HZ / hz);
	outb(PITCTR0_PORT, (u_char)(PIT_HZ / hz>>8));
}
