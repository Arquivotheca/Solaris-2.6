...
#pragma ident	"@(#)mmu.h	1.32	96/05/17 SMI"
...
/*
 * The following code assumes the tsb is not smaller than
 * 128k and it is not split.
 */
#define	GET_TSB_POINTER(tagacc, tsbp, tmp1, tmp2, tmp3)			\
	set	MMU_TSB, tmp1;						\
	ldxa	[tmp1]ASI_DMMU, tmp1;	/* tmp1= base and sz of tsb */	\
	sll	tagacc, TAGACC_CTX_SHIFT, tmp2;				\
	srl	tmp2, TAGACC_CTX_SHIFT, tmp2;	/* tmp2 = ctx # */	\
	and	tmp1, TSB_SIZESHIFT_MASK, tsbp;				\
	sllx	tmp2, tsbp, tmp2;	/* tmp2 = shifted ctx */ 	\
	and	tmp1, TSB_SZ_MASK, tmp3;				\
	set	1, tsbp;						\
	add	tmp3, TSB_START_SIZE, tmp3;				\
	sllx	tsbp, tmp3, tmp3;					\
	srlx	tmp1, TSBBASE_SHIFT, tmp1;				\
	sub	tmp3, 1, tmp3;		/* tmp3 = tsb offset mask */	\
	srlx	tagacc, MMU_PAGESHIFT, tsbp;	/* tsbp = va pg */	\
	sllx	tmp1, TSBBASE_SHIFT, tmp1;	/* tmp1 = tsb base */	\
	and	tsbp, tmp3, tsbp;					\
	xor	tsbp, tmp2, tmp2;	/* tmp2=offset into tsb */	\
	sllx	tmp2, TSB_ENTRY_SHIFT, tmp2;				\
	or	tmp2, tmp1, tsbp

/* CSTYLED */
#define	BUILD_TSB_TAG(vaddr, ctx, tag, tmp1)				\
	sllx	ctx, TTARGET_CTX_SHIFT, tag;				\
	srlx	vaddr, TTARGET_VA_SHIFT, tmp1;				\
	or	tmp1, tag, tag
