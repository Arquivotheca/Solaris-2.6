...
#pragma ident	"@(#)hat_sfmmu.h	1.76	96/10/17 SMI"
...
#define	ISM_SZ_SHIFT	16
#define	ISM_VB_SHIFT	16
#define	ISM_AL_SHIFT	MMU_PAGESHIFT4M		/* 4MB */
 ...
 */
#define	ISM_ALIGN	(MMU_PAGESIZE4M)	/* base va aligned to 4MB  */
#define	ISM_ALIGNED(va)	(((uint) va & (ISM_ALIGN - 1)) == 0)
#define	ISM_SHIFT(x) ((u_short) ((uint) x >> (ISM_AL_SHIFT)))
 ...
 * Also size is guarenteed to be in 4MB chunks.
 * With this restriction we only need a u_short for both base
 * and size.
 *
...
typedef union {
	struct ism_map {
		...
		u_short		ism_vbase;  /* base va of ISM for process */
		u_short		ism_size;   /* size of ISM segment */
	} ism_map_field;
	u_longlong_t	ism_map;	   /* for alignment */
} ism_map_t;

#define	ism_sfmmu	ism_map_field.ism_sfmmu
#define	ism_vbase	ism_map_field.ism_vbase
#define	ism_size	ism_map_field.ism_size
...
struct hat {
	...
	u_char		sfmmu_rmstat;	/* refmod stats refcnt */
...
#define	CTX_GAP		0x7F7	/* largest prime that will wrap every	*/
				/* 4 ctxs.				*/
...
struct hme_blk {
	...
	cpuset_t	hblk_cpuset;	/* cpu bit mask for kernel hblks */

	u_int		hblk_span;	/* span of memory hmeblk maps */

	struct {
		u_short	locked_cnt;	/* HAT_LOAD_LOCK ref cnt */
		u_int	notused:12;
...
struct tsbmiss {
	...
	uint 		dcache_line_mask; /* used to flush dcache */
	struct	ctx	*ctxs;
	int		uhashsz;
...
#define	TSBINFO_SZSHIFT	3		/* size of tsb_info */
#define	TSBMISS_SHIFT	6
#define	TSB_TAGACC	0		/* tmp 0 */
#define	TSBMISS_HMEBP	4		/* tmp 1 */
#define	TSBMISS_HATID	8		/* tmp 2 */


/*
 * The TSB
 * Only 128K- 1MB sizes is supported.  This is so xor of ctx to hash
 * tsb entry is easy and fast.
...
extern void	sfmmu_hblk_hash_rm(struct hmehash_bucket *,
			struct hme_blk *, u_longlong_t, struct hme_blk *);
 ...
 */
extern int	getctx();
extern u_int	sfmmu_vatopfn(caddr_t, sfmmu_t *);
extern u_int	sfmmu_user_vatopfn(caddr_t, sfmmu_t *);
extern void	sfmmu_memtte(tte_t *, u_int, u_int, int);
extern void	sfmmu_tteload(struct hat *, tte_t *, caddr_t, machpage_t *,
			u_int);
extern caddr_t	sfmmu_tsb_alloc(caddr_t, int);
...
extern uint	sfmmu_get_itsb();
extern uint	sfmmu_get_dtsb();
extern void	sfmmu_set_itsb(int, uint, uint);
extern void	sfmmu_set_dtsb(int, uint, uint);
extern void	sfmmu_unload_tsbctx(uint);
extern void	sfmmu_inv_tsb(caddr_t, uint);
extern u_int	sfmmu_unload_tsball();

extern sfmmu_t 	*ksfmmup;
extern struct ctx	*ctxs, *ectxs;
extern u_int		nctxs;
extern struct tsb_info tsb_bases[NCPU];
extern int		tsb_szcode;
