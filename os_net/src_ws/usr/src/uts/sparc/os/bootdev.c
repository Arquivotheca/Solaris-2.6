...
#pragma ident	"@(#)bootdev.c	1.10	96/05/22 SMI"
...
static int translate_to_v0_devname(char *devname, char *ret_buf);
...
i_devname_to_promname(char *dev_name, char *ret_buf)
{
	...
	/*
	 * On OBP 1.x sun4c, there is no prom_finddevice support and the device
	 * names are of the form sd(c,t,l).  So we map the physical
	 * device name to 1.x OBP format.
	 */
	if (prom_getversion() == 0) {
		if (translate_to_v0_devname(dev_name, ret_buf) != 0) {
			ret = EINVAL;
		} else {
			ret = 0;
		}
...
find_alternate_node(dev_info_t *parent_dip, char *devfs_nodename)
{
	...
	for (child_dip = ddi_get_child(parent_dip); child_dip != NULL;
		...
		nodeid = ddi_get_nodeid(child_dip);
...
/*
 * translate a /devices/.../dd@a,b:a name to  the form dd(c,u,p)
 * sd and le are the only valid boot devices
 */
static int
translate_to_v0_devname(char *devname, char *ret_buf)
{
	char *network = "/sbus@1,f8000000/le@0,c00000";
	char *disk = "/sbus@1,f8000000/esp@0,800000/sd@";

	if (strncmp(devname, disk, strlen(disk)) == 0) {
		char *leaf_cmpnt;
		...
		/* save the target */
		leaf_cmpnt = strrchr(devname, '/') + 1;
		ptr = strchr(leaf_cmpnt, '@');
		if (ptr == NULL) {
			return (-1);
		}
		ptr++;
		...
		opt_node = prom_findnode_byname(prom_nextnode(0),
		    "options", stk);
		prom_stack_fini(stk);
		if ((opt_node != OBP_BADNODE) && (opt_node != OBP_NONODE) &&
		    (prom_getproplen(opt_node, "sd-targets") > 0)) {
			prom_getprop(opt_node, "sd-targets", targs);
		}

		/*
		 * map the target number to an entry in sd-targets
		 */
		for (i = 0; i < strlen(targs); i++) {
			targbuf[0] = targs[i];
			/* have to reset ptr after each call to stoi() */
			ptr = targbuf;
			if (stoi((char **)&ptr) == target) {
				target = i;
				break;
			}
		}
		...
		sprintf(ret_buf, "sd(0,%d,%d)", target, unit);
	} else if (strncmp(devname, network, strlen(network)) == 0) {
		sprintf(ret_buf, "le()");
	} else {
		return (-1);
	}
...
srch_child_addrs(dev_info_t *parent_dip, char *addr, major_t major)
{
	...
	sprintf(wildcard_addr, "%s,0", addr);
...
/*
 * lock the per instance list of devinfo nodes for a given driver, maj
 * THis allows us to search for a particular node without worrying about
 * it being yanked out from underneath us.
 */
static void
...
/*
 * unlock the per instance list of devinfo nodes for a given
 * driver, maj.
 */
static void
