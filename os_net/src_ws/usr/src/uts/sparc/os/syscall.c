 ...
 */
#ident	"@(#)syscall.c	1.57	96/10/17 SMI"
...
void	psig_shared(struct regs *rp);
	...
	if (lwp->lwp_pcb.pcb_xregstat == XREGMODIFIED) {
		...
		if (p->p_warea)
			mapped = pr_mappage(sp, sizeof (struct rwindow),
				S_WRITE, 1);
		(void) _copyout((caddr_t)&lwp->lwp_pcb.pcb_xregs,
...
#ifndef	__sparcv9cpu	/* first 6 args already saved in v9 */
	...
	if (PTOU(p)->u_systrap) {
		if (prismember(&PTOU(p)->u_entrymask, code)) {
			...
			if (PTOU(p)->u_systrap &&
				 ...
				 * copy is in the registers.  For v9, we also
				 * reload lwp_args, since that's where we
				 * expect to find them when lwp_argsaved is 0.
				 */
				lwp->lwp_argsaved = 0;
#ifdef	__sparcv9cpu
				lwp->lwp_arg[0] = rp->r_o0;
...
psig_shared(struct regs *rp)
{
	...
	if (sp != rp->r_sp) {
		...
		if (_copyin((caddr_t)sp, t->t_stk, sizeof (struct rwindow))) {
			/*
