...
#ident	"@(#)hwmuldiv.s	1.22	95/03/30 SMI"	/* From SunOS 4.1 1.6 */
 ...
 */
	...
	mov	MMM_mul_CALL, %o1;
	sethi	%hi(MMM_mul_TBL), %o0;
	call	.mulcount;
	or	%o0, %lo(MMM_mul_TBL), %o0;
	restore;
#endif
 ...
 */
	...
	nop;
	save	%sp, -SA(MINFRAME), %sp;
	or	%g0, MMM_umul_CALL, %o1;
	sethi	%hi(MMM_umul_TBL), %o0;
	call	.mulcount;
	or	%o0, %lo(MMM_umul_TBL), %o0;
	restore;
#endif
...
multiply_check:
	...
	bnz	_sys_trap		! must do inline overflow, abort
	mov	T_UNIMP_INSTR, %l4	! trap number for _sys_trap
...
5:
	...
	ldd	[%l7], %g2 
	ld	[%l7 + 8], %g1 
	b	_sys_trap		! couldn't simulate quickly
...
.mulchksave:			
	...
	.global		MMM_mul_TBL
	.reserve	MMM_mul_TBL, 64*64*4+4, ".data", 4

	.global		MMM_umul_TBL
	.reserve	MMM_umul_TBL, 64*64*4+4, ".data", 4


/*
 *	the following routine gathers a scatter-plot of the arguments
 *	to the multiply routines.
 */

#define MMM_mul_CALL	8
#define MMM_umul_CALL	9
...
#define CNT		SCALE
#define OFFSET		%o4
#define SCRATCH4	OFFSET
#define	PROF_P		%o5
!
!	The tables look like this along both dimensions:
!
!  signed:
!
!		0
...
!
!	The address of the pointer to the table for the calling
!	routine was left for us in %o0.
!
!	A code for the calling routine was left in %o1
!
!
	.section	".text"
	.align	4;
	.global .mulcount
	.type	.mulcount,#function
.mulcount:
	sethi	%hi(mulprofing),PROF_P
	ld	[PROF_P+%lo(mulprofing)],SCRATCH4
	set	6,SCALE				!scale for first pass through
	tst	SCRATCH4
	bne	out				!check for recursion

!  check args
	tst	BASE				!base address initialized?
	be	oops
	tst	CALL				!call initialized?
	be	oops
	mov	%g0, OFFSET
	mov	ARG0, IN
	st	CALL, [BASE]
	add	BASE, 4, BASE			!skip ovwer table->call
!
.top:
	andcc	CALL,1,%g0			!is this an unsigned operation?
	bnz	unsigned
	tst	IN		!double duty: unsigned zero/signed positve

	bge	positive		!signed
	nop

	sub	%g0,IN,IN		!negative: make it positive
	add	OFFSET, 32, OFFSET	!offset to second half of table
positive:
	addcc	IN,IN,IN	!for signed, cond. codes are set here
unsigned:
	bz	.home	!for unsigned, cond. codes were set in the delay slot
	nop
.mloop:
	addcc	IN,IN,IN
	bcc	.mloop
	inc	OFFSET
.home:
	sll	OFFSET, SCALE, OFFSET	!found the highest order 1 bit
	mov	ARG1,IN			!back for the second argument
	tst	SCALE			!is this the first arg?
	bnz	.top
	mov	%g0, SCALE		!no shift for second arg

	sll	OFFSET,2,OFFSET		!scale for integers
	ld	[BASE+OFFSET],CNT
	inc	CNT
!
	retl
...
oops:
	.seg	".data"
$5:
	.ascii	"mulcount\72 bad arguments, called from 0x%x\12base address 0x%x, Call %d\12\0"
	.seg	".text"
	save	%sp, -64, %sp
	set	$5,%o0		!load string
	mov	%i7,%o1		!load return address
	mov	%i0,%o2		!load BASE
	call	printf,5
	mov	%i1,%o3		!load CALL
	ret
