...
#pragma ident  "@(#)modstubs.s	1.84	96/10/23 SMI"
...
/*
 * Macros for modstubbed system calls.
 */
...
1: \
	set	-SA(MINFRAME), %g1;				\
	save	%sp, %g1, %sp;	/* new frame same size */	\
	sub	%g0, %g1, %l4;  /* size of stack frame */	\
	sethi	%hi(fcnname/**/_info), %l5;			\
	b	stubs_common_code;				\
	or	%l5, %lo(fcnname/**/_info), %l5;		\
...
#define SCALL_STUB(module, fcnname, install_fcn, retfcn, weak) \
	.seg	".text";	\
	.global	fcnname;	\
fcnname: \
	save	%sp,-SA(MINFRAME), %sp;	/* new window */	\
	...
	ld	[%l5], %l0;					\
	cmp	%l1, %l0;					\
	bne,a	1f;		/* yes, so do the mod_hold */	\
	restore;						\
	mov	%l0, %g1;					\
	jmpl	%g1, %g0;	/* no, just jump to retfcn */	\
	restore;						\
1: \
	set	-SA(MINFRAME), %g1;				\
	save	%sp, %g1, %sp;	/* create frame same size */	\
	sub	%g0, %g1, %l4;  /* size of stack frame */	\
	sethi	%hi(fcnname/**/_info), %l5;			\
	b	stubs_common_code;				\
	or	%l5, %lo(fcnname/**/_info), %l5;		\
...
/*
 * Macro for modstubbed system calls whose modules are
 * not unloadable.
 */

#define	SCALL_NU_STUB(module, fcnname, retfcn)	\
		SCALL_UNLOADABLE(module, fcnname, retfcn, retfcn, 0)
