...
#pragma ident	"@(#)sxreg.h	1.5	93/07/28 SMI"
...
struct sx_register_address {
	...
	u_int	s_csr;		/* Control/Status register */
	u_int	s_ser;		/* Spam Error Register */
	u_int	s_pg_lo;	/* lower page bounds register	*/
	u_int	s_pg_hi;	/* Upper page bounds regiser	*/
	u_int	s_planereg;	/* Plane register		*/
	u_int	s_ropreg;	/* Raster operations control register */
	u_int	s_iqcounter;	/* instruction queue overflow counter */
	u_int	s_diagreg;	/* diagnostics control register */
	u_int	s_iq;		/* instruction queue		*/
	u_int	s_pad1;		/* reserved			*/
	u_int	s_idreg;	/* ID register			*/
	u_int	s_r0_init;	/* Write to R0 to zero		*/
	u_int	s_reset;	/* Write to reset SX		*/
	u_int	s_sync;		/* SX hardware sync		*/
				/* A write of any value to this */
				/* location, causes SX to issue */
				/* Relinquish & Retry to the host */
				/* processor until SX IQ empties */
	u_int	s_pad2[50];	/* Reserved			*/
	u_int	s_regfile[SX_NREGS];	/* Direct read/write */
	u_int	s_q_regfile[SX_NREGS];	/* Direct read; queued write */
};
