...
#pragma ident	"@(#)sxio.h	1.19	94/03/17 SMI"
...
struct sx_set_pgbnds {		/* for SX_SET_PGREGS command */
	...
	u_int	sp_vaddr;	/*  Virtual address which corresponds to */
				/*  physically contiguous DRAM */
	u_int	sp_len;		/* Length in bytes */
};
...
struct sx_valid_va {
	...
	u_int	sp_vaddr;	/* Range of user provided virtual address */
	u_int	sp_len;		/* and length */
	u_int	sp_base_vaddr;	/* Driver returns virtual address it maps */
	u_int	sp_base_len;	/* which falls within this subrange */
};
...
struct sx_original_va {
	...
	u_int	sp_sx_vaddr;	/* User specified SX mapped virtual addr */
	u_int	sp_orig_vaddr;	/* Driver returns corresponding original */
				/* virtual address */
};
...
typedef struct sx_cachectrl {
	...
	u_int   sc_orig_vaddr;	/* User specified original virtual addr */
	u_int   sc_len;		/* length in bytes of the address range */
...
struct sx_cntxt {
	...
	u_int	spc_csr;		/* Control/Status register */
	u_int	spc_ser;		/* Spam Error Register */
	u_int	spc_pg_lo;		/* lower page bounds register	*/
	u_int	spc_pg_hi;		/* Upper page bounds regiser	*/
	u_int	spc_planereg;		/* Plane register		*/
	u_int	spc_ropreg;		/* Raster operations control register */
	u_int	spc_iqcounter;		/* SX timer */
	u_int	spc_diagreg;
	u_int	spc_regfile[SX_NREGS];	/* SX register file */
};
...
struct sx_seg_data {	/* Per segment private data	*/
	...
	u_int	sd_prot;	/* Protections for addresses mapped by seg */
	u_int	sd_maxprot;	/* Max prot for addresses mapped by seg */
	...
	u_int	sd_valid;	/* This segment has at least one valid */
				/* translation set up */
	u_int	sd_objtype;	/* Type of underlying memory i.e physically */
				/* contiguous DRAM, VRAM etc. SX registers */
	u_int	sd_cntxtnum;	/* SX context number for this address space */
};
