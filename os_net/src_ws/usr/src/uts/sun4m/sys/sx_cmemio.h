...
#pragma ident	"@(#)sx_cmemio.h	1.17	94/01/14 SMI"
...
struct cmresv {
	u_int	cmr_offset;		/* unique offset for ever allocation */
	u_int	cmr_size;		/* size of allocation */
	int	cmr_pfbase;		/* starting pfn */
	int	cmr_refcnt;		/* # of ref'ing segs and clones */
	ulong	*cmr_proc;		/* owner for private allocations */
...
struct segsx_cmem_data {
	...
	u_int	offset;
	u_int	prot;	/* Protection for this segment */
	u_int	maxprot; /* Maximum allowable protection for this segment */
...
struct sx_cmem_list {
	...
	u_int   scl_lpfn;	/* beginning page frame number */
	u_int   scl_hpfn;	/* ending page frame number */
	u_int   scl_pages;	/* number of contig pages */
