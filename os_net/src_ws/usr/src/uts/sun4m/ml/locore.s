...
#pragma ident	"@(#)locore.s	1.189	96/10/30 SMI"
...
{}
	...
	! in sun4m locore...see bug 1248925).
...
.level14:
	sethi   %hi(snooping), %o1
	ld      [%o1 + %lo(snooping)], %o1
	tst     %o1
	be      1f
	nop
	save    %sp, -SA(MINFRAME), %sp ! get a new window
...
LEDPTR = 49
	! end of LEDPAT
	.word	0		! LEDIGNORE number of ticks to ignore
...
5:
	...
	! Now check to see if we actually want to be updating the
	! LED's right now...
	set	.led, %l5		! countdown to next update of LEDs
	ld	[%l5 + LEDIGNORE], %g1	! load the number of ticks to ignore
	subcc	%g1, 1, %g1		! decrement number remaining
	bg,a	3f			! we should ignore it this time
	st	%g1, [%l5 + LEDIGNORE]	! store the new count

	! We could be updating the LED's;  reload the ticks to ignore.
	sethi	%hi(hz), %g1		! load the value of Hz
	ld	[%g1 + %lo(hz)], %g1
	srl	%g1, 6, %g1		! The number to ignore is Hz / 64
	st	%g1, [%l5 + LEDIGNORE]
...
.fix_alignment:
	CPU_ADDR(%l5, %l4)		! load CPU struct addr to %l5 using %l4
	ld	[%l5 + CPU_THREAD], %l5	! load thread pointer
	ld	[%l5 + T_STACK], %l5	! load base of stack, same as mpcb
