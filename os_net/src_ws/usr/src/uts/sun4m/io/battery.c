 ...
 * All rights reserved.
 */
#pragma ident	"@(#)battery.c	1.16	96/09/24 SMI"
...
static int		battery_id;	/* timeout id */
...
_init(void)
{
	...
	if ((e = mod_install(&modlinkage)) == 0) {
		mutex_init(&battery_lock, "battery module lock",
		    MUTEX_DEFAULT, NULL);
		cv_init(&battery_cv, "battery module cv", CV_DEFAULT, NULL);
...
static void	batt_timeout(queue_t *);
...
batt_open(queue_t *rq, dev_t *dev, int flag, int sflag, cred_t *cred)
{
	...
	while (battery_state != MOD_OPEN) {
		...
		case MOD_CLOSED:
			...
			linkb(mp, nmp);
			prt_msg(mp);
			battery_id = timeout(batt_timeout, (caddr_t)WR(rq),
			    TIMEOUT);
...
battery_poll(queue_t *wq)
{
	...
	ASSERT(!MUTEX_HELD(&battery_lock));
	...
	*mp->b_wptr++ = (char)battery_cmds[battery_cmd];
	...
	prt_msg(mp);
	battery_id = timeout(batt_timeout, (caddr_t)wq, TIMEOUT);
	putnext(wq, mp);
}
