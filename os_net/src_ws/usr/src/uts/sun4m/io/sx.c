...
#pragma ident		"@(#)sx.c 1.116	96/10/17 SMI"
...
static	u_offset_t segsx_getoffset(struct seg *, caddr_t);
...
static	int	segsx_pagelock(struct seg *, caddr_t, u_int,
			struct page ***, enum lock_type, enum seg_rw);
static int	segsx_getmemid(struct seg *, caddr_t, memid_t *);
...
static struct modldrv modldrv = {
	...
	"Device/Segment driver for SX V1.116",
	&sx_ops	/* devops */
};
	...
	off_t len, u_int prot, u_int maxprot, u_int flags, cred_t *cred)
{
	...
	u_int pf;
	void map_addr(caddr_t *, u_int, offset_t, int);
...
/*ARGSUSED*/
static int
segsx_pagelock(struct seg *seg, caddr_t addr, u_int len,
