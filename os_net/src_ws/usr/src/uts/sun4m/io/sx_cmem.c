...
#pragma ident   "@(#)sx_cmem.c 1.83     96/09/24 SMI"
...
static int sx_cmem_rmapinfo(caddr_t arg);
	...
	switch (cmd) {
		...
		while (attempt_pages >= PMEMPAGE_1MB) {
			...
			for (tmp_pp = pp, p_count = attempt_pages; p_count;
			    tmp_pp = page_next(tmp_pp), p_count--) {
    ...
    off_t len, u_int prot, u_int maxprot, u_int flags, cred_t *cred)
{
	...
	if ((flags & MAP_FIXED) == 0) {
		 ...
		 * Pick an address
		 */
		map_addr(addr, len, (offset_t)0, 0);
		if (*addr == NULL)
			return (ENOMEM);
	} else {
		/*
...
static	u_offset_t segsx_cmem_getoffset(struct seg *, caddr_t);
...
static	int segsx_cmem_pagelock(struct seg *, caddr_t, u_int,
			struct page ***, enum lock_type, enum seg_rw);
static int	segsx_cmem_getmemid(struct seg *, caddr_t, memid_t *);
...
/*ARGSUSED*/
static int
segsx_cmem_pagelock(struct seg *seg, caddr_t addr, u_int len,
