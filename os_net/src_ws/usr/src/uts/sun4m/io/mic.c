...
#pragma ident	"@(#)mic.c	1.14	96/09/24 SMI"
...
static void	mic_txint(mic_port_t *);
...
static void	mic_reioctl(mic_port_t *);
static void	mic_resend(mic_port_t *);
...
mic_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		...
		for (i = 0; i < PORTS; i++) {
			...
			mutex_init(&port->port_lock, "mic port lock",
			    MUTEX_DRIVER, NULL);
			mutex_init(&port->tx_lock, "mic transmit lock",
			    MUTEX_DRIVER, unitp->mic_intr);
			mutex_init(&port->rx_lock, "mic receive lock",
			    MUTEX_DRIVER, unitp->mic_intr);
			cv_init(&port->tx_done_cv, "mic txdone cv",
			    CV_DRIVER, NULL);
			port->unitp = unitp;
...
mic_process(mic_port_t *port)
{
	...
	if ((mp = getq(q)) != NULL) {
		...
		case M_BREAK:
			...
			port->scc_regs->lcr |= SETBREAK;
			(void) timeout((void (*)())mic_txint, (caddr_t)port,
			    drv_usectohz(250000));
			break;
...
mic_ioctl(mic_port_t *port, queue_t *wq, mblk_t *mp)
{
	...
	if ((datasize = ttycommon_ioctl(tp, wq, mp, &error)) != 0) {
		...
		if (port->ioctl_id)
			unbufcall(port->ioctl_id);
		port->ioctl_id = bufcall(datasize, BPRI_HI, mic_reioctl,
		    (intptr_t)port);
		port->flags |= PORT_TXCHK;
		ddi_trigger_softintr(port->unitp->soft_id);
		return;
	...
	if (error == 0) {
		...
		case TCSBRK:
			...
			if (*(int *)mp->b_cont->b_rptr == 0) {
				...
				port->flags |= PORT_BREAK;
				port->scc_regs->lcr |= SETBREAK;
				(void) timeout((void (*)())mic_txint,
				    (caddr_t)port, drv_usectohz(250000));
			}
			break;
