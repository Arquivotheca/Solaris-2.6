...
#pragma ident	"@(#)pmc.c	1.15	96/09/24 SMI"
...
static int	platform_power(power_req *);
...
static struct modldrv modldrv = {
	...
	"Power Management prototype v1.15",
	&pmc_ops
};
...
pmc_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		...
		mutex_init(&unitp->pmc_lock, "pmc mutex", MUTEX_DRIVER,
		    unitp->pmcibc);
		cv_init(&unitp->a2d_cv, "pmc a2d cv", CV_DRIVER, NULL);
		...
		/*
		 * add to properties for power management
		 */
		prop_val = platform_power;
		if (ddi_prop_create(DDI_DEV_T_NONE, ddi_root_node(),
		    DDI_PROP_CANSLEEP, "platform-pm", (caddr_t)&prop_val, 4)
		    != DDI_SUCCESS)
			cmn_err(CE_WARN, "pmc: Unable to create property");
		break;
