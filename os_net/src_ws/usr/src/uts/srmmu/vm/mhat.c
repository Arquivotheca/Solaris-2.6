...
#pragma ident   "@(#)mhat.c 1.76     96/06/25 SMI"
...
hat_init()
{
	...
	for (hat = hatsNHATS - 1; hat >= hats; hat--) {
		mutex_init(&hat->hat_unload_other, "hat_unload_other",
		    MUTEX_DEFAULT, NULL);
 ...
 */
void
hat_chgprot(struct hat *hat, caddr_t addr, u_int len, u_int vprot)
{
	struct as *as = hat->hat_as;
