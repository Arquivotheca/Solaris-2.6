...
#pragma ident   "@(#)hat_srmmu.c 1.214     96/10/17 SMI"
...
const char *srmmu_version = "10/17/96 1.214";
	...
	int flags;
{
	...
	srmmu_pteload(as, addr, pp, &apte, 3, attr, flags);
}
...
load_l3:
			srmmu_pteload(as, addr, NULL, &ptpe.pte, 3, prot,
			    flags);
			pf++;
			addr += MMU_PAGESIZE;
			len -= MMU_PAGESIZE;
...
srmmu_vtop_clrattr(int opprot, u_int vprot, int *pprot)
{
	...
	cmn_err(CE_PANIC, "srmmu_vtop_clrattr: bad vprot 0x%x\n", vprot);
...
vac_retry:
	...
	if (pte_valid(&oldpte)) {
		...
		if (((*x) & ~PTE_RM_MASK) != ((*y) & ~PTE_RM_MASK)) {
			...
			(void) mmu_writepte(opte, *(u_int *)&value, addr,
			    level, as->a_hat,
			    (flags & HAT_LOAD_RELOAD) ? 0 : PTE_RM_MASK);
		} else {
#ifdef DEBUG
	...
	} else if ((flags & HAT_LOAD_RELOAD) != 0) {
	...
	page_t *rpp[];
{
	...
	*pmtx = mtx;
	srmmu_unload_loop(l3ptbl, 0, NL3PTEPERPT, flags, rpp);
	...
	page_t	**pp_to, **pp_from;
{
	...
	do {
		...
		if (!pte_valid(&t2pte)) {
			cmn_err(CE_PANIC,
			    "srmmu_pageflip: invalid kernel addr %X", kaddr);
			/*NOTREACHED*/
		}
