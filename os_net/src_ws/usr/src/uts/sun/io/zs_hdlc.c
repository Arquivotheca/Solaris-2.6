...
#pragma ident	"@(#)zs_hdlc.c 1.46	96/10/21 SMI"
		...
		if ((zss->sl_rstandby[n] = \
			...
			if (zss->sl_txstate != TX_OFF) { \
			    ...
			    zss->sl_bufcid = bufcall(zss->sl_mru, \
				    BPRI_MED, zsh_callback, (intptr_t)zs); \
			    break; \
			} else \
				mutex_exit(zs->zs_excl_hi); \
		    } \
...
zsh_attach(dev_info_t *dev, ddi_attach_cmd_t cmd)
{
	...
	for (; unit < maxzsh; unit += 4) {
		sprintf(name, "%d", unit);
		if (ddi_create_minor_node(dev, name, S_IFCHR, unit,
		...
		if ((unit + 1) >= maxzsh)
			break;
		sprintf(name, "%d", unit + 1);
		if (ddi_create_minor_node(dev, name, S_IFCHR, unit + 1,
		    DDI_PSEUDO, NULL) == DDI_FAILURE) {
			ddi_remove_minor_node(dev, NULL);
			return (DDI_FAILURE);
		}
...
zsh_open(queue_t *rq, dev_t *dev, int flag, int sflag, cred_t *cr)
{
	...
	if (sflag != CLONEOPEN) {
		...
		pm_busy_component(zs->zs_dip, unit%2+1);
...
zsh_close(queue_t *rq, int flag)
{
	...
	if (stp->str_state == STR_CLONE) {
		...
		pm_idle_component(zs->zs_dip, zs->zs_unit%2+1);
	}
