...
struct mdi_softc {
	...
	caddr_t	mdi_vrt_vaddr;	/* Shadow page containing vertical */
					/* retrace counter	*/
	page_t		*mdi_vrt_pp;	/* page for vrt cntr shadow page */
...
static struct mdi_data {	/* Per segment private data	*/
	...
	page_t	*dp;	/* page for this mapping */
};
...
mdi_ioctl(dev_t dev, int cmd, intptr_t arg, int mode, cred_t *credp, int *rvalp)
{
	...
	case SET_MONITOR_POWER: {
		if (arg < 0 || arg > 1)
			...
			error = mdi_set_mon_pwr(softc, (int)arg);
	}
	break;
...
static u_offset_t segmdi_getoffset(struct seg *, caddr_t);
...
static int segmdi_pagelock(struct seg *, caddr_t, u_int,
		struct page ***, enum lock_type, enum seg_rw);
static int segmdi_getmemid(struct seg *, caddr_t, memid_t *);
...
/*ARGSUSED*/
static int
segmdi_pagelock(struct seg *seg, caddr_t addr, u_int len,
...
mdi_mmap(dev_t dev, off_t offset, int prot)
{
	...
	if (mdi_getmapinfo(dev, offset, &objtype, &prot_allowed,
	    &map_allowed, &len_allowed, &pfnum, &pp) == -1)
		return (-1);
	...
	cred_t *cred)
{
	...
	if (mdi_getmapinfo(dev, offset, &objtype, &prot_allowed,
	    &maptype_allowed, &len_allowed, &pfnum, &pp) == -1)
		return (EACCES);
	...
	if ((flags & MAP_FIXED) == 0) {
		 ...
		 */
		(void) map_addr(addr, len, (offset_t)0, 0);
		if (*addr == NULL)
			return (ENOMEM);
	...
	ASSERT(pp == NULL || objtype == MDI_SHDW_VRT_MAP);
...
mdi_setupinfo(dev_info_t *dip, int instance)
{
	...
	softc->mdi_vrt_pp = page_numtopp_nolock(mp[MDI_SHDW_VRT_INDX].pagenum);
	if (softc->mdi_vrt_pp == NULL) {
		return (B_FALSE);
	}
