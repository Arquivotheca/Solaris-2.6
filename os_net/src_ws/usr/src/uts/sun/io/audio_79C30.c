...
#pragma ident	"@(#)audio_79C30.c	1.42	95/05/26 SMI"
...
/* XXX - Needed for hack to detect Galaxy CPU */
#include <sys/cpu.h>
...
audio_79C30_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		 ...
		 *
		 * XXX - This test for the Galaxy must be changed!
		 */
		 ...
		 * structure.
		 *
		 * XXX - With the sun4c architecture, we know that
		 * exactly one device is there; we should probe anyway,
		 * to be safe.
		 */
		unitp->dip = dip;
		...
		mutex_init(&audio_79C30_hilev, "audio_79C30 intr level 13 lock",
		    MUTEX_DRIVER, (void *)audio_79C30_trap_cookie);

		mutex_init(&unitp->lock, "audio lolevel lock", MUTEX_DRIVER,
		    (void *)icl);
...
audio_79C30_open(queue_t *q, dev_t *dp, int oflag, int sflag, cred_t *credp)
{
	...
	while (unitp->cpr_state != AMD_RESUMED) {
		ASSERT(! (unitp->control.as.info.open ||
		    unitp->input.as.info.open || unitp->output.as.info.open));
		UNLOCK_AS(as);
		ddi_dev_is_needed(unitp->dip, 0, 1);
		LOCK_AS(as);
	}
...
audio_79C30_ioctl(aud_stream_t *as, queue_t *q, mblk_t *mp)
{
	...
	case AUDIO_GETDEV:	/* return device type */
		...
		strcpy(devtp->name, AMD_DEV_NAME);
		strcpy(devtp->version, AMD_DEV_VERSION);
		strcpy(devtp->config, AMD_DEV_CONFIG_ONBRD1);
