...
#pragma ident	"@(#)stc.c 1.98	96/10/21 SMI"
 ...
 */
static void
stc_draintimeout(struct stc_line_t *line)
{
	...
	while (line->stc_sscnt) {
		...
		if ((bp = allocb(cc, BPRI_MED)) != NULL) {
			...
			if (!(line->stc_wbufcid = bufcall(cc, BPRI_HI,
				stc_drainbufcall, (intptr_t)line))) {
				mutex_enter(stc->stc_mutex);
 ...
 */
static void
silocopy(struct stc_line_t *line, u_char *buf, int count)
{
 ...
 */
static void
stc_reioctl(struct stc_line_t *line)
{
	register queue_t *q;
	register mblk_t *mp;
...
stc_ioctl(queue_t *q, mblk_t *mp)
{
	...
	if (datasize) {
		...
		unbufcall(line->stc_wbufcid);
	    if (!(line->stc_wbufcid =
		bufcall(datasize, BPRI_HI, stc_reioctl, (intptr_t)line))) {
		cmn_err(CE_CONT,
 ...
 */
static void
stc_restart(struct stc_line_t *line)
{
	register queue_t *q;
...
static void
stc_timeout(struct stc_line_t *line)
{
 ...
 */
static void
ppc_out(struct stc_line_t *line)
{
	register struct stc_unit_t *stc = line->stc;
	register struct ppcregs_t *regs = stc->ppc;
 ...
 */
static void
ppc_acktimeout(struct stc_line_t *line)
{
	register struct stc_unit_t *stc = line->stc;
	register struct ppcregs_t *regs = stc->ppc;
	register queue_t *q;
	register int busy_timeout;
