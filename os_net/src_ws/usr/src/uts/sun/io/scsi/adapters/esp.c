 ...
 * All rights reserved.
 */
#pragma ident  "@(#)esp.c 1.231     96/10/16 SMI"
...
_NOTE(SCHEME_PROTECTS_DATA("unique per pkt", scsi_pkt esp_cmd buf))
_NOTE(SCHEME_PROTECTS_DATA("stable data", scsi_address))
_NOTE(SCHEME_PROTECTS_DATA("No Mutex Needed", esp_poll_last))
...
static struct modldrv modldrv = {
	...
	"ESP SCSI HBA Driver 1.231", /* Name of the module. */
	&esp_ops,	/* driver ops */
};
...
_init(void)
{
	...
	rw_init(&esp_global_rwlock, "esp_global_rwlock", RW_DRIVER, NULL);
	...
	if ((rval == SCSIPROBE_EXISTS) &&
		...
		if (options != -1) {
			...
			if (options & SCSI_OPTIONS_FAST) {
				esp->e_default_period[tgt] = (u_char)
				    MIN_SYNC_PERIOD(esp);
			} else {
				esp->e_default_period[tgt] = (u_char)
...
esp_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	 ...
	 */
	esp = kmem_zalloc(sizeof (struct esp), KM_NOSLEEP);
	...
	(void) sprintf(buf, "esp%d_mutex", instance);
	mutex_init(ESP_MUTEX, buf, MUTEX_DRIVER, esp->e_iblock);

	/*
	 * initialize mutex for startQ
	 */
	(void) sprintf(buf, "esp%d_startQ_mutex", instance);
	...
	if (esp_softc == esp) {
		...
		if (esp_timeout_id == 0) {
			esp_timeout_id = timeout(esp_watch, (caddr_t)0,
			    esp_tick);
...
esp_dr_detach(dev_info_t *dev)
{
	...
	esp->e_dmaga_csr &= ~DMAGA_INTEN;
	esp->e_dma->dmaga_csr = esp->e_dmaga_csr;
	ddi_remove_intr(dev, (u_int) 0, esp->e_iblock);
	...
	if (esp_softc == esp) {
		...
		if (e == (struct esp *)NULL)
			esplog(esp, CE_PANIC, "Not in softc list!");
	}
	...
	/*
	 * esp_poll_last really only used for sun4m/4c, but to be complete...
	 */
	if (esp_poll_last == esp) {
		if (e && e->e_next)
		    esp_poll_last = e->e_next;
		else
		    esp_poll_last = esp_softc;
	}
	rw_exit(&esp_global_rwlock);
...
static u_char esp_max_spawn = 2;	/* max of 2 extra threads for 4 esps */
static u_char esp_cb_now_qlen = 5;
...
esp_create_callback_thread(struct esp *esp)
{
	...
	if ((esp_n_esps++ % esp_n_esps_per_callback_thread) == 0) {
		...
		(void) sprintf(buf, "esp%d_callback_cv",
		    esp_n_callback_threads);
		cv_init(&cb_info->c_cv, buf, CV_DRIVER, NULL);

		(void) sprintf(buf, "esp%d_callback_mutex",
		    esp_n_callback_threads);
...
/* #define	ESP_TEST_EXTRN_ALLOC */
	...
	if (pkt == NULL) {
		...
		if (cmd) {
			...
			p32 = (uint32_t *)(cmd->cmd_pkt.pkt_cdbp =
					(opaque_t)&cmd->cmd_cdb);
			*p32++	= 0;
			*p32++	= 0;
			*p32	= 0;
			p64 = (uint64_t *)(cmd->cmd_pkt.pkt_private =
					cmd->cmd_pkt_private);
			*p64	= 0;
...
_NOTE(SCHEME_PROTECTS_DATA("No Mutex Needed", esp0_intr_count))
_NOTE(SCHEME_PROTECTS_DATA("No Mutex Needed", espn_intr_count))
...
esp_poll_loop(caddr_t arg)
{
	 ...
	 * too much overhead and this is only run on sun4m/sun4c
	 * where the list never changes
	 */
...
step_done:
		...
		if (cmdamt > 0) {
			sp->cmd_pkt.pkt_state |= STATE_SENT_CMD;
			sp->cmd_cdbp = (u_char *)sp->cmd_cdbp + cmdamt;
		}
...
esp_handle_c_cmplt(register struct esp *esp)
{
	...
	register volatile u_char *c = (u_char *)esp->e_cmdarea;
...
esp_handle_cmd_start(struct esp *esp)
{
	...
	if (amt > 0) {
		...
		if (cmd_distance >= (u_int)CDB_GROUP5) {
			if (sp->cmd_pkt.pkt_reason == CMD_CMPLT)
...
again:
	mutex_enter(&esp_global_mutex);
	if (esp_timeout_initted) {
	    esp_timeout_id = timeout(esp_watch, (caddr_t)0, esp_tick);
	}
	mutex_exit(&esp_global_mutex);
 ...
 */
static void
esp_mark_packets(struct esp *esp, int slot, u_char reason, u_int stat)
{
	struct esp_cmd *sp;
