 ...
 * All rights reserved.
 */
#pragma ident  "@(#)fas.c 1.125     96/10/24 SMI"
 ...
 */
static int fas_open(dev_t *, int, int, cred_t *);
static int fas_close(dev_t, int, int, cred_t *);
static int fas_ioctl(dev_t, int, int, int, cred_t *, int *);

static struct cb_ops fas_cb_ops = {
	fas_open,
	fas_close,
	nodev,		/* strategy */
	nodev,		/* print */
...
static void fas_ncmds_checkdrain(caddr_t arg);
...
static struct modldrv modldrv = {
	...
	"FAS SCSI HBA Driver 1.125", /* Name of the module. */
	&fas_ops,	/* driver ops */
};
...
_init(void)
{
	...
	rw_init(&fas_global_rwlock, "fas_global_rwlock", RW_DRIVER, NULL);
...
fas_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	(void) sprintf(buf, "fas%d_mutex", instance);
	mutex_init(&fas->f_mutex, buf, MUTEX_DRIVER, fas->f_iblock);
	sprintf(buf, "fas%d_cv", instance);
	cv_init(&fas->f_cv, buf, CV_DRIVER, NULL);

	/*
	 * initialize mutex for waitQ
	 */
	(void) sprintf(buf, "fas%d_waitQ_mutex", instance);
	...
	if (ddi_create_minor_node(dip, "devctl", S_IFCHR, instance,
		...
		cmn_err(CE_WARN, "fas: cannot create devctl minor node");
		goto fail;
	}
	...
	if (fas_scsi_watchdog_tick == 0) {
		...
		IPRINTF2("fas scsi watchdog tick=%x, fas_tick=%x\n",
		    fas_scsi_watchdog_tick, fas_tick);
		if (fas_timeout_id == 0) {
			fas_timeout_id = timeout(fas_watch, (caddr_t)0,
			    fas_tick);
...
fas_dr_detach(dev_info_t *dip)
{
	...
	if (fas_head == fas) {
		...
		if (f == (struct fas *)NULL)
			cmn_err(CE_PANIC, "Not in softc list!");
	}
	...
	if (fas->f_quiesce_timeid) {
		untimeout(fas->f_quiesce_timeid);
	}
	 ...
	 */
	(void) kmem_free((caddr_t)fas, (sizeof (*fas)));
	fas_n_fass--;
...
/* ARGSUSED3 */
static int
fas_open(dev_t *devp, int flags, int otyp, cred_t *credp)
{
	int instance;
	struct fas *fas;
	...
	mutex_enter(FAS_MUTEX(fas));
	if ((flags & FEXCL) && (fas->f_softstate & FAS_SS_ISOPEN)) {
		mutex_exit(FAS_MUTEX(fas));
		return (EBUSY);
	}

	fas->f_softstate |= FAS_SS_ISOPEN;
	mutex_exit(FAS_MUTEX(fas));
	return (0);
}
...
/* ARGSUSED */
static int
fas_close(dev_t dev, int flag, int otyp, cred_t *credp)
{
	int instance;
	struct fas *fas;
...
/*
 * fas_ioctl: devctl hotplug controls
 */
/* ARGSUSED */
static int
fas_ioctl(dev_t dev, int cmd, int arg, int mode, cred_t *credp, int *rvalp)
{
	struct fas *fas;
	dev_info_t *self;
	...
	switch (cmd) {
		...
		case DEVCTL_BUS_QUIESCE:
			if (ndi_get_bus_state(self, &bus_state) == NDI_SUCCESS)
				if (bus_state == BUS_QUIESCED)
					break;
				if (fas_quiesce_bus(fas) != 0)
					rv = EIO;
				else
					ndi_set_bus_state(self, BUS_QUIESCED);
			break;
		...
		case DEVCTL_BUS_UNQUIESCE:
			...
			if (fas_unquiesce_bus(fas) != 0)
				rv = EIO;
			else
				ndi_set_bus_state(self, BUS_ACTIVE);
			break;
...
fas_quiesce_bus(struct fas *fas)
{
	...
	if (fas_check_outstanding(fas)) {
		...
		fas->f_quiesce_timeid = timeout(fas_ncmds_checkdrain,
		    (caddr_t)fas,
		    (FAS_QUIESCE_TIMEOUT * drv_usectohz(1000000)));
 ...
 */
static void
fas_ncmds_checkdrain(caddr_t arg)
{
...
/* #define	FAS_TEST_EXTRN_ALLOC */
	...
	if (pkt == NULL) {
		...
		if (cmd) {
			...
			p32 = (uint32_t *)(pkt->pkt_cdbp =
				(opaque_t)&cmd->cmd_cdb);
			*p32++	= 0;
			*p32++	= 0;
			*p32	= 0;
			p64 = (uint64_t *)(pkt->pkt_private =
				cmd->cmd_pkt_private);
			*p64++	= 0;
#ifdef	_LP64
			*p64	= 0;
...
fas_startcmd(register struct fas *fas, register struct fas_cmd *sp)
{
	...
	register volatile caddr_t tp = (caddr_t)fas->f_cmdarea;
	register struct scsi_pkt *pkt = CMD2PKT(sp);
...
#define	LOAD_CMDP	*(tp++)
	...
	if (TAGGED(target) && (sp->cmd_pkt_flags & FLAG_TAGMASK)) {
		...
		if ((sp->cmd_flags & CFLAG_CMDPROXY) == 0) {
			 ...
			 * the throttle is reset later in fas_watch()
			 */
	 ...
	 */
		...
		ASSERT((fas->f_wide_enabled & tshift) == 0);
...
fas_intr(caddr_t arg)
{
	...
	register int rval = DDI_INTR_CLAIMED;
...
fas_handle_c_cmplt(register struct fas *fas)
{
	register struct fas_cmd *sp = fas->f_current_sp;
	volatile struct fasreg *fasreg = fas->f_reg;
	register u_char sts, msg, intr, perr;
	...
	if (intr & FAS_INT_FCMP) {
		 ...
		 */
		*(sp->cmd_pkt->pkt_scbp) =
		    sts = fas_reg_read(fas, (u_char *)&fasreg->fas_fifo_data);
		fas->f_last_msgin = fas->f_imsgarea[0] =
		    msg = fas_reg_read(fas, (u_char *)&fasreg->fas_fifo_data);
...
fas_make_sdtr(struct fas *fas, int msgout_offset, int target)
{
	...
	u_char period = MIN_SYNC_PERIOD(fas);
	u_char offset = fas_default_offset;
...
fas_create_arq_pkt(struct fas *fas, struct scsi_address *ap)
{
	...
	u_char slot = ap->a_target * NLUNS_PER_TARGET | ap->a_lun;
	int rval = 0;
...
again:
	mutex_enter(&fas_global_mutex);
	if (fas_timeout_initted) {
		fas_timeout_id = timeout(fas_watch, (caddr_t)0, fas_tick);
	}
	mutex_exit(&fas_global_mutex);
 ...
 */
static void
fas_mark_packets(struct fas *fas, int slot, u_char reason, u_int stat)
{
	struct fas_cmd *sp = fas->f_readyf[slot];
...
fas_log(struct fas *fas, int level, const char *fmt, ...)
{
	...
	if (level == CE_CONT) {
		...
		scsi_log(dev, fas_label, level, "%s", buf);
	}
}
...
fas_dprintf(struct fas *fas, const char *fmt, ...)
{
	...
	scsi_log(dev, fas_label, SCSI_DEBUG, "%s", buf);
}
#endif
