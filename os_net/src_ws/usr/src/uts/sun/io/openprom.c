/*
 * Copyright (c) 1989-1993, 1996 by Sun Microsystems, Inc.
 * All Rights Reserved.
 */

#pragma ident	"@(#)openprom.c	1.30	96/10/18 SMI"	/* SVr4 */
	...
	cred_t *credp, int *rvalp)
{
	...
	if ((int)userbufsize <= 0)
		return (EINVAL);

	opp = kmem_zalloc(userbufsize + sizeof (u_int) + 1, KM_SLEEP);
	...
	case OPROMGETPROP:
		 ...
		 */
		if ((error = copyinstr(((caddr_t)arg + sizeof (u_int)),
		    opp->oprom_array, userbufsize, &actual_len)) != 0) {
			break;
		}
		if ((actual_len == 0) || (actual_len > OBP_MAXPROPNAME)) {
			error = EINVAL;
			break;
		}
		...
		if (valsize > 0 && valsize <= userbufsize) {
			 ...
			 * zero length property value case.
			 */
			opp->oprom_size = 0;
			(void) strcpy(opp->oprom_array, "");
			valsize = 1;
		}
		if (copyout(opp, (void *)arg, (valsize + sizeof (u_int))) != 0)
			error = EFAULT;
		break;
	...
	/*
	 * convert a prom device path to a devfs device path
	 */
	case OPROMPROM2DEVNAME: {
		char *dev_name;
		...
		/*
		 * convert the prom path to an equivalent devfs path
		 */
		...
		if (copyout(opp, (void *)arg, sizeof (u_int) + valsize) != 0)
			error = EFAULT;
	...
	case OPROMPATH2DRV: {
		...
		if (copyout(opp, (void *)arg,
		    sizeof (u_int) + opp->oprom_size + 1) != 0)
			error = EFAULT;
	}	break;
