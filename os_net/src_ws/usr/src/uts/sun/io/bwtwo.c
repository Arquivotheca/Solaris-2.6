/*
 * Copyright 1991-1996, by Sun Microsystems, Inc.
 * All rights reserved.
 */

#pragma	ident	"@(#)bwtwo.c	1.48	96/09/24 SMI"
/*
...
bw2_attach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		...
		if (pm_create_components(softc->devi, 2) == DDI_SUCCESS) {
			pm_busy_component(softc->devi, 0);
			pm_set_normal_power(softc->devi, 0, 1);
			pm_set_normal_power(softc->devi, 1, 255);
		} else {
			goto failed;
		}
...
failed:
	...
	case DDI_RESUME:
		 ...
		 */
		if ((power = pm_get_normal_power(softc->devi, 1)) ==
			DDI_FAILURE) {
			cmn_err(CE_WARN, "bw2_attach(DDI_RESUME) can't get "
				"normal power.");
			return (DDI_FAILURE);
		}
		if (bw2_power(devi, 1, power) != DDI_SUCCESS) {
			cmn_err(CE_WARN, "bw2_attach: bw2_power failed.");
			return (DDI_FAILURE);
		}
		return (DDI_SUCCESS);
...
bw2_power(dev_info_t *devi, int cmpt, int level)
{
	 ...
	 */
	sense_code = (int)softc->hw_type & MFB_SR_RES_MASK;
	if (sense_code == MFB_SR_RES_MASK)
