...
#ident	"@(#)cgsix.c	1.94	96/10/01 SMI"	/* SunOS-4.1 1.25	*/
...
static int cg6map_map(devmap_cookie_t, dev_t, u_int, offset_t, size_t,
		void **);
...
static struct modldrv modldrv = {
	...
	"cgsix driver v1.94",	/* Name of the module. */
...
cg6_probe(dev_info_t *devi)
{
	...
	if (ddi_peek32(devi, fbc, &id) == DDI_FAILURE) {
...
cg6_attach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	case DDI_RESUME:
		...
		cmn_err(CE_WARN, "cg6_attach(DDI_RESUME) can't get normal "
		    "power");
		return (DDI_FAILURE);
	    }
	    if (cg6_power(devi, 1, power) != DDI_SUCCESS) {
		cmn_err(CE_WARN, "cg6_attach: cg6_power failed");
			return (DDI_FAILURE);
	    }
	    return (DDI_SUCCESS);
...
cg6_power(dev_info_t *dip, int cmpt, int level)
{
	...
	/*
	 * If there is no CRT monitor connected, the system can be
	 * using flat panel. Let's call ddi_power() to ask the pmc
	 * chip to further power manage the flat panel.
	 */
	sense_code = *S_FHC(softc) >> FHC_CONFIG_FBID_SHIFT & 7;
	if (sense_code == 0)
	    ...
	    cred_t	*cred)
{
	 ...
	 */
	error = devmap_setup(dev, (offset_t)off, as, addrp, (u_long)len, prot,
			maxprot, flags, cred);
...
/* ARGSUSED */
static int
cg6map_map(devmap_cookie_t dhp, dev_t dev, u_int flags, offset_t off,
	size_t len, void **pvtp)
{
	...
	pvt = cg6_pvt_alloc(ctx, maptype, (u_int)off, (u_int)len, softc);
	pvt->dhp = dhp;
...
{
	register struct cg6map_pvt *p = (struct cg6map_pvt *)pvtp;
	struct cg6_softc *softc = p->softc;
...
off = %x, len = %x, dhp1 = %x, dhp2 = %x\n",
			pvtp, dhp, (u_int)off, len, new_dhp1, new_dhp2));
	...
	if (new_dhp2 != NULL) {
	    ...
	    ptmp = cg6_pvt_alloc(ctx, p->type, (u_int)(off + len),
			(u_int)length, softc);
		ptmp->dhp = new_dhp2;
	...
	void **newpvt)
{
	...
	pvt = cg6_pvt_alloc(ctx, p->type,
			(u_int)p->offset, (u_int)p->len, softc);
	...
	size_t *maplen, uint_t model)
{
	...
	DEBUGF(2, (CE_CONT, "cg6_devmap(%d), off=0x%x, len=%x, dhp=%x\n",
		getminor(dev), (u_int) off, len, dhp));
