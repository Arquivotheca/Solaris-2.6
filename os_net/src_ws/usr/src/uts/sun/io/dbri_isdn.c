...
#pragma ident	"@(#)dbri_isdn.c	1.34	94/03/31 SMI"
		...
		if (!unitp->keep_alive_running) {
			...
			if (unitp->keepalive_id == 0) {
				unitp->keepalive_id = timeout(dbri_keep_alive,
				    (caddr_t)unitp, TICKS(Keepalive_timer));
			}
		}
 ...
 */
static void
dbri_te_timer(caddr_t arg)
{
	aud_stream_t *as = (aud_stream_t *)(void *)arg;
	dbri_unit_t *unitp;
	dbri_bri_status_t *bs;
...
dbri_nttimer_t101(caddr_t arg)
{
	...
	if ((bs->sbri.tss == DBRI_NTINFO0_G1) ||
		...
		unitp->nttimer_t102_id = timeout(dbri_nttimer_t102,
		    (caddr_t)unitp, TICKS(bs->i_var.t102));
...
dbri_te_ph_activate_req(aud_stream_t *as)
{
	...
	if ((bs->i_info.activation != ISDN_ACTIVATED) &&
		...
		if (unitp->tetimer_id)
			(void) untimeout(unitp->tetimer_id);
		unitp->tetimer_id = timeout(dbri_te_timer, (caddr_t)as,
		    TICKS(bs->i_var.t103));
	}
}
...
dbri_nt_ph_activate_req(aud_stream_t *as)
{
	...
	unitp->nttimer_t101_id = timeout(dbri_nttimer_t101, (caddr_t)unitp,
	    TICKS(bs->i_var.t101));
}
...
dbri_nt_mph_deactivate_req(dbri_unit_t *unitp)
{
	...
	if (unitp->nttimer_t102_id)
		(void) untimeout(unitp->nttimer_t102_id);
	unitp->nttimer_t102_id = timeout(dbri_nttimer_t102, (caddr_t)unitp,
	    TICKS(bs->i_var.t102));
