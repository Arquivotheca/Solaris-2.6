...
#pragma ident	"@(#)socreg.h	1.6	95/02/10 SMI"
...
typedef union soc_cr_register {
	struct cr {
		u_int	aaa:5;
		u_int	ramsel:3;	/* Ram bank select. */
		u_int	bbb:6;
		u_int	eepromsel:2;	/* Eeprom bank select. */
		u_int	ccc:5;
		u_int	burst64:3;	/* Sbus Burst size, 64 bit mode. */
		u_int	ddd:2;
		u_int	parenable:1;	/* Partity test enable. */
		u_int	parsbus:1;	/* Sbus Parity checking. */
		u_int	sbusmode:1;	/* Enhanced Sbus mode. */
		u_int	sbusburst:3;	/* Sbus burst size. */
	} reg;
	u_int	w;
} soc_cr_reg_t;
...
typedef union soc_sae_register {
	struct sae {
		u_int	aaa:29;			/* Reserved. */
		u_int	alignment_err:1;	/* Soc Alignment Error. */
		u_int	bad_size_err:1;		/* Bad Size error. */
		u_int	parity_err:1;		/* Parity Error. */
	} reg;
	u_int	w;
} soc_sae_reg_t;
...
typedef union soc_csr_register {
	struct csr {
		u_int	comm_param:8;	/* Communication Parameters. */
		u_int	aaa:4;
		u_int	soc_to_host:4;	/* Soc to host attention. */
		u_int	bbb:4;
		u_int	host_to_soc:4;	/* Host to soc attention. */
		u_int	sae:1;		/* Slave access error indicator. */
		u_int	ccc:3;
		u_int	int_pending:1;	/* Interrupt Pending. */
		u_int	nqcmd:1;	/* Non queued command */
		u_int	idle:1;		/* SOC idle indicator. */
		u_int	reset:1;	/* Software Reset. */
	} reg;
	u_int	w;
} soc_csr_reg_t;
