...
#pragma ident	"@(#)dkmpio.h	1.4	93/11/01 SMI"
...
struct dk_gmpstate {
	...
	u_int		dkg_fail_state;		/* output: fail state */
	u_int		dkg_current_status; /* output: current drive status */
...
struct dk_mpdrv_status {
	u_int	dkd_cum_drv_soft_errors; /* cumulative drive soft errors */
	u_int	dkd_cum_drv_hard_errors; /* cumulative drive media errors */
	u_int	dkd_cum_drv_retries;	/* cumulative successful drive */
 ...
 */
struct dk_mpflags {
	u_int	dkf_config_flags;		/* config flags, ro */
	u_int	dkf_control_flags;		/* control flags, rw */
...
struct dk_mpinfo {
	...
	u_int			dke_max_quiesce; /* maxtime to quiesce drive */
 ...
 */
struct dk_mpcmd {
	u_int		dkc_mpcmd;		/* command */
	u_int		dkc_mpflags;		/* execution flags */
