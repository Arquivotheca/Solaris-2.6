/*
 * Copyright 1991 (c), Sun Microsystems, inc.
 */

#ifndef	_SYS_CG8REG_H
#define	_SYS_CG8REG_H

#pragma ident	"@(#)cg8reg.h	1.7	93/04/22 SMI"
...
#if !defined(sun4c) && !defined(sun4m) && !defined(sun4d) && !defined(sun4e)
...
typedef struct	{	/* Fb attributes (from on-board PROM): */
	...
	u_int	simultaneous_8_24:1; /* 1 if sep. luts for 8-bit & 24-bit. */
...
typedef struct {	/* Brooktree 473 layout: */
	...
	u_char	ram_read_addr;	/* Address register: read from color palette */
	u_char	overlay_write_addr; /* Address register: writes overlay */
				/* palette. */
	u_char	overlay;	/* Data register for overlay. */
	u_char	control;	/* Control register. */
	u_char	overlay_read_addr; /* Address register: reads overlay */
				/* palette. */
} Bt473_Regs;
...
typedef struct	{
	...
	u_short	x_source_start;	/* X sig start (subset of incoming signal) */
	u_short	x_source_end;	/* X sig end (subset of incoming signal). */
	u_short	x_source_scale;	/* X sig scaling:pixel drop rate multiplier */
	u_short	fill2;		/* Unused. */
	u_short	y_source_start;	/* Y sig start (subset of incoming signal) */
	u_short	y_source_end;	/* Y sig end (subset of incoming signal). */
	u_short	y_source_scale;	/* Y sig scaling:scan line drop rate mult. */
	u_short	fb_pitch;	/* # pixels(visible & not) bet. scan lines */
 ...
 */
#define	I2C_ADDR_EEPROM (u_int)0xa0	/* Dev. address: eeprom for confg. */
#define	I2C_ADDR_DACS	(u_int)0x88	/* Dev. address: d to a converters */
#define	I2C_DAC0_CMD	(u_int)0x00	/* Com: d to a converter 0 control */
#define	I2C_DAC1_CMD	(u_int)0x01	/* Com: d to a converter 1 control */
#define	I2C_DAC2_CMD	(u_int)0x02	/* Com: d to a converter 2 control */
#define	I2C_DAC3_CMD	(u_int)0x03	/* Com: d to a converter 3 control */
#define	I2C_POD_CMD	(u_int)0x08	/* Com: port output data control. */
#define	I2C_POD_NTSC	(u_int)0x00	/* Data: ntsc timing - port out data */
#define	I2C_POD_PAL	(u_int)0x14	/* Data: pal timing - port out data */
...
#define	I2C_HALF_LEVEL	(u_int)0x20	/* Data: half level for d/a conv. */
...
typedef struct {	/* PIP Initialization record: */
	...
	u_short	x_source_start;	/* ... starting pixel of signal in x */
	u_short	x_source_end;	/* ... ending pixel of signal in x */
	u_short	y_source_start; /* ... starting pixel of signal in y */
	u_short	y_source_end;	/* ... ending pixel of signal in y */
