...
#pragma ident	"@(#)dbrireg.h	1.6	92/11/16 SMI"
...
typedef struct dbri_intq {
	volatile struct dbri_intq *nextq;	/* next interrupt queue ptr */
...
union dbri_md {
	volatile struct {
		...
		uchar_t *bufp;	/* buffer pointer */
		union dbri_md *fp; /* forward descriptor pointer */
	...
	volatile unsigned int _words[4];
};
