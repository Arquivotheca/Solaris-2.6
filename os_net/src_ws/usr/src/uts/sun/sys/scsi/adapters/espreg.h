...
#pragma ident	"@(#)espreg.h	1.24	96/09/09 SMI"
...
#define	ESP_CLOCK_TICK(esp)	\
	((u_int)ESP_CLOCK_DELAY * (u_int)(esp)->e_clock_conv * \
	(u_int)(esp)->e_clock_cycle) / (u_int)1000
...
#define	MIN_SYNC_TIME(esp)	\
	((u_int)MIN_SYNC((esp)) * (u_int)((esp)->e_clock_cycle)) / \
	    (u_int) 1000
#define	MAX_SYNC_TIME(esp)	\
	((u_int)MAX_SYNC((esp)) * (u_int)((esp)->e_clock_cycle)) / \
	    (u_int) 1000
