...
#pragma ident	"@(#)fasreg.h	1.13	96/09/06 SMI"
...
#define	FAS_CLOCK_TICK(fas)	\
	((u_int)FAS_CLOCK_DELAY * (u_int)(fas)->f_clock_conv * \
	(u_int)(fas)->f_clock_cycle) / (u_int)1000
...
#define	MIN_SYNC_TIME(fas)	\
	((u_int)MIN_SYNC((fas)) * (u_int)((fas)->f_clock_cycle)) / \
	    (u_int)1000
#define	MAX_SYNC_TIME(fas)	\
	((u_int)MAX_SYNC((fas)) * (u_int)((fas)->f_clock_cycle)) / \
	    (u_int)1000
