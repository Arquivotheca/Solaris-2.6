...
#pragma ident	"@(#)fc.h	1.4	95/02/10 SMI"
...
typedef struct FC2_FRAME_HDR {
	u_int	r_ctl:8,	d_id:24;
	u_int	reserved1:8,	s_id:24;
	u_int	type:8,		f_ctl:24;
	u_int	seq_id:8,	df_ctl:8,	seq_cnt:16;
	u_short	ox_id, rx_id;
	u_int	ro;
...
typedef struct LOGI_PAYLOAD {
	u_int	code;
	u_char	common_service_params[16];
	u_char	port_name[8];
	u_char	node_name[8];
	u_char	class1_service_params[16];
	u_char	class2_service_params[16];
	u_char	class3_service_params[16];
...
typedef struct fc_dataseg {
	u_int	fc_base;	/* Address of buffer. */
	u_int	fc_count;	/* Length of buffer. */
} fc_dataseg_t;
