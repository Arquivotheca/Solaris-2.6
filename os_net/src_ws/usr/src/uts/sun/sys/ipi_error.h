...
#ifndef	_SYS_IPI_ERROR_H
#define	_SYS_IPI_ERROR_H

#pragma ident	"@(#)ipi_error.h	1.5	93/02/04 SMI"

/*
 * Definitions for IPI error handling.
 */
...
#define	IE_MCH		10	/* handle machine exception */
...
#define	IE_RE_INIT	0x00200000	/* ctlr/device must be re-initializd */
...
#define	IE_REQUEUE	0x00001000	/* requeue command entirely */
 ...
 */
#define	IE_RESP_MASK	\
	(IE_PRINT | IE_CMD | IE_RETRY | IE_DUMP | \
...
/* #define	IE_DBG	0 */ /* define as IE_MSG if debugging msgs needed */
...
IPI_ERR_BIT(1, 4, IE_CMD | IE_PRINT,		"Operation Timeout") \
IPI_ERR_BIT(1, 3, IE_CMD | IE_PRINT,		"Physical Interface Check") \
IPI_ERR_BIT(1, 2, IE_RESET | IE_MSG,		"Slave Initiated Reset") \
IPI_ERR_BIT(1, 1, IE_MSG,			"Environmental Error") \
...
IPI_ERR_BIT(3, 0, IE_CMD | IE_DUMP,		"Unexpected Master Action") \
	\
...
IPI_ERR_BIT(4, 4, IE_CMD | IE_MSG,		"Position Lost") \
	\
IPI_ERR_BIT(5, 7, IE_MCH | IE_CMD | IE_MSG,	"VME bus timeout") \
IPI_ERR_BIT(5, 6, IE_MCH | IE_CMD | IE_MSG,	"VME bus unavailable") \
IPI_ERR_BIT(5, 5, IE_MCH | IE_CMD | IE_MSG,	"VME bus error") \
	\
	\
...
/*
 * IPI error table structure
 */

typedef struct ipi_errtab {
