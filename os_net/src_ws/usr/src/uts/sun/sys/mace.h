...
#pragma ident	"@(#)mace.h	1.6	93/06/08 SMI"
 ...
 */
struct	mace {
	u_char	rcvfifo;	/* R0 receive FIFO [15-00] (R) */
	u_char	xmtfifo;	/* R1 transmit FIFO [15-00] (W) */
	u_char	xmtfc;		/* R2 transmit frame control (RW) */
	u_char	xmtfs;		/* R3 transmit frame status (R) */
	u_char	xmtrc;		/* R4 transmit retry count (R) */
	u_char	rcvfc;		/* R5 receive frame control (RW) */
	u_char	rcvfs;		/* R6 receive frame status [31-00] (RO) */
	u_char	fifofc;		/* R7 FIFO frame count (R) */
	u_char	ir;		/* R8 interrupt register (R) */
	u_char	imr;		/* R9 interrupt mask register (RW) */
	u_char	pr;		/* R10 poll register (R) */
	u_char	biucc;		/* R11 BIU configuration control (RW) */
	u_char	fifocc;		/* R12 FIFO configuration control (RW) */
	u_char	maccc;		/* R13 MAC configuration control (RW) */
	u_char	plscc;		/* R14 PLS configuration control (RW) */
	u_char	phycc;		/* R15 PHY configuration control (RW) */
	u_char	chipidlo;	/* R16 chip id register [07-00] (R) */
	u_char	chipidhi;	/* R17 chip id register [15-08] (R) */
	u_char	iac;		/* R18 internal address configuration (RW) */
	u_char	reserved0;	/* R19 reserved */
	u_char	ladrf;		/* R20 logical address filter [63-00] (RW) */
	u_char	padr;		/* R21 physical address [47-00] (RW) */
	u_char	reserved1;	/* R22 reserved */
	u_char	reserved2;	/* R23 reserved */
	u_char	mpc;		/* R24 missed packet count (R) */
	u_char	reserved3;	/* R25 reserved */
	u_char	rntpc;		/* R26 runt packet count (R) */
	u_char	rcvcc;		/* R27 receive collision count (R) */
	u_char	reserved4;	/* R28 reserved */
	u_char	utr;		/* R29 user test register (RW) */
	u_char	rtr1;		/* R30 reserved test register 1 */
	u_char	rtr2;		/* R31 reserved test register 2 */
};
