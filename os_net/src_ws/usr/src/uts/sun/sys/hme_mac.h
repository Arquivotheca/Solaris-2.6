...
#pragma ident	"@(#)hme_mac.h	1.12	95/10/19 SMI"
...
struct hme_tmd {
	u_int	tmd_flags;	/* OWN, SOP, EOP, cksum ctl and bufize */
	u_int	tmd_addr;	/* 8-bye aligned buffer address */
};
...
struct hme_rmd {
	u_int	rmd_flags;	/* OWN, OVFLOW, buf/data size, cksum */
	u_int	rmd_addr;	/* 8-byte aligned buffer address */
};
...
struct hme_global {
    u_int reset;		/* Global Software Reset Command */
    u_int config;		/* Global Configuration Register */
    u_int reserved[62];
    u_int status;		/* Global Status Register */
    u_int intmask;		/* Global Interrupt Mask Register */
};
...
struct hme_etx {
    u_int txpend;		/* Transmit Pending Command */
    u_int config;		/* ETX Configuration Register */
    u_int txring;		/* Transmit Descriptor Ring Pointer */
    u_int txbuf_base;		/* Transmit Data Buffer Base Address */
    u_int txbuf_disp;		/* Transmit Data Buffer Displacement */
    u_int txfifo_wr_ptr;	/* TxFIFO Write Pointer */
    u_int txfifo_sdwr_ptr;	/* TxFIFO Shadow Write Pointer */
    u_int txfifo_rd_ptr;	/* TxFIFO Read pointer */
    u_int txfifo_sdrd_ptr;	/* TxFIFO Shadow Read pointer */
    u_int txfifo_pkt_cnt;	/* TxFIFO Packet Counter */
    u_int state_mach;		/* ETX State Machine Register */
    u_int txring_size;		/* Descriptor Ring Size */
    u_int txbuf_ptr;		/* Transmit Data Buffer Pointer */
};

struct hme_txfifo_aperture {
    u_int txfifo_loapert[512];	/* TxFIFO Lower Aperture */
    u_int txfifo_hiapert[512];	/* TxFIFO Higher Aperture */
};
...
struct hme_erx {
    u_int config;		/* ERX Configuration Register */
    u_int rxring;		/* Receive Descriptor Ring Pointer */
    u_int rxbuf_ptr;		/* Receive Data Buffer Pointer */
    u_int rxfifo_wr_ptr;	/* RxFIFO Write Pointer */
    u_int rxfifo_sdwr_ptr;	/* RxFIFO Shadow Write Pointer */
    u_int rxfifo_rd_ptr;	/* RxFIFO Read pointer */
    u_int rxfifo_pkt_cnt;	/* RxFIFO Packet Counter */
    u_int state_mach;		/* ERX State Machine Register */
};

struct hme_rxfifo_aperture {
    u_int rxfifo_loapert[512];	/* RxFIFO Lower Aperture */
    u_int rxfifo_hiapert[512];	/* RxFIFO Higher Aperture */
};
 ...
 */
struct	hme_bmac {
	u_int	xifc;		/* XIF Configuration register [9-0] (RW) */
	u_int	pad1[129];	/* XXX unused */
	...
	u_int	fccnt;		/* first succesful coll. counter [15-0] (RW) */
	...
	u_int	pad2[44];	/* XXX Unused */
	u_int	rxrst;		/* rx software reset register (RW) */
	u_int	rxcfg;		/* rx configuration register [12-0] (RW) */
...
struct hme_mif {
	u_int mif_bbclk;	/* MIF Bit Bang Clock */
	u_int mif_bbdata;	/* MIF Bit Bang Data */
	u_int mif_bbopenb;	/* MIF Bit Bang Output Enable */
	u_int mif_frame;	/* MIF Frame - ctl and data */
	u_int mif_cfg;		/* MIF Configuration */
	u_int mif_imask;	/* MIF Interrupt mask */
	u_int mif_bsts;		/* MIF Basic/Status register */
	u_int mif_fsm;		/* MIF State machine register */
};
...
#ifdef MPSAS
#define	HME_PHYRST_MAXDELAY	(1000000)
#else
 ...
 */
#ifdef	MPSAS
#define	HMEMAXRSTDELAY	(1000000)
#define	HMEPERIOD	(100000)	/* period to wait */
#else
...
#define	HMEDELAY(c, n) \
	{ \
		register int N = n / hme_mpsas_period; \
		while (--N > 0) { \
			if (c) \
				break; \
