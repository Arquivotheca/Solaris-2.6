...
#pragma ident	"@(#)cg14io.h	1.17	94/05/11 SMI"
...
struct mdi_set_counters {
	...
	u_short	m_xcs;		/* Transfer cycle clear		*/
	u_short	m_xcc;		/* Transfer cycle clear		*/
};
...
struct	mdi_set_clut {
	...
	u_char *alpha;		/* Alpha values of the entry	*/
	u_char *red;		/* Red component of an entry	*/
	u_char *green;		/* Green component of an entry */
	u_char *blue;		/* Blue component of an entry	*/
};
...
struct	mdi_set_xlut {
	...
	int index;	 /* Starting index into the table */
	int count;	 /* Count of entries to update */
	u_char *xbuf;	 /* Component value of an entry	*/
	u_char *maskbuf; /* Array of mask of bits to preserved. */
...
struct	mdi_set_degammalut {
	...
	int count;	/* Count of entries to read/update */
	u_char *degamma; /* Table of degamma entries */
};
...
struct mdi_cursor_info {
	u_int	curs_enable0[32];	/* Cursor enable plane */
	u_char	curs_ctl;		/* Cursor control reg */
	u_short	curs_xpos;		/* Cursor x position */
	u_short	curs_ypos;		/* Cursor y position */
};
