...
#pragma ident	"@(#)openprom.h	1.17	94/10/13 SMI"
...
/*
 * Needed constant for sunmon Romvec location, since it isn't passed in:
 */
#define	SUNMON_ROMVEC		(0xffe81000)

/*
 * Don't mess with this, this is for SUNMON ONLY (i.e. sun4, ok?)
 */
#define	SUNMON_ENTER_MON	(*romp->sunmon.v_abortent)
#define	SUNMON_ABORTENT		(*romp->sunmon.v_abortent)
#define	SUNMON_EXIT_TO_MON	(*romp->sunmon.v_exit_to_mon)
#define	SUNMON_BOOTPARAM	(*romp->sunmon.v_bootparam)
#define	SUNMON_FWRITESTR	(*romp->sunmon.v_fwritestr)
#define	SUNMON_FBADDR		(romp->sunmon.v_fbaddr)
#define	SUNMON_FBTYPE		(*romp->sunmon.v_fbtype)
#define	SUNMON_INITGETKEY	(*romp->sunmon.v_initgetkey)
#define	SUNMON_MEMORYSIZE	(*romp->sunmon.v_memorysize)
#define	SUNMON_MEMORYBITMAP	(romp->sunmon.v_memorybitmap)
#define	SUNMON_MEMORYAVAIL	(*romp->sunmon.v_memoryavail)
#define	SUNMON_SETCXSEGMAP	(*romp->sunmon.v_setcxsegmap)
#define	SUNMON_MAYPUT		(*romp->sunmon.v_mayput)
#define	SUNMON_MAYGET		(*romp->sunmon.v_mayget)
#define	SUNMON_BOOT		(*romp->sunmon.v_boot_me)
#define	SUNMON_INSOURCE		(*romp->sunmon.v_insource)
#define	SUNMON_OUTSINK		(*romp->sunmon.v_outsink)
#define	SUNMON_HANDLER		(*romp->sunmon.v_vector_cmd)
 ...
 */
#define	OBP_SETCXSEGMAP		(*romp->obp.op_setcxsegmap)
