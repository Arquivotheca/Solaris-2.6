...
#pragma ident	"@(#)i82586.h	1.5	93/01/13 SMI"
...
struct tie_device {
	u_short	tie_csr;
	u_char	tie_unused[SCSIREG];	/* SCSI registers & interrupt vector */
	u_char	tie_ivec;		/* Ethernet interrupt vector */
};
...
typedef u_short		ieint_t;	/* 16 bit integers */
typedef u_long		ieaddr_t;	/* data (24-bit) addresses */
...
struct iescp {
	u_short		iescp_unused0;	/* pad for sparc aligment */
	u_char		iescp_sysbus;	/* bus width: 0 => 16, 1 => 8 */
	u_char		iescp_unused1[IESCPPAD];
 ...
 */
struct ieiscp {
	u_char		ieiscp_busy;	/* 1 => initialization in progress */
	u_char		ieiscp_unused;	/* unused */
 ...
 */
struct iescb {
	u_short	iescb_status;		/* chip status word */
	u_short	iescb_cmd;		/* command word */
 ...
 */
struct iecb {
	u_short	iecb_status;		/* status word */
	u_short	iecb_cmd;		/* command word */
...
struct ieiaddr {
	...
	u_char	ieia_addr[IEETHERADDRL];	/* the actual address */
};
...
struct iemcaddr {
	...
	u_char	iemc_addr[IEMCADDRMAX*6];	/* pool of addresses */
};
...
struct ieconf {
	...
	u_char	ieconf_bytes;	/* # of conf bytes: only the low-order 4 bits */
	u_char	ieconf_fifolim;	/* fifo limit: only the low-order 4 bits */
	u_short	ieconf_data0;
	u_char	ieconf_data1;
	u_char	ieconf_space;	/* interframe spacing */
	u_char	ieconf_slttml8;	/* low bits of slot time */
	u_char  ieconf_data2;
	u_short ieconf_data3;
	u_char	ieconf_minfrm;	/* min frame length */
	u_char	ieconf_pad;
};
 ...
 */
struct ietcb {
	u_short	ietcb_status;		/* transmit command block status */
	u_short	ietcb_command;		/* command # */
	ieoff_t	ietcb_next;		/* next TCB */
	ieoff_t	ietcb_tbd;		/* pointer to buffer descriptor */
	u_char	ietcb_dhost[6];		/* destination address field */
	u_short	ietcb_type;		/* Ethernet packet type field */
};
 ...
 */
struct ietbd {
	u_char	 ietbd_cntlo;		/* Low order 8 bits of count */
	u_char	 ietbd_eofcnthi;
...
struct ierfd {
	...
	u_char	ierfd_pad;		/* unused */
	u_char	ierfd_command;		/* rfd command word */
 ...
 */
struct ierbd {
	u_char ierbd_cntlo;		/* Low order 8 bits of count */
	u_char ierbd_status;		/* rbd status word */
	ieoff_t ierbd_next;		/* next RBD */
	ieaddr_t ierbd_buf;		/* pointer to buffer */
	u_char	ierbd_sizelo;		/* Low order 8 bits of buffer size */
	u_char	ierbd_elsize;		/* high order bits and el */
	u_short	pad;			/* For 32-bit alignment */
};
