...
#pragma ident	"@(#)stcreg.h	1.14	93/11/22 SMI"
...
struct stcregs_t {
	...
	FILLER(0, 1);
	volatile u_char	ccr;	/* CCR - Channel Command Register */
	volatile u_char	ier;	/* IER - Interrupt Enable Register */
	volatile u_char	cor1;	/* COR1 - Channel Option Register 1 */
	volatile u_char	cor2;	/* COR2 - Channel Option Register 2 */
	volatile u_char	cor3;	/* COR3 - Channel Option Register 3 */
	volatile u_char	ccsr;	/* CCSR - Channel Control Status Register */
	volatile u_char	rdcr;	/* RDCR - Receive Data Count Register */
	FILLER(1, 1);
	volatile u_char	schr1;	/* SCHR1 - Special Character Register 1 */
	volatile u_char	schr2;	/* SCHR2 - Special Character Register 2 */
	volatile u_char	schr3;	/* SCHR3 - Special Character Register 3 */
	volatile u_char	schr4;	/* SCHR4 - Special Character Register 4 */
	FILLER(2, 3);
	volatile u_char	mcor1;	/* MCOR1 - Modem Change Option Register 1 */
	volatile u_char	mcor2;	/* MCOR2 - Modem Change Option Register 2 */
	volatile u_char	mcr;	/* MCR - Modem Change Register */
	FILLER(3, 5);
	volatile u_char	rtpr;	/* RTPR - Receive Timeout Period Register */
	FILLER(4, 15);
	volatile u_char	msvr;	/* MSVR - Modem Signal Value Register */
	FILLER(5, 8);
	volatile u_char	rbprh;	/* RBPRH - Rcv Baud Rate Period Register High */
	volatile u_char	rbprl;	/* RBPRL - Rcv Baud Rate Period Register Low */
	FILLER(6, 6);
	volatile u_char	tbprh;	/* TBPRH - Xmit Baud Rate Period Register Hi */
	volatile u_char	tbprl;	/* TBPRL - Xmit Baud Rate Period Register Low */

	/* global registers - (V) means only available in interrupt routine */
	FILLER(7, 5);
	volatile u_char	givr;	/* GIVR - Global Interrupt Vector Register */
	volatile u_char	gicr;	/* GICR - Global Interruping Channel Register */
	FILLER(8, 31);
	volatile u_char	pilr1;	/* PILR1 - Priority Interrupt Level Reg 1 */
	volatile u_char	pilr2;	/* PILR2 - Priority Interrupt Level Reg 2 */
	volatile u_char	pilr3;	/* PILR3 - Priority Interrupt Level Reg 3 */
	volatile u_char	car;	/* CAR - Channel Access Register */
	FILLER(9, 6);
	volatile u_char	gfrcr;	/* GFRCR - Global Firmware Revision Register */
	FILLER(10, 4);
	volatile u_char	pprh;	/* PPRH - Prescaler Period Register High */
	volatile u_char	pprl;	/* PPRL - Prescaler Period Register Low */
	FILLER(11, 6);
	volatile u_char	rdr;	/* RDR - Receiver Data Register (V) */
	FILLER(12, 1);
	volatile u_char	rcsr;	/* RCSR - Receiver Character Status Reg (V) */
	volatile u_char	tdr;	/* TDR - Transmit Data Register (V) */
	FILLER(13, 3);
	volatile u_char	eoir;	/* EOIR - End of Interrupt Register (V) */
};
 ...
 */
struct stciack_t {
	volatile u_char	pilr1;	/* group 1 (modem signal change) interrupts */
	volatile u_char	pilr2;	/* group 2 (transmit data) interrupts */
	volatile u_char	pilr3;	/* group 3 (receive data/exception) intr */
};
...
struct ppcregs_t {
	/* for input */
	volatile u_char	in_pdata;	/* data I/O register */
	volatile u_char	in_pstat;	/* status register */
	volatile u_char	in_pcon;	/* control register */
	volatile u_char	in_pwierd;	/* wierd register */
	/* for output */
	volatile u_char	pdata;		/* data I/O register */
	volatile u_char	pstat;		/* status register */
	volatile u_char	pcon;		/* control register */
	volatile u_char	pwierd;		/* wierd register */
	FILLER(14, (0xffc-PPC_OFFSET-8)); /* should be last 4 bytes of page */
	volatile u_char	ipdata;	/* mirror of data register in iack space */
	volatile u_char	ipstat;	/* mirror of status register in iack space */
	volatile u_char	ipcon;	/* mirror of control register in iack space */
	volatile u_char	ipwierd; /* mirror of wierd register in iack space */
};
