...
#pragma ident	"@(#)cg14reg.h	1.10	94/02/15 SMI"
...
struct mdi_register_address {
	...
	u_char	m_ppr;		/* Packed pixel register	*/
	u_char	m_tmsr0;	/* Test mode status register 0  */
	u_char	m_tmsr1;	/* Test mode status register 1  */
	u_char	m_msr;		/* Master status register	*/
	u_char	m_fsr;		/* Fault status register	*/

	struct {
		u_char	revision:4;	/* MDI revision */
		u_char	impl:4;		/* MDI implementation */
	} m_rsr;

	u_char	m_ccr;		/* Clock control register	*/
	u_int 	m_tmr;		/* Test mode Read Back register */
	u_char	m_mod;		/* Monitor Operation data reg	*/
	u_char	m_acr;		/* Aux control reg		*/
	u_char	m_pad0[6];	/* Reserved			*/
	u_short	m_hct;		/* Horizontal Counter		*/
	u_short	m_vct;		/* Vertical Counter		*/
	u_short	m_hbs;		/* Horizontal Blank Start	*/
	u_short	m_hbc;		/* Horizontal Blank Clear	*/
	u_short	m_hss;		/* Horizontal Sync Set		*/
	u_short	m_hsc;		/* Horizontal Sync Set		*/
	u_short	m_csc;		/* Composite sync clear		*/
	u_short	m_vbs;		/* Vertical blank start		*/
	u_short	m_vbc;		/* Vertical Blank Clear	*/
	u_short	m_vss;		/* Verical Sync Set		*/
	u_short	m_vsc;		/* Verical Sync Clear		*/
	u_short m_xcs;		/* XXX Gone in VSIMM 2 */
	u_short m_xcc;		/* XXX Gone in VSIMM 2 */
	u_short	m_fsa;		/* Fault status address		*/
	u_short	m_adr;		/* Address register (autoincrements) */
	u_char	m_pad2[0xce];	/* Reserved			*/

	/* PCG registers */
	u_char	m_pcg[0x100];	/* Pixel Clock generator regs	*/

	/* VBC registers */
	struct {
		u_int	res0:8;
		u_int	framebase:12;	/* frame base row addr */
		u_int	res1:12;
	} v_vbr;
	...
	union {
		...
		struct {
			u_int	res0:21;
			u_int	vconfig:2;	/* vconfig */
			u_int	r_setup:9;	/* reload setup time */
		} v_mcr1;

		/* VSIMM 2 only */
		struct {
			u_int	res0:28;
			u_int	fbconfig:2;	/* fb config */
			u_int	trc:1;		/* test row cntr */
			u_int	refresh:1;	/* refresh enable */
		} v_mcr2;
	} v_mcr;
	...
	union {
		struct {
			u_int	res0:21;
			u_int	ref_ena:1;	/* refresh enable */
			u_int	ref_req:10;	/* refresh req interval */
		} v_vcr1;
	...
	struct {
		u_int   res0:18;
		u_int	hires:1;	/* 1 = 8MB */
		u_int	ramspeed:1;	/* 1 = 60ns */
		u_int	version:2;	/* version */
		u_int	cad:10;		/* current row address */
	} v_vca;

	u_char	m_pad3[0xf0];	/* Reserved 			*/
...
struct mdi_cursor_address {
	...
	u_long curs_cpl0[MDI_CURS_SIZE]; /* Enable plane 0 space */
	u_long curs_cpl1[MDI_CURS_SIZE]; /* Color selection plane */
	u_char curs_ccr;		 /* Cursor control register */
	u_char curs_pad0[3];		 /* Reserved */
	u_short curs_xcu;		 /* X-cursor location	*/
	u_short curs_ycu;		 /* Y-cursor location	*/
	u_int curs_cc1;			 /* Cursor color register 1 */
	u_int curs_cc2;			 /* Cursor color register 2 */
	u_int curs_pad1[0x1bc];		 /* Reserved		*/
	u_long curs_cpl0i[MDI_CURS_SIZE]; /* Enable plane 0 autoinc space */
	u_long curs_cpl1i[MDI_CURS_SIZE]; /* Color selection plane autoinc sp */
};
...
struct mdi_xlut_address {
	...
	u_char	x_xlut[MDI_CMAP_ENTRIES];
	u_char	x_xlutd[MDI_CMAP_ENTRIES];
	u_char	x_pad0[0x600];		/*	Reserved	*/
	u_char	x_xlut_inc[MDI_CMAP_ENTRIES];
	u_char	x_xlutd_inc[MDI_CMAP_ENTRIES];
};
...
struct mdi_clut_address {
	u_int	c_clut[MDI_CMAP_ENTRIES];
	u_int	c_clutd[MDI_CMAP_ENTRIES];
	u_int	c_clut_inc[MDI_CMAP_ENTRIES];
	u_int	c_clutd_inc[MDI_CMAP_ENTRIES];
};
...
struct mdi_dac_address {
	u_char	dac_addr_reg;		/* DAC address register */
	u_char	dac_pad0[0xff];		/* Reserved		*/
	u_char	dac_glut;		/* the gamma table */
	u_char	dac_pad1[0xff];		/* Reserved		*/
	u_char	dac_reg_select;		/* DAC register select */
	u_char	dac_pad2[0xff];		/* Reserved		*/
	u_char	dac_mode_reg;		/* DAC mode register */
};
