...
#pragma ident	"@(#)ecppio.h	2.7	95/10/31 SMI"
...
#define	ECPPIOC_CAPABILITY	_IOR('p', 72, u_char)
#define	ECPPIOC_SETREGS		_IOW('p', 73, struct ecpp_regs)
#define	ECPPIOC_GETREGS		_IOR('p', 74, struct ecpp_regs)
#define	ECPPIOC_SETPORT		_IOW('p', 77, u_char)
#define	ECPPIOC_GETPORT		_IOR('p', 78, u_char)
#define	ECPPIOC_SETDATA		_IOW('p', 79, u_char)
#define	ECPPIOC_GETDATA		_IOR('p', 80, u_char)
 ...
 * i suppose.  maybe the ioctl simply returns a u_char
 * the u_char has meaning depending on what mode it is in.
 * compatibility mode: u_char  3 of 8 bits indicate PE, SELECT, & ERR.
 *
 * ECP mode
 * 1 bit of the uchar indicates if the back-channel buffer has any
