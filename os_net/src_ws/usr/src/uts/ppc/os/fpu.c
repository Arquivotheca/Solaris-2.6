...
fp_fork(kthread_id_t t, kthread_id_t ct)
{
	...
	register pcb_t *cpcb, *pcb;
	register struct fpu *cfp;		/* child's fpu context */
	register struct fpu *fp;		/* parent's fpu context */
 ...
 */
void
fp_free(register struct fpu *fp)
{
	kpreempt_disable();
...
/* ARGSUSED */
void
fp_save(register struct fpu *fp)
{
	kpreempt_disable();
	if (!fp || fp->fpu_valid) {
		kpreempt_enable();
...
/*
 * fp_restore(register struct fpu *fp)
 *
...
no_fpu_fault(struct regs *rp)
{
	...
	register pcb_t *pcb = &lwp->lwp_pcb;
	register struct fpu *fp = &pcb->pcb_fpu;
