...
int		mutex_insert_waiter(struct adaptive_mutex *, turnstile_id_t);
...
spin:
		...
		while ((owner_lock = lp->m_adaptive.m_owner_lock) != 0) {
			...
			if (waiters == 0) {
				...
				if (rlp->m_waiters == 0) {
					...
					if (!mutex_insert_waiter(rlp,
					    ts->ts_id)) {
						disp_lock_exit(&ts->ts_wlock);
						disp_lock_exit_high(dlp);
						continue;
					}
					disp_lock_exit_high(dlp);
				} else {
			 ...
			 * re-evaluate.
			 */
			if (lp->m_adaptive.m_owner_lock == owner_lock) {
...
mutex_adaptive_exit(mutex_impl_t *lp)
{
	...
	cmn_err(CE_PANIC, "mutex_adaptive_exit: mutex not owned by thread, "
	    "lp %x owner %x waiters %x curthread %x\n",
	    (int)lp, m.m_adaptive.m_owner_lock,
...
#endif /* DEBUG */
	lp->m_spin.m_owner_lock = MUTEX_NOT_FREE;
...
mutex_spin_enter(mutex_impl_t *lp)
{
	 ...
	 * interrupts.
	 *
	 * Also note that the mutex spl fields are only chars, but this
	 * is enough for lock_set_spl on PowerPCs.  Other machines may have
...
mutex_spstat_init(mutex_impl_t *lp, char *name, kmutex_type_t type, void *arg)
{
	...
	sp = mutex_stats_alloc(KM_SLEEP);	/* allocates zeroed struct */
	MUTEX_STATS_SET(lp, sp);	/* set stats pointer and owner_lock */
	(void) sprintf_len(LOCK_NAME_LEN, sp->name, name,
	    (u_long)lp);	/* handle %x */
	sp->lkinfo.lk_name = sp->name;
	sp->lkstat = lkstat_alloc(&sp->lkinfo, KM_SLEEP);
	...
	if (kmem_ready) {
		if (mutex_stats_cache == NULL) {
			...
			mutex_stats_cache =
				...
				sizeof (struct mutex_stats), MUTEX_PTR_ALIGN,
