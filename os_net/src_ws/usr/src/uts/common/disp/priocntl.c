...
#pragma ident	"@(#)priocntl.c	1.40	95/12/01 SMI"	/* from SVr4.0 1.15 */
...
rval_t				*rvp;
{
	...
	case PC_GETPRIRANGE:
		if (copyin(uap->arg, (caddr_t)&pcpri, sizeof (pcpri_t)))
			return (EFAULT);
		...
		if (!error) {
			if (copyout((caddr_t)&pcpri, uap->arg,
						sizeof (pcpri))) {
				return (EFAULT);
			}
		}
