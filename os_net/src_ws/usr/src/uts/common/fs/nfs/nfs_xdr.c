...
#ident	"@(#)nfs_xdr.c	1.58	96/09/10 SMI"	/* SVr4.0 1.6	*/
	...
	if (xdrs->x_op == XDR_ENCODE) {
		...
		if (wa->wa_mblk != NULL && xdrs->x_ops == &xdrmblk_ops) {
			...
			if (mp != NULL) {
				...
				if (xdrmblk_putmblk(xdrs, mp, wa->wa_count)
				    == TRUE) {
					return (TRUE);
				} else
					freeb(mp);
		...
		return (xdr_bytes(xdrs, &wa->wa_data, (u_int *)&wa->wa_count,
			NFS_MAXDATA));
	}
	...
	struct nfsrddirres *rd;
{
	...
	for (size = rd->rd_size, dp = rd->rd_entries;
		...
		if (dp->d_ino > MAXOFF_T && nfs2_limit_inum)
			return (FALSE);
		ino = (u_long) dp->d_ino;
		off = (u_long) dp->d_off;
		name = dp->d_name;
