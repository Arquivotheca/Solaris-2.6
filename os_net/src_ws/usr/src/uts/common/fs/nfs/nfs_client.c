...
#pragma ident	"@(#)nfs_client.c	1.85	96/10/17 SMI"
...
nfs_cache_check(vnode_t *vp, timestruc_t ctime, timestruc_t mtime,
	len_t fsize, long *seqp, cred_t *cr)
{
	rnode_t *rp;
	vsecattr_t *vsp;
	int mtime_changed;
...
nattr_to_vattr(vnode_t *vp, struct nfsfattr *na, struct vattr *vap)
{
	...
	case NFSOC:
		...
		vap->va_type = VSOCK;
		vap->va_mode = (vap->va_mode & ~S_IFMT) | S_IFSOCK;
		break;
...
int
nfs_async_putapage(vnode_t *vp, page_t *pp, u_offset_t off, u_int len,
...
int
nfs_async_pageio(vnode_t *vp, page_t *pp, u_offset_t io_off, u_int io_len,
...
writerp(rnode_t *rp, caddr_t base, int tcount, struct uio *uio)
{
	...
	do {
		...
		n = uio->uio_loffset - offset; /* n = # of bytes written */
		base += n;
		tcount -= n;
		/*
...
int
nfs_putpages(vnode_t *vp, u_offset_t off, u_int len, int flags, cred_t *cr)
{
	register rnode_t *rp;
	register page_t *pp;
...
nfs_lockrelease(vnode_t *vp, int flag, offset_t offset, cred_t *cr)
{
...
#ifdef DEBUG
			nfs_perror(ret,
				"NFS lock release error on vp %x: %m.\n", vp);
#endif
		}
	 ...
	 */
	buf = (char *)kmem_alloc(MAX_SHR_OWNER_LEN, KM_SLEEP);
	while (nfs_remove_locking_id(vp, RLMPL_OWNER,
					(char *)NULL, buf, &shr.own_len)) {
		shr.owner = buf;
		shr.access = 0;
		shr.deny = 0;
		shr.sysid = 0;
		shr.pid = curproc->p_pid;
...
#ifdef DEBUG
			nfs_perror(ret,
				"NFS share release error on vp %x: %m.\n", vp);
#endif
		}
	}
	kmem_free(buf, MAX_SHR_OWNER_LEN);

	ASSERT(shr_has_remote_shares(vp, 0) == 0);
	return (rv);
}
...
nfs_add_locking_id(vnode_t *vp, pid_t pid, int type, char *id, int len)
{
	...
	new->lmpl_type = type;
	new->lmpl_pid = pid;
	new->lmpl_owner = (char *)kmem_alloc(len, KM_SLEEP);
	...
	for (cur = rp->r_lmpl; cur != (lmpl_t *)NULL; cur = cur->lmpl_next) {
		if (cur->lmpl_pid == pid &&
			...
			kmem_free((void *)new->lmpl_owner, len);
	...
	if (cur == (lmpl_t *)NULL) {
		*lmplp = new;
		...
		if (list_len > nfs_lmpl_high_water) {
			cmn_err(CE_WARN,
				"nfs_add_locking_id: long list vp=0x%x is %d",
				(int)vp, list_len);
		}
#endif DEBUG
	}
	...
	if (share_debug) {
		...
		for (cur = rp->r_lmpl; cur != (lmpl_t *)NULL;
			...
			if (cur->lmpl_type == RLMPL_PID) {
				...
				cmn_err(CE_PANIC,
					"nali: unrecognised lmpl_type %d",
					cur->lmpl_type);
			}
		}

		cmn_err(CE_CONT,
		"nali(%s): %d PIDs + %d OWNs = %d items left on r_lmpl\n",
			(type == RLMPL_PID) ? "P" : "O",
			npids,
			nowners,
			nitems);
	...
	for (cur = rp->r_lmpl; cur != (lmpl_t *)NULL; cur = cur->lmpl_next) {
		if (cur->lmpl_type == type &&
			...
			*lmplp = cur->lmpl_next;
			...
			if (rid != NULL) {
				bcopy(cur->lmpl_owner, (void *)rid,
					cur->lmpl_own_len);
				*rlen = cur->lmpl_own_len;
			}
			kmem_free((void *)cur->lmpl_owner, cur->lmpl_own_len);
