...
#pragma ident	"@(#)nfs_auth.c 1.20     96/07/29 SMI"
...
nfsauth_init(void)
{
	 ...
	 */
	mutex_init(&ch_list_lock, "ch_list_lock", MUTEX_DEFAULT, NULL);
	/*
	 * Initialize client handle cache count mutex
	 */
	mutex_init(&nfsauth_ch_cache_lock, "nfsauth_ch_cache_lock",
			MUTEX_DEFAULT, NULL);
...
nfsauth_clget(CLIENT **chp)
{
	...
	strcpy(addrbuf, utsname.nodename);
	strcat(addrbuf, ".nfsauth");
	addr.buf = addrbuf;
	addr.len = strlen(addrbuf);
...
nfsauth_access(struct exportinfo *exi, struct svc_req *req)
{
	...
	/*
	 * First check the access restrictions
	 * on the filesystem.  If there are no
	 * lists associated with this flavor
	 * then there's no need to make an
	 * expensive call to the nfsauth service
	 * or to cache anything.
	 */
	...
	for (i = 0; i < exi->exi_export.ex_seccnt; i++) {
		...
		perm = sp[i].s_flags;
		if (perm & M_ROOT)
			goto check;	/* have to check root list */
		if (perm == M_RO)
			return (NFSAUTH_RO);
		if (perm == M_RW)
			return (NFSAUTH_RW);
	...
	return ((int)NFSAUTH_DENIED); /* flavor not valid for this export */
