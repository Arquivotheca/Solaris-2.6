...
#pragma ident	"@(#)nfs_srv.c	1.27	96/10/16 SMI"
/* SVr4.0 1.21 */
...
#define	BIG(va)		 ((va.va_size > (u_longlong_t) MAXOFF_T) &&  \
			    (va.va_type == VREG || va.va_type == VDIR) || \
			    ((va.va_nodeid > MAXOFF_T) && nfs2_limit_inum))
/*
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	TRACE_1(TR_FAC_NFS, TR_VOP_LOOKUP_START,
		"vop_lookup_start:(%K)", callee());
	lerror = VOP_LOOKUP(vp, args->sla_from.da_name, &svp, NULL, 0, NULL,
			    cr);
	TRACE_0(TR_FAC_NFS, TR_VOP_LOOKUP_END,
		"vop_lookup_end:");
	if (!lerror) {
	...
	struct exportinfo *exi, struct svc_req *req, cred_t *cr)
{
	...
	rd->rd_entries =
	    (struct dirent64 *) kmem_alloc(rd->rd_bufsize, KM_SLEEP);
