...
#pragma ident	"@(#)nfs_server.c	1.106	96/10/16 SMI"
/* SVr4.0 1.21 */
...
nfs_svc(struct nfs_svc_args *uap)
{
	...
	error = copyinstr((caddr_t)uap->netid, buf, KNC_STRSIZE, &len);
	if (error) {
		RELEASEF(uap->fd);
		return (error);
	}

	addrmask = uap->addrmask;
	addrmask.buf = mem_alloc(uap->addrmask.maxlen);
	error = copyin((caddr_t) uap->addrmask.buf,
			(caddr_t) addrmask.buf, uap->addrmask.len);
	if (error) {
		RELEASEF(uap->fd);
		kmem_free(addrmask.buf, uap->addrmask.maxlen);
		return (error);
	}

	/* Create a transport handle. */
	if ((error = svc_tli_kcreate(fp, readsize, buf, &addrmask,
		uap->maxthreads, &xprt)) != 0) {
		RELEASEF(uap->fd);
		kmem_free(addrmask.buf, uap->addrmask.maxlen);
		return (error);
	}
...
rfs_dispatch(struct svc_req *req, SVCXPRT *xprt)
{
	...
	TRACE_0(TR_FAC_NFS, TR_SVC_GETARGS_START,
		"svc_getargs_start:");
#ifdef DEBUG
	if (rfs_no_fast_xdrargs || disp->dis_fastxdrargs == NULL_xdrproc_t ||
	    ! SVC_GETARGS(xprt, disp->dis_fastxdrargs, (char *)&args)) {
#else
	if (disp->dis_fastxdrargs == NULL_xdrproc_t ||
	    ! SVC_GETARGS(xprt, disp->dis_fastxdrargs, (char *)&args)) {
#endif
	...
	if (disp->dis_getfh != NULL) {
		...
		TRACE_0(TR_FAC_NFS, TR_FINDEXPORT_END,
			"findexport_end:");
		if (exi != NULL) {
			authres = checkauth(exi, req, cr, anon_ok);
			/*
...
checkauth(struct exportinfo *exi, struct svc_req *req, cred_t *cr, int anon_ok)
{
	...
	/*
	 * Allow anyone to have access to place-holder
	 * (not exported) root so that a MCL can be
	 * done via the public filehandle in the
	 * default location. Since exi does not
	 * reference a real export, any attempt to
	 * do a fhtovp() will fail except in rfs_lookup()
	 * which detects this situation and does the
	 * lookup relative to the rootdir.
	 */
	...
	if (access & NFSAUTH_DENIED) {
		if (anon_ok != 0)
			...
			cmn_err(CE_NOTE, "invalid access");
			return (0);
		}
	}
	...
	default:
		...
		if (!secp) {
			cmn_err(CE_NOTE, "no secinfo data.");
			return (0);
		}

		if (!checkwin(rpcflavor, secp->s_window, req)) {
			cmn_err(CE_NOTE, "invalid auth window value.");
			return (0);
		}
		...
		struct exportinfo **exi)
{
	 ...
	 */
	if (error = nfs_check_vpexi(mc_dvp, *vpp, cr, exi)) {
		VN_RELE(*vpp);
		goto publicfh_done;
	}
	...
	if (((*exi)->exi_export.ex_flags & EX_INDEX) &&
		...
		if (error == ENOENT) {
			*vpp = tvp;
			 ...
			 * for odd case where a directory is provided asindex
	...
	int pathflag)			/* flag to identify path, e.g. URL */
{
	...
	if (error == 0) {
		...
		VN_HOLD(startdvp);
		error = lookuppnvp(&pn, NO_FOLLOW, dirvpp, compvpp,
			rootdir, startdvp, cr);
	}
