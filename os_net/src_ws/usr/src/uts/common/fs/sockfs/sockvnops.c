...
#pragma ident	"@(#)sockvnops.c	1.19	96/10/09 SMI"
...
sock_open(struct vnode **vpp, int flag, struct cred *cr)
{
	...
	TRACE_4(TR_FAC_SOCKFS, TR_SOCKFS_OPEN,
		"sockfs open:maj %d vp %x so %x error %d", maj,
		*vpp, so, error);
	return (error);
}
...
sock_close(
	...
	struct cred	*cr)
{
	register struct sonode *so;
	dev_t dev;
	int error = 0;

	so = VTOSO(vp);

	dprintso(so, 1, ("sock_close(0x%x, %x, %d) %s\n",
	...
	struct cred		*cr)
{
	...
	dprintso(so, 1, ("sock_read(0x%x) %s\n",
	...
	struct cred		*cr)
{
	...
	dprintso(so, 1, ("sock_write(0x%x) %s\n",
			so, pr_state(so->so_state, so->so_mode)));
	...
	if (!(so_mode & SM_CONNREQUIRED)) {
		 ...
		 */
		so_update_attrs(so, SMOD);
		return (sosend_dgram(so, so->so_faddr_sa, so->so_faddr_len,
				    uiop, 0));
	}
	so_update_attrs(so, SMOD);
...
static int
so_copyin(char *from, char *to, int size, int fromkernel)
{
	if (fromkernel) {
		bcopy(from, to, size);
		return (0);
...
static int
so_copyout(char *from, char *to, int size, int tokernel)
{
	if (tokernel) {
		bcopy(from, to, size);
		return (0);
    ...
    int *rvalp)
{
	...
	ASSERT(vp->v_type == VSOCK);
	dprintso(so, 0, ("sock_ioctl: cmd 0x%x, arg 0x%x, state %s\n",
	...
	if (so->so_version == SOV_STREAM) {
		...
		if (cmd == I_PUSH) {
			...
			error = so_copyin((char *)arg, mname, FMNAMESZ,
					(mode & FKIOCTL));
	...
	case FIOASYNC: {
		...
		if (so_copyin((char *)arg, (char *)&value, sizeof (int),
				(mode & FKIOCTL)))
			return (EFAULT);
	...
	case FIOSETOWN: {
		...
		if (so_copyin((char *)arg, (char *)&ss.ss_pid, sizeof (pid_t),
				(mode & FKIOCTL)))
			return (EFAULT);
	...
	case SIOCGPGRP:
	case FIOGETOWN:
		if (so_copyout((char *)&so->so_pgrp, (char *)arg,
				sizeof (pid_t), (mode & FKIOCTL)))
			return (EFAULT);
		return (0);
	...
	case SIOCATMARK: {
		...
		if (so->so_state & SS_RCVATMARK)
			retval = 1;
		else if (!(so->so_state & SS_OOBPEND)) {
			/*
			 * No SIGURG has been generated -- there is no pending
			 * or present urgent data. Thus can't possibly be
			 * at the mark.
			 */
			retval = 0;
		} else {
			/*
			 * Have the stream head wait until there is either
			 * some messages on the read queue, or STRATMARK
			 * or STRNOTATMARK gets set. The STRNOTATMARK flag
			 * is used so that the transport can send up
			 * a MSGNOTMARKNEXT M_DATA to indicate that it is not
			 * at the mark and additional data is not about
			 * to be send upstream.
			 */
			retval = strwaitmark(vp);
		}
		if (so_copyout((char *)&retval, (char *)arg, sizeof (int),
				(mode & FKIOCTL)))
			return (EFAULT);
		return (0);
	}
	case I_LIST: {
		struct str_list ustrlist, kstrlist;
		struct str_mlist *freebuf;
		int freebufsize, num_modules, space;
		int i;
		 ...
		 */
		error = so_copyin((char *)arg, (caddr_t)&ustrlist,
				sizeof (struct str_list), (mode & FKIOCTL));
		if (error)
			return (error);

		if (ustrlist.sl_nmods <= 0)
			return (EINVAL);

		if (ustrlist.sl_nmods == 1)
			return (ENOSPC);

		kstrlist.sl_nmods = ustrlist.sl_nmods - 1;
		freebufsize = kstrlist.sl_nmods * sizeof (struct str_mlist);
		kstrlist.sl_modlist = freebuf =
			kmem_alloc(freebufsize, KM_SLEEP);

		dprintso(so, 0, ("I_LIST: sl_nmods %d, freebufsize %d\n",
				ustrlist.sl_nmods, freebufsize));
		...
		for (i = 0; i < so->so_pushcnt; i++) {
			error = so_copyout((caddr_t)kstrlist.sl_modlist,
					(caddr_t)ustrlist.sl_modlist,
					FMNAMESZ, (mode & FKIOCTL));
			if (error)
				goto done;
			num_modules++;
			kstrlist.sl_modlist++;
		...
		ASSERT(space > 0);
		/* Copyout "sockmod" */
		error = so_copyout("sockmod", (caddr_t)ustrlist.sl_modlist,
				FMNAMESZ, (mode & FKIOCTL));
		if (error)
			goto done;
		num_modules++;
		...
		ASSERT(space >= 0);
		error = so_copyout((caddr_t)&num_modules, (char *)arg,
				sizeof (int), (mode & FKIOCTL));
		if (error)
			goto done;
		*rvalp = num_modules;
	done:
	...
	case I_LOOK:
		...
		if (so->so_pushcnt == 0) {
			/* return sockmod */
			error = so_copyout("sockmod", (char *)arg, FMNAMESZ,
				(mode & FKIOCTL));
		} else {
			error = strioctl(vp, cmd, arg, mode, U_TO_K, cr,
					rvalp);
	...
	struct cred	*cr)
{
	...
	vap->va_fsid = fsid;
	vap->va_nodeid = (long)so & 0xFFFF;	/* XXX -- must fix */
	vap->va_nlink = 0;
	vap->va_size = 0;
	vap->va_rdev = dev;
...
sock_setattr(
	...
	struct cred	*cr)
{
	register struct sonode *so = VTOSO(vp);
	int error;
...
{
	register struct vnode *accessvp;
	register struct sonode *so = VTOSO(vp);
	...
	struct pollhead **phpp)
{
	...
	dprintso(so, 0, ("sock_poll(0x%x): state %s err %d\n",
	...
	if (so->so_error != 0) {
		/* Return error as IN|OUT to match BSD behavior */
		*reventsp |= (POLLIN|POLLRDNORM|POLLOUT) & origevents;
		ASSERT(*reventsp != 0 || *phpp != NULL);
	}
...
)
{
	...
	dprintso(so, 1, ("sock_getmsg(0x%x) %s\n",
		so, pr_state(so->so_state, so->so_mode)));
...
)
{
	...
	dprintso(so, 1, ("sock_putmsg(0x%x) %s\n",
		so, pr_state(so->so_state, so->so_mode)));
