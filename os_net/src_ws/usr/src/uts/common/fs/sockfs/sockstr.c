...
#pragma ident	"@(#)sockstr.c	1.18	96/10/14 SMI"
...
int so_no_tinfo = 0;
#else
#define	so_no_tinfo	0
#endif /* SOCK_TEST */

static int do_tinfo(struct sonode *so);
...
so_sock2stream(struct sonode *so)
{
	...
	while ((mp = so->so_conn_ind_head) != NULL) {
		...
		dprintso(so, 0,
			("so_sock2stream(%x): moving T_CONN_IND\n",
			so));
...
so_strinit(struct sonode *so, struct sonode *tso)
{
	...
	if (so->so_addr_size == 0) {
		...
		so->so_addr_size = sizeof (struct sockaddr);
	...
	so->so_laddr_maxlen = so->so_addr_size;
	so->so_laddr_sa = kmem_alloc(so->so_addr_size, KM_SLEEP);
	so->so_faddr_maxlen = so->so_addr_size;
	...
	if (so->so_family == AF_UNIX) {
		...
		so->so_ux_laddr.sou_magic = 0;
		so->so_ux_faddr.sou_vp = NULL;
		so->so_ux_faddr.sou_magic = 0;
	}
...
do_tinfo(struct sonode *so)
{
	...
	dprintso(so, 1, ("do_tinfo(%x)\n", so));

	/* Send T_INFO_REQ */
	tir.PRIM_type = T_INFO_REQ;
	mp = soallocproto1((char *)&tir, sizeof (tir),
	...
	ASSERT(mp);
	tia = (struct T_info_ack *)mp->b_rptr;
	...
	if (so->so_family == AF_UNIX) {
		...
		if (so->so_addr_size == -1) {
			...
			so->so_addr_size = MAXPATHLEN + sizeof (short) + 1;
		}
	...
	dprintso(so, 1, (
	"do_tinfo: serv %d tsdu %d, etsdu %d, addr %d, opt %d, tidu %d\n",
		so->so_serv_type, so->so_tsdu_size, so->so_etsdu_size,
		so->so_addr_size, so->so_opt_size,
		so->so_tidu_size));

	dprintso(so, 1, ("do_info: so_state %s\n",
 ...
 */
int
sowaitprim(struct sonode *so, tpi_prim_t request_prim, tpi_prim_t ack_prim,
	    int min_size, mblk_t **mpp)
{
	mblk_t *mp;
	union T_primitives *tpr;
	int error;

	dprintso(so, 1, ("sowaitprim(%x, %d, %d, %d, %x)\n",
		so, request_prim, ack_prim, min_size, mpp));

	ASSERT(MUTEX_HELD(&so->so_lock));

	error = sowaitack(so, &mp);
	if (error)
		return (error);

	dprintso(so, 1, ("got msg %x\n", mp));
	...
	if (tpr->type == ack_prim &&
		...
		if (mp->b_wptr - mp->b_rptr >= min_size) {
 ...
 */
int
sowaitokack(struct sonode *so, tpi_prim_t request_prim)
{
	mblk_t *mp;
	int error;

	error = sowaitprim(so, request_prim, T_OK_ACK,
			    sizeof (struct T_ok_ack), &mp);
	if (error)
		return (error);
	freemsg(mp);
	return (0);
...
soqueueack(struct sonode *so, mblk_t *mp)
{
	if (mp->b_datap->db_type != M_PCPROTO) {
		cmn_err(CE_WARN,
		    "sockfs: received unexpected M_PROTO TPI ack. Prim %ld\n",
		    *(tpi_prim_t *)mp->b_rptr);
		freemsg(mp);
		return;
	}
 ...
 */
int
sowaitack(struct sonode *so, mblk_t **mpp)
{
	ASSERT(MUTEX_HELD(&so->so_lock));

	while (so->so_ack_mp == NULL) {
#ifdef SOCK_TEST
		if (sock_test_timelimit != 0) {
			/*
			 * Only wait for the time limit.
			 */
			clock_t now;

			time_to_wait(&now, sock_test_timelimit);
 ...
 */
int
soflushconnind(struct sonode *so, tpi_misc_t seqno)
{
	mblk_t *prevmp, *mp;
...
sowaitconnected(struct sonode *so, int fmode, int nosig)
{
	...
	while ((so->so_state & (SS_ISCONNECTED|SS_ISCONNECTING)) ==
		...
		dprintso(so, 1, ("waiting for SS_ISCONNECTED on %x\n", so));
		if (fmode & (FNDELAY|FNONBLOCK))
			return (EINPROGRESS);
		...
		strsigset_t *allmsgsigs, strpollset_t *pollwakeups)
{
	...
	dprintso(so, 1, ("strsock_proto(%x, %x)\n", vp, mp));
	...
	*firstmsgsigs = *wakeups = *allmsgsigs = *pollwakeups = 0;
	...
	if (mp->b_wptr - mp->b_rptr < sizeof (tpr->type)) {
		...
		cmn_err(CE_WARN,
			"sockfs: Too short TPI message received. Len = %d\n",
			mp->b_wptr - mp->b_rptr);
		freemsg(mp);
		return (NULL);
	}
	if (!ISALIGNED_tpi_prim_t(mp->b_rptr)) {
		...
		cmn_err(CE_WARN,
		    "sockfs: Unaligned TPI message received. rptr = 0x%lx\n",
		    (u_long)mp->b_rptr);
		freemsg(mp);
		return (NULL);
	}
	...
	case T_DATA_IND:
		if (mp->b_wptr - mp->b_rptr < sizeof (struct T_data_ind)) {
			cmn_err(CE_WARN,
			    "sockfs: Too short T_DATA_IND. Len = %d\n",
			    mp->b_wptr - mp->b_rptr);
			freemsg(mp);
			return (NULL);
		}
	...
	case T_UNITDATA_IND: {
		...
		if (mp->b_wptr - mp->b_rptr < sizeof (struct T_unitdata_ind)) {
			cmn_err(CE_WARN,
			    "sockfs: Too short T_UNITDATA_IND. Len = %d\n",
			    mp->b_wptr - mp->b_rptr);
			freemsg(mp);
			return (NULL);
		}
		...
		if ((so->so_mode & SM_CONNREQUIRED) ||
			...
			if (so->so_family == AF_UNIX && msgdsize(mp) == 0 &&
			    tudi->OPT_length != 0) {
				char *opt;
				int optlen = tudi->OPT_length;

				opt = (char *)sogetoff(mp, tudi->OPT_offset,
		 ...
		 * a sockaddr_in.
		 */
		addr = (char *)sogetoff(mp, tudi->SRC_offset, addrlen,
				ALIGN_TPI_size);
		if (addr == NULL) {
			freemsg(mp);
			mutex_enter(&so->so_lock);
		...
		if (so->so_family == AF_INET) {
			...
			mutex_enter(&so->so_lock);
			ASSERT(so->so_faddr_len == sizeof (struct sockaddr_in));
	...
	case T_OPTDATA_IND: {
		...
		if (mp->b_wptr - mp->b_rptr < sizeof (struct T_optdata_ind)) {
			cmn_err(CE_WARN,
			    "sockfs: Too short T_OPTDATA_IND. Len = %d\n",
			    mp->b_wptr - mp->b_rptr);
			freemsg(mp);
			return (NULL);
		}
		...
		if (so->so_family == AF_UNIX && msgdsize(mp->b_cont) == 0 &&
			 ...
			 */
			char *opt;
			int optlen = tdi->OPT_length;

			opt = (char *)sogetoff(mp, tdi->OPT_offset,
				optlen, ALIGN_TPI_opthdr_size);
			if (opt == NULL) {
				/* The len/off falls outside mp */
				freemsg(mp);
	...
	case T_EXDATA_IND: {
		...
		if (mp->b_wptr - mp->b_rptr < sizeof (struct T_exdata_ind)) {
			cmn_err(CE_WARN,
			    "sockfs: Too short T_EXDATA_IND. Len = %d\n",
			    mp->b_wptr - mp->b_rptr);
			freemsg(mp);
			return (NULL);
		}
		 ...
		 */
		dprintso(so, 1,
			("T_EXDATA_IND(0x%x): counts %d/%d state %s\n",
		...
		if (mdata == NULL) {
			dprintso(so, 1,
				("after outofline T_EXDATA_IND(0x%x): "
			...
			dprintso(so, 1,
				("after inline T_EXDATA_IND(0x%x): "
				"counts %d/%d  poll 0x%x sig 0x%x state %s\n",
				vp, so->so_oobsigcnt,
				so->so_oobcnt, *pollwakeups, *allmsgsigs,
				pr_state(so->so_state, so->so_mode)));
	...
	case T_CONN_CON: {
		...
		if (mp->b_wptr - mp->b_rptr < sizeof (struct T_conn_con)) {
			cmn_err(CE_WARN,
			    "sockfs: Too short T_CONN_CON. Len = %d\n",
			    mp->b_wptr - mp->b_rptr);
			freemsg(mp);
			return (NULL);
		}
		...
		addr = (char *)sogetoff(mp, conn_con->RES_offset, addrlen, 1);
		if (addr == NULL) {
			cmn_err(CE_WARN,
		 ...
		 */
		if (so->so_family != AF_UNIX) {
			so->so_faddr_len = addrlen;
			ASSERT(so->so_faddr_len <= so->so_faddr_maxlen);
			bcopy((char *)addr, (char *)so->so_faddr_sa, addrlen);
		}
	...
	case T_CONN_IND:
		...
		if (mp->b_wptr - mp->b_rptr < sizeof (struct T_conn_ind)) {
			cmn_err(CE_WARN,
			    "sockfs: Too short T_CONN_IND. Len = %d\n",
			    mp->b_wptr - mp->b_rptr);
			freemsg(mp);
			return (NULL);
		}
	...
	case T_ORDREL_IND:
		if (mp->b_wptr - mp->b_rptr < sizeof (struct T_ordrel_ind)) {
			cmn_err(CE_WARN,
			    "sockfs: Too short T_ORDREL_IND. Len = %d\n",
			    mp->b_wptr - mp->b_rptr);
			freemsg(mp);
			return (NULL);
		}
	...
	case T_DISCON_IND:
		if (mp->b_wptr - mp->b_rptr < sizeof (struct T_discon_ind)) {
			cmn_err(CE_WARN,
			    "sockfs: Too short T_DISCON_IND. Len = %d\n",
			    mp->b_wptr - mp->b_rptr);
			freemsg(mp);
			return (NULL);
		}
	...
	case T_UDERROR_IND: {
		...
		if (mp->b_wptr - mp->b_rptr < sizeof (struct T_uderror_ind)) {
			cmn_err(CE_WARN,
			    "sockfs: Too short T_UDERROR_IND. Len = %d\n",
			    mp->b_wptr - mp->b_rptr);
			freemsg(mp);
			return (NULL);
		}
		...
		addr = (char *)sogetoff(mp, tudi->DEST_offset, addrlen, 1);
		if (addr == NULL) {
			cmn_err(CE_WARN,
		...
		if (so->so_state & SS_ISCONNECTED) {
			...
			if (so->so_family == AF_UNIX) {
				faddr = (char *)&so->so_ux_faddr;
				faddr_len = sizeof (so->so_ux_faddr);
			} else {
				faddr = (char *)so->so_faddr_sa;
				faddr_len = so->so_faddr_len;
			}
	...
	case T_ERROR_ACK:
		...
		if (mp->b_wptr - mp->b_rptr < sizeof (struct T_error_ack)) {
			cmn_err(CE_WARN,
			    "sockfs: Too short T_ERROR_ACK. Len = %d\n",
			    mp->b_wptr - mp->b_rptr);
			freemsg(mp);
			return (NULL);
		}
	...
	case T_OK_ACK:
		if (mp->b_wptr - mp->b_rptr < sizeof (struct T_ok_ack)) {
			cmn_err(CE_WARN,
			    "sockfs: Too short T_OK_ACK. Len = %d\n",
			    mp->b_wptr - mp->b_rptr);
			freemsg(mp);
			return (NULL);
		}
	...
	case T_INFO_ACK:
		if (mp->b_wptr - mp->b_rptr < sizeof (struct T_info_ack)) {
			cmn_err(CE_WARN,
			    "sockfs: Too short T_INFO_ACK. Len = %d\n",
			    mp->b_wptr - mp->b_rptr);
			freemsg(mp);
			return (NULL);
		}
	...
	case T_BIND_ACK:
		if (mp->b_wptr - mp->b_rptr < sizeof (struct T_bind_ack)) {
			cmn_err(CE_WARN,
			    "sockfs: Too short T_BIND_ACK. Len = %d\n",
			    mp->b_wptr - mp->b_rptr);
			freemsg(mp);
			return (NULL);
		}
	...
	case T_OPTMGMT_ACK:
		if (mp->b_wptr - mp->b_rptr < sizeof (struct T_optmgmt_ack)) {
			cmn_err(CE_WARN,
			    "sockfs: Too short T_OPTMGMT_ACK. Len = %d\n",
			    mp->b_wptr - mp->b_rptr);
			freemsg(mp);
			return (NULL);
		}
		...
		strsigset_t *allmsgsigs, strpollset_t *pollwakeups)
{
	...
	dprintso(so, 1, ("strsock_misc(%x, %x, 0x%x)\n",
	...
	*wakeups = *allmsgsigs = *firstmsgsigs = *pollwakeups = 0;
	...
	case M_PCSIG:
		...
		if (*mp->b_rptr == SIGURG) {
			mutex_enter(&so->so_lock);
			dprintso(so, 1,
				("SIGURG(0x%x): counts %d/%d state %s\n",
				vp, so->so_oobsigcnt,
				so->so_oobcnt,
				pr_state(so->so_state, so->so_mode)));
			so_oob_sig(so, 1, allmsgsigs, pollwakeups);
			dprintso(so, 1,
				("after SIGURG(0x%x): counts %d/%d "
