...
#pragma ident   "@(#)specvnops.c 1.104     96/09/24 SMI"
...
static int spec_putapage(struct vnode *, page_t *, u_offset_t *, u_int *, int,
	struct cred *);
static struct buf *spec_startio(struct vnode *, page_t *, u_int, u_int,
			int);
static int spec_getapage(struct vnode *, u_offset_t, u_int, u_int *, page_t **,
    u_int, struct seg *, caddr_t, enum seg_rw, struct cred *);
...
static int spec_pageio(struct vnode *, page_t *, u_offset_t, u_int, int,
cred_t *);
...
spec_open(struct vnode **vpp, int flag, struct cred *cr)
{
	...
	case VBLK:
		 ...
		 */
		if (error == 0) {
			register u_int sn_size = csp->s_size;
			if (sn_size == 0 || sn_size == UNKNOWN_SIZE) {
				sn_size = bdev_size(dev);
				if (sn_size == (u_int)-1 || sn_size >
					btodb(MAXOFF_T))
	...
	struct cred		*cr)
{
	...
	if (vp->v_type == VCHR) {
		...
		dip = e_pm_busy(csp->s_dip, PMC_READ);
		error = cdev_read(dev, uiop, cr);
		e_pm_unbusy(dip, PMC_READ);
		return (error);
	}
	...
	do {
		...
		base = segmap_getmapflt(segkmap, blkvp,
			(u_offset_t)(off + on), (u_int)n, 1, S_READ);
	...
	struct cred *cr)
{
	...
	if (vp->v_type == VCHR) {
		...
		smark(sp, SUPD);
		ASSERT(STREAMSTAB(getmajor(dev)) == 0);
		dip = e_pm_busy(csp->s_dip, PMC_WRITE);
		error = cdev_write(dev, uiop, cr);
		e_pm_unbusy(dip, PMC_WRITE);
		return (error);
	}
	...
	do {
		...
		base = segmap_getmapflt(segkmap, blkvp,
		    (u_offset_t)(off + on), (u_int)n, !pagecreate, S_WRITE);
    ...
    int *rvalp)
{
	...
	if (STREAMSTAB(getmajor(dev))) {
		...
		dev_info_t *dip;
		dip = e_pm_busy(csp->s_dip, PMC_IOCTL);
		error = cdev_ioctl(dev, cmd, arg, mode, cr, rvalp);
		e_pm_unbusy(dip, PMC_IOCTL);
	}
	return (error);
}
...
spec_getattr(struct vnode *vp, struct vattr *vap, int flags, struct cred *cr)
{
	...
	if (realvp == NULL) {
		...
		vap->va_fsid = fsid;
		/* XXX -- must fix */
		vap->va_nodeid = (ino64_t)((long)sp & 0xFFFF);
		vap->va_nlink = 0;
	...
	struct cred	*cr)
{
	...
	TRACE_4(TR_FAC_SPECFS, TR_SPECFS_GETPAGE,
		"specfs getpage:vp %p off %llx len %d snode %p",
		vp, off, len, sp);
...
reread:
	...
	if (spec_ra && sp->s_nextr == (u_int)off)
		dora = 1;
	else
		dora = 0;
	...
	struct cred	*cr)
{
	...
	TRACE_4(TR_FAC_SPECFS, TR_SPECFS_PUTPAGE,
		"specfs putpage:vp %p off %llx len %d snode %p",
		vp, off, len, sp);
	...
	struct cred *cred)
{
	...
	if ((flags & MAP_FIXED) == 0) {
		 ...
		 */
		map_addr(addrp, len, (offset_t)off, 0);
		if (*addrp == NULL) {
			as_rangeunlock(as);
			return (ENOMEM);
		}
	...
	cred_t	*cr)
{
	...
	bp = spec_startio(vp, pp, (u_int)io_off, io_len, flags);
