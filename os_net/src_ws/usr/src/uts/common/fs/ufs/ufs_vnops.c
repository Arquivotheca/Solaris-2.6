...
#pragma	ident	"@(#)ufs_vnops.c	2.263	96/10/25 SMI"
...
static	int ufs_getpage_miss(struct vnode *, u_offset_t, u_int, struct seg *,
...
static	int ufs_pageio(struct vnode *, struct page *, u_offset_t, u_int, int,
			struct cred *);
static	int ufs_dump(vnode_t *, caddr_t, int, int);
...
wrip(struct inode *ip, struct uio *uio, int ioflag, struct cred *cr)
{
	...
	do {
		...
		if (uoff + n > ip->i_size) {
			...
			rw_store_double((uoff + n), &ip->i_size);
	...
	int		*rvalp)
{
	...
	switch (cmd) {
		...
		case _FIOISBUSY:
			...
			if (!suser(cr))
				return (EPERM);
			error = ufs_fioisbusy(vp, (u_long *)arg, cr);
			return (error);
	...
	struct cred *cr;
{
	...
	if (vap->va_mask == AT_SIZE) {
		 ...
		 */
		/*
		 * Large Files: We cannot do this any more? With Large Files
		 * i_size is a 64bit type and loads are no more atomic.
		 * We might end up having bad values in va_size.
		 * Guess.. this was done for increasing Laddis performance
		 * number.
		 * The idea behind using ifdef _LP64_ is in Solaris64
		 * the 64bit loads will be atomic.
		 */
#ifdef _LP64
		vap->va_size = ip->i_size;
#else
		rw_load_double(&vap->va_size, &ip->i_size, &ip->i_contents);
#endif
	...
	struct cred *cr;
{
	 ...
	 */
	if (ufs_idle_q.uq_ne > ufs_idle_q.uq_maxne)
	...
	struct cred *cr;		/* user credentials */
{
	...
	if (error == 0) {
		...
		error = ufs_rdwri(UIO_WRITE, FWRITE, ip, target,
		    (int)strlen(target), (offset_t)0, UIO_SYSSPACE,
			&leftover, cr);
		if (error) {
			rw_exit(&ip->i_contents);
			rw_exit(&ip->i_rwlock);
...
retrylock:
	 ...
	 * is held in bmap routines which sets the i_owner field.
	 */
...
ufs_dump(vnode_t *vp, caddr_t addr, int ldbn, int dblks)
{
	...
	rw_load_double(&file_size, &ip->i_size, &ip->i_contents);
