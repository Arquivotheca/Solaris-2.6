...
#pragma ident	"@(#)ufs_panic.c	1.15	96/10/17 SMI"
...
sf_found_lock_fix_cmn(ufs_failure_t *f, ufs_failure_states_t s)
{
	...
	if (s & UF_LOCKED) {
		...
		HIDEOUS((": time:%d, too long:%ld, entered_tm:%d ",
...
dump_uf(ufs_failure_t *f, int i)
{
	...
	if (f->uf_orig)
		printf("\tOriginal failure: 0x%p \"%s\"\n",
		    f->uf_orig, f->uf_orig->uf_panic_str);

	printf("\tUfsvfs: 0x%p\t\tVfs_lockp: 0x%p\n",
		    f->uf_ufsvfsp, f->uf_vfs_lockp);
	printf("\tVfs_fsfxp: 0x%p\n", f->uf_vfs_ufsfxp);
	printf("\tVfs_bufp: 0x%p", f->uf_bp);
	...
	printf("\tBegin: 0x%x\tEntered: 0x%x\tEnd: 0x%x\n",
