/*
 * Copyright 1992,1996 by Sun Microsystems, Inc.
 *	All rights reserved.
 */
#ident	"@(#)ufs_acl.c	1.52	96/09/04 SMI"
#include <sys/types.h>
#include <sys/stat.h>
#include <sys/errno.h>
#include <sys/kmem.h>
...
vsecattr2aclentry(vsecattr_t *vsap, si_t **spp)
{
	...
	rw_init(&sp->s_lock, "inode acl lock", RW_DEFAULT, NULL);
 ...
 */
{
	...
	rw_init(&tsp->s_lock, "inode acl lock", RW_DEFAULT, NULL);
...
ufs_acl_cp(si_t *sp)
{
	...
	rw_init(&dsp->s_lock, "inode acl lock", RW_DEFAULT, NULL);
