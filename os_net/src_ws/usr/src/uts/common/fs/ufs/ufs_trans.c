...
#ident		"@(#)ufs_trans.c 1.48     96/09/06 SMI"
	...
	int level)
{
	struct ufsvfs	*ufsvfsp	= ip->i_ufsvfs;
	struct fs	*fs		= ufsvfsp->vfs_fs;
	int		ne		= fs->fs_bsize / (int)sizeof (daddr_t);
	int		i;
	struct buf	*bp;
	daddr_t		*blkp;
...
ufs_trans_quota(struct dquot *dqp)
{
	...
	if ((dqp->dq_flags & DQ_MOD) == 0) {
		cmn_err(CE_WARN, "ufs_trans_quota: dqp %p not modified\n", dqp);
		return;
	}
	ASSERT((dqp->dq_mof != 0) && (dqp->dq_mof != UFS_HOLE));

	TRANS_DELTA(qip->i_ufsvfs, (offset_t) dqp->dq_mof,
	    sizeof (struct dqblk), DT_QR, ufs_trans_push_quota, (u_long)dqp);
}
 ...
 * Issue write request.
 *
 * If the request is too large to fit in the log, split it into smaller chunks.
 */
int
ufs_trans_write(
	struct inode *ip,
	...
	long resid)
{
	...
	/*
	 * There is enough log space; request will not have to be split up
	 */
	if (!resid)
