...
#ident	"@(#)ufs_bmap.c	2.85	96/10/17 SMI"
...
#define	BUGID_1252003_NOTFIXED

#ifdef BUGID_1252003_NOTFIXED
int Bugid_1252003_Notfixed = 0;
#endif
...
bmap_read(struct inode *ip, u_offset_t off, daddr_t *bnp, int *lenp)
{
...
#ifdef BUGID_1252003_NOTFIXED
	if (Bugid_1252003_Notfixed)
		cmn_err(CE_CONT,
			" #0 this damn sysinit is not getting loaded \n");
#endif
	lbn = (daddr_t)lblkno(fs, off);
#ifdef BUGID_1252003_NOTFIXED
	if (Bugid_1252003_Notfixed)
		cmn_err(CE_CONT,
			" #1 this damn sysinit is not getting loaded \n");
#endif
	...
	if (Bugid_1252003_Notfixed)
		cmn_err(CE_CONT,
			" #2 this damn sysinit is not getting loaded \n");
#endif
	/*
	...
	if (Bugid_1252003_Notfixed)
		cmn_err(CE_CONT,
			" #3 this damn sysinit is not getting loaded \n");
#endif
	/*
	...
	struct cred	*cr)
{
	...
	kthread_id_t	ownerchk = (ip->i_owner);
	...
	if (lbn < 0)
		return (EFBIG);
	llbn = (daddr_t)lblkno(fs, ip->i_size - 1);
	...
	if (llbn < NDADDR && llbn < lbn && (ob = ip->i_db[llbn]) != 0) {
		...
		if (osize < bsize && osize > 0) {
			...
			ip->i_db[llbn] = nb;
			rw_store_double(
			    ((u_offset_t)(llbn + 1)) << fs->fs_bshift,
			    &ip->i_size);
 ...
 */
static int
findextent(struct fs *fs, daddr_t *sbp, int n, int *lenp)
{
