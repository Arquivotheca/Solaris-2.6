...
#pragma	ident	"@(#)ufs_inode.c	2.153	96/09/04 SMI"
/* From SunOS 2.56 */
...
kmutex_t	ino_new_mutex;		/* protects ino_new count */
...
ufsinit(struct vfssw *vswp, int fstype)
{
	...
	mutex_init(&ino_new_mutex, "ino_new lock", MUTEX_DEFAULT, DEFAULT_WT);
...
ufs_inode_cache_constructor(void *buf, void *cdrarg, int kmflags)
{
	...
	mutex_init(&vp->v_lock, "ufs v_lock", MUTEX_DEFAULT, DEFAULT_WT);

	cv_init(&ip->i_wrcv, "write_lock", CV_DRIVER, NULL);
	cv_init(&vp->v_cv, "vnode cv", CV_DEFAULT, NULL);

	ins.in_malloc.value.ul++;
	ins.in_maxreached.value.ul =
			MAX(ins.in_maxreached.value.ul, ins.in_size.value.ul);
...
again:
	...
	mutex_enter(&ino_new_mutex);
	ino_new++;
	ins.in_size.value.ul = ino_new;
	mutex_exit(&ino_new_mutex);

	ins.in_kcalloc.value.ul++;
...
ufs_itrunc(struct inode *oip, u_offset_t length, int flags, cred_t *cr)
{
	...
	if (bn != 0) {
		...
		rw_store_double(length, &ip->i_size);
