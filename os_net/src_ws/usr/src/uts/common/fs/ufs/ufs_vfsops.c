 ...
 */
#pragma	ident	"@(#)ufs_vfsops.c	2.153	96/10/22 SMI"
	...
	enum whymountroot why;
{
	...
	if (why == ROOT_INIT || why == ROOT_BACKMOUNT ||
		 ...
		 */
		ASSERT(!(TRANS_ISTRANS(((struct ufsvfs *)vfsp->vfs_data))));
		/*
		 * if we can acquire the vfs lock; then do a sync sync
		 *	else do an async sync
		 */
		if (vfs_lock(vfsp) == 0) {
			ufs_flush(vfsp);
			vfs_unlock(vfsp);
	...
	error = vfs_lock(vfsp);
	if (error) {
		if (why == ROOT_INIT || why == ROOT_FRONTMOUNT ||
		    why == ROOT_BACKMOUNT)
			module_keepcnt--;
		return (error);
	}
...
#define	BOOT_TIME_LIMIT	(180*hz)
	...
	if (fspt->fs_state + (long)fspt->fs_time == FSOKAY &&
		...
		kmem_free((caddr_t)fsp->fs_u.fs_csp, (u_int)fsp->fs_cssize);
...
int args_len;
{
	...
	mutex_init(&ufsvfsp->vfs_rename_lock, "ufs rename lock",
	    MUTEX_DEFAULT, NULL);
...
ufs_unmount(struct vfs *vfsp, struct cred *cr)
{
	...
	kmem_free(fs->fs_u.fs_csp, (size_t)fs->fs_cssize);
	...
	bflush(dev);
	/* force-invalidate, even if device errors not allowing flush */
	bfinval(dev, 1);
	VN_RELE(bvp);
	...
	if (ulp->ul_lockfs.lf_comlen && ulp->ul_lockfs.lf_comment)
		kmem_free(ulp->ul_lockfs.lf_comment,
			(size_t)ulp->ul_lockfs.lf_comlen);
	/*
...
sbupdate(struct vfs *vfsp)
{
	...
	for (i = 0; i < blks; i += fs->fs_frag) {
		...
		bp = getblk(vfsp->vfs_dev,
			...
			fs->fs_bsize);
		bcopy(space, bp->b_un.b_addr, (size_t)size);
		space += size;
		bp->b_bcount = size;
		bwrite(bp);
	}
