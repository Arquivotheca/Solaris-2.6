...
#pragma ident   "@(#)auto_vnops.c 1.42     96/07/04 SMI"
...
)
{
	...
	AUTOFS_DPRINT((4, "auto_open: *vpp=%x\n", *vpp));
...
done:	AUTOFS_DPRINT((5, "auto_open: *vpp=%x error=%d\n", *vpp, error));
	return (error);
}
...
)
{
	...
	AUTOFS_DPRINT((4, "auto_getattr vp %x\n", vp));
...
)
{
	...
	AUTOFS_DPRINT((4, "auto_setattr vp %x\n", vp));
...
done:	AUTOFS_DPRINT((5, "auto_setattr: error=%d\n", error));
	return (error);
}
...
)
{
	...
	AUTOFS_DPRINT((4, "auto_access: vp=%x\n", vp));
...
done:	AUTOFS_DPRINT((5, "auto_access: error=%d\n", error));
	return (error);
}
...
)
{
	...
	AUTOFS_DPRINT((3, "auto_lookup: dvp=%x (%s) name=%s\n",
		dvp, dfnip->fi_map, nm));
...
top:
	...
	AUTOFS_DPRINT((3, "auto_lookup: dvp=%x dfnp=%x\n", dvp, dfnp));
	...
	case AUTOFS_MOUNT:
		...
		break;
	default:
		auto_log(CE_WARN, "auto_lookup: unknown operation\n");
	}

	AUTOFS_DPRINT((5, "auto_lookup: name=%s *vpp=%x return=%d\n",
		nm, *vpp, error));
...
)
{
	...
	AUTOFS_DPRINT((4, "auto_create dvp %x nm %s\n", dvp, nm));
...
done:	AUTOFS_DPRINT((5, "auto_create: error=%d\n", error));
	return (error);
}
...
)
{
	...
	AUTOFS_DPRINT((4, "auto_remove dvp %x nm %s\n", dvp, nm));
...
done:	AUTOFS_DPRINT((5, "auto_remove: error=%d\n", error));
	return (error);
}
...
)
{
	...
	AUTOFS_DPRINT((4, "auto_link tdvp %x svp %x nm %s\n", tdvp, svp, nm));
...
done:	AUTOFS_DPRINT((5, "auto_link error=%d\n", error));
	return (error);
}
...
)
{
	...
	AUTOFS_DPRINT((4, "auto_rename odvp %x onm %s to ndvp %x nnm %s\n",
		odvp, onm, ndvp, nnm));
...
done:	AUTOFS_DPRINT((5, "auto_rename error=%d\n", error));
	return (error);
}
...
)
{
	...
	AUTOFS_DPRINT((4, "auto_mkdir dvp %x nm %s\n", dvp, nm));
...
done:	AUTOFS_DPRINT((5, "auto_mkdir: error=%d\n", error));
	return (error);
}
...
)
{
	...
	AUTOFS_DPRINT((4, "auto_rmdir: vp=%x nm=%s\n", dvp, nm));
...
done:	AUTOFS_DPRINT((5, "auto_rmdir: error=%d\n", error));
	return (error);
}
...
)
{
	fnnode_t *fnp = vntofn(vp);
	fnnode_t *cfnp, *nfnp;
	register dirent64_t *dp, *tmp, *prev = NULL;
	register u_long offset;
	register u_long outcount = 0, count = 0;
	int namelen;
	void *outbuf;
	int error = 0;
	int reached_max = 0;
	...
	AUTOFS_DPRINT((4, "auto_readdir vp=%x offset=%ld\n",
		vp, uiop->uio_offset));
	...
	outbuf = kmem_zalloc(alloc_count, KM_SLEEP);
	dp = outbuf;
...
again:
		...
		rda.rda_count = rd.rd_rddir.rddir_size = alloc_count;
		error = auto_calldaemon(fnip, AUTOFS_READDIR,
					xdr_autofs_rddirargs, (caddr_t)&rda,
					xdr_autofs_rddirres, (caddr_t)&rd,
					cred, TRUE);
		/*
		...
		if (rd.rd_rddir.rddir_size) {
			...
			do {
				...
				if (!reached_max) {
					tmp = (dirent64_t *)((int)tmp +
					    tmp->d_reclen);
				}
			} while (!reached_max);

			if (outcount) {
				error = uiomove((caddr_t)dp, (long)outcount,
						UIO_READ, uiop);
			}
			...
			if (rd.rd_rddir.rddir_eof == 0) {
				/*
				 * alloc_count not large enough
				 * for one directory entry
				 */
				error = EINVAL;
			}
		}
		...
		if (!error && !myeof && outcount == 0) {
			/*
			 * call daemon with new cookie,
			 * all previous elements happened to
			 * be duplicates
			 */
			dp = outbuf;
			goto again;
	...
	if (!error) {
		if (reached_max) {
			...
			if (outcount == 0)
			    ...
			    (fnip->fi_flags & MF_DIRECT) || (fnp->fn_trigger) ||
			    (((vp->v_flag & VROOT) == 0) &&
...
done:	kmem_free(outbuf, alloc_count);
	AUTOFS_DPRINT((5, "auto_readdir vp=%x offset=%ld eof=%d\n",
		vp, uiop->uio_offset, myeof));
	return (error);
}
...
)
{
	...
	AUTOFS_DPRINT((4, "auto_symlink: dvp=%x lnknm=%s tnm=%s\n",
		dvp, lnknm, tnm));
...
done:	AUTOFS_DPRINT((5, "auto_symlink: error=%d\n", error));
	return (error);
}
...
)
{
	...
	AUTOFS_DPRINT((4, "auto_readlink: vp=%x\n", vp));
	...
	if (vp->v_type != VLNK)
		...
		fnp->fn_atime = hrestime;
		error = uiomove(fnp->fn_symlink,
			(int)MIN(fnp->fn_symlinklen, uiop->uio_resid),
			UIO_READ, uiop);
	}
...
)
{
	...
	AUTOFS_DPRINT((4, "auto_inactive: vp=%x v_count=%d fn_link=%d\n",
	    vp, vp->v_count, fnp->fn_linkcnt));
	...
	if (count == 0) {
		...
		if (fnp->fn_linkcnt == 1) {
			...
			auto_freefnnode(fnp);
			AUTOFS_DPRINT((5,
				"auto_inactive: (exit) vp=%x freed\n", vp));
			return;
		}
	}
	rw_exit(&dfnp->fn_rwlock);

	AUTOFS_DPRINT((5, "auto_inactive: (exit) vp=%x v_count=%d fn_link=%d\n",
	    vp, vp->v_count, fnp->fn_linkcnt));
}
...
auto_trigger_mount(vnode_t *vp, cred_t *cred, vnode_t **newvp)
{
	...
	AUTOFS_DPRINT((4, "auto_trigger_mount: vp=%x\n", vp));
...
done:	AUTOFS_DPRINT((5, "auto_trigger_mount: error=%d\n", error));
	return (error);
}
