 ...
 * All rights reserved.
 */
#pragma ident   "@(#)cachefs_cnode.c 1.130     96/07/31 SMI"
...
cachefs_cnode_idle(struct vnode *vp, cred_t *cr)
{
	...
	for (;;) {
		...
		/* free up resources */
		VN_RELE(unldvp);
		cachefs_kmem_free((caddr_t)unlname, MAXNAMELEN);
		crfree(unlcred);
		break;
	}
	...
	cachefs_mutex_enter(&fscp->fs_idlelock);
	...
	cachefs_mutex_exit(&fscp->fs_idlelock);
...
cachefs_cnode_inactive(register struct vnode *vp, cred_t *cr)
{
	...
	if ((cp->c_flags & CN_NOCACHE) && (cp->c_metadata.md_flags & MD_FILE) &&
		...
		CFS_DEBUG(CFSDEBUG_INVALIDATE)
			printf("c_cnode_inactive: invalidating %lu\n",
			    cp->c_id.cid_fileno);
#endif
...
#ifdef CFSDEBUG
		CFS_DEBUG(CFSDEBUG_INVALIDATE)
			printf("cachefs_cnode_inactive: %lu vp %x\n",
			    vp->v_count, (int) vp);
#endif
		return;
	}
	...
	cachefs_mutex_destroy(&cp->c_statelock);
	cachefs_mutex_destroy(&cp->c_iomutex);
...
cachefs_cnode_listadd(struct cnode *cp)
{
	...
	ASSERT(CACHEFS_MUTEX_HELD(&fgp->fg_cnodelock));
...
cachefs_cnode_idleadd(struct cnode *cp)
{
	...
	ASSERT(CACHEFS_MUTEX_HELD(&fscp->fs_idlelock));
    ...
    struct cnode **cpp)
{
	...
	CFS_DEBUG(CFSDEBUG_CNODE)
		printf("cachefs_cnode_find: ENTER fileno %lu fgp %x\n",
		    cidp->cid_fileno, (int)fgp);
#endif
	ASSERT(CACHEFS_MUTEX_HELD(&fgp->fg_cnodelock));
	...
	for (cp = fgp->fg_cnodelist; cp != NULL; cp = cp->c_next) {
		...
		if ((cookiep->fid_len != cp->c_cookie.fid_len) ||
			...
			CFS_DEBUG(CFSDEBUG_GENERAL) {
				printf("cachefs: dup fileno %lu, cp %x\n",
				    cidp->cid_fileno, (int)cp);
			}
#endif
			badcookie = 1;
		}
...
#ifdef CFSDEBUG
	CFS_DEBUG(CFSDEBUG_CNODE)
		printf("cachefs_cnode_find: EXIT cp %x\n", (int)*cpp);
#endif
	return (badcookie);
}
    ...
    int flag, cred_t *cr)
{
	...
	cachefs_mutex_init(&cp->c_statelock, "cnode state lock",
		MUTEX_DEFAULT, NULL);
	cachefs_mutex_init(&cp->c_iomutex, "IO Request Mutex",
		MUTEX_DEFAULT, NULL);
...
#ifdef CFSDEBUG
	CFS_DEBUG(CFSDEBUG_CNODE)
		printf("cachefs_cnode_make: ENTER fileno %lu\n",
		    cidp->cid_fileno);
#endif
...
#ifdef CFSDEBUG
	CFS_DEBUG(CFSDEBUG_CNODE)
		printf("cachefs_cnode_make: EXIT cp %x, error %d\n",
		    (int)*cpp, error);
#endif
	return (error);
}
...
cachefs_cnode_move(cnode_t *cp)
{
	...
	error = cachefs_cnode_make(&cid, fscp, NULL, NULL, NULL, kcred,
	    0, &xcp);
	if (error == 0) {
		cachefs_mutex_enter(&xcp->c_statelock);
		cachefs_cnode_stale(xcp);
		cachefs_mutex_exit(&xcp->c_statelock);
		VN_RELE(CTOV(xcp));
		xcp = NULL;
		error = 0;
	}
	...
	cachefs_mutex_enter(&ofgp->fg_cnodelock);
	cachefs_mutex_enter(&cp->c_statelock);
	if (cp->c_frontvp) {
		VN_RELE(cp->c_frontvp);
		cp->c_frontvp = NULL;
	...
	cp->c_filegrp = NULL;
	cachefs_mutex_exit(&cp->c_statelock);
	cachefs_mutex_exit(&ofgp->fg_cnodelock);
...
#ifdef CFSDEBUG
		CFS_DEBUG(CFSDEBUG_CNODE)
			printf("cachefs_cnode_lostfound cp %x cannot save\n",
			    cp);
#endif
		error = EINVAL;
		goto out;
...
#define		MAXTRIES 1000
		...
		for (index = 0; index < MAXTRIES; index++) {
			sprintf(namebuf, "%s.%lx", strp, hrestime.tv_nsec *
			    cp->c_id.cid_fileno * index);
...
cnode_enable_caching(struct cnode *cp)
{
	...
	if (error == 0) {
		...
		if (bcmp((caddr_t)&md.md_cookie.fid_data,
			...
			CFS_DEBUG(CFSDEBUG_GENERAL) {
				printf(
				    "fileno %d ignores cached data due "
				    "to cookie and/or mtime mismatch\n",
				    (int)cp->c_id.cid_fileno);
			}
#endif
		}
	...
	cachefs_pack_common(CTOV(cp), kcred);
}
