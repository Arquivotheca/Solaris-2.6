 ...
 * All rights reserved.
 */
#pragma ident   "@(#)cachefs_vfsops.c 1.125     96/10/17 SMI"
...
cachefs_mutex_t cachefs_kstat_key_lock;
...
cachefs_mutex_t cachefs_cachelock;			/* Cache list mutex */
cachefscache_t *cachefs_cachelist = NULL;		/* Cache struct list */

int cachefs_mount_retries = 3;
cachefs_mutex_t cachefs_minor_lock;		/* Lock for minor device map */
int cachefs_major = 0;
int cachefs_minor = 0;
...
cachefs_mount(vfs_t *vfsp, vnode_t *mvp, struct mounta *uap, cred_t *cr)
{
	...
	if (map->cfs_backfs) {
		...
		attrp = (struct vattr *)cachefs_kmem_alloc(
		    sizeof (struct vattr),
		    KM_SLEEP);
...
again:
	...
	if (cachep == NULL) {
		...
		if ((map->cfs_options.opt_flags & CFS_NOFILL) == 0)
			cachefs_cache_activate_rw(cachep);
		else
			map->cfs_options.opt_flags &= ~CFS_NOFILL;

		cachep->c_next = cachefs_cachelist;
		cachefs_cachelist = cachep;
	} else if (map->cfs_options.opt_flags & CFS_NOFILL) {
		cachefs_mutex_exit(&cachefs_cachelock);
		cmn_err(CE_WARN,
	...
	if (fscp == NULL) {
		...
		error = fscache_compare_options(fscp, &map->cfs_options);
		if (error) {
			cmn_err(CE_WARN,
		...
		fscp->fs_info.fi_mntflags = map->cfs_options.opt_flags;
		fscp->fs_info.fi_popsize = map->cfs_options.opt_popsize;
		fscp->fs_info.fi_fgsize = map->cfs_options.opt_fgsize;
	...
	if (fscp->fs_fscdirvp) {
		...
		if ((error == 0) &&
		    !(map->cfs_options.opt_flags & CFS_DISCONNECTABLE)) {
			cachefs_mutex_exit(&cachep->c_fslistlock);
			cachefs_mutex_exit(&cachefs_cachelock);
	...
	if (fscp->fs_mntpt == NULL) {
		...
		strl = 0;
		tmpstr[0] = '\0';
		copyinstr(uap->dir, tmpstr, MAXPATHLEN, (size_t *)&strl);
		if (strl > 1) {
			fscp->fs_mntpt = (char *)kmem_zalloc(strl, KM_SLEEP);
			strncpy(fscp->fs_mntpt, tmpstr, strl);
		}
		/*
	...
	if (fscp->fs_hostname == NULL) {
		strl = 0;
		tmpstr[0] = '\0';
		copyinstr(map->cfs_hostname, tmpstr, MAXPATHLEN,
		    (size_t *)&strl);
		if (strl > 1) {
			fscp->fs_hostname = (char *)kmem_zalloc(strl,
			    KM_SLEEP);
			strncpy(fscp->fs_hostname, tmpstr, strl);
		}
		/*
	...
	if (fscp->fs_backfsname == NULL) {
		strl = 0;
		tmpstr[0] = '\0';
		copyinstr(map->cfs_backfsname, tmpstr, MAXPATHLEN,
		    (size_t *)&strl);
		if (strl > 1) {
			fscp->fs_backfsname = (char *)kmem_zalloc(strl,
			    KM_SLEEP);
			strncpy(fscp->fs_backfsname, tmpstr, strl);
		}
		/*
	...
	cachefs_kstat_mount(fscp,
	    uap->dir, backmntpt, map->cfs_cachedir, map->cfs_cacheid);
    ...
    char *umountpoint, char *ubackfs, char *ucachedir, char *cacheid)
{
	...
	if (backfs) {
		...
		strcpy(backfs, "no back file system");
	}
	...
	cachefs_mutex_enter(&cachefs_kstat_key_lock);
	...
	cachefs_mutex_enter(&fscp->fs_fslock); /* protect fscp */

	fscp->fs_kstat_id = rc;

	cachefs_mutex_exit(&fscp->fs_fslock); /* finished with fscp */
	/* finished cachefs_kstat_key */
	cachefs_mutex_exit(&cachefs_kstat_key_lock);
	 ...
	 */
	ASSERT(! CACHEFS_MUTEX_HELD(&cachefs_kstat_key_lock));
	/* really should be EVERY cachep's c_log_mutex */
	ASSERT(! CACHEFS_MUTEX_HELD(&cachep->c_log_mutex));
	...
	if (ksp != NULL) {
		...
		ksp->ks_lock = CFSMUTEX2KMUTEX(&cachep->c_log_mutex);
...
cachefs_unmount(vfs_t *vfsp, cred_t *cr)
{
	...
	if (fscp->fs_inum_size > 0) {
		cachefs_kmem_free((caddr_t)fscp->fs_inum_trans,
...
cachefs_delete_cachep(cachefscache_t *cachep)
{
	...
	ASSERT(CACHEFS_MUTEX_HELD(&cachefs_cachelock));
...
/* XXX not quite right */
	...
	if (cachep->c_flags & CACHE_NOCACHE) {
		...
		error = fscache_enable(fscp, fsid, map->cfs_cacheid,
			&map->cfs_options, fscp->fs_info.fi_root);
		cachefs_mutex_exit(&cachep->c_fslistlock);
		if (error) {
	...
	if (map->cfs_backfs && (map->cfs_options.opt_flags & CFS_SLIDE)) {
		/* get the back file system root vp */
		error = lookupname(map->cfs_backfs, UIO_USERSPACE, FOLLOW,
			NULLVPP, &backrootvp);
		if (error)
