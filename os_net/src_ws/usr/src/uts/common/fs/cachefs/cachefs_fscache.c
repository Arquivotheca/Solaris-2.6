 ...
 * All rights reserved.
 */
#pragma ident   "@(#)cachefs_fscache.c 1.48     96/07/28 SMI"
...
fscache_create(cachefscache_t *cachep)
{
	...
	fscp = (fscache_t *)
	    kmem_cache_alloc(cachefs_fscache_cache, KM_SLEEP);

	bzero((caddr_t)fscp, sizeof (*fscp));

	cachefs_mutex_init(&fscp->fs_fslock, "fscache contents",
		MUTEX_DEFAULT, NULL);
	cachefs_mutex_init(&fscp->fs_idlelock, "idle list lock",
		MUTEX_DEFAULT, NULL);
	cachefs_mutex_init(&fscp->fs_dlock, "dlog lock", MUTEX_DEFAULT, NULL);
	cachefs_mutex_init(&fscp->fs_cdlock, "fc_cd* lock",
		MUTEX_DEFAULT, NULL);
	cv_init(&fscp->fs_cdwaitcv, "cd wait cv", CV_DEFAULT, NULL);
...
fscache_destroy(fscache_t *fscp)
{
	...
	if (fscp->fs_mntpt) {
		...
		if (strl > 0) kmem_free(fscp->fs_mntpt, strl + 1);
	}
	if (fscp->fs_hostname) {
		strl = strlen(fscp->fs_hostname);
		if (strl > 0) kmem_free(fscp->fs_hostname, strl + 1);
	}
	if (fscp->fs_backfsname) {
		strl = strlen(fscp->fs_backfsname);
		if (strl > 0) kmem_free(fscp->fs_backfsname, strl + 1);
	}
	...
	cachefs_mutex_destroy(&fscp->fs_fslock);
	cachefs_mutex_destroy(&fscp->fs_idlelock);
	cachefs_mutex_destroy(&fscp->fs_dlock);
	cachefs_mutex_destroy(&fscp->fs_cdlock);
...
fscache_list_find(cachefscache_t *cachep, ino64_t fsid)
{
	...
	ASSERT(CACHEFS_MUTEX_HELD(&cachep->c_fslistlock));
