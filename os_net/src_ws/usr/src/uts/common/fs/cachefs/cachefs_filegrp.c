 ...
 * All rights reserved.
 */
#pragma ident   "@(#)cachefs_filegrp.c 1.55     96/07/31 SMI"
...
filegrp_cache_create(void *voidp, void *cdrarg, int kmflags)
{
	...
	cachefs_mutex_init(&fgp->fg_mutex, "filegrp access lock",
		MUTEX_DEFAULT, NULL);
	cachefs_mutex_init(&fgp->fg_cnodelock, "filegrp cnode list lock",
	    MUTEX_DEFAULT, NULL);
	return (0);
}
...
filegrp_cache_destroy(void *voidp, void *cdrarg)
{
	...
	cachefs_mutex_destroy(&fgp->fg_mutex);
	cachefs_mutex_destroy(&fgp->fg_cnodelock);
}
...
filegrp_create(struct fscache *fscp, cfs_cid_t *cidp)
{
	...
	if (bugid_1249206) {
		cmn_err(CE_CONT,
			"cid_fileno for fscp %lx fgp %lx is %lld \n",
				(u_int)fscp, (u_int)fgp, fgp->fg_id.cid_fileno);
		cmn_err(CE_CONT,
			"sent fileno is %lld fgsize %d nfgsize %lld \n",
			...
			if (error) {
				...
				CFS_DEBUG(CFSDEBUG_FILEGRP)
					printf("ffhold: no dir, errno %d, "
					    "fileno %x\n",
					    error, (int) fgp->fg_id.cid_fileno);
#endif
				goto out;
			}
		}
...
filegrp_ffrele(filegrp_t *fgp)
{
	...
	if (fgp->fg_header->ach_nffs == 0) {
		...
		if (error == 0) {
			...
			cmn_err(CE_WARN,
			    "cachefs_ffrele:"
			    " frontfs cache error %d, run fsck\n", error);
		}
    ...
    struct cachefs_metadata *mdp)
{
	...
	index = cidp->cid_fileno - fgp->fg_id.cid_fileno;
	if (fgp->fg_offsets[index].ach_written == 0)
		return (ENOENT);

	error = vn_rdwr(UIO_READ, fgp->fg_attrvp,
	    (caddr_t)mdp, sizeof (struct cachefs_metadata), (offset_t)slot,
    ...
    struct cachefs_metadata *mdp)
{
	...
	error = vn_rdwr(UIO_WRITE, fgp->fg_attrvp, (caddr_t)mdp,
			sizeof (struct cachefs_metadata), (offset_t)slot,
...
filegrp_destroy_metadata(filegrp_t *fgp, cfs_cid_t *cidp)
{
	...
	i = cidp->cid_fileno - fgp->fg_id.cid_fileno;
	...
	if (fgp->fg_alloclist[i] & mask)
		...
		cmn_err(CE_WARN,
		    ...
		    cidp->cid_fileno, i, bitno, (int)fgp);
...
{
	long fgsize = fscp->fs_info.fi_fgsize;
	struct filegrp *fgp;
	ino64_t fxx;
	int findex;
	ino64_t fileno;
	...
	findex = fxx & (CFS_FS_FGP_BUCKET_SIZE -1);
...
filegrp_list_add(struct fscache *fscp, filegrp_t *fgp)
{
	...
	/* hash into array of file groups */
	findex = (fgp->fg_id.cid_fileno / fgsize) &
...
{
	ASSERT(CACHEFS_MUTEX_HELD(&fgp->fg_mutex));
...
filegrp_write_space(vnode_t *vp, int offset, int cnt)
{
	...
	while (cnt > 0) {
		...
		xx, (offset_t)offset, UIO_SYSSPACE, 0, (rlim64_t)RLIM_INFINITY,
			kcred, NULL);
		if (error)
			break;
		offset += xx;
