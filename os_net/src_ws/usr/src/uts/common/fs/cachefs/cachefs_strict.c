 ...
 * All rights reserved.
 */
#pragma ident   "@(#)cachefs_strict.c 1.51     96/05/15 SMI"
...
static u_long cachefs_gettime_cached_object(struct fscache *fscp,
    struct cnode *cp, u_long mtime);
	...
	int verify_what, cred_t *cr)
{
	...
	if ((!C_CACHE_VALID(mdp->md_vattr.va_mtime, attrs.va_mtime) ||
		...
		CFS_DEBUG(CFSDEBUG_INVALIDATE)
			printf("c_strict_check: invalidating %lu\n",
			    cp->c_id.cid_fileno);
#endif
		if (CTOV(cp)->v_pages) {
