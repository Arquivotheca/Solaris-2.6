 ...
 * All rights reserved.
 */
#pragma ident   "@(#)cachefs_vnops.c 1.201     96/10/22 SMI"
...
static int cachefs_push_connected(vnode_t *vp, struct buf *bp, u_int iolen,
    u_offset_t iooff, cred_t *cr);
static int cachefs_push_front(vnode_t *vp, struct buf *bp, u_int iolen,
    u_offset_t iooff, cred_t *cr);
...
static	int	cachefs_getpage(struct vnode *, offset_t, u_int, u_int *,
			...
			enum seg_rw, cred_t *);
static	int	cachefs_getapage(struct vnode *, u_offset_t, u_int, u_int *,
			struct page *[], u_int, struct seg *, caddr_t,
			enum seg_rw, cred_t *);
static	int	cachefs_getapage_back(struct vnode *, u_offset_t, u_int,
		u_int *, struct page *[], u_int, struct seg *, caddr_t,
			enum seg_rw, cred_t *);
static	int	cachefs_putpage(struct vnode *, offset_t, u_int, int,
			cred_t *);
static	int	cachefs_map(struct vnode *, offset_t, struct as *,
			caddr_t *, u_int, u_char, u_char, u_int, cred_t *);
static	int	cachefs_addmap(struct vnode *, offset_t, struct as *,
			caddr_t, u_int, u_char, u_char, u_int, cred_t *);
static	int	cachefs_delmap(struct vnode *, offset_t, struct as *,
			caddr_t, u_int, u_int, u_int, u_int, cred_t *);
...
cachefs_write(vnode_t *vp, uio_t *uiop, int ioflag, cred_t *cr)
{
	...
	CFS_DEBUG(CFSDEBUG_VOPS)
		printf(
		"cachefs_write: ENTER vp %x offset %llu count %d cflags %x\n",
			(int)vp, uiop->uio_loffset, uiop->uio_resid,
			cp->c_flags);
#endif
	if (vp->v_type != VREG) {
		error = EISDIR;
...
cachefs_charge_page(struct cnode *cp, u_offset_t offset)
{
	...
	error = 0;
	blockoff = offset & (u_offset_t)MAXBMASK;
...
cachefs_writepage(vnode_t *vp, caddr_t base, int tcount, uio_t *uiop)
{
	...
	CFS_DEBUG(CFSDEBUG_VOPS)
		printf(
		    "cachefs_writepage: ENTER vp %x offset %llu len %d\\\n",
		    (int)vp, uiop->uio_loffset, uiop->uio_resid);
#endif
	...
	do {
		offset = uiop->uio_loffset;
		lastpage_off = (cp->c_size - 1) & (u_offset_t)PAGEMASK;
		...
		n = uiop->uio_loffset - offset; /* n = # of bytes written */
		base += n;
		tcount -= n;
 ...
 */
static int
cachefs_push(vnode_t *vp, page_t *pp, u_offset_t *offp, u_int *lenp,
    int flags, cred_t *cr)
{
	struct cnode *cp = VTOC(vp);
	struct buf *bp;
	...
	iolen  = MIN(bp->b_bcount, cp->c_size - ldbtob(bp->b_blkno));
	...
	/* else if disconnected */
		...
		ASSERT(CFS_ISFS_NONSHARED(fscp));
 ...
 */
static int
cachefs_push_connected(vnode_t *vp, struct buf *bp, u_int iolen,
    u_offset_t iooff, cred_t *cr)
{
	struct cnode *cp = VTOC(vp);
	int error = 0;
 ...
 */
static int
cachefs_push_front(vnode_t *vp, struct buf *bp, u_int iolen,
    u_offset_t iooff, cred_t *cr)
{
	struct cnode *cp = VTOC(vp);
	fscache_t *fscp = C_TO_FSCACHE(cp);
	...
	error = bp->b_error = vn_rdwr(UIO_WRITE, frontvp,
	    bp->b_un.b_addr, (int)iolen, iooff, UIO_SYSSPACE, 0,
	...
	int *rvalp)
{
	...
	extern cachefs_mutex_t cachefs_cachelock;
	...
	switch (cmd) {
	case CACHEFSIO_PACK:
		kdata = cachefs_kmem_alloc(sizeof (cachefsio_pack_t),
		    KM_SLEEP);
		packp = (cachefsio_pack_t *)kdata;
		error = xcopyin((caddr_t)arg, kdata, sizeof (cachefsio_pack_t));
		if (!error)
			error = cachefs_pack(vp, packp->p_name, cred);
		cachefs_kmem_free(kdata, sizeof (cachefsio_pack_t));
		break;
	...
	case CACHEFSIO_PACKINFO:
		...
		if (!error)
			error = cachefs_packinfo(vp, packp->p_name,
			    &packp->p_status, cred);
		if (!error)
			error = xcopyout(kdata, (caddr_t)arg,
	...
	case CACHEFSIO_DCMD:
		...
		error = xcopyin((caddr_t)arg, (caddr_t)&dcmd, sizeof (dcmd));
		if (error)
			return (error);
		...
		dinp = NULL;
		if (dcmd.d_slen) {
			dinp = cachefs_kmem_alloc(dcmd.d_slen, KM_SLEEP);
			error = xcopyin((caddr_t)dcmd.d_sdata, dinp,
			    dcmd.d_slen);
			if (error)
				return (error);
		}
		...
		doutp = NULL;
		if (dcmd.d_rlen)
			doutp = cachefs_kmem_alloc(dcmd.d_rlen, KM_SLEEP);
		...
		/* copy out the result */
		if ((error == 0) && doutp)
			error = xcopyout((caddr_t)doutp, dcmd.d_rdata,
			    dcmd.d_rlen);

		/* free allocated memory */
		if (dinp)
			cachefs_kmem_free(dinp, dcmd.d_slen);
		if (doutp)
			cachefs_kmem_free(doutp, dcmd.d_rlen);
	...
	case _FIOCOD:
		...
		if (arg) {
			...
			cachefs_mutex_enter(&cachefs_cachelock);
			...
			cachefs_mutex_exit(&cachefs_cachelock);
		} else {
			if (CFS_ISFS_CODCONST(fscp)) {
...
cachefs_setattr_common(vnode_t *vp, vattr_t *vap, int flags, cred_t *cr)
{
	...
	if (mask & AT_SIZE) {
		...
		pvn_vplist_dirty(vp, cp->c_size, cachefs_push,
			B_TRUNC | B_INVAL, cr);
	}
...
cachefs_readlink_connected(vnode_t *vp, uio_t *uiop, cred_t *cr)
{
	...
	if (cp->c_metadata.md_flags & MD_FASTSYMLNK) {
		error = uiomove((caddr_t)cp->c_metadata.md_allocinfo,
		    (int)MIN(cp->c_size, uiop->uio_resid), UIO_READ,
		    uiop);
#ifdef CFSDEBUG
		readcache = 1;
		goto out;
	...
	error = uiomove(buf, (int)MIN(buflen, uiop->uio_resid),
		UIO_READ, uiop);
...
out:
	if (error == 0) {
		...
		cp = VTOC(*vpp);
		if (bcmp((caddr_t)&cp->c_metadata.md_parent, (caddr_t)&dircid,
		    sizeof (cfs_cid_t)) != 0) {
			cachefs_mutex_enter(&cp->c_statelock);
...
#ifdef CFSDEBUG
	CFS_DEBUG(CFSDEBUG_VOPS)
		printf("cachefs_create: ENTER dvp %x, nm %s\n",
		    (int)dvp, nm);
#endif
	for (;;) {
		/* get (or renew) access to the file system */
	...
	enum vcexcl exclusive, int mode, vnode_t **vpp, cred_t *cr)
{
	...
	cachefs_mutex_enter(&ncp->c_statelock);

	/* get the front file now instead of later */
	if (vap->va_type == VREG) {
		error = cachefs_getfrontfile(ncp);
		if (error) {
			cachefs_mutex_exit(&ncp->c_statelock);
			goto out;
		}
...
#ifdef CFSDEBUG
	CFS_DEBUG(CFSDEBUG_VOPS)
		printf("cachefs_link: ENTER fvp %x tdvp %x tnm %s \n",
			(int)fvp, (int)tdvp, tnm);
#endif
...
#ifdef CFSDEBUG
	CFS_DEBUG(CFSDEBUG_VOPS)
		printf("cachefs_link: EXIT fvp %x tdvp %x tnm %s \n",
			(int)fvp, (int)tdvp, tnm);
#endif
	return (error);
}
...
cachefs_link_connected(vnode_t *tdvp, vnode_t *fvp, char *tnm, cred_t *cr)
{
	...
	if (tdcp != fcp) {
		cachefs_mutex_enter(&fcp->c_statelock);

		if (fcp->c_backvp == NULL) {
			error = cachefs_getbackvp(fscp, fcp);
			if (error) {
				cachefs_mutex_exit(&fcp->c_statelock);
				goto out;
			}
		}
 ...
 * cnodes atomically.
 */
cachefs_mutex_t cachefs_rename_lock;
    ...
    char *nnm, cred_t *cr)
{
	...
	for (;;) {
		...
		/* must be root to remove dirs */
		if (delvp && delvp->v_type == VDIR && !suser(cr)) {
			error = EPERM;
			break;
		}
    ...
    char *nnm, cred_t *cr, vnode_t *delvp)
{
	...
	if (odcp != ndcp) {
		cachefs_mutex_enter(&ndcp->c_statelock);
		if (ndcp->c_backvp == NULL) {
			error = cachefs_getbackvp(fscp, ndcp);
			if (error) {
				cachefs_mutex_exit(&ndcp->c_statelock);
				goto out;
			}
		}
	...
	if (delvp) {
		cachefs_mutex_enter(&delcp->c_statelock);
		...
		cachefs_mutex_exit(&delcp->c_statelock);
	}
	...
	cachefs_mutex_enter(&recp->c_statelock);
	CFSOP_MODIFY_COBJECT(fscp, recp, cr);
	cachefs_mutex_exit(&recp->c_statelock);
...
#ifdef CFSDEBUG
	CFS_DEBUG(CFSDEBUG_VOPS)
		printf("cachefs_mkdir: ENTER dvp %x\n", (int)dvp);
#endif
    ...
    vnode_t **vpp, cred_t *cr)
{
	...
	*vpp = CTOV(newcp);
	...
	if (bcmp((caddr_t)&newcp->c_metadata.md_parent, (caddr_t)&dircid,
	    sizeof (cfs_cid_t)) != 0) {
		cachefs_mutex_enter(&newcp->c_statelock);
		newcp->c_metadata.md_parent = dircid;
		newcp->c_flags |= CN_UPDATED;
		cachefs_mutex_exit(&newcp->c_statelock);
	}
out:
	if (vp)
		VN_RELE(vp);
...
#ifdef CFSDEBUG
	CFS_DEBUG(CFSDEBUG_VOPS)
		printf("cachefs_symlink: ENTER dvp %x lnm %s tnm %s\n",
		    (int)dvp, lnm, tnm);
#endif
    ...
    char *tnm, cred_t *cr)
{
	...
	cachefs_mutex_enter(&newcp->c_statelock);
	error = cachefs_stuffsymlink(newcp, tnm, newcp->c_size);
	if (error) {
	...
	caddr_t addr, enum seg_rw rw, cred_t *cr)
{
	...
	CFS_DEBUG(CFSDEBUG_VOPS)
		printf("cachefs_getpage: ENTER vp %x off %lld len %lld rw %d\n",
		    (int)vp, offx, len, rw);
#endif
...
/*ARGSUSED*/
static int
cachefs_getapage(struct vnode *vp, u_offset_t off, u_int len, u_int *protp,
...
getpages:
		...
		for (ppp = ourpl; *ppp; ppp++) {
			...
			hat_pageunload((*ppp), HAT_FORCE_PGUNLOAD);
...
/*ARGSUSED*/
static int
cachefs_getapage_back(struct vnode *vp, u_offset_t off, u_int len, u_int *protp,
	struct page *pl[], u_int plsz, struct seg *seg, caddr_t addr,
	enum seg_rw rw, cred_t *cr)
{
	cnode_t *cp = VTOC(vp);
...
static int
cachefs_putpage(vnode_t *vp, offset_t off, u_int len,
    int flags, cred_t *cr)
{
	cnode_t *cp = VTOC(vp);
	int error = 0;
...
/*ARGSUSED*/
int
cachefs_putpage_common(struct vnode *vp, offset_t off, u_int len,
    int flags, cred_t *cr)
{
	struct cnode *cp  = VTOC(vp);
	struct page *pp;
...
/*ARGSUSED*/
static int
cachefs_pageio(struct vnode *vp, page_t *pp, u_offset_t io_off, u_int io_len,
	int flags, cred_t *cr)
{
	return (ENOSYS);
}
    ...
    vsecattr_t *vsec, int flag, cred_t *cr)
{
	...
	/* only superuser or owner can set acl */
	if ((cp->c_metadata.md_vattr.va_uid != cr->cr_uid) &&
	    (! suser(cr))) {
		/* XXX EINVAL?  that's what ufs does, but... */
		error = EINVAL;
		goto out;
	}
...
#ifdef CFSDEBUG
	CFS_DEBUG(CFSDEBUG_VOPS)
		printf("cachefs_shrlock: ENTER vp %x\n", (int) vp);
#endif
...
cachefs_cacheacl(cnode_t *cp, vsecattr_t *vsecp)
{
	...
	if (vsecp == NULL) {
		...
		if (error != 0)
			goto out;
		vsecp = &vsec;
		bzero((caddr_t)&vsec, sizeof (vsec));
		vsecp->vsa_mask =
...
out:
	...
	if (gotvsec) {
		...
		cachefs_kmem_free((caddr_t)vsecp->vsa_aclentp,
...
cachefs_createacl(cnode_t *dcp, cnode_t *newcp)
{
	...
	ASSERT(CACHEFS_MUTEX_HELD(&newcp->c_statelock));
	...
	if ((vsec.vsa_dfaclcnt > 0) && (vsec.vsa_dfaclentp != NULL)) {
		...
		if (newcp->c_attr.va_type == VDIR) {
			vsec.vsa_dfaclentp = (aclent_t *)
			    kmem_alloc(vsec.vsa_aclcnt * sizeof (aclent_t),
				KM_SLEEP);
...
cachefs_mutex_t cachefs_newnum_lock;
...
cachefs_newname(void)
{
	...
	cachefs_mutex_enter(&cachefs_newnum_lock);
	if (newnum == 0) {
		newnum = hrestime.tv_sec & 0xfffff;
		newnum |= 0x10000;
	}
	id = newnum++;
	cachefs_mutex_exit(&cachefs_newnum_lock);

	news = (char *)cachefs_kmem_alloc(MAXNAMELEN, KM_SLEEP);
	s = news;
	p = cachefs_prefix;
	while (*p != '\0')
		*s++ = *p++;
...
cachefs_remove_dolink(vnode_t *dvp, vnode_t *vp, char *nm, cred_t *cr)
{
	...
	if (error) {
		cachefs_kmem_free((caddr_t)tmpname, MAXNAMELEN);
		return (error);
	}
	...
	if (cp->c_unldvp) {
		VN_RELE(cp->c_unldvp);
		cachefs_kmem_free((caddr_t)cp->c_unlname, MAXNAMELEN);
		crfree(cp->c_unlcred);
	}
