#pragma ident  "@(#)tmp_dir.c 1.34    96/06/11 SMI"
...
tdirtrunc(struct tmount *tm, struct tmpnode *dir, struct cred *cred)
{
	...
	for (tdp = dir->tn_dir; tdp; tdp = dir->tn_dir) {
		...
		if (tmpfs_hash_out(tdp) == 0) {
			cmn_err(CE_PANIC, "tdirtrunc: missing node");
		}
	...
	struct cred	*cred)
{
	...
	if (tmpfs_hash_in(tdp) == 0) {
		cmn_err(CE_PANIC, "tdiraddentry: already here");
	}
	...
	struct cred	*cred)
{
	...
	*/
	...
	if (op == DE_MKDIR) {
		rw_enter(&tp->tn_rwlock, RW_WRITER);
		if ((error = tdirinit(tm, dir, tp, cred)) != 0)
	...
	struct cred	*cred)
{
	...
	tdp = tmpfs_hash_lookup(name, parent, 1, foundit);
	if (tdp) {
		ASSERT(*foundit);
		*whereitwas = tdp;
		return (0);
