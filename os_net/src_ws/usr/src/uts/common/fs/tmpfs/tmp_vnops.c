...
#pragma ident   "@(#)tmp_vnops.c 1.110     96/10/21 SMI"
...
static int	tmp_getapage(struct vnode *, u_offset_t, u_int, u_int *,
	page_t **, u_int, struct seg *, caddr_t, enum seg_rw, struct cred *);
static int 	tmp_putapage(struct vnode *, page_t *, u_offset_t *, u_int *,
	int, struct cred *);
	...
	"%s: File system full, swap space limit exceeded\n",
		...
		if (error) {
			if (tn_size_changed) {
				 ...
				 * of them.
				 */
				tmpnode_trunc(tm, tp, (u_long)old_tn_size, cr);
			}
		} else {
			/*
	...
	struct uio *uio)
{
	...
	TMP_PRINT(T_DEBUG, "rdtmp: tp %x off %lld res %ld rw %d\n",
		    tp, uio->uio_loffset, uio->uio_resid, 0, 0);
	TRACE_1(TR_FAC_TMPFS, TR_TMPFS_RWTMP_START,
...
tmp_setattr(struct vnode *vp, struct vattr *vap, int flags, struct cred *cred)
{
	...
	if (mask & AT_SIZE) {
		...
		rw_enter(&tp->tn_contents, RW_WRITER);
		error = tmpnode_trunc(tm, tp, (u_long)vap->va_size, cred);
		rw_exit(&tp->tn_contents);
	...
	while (tdp) {
		...
		if (offset >= uiop->uio_offset) {
			...
			if (tmpcheck &&
				...
				dp->d_ino =
					(ino64_t)tdp->td_tmpnode->tn_nodeid;
	...
	struct cred *cr)
{
	...
	TMP_PRINT(T_DEBUG, "tmp_getpage: vp %x, off %llx, len %x\n",
		vp, off, len, 0, 0);
...
again:
	if (pp = page_lookup(vp, (u_offset_t)off,
		...
		pp = page_create_va(vp, (u_offset_t)off, PAGESIZE,
		 ...
		 */
		err = swap_getphysname(vp, (u_int)off, &pvp, &poff);
		if (err) {
			cmn_err(CE_PANIC,
	...
	struct cred *cr)
{
	...
	TMP_PRINT(T_DEBUG,
		"tmp_putpage: vp %x, off %llx, len %x, flags %x\n",
		vp, off, len, flags, 0);
	...
	struct cred *cred)
{
	...
	if ((flags & MAP_FIXED) == 0) {
		map_addr(addrp, len, (offset_t)off, 1);
		if (*addrp == NULL) {
			as_rangeunlock(as);
			return (ENOMEM);
		}
...
static int
tmp_freesp(struct vnode *vp, struct flock64 *lp, int flag, struct cred *cred)
{
	register int i;
...
{
	TMP_PRINT(T_DEBUG, "tmp_seek: tp %x ooff %llx newoff %llx\n",
	    VTOTN(vp), ooff, *noffp, 0, 0);
