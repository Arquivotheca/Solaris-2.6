 ...
 * All rights reserved.
 */
#ident  "@(#)namevfs.c 1.41     96/07/30 SMI" /* from S5R4 1.28 */
...
nm_mount(vfs_t *vfsp, vnode_t *mvp, struct mounta *uap, cred_t *crp)
{
	...
	mutex_init(&nodep->nm_lock, "namefs node lock", MUTEX_DEFAULT, NULL);
	...
	vattrp->va_nodeid  = (ino64_t)(uintptr_t)nodep;
	vattrp->va_nlink   = 1;
	 ...
	 */
	newvp = NMTOV(nodep);
	mutex_init(&newvp->v_lock, "namefs v_lock", MUTEX_DEFAULT, NULL);
...
nameinit(struct vfssw *vswp, int fstype)
{
	...
	mutex_init(&ntable_lock, "namenode hash lock", MUTEX_DEFAULT, NULL);
	...
	namevfs.vfs_fsid.val[0] = namedev;
	namevfs.vfs_fsid.val[1] = namefstype;
