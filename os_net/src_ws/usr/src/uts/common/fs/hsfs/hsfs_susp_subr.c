...
#pragma ident	"@(#)hsfs_susp_subr.c	1.11	95/02/26 SMI"
...
hs_check_root_dirent(struct vnode *vp, struct hs_direntry *hdp)
{
	...
	if (vp->v_type != VDIR) {
		cmn_err(CE_NOTE,
			"hs_check_root_dirent: vp (0x%x) not a directory",
			(int)vp);
		return;
	}
...
get_cont_area(struct hsfs *fsp, u_char **buf_pp, cont_info_t *cont_info_p)
{
	...
	/*
	 * Sanity check to see whether the cont_len will lead to
	 * accessing the allocated buffer above more than HS_SECTOR_SIZE.
	 * (bugid # 1188316).
	 */

	if ((cont_info_p->cont_offset + cont_info_p->cont_len - 1) >=
					HS_SECTOR_SIZE) {
		cont_info_p->cont_len =
			(HS_SECTOR_SIZE - cont_info_p->cont_offset);
	}
	brelse(secbp);
	return (0);
}
