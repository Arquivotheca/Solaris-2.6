...
#pragma ident	"@(#)hsfs_rrip.c	1.10	94/01/21 SMI"
						...
						/* this value */
{
	...
	strncpy((char *)(to_string + tmp_name_len),
...
rrip_sym_link(sig_args_t *sig_args_p)
{
	...
	if (hdp->sym_link != (char *)NULL) {
		sym_link_len = strlen(strcpy(sym_link, hdp->sym_link));
	} else {
		sym_link[0] = '\0';
		sym_link_len = 0;
	}
	...
	strcpy(tmp_sym_link, sym_link);

	if (hdp->sym_link != (char *)NULL)
		kmem_free(hdp->sym_link, hdp->ext_size + 1);
					...
					/* all that good info in */
{
	 ...
	 */
	strcpy(tmp_name, from);		/* keep original */
