...
#ident	"@(#)vnode.c	1.37	96/08/16 SMI"
	...
	enum create crwhy)
{
	...
	if (filemode & FCREAT) {
		...
		if (!(filemode & FOFFMAX) && (vp->v_type == VREG)) {
			...
			if (vattr.va_size > (u_offset_t)MAXOFF_T) {
				/*
	...
	int flag)
{
	...
	*vpp = NULL;
	...
	if ((*vpp != NULL && (mode & VWRITE) && isrofile(*vpp)) ||
		 ...
		 * of regular file fails if the file size exceeds MAXOFF_T.
		 */
		if (why != CRMKDIR &&
		    !(flag & FOFFMAX) &&
		    (vp->v_type == VREG)) {
			...
			va.va_mask = AT_SIZE;
			if ((error = VOP_GETATTR(vp, &va, 0, CRED()))) {
				VN_RELE(vp);
				goto out;
			}
