...
#pragma ident	"@(#)audit_kernel.h	1.16	93/07/09 SMI"
 ...
 */
struct cwrd {
	long    cwrd_ref;	/* reference count */
	u_int   cwrd_ldbuf;	/* length of directory buffer */
	u_int   cwrd_dirlen;	/* length of directory path (+ \0) */
	caddr_t cwrd_dir;	/* directory */
	u_int   cwrd_lrbuf;	/* length of root buffer */
	u_int   cwrd_rootlen;	/* length of root path (+ \0) */
...
struct t_audit_data {
	...
	long	tad_ctrl;	/* audit control/status flags */
	long	tad_flag;	/* to audit or not to audit */
	u_int	tad_pathlen;	/* saved length from vfs_lookup */
...
struct f_audit_data {
	...
	long		fad_flags;	/* audit control flags */
	u_int		fad_lpbuf;	/* length of path buffer */
	u_int		fad_pathlen;	/* path length */
...
#define	AU_CNTL		32 * sizeof (au_buff_t)	/* and 32 control structs */
...
struct audit_queue {
	...
	int	cnt;		/* number elements on queue */
	u_long	hiwater;	/* high water mark to block */
	u_long	lowater;	/* low water mark to restart */
	u_int	bufsz;		/* audit trail write buffer size */
	u_int	buflen;		/* audit trail buffer length in use */
	u_long	delay;		/* delay before flushing queue */
...
#ifdef	_KERNEL
au_buff_t *au_get_buff(), *au_free_buff();
#endif
