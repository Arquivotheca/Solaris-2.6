...
{
	register au_buff_t *buffer = NULL;
...
mutex_init(&au_tlist_lock, "thread list lock", MUTEX_DEFAULT, DEFAULT_WT);
mutex_init(&au_flist_lock, "file list lock", MUTEX_DEFAULT, DEFAULT_WT);
mutex_init(&au_plist_lock, "process list lock", MUTEX_DEFAULT, DEFAULT_WT);
mutex_init(&au_free_queue_lock, "free queue lock", MUTEX_DEFAULT, DEFAULT_WT);
	/*
