...
#pragma ident	"@(#)tnf_trace.c	1.18	95/03/21 SMI"
...
tnf_kernel_schedule(tnf_ops_t *ops, tnf_schedule_t *sched)
{
	...
	sched->record_gen = ((tnf_block_header_t *)
	    ((unsigned)buffer & TNF_BLOCK_MASK))->generation;
...
tnf_opaque_array_1(tnf_ops_t *ops, void **opaques,
	tnf_reference_t reference, tnf_tag_data_t *tag_data)
{
	...
	while (*tmp) {
		*ref_p = (tnf_reference_t)
			tnf_opaque(ops, *tmp, (tnf_reference_t)ref_p);
		tmp++;
		ref_p++;
	}
