...
#pragma ident	"@(#)tnf_buf.h	1.17	96/03/01 SMI"
...
typedef struct {
	...
	struct {
		volatile u_long	gen;		/* generation */
		volatile u_long	block[2];	/* block number */
	} next_alloc;
	u_long			next_tag_alloc;	/* block counter */
	u_long			next_fw_alloc;	/* byte offset */
