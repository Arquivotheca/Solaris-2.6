...
#pragma ident	"@(#)clnt.h	1.44	96/10/17 SMI"
 ...
 * kRPC internal function. Not for general use. Subject to rapid change.
 */
extern void clnt_dispatch_notifyall(queue_t *, long, long);
 ...
 */
#ifdef __STDC__
extern CLIENT * clnt_create_vers_timed(const char *, const u_long, u_long *,
const u_long,	const u_long, const char *, const struct timeval *);
/*
 *	const char *host;		-- hostname
 *	const u_long prog;		-- program number
 *	u_long *vers_out;	-- servers highest available version number
 ...
 */
#ifdef __STDC__
extern  CLIENT * clnt_door_create(const u_long, const u_long, const u_int);
/*
 *	const u_long prog;			-- program number
 *	const u_long vers;			-- version number
 *	const u_int sendsz;			-- max send size
 */
#else
