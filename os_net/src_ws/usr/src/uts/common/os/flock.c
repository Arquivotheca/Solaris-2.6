...
#pragma ident   "@(#)flock.c 1.13     96/10/09 SMI"
...
flk_convert_lock_data(vnode_t *vp, flock64_t *flp,
	u_offset_t *start, u_offset_t *end, offset_t offset, offset_t max)
{
	struct vattr	vattr;
	int	error;
...
check_owner_locks(graph_t *gp, pid_t pid, long sysid, vnode_t *vp)
{
	...
	if (lock) {
		while (lock != ACTIVE_HEAD(gp) && (lock->l_vnode == vp)) {
			if (lock->l_flock.l_pid == pid &&
				lock->l_flock.l_sysid == sysid)
cmn_err(CE_PANIC, "owner pid %d 's lock %x in active queue", pid, lock);
			lock = lock->l_next;
		}
	}
	SET_LOCK_TO_FIRST_SLEEP_VP(gp, lock, vp);
	...
	if (lock) {
		while (lock != SLEEPING_HEAD(gp) && (lock->l_vnode == vp)) {
			if (lock->l_flock.l_pid == pid &&
				lock->l_flock.l_sysid == sysid)
cmn_err(CE_PANIC, "owner pid %d 's lock %x in sleep queue", pid, lock);
			lock = lock->l_next;
		}
	}
}
