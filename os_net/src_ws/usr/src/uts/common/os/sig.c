...
#ident	"@(#)sig.c	1.100	96/08/22 SMI"	/* from SVr4.0 1.52 */
	...
	if (sigismember(&coredefault, sig)) {
		...
		if (p->p_flag & SKILLED)
			...
			if (core("core", ttoproc(t), CRED(),
			    (u_int)U_CURLIMIT(&u, RLIMIT_CORE), sig) == 0)
				code = CLD_DUMPED;
#ifdef C2_AUDIT
			if (audit_active)		/* audit core dump */
...
{
	register int i;
	register sigqueue_t *sq, *next;
	register sigqhdr_t *sqh = *sqhp;

	i = (maxcount * size) + sizeof (sigqhdr_t);
	sqh = *sqhp = (sigqhdr_t *)kmem_alloc(i, KM_NOSLEEP);
	if (sqh == NULL)
		return (-1);
	...
	sqh->sqb_free = sq = (sigqueue_t *)
			((unsigned int)sqh + sizeof (sigqhdr_t));
	for (i = maxcount - 1; i > 0; i--) {
		next = (sigqueue_t *)((unsigned int)sq + size);
		sq->sq_next = next;
		sq = next;
	}
	sq->sq_next = NULL;
	mutex_init(&sqh->sqb_lock, "sigqueue structures",
					MUTEX_DEFAULT, NULL);
	return (0);
}
...
sigqalloc(sigqhdr_t **sqhp)
{
	...
	if (sqh != NULL) {
		...
		if (sqh->sqb_count > 0) {
			...
			struct_zero((caddr_t)&sq->sq_info,
						sizeof (k_siginfo_t));
			sq->sq_backptr = sqh;
		} else {
			mutex_exit(&sqh->sqb_lock);
