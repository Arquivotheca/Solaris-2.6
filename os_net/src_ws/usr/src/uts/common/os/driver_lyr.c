...
#pragma	ident	"@(#)driver_lyr.c	1.4	96/05/29 SMI"
...
ddi_lyr_create_vp(dev_t dev)
{
	...
	major = getmajor(dev);
	dip = (struct dev_info *)devnamesp[major].dn_head;

	for (minordata = dip->devi_minor; minordata != NULL;
	    minordata = minordata->next) {
		if (minordata->type != DDM_ALIAS) {
			if (minordata->ddm_dev == dev)
				if (minordata->ddm_spec_type == S_IFCHR)
					vp = makespecvp(dev, VCHR);
				else if (minordata->ddm_spec_type == S_IFBLK)
					vp = makespecvp(dev, VBLK);
				else
					return (NULL);
		} else {
			if (minordata->ddm_adev == dev)
				if (minordata->ddm_aspec_type == S_IFCHR)
					vp = makespecvp(dev, VCHR);
				else if (minordata->ddm_spec_type == S_IFBLK)
					vp = makespecvp(dev, VBLK);
				else
...
int
ddi_lyr_ioctl(ddi_lyr_handle_t lh, int cmd, int arg, int mode,
	cred_t *cr, int *rvalp)
{
