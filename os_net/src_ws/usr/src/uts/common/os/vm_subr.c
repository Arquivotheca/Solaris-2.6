...
#ident	"@(#)vm_subr.c	1.84	96/07/28 SMI"
	...
	int rw, void (*mincnt)(struct buf *), struct uio *uio)
{
	...
	TRACE_1(TR_FAC_PHYSIO, TR_PHYSIO_START, "physio_start: bp %x", bp);
	...
	TRACE_1(TR_FAC_PHYSIO, TR_PHYSIO_GETBUF_END, "getbuf_end: bp %x", bp);
	...
	while (uio->uio_iovcnt > 0) {
		...
		while (iov->iov_len > 0) {
			...
			dip = e_pm_busy_dev(bp->b_edev, PMC_PHYSIO);
			(void) (*strat)(bp);
			error = biowait(bp);
			e_pm_unbusy(dip, PMC_PHYSIO);
			if (unsafe)
				mutex_exit(&unsafe_driver);
			/*
			 * unlock the pages
	...
	TRACE_1(TR_FAC_PHYSIO, TR_PHYSIO_END, "physio_end: bp %x", bp);
 ...
 */
int
useracc(caddr_t addr, u_int count, int access)
{
	u_int prot;
...
cow_mapin(struct as *as, caddr_t uaddr, caddr_t kaddr, struct page **cached_ppp,
	struct anon **app, u_int *lenp, int cow)
{
	struct		hat *hat;
	struct seg	*seg;
	caddr_t		base;
...
tryagain:
	 ...
	 */
	res = hat_softlock(hat, uaddr, (size_t *)&size, &ppp[0],
	    cow ? HAT_COW : 0);
	size = total - size;
	*lenp += size;
	size = size >> PAGESHIFT;
