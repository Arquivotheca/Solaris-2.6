...
#pragma ident	"@(#)schedctl.c	1.6	96/07/01 SMI"
...
static kmutex_t		schedctl_lock;

extern void		installctx(kthread_id_t, int, void (*)(), void (*)(),
			    void (*)(), void (*)());
extern int		removectx(kthread_id_t, int, void (*)(), void (*)(),
			    void (*)(), void (*)());
extern caddr_t		ppmapin(page_t *, u_int, caddr_t);
...
schedctl_init()
{
	...
	mutex_init(&schedctl_lock, "schedctl lock", MUTEX_DEFAULT, NULL);
}
...
schedctl(unsigned int flags, int upcall_did, sc_shared_t **addrp)
{
	...
	/* check arguments */
	if (flags & ~(SC_STATE | SC_BLOCK | SC_PRIORITY | SC_PREEMPT))
	 ...
	 * so SIGWAITING will still work.  This is needed in case the
	 * application does something stupid and closes the door fd.
	 */
	if (flagdiff & SC_BLOCK) {
		if ((fp = GETF(upcall_did)) != NULL) {
		...
		mutex_enter(&schedctl_lock);
		p->p_sc_unblocked++;
		mutex_exit(&schedctl_lock);
		tdp->sc_door = vp;
	}
	...
	if (flagdiff & SC_STATE) {
		if (tdp->sc_flags & (SC_BLOCK | SC_PREEMPT | SC_PRIORITY))
			(void) removectx(t, NULL, schedctl_null, schedctl_null,
			    schedctl_fork, schedctl_null);
		installctx(t, (int)ssp, schedctl_save, schedctl_restore,
		    schedctl_fork, schedctl_null);
...
{
	kthread_t	*t = curthread;
	sc_data_t	*tdp = curthread->t_schedctl;
	proc_t		*p = ttoproc(curthread);

	ASSERT(tdp != NULL && (tdp->sc_flags & SC_BLOCK));

	mutex_enter(&schedctl_lock);
	if (--p->p_sc_unblocked <= 0) {
		ASSERT(p->p_sc_unblocked == 0);
		mutex_exit(&schedctl_lock);
		if (p->p_flag & SWAITSIG) {
			if (tdp->sc_door == NULL ||
			    (t->t_handoff = door_get_activation(tdp->sc_door))
			    == NULL) {
...
schedctl_cleanup()
{
	...
	curthread->t_schedctl = NULL;
	if (tdp->sc_door != NULL)
		VN_RELE(DTOV(tdp->sc_door));
	...
	if (tdp->sc_flags & SC_STATE) {
		 ...
		 */
		(void) removectx(curthread, (int)(tdp->sc_shared),
		 ...
		 */
		(void) removectx(curthread, NULL, schedctl_null,
 ...
 * Called by ts_preempt and ts_tick.
 */
int
...
schedctl_shared_init()
{
	...
	mutex_init(&spc_lock, "schedctl shared data lock",
	    MUTEX_DEFAULT, NULL);
}
...
schedctl_map(struct anon_map *amp, caddr_t *uaddrp)
{
	...
	vn_a.flags = 0;
	vn_a.amp = amp;
	error = as_map(as, addr, PAGESIZE, segvn_create, (caddr_t)&vn_a);
	as_rangeunlock(as);
