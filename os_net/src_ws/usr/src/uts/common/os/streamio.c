 ...
 * All rights reserved.
 */
#ident	"@(#)streamio.c	1.186	96/10/22 SMI"
...
one_more:
	...
	if (pollwakeups == (POLLIN | POLLRDNORM)) {
		...
		if (stp->sd_rput_opt & SR_POLLIN) {
			...
			mutex_exit(&stp->sd_lock);
			pollwakeup_safe(&stp->sd_pollist, pollwakeups);
		} else
			mutex_exit(&stp->sd_lock);
 ...
 */
static int
strput(struct stdata *stp, mblk_t *mctl, struct uio *uiop, int *iosize,
	int b_flag, int pri, int flags)
{
	...
	if (uiop != NULL && count >= 0) {
		...
		if ((stp->sd_flag & STMAPINOK) &&
		    ...
		    ((u_int)uiop->uio_iov->iov_base & PAGEOFFSET) == 0 &&
	...
	cred_t *crp, int *rvalp)
{
	...
	/*
	 * Ensure that that there is a model identifier in flag.
	 */
	if ((flag & FMODELS) == 0) {
		/* later */
	...
	default:
		...
		strioc.ic_dp = (char *)&arg;
#ifdef _VPIX
		return (strdoioctl(stp, &strioc, bp1, flag, copyflag,
		    (char *)NULL, crp, rvalp));
#else
	...
	case I_PEEK:
		...
		mp = rdq->q_first;
		/*
		 * Skip the invalid messages for MSGNOGET
		 */
		while (mp) {
			if (!(mp->b_flag & MSGNOGET))
				break;
		...
		ASSERT(mp == NULL || mp->b_datap->db_type != M_SIG);
		...
		if (strpeek.ctlbuf.maxlen < 0)
			strpeek.ctlbuf.len = -1;
		else if (strpeek.ctlbuf.maxlen == 0)
		...
		if (strpeek.databuf.maxlen < 0)
			strpeek.databuf.len = -1;
		else if (strpeek.databuf.maxlen == 0)
		 ...
		 * blocks above.
		 */
		if ((fmp == NULL) && (strpeek.databuf.maxlen > 0)) {
			infod_t infod;
		 ...
		 */
		if (strpeek.ctlbuf.len == -1 && strpeek.databuf.len == -1)
			*rvalp = 0;
		else
			*rvalp = 1;
		return (0);
...
#define	_LASTMARK	0x8000	/* Distinct from MSG*MARK */
	...
	for (;;) {
		...
		} else if ((*flagsp & MSG_HIPRI) && (stp->sd_flag & STRPRI) &&
			    (bp = getq_noenab(q)) != NULL) {
			/* Asked for HIPRI and got one */
			ASSERT(bp->b_datap->db_type >= QPCTL);
			break;
		} else if ((*flagsp & MSG_BAND) && q->q_first != NULL &&
		...
		TRACE_2(TR_FAC_STREAMS_FR, TR_STRGETMSG_WAIT,
			...
			vp, uiop);
		if (((error = strwaitq(stp, GETWAIT, (off_t)0, fmode, -1,
			&done)) != 0) || done) {
	...
	ASSERT(bp->b_datap->db_type != M_SIG);
	mutex_exit(&stp->sd_lock);
	...
	if (mdata->maxlen >= 0 && bp) {
		 ...
		 * do-while loop).
		 */
		int oldresid = uiop->uio_resid;

		bp = struiocopyout(stp, bp, uiop, &error);
		if (error) {
			/* XXX savemp is not being freed! */
			mutex_enter(&stp->sd_lock);
			stp->sd_flag &= ~STRPRI;
			more = 0;
			freemsg(bp);
...
getmout:
	 ...
	 *	Received a hipri message, q_first is not hipri.
	 *	Received a band X message, and q_first is band Y.
...
retry:
	...
	for (;;) {
		...
		} else if ((flags & MSG_HIPRI) && (stp->sd_flag & STRPRI) &&
			    (bp = getq_noenab(q)) != NULL) {
			/* Asked for HIPRI and got one */
			ASSERT(bp->b_datap->db_type >= QPCTL);
			break;
		} else if ((flags & MSG_BAND) && q->q_first != NULL &&
			    ((q->q_first->b_band >= *prip) ||
			    (stp->sd_flag & STRPRI)) &&
			    (bp = getq_noenab(q)) != NULL) {
			/*
	 ...
	 * First process PROTO or PCPROTO blocks, if any.
	 */
	if (mctlp != NULL && bp && bp->b_datap->db_type != M_DATA) {
		mblk_t *nbp;
...
)
{
	...
	for (;;) {
		...
		error = strput(stp, mp, uiop, &mdata->len, 0, pri, flag);
		if (error == 0)
			break;
		...
		if (((error = strwaitq(stp, WRITEWAIT, (off_t)0, fmode, -1,
		    &done)) != 0) || done) {
			mutex_exit(&stp->sd_lock);
			TRACE_3(TR_FAC_STREAMS_FR, TR_STRPUTMSG_OUT,
				"strputmsg out:q %X out %d error %d",
	...
	int fmode)
{
	...
	for (;;) {
		 ...
		 * strput will always free the ctl mblk - even when strput
		 * fails.
		 * Pass a dup to strput.
		 */
		while ((mp = dupmsg(mctl)) == NULL) {
			if (mctl == NULL)
				break;
