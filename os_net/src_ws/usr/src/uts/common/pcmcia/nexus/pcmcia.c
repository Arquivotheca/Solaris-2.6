...
#ident "@(#)pcmcia.c	1.72	96/10/19 SMI"
...
int pcmcia_dma_mctl(dev_info_t *, dev_info_t *, ddi_dma_handle_t,
			enum ddi_dma_ctlops, off_t *, u_int *,
			caddr_t *, u_int);
 ...
 */
int pcm_get_mem(dev_info_t *, int, uint_t *);
int pcm_get_io(dev_info_t *, int, uint_t *);
void pcm_return_io(dev_info_t *, u_int, int);
int pcm_get_intr(dev_info_t *, int);
int pcm_return_intr(dev_info_t *, int);
...
_init()
{
	...
	mutex_init(&pcmcia_global_lock, "PCMCIA nexus lock",
			MUTEX_DEFAULT, NULL);
	cv_init(&pcmcia_condvar, "PCMCIA Init Var", CV_DRIVER, NULL);
...
pcmcia_attach(dev_info_t *dip, struct pcmcia_adapter_nexus_private *adapter)
{
	...
	if (pcmcia_num_adapters == 0) {
		...
		cs_event(PCE_SS_INIT_STATE, PCE_SS_STATE_INIT, 0);
	}
...
int pcmcia_ioctl(dev_t dev, int cmd, int arg, int mode,
	...
	ddi_ctl_enum_t ctlop, void *arg, void *result)
{
	...
	case DDI_CTLOPS_INITCHILD:
		...
		if (strcmp("pcs", ddi_node_name((dev_info_t *)arg)) == 0) {
			if (ppd == NULL)
				return (DDI_FAILURE);
			sprintf(name, "%x", (int)ppd->ppd_reg[0].phys_hi);
		...
		if (DEVI(arg)->devi_nodeid == DEVI_PSEUDO_NODEID) {
			pcmcia_merge_conf((dev_info_t *)arg);
		...
		if (ppd != NULL && ppd->ppd_function > 0) {
			...
			sprintf(name, "%x,%x", sock, func);
		} else {
			sprintf(name, "%x", ppd->ppd_socket);
		}
		ddi_set_name_addr((dev_info_t *)arg, name);
...
#if defined(PCMCIA_DEBUG)
		if (pcmcia_debug) {
			cmn_err(CE_WARN, "power: %d: %x, %d, %d [%s]\n",
				pm->request_type,
				pm->req.set_power_req.who,
...
#endif
		/*
		 * this should work on x86 and PPC.
	...
	off_t offset, off_t len, caddr_t *vaddrp)
{
	...
	if (mp->map_type == DDI_MT_RNUMBER) {
		regs = pcmcia_rnum_to_regspec(rdip, mp->map_obj.rnumber);
		if (regs == NULL) {
			return (DDI_ME_RNUMBER_RANGE);
		}
		/* convert to respec request */
		mp->map_type = DDI_MT_REGSPEC;
		mp->map_obj.rp = regs;
	}
	tmp_reg = *(mp->map_obj.rp);
	regs = mp->map_obj.rp = &tmp_reg;
	base = regs->regspec_addr;
	if (base == 0 && offset != 0) {
		/*
	...
	regs->regspec_addr += (u_int) offset;
	if (len != 0) {
		if (len > regs->regspec_size) {
			return (DDI_ME_INVAL);
		}
		regs->regspec_size = len;
	}
	/*
	...
	switch (PC_GET_REG_TYPE(regs->regspec_bustype)) {
	...
	/*
	 * now setup for a resource request.  Factor in
	 * the basic encoded info and then add the constraints
	 * the adapter imposes before allocating.
	 *
	 * note that we don't care about the window association.
	 * we leave that to Socket Services.  We could do it
	 * here but that might be problematical.  Only the actual
	 * window to physical memory is important at this point.
	 */
	ppd = (struct pcmcia_parent_private *)ddi_get_parent_data(rdip);
	adapt = pcmcia_sockets[ppd->ppd_socket]->ls_adapter;
	/* does adapter own resources? */
	if (adapt->pca_flags & check) {
		ra_return_t ret;
		ddi_map_req_t mr;
		uchar_t regbuf[2 * sizeof (pci_regspec_t)];
		ra.ra_len = regs->regspec_size;
		ra.ra_addr_hi = 0;
		ra.ra_addr_lo = regs->regspec_addr;
		ra.ra_flags = 0;
		if (base != 0)
			ra.ra_flags |= RA_ALLOC_SPECIFIED;
#if 0
		if (adapt->pca_flags & PCA_RES_CONSTRAINT) {
			/* XXX need to resolve constraints XXX */
		}
#endif
		if (ra_type == &ra_io) {
			ra.ra_flags |= RA_ALIGN_SIZE | RA_ALLOC_POW2;
		}
		if (ra_alloc(ra_type, &ra, &ret) == 0)
			return (DDI_ME_NORESOURCES);
		/*
		 * we have an address to map
		 * it is now necessary to put the regspec into
		 * parent form and ask to be mapped
		 */
		mr = *mp;
		mr.map_type = DDI_MT_REGSPEC;
		mr.map_obj.rp = pcmcia_cons_regspec(dip, ra_type == &ra_io ?
			...
			dev_info_t *dip)
{
	...
	strcpy(pcmcia_adapters[i]->pca_name, ddi_get_name(dip));
	pcmcia_adapters[i]->
	...
	if (ls_if != NULL) {
		...
		for (sock = 0; sock < conf.NumSockets;
			...
			pcmcia_sockets[n]->ls_intr_pri = 11;
#if defined(PCMCIA_DEBUG)
			if (pcmcia_debug)
				cmn_err(CE_CONT,
					...
					sock, n);
#endif
			mutex_init(&pcmcia_sockets[n]->ls_ilock,
					"MFC Socket Lock",
...
pcmcia_number_socket(dev_info_t *dip, int localsocket)
{
	...
	if (child != NULL) {
		i_ddi_bind_node_to_driver(child);
...
pcm_search_devinfo(dev_info_t *dip, struct pcm_device_info *info, int socket)
{
	...
	for (dip = (dev_info_t *)DEVI(dip)->devi_child;
		...
		if ((info->pd_flags & (PCM_NAME_1275 | PCM_MULTI_FUNCTION)) ==
			...
			sprintf(bf, "%s,%x", info->pd_bind_name,
				info->pd_function);
		...
		if (info->pd_flags & PCM_NAME_GENERIC) {
			sprintf(bf, "%s,%s", PCMDEV_NAMEPREF,
			...
			client_handle_t handle)
{
	...
	if ((i = csx_GetFirstTuple(handle, &tuple)) ==
		...
		i = csx_Parse_CISTPL_MANFID(handle, &tuple,
					    &manfid);
		if (i == SUCCESS) {
			sprintf(info->pd_bind_name, "%s%x,%x",
				PCMDEV_NAMEPREF,
...
pcmcia_add_compatible(dev_info_t *dip, struct pcm_device_info *info)
{
	...
	if (info->pd_flags & PCM_NAME_VERS1) {
		sprintf(cp, "%s,%s", PCMDEV_NAMEPREF, info->pd_vers1_name);
	...
	if ((info->pd_flags & (PCM_NAME_1275 | PCM_MULTI_FUNCTION)) ==
		...
		sprintf(cp, "%s.%x", info->pd_bind_name, info->pd_function);
		length += i = strlen(cp) + 1;
		cp += i;
	}
	if (info->pd_flags & PCM_NAME_1275 &&
	    (length + strlen(info->pd_bind_name)) < MAXPATHLEN) {
		strcpy(cp, info->pd_bind_name);
	...
	if (info->pd_flags & PCM_NAME_GENERIC &&
		...
		sprintf(cp, "%s,%s", PCMDEV_NAMEPREF, info->pd_generic_name);
		length += i = strlen(cp) + 1;
		cp += i;
		/* now the simple generic name */
		if ((length + strlen(info->pd_generic_name)) < MAXPATHLEN) {
			strcpy(cp, info->pd_generic_name);
			length += i = strlen(cp) + 1;
			cp += i;
		}
	}
	...
	if (length > 0) {
		...
		if (DEVI(dip)->devi_compat_names) {
			...
			if (ddi_getproplen(DDI_DEV_T_NONE, dip, 0,
				...
				ddi_prop_create(DDI_DEV_T_NONE, dip, 0,
						"compat",
						buff, length);
			else
				ddi_prop_modify(DDI_DEV_T_NONE, dip, 0,
						"compat",
						buff, length);
#endif
		}
...
pcmcia_create_dev_info(int socket)
{
	...
	if ((i = csx_ValidateCIS(card_info.pd_handle,
	    ...
	    cisinfo.Tuples == 0) {
		/* no CIS means memory */
		strcpy(card_info.pd_generic_name, "memory");
		card_info.pd_flags |= PCM_NAME_GENERIC |
			PCM_OTHER_NOCIS | PCM_NAME_1275;
		strcpy(card_info.pd_bind_name, "pccard,memory");
		strcpy(card_info.pd_generic_name, "memory");
		...
		for (i = 0; i < functions; i++) {
			...
			if (flags & PCM_NAME_VERS1) {
			    strcpy(card_info.pd_bind_name, PCMDEV_NAMEPREF);
			    card_info.pd_bind_name[sizeof (PCMDEV_NAMEPREF)] =
					',';
			    strncpy(card_info.pd_bind_name +
				 ...
				 */
				strcpy(card_info.pd_generic_name,
					"pccard,memory");
				card_info.pd_flags |= PCM_NAME_GENERIC;
				strcpy(card_info.pd_bind_name,
					"pccard,memory");
			}
		    }
...
pcmcia_init_devinfo(dev_info_t *pdip, struct pcm_device_info *info)
{
	...
	if (dip != NULL &&
		...
		if (ppd->ppd_nreg > 0)
			ddi_prop_create(DDI_DEV_T_NONE, dip, 0, "reg",
					(caddr_t)ppd->ppd_reg,
					ppd->ppd_nreg *
					sizeof (struct pcm_regs));
		if (ppd->ppd_intr) {
			ddi_prop_create(DDI_DEV_T_NONE, dip, 0, "interrupts",
	 ...
	 * to the device tree.
	 */
	pcm_event_manager(PCE_DEV_IDENT, info->pd_socket, ddi_get_name(dip));
	/*
...
GetCookiesAndDip(sservice_t *serv)
{
	 ...
	 */
	strcpy(ai->name, socket->ls_adapter->pca_name);
...
#if use_regspec
	regs = pcmcia_find_regspec(sockp->ls_dip[func], win->base,
				    win->WindowSize, win->state);
	if (reg != -1) {
		...
		retval = ddi_regs_map_setup(sockp->ls_dip[func], reg, &xxx,
					    win->base, win->WindowSize,
					    &attr, &win->handle);
	} else {
		retval = BAD_BASE;
	}
...
{
	sprintf(path, "%s@%s:%s", ddi_node_name(dip),
...
pcmcia_create_device(ss_make_device_node_t *init)
{
	...
	if (init->flags & SS_CSINITDEV_CREATE_DEVICE) {
		...
		UNLOCK_DEV_OPS(&devnamesp[major].dn_lock);
		sprintf(device.path + strlen(device.path), ":%s", init->name);

		strcpy(device.driver, ddi_binding_name(init->dip));
#if defined(PCMCIA_DEBUG)
		if (pcmcia_debug)
			cmn_err(CE_CONT,
		...
		if (md != NULL) {
			*minors = md;
			for (dp = DEVI(dip)->devi_minor;
				...
				strcpy(md->driver, ddi_binding_name(dip));
				...
				DECR_DEV_OPS_REF(ops);
				UNLOCK_DEV_OPS(&devnamesp[major].dn_lock);
				sprintf(md->path + strlen(md->path),
					":%s", dp->ddm_name);
				if (dp->next == NULL)
					/* no more */
	...
	for (i = 0; i < adapt->pca_numsockets; i++) {
		...
		if (pcmcia_sockets[s]->ls_flags & PCS_SUSPENDED) {
			if (pcmcia_sockets[s]->ls_flags &
			    (1 << PCE_PM_RESUME)) {
				cs_event(PCE_PM_RESUME, s, 0);
				pcm_event_manager(PCE_PM_RESUME, s, NULL);
			}
			cs_event(PCE_CARD_REMOVAL, s, 0);
...
pcmcia_wait_insert(dev_info_t *dip)
{
	...
	for (tries = adapt->pca_numsockets * 10; tries > 0; tries--) {
		...
		if (!done) {
			...
			cv_timedwait(&pcmcia_condvar, &pcmcia_global_lock,
...
pcmcia_alloc_mem(dev_info_t *dip, ra_request_t *req, ra_return_t *ret)
{
	struct pcmcia_adapter *adapt;
	int r;
	adapt = pcmcia_get_adapter(dip);
	if (adapt == NULL)
		return (DDI_FAILURE);

	if (adapt->pca_mem)
		r = ra_alloc(&adapt->pca_mem, req, ret);
	else
		r = ra_alloc(&ra_mem, req, ret);
	return (r);
}
...
pcmcia_alloc_io(dev_info_t *dip, ra_request_t *req, ra_return_t *ret)
{
	...
	if (adapt->pca_io)
		r = ra_alloc(&adapt->pca_io, req, ret);
	else
		r = ra_alloc(&ra_io, req, ret);
	return (r);
}
...
pcmcia_free_mem(dev_info_t *dip, ra_return_t *ret)
{
	...
	if (adapt == NULL)
		return (DDI_FAILURE);
	if (adapt->pca_mem)
		ra_free(&adapt->pca_mem, ret->ra_addr_lo, ret->ra_len);
	else
		ra_free(&ra_mem, ret->ra_addr_lo, ret->ra_len);
	return (DDI_SUCCESS);
}
...
pcmcia_free_io(dev_info_t *dip, ra_return_t *ret)
{
	...
	if (adapt == NULL)
		return (DDI_FAILURE);
	if (adapt->pca_mem)
		ra_free(&adapt->pca_io, ret->ra_addr_lo, ret->ra_len);
	else
		ra_free(&ra_io, ret->ra_addr_lo, ret->ra_len);
	return (DDI_SUCCESS);
}

pcmcia_init_mem(dev_info_t *dip, ra_return_t *ret)
{
	struct pcmcia_adapter *adapt;
	adapt = pcmcia_get_adapter(dip);
	if (adapt == NULL)
...
pcmcia_init_io(dev_info_t *dip, ra_return_t *ret)
{
	struct pcmcia_adapter *adapt;
	adapt = pcmcia_get_adapter(dip);
	if (adapt == NULL)
