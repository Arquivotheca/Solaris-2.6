...
CardServices(int function, ...)
{
	 ...
	 */
		...
		strcpy(cft->text, cs_error2text(cft->item, CSFUN2TEXT_RETURN));
		retcode = CS_SUCCESS;
		}
		break;
	    case CS_DDI_Info:
...
cs_request_configuration(client_handle_t client_handle, config_req_t *cr)
{
	...
	if (cs_rc1_delay)
	    delay(drv_usectohz(cs_rc1_delay * 1000));
	...
	if (cs_rc2_delay)
	    delay(drv_usectohz(cs_rc2_delay * 1000));
...
cs_release_configuration(client_handle_t client_handle, release_config_t *rcfg)
{
	...
	if (cs_rq_delay)
	    delay(drv_usectohz(cs_rq_delay * 1000));
...
cs_get_cardservices_info(client_handle_t ch, get_cardservices_info_t *gcsi)
{
	...
	strncpy(gcsi->VendorString, CS_GET_CARDSERVICES_INFO_VENDOR_STRING,
					...
					get_physical_adapter_info_t *gpai)
{
	...
	strcpy(gpai->name, sp->adapter.name);
...
cs_event2text(event2text_t *e2t, int event_source)
{
	...
	if (!event_source) {
		...
		if (cs_ss_event_text[event].ss_event == e2t->event) {
		    strcpy(e2t->text, cs_ss_event_text[event].text);
		    return (CS_SUCCESS);
		}
	    }
	    strcpy(e2t->text, cs_ss_event_text[MAX_CS_EVENTS].text);
	    return (CS_SUCCESS);
	} else {
		/*
		 * This is a CS event
...
cs_add_socket(uint_t sn)
{
	 ...
	 */
	sp->adapter.flags = 0;
	strcpy(sp->adapter.name, gcad->adapter_info.name);
...
cs_get_client_info(client_handle_t client_handle, client_info_t *ci)
{
	...
	if (cinfo->Attributes & CS_CLIENT_INFO_VALID) {
	    ...
	    ci->Attributes |= (client->flags & (CS_CLIENT_INFO_CLIENT_MASK |
						INFO_CARD_FLAGS_MASK));
	    strcpy(ci->DriverName, client->driver_name);
	    if (cs_card_for_client(client))
