...
#ident "@(#)cis_handlers.c	1.39	96/10/23 SMI"
					...
					uint32_t flags, void *arg)
{
	...
	if (flags & HANDTPL_PARSE_LTUPLE) {
		...
		for (cs->ns = 0; GET_LEN(tp) > 0 &&
			...
			strcpy(cs->pi[cs->ns++], cis_getstr(tp));
		} /* for */
					...
					uint32_t flags, void *arg)
{
	...
	if (flags & HANDTPL_PARSE_LTUPLE) {
		...
		strcpy(cs->oem, cis_getstr(tp));

		if (GET_LEN(tp) > 0)
		    strcpy(cs->info, cis_getstr(tp));
		else
		    strcpy(cs->info, "(no info)");
	}
	...
	if (flags & (HANDTPL_COPY_DONE | HANDTPL_PARSE_LTUPLE)) {
		...
		if ((tl = tp->len) >= (cisdata_t)MIN_LINKTARGET_LENGTH) {
			...
			if (flags & HANDTPL_PARSE_LTUPLE) {
				...
				cs->length = tl;
				strncpy(cs->tpltg_tag, (char *)cp, cs->length);
