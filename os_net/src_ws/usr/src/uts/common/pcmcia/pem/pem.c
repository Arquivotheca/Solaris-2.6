 ...
 * All rights reserved.
 */
#pragma ident "@(#)pem.c	1.39	96/10/18 SMI"
...
_init(void)
{
	...
	mutex_init(&pem_global_lock, "pem global lock", MUTEX_DRIVER, NULL);
...
pem_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		...
		ddi_add_softintr(dip, DDI_SOFTINT_MED, &pem_intr_id,
					&pem_iblock,
					0,
					pem_soft_intr, (caddr_t)dip);

		mutex_init(&pem_intr_lock, "pem interrupt lock",
				MUTEX_SPIN, (void *) ipltospl(SPL7));
		break;
...
pem_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	case DDI_DETACH:
		...
		pcmcia_set_em_handler(NULL, NULL, 0, 0x1234, NULL, NULL);
