...
#pragma ident	"@(#)page.h	1.98	96/07/31 SMI"
...
#define	PAGE_HASH_FUNC(vp, off) \
	((((u_int)(off) >> PAGESHIFT) + ((int)(vp) >> PAGE_HASHVPSHIFT)) & \
		(PAGE_HASHSZ - 1))
...
extern	u_int	total_pages;		/* total pages in the system */
...
page_t	*page_create(struct vnode *, u_offset_t, u_int, u_int);
page_t	*page_create_va(struct vnode *, u_offset_t, u_int, u_int,
	struct as *, caddr_t);
int	page_create_wait(u_int npages, u_int flags);
void    page_create_putback(int npages);
...
page_t  *page_nextn(page_t *, u_int);
...
void page_set_props(page_t *, u_int);
