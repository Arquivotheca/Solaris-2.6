...
#pragma ident   "@(#)vm_swap.c 1.74     96/10/18 SMI"
...
int
swapctl(void *uap, rval_t *rvp)
{
	register struct swapcmda *scp = (struct swapcmda *)uap;
	...
	case SC_AINFO:
		...
		ai.ani_free = k_anoninfo.ani_free +
					MAX(availrmem - swapfs_minfree, 0);

		ai.ani_resv = k_anoninfo.ani_phys_resv
					+ k_anoninfo.ani_mem_resv;

		if (copyout(&ai, scp->sc_arg, sizeof (struct anoninfo)) != 0)
			return (EFAULT);
		return (0);

	case SC_LIST:
		if (copyin(scp->sc_arg, &length, sizeof (int)) != 0)
			return (EFAULT);
beginning:
		...
		csip = (struct swapinfo *)
			kmem_zalloc(tmp_nswapfiles * sizeof (struct swapinfo),
			KM_SLEEP);
retry:
		...
		ust = (swapent_t *)((swaptbl_t *)scp->sc_arg)->swt_ent;
...
lout:
	...
	if (copyin(scp->sc_arg, &sr, sizeof (swapres_t)))
		return (EFAULT);
