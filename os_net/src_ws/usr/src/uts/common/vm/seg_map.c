...
#pragma	ident	"@(#)seg_map.c	1.89	96/08/08	SMI"
...
static u_offset_t	segmap_getoffset(struct seg *, caddr_t);
...
static int	segmap_pagelock(struct seg *, caddr_t, u_int,
			struct page ***, enum lock_type, enum seg_rw);
static void	segmap_badop(void);
static int	segmap_getmemid(struct seg *, caddr_t, memid_t *);
...
static void	segmap_smapadd(struct smfree *, struct smap *);
static void	segmap_smapsub(struct smfree *, struct smap *, int);
static struct smap *segmap_hashin(struct smap *, struct vnode *,
		u_offset_t, int);
static void	segmap_hashout(struct smap *);
	...
	void *argsp;
{
	 ...
	 */
	smd->smd_prot = a->prot;
	smd_smap = smd->smd_sm = (struct smap *)
	 ...
	 */
	if (a->nfreelist < 1) {
		cmn_err(CE_PANIC, "no nfreelist %d\n", a->nfreelist);
	}
	...
	if (smd->smd_nfree & (smd->smd_nfree - 1)) {
		...
		cmn_err(CE_PANIC, "bad ncolor %d\n", smd->smd_nfree);
	}

	smd_free = smd->smd_free = (struct smfree *)
		kmem_zalloc(smd->smd_nfree * sizeof (struct smfree),
			KM_SLEEP);
	...
	smd_hash = smd->smd_hash = (struct smap **)
	...
	for (i = 0; i < smd->smd_nfree; i++) {
		...
		sm = &smd->smd_free[i];
		sprintf(lkname, "sm_mtx_%d", i);
		mutex_init(&sm->sm_mtx, lkname, MUTEX_DEFAULT, DEFAULT_WT);
	}

	for (i = 0; i < NSMAP_MTX; i++) {
		char lkname[20];

		sprintf(lkname, "smap_mtx_%d", i);
		mutex_init(&smap_mtx[i], lkname, MUTEX_DEFAULT, DEFAULT_WT);
	}

	for (i = 0; i < NSHASH_MTX; i++) {
		char lkname[20];

		sprintf(lkname, "smap_hash_%d", i);
		mutex_init(&shash_mtx[i], lkname, MUTEX_DEFAULT, DEFAULT_WT);
	}
	...
	register struct smap *smp;
{
	...
	vp = smp->sm_vp;
	off = smp->sm_off + (u_offset_t)((u_int)addr & MAXBOFFSET);
	...
	for (adr = addr; adr < addr + len; adr += PAGESIZE, off += PAGESIZE) {
		...
		if (rw == S_WRITE) {
			...
			TRACE_3(TR_FAC_VM, TR_SEGMAP_FAULT,
				"segmap_fault:pp %x vp %x offset %llx",
				pp, vp, off);
			hat_setref(pp);
		}
	...
	enum seg_rw rw;
{
	...
	addroff = (u_offset_t)((u_int)addr & MAXBOFFSET);
	...
	caddr_t addr;
{
	...
	err = VOP_GETPAGE(vp, (offset_t)(off + ((offset_t)((u_int)addr
 ...
 */
static void
segmap_smapadd(sm, smp)
	register struct smfree *sm;
	register struct smap *smp;
{
	...
	int softlock;
{
	...
	for (; addr < eaddr; addr += PAGESIZE, off += PAGESIZE) {
		...
		if (pp == NULL) {
			 ...
			 */
			bitindex =
			    (u_short)((int)(off - smp->sm_off) / PAGESIZE);
			/*
		...
		TRACE_5(TR_FAC_VM, TR_SEGMAP_PAGECREATE,
		    "segmap_pagecreate:seg %x addr %x pp %x vp %x offset %llx",
		    seg, addr, pp, vp, off);
	}
}
...
vrfy_smp:
	if (smp != NULL) {
		...
		if (hwshift == 0) {
			 ...
			 */
			color = ((((u_int) vp << 2) + baseoff) >> MAXBSHIFT)
			    & colormsk;
		} else {
			/*
			 ...
			 * used more heavily.
			 */
			color = (((((u_int)(vp) << (hwshift - 6)) & ~hwmsk) +
			    (CPU->cpu_id << hwshift) +
	...
	TRACE_4(TR_FAC_VM, TR_SEGMAP_GETMAP,
		"segmap_getmap:seg %x addr %x vp %x offset %llx",
	...
	u_int flags;
{
	...
	if (smp->sm_refcnt == 1) {
		while (bitindex = highbit((u_long)smp->sm_bitmap)) {
			...
			smp->sm_bitmap &= ~SMAP_BIT_MASK(bitindex);
			hat_unlock(kas.a_hat, naddr, PAGESIZE);
			page_unlock(pp);
		}
	}
...
/*ARGSUSED*/
static int
segmap_pagelock(struct seg *seg, caddr_t addr, u_int len,
