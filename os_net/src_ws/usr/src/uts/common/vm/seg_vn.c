...
#ident		"@(#)seg_vn.c 1.156	96/06/13	SMI"
...
static u_offset_t	segvn_getoffset(struct seg *, caddr_t);
...
static int	segvn_pagelock(struct seg *, caddr_t, u_int,
		    struct page ***, enum lock_type, enum seg_rw);
static int	segvn_getmemid(struct seg *, caddr_t, memid_t *);
#else
...
static u_offset_t	segvn_getoffset(/* seg, addr */);
...
static int	segvn_pagelock(/* seg, addr, len, page, type, rw */);
static int	segvn_getmemid(/* seg, caddr_t, memid_t */);
...
static int	segvn_faultpage(struct hat *, struct seg *, caddr_t, u_offset_t,
...
static void segvn_reclaim(struct seg *, caddr_t, u_int, struct page **,
	enum seg_rw);
...
retry:
	...
	if (addr == seg->s_base) {
		...
		if (svd->swresv) {
			if (svd->flags & MAP_NORESERVE) {
				ASSERT(amp);
				oswresv = svd->swresv;
				svd->swresv = (u_int)anon_pages(amp->anon,
				    svd->anon_index, npages);
				anon_unresv((u_int)(oswresv - svd->swresv));
			} else {
				anon_unresv((u_int)len);
				svd->swresv -= len;
			}
			TRACE_5(TR_FAC_VM, TR_ANON_PROC,
				"anon proc:%u %u %u %u %u",
	...
	if (svd->swresv) {
		if (svd->flags & MAP_NORESERVE) {
			...
			nsvd->swresv = (u_int)anon_pages(amp->anon,
	...
	if ((amp = svd->amp) != NULL) {
		...
		if (--amp->refcnt == 0) {
			if (svd->type == MAP_PRIVATE) {
				 ...
				 */
				anon_free(amp->anon, (u_int)amp->size);
				if (amp->swresv) {
					anon_unresv((u_int)amp->swresv);
					TRACE_5(TR_FAC_VM, TR_ANON_PROC,
						"anon proc:%u %u %u %u %u",
						curproc->p_pid, seg,
			 ...
			 */
			app = &amp->anon[svd->anon_index];
			anon_free(app, (u_int)seg->s_size);
			mutex_exit(&amp->lock);
		} else {
			mutex_exit(&amp->lock);
		}
	...
	enum seg_rw rw;
{
	...
	for (adr = addr; adr < addr + len; adr += PAGESIZE) {
		...
		if (pp == NULL) {
			cmn_err(CE_PANIC,
			    "segvn_softunlock: addr %x, ap %x, vp %x, off %llx",
			    (int)adr, (int)ap, (int)vp, offset);
		}
		...
		TRACE_3(TR_FAC_VM, TR_SEGVN_FAULT,
			"segvn_fault:pp %x vp %x offset %llx", pp, vp, offset);
		page_unlock(pp);
	}
	...
	enum seg_rw rw;			/* type of access at fault */
{
	...
	TRACE_3(TR_FAC_VM, TR_SEGVN_FAULT,
		"segvn_fault:pp %x vp %x offset %llx",
...
slow:
	...
	off = svd->offset + (u_int)(addr - seg->s_base);
	...
	*plp = (page_t *)NULL;
	...
	for (ppp = plp; (pp = *ppp) != NULL; ppp++) {
		...
		if (pp->p_offset >=  svd->offset &&
			...
			diff = (u_int)(pp->p_offset - svd->offset);

			/*
			 * Large Files: Following is the assertion
			 * validating the above cast.
			 */
			ASSERT((pp->p_offset - svd->offset) <= UINT_MAX);
	...
	for (page = 0; page < npages; page++) {
		...
		if (amp != NULL) {
			...
			if (ap != NULL) {
				...
				vp = svd->vp;
				off = svd->offset + ptob((u_offset_t)page);
			}
			mutex_exit(&amp->lock);
		} else {
			vp = svd->vp;
 ...
 */
static int
segvn_pagelock(struct seg *seg, caddr_t addr, u_int len, struct page ***ppp,
    enum lock_type type, enum seg_rw rw)
{
	struct segvn_data *svd = (struct segvn_data *)seg->s_data;
	u_int np, npages = (len >> PAGESHIFT);
	u_int anon_index;
	u_int protchk;
	struct anon_map *amp;
	...
	TRACE_2(TR_FAC_PHYSIO, TR_PHYSIO_SEGVN_START,
		"segvn_pagelock: start seg %x addr %x", seg, addr);
	...
	if (type == L_PAGEUNLOCK) {
		...
		TRACE_2(TR_FAC_PHYSIO, TR_PHYSIO_SEGVN_UNLOCK_END,
			"segvn_pagelock: unlock seg %x addr %x", seg, addr);
		return (0);
		...
		SEGVN_LOCK_EXIT(seg->s_as, &svd->lock);
		TRACE_2(TR_FAC_PHYSIO, TR_PHYSIO_SEGVN_UNLOCK_END,
			"segvn_pagelock: reclaim seg %x addr %x", seg, addr);
		return (0);
	}
	...
	if (pplist != NULL) {
		...
		*ppp = pplist;
		...
		TRACE_2(TR_FAC_PHYSIO, TR_PHYSIO_SEGVN_HIT_END,
			"segvn_pagelock: cache hit seg %x addr %x", seg, addr);
		return (0);
	} else {
	...
	if (svd->vp != NULL) {
		...
		*ppp = NULL;
		TRACE_2(TR_FAC_PHYSIO, TR_PHYSIO_SEGVN_MISS_END,
		    "segvn_pagelock: mapped vnode seg %x addr %x", seg, addr);
		return (ENOTSUP);
	}
	...
	pplist = (page_t **)kmem_alloc(sizeof (page_t *) * npages, KM_SLEEP);
	*ppp = pl = pplist;
	...
	if (a >= addr + len) {
		...
		TRACE_2(TR_FAC_PHYSIO, TR_PHYSIO_SEGVN_FILL_END,
			"segvn_pagelock: cache fill seg %x addr %x", seg, addr);
		return (0);
	}
	pplist = pl;
	np = ((a - addr) >> PAGESHIFT);
...
out:
	...
	*ppp = NULL;
	TRACE_2(TR_FAC_PHYSIO, TR_PHYSIO_SEGVN_MISS_END,
		"segvn_pagelock: cache miss seg %x addr %x", seg, addr);
	return (EFAULT);
}
...
static void
segvn_reclaim(struct seg *seg, caddr_t addr, u_int len, struct page **pplist,
	enum seg_rw rw)
{
	struct segvn_data *svd = (struct segvn_data *)seg->s_data;
	u_int np, npages;
