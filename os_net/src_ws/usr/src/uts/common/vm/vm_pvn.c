...
#pragma ident   "@(#)vm_pvn.c 1.147     96/06/07 SMI"
	...
	int isra;
{
	...
	if (pagesavail < btopr(vp_len)) {
		...
		if (pagesavail < btopr(vp_len)) {
			 ...
			 */
			vp_len = ptob((u_offset_t)pagesavail);
		}
	}
	...
	while (plist != NULL) {
		...
		if (flags & B_ERROR) {
			...
			TRACE_3(TR_FAC_VM, TR_PAGE_WS_OUT,
				"page_ws_out:pp %x vp %x offset %llx",
			...
			ck_refmod:
				if (!(ppattr & (P_REF | P_MOD))) {
					...
					"page_ws_free:pp %x vp %x offset %llx",
					pp, pp->p_vnode, pp->p_offset);
				}
			} else {
				/*
	...
	int flags;
{
	...
	TRACE_3(TR_FAC_VM, TR_PVN_GETDIRTY,
		"pvn_getdirty:pp %x vp %x offset %llx",
		pp, pp->p_vnode, pp->p_offset);
...
pvn_getpages(getapage, vp, off, len, protp, pl, plsz, seg, addr, rw, cred)
	int (*getapage)(vnode_t *, u_offset_t, u_int, u_int *, page_t *[],
