...
#pragma ident	"@(#)vpage.h	1.20	96/01/10 SMI"
 ...
 */
struct vpage {
	u_char nvp_prot;	/* see <sys/mman.h> prot flags */
	u_char nvp_advice;	/* pplock & <sys/mman.h> madvise flags */
};

/*
 * This was changed from a bitfield to flags/macros in order
 * to conserve space (u_char bitfields are not ANSI).  This could
 * have been condensed to a u_char, but at the expense of complexity.
...
#define	VPP_ISPPLOCK(vpp) \
	((u_char)((vpp)->nvp_advice & VP_PPLOCK_MASK))
