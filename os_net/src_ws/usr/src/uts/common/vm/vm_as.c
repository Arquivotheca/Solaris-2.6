...
#ident	"@(#)vm_as.c	1.105	96/09/12 SMI"
...
static void as_setwatchprot(struct as *, struct seg *, caddr_t, u_int, u_int);
static void as_clearwatchprot(struct as *, struct seg *, caddr_t, u_int);
 ...
 */
void
as_pageunlock(struct as *as, struct page **pp, caddr_t addr, u_int size,
	enum seg_rw rw)
{
	struct seg *seg;
	u_int rsize;
 ...
 */
faultcode_t
as_pageflip(as_to, addr_to, kaddr, sizep)
	struct	as *as_to;
	...
	u_int	*sizep;
{
	 ...
	 */
	res = hat_pageflip(as_to->a_hat, addr_to, kaddr, (size_t *)sizep,
	    &pp_to[0], &pp_from[0]);
	if (total > MAXPAGEFLIPSIZ) {
	...
	while (--i >= 0) {
		ASSERT(se_excl_assert(&pp_to[i]->p_selock));
		ASSERT(se_excl_assert(&pp_from[i]->p_selock));
 ...
 */
static void
as_setwatchprot(struct as *as, struct seg *seg, caddr_t addr, u_int size,
	u_int prot)
{
	struct watched_page *pwp;
 ...
 */
static void
as_clearwatchprot(struct as *as, struct seg *seg, caddr_t addr, u_int size)
{
	caddr_t eaddr = addr + size;
	struct watched_page *pwp;
	u_int prot;
