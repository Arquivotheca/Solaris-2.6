...
#ident	"@(#)conf.c	5.95	93/09/12 SMI"	/* from SunOS4.0 5.63	*/
...
struct cb_ops no_cb_ops = {
	...
	nopropop,	/* cb_prop_op	*/
	0,		/* stream tab	*/
	0		/* char/blk driver compatibility flag */
};
...
#define	CB_DRV_INSTALLED(dev_opsp) \
		((dev_opsp) != &nodev_ops && (dev_opsp) != &mod_nodev_ops)
