/*
 * Copyright (c) 1992-1996, by Sun Microsytems, Inc.
 * All rights reserved.
 */
...
#pragma ident	"@(#)seg_drv.c	1.32	96/08/08 SMI"
...
static u_offset_t	segdrv_getoffset(struct seg *, caddr_t);
...
static int	segdrv_pagelock(struct seg *, caddr_t, u_int,
			struct page ***, enum lock_type, enum seg_rw);
static int	segdrv_getmemid(struct seg *, caddr_t, memid_t *);
		...
		if (fptr) {
			...
			if (error > 0) {
				if (newsdp->vpage != NULL)
					kmem_free((caddr_t)newsdp->vpage,
					    nbytes);
				VN_RELE(sdp->vp);
				newseg->s_data = NULL;
...
segdrv_getoffset(register struct seg *seg, caddr_t addr)
{
	...
	{
	register u_offset_t (*fptr)() = sdp->client_segops->getoffset;
	if (fptr)
...
/*ARGSUSED*/
static int
segdrv_pagelock(struct seg *seg, caddr_t addr, u_int len,
...
{
	register struct segdrv_data *sdp = (struct segdrv_data *)seg->s_data;
	register int (*fptr)() = sdp->client_segops->getmemid;
