...
#pragma ident  "@(#)sedev.h 1.11     96/10/15 SMI"
...
#define	HDLC_DEVICE	(1 << (NBITSMINOR - 2))
#define	CLONE_DEVICE	(1 << (NBITSMINOR - 3))
...
typedef
    struct se_regs {
	    u_char sab_xfifo[SAB_FIFO_SIZE]; /* transmit fifo */
	    u_char sab_cmdr;	/* Command register */
	    u_char sab_pre;	/* Preamble register (hdlc only) */
	    u_char sab_mode;	/* Mode register */
	    u_char sab_timr;	/* Timer register */
	    u_char sab_xon;	/* Xon character */
	    u_char sab_xoff;	/* xoff character */
	    u_char sab_tcr;	/* Termination character */
	    u_char sab_dafo;	/* Data format */
	    u_char sab_rfc;	/* rfifo control register */
	    u_char sab_ral2;	/* receive address low register (hdlc only) */
	    u_char sab_xbcl;	/* Transmit byte count low */
	    u_char sab_xbch;	/* transmit byte count high */
	    u_char sab_ccr0;	/* Channel configuration register 0 */
	    u_char sab_ccr1;	/* Channel configuration register 1 */
	    u_char sab_ccr2;	/* Channel configuration register 2 */
	    u_char sab_ccr3;	/* Channel configuration register 3 */
	    u_char sab_tsax;	/* Time-slot assignment register transmit */
	    u_char sab_tsar;	/* Time-slot assignment register receive */
	    u_char sab_xccr;	/* transmit channel capacity register */
	    u_char sab_rccr;	/* receive channel capacity register */
	    u_char sab_bgr;	/* Baud Rate generator register */
	    u_char sab_tic;	/* Transmit immediate character */
	    u_char sab_mxn;	/* Mask for XON character */
	    u_char sab_mxf;	/* Mask for XOFF character */
	    u_char sab_iva;	/* Interrupt Vector Address */
	    u_char sab_ipc;	/* Interrupt port configuration */
	    u_short sab_imr;	/* Interrupt mask (bytes 0 and 1) */
	    u_char sab_pvr;	/* Port value register */
	    u_char sab_pim;	/* Port interrupt mask */
	    u_char sab_pcr;	/* Port configuration register */
	    u_char sab_ccr4;	/* Channel configuration register 4 (hdlc) */
	} se_regs_t;
...
typedef
	 ...
	 */
	int		za_break_timer;	/* Timerid for sending break */
	u_char		za_rcv_mask;	/* Mask parity from incoming chars */
	u_char		za_rcv_flag;	/* Receive flags of various types */
	u_char		za_ext;		/* modem status change flag */
	u_char		za_flon_mask;	/* Work around bug in v2.2 chip */
} se_asyncl_t;
...
typedef
	...
	u_char		sl_m_error;
	u_char		sl_clockmode;	/* clock to enable for xmit/rcv */
	struct {
...
typedef
	...
	cond_t		*sc_flags_cv;	/* condition variable for flags */
	...
	int		sc_bufcid;
	int		sc_kick_rcv_id;	/* Callback id for kick_rcv */

	short		sc_bufsize;	/* Buffer size to use (async,hdlc) */
	u_char		sc_chipv23;	/* Chip version number (2.2 vs 2.3) */
	u_char		sc_char_pending; /* Pending flow control character */

	u_char		sc_flag_softint; /* Need software interrupt flag */
	u_char		sc_suspend;	/* Device is suspended for CPR */
	u_char		sc_xmit_active;	/* Transmit is known to be active */
	u_char		sc_soft_active;	/* Software interrupt active */

	u_short		sc_imr;		/* Interrupt mask as set */
	u_char		sc_softcar;	/* Softcarrier */
	u_char		sc_closing;	/* Device is closing */

	u_char		sc_xmit_done;	/* Flag indicating xmit need softint */
	u_char		sc_dtrbit;	/* Locations of shared bits  */
	u_char		sc_dsrbit;
	u_char		sc_rtsbit;
	union {
...
typedef
	...
	u_char		sec_ipc;	/* Interrupt port configuration */
	u_char		sec_pim;	/* Port interrupt mask */
	u_char		sec_pvr;	/* (p)port value register */
	u_char		sec_pcr;	/* (p)port configuration register */
} se_chip_t;

/* Protocol specific operations */

typedef struct se_ops {
	void (*portint)(se_ctl_t *, u_short); /* per-port interrupt hdlr */
