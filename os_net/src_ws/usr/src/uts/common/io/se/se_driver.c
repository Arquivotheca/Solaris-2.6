...
#pragma ident  "@(#)se_driver.c 1.45     96/10/17 SMI"
...
static int se_async_open(se_ctl_t *, queue_t *, int, cred_t *);
static int se_hdlc_open(se_ctl_t *, queue_t *, int, cred_t *);
...
static void se_callback(se_ctl_t *);
static void se_kick_rcv(caddr_t);
...
static struct modldrv modldrv = {
	&mod_driverops, "Siemens SAB 82532 ESCC2 1.45", &se_dev_ops};
...
se_wait_suspend(se_ctl_t *sc)
{
	...
	ddi_dev_is_needed(sc->sc_chip->sec_dip, sc->h.sc_unit, 1);
...
se_fillsby(se_ctl_t *sc)
{
	...
	while ((sc->sc_rstandby_ptr < SE_MAX_RSTANDBY) &&
		...
		if (bp == NULL) {
			if (sc->sc_bufcid == 0) sc->sc_bufcid =
			    bufcall(sc->sc_bufsize, BPRI_MED,
				    se_callback, (intptr_t)sc);
			return;
		} else {
			DRIVER_LOCK;
	...
	if (sc->sc_rstandby_ptr < SE_MAX_RSTANDBY) { /* if max_rdone... */
		if (sc->sc_kick_rcv_id == 0) /* schedule wakeup */
		    sc->sc_kick_rcv_id =
			timeout(se_kick_rcv, (caddr_t)sc, KICK_RCV_TIMER);
	}
}
...
se_close(queue_t *rq, int flag)
{
	...
	if (sc->h.sc_protocol != CLON_PROTOCOL) {
		...
		pm_idle_component(sc->sc_chip->sec_dip, sc->h.sc_unit%2+1);
	}
	return (rval);
}
...
se_open(queue_t *rq, dev_t *dev, int flag, int sflag, cred_t *cr)
{
	...
	pm_busy_component(sc->sc_chip->sec_dip, unit%2+1);
	ddi_dev_is_needed(sc->sc_chip->sec_dip, unit%2+1, 1);

	if (rval == 0) do {
		rval = (sc->h.sc_ops->open)(sc, rq, flag, cr); /* Open */
...
{
	u_int rval = DDI_INTR_UNCLAIMED; /* did/did not handle int. */
...
se_mctl(se_ctl_t *sc, u_int bits, u_int how)
{
	...
	u_long timeval;		/* Timestamp for dtrlow purposes */
 ...
 */
			...
			while (timeval <= (sc->sc_dtrlow + se_default_dtrlow)) {
				DRIVER_UNLOCK;
				dki_cond_wait(((cond_t *)&lbolt_cv), PRISAME,
					    (lock_t *)sc->h.sc_excl, 0);
				SE_CHECK_SUSPEND(sc);
				DRIVER_LOCK;
				drv_getparm(TIME, &timeval);
			}
...
se_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	if (cmd == DDI_RESUME) {
		if (sec == NULL)
		    return (DDI_FAILURE);
		ddi_putb(sec->sec_porta->sc_handle,
	...
	if (!se_initialized) {	/* First pass through, set up globals */
		...
		mutex_init(&se_hi_excl, "se_hi_excl", MUTEX_DRIVER,
			se_hi_iblock);
	...
	chip_version = ddi_getb(handle, &rz->sab_vstr) & 0x0f;
...
/* but not necessarily. Allow re-attaching an existing device to work. */
	...
	if (sec == NULL) {	/* If we are adding a device, allocate */
		...
		mutex_init(sca->h.sc_excl, "sc_excl lock",
			MUTEX_DRIVER, se_iblock);
		...
		mutex_init(scb->h.sc_excl, "sc_excl lock",
			MUTEX_DRIVER, se_iblock);
	...
	ddi_putb(sca->sc_handle, &sca->sc_reg->sab_pcr, sec->sec_pcr);
	ddi_putb(sca->sc_handle, &sca->sc_reg->sab_pvr, sec->sec_pvr);
...
/*
 * Note: Below six levels of "if" are not indented.
 */

	/* hdlc devices */
	sprintf(name, "%c,hdlc", sca->h.sc_unit + '0');
	if (ddi_create_minor_node(dip, name, S_IFCHR,
				sca->h.sc_unit | HDLC_DEVICE,
				DDI_PSEUDO, NULL) != DDI_FAILURE) {
	sprintf(name, "%c,hdlc", scb->h.sc_unit + '0');
	if (ddi_create_minor_node(dip, name, S_IFCHR,
				scb->h.sc_unit | HDLC_DEVICE,
				DDI_PSEUDO, NULL) != DDI_FAILURE) {

	sprintf(name, "%c", sca->h.sc_unit + 'a');
	if (ddi_create_minor_node(dip, name, S_IFCHR,
				sca->h.sc_unit,
				DDI_NT_SERIAL_MB, NULL) != DDI_FAILURE) {
	sprintf(name, "%c", scb->h.sc_unit + 'a');
	if (ddi_create_minor_node(dip, name, S_IFCHR,
				scb->h.sc_unit,
				DDI_NT_SERIAL_MB, NULL) != DDI_FAILURE) {
	/* outdial "cua" devices */
	sprintf(name, "%c,cu", sca->h.sc_unit + 'a');
	if (ddi_create_minor_node(dip, name, S_IFCHR,
				sca->h.sc_unit | OUTLINE,
				DDI_NT_SERIAL_MB_DO, NULL) != DDI_FAILURE) {
	sprintf(name, "%c,cu", scb->h.sc_unit + 'a');
	if (ddi_create_minor_node(dip, name, S_IFCHR,
	...
	/* Fall through to here if one of the creates failed */
	cmn_err(CE_WARN, "se:[%d] Failed to create node %s", chipno, name);
	ddi_remove_minor_node(dip, NULL);
	return (DDI_FAILURE);
...
static void
se_kick_rcv(caddr_t arg)
{
	se_ctl_t *sc = (void *)arg;
...
/*
 * Note - We handle the ALLS interrupt before the XPR interrupt. If we get
 * an interrupt with both bits set, we will clear xmit_active, and then
 * txint can turn xmit_active back on if it starts things going again.
 */
	...
	if (rq == NULL) {
		if (sc->z.za.zas.zas_wopen && sc->z.za.za_ext &&
			...
			if ((mstat & TIOCM_CD) ||
				...
				dki_cond_broadcast(sc->sc_flags_cv, NOPRMT);
			}
		}
 ...
 */
	...
	if (unhangup || (sc->z.za.zas.zas_draining && !sc->sc_xmit_active))
	    dki_cond_broadcast(sc->sc_flags_cv, NOPRMT); /* Wakeup! */
	if (unhangup)
	    putnextctl(rq, M_UNHANGUP);	/* Send a message saying unhangup */

	if (hangup)
	    putnextctl(rq, M_HANGUP);

	if (mbreak)
	    putnextctl(rq, M_BREAK);
 ...
 */
	...
	while (canputnext(rq)) {
		...
		if (sc->sc_kick_rcv_id == 0)
			...
			timeout(se_kick_rcv, (caddr_t)sc, 3);
	}
	...
	while (mp = getq(wq))	/* Loop taking messages off of write queue */
		...
		case (M_BREAK): {		/* Request to send a break */
			...
			sc->z.za.za_break_timer =
			    timeout(se_break_end, (caddr_t)sc, hz/4);
			break;
		}

		case (M_IOCTL): { /* Request to perform an ioctl */
			if (sc->sc_xmit_active) {
				putbq(wq, mp); /* If busy, try later */
				return;
			}
		...
		case (M_DATA): {
			...
			if (sc->z.za.zas.zas_stopped || sc->sc_wr_cur) {
				putbq(wq, mp); /* try later */
				return;
			}
...
static void
se_async_program(se_ctl_t *sc, int toggle_baud_rate)
{
 ...
 */
	...
	REG_GETW(sc, sab_isr);	/* Wipe pending interrupts */
...
se_async_suspend(se_ctl_t *sc)
{
	...
	if (sc->z.za.zas.zas_break) {
		sc->z.za.zas.zas_break = 0;
		untimeout(sc->z.za.za_break_timer);
	}
...
se_async_resume(se_ctl_t *sc)
{
	...
	sc->sc_xmit_active = 0;
	se_async_program(sc, TRUE); /* Initialize the chip */
...
static int
se_async_open(se_ctl_t *sc, queue_t *rq, int flag, cred_t *cr)
{
	int    mbits, len;
 ...
 */
	if (!(flag & (FNDELAY|FNONBLOCK)) &&
		...
		if (dki_cond_wait_sig(sc->sc_flags_cv, PRISAME,
					(lock_t *)sc->h.sc_excl, 0) == FALSE) {
			/*
 ...
 */
	...
	while (sc->sc_soft_active ||
		...
		drv_getparm(TIME, &now); /* Get current time to re-compute */
		if ((now - close_time) > 5) break; /* Don't wait forever. */
 ...
 */
	...
	sc->h.sc_ops = &se_null_ops; /* Not async operations any more. */
	sc->h.sc_protocol = NULL_PROTOCOL;
	sc->sc_closing = 0;

	dki_cond_broadcast(sc->sc_flags_cv, NOPRMT); /* Notify any waiters */
 ...
 */
	...
	switch (mp->b_datap->db_type) {
	    ...
	    case (M_DELAY):
	    case (M_BREAK):
		putq(wq, mp);	/* queue them up behind other activity */
 ...
 */
			...
			sc->sc_xmithead = sc->sc_xmitblk = NULL;
			sc->sc_wr_lim = sc->sc_wr_cur = NULL;
			PUT_CMDR(sc, SAB_CMDR_XRES); /* zap chip */
			sc->sc_xmit_active = 0;	/* Not xmit active. */
			DRIVER_UNLOCK;
...
se_async_ioctl(se_ctl_t *sc, queue_t *wq, mblk_t *mp)
{
	...
	if (error == 0) {
		se_async_program(sc, FALSE); /* Assume chip needs to be told */
	} else if (error < 0) {
...
static int
se_hdlc_open(se_ctl_t *sc, queue_t *rq, int flag, cred_t *cr)
{
#ifdef lint
	flag = flag;
	cr = cr;
...
se_hdlc_close(se_ctl_t *sc, queue_t *rq)
{
	...
	if (sc->z.zh.sl_wd_id) {
		untimeout(sc->z.zh.sl_wd_id); /* Stop the watchdog */
		sc->z.zh.sl_wd_id = 0;
	}
	if (sc->sc_bufcid) {
...
se_hdlc_program(se_ctl_t *sc)
{
	...
	REG_GETW(sc, sab_isr);	/* Wipe any pre-existing error status */
...
se_hdlc_watchdog(se_ctl_t *sc)
{
	...
	if (sc->sc_xmit_active)  { /* transmitter active : start new timeout */
		...
		sc->z.zh.sl_wd_id = timeout(se_hdlc_watchdog, (caddr_t)sc,
...
se_hdlc_rmeint(se_ctl_t *sc)
{
	...
	if ((sc->sc_rd_cur - sc->sc_rcvblk->b_rptr & 0xff) != rbcl)
	    cmn_err(CE_WARN, "se%d rmeint bad sizes %d %d", sc->h.sc_unit,
		    rbcl, sc->sc_rd_cur - sc->sc_rcvblk->b_rptr);
...
se_hdlc_setmstat(se_ctl_t *sc, int event)
{
	...
	drv_getparm(TIME, (ulong_t *)&sls->tstamp.tv_sec); /* get timestamp */
	...
	switch (iocp->ioc_cmd) {
		...
		*(int *)mp->b_cont->b_rptr = sc->sc_bufsize;
		...
		if (mp->b_cont) {
			...
			se_mctl(sc, TIOCM_DTR, DMBIS);
		    } else {
			se_mctl(sc, TIOCM_DTR, DMBIC);
		    }
		}
		DRIVER_UNLOCK;
		break;
...
se_clone_open(queue_t *rq, dev_t *dev)
{
	...
	if (scl == NULL) {
		...
		scl->h.sc_excl =
		    (kmutex_t *)kmem_zalloc(sizeof (kmutex_t), KM_SLEEP);
		mutex_init(scl->h.sc_excl, "sc_excl lock",
			    MUTEX_DRIVER, se_iblock);
...
se_clone_wput(se_ctl_t *sc, queue_t *wq, mblk_t *mp)
{
	...
	switch (mp->b_datap->db_type) {
		...
		if (error) {
			PORT_UNLOCK(sc);
			putnextctl1(RD(wq), M_ERROR, error); /* Complain */
			PORT_LOCK(sc);
		}
		break;
	    ...
	    default:
		PORT_UNLOCK(sc);
		putnextctl1(RD(wq), M_ERROR, EPROTO); /* Complain */
		PORT_LOCK(sc);
	}
	return (0);
}
