...
#pragma ident	"@(#)vjcomp.c	1.13	94/02/16 SMI"
	...
	vjstruct_t *comp;
{
	...
	if (((u_short *)ip)[0] != ((u_short *)&cs->cs_ip)[0] ||
	    ...
	    (deltaS > 5 &&
	    bcmp((caddr_t) (ip + 1), (caddr_t) (&cs->cs_ip + 1),
	    (deltaS - 5) << 2)) ||
	    (th->th_off > 5 &&
	    bcmp((caddr_t) (th + 1), (caddr_t) (oth + 1),
	    (th->th_off - 5) << 2)))
		goto uncompressed;
 ...
 */
	...
	bcopy((caddr_t) ip, (caddr_t) &cs->cs_ip, hlen);
	...
	*cp++ = (u_char) (deltaA >> 8);
	*cp++ = (u_char) deltaA;
	bcopy((caddr_t) new_seq, (caddr_t) cp, deltaS);
 ...
 */
	...
	bcopy((caddr_t) &cs-> cs_ip, (caddr_t) newmp-> b_wptr, cs-> cs_hlen);
