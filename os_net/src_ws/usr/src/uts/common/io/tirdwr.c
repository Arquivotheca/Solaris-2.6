...
#ident	"@(#)tirdwr.c	1.29	96/07/29 SMI" /* from S5R4 1.4 */
 ...
 */
static	int tirdwropen(), tirdwrclose();
...
cred_t			*cr;
{
	...
	(void) strqset(WR(q), QMAXPSZ, 0, WR(q)->q_next->q_maxpsz);
	(void) strqset(q, QMAXPSZ, 0, q->q_next->q_maxpsz);
