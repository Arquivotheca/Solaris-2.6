#pragma ident	"@(#)ppp_common.c 1.15	95/03/07 SMI"
...
static void ppp_timeout(pppMachine_t *);
 ...
 */
		...
		putnextctl1(machp->readq, M_ERROR, EPROTONOSUPPORT);
		goto discard;
 ...
 */
		...
		adjmsg(machp->pkt, machp->linkp->conf.pppLinkRemoteMRU +
							PPP_FR_HDRSZ - pktsz);
	}
 ...
 */
	...
	PPP_MEMDBG2("add_opt: allocated %d bytes, addr=%x\n",
		sizeof (pppOption_t), op);
...
pppMachine_t	*machp;
{
	...
	machp->restart = timeout(ppp_timeout, (caddr_t) machp,
	    (machp->linkp->conf.pppLinkRestartTimerValue*hz)/1000);
}
...
pppMachine_t	*machp;
{
	...
	free_opts(machp->outbound, machp->optsz);
	free_opts(machp->inbound, machp->optsz);
