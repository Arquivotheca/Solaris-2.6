...
#ident	"@(#)sad.c	1.30	94/09/29 SMI"	/* from S5R4 1.20	*/
...
/* ARGSUSED */
static int
sad_info(dev_info_t *dip, ddi_info_cmd_t infocmd, void *arg,
	void **result)
{
	register int error;
	...
	mblk_t *mp;	/* message pointer */
{
	...
	case SAD_SAP:
		...
		default:
			cmn_err(CE_WARN,
			    "apush_iocdata: cp_private bad in SAD_SAP: %d\n",
			    csp->cp_private);
			freemsg(mp);
			break;
		} /* switch (cp_private) */
	...
	case SAD_GAP:
		...
		default:
			cmn_err(CE_WARN,
			    "apush_iocdata: cp_private bad case SAD_GAP: %d\n",
			    csp->cp_private);
			freemsg(mp);
			break;
		} /* switch (cp_private) */
	...
	mblk_t *mp;	/* message pointer */
{
	...
	default:
		cmn_err(CE_WARN, "vml_iocdata: invalid cp_private value: %d\n",
		    csp->cp_private);
		freemsg(mp);
		break;
	} /* switch (cp_private) */
...
int	major;
{
	register struct dev_ops *ops = NULL;
	register int ret;

	if ((long)etoimajor(major) == -1)
		return (EINVAL);
