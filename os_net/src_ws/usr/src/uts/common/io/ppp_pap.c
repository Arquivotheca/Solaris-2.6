...
#pragma ident	"@(#)ppp_pap.c 1.17	94/02/16 SMI"
...
papMachine_t	*machp;
{
	...
	machp -> req_restart = timeout(pap_req_timeout, (caddr_t) machp,
		(machp -> papRestartTimerValue*hz)/1000);
}
...
{
	cancel_wait_timer(machp);
	machp -> req_wait_timer = timeout(req_wait_timeout, (caddr_t) machp,
		(machp -> papWaitTimerValue*hz)/1000);
}
