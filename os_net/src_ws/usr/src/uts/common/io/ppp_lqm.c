#ident "@(#)ppp_lqm.c 1.7 94/01/11 Copyright 1993 SMI"
...
static void lqm_timeout(lqmMachine_t *machp);
...
{
	cancel_lqm_timer(machp);
	machp -> lqm_send = timeout(lqm_timeout, (caddr_t) machp,
		(machp -> linkp-> lqmstruct.out_rep_period*10*hz)/1000);
}
...
mblk_t			*mp;
{
	...
	if (machp -> linkp -> conf.pppLinkLocalMagicNumber &&
	    lqr -> magic_num == machp -> linkp ->
	    conf.pppLinkLocalMagicNumber) {
