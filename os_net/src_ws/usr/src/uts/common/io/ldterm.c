...
#ident	"@(#)ldterm.c	1.75	96/10/17 SMI"	/* SVr4.0 1.36	*/
 ...
 * publication.
 *
 * (c) 1986,1987,1988,1989,1996  Sun Microsystems, Inc (c)
...
ldtermclose(queue_t *q, int cflag, cred_t *crp)
{
	...
	while ((bp = mkiocb(TCSBRK)) == NULL) {
		...
		id = qbufcall(q, sizeof (union ioctypes), BPRI_MED,
				dummy_callback, 0);
		if (!qwait_sig(q)) {
			qunbufcall(q, id);
			goto tidy_up;
	...
	case EUC_WSET:
		...
		setgetfail2:
			...
			bcopy((caddr_t) mp->b_cont->b_rptr,
				(caddr_t) dmp_cont->b_rptr, EUCSIZE);
