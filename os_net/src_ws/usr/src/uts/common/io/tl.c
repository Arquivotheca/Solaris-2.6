...
#pragma ident "@(#)tl.c 1.31	96/10/17 SMI"
...
opdes_t	tl_opt_arr[] = {
	...
	{ SO_TYPE,	SOL_SOCKET, OA_R, OA_R, OP_PASSNEXT, sizeof (long), 0 },
};
...
typedef struct tl_endpt {
	...
	int	te_servtype;		/* ticots, ticotsord or ticlts */
	int	te_mode;		/* te_servtype + name space */
	...
	unsigned long te_qlen;		/* max conn requests */
...
typedef struct tl_icon {
	...
	uint32_t	ti_seqno;	/* Sequence number */
...
static int tl_get_any_addr(int, tl_addr_t *, tl_addr_t *);
...
static tl_icon_t **tl_icon_find(tl_endpt_t *, uint32_t);
static int tl_icon_queuemsg(tl_endpt_t *, uint32_t, mblk_t *);
static int tl_icon_hasprim(tl_endpt_t *, uint32_t, int);
...
static int tl_default_opt(queue_t *, int, int, u_char *);
static int tl_get_opt(queue_t *, int, int, u_char *);
static int tl_set_opt(queue_t *, u_int, int, int,
	u_int, u_char *, u_int *, u_char *);
...
mblk_t *mp;
{
	...
	char *addr_startp;
	int save_state;
	long msz, basize, save_prim_type;
...
mblk_t *mp;
{
	...
	if (((alen > 0) && (((aoff+alen) > msz) || ((aoff+alen) < 0))) ||
	    ((olen > 0) && (((ooff+olen) > msz) || ((ooff+olen) < 0))) ||
	    (olen < 0) || (ooff < 0)) {
		STRLOG(TL_ID, tep->te_minor, 1, SL_TRACE|SL_ERROR,
	...
	if (peer_tep->te_flag & TL_SETCRED) {
		...
		ci->OPT_offset = TL_ALIGN(ci->SRC_offset + ci->SRC_length);
		ci->OPT_length = olen;
		bcopy(((char *)mp1->b_rptr) + ooff,
			((char *)ci) + ci->OPT_offset, olen);
	} else {
		ci->OPT_offset = 0;
		ci->OPT_length = 0;
	}
...
mblk_t *mp;
{
	 ...
	 */
	tipp = tl_icon_find(tep, (uint32_t)cl_ep);
	tip = *tipp;
	if (tip == NULL) {
		STRLOG(TL_ID, tep->te_minor, 2, SL_TRACE|SL_ERROR,
	 ...
	 */
	ASSERT(tip->ti_tep == NULL || cl_ep == tip->ti_tep);
	cl_ep = tip->ti_tep;
...
mblk_t *mp;
{
	...
	if (tep->te_nicon > 0) { /* server */
		 ...
		 */
		tipp = tl_icon_find(tep, (uint32_t)peer_tep);
		tip = *tipp;
		if (tip == NULL) {
			STRLOG(TL_ID, tep->te_minor, 2, SL_TRACE|SL_ERROR,
		 ...
		 * the code below will avoid any action on the client side.
		 */
		ASSERT(tip->ti_tep == NULL || peer_tep == tip->ti_tep);
		peer_tep = tip->ti_tep;
	}
	...
	if (tep->te_nicon > 0) { /* listener */
		...
		*tipp = tip->ti_next;
		 ...
		 */
		tipp = tl_icon_find(peer_tep, (uint32_t)tep);
		tip = *tipp;
		ASSERT(tip != NULL);
		...
		if ((tep->te_mode & TL_SOCKET) && !tl_disable_early_connect) {
			 ...
			 * messages.
			 */
			(void) tl_icon_queuemsg(peer_tep, (uint32_t)tep, dimp);
			peer_tep = NULL;
			dimp = NULL;
			/*
...
mblk_t *mp;
{
	...
	ack_sz = sizeof (struct T_addr_ack);
	ack_sz += TL_ALIGN(tep->te_ap.ta_alen);
	peer_tep = tep->te_conp;
	...
	addr_startp = (char *) &taa[1];

	bcopy(tep->te_ap.ta_abuf, addr_startp,
	    tep->te_ap.ta_alen);

	taa->REMADDR_length = peer_tep->te_ap.ta_alen;
	taa->REMADDR_offset = TL_ALIGN(taa->LOCADDR_offset +
	    taa->LOCADDR_length);
	addr_startp = (char *)ackmp->b_rptr + taa->REMADDR_offset;
...
mblk_t *mp;
{
	...
	if (tep->te_servtype == TL_TICLTS) {
		bcopy(&tl_clts_info_ack, ia, sizeof (tl_clts_info_ack));
		ia->TSDU_size = tl_tidusz; /* TSDU and TIDU size are same */
	} else {
		bcopy(&tl_cots_info_ack, ia, sizeof (tl_cots_info_ack));
...
mblk_t *mp;
{
	...
	case TS_DATA_XFER:
		...
		(void) tl_icon_queuemsg(tep->te_oconp, (uint32_t)tep, mp);
		return (0);
...
mblk_t *mp;
{
	...
	struct T_unitdata_req *udreq;
	long msz, alen, aoff, olen, ooff, ui_sz;
	...
	if (msz >= ui_sz && alen >= tep->te_ap.ta_alen &&
		...
		addr_startp = (char *)mp->b_rptr + udind->SRC_offset;
		bcopy(tep->te_ap.ta_abuf, addr_startp, tep->te_ap.ta_alen);
	} else {
		...
		if (peer_tep->te_flag & TL_SETCRED) {
			...
			bcopy((char *)udreq + ooff,
				(char *)udind + udind->OPT_offset,
				olen);
		}
	...
	(void) putnext(peer_tep->te_rq, mp);
	return (0);
}
...
tl_addr_t *req;
{
	...
	char *abuf;
	int alen;
	unsigned int loopcnt;	/* Limit loop to 2^32 */
	...
	abuf = (char *) kmem_zalloc(alen, KM_NOSLEEP);
	if (abuf == NULL)
	...
	if (req != NULL) {
		...
		bcopy(req->ta_abuf, myaddr.ta_abuf, req->ta_alen);
	}

	loopcnt = 0;
	for (;;) {
		bcopy(def_tap->ta_abuf, myaddr.ta_abuf, def_tap->ta_alen);
	...
	(void) kmem_free(abuf, alen);

	STRLOG(TL_ID, -1, 1, SL_ERROR,
...
tl_endpt_t *tep;
{
	...
	if (tep->te_nicon > 0) {
		...
		if ((tep->te_mode & TL_SOCKET) && !tl_disable_early_connect &&
		    ...
		    !tl_icon_hasprim(srv_tep, (uint32_t)tep, T_ORDREL_IND)) {
			/*
		 ...
		 * a t_connind_t for the client.
		 */
		tipp = tl_icon_find(srv_tep, (uint32_t)tep);
		tip = *tipp;
		ASSERT(tip != NULL);
		ASSERT(tip->ti_tep != NULL);
		ASSERT(tep == tip->ti_tep);
		if ((tep->te_mode & TL_SOCKET) && !tl_disable_early_connect) {
			 ...
			 * is going away.
			 */
			(void) tl_icon_queuemsg(srv_tep, (uint32_t)tep, d_mp);
			d_mp = NULL;
		...
		if (d_mp != NULL) {
			STRLOG(TL_ID, tep->te_minor, 3, SL_TRACE,
				...
				srv_tep->te_state);
			ASSERT(*(int *)(d_mp->b_rptr) == T_DISCON_IND);
 ...
 */
static tl_icon_t **
tl_icon_find(tl_endpt_t *tep, uint32_t seqno)
{
	tl_icon_t **tipp, *tip;
 ...
 */
static int
tl_icon_queuemsg(tl_endpt_t *tep, uint32_t seqno, mblk_t *nmp)
{
	tl_icon_t		*tip;
	mblk_t			**mpp, *mp;
	int			prim, nprim;
 ...
 */
static int
tl_icon_hasprim(tl_endpt_t *tep, uint32_t seqno, int prim)
{
	tl_icon_t		*tip;
	mblk_t			*mp;
	int			prim2;
...
/* ARGSUSED */
static int
tl_default_opt(wq, level, name, ptr)
queue_t *wq;
int level;
int name;
u_char *ptr;
...
/* ARGSUSED */
static int
tl_set_opt(wq, mgmt_flags, level, name, inlen, invalp, outlenp, outvalp)
queue_t *wq;
u_int	mgmt_flags;
int	level, name;
u_int	inlen;
