...
#pragma ident	"@(#)nfs_clnt.h	1.51	96/06/22 SMI"
/* SVr4.0 1.8	*/
...
#define	mi_knetconfig	mi_curr_serv->sv_knconf
#define	mi_addr		mi_curr_serv->sv_addr
#define	mi_hostname	mi_curr_serv->sv_hostname
#define	mi_authflavor	mi_curr_serv->sv_secdata->nfsflavor
#define	mi_secdata	mi_curr_serv->sv_secdata
...
typedef struct mntinfo {
	 ...
	 */
	u_long		mi_noresponse;	/* server not responding count */
	u_long		mi_failover; 	/* failover to new server count */
	u_long		mi_remap;	/* remap to new server count */
} mntinfo_t;
 ...
 */
struct nfs_owner {
	long	magic;		/* magic uniquifying number */
