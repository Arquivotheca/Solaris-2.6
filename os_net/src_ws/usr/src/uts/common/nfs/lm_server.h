 ...
 *  publication.
 *
 *	Copyright (c) 1986-1991,1994,1996 Sun Microsystems, Inc.
...
#pragma ident	"@(#)lm_server.h	1.18	96/06/25 SMI"
 ...
 * lm_lck protects the fields `fh', `vp', `count', and the `blocked'
 * list.
...
extern int lm_shr_sysid_has_locks(sysid_t);
...
struct lm_nlm_disp {
	void (*proc)(void *gen_args, void *gen_res, 	/* proc to call */
		struct lm_nlm_disp *disp, struct lm_sysid *ls, u_int xid);
	unsigned char do_disp_reply;
	unsigned char do_block_reply;
	unsigned char callback_reply;
...
enum lm_disp_reply { LM_REPLY, LM_DONT, LM_CALLBACK, LM_SENDREPLY};
