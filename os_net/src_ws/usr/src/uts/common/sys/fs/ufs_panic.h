...
#pragma ident	"@(#)ufs_panic.h	1.6	94/11/09 SMI"
...
typedef struct vfs_ufsfx {
	long		 fx_count;		/* # of failures in this fs */
	time_t		 fx_prev_start_tm;	/* when prev. event started */
	long		 fx_flags;		/* see ufs_panic.h for the */
	long		 fx_too_soon;		/* following limits and flags */
	long		 fx_too_many;		/* following limits and flags */
...
int	ufsfx_mount(struct ufsvfs *, int, long);
