...
#pragma ident	"@(#)cachefs_fs.h	1.100	96/10/22 SMI"
...
#ifdef DEBUG
#define	CFS_MUTEX_DEBUG
#endif

#ifdef CFS_MUTEX_DEBUG
/* n.b.: CFSMUTEX2KMUTEX() is a no-op because cm_mutex is at the front */
#define	CFSMUTEX2KMUTEX(x)	(x)
struct cachefs_mutex {
#define	DEBUG_STACK_DEPTH	15
	kmutex_t	cm_mutex;
	u_int		cm_stack[DEBUG_STACK_DEPTH];
};

typedef struct cachefs_mutex cachefs_mutex_t;

#else
#define	CFSMUTEX2KMUTEX(x)	(x)
#define	cachefs_mutex_t	kmutex_t
...
struct cachefs_workq {
	...
	cachefs_mutex_t		wq_queue_lock;		/* protect queue */
...
struct cachefs_rl_listhead {
	u_int		rli_front;		/* front of list */
	u_int		rli_back;		/* back of list */
	long		rli_blkcnt;		/* number of 8k blocks */
	long		rli_itemcnt;		/* number of items on list */
};
...
struct cachefs_rl_info {
	u_int	rl_entries;		/* number of entries allocated in rl */
	time_t	rl_gctime;		/* time of item on front of gc list */
...
typedef struct rl_debug {
	...
	int db_stackheight;
	u_int db_stack[CACHEFS_RLDB_STACKSIZE];
...
extern cachefs_mutex_t cachefs_rl_debug_mutex;
#endif /* CFSRLDEBUG */
...
typedef struct rl_entry {
	u_int rl_attrc: 1;
	u_int rl_fsck: 1; /* used by fsck; true => rl_current is correct */
	u_int rl_local: 1; /* 1 means a local file */

#ifdef CFSRLDEBUG
	time_t rl_dbvalid; /* this == cachefs_dbvalid => trust rl_debug */
	rl_debug_t *rl_debug;
#endif /* CFSRLDEBUG */
...
#define	CACHEFS_RLPMBS	(MAXBSIZE / sizeof (rl_entry_t))
...
struct cachefscache {
	...
	u_int			c_rl_window;	/* window mapped in */
	...
	cachefs_mutex_t		c_contentslock; /* protect cache struct */
	cachefs_mutex_t		c_fslistlock;	/* protect fscache list */
	cachefs_mutex_t		c_mflock;	/* protect modified fixes */
	...
	u_int			c_apop_inqueue;	/* # async pops queued */
...
struct cachefs_metadata {
	...
	u_int			md_resettimes;	/* when to reset local times */
	ino64_t			md_localfileno;	/* persistent local inum */
	u_int			md_resetfileno;	/* when to reset local fileno */
	u_long			md_seq;		/* seq number for putpage */
...
struct attrcache_header {
	...
	u_int		ach_rlno;		/* rl entry for this file */
...
struct cnode {
	...
	int			c_error;
	cachefs_mutex_t		c_statelock;	/* statelock */
...
struct cachefs_req {
	...
	cachefs_mutex_t cfs_req_lock;	/* Protects contents */
};
...
void cachefs_update_allocmap(cnode_t *, u_offset_t, u_int);
...
/*
void cachefs_cluster_allocmap(struct cnode *, u_offset_t, u_offset_t *, u_int *,
int);
*/
void cachefs_cluster_allocmap(u_offset_t, u_offset_t *, u_int *, int,
		struct cnode *);
int cachefs_populate(cnode_t *, u_offset_t, int, vnode_t *, vnode_t *,
	u_offset_t, cred_t *);
...
ulong_t cachefs_cred_checksum(cred_t *cr);
...
void cachefs_rlent_moveto_nolock(cachefscache_t *cachep,
    enum cachefs_rl_type type, u_int entno, u_long);
void cachefs_rlent_moveto(cachefscache_t *, enum cachefs_rl_type, u_int,
    u_long);
void cachefs_rlent_verify(cachefscache_t *, enum cachefs_rl_type, u_int);
void cachefs_rl_changefileno(cachefscache_t *cachep, u_int entno,
	ino64_t fileno);
int cachefs_rlent_data(cachefscache_t *cachep, rl_entry_t *valp, u_int *entnop);
void cachefs_move_modified_to_mf(cachefscache_t *cachep, fscache_t *fscp);
int cachefs_allocblocks(cachefscache_t *, int, enum cachefs_rl_type);
void cachefs_freeblocks(cachefscache_t *, int, enum cachefs_rl_type);
...
#ifdef CFS_MUTEX_DEBUG
void cachefs_mutex_enter(cachefs_mutex_t *);
void cachefs_mutex_exit(cachefs_mutex_t *);
void cachefs_mutex_init(cachefs_mutex_t *, char *, kmutex_type_t, void *);
void cachefs_mutex_destroy(cachefs_mutex_t *);
int cachefs_cv_wait_sig(kcondvar_t *, cachefs_mutex_t *);
int cachefs_cv_timedwait(kcondvar_t *, cachefs_mutex_t *, long);
#define	CACHEFS_MUTEX_HELD(a)	MUTEX_HELD(&(a)->cm_mutex)
#else
#define	cachefs_mutex_enter	mutex_enter
#define	cachefs_mutex_exit	mutex_exit
#define	cachefs_mutex_init	mutex_init
#define	cachefs_mutex_destroy	mutex_destroy
#define	cachefs_cv_wait_sig	cv_wait_sig
#define	cachefs_cv_timedwait	cv_timedwait
#define	CACHEFS_MUTEX_HELD	MUTEX_HELD
#endif	/* CFS_MUTEX_DEBUG */
