...
#pragma ident	"@(#)cachefs_dlog.h	1.14	96/04/19 SMI"
 ...
 * Put in a long at the start of the file.
...
struct cfs_dlog_tm {
	timestruc_t	tm_mtime;	/* cached mtime on file */
	timestruc_t	tm_ctime;	/* cached ctime on file */
};
typedef struct cfs_dlog_tm cfs_dlog_tm_t;

/* structure populated for setattr */
struct cfs_dlog_setattr {
	vattr_t		dl_attrs;	/* attrs to set file to */
...
struct cfs_dlog_setsecattr {
	...
	cfs_dlog_tm_t	dl_times;	/* ctime and mtime on file */
	u_long		dl_mask;	/* mask field in vsecattr_t */
...
struct cfs_dlog_mkdir {
	...
	vattr_t		dl_attrs;	/* attrs to insert with */
	fid_t		dl_fid;		/* blank fid */
	cred_t		dl_cred;	/* credentials to use */
	char		dl_buffer[CFS_DLOG_BUFSIZE + MAXNAMELEN];
};
...
#define	CFS_DLOG_ENTRY_MAXSIZE	(sizeof (struct cfs_dlog_setsecattr) + \
...
u_long cachefs_dlog_seqnext(fscache_t *fscp);
#endif
