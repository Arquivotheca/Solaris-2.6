...
#pragma ident	"@(#)autofs.h	1.15	96/04/11 SMI"
...
struct fnnode {
	...
	int		fn_flags;
	u_long		fn_size;		/* size of directory */
	...
	long		fn_ref_time;		/* time last referenced */
	long		fn_unmount_ref_time;	/* last time unmount was done */
	kcondvar_t	fn_cv_mount;		/* mount blocking variable */
};
typedef struct fnnode fnnode_t;
...
extern struct mod_ops mod_fsops;	/* Module linkage info for kernel */

/*
 * Utility routines
 */
extern int auto_inkernel_mount(struct mounta *, vnode_t *);
...
extern void auto_log(int level, char *fmt, ...);
extern void auto_dprint(int level, char *fmt, ...);
extern int auto_calldaemon(fninfo_t *, int, xdrproc_t, char *,
	xdrproc_t, char *, cred_t *, bool_t);
