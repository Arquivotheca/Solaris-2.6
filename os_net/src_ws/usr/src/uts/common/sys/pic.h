...
#pragma ident	"@(#)pic.h	1.11	96/09/12 SMI"
...
#if defined(__ppc)
#define	PIC_VECTBASE    0x0		/* Vectors for external interrupts */
					/* start at 0 for PowerPC. */
#endif	/* defined(__ppc) */
...
struct standard_pic {
	short c_npic;
	u_char c_curmask[NPIC];
	u_char c_iplmask[MAXIPL*NPIC];
};
