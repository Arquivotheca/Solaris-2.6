...
#pragma ident	"@(#)commands.h	1.26	96/09/05 SMI"
...
union scsi_cdb {		/* scsi command description block */
	struct {
		...
		u_char lun	:3;	/* lun (byte 1) (reserved in SCSI-3) */
...
#define		low_count	cdb_un.sg.g0.count0
		struct scsi_g0 {
			...
			u_char naca	:1; /* normal ACA  		*/
			u_char rsvd	:3; /* reserved 		*/
			u_char vu_56	:1; /* vendor unique (byte 5 bit6) */
			u_char vu_57	:1; /* vendor unique (byte 5 bit7) */
#elif defined(_BIT_FIELDS_HTOL)
...
#define		g1_link		cdb_un.sg.g1.link
		struct scsi_g1 {
			...
			u_char vu_96	:1; /* vendor unique (byte 9 bit6) */
			u_char vu_97	:1; /* vendor unique (byte 9 bit7) */
#elif defined(_BIT_FIELDS_HTOL)
...
#define		g4_link			cdb_un.sg.g4.link
		struct scsi_g4 {
			...
			u_char addr1;
			u_char addr0;
			u_char addtl_cdb_data3;
			u_char addtl_cdb_data2;
			u_char addtl_cdb_data1;
			u_char addtl_cdb_data0;
			u_char count3;	/* transfer length (msb) */
			u_char count2;
			u_char count1;
			u_char count0;	/* transfer length (lsb) */
			...
			u_char naca	:1; /* normal ACA		*/
			u_char rsvd0	:3; /* reserved 		*/
			u_char vu_156	:1; /* vendor unique (byte 15 bit6) */
			u_char vu_157	:1; /* vendor unique (byte 15 bit7) */
#elif defined(_BIT_FIELDS_HTOL)
			u_char vu_157	:1; /* vendor unique (byte 15 bit 7) */
			u_char vu_156	:1; /* vendor unique (byte 15 bit 6) */
			u_char rsvd0	:3; /* reserved */
			u_char naca	:1; /* normal ACA		*/
			u_char flag	:1; /* interrupt when done */
			u_char link	:1; /* another command follows */
...
#define		scc5_link	cdb_un.sg.g5.link
		struct scsi_g5 {
			...
			u_char naca	:1; /* normal ACA		*/
			u_char rsvd0	:3; /* reserved 		*/
			u_char vu_116	:1; /* vendor unique (byte 11 bit6) */
			u_char vu_117	:1; /* vendor unique (byte 11 bit7) */
#elif defined(_BIT_FIELDS_HTOL)
...
#error	One of _BIT_FIELDS_LTOH or _BIT_FIELDS_HTOL must be defined
	...
	u_char cdb_opaque[SCSI_CDB_SIZE]; /* addressed as opaque char array */
	u_long cdb_long[SCSI_CDB_SIZE / sizeof (long)]; /* longword array */
};
 ...
 *
 * Work only with pre-SCSI-3 because the put lun as part of CDB.
...
extern void 	makecom_g5(struct scsi_pkt *pkt, struct scsi_device *devp,
				int flag, int cmd, int addr, int cnt);
extern int	scsi_setup_cdb(union scsi_cdb *cdbp, u_char cmd, u_int addr,
				u_int cnt, u_int addtl_cdb_data);
