...
#pragma ident	"@(#)mode.h	1.16	96/06/07 SMI"
...
#if defined(_BIT_FIELDS_LTOH)
	u_char		dtdc	: 3,	/* data transfer disconenct control */
...
#elif defined(_BIT_FIELDS_HTOL)
	u_char		emdp	: 1,	/* enable modify data pointers */
			fard	: 1,	/* fair for read */
			fawrt	: 1,	/* fair for write */
			fastat	: 1,	/* fair for status */
			dimm	: 1,	/* disconnect immediate */
...
#endif	/* _BIT_FIELDS_LTOH */
	u_char	reserved;
	u_short	first_burst_sz;		/* first burst size */
};
...
struct mode_control_scsi3 {
	...
	u_char		eanp	: 1,	/* Enable AEN permission */
	...
	u_char		eeca	: 1,	/* enable extended contingent */
					/* allegiance (only pre-SCSI-3) */
			rac	: 1,	/* report a check */
			byprtm	: 1,	/* By port message */
			bybths	: 1,	/* By both RESET signal */
...
#endif	/* _BIT_FIELDS_LTOH */
	u_char	reserved;
	u_short	ready_aen_holdoff;	/* Ready AEN holdoff period */
	u_short	busy_timeout;		/* Busy timeout period */
	u_char	reserved_2[2];
};
