...
#pragma ident	"@(#)stream.h	1.67	96/09/27 SMI"	/* SVr4.0 11.44	*/
 ...
 *
 * db_ref and db_free are protected by db_lock if the message is ever dup'ed.
 * The other fields are set only at allocation time, when the allocator has
 * exclusive access.
 */
...
#define	DBLK_UNSAFE	0x10		/* XXX allocated by unsafe driver */
 ...
 */
extern mblk_t *allocb(size_t, uint);
extern mblk_t *esballoc(unsigned char *, size_t, uint, frtn_t *);
extern mblk_t *mkiocb(uint cmd);
extern int testb(size_t, uint);
extern int bufcall(size_t, uint, void (*)(), intptr_t);
extern int esbbcall(uint, void (*)(), intptr_t);
...
extern int qtimeout(queue_t *, void (*)(), caddr_t, clock_t);
extern int qbufcall(queue_t *, size_t, uint, void (*)(), intptr_t);
extern int quntimeout(queue_t *, int);
extern void qunbufcall(queue_t *, int);
