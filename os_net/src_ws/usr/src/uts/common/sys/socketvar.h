...
#pragma ident	"@(#)socketvar.h	1.25	96/09/13 SMI"	/* SVr4.0 1.3 */
...
/*
 * Allow for modifications to the TPI types using the following
 * derived types.
 * Needs to change if/when TPI changes from using "long" everywhere.
 */
typedef	long	tpi_prim_t;		/* Type for *_prim */
typedef long	tpi_length_t;		/* Type for *_length */
typedef long	tpi_offset_t;		/* Type for *_offset */
typedef long	tpi_info_t;		/* Type for values in T_info_ack */
typedef	long	tpi_misc_t;		/* Misc TPI fields */

/*
 * Alignment check for the start of TPI messages.
 */
#define	ALIGN_TPI_size		(sizeof (long))
#define	ALIGN_TPI_opthdr_size	(sizeof (long))
#define	ISALIGNED_tpi_prim_t(addr) \
		(((u_long)(addr) & (sizeof (tpi_prim_t) - 1)) == 0)
#define	ISALIGNED_tpi_opthdr(addr) \
		(((u_long)(addr) & (ALIGN_TPI_opthdr_size - 1)) == 0)

/*
 * Roundup rule for TPI opthdrs.
 * Needs to change if opthdr ever has different alignment than long.
 */
...
struct soaddr {
	...
	int		soa_len;	/* Length in bytes for kmem_free */
	int		soa_maxlen;	/* Allocated length */
};
 ...
 * provider.
 *
 * The locking of sockfs uses the so_lock mutex plus the SLOCKED
 * and SREADLOCKED flags in so_flag. The mutex protects all the state
 * in the sonode. The SLOCKED flag is used to single-thread operations from
 * sockfs users to prevent e.g. multiple bind() calls to operate on the
 * same sonode concurrently. The SREADLOCKED flag is used to ensure that
...
struct sonode {
	...
	kcondvar_t	so_want_cv;	/* wait due to SLOCKED */

	/* These fields are protected by so_lock */
	u_int	so_state;		/* internal state flags SS_*, below */
	u_int	so_mode;		/* characteristics on socket. SM_* */
	...
	ushort	so_flag;		/* flags, see below */
	...
	u_short	so_delayed_error;	/* From T_uderror_ind */
	...
	u_int	so_oobsigcnt;		/* Number of SIGURG generated */
	u_int	so_oobcnt;		/* Number of T_EXDATA_IND queued */
	pid_t	so_pgrp;		/* pgrp for signals */

	/* From T_info_ack */
	tpi_info_t	so_tsdu_size;
	tpi_info_t	so_etsdu_size;
	tpi_info_t	so_addr_size;
	tpi_info_t	so_opt_size;
	tpi_info_t	so_tidu_size;
	tpi_info_t	so_serv_type;
...
/* flags */
#define	SMOD		0x01		/* update socket modification time */
#define	SACC		0x02		/* update socket access time */

#define	SLOCKED		0x10		/* use to serialize open/closes */
#define	SREADLOCKED	0x20		/* serialize kstrgetmsg calls */
#define	SWANT		0x40		/* some process waiting on lock */
#define	SCLONE		0x80		/* child of clone driver */
...
#ifdef DEBUG
char	*pr_state(u_int, u_int);
char	*pr_addr(int, struct sockaddr *, int);
...
{ if (sockprinterr && ((so)->so_options & SO_DEBUG))		\
	printf("socket(0x%x) error %d: line %d file %s\n",	\
...
u_char		*sogetoff(mblk_t *, int, int, int);
void		so_getopt_srcaddr(char *, tpi_length_t,
			char **, tpi_length_t *);
int		so_getopt_unix_close(char *, tpi_length_t);
int		so_addr_verify(struct sonode *, struct sockaddr *, int);
int		so_ux_addr_xlate(struct sonode *, struct sockaddr *, int, int,
			char **, tpi_length_t *);
void		fdbuf_free(struct fdbuf *);
mblk_t *	fdbuf_allocmsg(int, struct fdbuf *);
int		fdbuf_create(char *, int, struct fdbuf **);
void		so_closefds(char *, int, int, int);
int		so_getfdopt(char *, int, int, char **, int *);
int		so_optlen(char *, int, int);
void		so_cmsg2opt(char *, int, int, mblk_t *);
int		so_cmsglen(mblk_t *, char *, int, int);
int		so_opt2cmsg(mblk_t *, char *, int, int, char *, int);
...
mblk_t		*soallocproto(int, int);
mblk_t		*soallocproto1(char *, int, int, int);
void		soappendmsg(mblk_t *, char *, int);
mblk_t		*soallocproto2(char *, int, char *, int, int, int);
mblk_t		*soallocproto3(char *, int, char *, int,
			char *, int, int, int);

int		sowaitprim(struct sonode *, tpi_prim_t, tpi_prim_t,
			int, mblk_t **);
int		sowaitokack(struct sonode *, tpi_prim_t);
int		sowaitack(struct sonode *, mblk_t **);
void		soqueueack(struct sonode *, mblk_t *);
int		sowaitconnind(struct sonode *, int, mblk_t **);
void		soqueueconnind(struct sonode *, mblk_t *);
int		soflushconnind(struct sonode *, tpi_misc_t);
int		sowaitconnected(struct sonode *, int, int);

int		sosend_dgram(struct sonode *, struct sockaddr *,
			int, struct uio *, int);
int		sosend_svc(struct sonode *, struct uio *, int, int, int);

int		so_strinit(struct sonode *, struct sonode *);
int		sobind(struct sonode *, struct sockaddr *, int, int, int);
...
int		soconnect(struct sonode *, struct sockaddr *, int, int, int);
int		sodisconnect(struct sonode *, int, int);
...
int		sogetsockopt(struct sonode *, int, int, void *, int *);
int		sosetsockopt(struct sonode *, int, int, void *, int);
