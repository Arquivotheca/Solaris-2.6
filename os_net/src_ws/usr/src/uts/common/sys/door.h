...
#pragma ident	"@(#)door.h	1.11	96/07/23 SMI"
...
typedef struct door_arg {
	...
	size_t		desc_num;	/* Argument/result descriptor size */
...
typedef struct door_node {
	...
	u_int		door_active;	/* Number of active invocations */
...
int	door_call(int, struct door_arg *);
int	door_return(caddr_t, u_int, door_desc_t *, u_int, caddr_t);
