...
#pragma ident	"@(#)pcic_var.h	1.22	96/09/04 SMI"
...
typedef struct pcicdev_t {
	...
	u_int			pc_type;
	char			*pc_chipname;
	u_int			pc_irqs;	/* the possible IRQ levels */
	u_int			pc_smi;		/* SMI IRQ */
	u_int			pc_irq;		/* IO IRQ */
	int			pc_io_type;
	...
	kmutex_t		pc_intr;	/* interrupt list lock */
	int			pc_numsockets;
	...
	int			pc_pmtimer;	/* timeout for simulating PM */
	...
	int			pc_lastreg;
	uint_t			pc_base;	/* first possible mem-addr */
...
#define	PCIC_PCI1130_DEVID	0x1130
#define	PCIC_PCI1050_DEVID	0x1050
