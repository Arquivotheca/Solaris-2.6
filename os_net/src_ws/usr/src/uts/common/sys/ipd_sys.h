...
#pragma ident	"@(#)ipd_sys.h	1.7	94/01/04 SMI"
...
struct ipd_str {
	...
	u_long			st_state;	/* DLPI state */
	u_long			st_sap;		/* sap attached */
	u_long			st_minor;	/* corresponding minor */
...
struct ipd_softc {
	...
	ulong_t			if_ipackets;	/* packets in */
	ulong_t			if_ierrors;	/* errors in */
	ulong_t			if_opackets;	/* packets out */
	ulong_t			if_oerrors;	/* errors out */
	ulong_t			if_nocanput;	/* canput failed */
	ulong_t			if_allocbfail;	/* allocb failed */
...
struct  ipdpriminfo {
	...
	u_long	pi_state;		/* acceptable starting state */
