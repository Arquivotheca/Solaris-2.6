...
#pragma ident	"@(#)optcom.c	1.20	96/10/13 SMI"
 ...
 * Function prototypes
 */
static int process_topthdrs_first_pass(mblk_t *mp, int priv,
    optdb_obj_t *dbobjp, boolean_t *pass_to_nextp, int *toa_lenp);
static int do_options_second_pass(queue_t *q, mblk_t *reqmp,
    mblk_t *ackmp, int priv, optdb_obj_t *dbobjp, int *worst_statusp);
static int get_worst_status(int status, int current_worst_status);
static int do_opt_default(queue_t *q, struct T_opthdr *opt, u_char **optrp,
    int *worst_statusp, int priv, optdb_obj_t *dbobjp);
static void do_opt_current(queue_t *q, struct T_opthdr *opt, u_char **optrp,
    int *worst_statusp, int priv, optdb_obj_t *dbobjp);
static void do_opt_check_or_negotiate(queue_t *q, struct T_opthdr *opt,
    int mgmt_flags, u_char **optrp, int *worst_statusp, int priv,
    optdb_obj_t *dbobjp);
static opdes_t *opt_chk_lookup(u_int level, u_int name, opdes_t *opt_arr,
	u_int opt_arr_cnt);
static int
opt_level_allopts_lengths(u_int level, opdes_t *opt_arr, u_int opt_arr_cnt);
u_int optcom_max_optbuf_len(opdes_t *opt_arr, u_int opt_arr_cnt);
static boolean_t opt_length_ok(opdes_t *optd, struct T_opthdr *opt);
 ...
 */
void
svr4_optcom_req(q, mp, priv, dbobjp)
	queue_t	* q;
	mblk_t	* mp;
	int	priv;
	optdb_obj_t *dbobjp;
...
no_mem:;
		...
		for (optd = opt_arr; optd < &opt_arr[opt_arr_cnt]; optd++) {
			...
			if (!(optd->opdes_props & OP_DEF_FN) ||
			    ((opt->len = (*deffn)(q, opt->level, opt->name,
				(u_char *)&opt[1])) < 0)) {
				/*
				...
				case sizeof (int64_t):
					*(int64_t *)&opt[1] = (int64_t)
					    optd->opdes_default;
					break;
				case sizeof (long):
					*(long *)&opt[1] =
					    (long) optd->opdes_default;
					break;
				case sizeof (short):
					*(short *)&opt[1] =
					    (short) optd->opdes_default;
					break;
				case sizeof (char):
					*(char *)&opt[1] =
					    (char) optd->opdes_default;
					break;
				default:
					/*
					 ...
					 * fixed - use bcopy
					 */
					bcopy((caddr_t)optd->opdes_defbuf,
					    (caddr_t) &opt[1],
					    optd->opdes_size);
					break;
				}
	...
	for (opt = opt_start; opt < opt_end; opt = next_opt) {
		...
		case T_NEGOTIATE:
			 ...
			 * XXX A "bad option" return for even fixed length
			 * options, while compatible with earlier release is not
			 * compatible with expectations of errno code expected
			 * in Socket Posix draft standard and SunOS 4.X
			 * behavior for bad lengths.
			 */
 ...
 */
void
tpi_optcom_req(q, mp, priv, dbobjp)
	queue_t	* q;
	mblk_t	* mp;
	int	priv;
	optdb_obj_t *dbobjp;
...
bad_flag:;
	 ...
	 */
	toa = (struct T_optmgmt_ack *)ALIGN32(toa_mp->b_rptr);
	toa->OPT_length = toa_mp->b_wptr - (toa_mp->b_rptr +
...
static int
process_topthdrs_first_pass(mp, priv, dbobjp, pass_to_nextp, toa_lenp)
	mblk_t *mp;
	int priv;
	optdb_obj_t *dbobjp;
	boolean_t *pass_to_nextp;
	int *toa_lenp;
{
	opdes_t	*opt_arr = dbobjp->odb_opt_des_arr;
	u_int opt_arr_cnt = dbobjp->odb_opt_arr_cnt;
	...
	for (opt = opt_start; opt < opt_end; opt = next_opt) {
		...
		if (opt->name != T_ALLOPT) {
			...
			if (optd == NULL) {
				 ...
				 * Option not found
				 *
				 * TPI provider always initializes the "not
				 * supported" (or whatever) status for the
				 * options. Other levels leave status unchanged
				 * if they do not understand an option.
				 */
				if (topmost_tpiprovider)
 ...
 */
static int
do_options_second_pass(q, reqmp, ackmp, priv, dbobjp, worst_statusp)
	queue_t *q;
	mblk_t *reqmp;
	mblk_t *ackmp;
	int priv;
	...
	int *worst_statusp;
{
	...
	u_char *optr;
	struct T_optmgmt_req * tor =
	    (struct T_optmgmt_req *)ALIGN32(reqmp->b_rptr);
...
bad_opt:;
	...
	for (opt = opt_start; opt < opt_end; opt = next_opt) {
		...
		if (failed_option) {
			...
			bcopy((caddr_t) opt, (caddr_t) optr, opt->len);
		...
		case T_DEFAULT:
			...
			if (do_opt_default(q, opt, &optr, worst_statusp,
				...
				if (topmost_tpiprovider)
					opt->status = T_FAILURE;
				bcopy((caddr_t) opt, (caddr_t) optr,
				    opt->len);
				optr += ROUNDUP_TPIOPT(opt->len);
				*worst_statusp = get_worst_status(opt->status,
				    *worst_statusp);
...
static int
get_worst_status(status, current_worst_status)
int status, current_worst_status;
{
	/*
...
static int
do_opt_default(q, reqopt, resptrp, worst_statusp, priv, dbobjp)
	queue_t *q;
	struct T_opthdr *reqopt;
	u_char **resptrp;
	int *worst_statusp;
	int priv;
	optdb_obj_t *dbobjp;
...
static void
do_opt_current(q, reqopt, resptrp, worst_statusp, priv, dbobjp)
	queue_t *q;
	struct T_opthdr *reqopt;
	u_char **resptrp;
	int *worst_statusp;
	...
	optdb_obj_t *dbobjp;
{
	...
	if (reqopt->name != T_ALLOPT) {
		...
		*resptrp += sizeof (struct T_opthdr);
		...
		if (optlen >= 0) {
			topth->len = optlen + sizeof (struct T_opthdr);
			topth->level = reqopt->level;
			topth->name = reqopt->name;
			topth->status = reqopt->status;
	...
	if (*resptrp == initptr) {
		...
		bcopy((caddr_t) reqopt, (caddr_t) *resptrp, reqopt->len);
...
static void
do_opt_check_or_negotiate(q, reqopt, mgmt_flags, resptrp, worst_statusp, priv,
			dbobjp)
	queue_t *q;
	struct T_opthdr *reqopt;
	int mgmt_flags;
...
bad_opt:;
	 ...
	 */
	*opt_lenp = optr - init_optr;
	if (*opt_lenp > 0) {
		bcopy(init_optr, (void *) opt_start, *opt_lenp);
	}
	freeb(copy_mp_head);
	return (OB_SUCCESS);
...
static opdes_t *
opt_chk_lookup(level, name, opt_arr, opt_arr_cnt)
	u_int level;
	u_int name;
	opdes_t	*opt_arr;
	u_int opt_arr_cnt;
...
static int
opt_level_allopts_lengths(level, opt_arr, opt_arr_cnt)
	u_int level;
	opdes_t	*opt_arr;
	u_int opt_arr_cnt;
{
	opdes_t		*optd;
	int allopt_len = 0;	/* 0 implies no option at this level */
...
u_int
optcom_max_optbuf_len(opt_arr, opt_arr_cnt)
	opdes_t	*opt_arr;
	u_int opt_arr_cnt;
{
	u_int max_optbuf_len = sizeof (struct T_info_ack);
	opdes_t		*optd;

	for (optd = opt_arr; optd < &opt_arr[opt_arr_cnt]; optd++) {
		max_optbuf_len += sizeof (struct T_opthdr) +
		    (u_int)ROUNDUP_TPIOPT(optd->opdes_size);
	}
