#ifndef lint
#pragma ident "@(#) swi_module.c 1.1 95/02/13"
#endif
/*
...
get_machinetype(void)
{
	...
	enter_swlib("get_machinetype");
	m = swi_get_machinetype();
	exit_swlib();
	return (m);
}
...
{
	enter_swlib("set_machinetype");
	swi_set_machinetype(type);
	exit_swlib();
	return;
}
...
{
	enter_swlib("set_rootdir");
	swi_set_rootdir(newrootdir);
	exit_swlib();
	return;
}
...
int
mark_module_action(Module * modp, Action action)
{
	int i;

	enter_swlib("mark_module_action");
	i = swi_mark_module_action(modp, action);
	exit_swlib();
	return (i);
}
