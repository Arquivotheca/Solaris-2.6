#ifndef lint
#pragma ident "@(#) platform.c 1.8 95/07/19"
#endif
/*
...
load_platforms(Module * prod)
{
	...
	strcpy(buf, prod->info.prod->p_pkgdir);
	strcat(buf, "/.platform");
	dirp = opendir(buf);
	if (dirp == (DIR *)0)
		return (ERR_NODIR);
...
load_installed_platforms(Module * prod)
{
	...
	strcpy(buf, prod->info.prod->p_rootdir);
	strcat(buf, "/var/sadm/system/admin");
...
swi_write_platform_file(char *rootdir, Module *prod)
{
	...
	if (!prod->info.prod->p_platgrp)
		return (SUCCESS);
	sprintf(path, "%s/var/sadm/system/admin/.platform", rootdir);
	for (ap = prod->info.prod->p_arches; ap; ap = ap->a_next) {
		...
		for (pgp = prod->info.prod->p_platgrp; pgp; pgp = pgp->next) {
			...
			if (!streqn(pgp->pltgrp_isa, ap->a_arch, len) ||
			...
			fprintf(fp, "PLATFORM_GROUP=%s\n", pgp->pltgrp_name);
			fprintf(fp, "INST_ARCH=%s\n", pgp->pltgrp_isa);
			...
			for (pp = pgp->pltgrp_members; pp; pp = pp->next) {
				fprintf(fp, "PLATFORM_NAME=%s\n",
				    pp->plat_name);
				if (pp->plat_uname_id)
					fprintf(fp, "PLATFORM_ID=%s\n",
					    pp->plat_uname_id);
				if (pp->plat_machine)
					fprintf(fp, "MACHINE_TYPE=%s\n",
					    pp->plat_machine);
				fprintf(fp, "IN_PLATFORM_GROUP=%s\n",
				    pgp->pltgrp_name);
			}
		}
	}
    ...
    Product *prod2)
{
	...
	for (pgp = merge_pltgrp; pgp; pgp = pgp->next) {
		...
		if (!started) {
			scriptwrite(fp, start_platform,
			    "ROOT", rootdir, (char *)0);
			started = 1;
		}
		sprintf(buf, "PLATFORM_GROUP=%s", pgp->pltgrp_name);
		scriptwrite(fp, generic, "LINE", buf, (char *)0);
		sprintf(buf, "INST_ARCH=%s", pgp->pltgrp_isa);
		scriptwrite(fp, generic, "LINE", buf, (char *)0);

		/* write out individual platform entries */
		for (pp = pgp->pltgrp_members; pp; pp = pp->next) {
			sprintf(buf, "PLATFORM_NAME=%s", pp->plat_name);
			scriptwrite(fp, generic, "LINE", buf, (char *)0);
			if (pp->plat_uname_id) {
				sprintf(buf, "PLATFORM_ID=%s",
				    pp->plat_uname_id);
				scriptwrite(fp, generic, "LINE", buf,
				    (char *)0);
			}
			if (pp->plat_machine) {
				sprintf(buf, "MACHINE_TYPE=%s",
				    pp->plat_machine);
				scriptwrite(fp, generic, "LINE", buf,
				    (char *)0);
			}
			sprintf(buf, "IN_PLATFORM_GROUP=%s", pgp->pltgrp_name);
			scriptwrite(fp, generic, "LINE", buf, (char *)0);
		}
	}
	/* end_softinfo is a generic "end" template, so use it */
	if (started)
...
upg_write_plat_softinfo(FILE *fp, Product *prod1, Product *prod2)
{
	...
	for (pgp = merge_pltgrp; pgp; pgp = pgp->next) {
		...
		scriptwrite(fp, platgrp_softinfo,
		    "ISA", pgp->pltgrp_isa,
		    "PLATGRP", pgp->pltgrp_name, (char *)0);

		for (pp = pgp->pltgrp_members; pp; pp = pp->next) {
			scriptwrite(fp, platmember_softinfo,
