#ifndef lint
#ident   "@(#)install.c 1.63 96/02/08 SMI"
#endif lint
/*
...
static int 	create_null_product(Module *);
...
swi_load_installed(char * rootdir, int service)
{
	...
	sprintf(file, "%s/%s/etc", get_rootdir(), rootdir);
	new->info.media->med_hostname = load_hostname(file);
	
	sprintf(file, "%s", INST_RELEASE_read_path(rootdir));
	...
	if (service == FALSE) {
		 ...
		 */
		sprintf(file, "%s", services_read_path(rootdir));
		...
		while ((dp = readdir(dirp)) != (struct dirent *)0) {
			...
			if (ret == 0 &&
				...
				(void) sp_load_contents(new->sub->info.prod,
						new_svc->sub->info.prod);
				space_called = 1;
			}
			strcpy(os_version, new_svc->info.media->med_volume);
		...
		if (space_called == 0)
			(void) sp_load_contents(new->sub->info.prod, NULL);
	} else {
...
remove_extra_clusters(Module * prod)
{
	...
	if (prod->info.prod->p_rootdir != NULL)
		...
		sprintf(tmp, "%s", CLUSTER_read_path("/"));
...
load_installed_product(Module * prod, int service)
{
	...
	if (prod->info.prod->p_rootdir != NULL)
		...
		sprintf(tmp, "%s", clustertoc_read_path("/"));
...
remove_service_pkgs(Node * np, caddr_t data)
{
	...
	while (i) {
		...
		if ((i->m_sunw_ptype == PTYPE_ROOT &&
		    ...
		    (i->m_basedir &&
		    streqn(i->m_basedir, "/export/exec/", 13)) ||
...
split_service(Product * prod)
{
	...
	for (pkg = pkg->next; pkg != prod->p_packages->list; pkg = pkg_tmp) {
		 ...
		 */
		if ((info->m_basedir &&
		    !streqn(info->m_basedir, "/export/exec/", 13)) &&
		    (info->m_sunw_ptype == PTYPE_UNKNOWN ||
		    info->m_sunw_ptype == 0 ||
		    streq(info->m_basedir, "/opt") ||
		    streqn(info->m_basedir, "/opt/", 5))) {
...
set_loaded(Module * prod)
{
	...
	for (np = np->next; np != prod->info.prod->p_packages->list;
							np = np->next) {
		for (tmp_np = np; tmp_np != NULL; tmp_np =
					((Modinfo*)tmp_np->data)->m_instances) {
			info = (Modinfo *) tmp_np->data;
...
is_new_var_sadm(char	*rootdir)
{
	...
	sprintf(tmpFile, "%s/%s/var/sadm/system/admin/INST_RELEASE",
	    get_rootdir(), rootdir);
	...
	if ((fp = fopen(tmpFile, "r")) == (FILE *) NULL) {
		 ...
		 */
		sprintf(tmpFile, "%s/%s/var/sadm/softinfo/INST_RELEASE",
		    get_rootdir(), rootdir);
		if ((fp = fopen(tmpFile, "r")) == (FILE *) NULL) {
			/*
...
services_read_path(char	*rootdir)
{
	...
	if (is_new_var_sadm(rootdir))
		sprintf(services_tmp_path,
		    "%s/%s/var/sadm/system/admin/services",
		    get_rootdir(), rootdir);
	else
		sprintf(services_tmp_path, "%s/%s/var/sadm/softinfo",
		    get_rootdir(), rootdir);
...
load_softinfo(Module *svc)
{
	...
	sprintf(softinfo_file, "%s/%s", servdir, svc->info.media->med_volume);

	if ((fp = fopen(softinfo_file, "r")) == (FILE *)NULL)
		return (FAILURE);

	soft_desc = (struct softinfo_desc *)xcalloc((size_t) sizeof
	...
	while (fgets(buf, BUFSIZ, fp)) {
		...
		if (streq(key, "KVM_PATH")) {
			...
			*cp2 = '\0';
			if (!string_in_list(soft_desc->soft_arches, cp)) {
				sl = (StringList *)xcalloc((size_t) sizeof
				   (StringList));
			...
			*cp3 = '\0';
			...
			if (cp3 != NULL && !streq(cp3, ".all") &&
			    !string_in_list(soft_desc->soft_arches, cp)) {
				sl = (StringList *)xcalloc((size_t) sizeof
				   (StringList));
				sl->string_ptr = xstrdup(cp);
				link_to((Item **)&soft_desc->soft_arches,
			...
			if (!string_in_list(soft_desc->soft_packages, cp2)) {
				sl = (StringList *)xcalloc((size_t) sizeof
				   (StringList));
	 ...
	 */
	free_StringList(soft_desc->soft_arches);
	free_StringList(soft_desc->soft_packages);
	free(soft_desc);
	return (SUCCESS);
}
