#ifndef lint
#pragma ident "@(#)api_services.c 1.20 95/10/10 SMI"
#endif
/*
	...
	for (ap = prodinfo->p_arches; ap; ap = ap->a_next) {
		...
		*karch++ = '\0';
		...
		for (modp = prodinfo->p_locale; modp; modp = modp->next) {
			...
			if ((cp = get_C_lang_from_locale(mediamod->sub,
...
list_avail_svc_platforms(char *media, SW_service *svc, SW_error_info **err_info)
{
	...
	strcpy(fullarch, svc->sw_svc_isa);
	strcat(fullarch, ".");
	strcat(fullarch, svc->sw_svc_plat);
	for (ap = prodinfo->p_arches; ap; ap = ap->a_next) {
...
list_installed_svc_platforms(SW_service *svc, SW_error_info **err_info)
{
	...
	sprintf(service_name, "%s_%s", svc->sw_svc_os, svc->sw_svc_version);
	sprintf(platarch, "%s.%s", svc->sw_svc_isa, svc->sw_svc_plat);
	sprintf(softinfo_file, "%s/%s", servdir, service_name);
	...
	SW_error_info **err_info)
{
	...
	if (status != SUCCESS) {
		*err_info = alloc_sw_error(SW_INSUFFICIENT_SPACE);
		(*err_info)->sw_space_results = gen_final_space_report();
		return (-1); 
	}
...
get_createroot_info(SW_service *svc, SW_error_info **err_info)
{
	...
	sprintf(service_name, "%s_%s", svc->sw_svc_os, svc->sw_svc_version);
	sprintf(instarch, "%s.all:", svc->sw_svc_isa);
	sprintf(platarch, "%s.%s:", svc->sw_svc_isa, svc->sw_svc_plat);
	sprintf(softinfo_file, "%s/%s", servdir, service_name);
	...
	while (fgets(buf, BUFSIZ, fp)) {
		...
		if (streq(key, "USR_PATH")) {
			if (!streqn(cp, instarch, strlen(instarch)))
				continue;
			...
			if (!streqn(cp, platarch, strlen(platarch)))
				continue;
			for (remmntp = cri->sw_root_remmnt; remmntp;
			    remmntp = remmntp->next) {
				if (streq(remmntp->sw_remmnt_mntpnt,
			...
			if (!streqn(cp, instarch, strlen(instarch)) &&
			    !streqn(cp, platarch, strlen(platarch)) &&
			    !streqn(cp, ALL_ALL, strlen(ALL_ALL)))
				continue;
			cp = strchr(buf, ':');
			if (!cp) {
				free_createroot_info(cri);
			...
			if (! is_pkg(pkgp->sw_pkg_dir)) {
				...
				if ((pp = popen(command, "r")) == NULL) {
					pkgp->sw_pkg_size = 0;
				} else if (fgets(buf, BUFSIZ, pp) != NULL) {
					buf[strlen(buf)-1] = '\0';
					(void) sscanf(buf, "%ld %*s",
					    &pkgp->sw_pkg_size);
				}
				(void) pclose(pp);
			...
			if (is_4X && streqn(cp, ALL_ALL, strlen(ALL_ALL)))
				continue;
...
do_add_service(SW_service_mod *msp, SW_error_info **err_info)
{
	...
	sprintf(fullarch, "%s.%s", msp->sw_svmod_newservice->sw_svc_isa,
    ...
    SW_error_info **err_info)
{
	...
	while (fgets(buf, BUFSIZ, fp)) {
		...
		if (streq(key, "VERSION")) {
			...
			version = xstrdup(cp);
			sprintf(svc_name, "%s_%s", os, version);
		...
		if (streq(key, "SPOOLED_ROOT") || streq(key, "KVM_PATH") ||
			...
			strcpy(fullarch, isa);  /* save the full arch string */
...
is_pkg(char *spooled_root)
{
        ...
        sprintf(pkginfo_cmd, "/usr/bin/pkginfo  -d %s >> /dev/null 2>&1",
                spooled_root);
