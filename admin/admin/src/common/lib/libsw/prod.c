#ifndef lint
#pragma ident "@(#) prod.c 1.66 96/04/22"
#endif
/*
...
swi_get_clustertoc_path(Module * mod)
{
	...
	if (path_is_readable(cluster_toc) == FAILURE) {
		sprintf(cluster_toc, "%s/locale/%s/%s",
	 ...
	 * on the top level.  (Provides backwards compatibility w/Jupiter
 	 ...
 	 */
	if (path_is_readable(cluster_toc) == FAILURE) {
		sprintf(cluster_toc, "%s/%s", prod->info.prod->p_pkgdir,
				CLUSTER_TOC_NAME);
	}

	/*
	 * make sure the file is readable.  If not, we have no other places
	 * to look so return NULL.
	 */
	if (path_is_readable(cluster_toc) == FAILURE) {
	    return (NULL);
	}
 ...
 */
int
swi_path_is_readable(char * fn)
{
#ifdef SW_LIB_LOGGING
	sw_lib_log_hook("path_is_readable");
#endif
 ...
 *	use_packagetoc - flag to load_product() to indicate if a .packagetoc
 *			 file is to be used during the load
 * Return:
 ...
 */
int
load_all_products(Module *media, int use_packagetoc)
{
	Module * mod;
	...
	for (mod = mod->sub; mod != NULL; mod = mod->next) {
		if ((tmp = load_product(mod, use_packagetoc)) != SUCCESS)
			return_val = tmp;
	}
...
load_pkginfo(Module * mod, char * name, int options)
{
	...
	while (fgets(buf, BUFSIZ, mp) != (char *)NULL) {
		if (strncmp(buf, "PKG=", 4) == 0) {
			...
			if (add_locale(mod, info->m_locale) != SUCCESS) {
				free_modinfo(info);
				return (ERR_INVALID);
			}	
			...
			info->m_icon = (File *)crackfile(dirname, buf, ICONFILE);
...
get_value(char * str, char delim)
{
	...
	for (; cp && *cp && (*cp == ' '); ++cp);	/* chop white space */
...
promote_clusters(Module * prod, Module * hdclst)
{
	...
	for (mod = hdclst; mod != NULL; mod = mod->next) {
		 ...
		 */
		if (curclst == NULL) {
			prod->sub = curclst = (Module *)xcalloc(sizeof(Module));
			curclst->prev = NULL;
...
load_packagetoc(Module * mod)
{
	...
	while (fgets(buf, BUFSIZ, fp)) {
		...
		else if (strncmp(buf, "PKG=", 4) == 0)
		{
			...
			(void) add_locale(mod, info->m_locale);
		} else if (strncmp(buf, "SUNW_PKGLIST=", 13) == 0) {
			l10n = TRUE;
			info->m_l10n_pkglist =
...
arch_setup_package(Node * np, caddr_t data)
{
	...
	info2->m_name = info->m_name;
	info2->m_shared = NULLPKG;
	info2->m_arch = xstrdup(get_default_arch());
	info2->m_expand_arch = xstrdup(get_default_arch());
	info2->m_pdepends = info->m_pdepends;
	info2->m_idepends = info->m_idepends;
	info2->m_rdepends = info->m_rdepends;
	info2->m_l10n_pkglist = info->m_l10n_pkglist;
	info2->m_locale = info->m_locale;
	info2->m_desc = xstrdup(dgettext("SUNW_INSTALL_SWLIB", "This package is not supported on this architecture"));
 ...
 *	if use_packagetoc is TRUE and available, otherwise parse packages/4.x 
 *	software
 * Parameters:
 *	prod	       -
 *	use_packagetoc -
 ...
 */
static int
load_product(Module * prod, int use_packagetoc)
{
	struct dirent	*dp;
	DIR		*dirp;
	int 		 pkgtoc = FAILURE;
...
load_subclusters(FILE * fp, char * buf, Module * parent, Module * prod)
{
	...
	do {
		...
		if (strncmp(buf, "SUNW_CSRMEMBER", 14) == 0) {
			if (current) {
				current->next = (Module *) xcalloc(sizeof(Module));
				current->next->prev = current;
				current = current->next;
			} else {
				head = current = (Module *) xcalloc(sizeof(Module));
		...
		/* HACK reading a cluster definition, mark any packages...  */
				...
				if ((np = add_null_pkg(prod, get_value(buf, '='))) == NULL)
					return (NULL);
				info = (Modinfo *) np->data;
				current->info.mod = info;
				current->type = PACKAGE;
...
load_4x(Module * prod, char * name)
{
	...
	while (fgets(buf, BUFSIZ, fp)) {
		...
		case 'I':	/* install script */
			info->m_install = (File*)crackfile(dirname, buf, RUNFILE);
			break;
