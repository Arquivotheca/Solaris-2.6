#ifndef lint
#ident	 "@(#)dump.c 1.38 96/02/08 SMI"
#endif
/*
...
swi_dumptree(char * filename)
{
	...
	while (mod != NULL) {
		if (mod->info.media->med_type == INSTALLED_SVC ||
		    mod->info.media->med_type == INSTALLED) {
			fprintf(dfp, "dumping local environment:\n");
			dumpmod(mod);
			if (s_newproductmod &&
...
dump_modinfo_fields(Modinfo * mi)
{
	...
	case (PARTIAL):		(void) fprintf(dfp, "PARTIAL"); break;
	...
	if (mi->m_pkg_dir)
		fprintf(dfp, "%sm_pkg_dir = %s\n", indent[dump_level], mi->m_pkg_dir);
	if (mi->m_basedir)
		(void) fprintf(dfp,
	...
	if (mi->m_pkg_hist) {
		...
		if (mi->m_pkg_hist->deleted_files) {
			strncpy(deleted_file_buf, mi->m_pkg_hist->deleted_files,
			    511);
...
dump_mediainfo(Media * med)
{
	...
	if (med->med_locmap) {
		fprintf(dfp, "%sLOCMAPs\n", indent[dump_level]);
		dump_level++;
		for (lmap = med->med_locmap; lmap; lmap = lmap->next) {
			fprintf(dfp, "\n%slocmap_partial = %s\n",
			    indent[dump_level], lmap->locmap_partial);
			if (lmap->locmap_description)
				fprintf(dfp, "%slocmap_description = %s\n",
				    indent[dump_level],
				    lmap->locmap_description);
			if (lmap->locmap_base) {
				fprintf(dfp, "%slocmap_base = ",
				    indent[dump_level]);
				for (str = lmap->locmap_base; str;
				    str = str->next)
					fprintf(dfp, " %s", str->string_ptr);
				fprintf(dfp, "\n");
			}
		}
		dump_level--;
	...
	if (med->med_hostname) {
		...
		fprintf(dfp, "%smed_hostname = ",indent[dump_level]);
		while (host) {
