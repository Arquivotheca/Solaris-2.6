#ifndef lint
#pragma ident "@(#)soft_media.c 1.5 96/06/10 SMI"
#endif
/*
...
doeject(char * device)
{
	...
	if (strncmp(device, "/dev/dsk/", 9) == 0)
		(void) sprintf(rawdevice, "/dev/r%s", strstr(device, "dsk/"));
	else if (strncmp(device, "/dev/rdsk/", 10) == 0)
		(void) sprintf(rawdevice, "%s", device);
	else if ((cp = strrchr(device,  '/')) == (char *)NULL)
		return;
	else {
		cp++;
		if (*cp == 'r')
			(void) sprintf(rawdevice, "/dev/%s", cp);
		else
			(void) sprintf(rawdevice, "/dev/r%s", cp);
	}
...
read_product(Module * mod)
{
	...
	if (media->info.media->med_type == INSTALLED ||
		...
		(void) sprintf(buf, "%s/%s", get_rootdir(),
