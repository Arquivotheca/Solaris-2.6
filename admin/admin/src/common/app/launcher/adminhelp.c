...
#pragma ident   "@(#)adminhelp.c 1.9	94/09/08 SMI"
...
create_widgets(Widget parent, char help_category, char *textfile)
{
	...
	for (i = 0; i < 3; i++) {
		...
		switch (i) {
		  case TOP:
			c = TOPIC;
			label = catgets(catd, 1, 15, "Topics");
			break;
		  case HOW:
			c = HOWTO;
			label = catgets(catd, 1, 11, "How To");
			break;
		  case REF:
			c = REFER;
			label = catgets(catd, 1, 12, "Reference");
			break;
		  default:
			break;
		}
	...
	xmstr = XmStringCreateLocalized(catgets( catd, 1, 10, "Previous"));
	...
	xmstr = XmStringCreateLocalized(catgets( catd, 1, 1, "Done"));
...
read_subjfile(char *textfile, int selected)
{
	...
	if (!(fp = fopen(tocfile, "r"))) {
		(void) fprintf(stderr, catgets( catd, 1, 2, "Can't open %s\n"), tocfile);
		return(NOTOPEN);
	}
	...
	for (i = 1; fgets(label, 1000, fp); i++) {
		...
		if (*label == '\t') {
			syntax(tocfile, i, catgets( catd, 1, 3, "unexpected leading tab"));
			continue;
		}
		if (*label == '\n') {	/* skip blank lines too */
			syntax(tocfile, i, catgets( catd, 1, 4, "blank line"));
			continue;
		}
		*strchr(label, '\n') = '\0';
		i++;
		if (!fgets(buf, 1000, fp)) {
			syntax(tocfile, i, catgets(catd, 1, 13, "unexpected end-of-file"));
			break;
		}
		...
		if (sscanf(buf, "%s", filename) != 1)
			syntax(tocfile, i, catgets( catd, 1, 5, "too many tokens"));
	...
	if (!selected) {
		if (count > 0)
			...
			syntax(tocfile, 0, catgets( catd, 1, 6, "empty file"));
			return(EMPTYFILE);
		}
	}
...
set_data(void)
{
	...
	if(!(fp = fopen(textfile, "r"))) {
		(void) sprintf(errbuf, catgets(catd, 1, 14, "File not found:\n  %s\n"), textfile);
		XtVaSetValues(helptext, XmNvalue, errbuf, NULL);
	} else {
		if (fseek(fp, 0L, SEEK_END) == -1) {
			(void) fprintf(stderr,
				catgets( catd, 1, 7, "can't seek in file: %s\n"),
				textfile);
			return(NOSEEK);
		}
		len = ftell(fp);
		...
		if ((ret == 0) && ferror(fp)) {
			(void) fprintf(stderr,
				catgets( catd, 1, 8, "can't read file: %s\n"),
				textfile);
			return(NOREAD);
		}
		buf[len] = '\0';
...
{
	(void) fprintf(stderr,
	catgets( catd, 1, 9, "syntax error, file=%s\n\tline number=%d, \t=%s=\n"),
