 ...
 */
#ident  "@(#)group.c 1.1 92/06/22"

#ifndef lint
static char	sccsid[] =
	"@(#)group.c	2.27 91/10/15 Copyright 1991 Sun Microsystems";
#endif
...
#ifdef __STDC__
static	void		layout_none(Group_public *);
static	void		layout_row(Group_public *);
static	void		layout_col(Group_public *);
static	void		layout_rowcol(Group_public *);
static	void		group_set_xy(Group_public *, int, int);
static	void		group_set_members(Group_public *, Xv_opaque *);
static	void		get_rowcol_info(Group_public *, int *, int *);
static	int		get_value_x(Group_public *, int);
static	Rect		*get_rect_for_group(Group_public *);
static	Group_public	*get_parent_group(Group_public *);
static	void		get_compass_point(
				Xv_opaque, GROUP_COMPASS_POINTS, int *, int *);
static	void		place_cell(
				Group_public *, int, int, int, int, int, int);
#else
...
#endif
{
	...
	for (attrs = avlist; *attrs; attrs = attr_next(attrs)) {
		...
		case GROUP_ROW_ALIGNMENT:
			need_layout = TRUE;
			group_private->row_alignment =
				(GROUP_ROW_ALIGNMENTS)attrs[1];
			break;

		case GROUP_COLUMN_ALIGNMENT:
			need_layout = TRUE;
			group_private->col_alignment =
				(GROUP_COLUMN_ALIGNMENTS)attrs[1];
			break;
...
Pkg_private Xv_opaque
#ifdef __STDC__
group_get(Group_public *group_public,
	int		*status,
	Attr_attribute	attr,
	Attr_avlist	args)
#else
...
#endif
{
	 ...
	 */
	r = get_rect_for_group((Group_public *)group_public);
	...
	if (group_private->members) {
		...
		group_private->value_rect.r_left =
				group_private->group_rect.r_left;
		group_private->value_rect.r_top =
				group_private->group_rect.r_top;
	}
	 ...
	 */
	if (parent = get_parent_group((Group_public *)group_public))
...
#endif
{
	 ...
	 */
	group_private->members =
		(Xv_opaque *)calloc(num+1, sizeof (Xv_opaque));
...
#endif
{
	...
	for (i = 0; group_private->members[i]; i++)
	{
		...
		xv_set(cur,
			XV_X,	x + (xv_get(cur, XV_X) -
					group_private->group_rect.r_left),
			XV_Y,	y + (xv_get(cur, XV_Y) -
					group_private->group_rect.r_top),
			NULL);
	}
...
#endif
{
	...
	get_compass_point(group_private->anchor_obj,
				group_private->anchor_point,
				&anchor_x, &anchor_y);
	get_compass_point(group_public,
				group_private->reference_point,
				&ref_x, &ref_y);
	...
	for (i = 0; members[i]; i++) {
		...
		if (group_private->flags & ROWFIRST) {
			...
			if (++current_row >= group_private->rows) {
				current_col++;
				current_row = 0;
				if ((group_private->col_alignment ==
				    GROUP_LABELS) && members[i+1])
					vx = get_value_x(
						group_public, current_col);
			}
		}
	}
}
...
static void
#ifdef __STDC__
place_cell(Group_public	*group_public,
	int		i,
	int		cell_width,
	int		cell_height,
	int		vx,
...
#endif
{
	...
	else
	{
		...
		{
			if (r1 = (Rect *)xv_get(
					group_private->members[i], XV_RECT))
