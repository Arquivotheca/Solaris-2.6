...
#define	TTY_HELP_NOHELP_TITLE	dgettext("SUNW_INSTALL_SYSID", "No Help is available.")
#define	TTY_HELP_NOHELP_NOTICE	dgettext("SUNW_INSTALL_SYSID",\
...
get_help_category(void)
{
	...
	/* load up choices */
	if (intro == (char *)0) {
		intro = dgettext("SUNW_INSTALL_SYSID",
	...
	/* show title  */
	wheader(hwin, dgettext("SUNW_INSTALL_SYSID", "Help Main Index"));
...
get_help_subject(h_index_t *cat, char *cat_title)
{
	...
	char		*ptr;		/* scratch pointer for dgettext() */
	...
	/* show title */
	wheader(hwin, dgettext("SUNW_INSTALL_SYSID", "Help Subjects"));
...
load_index(char *cat, char *fname)
{
	...
	for (i = 1; r = fgets(label, 1024, fp); i++) {
		...
		if (!r) {
			...
			syntax(tocfile, i, dgettext("SUNW_INSTALL_SYSID", "unexpected end-of-file"));
			break;
		...
		*strchr(label, '\n') = '\0';
		...
		while (*buf != '\t') {
			...
			if (!(r = fgets(buf, 1024, fp))) {
				...
				syntax(tocfile, i,
				    dgettext("SUNW_INSTALL_SYSID", "unexpected end-of-file"));
				break;
