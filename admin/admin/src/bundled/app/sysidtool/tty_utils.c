...
#pragma	ident	"@(#)tty_utils.c 1.17 96/07/16"
...
werror(WINDOW *w, int row, int col, int width, char *fmt, ...)
{
	...
	if (fmt != (char *)0) {
		...
		va_start(ap, fmt);
		n = wword_wrap(w, row, col, width, buf) + 2;
		va_end(ap);
	} else
		n = 0;

	(void) wword_wrap(w, row + n, col, width,
			dgettext("SUNW_INSTALL_SYSID", "Press Return to continue"));
 ...
 * in calls to dgettext() somewhere else in the program in order for
...
wfooter_init(int force_alternates)
{
	...
	f_keys[index(F_SPARE3)].f_func = DESC_F_SPARE3;
	...
	for (i = 0; i < MAXFUNC; i++) {
		if (!force_alternates && tigetstr(f_keys[i].f_keycap)) {
			/* XXX need to dgettext f_special */
			(void) sprintf(keystr, "%s_%s",
				f_keys[i].f_special,
				f_keys[i].f_func);
		} else {
			/* XXX need to dgettext f_fallback */
			(void) sprintf(keystr, "%s_%s",
				f_keys[i].f_fallback,
				f_keys[i].f_func);
		}
 ...
 * `r', `c' return last row
 *
 * This function is identical to the one used by ttinstall... this needs
 * to be commonized and put into libspmitty for use by all curses apps
 */
int
