...
#pragma	ident	"@(#)tty_form.c 1.17 96/06/13"
...
#ifdef TEXT_DOMAIN		/* fix this (somehow...) */
#include "tty_msgs.h"
#else
#include "kdconf_msgs.h"

#define	YES			KDCONF_MSGS(KDCONF_YES)
#define	NO			KDCONF_MSGS(KDCONF_NO)
#define	CONTINUE_TO_ADVANCE	KDCONF_MSGS(KDCONF_CONTINUE)
#endif
...
form_exec(Form *form, char *(*err_to_string)(int, int, ...))
{
	...
	flush_input();	/* no typeahead between forms */
	...
	for (;;) {
		...
		if ((f->flags & FF_RDONLY) == 0 &&
			...
			/*
			 * Update summary field only when
			 * all fields have been validated.
			 * Otherwise we can end up printing garbage in the
			 * summary field (this was happening in the date field).
			 */
	...
	char	*(*err_to_string)(int, int, ...))
{
	...
	if (f->type == FIELD_TEXT && (f->flags & FF_RDONLY) == 0) {
