#ifndef lint
#pragma ident "@(#)atconfig.c 1.10 96/04/09"
#endif
/*
 ...
 *	SAVE_* is where we'll squirrel away the saved files
 *	FILE_* are the files we're trying to save
 */
#define	LOGFILE "/dev/null"
#define	OLDROOT "/a"
#define	ROOTPROG "/usr/sbin/install.d/rootdev"

#define	SAVE_SYSINIT_DIR "/tmp/root/kernel/misc"
#define	FILE_INIT1 OLDROOT "/.atconfig"
#define	FILE_INIT2 OLDROOT "/kernel/misc/sysinit"

#define	SAVE_ESCD_DIR "/tmp/root/platform/i86pc/boot"
#define	FILE_ESCD OLDROOT "/platform/i86pc/boot/escd.rf"

#define	SAVE_BOOTENV_DIR "/tmp/root/platform/i86pc/boot/solaris"
#define	FILE_BOOTENV OLDROOT "/platform/i86pc/boot/solaris/bootenv.rc"
 ...
 *	in two places (FILE_INIT1 and FILE_INIT2) instead of one and
 *	it refuses to copy a sysinit module that hasn't been patched.
 *	it also copies files used by the new "devconf" booting system.
 *
...
main(int argc, char *argv[])
{
	...
	if ((mkdirp(SAVE_SYSINIT_DIR, 0777) < 0) &&
	    (access(SAVE_SYSINIT_DIR, F_OK) < 0)) {
		tolog("mkdirp failed for " SAVE_SYSINIT_DIR);
		exit(1);
	}

	/* make the save place to hold the escd file */
	if ((mkdirp(SAVE_ESCD_DIR, 0777) < 0) &&
	    (access(SAVE_ESCD_DIR, F_OK) < 0)) {
		tolog("mkdirp failed for " SAVE_ESCD_DIR);
		/* don't exit here, since we can still handle other stuff */
	}

	/* make the save place to hold the bootenv file */
	if ((mkdirp(SAVE_BOOTENV_DIR, 0777) < 0) &&
	    (access(SAVE_BOOTENV_DIR, F_OK) < 0)) {
		tolog("mkdirp failed for " SAVE_BOOTENV_DIR);
		/* don't exit here, since we can still handle other stuff */
	}
	...
	if (trymount(rootdev, OLDROOT)) {
		...
		if (ser_exists(FILE_INIT1)) {
			tolog("copy" FILE_INIT1);
			(void) run("cp %s %s", FILE_INIT1, SAVE_SYSINIT_DIR);
		} else if (ser_exists(FILE_INIT2)) {
			tolog("copy" FILE_INIT2);
			(void) run("cp %s %s", FILE_INIT2, SAVE_SYSINIT_DIR);
		}

		/* Look for the 'eeprom' files on the root */
		if (access(FILE_ESCD, F_OK) == 0) {
			tolog("copy" FILE_ESCD);
			(void) run("cp %s %s", FILE_ESCD, SAVE_ESCD_DIR);
		}
		if (access(FILE_BOOTENV, F_OK) == 0) {
			tolog("copy" FILE_BOOTENV);
			(void) run("cp %s %s", FILE_BOOTENV, SAVE_BOOTENV_DIR);
		}
