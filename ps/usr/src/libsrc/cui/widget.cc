#pragma ident "@(#)widget.cc   1.7     93/07/23 SMI"
...
int Widget::addCallback(char *name, CUI_CallbackType type)
{
	CUI_CallbackProc callback = CUI_lookupCallback(name);
	if(!callback)
		CUI_fatal(dgettext( CUI_MESSAGES, "Can't find callback '%s'"), name);
