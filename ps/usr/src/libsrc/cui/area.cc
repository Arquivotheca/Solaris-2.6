#pragma ident "@(#)area.cc   1.9     93/07/22 SMI" 
...
int ControlArea::realize(void)
{
	...
	if(parent && parent->getControlArea())
	{
		...
		else
		{
			...
			{
				long flags = children[i]->flagValues();
				if(flags & CUI_SENSITIVE)
				{
					active = TRUE;
					break;
...
int ControlArea::doKey(int key, Widget *)
{
	...
	if(current)
	{
		if(key == REQ_LOCAL_HOME || key == REQ_LOCAL_END)
		{
			...
			{
				VirtualControl *control = (VirtualControl *)current;
				current = control->widgetPtr();
				if(current->getId() == CUI_CONTROL_AREA_ID)
				{
            		FORM *form = current->ETIform();
					int request = REQ_FIRST_FIELD;
					if(key == REQ_LOCAL_END)
						request = REQ_LAST_FIELD;
					retcode = form_driver(form, request);
...
int ControlArea::initWindow(void)
{
	...
	if(containingArea)
	{
		int name = CUI_compileString(containingArea->getName());
		CUI_setArg(windowResources[i], parentWindowId, name); i++;
	}
