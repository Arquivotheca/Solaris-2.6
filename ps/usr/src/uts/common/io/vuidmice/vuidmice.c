...
#pragma	ident "@(#)vuidmice.c	1.11	95/01/16 SMI"
...
_init(void)
{
	...
	mutex_init(&vuidmice_lock, "vuidmice lock", MUTEX_DEFAULT, NULL);
	  ...
	  const int oflag, const int sflag, const cred_t *const crp)
{
	...
	qp->q_ptr = (struct MouseStateInfo *)
			kmem_zalloc(sizeof(struct MouseStateInfo), KM_SLEEP);
...
vuidmice_rput(queue_t *const qp, mblk_t *mp)
{
	...
	if ( M_TYPE(mp) == M_DATA ) {
		if ( (int)STATEP->format ==  VUID_FIRM_EVENT )
			return putq(qp, mp);   /* queue this message & return */
