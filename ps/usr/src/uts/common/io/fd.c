...
#pragma ident	"@(#)fd.c	1.42	96/05/27 SMI"
...
static int fd_build_user_vtoc(struct fcu_obj *, struct fdisk *, struct vtoc *);
...
fd_probe(register dev_info_t *dip)
{
	...
	default:		/* need to check conf file */
		...
		if (ddi_prop_op(DDI_DEV_T_ANY, dip, PROP_LEN_AND_VAL_BUF,
		    ...
		    DDI_PROP_SUCCESS) {
			FDERRPRINT(FDEP_L3, FDEM_ATTA,
			    ("fd_probe failed density: dip %x unit %d\n",
			    dip, unit_num));
			return (DDI_PROBE_FAILURE);
		}
		...
		if (ddi_prop_op(DDI_DEV_T_ANY, dip, PROP_LEN_AND_VAL_BUF,
		    ...
		    DDI_PROP_SUCCESS) {
			FDERRPRINT(FDEP_L3, FDEM_ATTA,
			    ("fd_probe failed size: dip %x unit %d\n",
			    dip, unit_num));
			return (DDI_PROBE_FAILURE);
		}
	}
	FDERRPRINT(FDEP_L3, FDEM_ATTA,
	    ("fd_probe dip %x unit %d\n", dip, unit_num));
...
fd_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	case DDI_ATTACH:
		...
		if (drive_type == 0) {
			FDERRPRINT(FDEP_L3, FDEM_ATTA,
			    ("fd_attach failed type: dip %x unit %d\n",
			    dip, unit_num));
			return (DDI_FAILURE);
		}
		...
		sprintf(name, "fd%dlock", drive_num);
		mutex_init(&fjp->fj_lock, name, MUTEX_DRIVER, *fjp->fj_iblock);
		sprintf(name, "fd%dsema", drive_num);
		sema_init(&fdp->d_ocsem, 1, name, SEMA_DRIVER, NULL);
		...
		*fjp->fj_attr = fdtypes[fdp->d_deffdtype];
		...
		FDERRPRINT(FDEP_L3, FDEM_ATTA,
		    ("fd_attach: dip %x unit %d\n", dip, unit_num));
		sprintf(name, "fd%d", drive_num);
...
fd_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	FDERRPRINT(FDEP_L3, FDEM_ATTA, ("fd_detach dip %x\n", dip));
...
fd_open(dev_t *devp, int flag, int otyp, cred_t *cred_p)
{
	...
	if ((fdp->d_exclmask & pbit) || ((flag & FEXCL) && part_is_open)) {
		FDERRPRINT(FDEP_L0, FDEM_OPEN,
		    ("fd_open: exclparts %x openparts %x lyrcnt %x pbit %x\n",
...
fdgetlabel(struct fcu_obj *fjp, int unit)
{
	...
	FDERRPRINT(FDEP_L0, FDEM_GETL, ("fdgetlabel fd unit %d\n", unit));
...
nolabel:
	...
	sprintf(fdp->d_vtoc_asciilabel, newlabel, fjp->fj_chars->fdc_ncyl,
	    fjp->fj_chars->fdc_nhead, fjp->fj_chars->fdc_secptrack);
	fjp->fj_flags |= FUNIT_UNLABELED;
...
fd_close(dev_t dev, int flag, int otyp, cred_t *cred_p)
{
	...
	if (part_is_closed) {
		...
		FDERRPRINT(FDEP_L0, FDEM_CLOS,
		    ("fd_close: exclparts %x openparts %x lyrcnt %x\n",
...
fd_strategy(register struct buf *bp)
{
	...
	FDERRPRINT(FDEP_L1, FDEM_STRA, ("fd_strategy: bp = 0x%x, dev = 0x%x\n",
	...
	if (fjp->fj_chars->fdc_sec_size > NBPSCTR && (bp->b_blkno & 1))  {
		FDERRPRINT(FDEP_L3, FDEM_STRA,
		    ("fd%d: block %d is not start of sector!\n",
	...
	if (bp->b_bcount % fjp->fj_chars->fdc_sec_size)  {
		FDERRPRINT(FDEP_L3, FDEM_STRA,
		    ("fd%d: count %d must be a multiple of %d\n",
...
fdstart(register struct fcu_obj *fjp)
{
	...
	struct partition *pp;
	u_int ptend;
	u_int bincyl;		/* (the number of the desired) block in cyl. */
	u_int blk, len, tlen;
...
("  blk 0x%x addr 0x%x len 0x%x cyl %d head %d sec %d\n  resid 0x%x, tlen %d\n",
		...
		FDERRPRINT(FDEP_L0, FDEM_STRT,
		    ("fdstart done: b_resid %d, b_count %d\n",
		    bp->b_resid, bp->b_bcount));
		if (fdp->d_iostat) {
			if (bp->b_flags & B_READ) {
				KIOSP->reads++;
...
fd_ioctl(dev_t dev, int cmd, int arg, int flag, cred_t *cred_p, int *rval_p)
{
	...
	FDERRPRINT(FDEP_L1, FDEM_IOCT, ("fd_ioctl fd unit %d: cmd %x, arg %x\n",
	    unit, cmd, arg));
...
("fd_ioctl fd unit %d: FDIOSCHAR odd xfer rate %dkbs\n",
	...
	case FDIOCMD:
	{
		...
		if (fc.fdc_cmd == FDCMD_READ || fc.fdc_cmd == FDCMD_WRITE) {
			...
			aiov.iov_len = (u_int)fc.fdc_secnt *
			    fjp->fj_chars->fdc_sec_size;
			uio->uio_iov = &aiov;
...
fd_rawioctl(struct fcu_obj *fjp, int unit, caddr_t arg, int mode)
{
	...
	case FDRAW_FORMAT:
		...
		if (ddi_copyin(uaddr, fdr.fdr_addr, (u_int)fdr.fdr_nbytes,
		    mode)) {
...
fd_media_watch(caddr_t arg)
{
	...
	unit = fd_getdrive((dev_t)arg, &fjp, &fdp);
...
fd_check_media(dev_t dev, enum dkio_state state)
{
	...
	while (fdp->d_media_state == state) {
		...
		fdp->d_media_timeout_id = timeout(fd_media_watch,
		    (caddr_t)dev, fdp->d_media_timeout);
