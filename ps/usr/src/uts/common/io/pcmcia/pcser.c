...
#ident "@(#)pcser.c	1.72     96/09/03 SMI"
 ...
 */
#include <sys/pcser_io.h>
#include <sys/pcser_var.h>
#include <sys/pcser_reg.h>
#include <sys/pcser_conf.h>
...
static void pcser_card_ready_timeout(pcser_unit_t *);
static void pcser_readywait_timeout(pcser_unit_t *);
 ...
 */
static int pcser_open();
static int pcser_close(/*queue_t *q, int flag*/);
static int pcser_wput(/*queue_t *q, mblk_t *mp*/);
static int pcser_ioctl(/*queue_t *q, mblk_t *mp*/);
static void pcser_reioctl(/*pcser_line_t *line */);
static void pcser_restart(/*pcser_line_t *line */);
static void pcser_start(/*pcser_line_t *line */);
static void pcser_param(/*pcser_line_t *line */);
static int pcser_xmit(pcser_unit_t *, u_char, int);
static int pcser_rcv(pcser_unit_t *);
static int pcser_rcvex(pcser_unit_t *, register u_char, register u_char);
static int pcser_modem(pcser_unit_t *, u_char, int);
static void pcser_drainsilo(/*pcser_line_t *line */);
static void pcser_draintimeout(/*pcser_line_t *line */);
static void silocopy(/*pcser_line_t *line, u_char *buf, int count*/);
static void pcser_xwait(/*pcser_line_t *line*/);
static void pcser_timeout(/*pcser_line_t *line*/);
static u_int pcser_poll();
static u_int pcser_softint();
static void pcser_txflush(/*pcser_line_t *line */);
static int pcser_identify(/* dev_info_t *, ddi_attach_cmd_t */);
static int pcser_attach(/* dev_info_t *dip, ddi_detach_cmd_t cmd */);
static int pcser_detach(/* dev_info_t *dip, ddi_detach_cmd_t cmd */);
static int pcser_getinfo(/* dev_info_t *, ddi_info_cmd_t, void *, void ** */);
static void pcser_ignore_cd_timeout(struct pcser_line_t *);
...
/*
 * pcser_identify(dev_info_t *dip) - this driver entry point idenifies the
 ...
 */
static int
pcser_identify(dev_info_t *dip)
{

	char *name = ddi_get_name(dip);

	if (strcmp(name, pcser_name) == 0) {
...
pcser_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	client_reg.dip = dip;
	strcpy(client_reg.driver_name, pcser_name);
	 ...
	 */
	mutex_init(&pcser->event_hilock, "pcser->event_hilock", MUTEX_DRIVER,
						*(client_reg.iblk_cookie));

	mutex_init(&pcser->noirq_mutex, "pcser->noirq_mutex",
				MUTEX_DRIVER, (void *)pcser->soft_blk_cookie);

	pcser->pcser_mutex = &pcser->noirq_mutex;

	cv_init(&pcser->readywait_cv, "pcser->readywait_cv", CV_DRIVER,
								(void *)NULL);

	line = &pcser->line;
	mutex_init(&line->line_mutex, "line->line_mutex", MUTEX_DRIVER,
					(void *)pcser->soft_blk_cookie);
	cv_init(&line->cvp, "line->cvp", CV_DRIVER, (void *)NULL);

	line = &pcser->control_line;
	mutex_init(&line->line_mutex, "control_line->line_mutex",
				MUTEX_DRIVER, (void *)pcser->soft_blk_cookie);
	cv_init(&line->cvp, "control_line->cvp", CV_DRIVER, (void *)NULL);
	 ...
	 *	fail.
	 */
	csx_GetStatus(pcser->client_handle, &get_status);
...
pcser_auctl(pcser_line_t *line, int action)
{
	...
	csx_AccessConfigurationRegister(pcser->client_handle,
							&access_config_reg);
...
pcser_softint(pcser_unit_t *pcser)
{
	...
	if (pcser->flags & PCSER_ATTACHOK) {
		...
		if ((state = (line->state & work_flags)))
 ...
 */
static int
pcser_xmit(pcser_unit_t *pcser, u_char msr, int call_state)
{
	register pcser_line_t *line = &pcser->line;
	register int i;
...
pcser_rcv(pcser_unit_t *pcser)
{
	...
	while ((rcsr = csx_Get8(line->handle, PCSER_REGS_LSR)) &
	    ...
	    if ((iflag & IGNPAR) || !((iflag & PARMRK) &&
				!(iflag & ISTRIP) && (c == (u_char)'\377'))) {
		PUTSILO(line, c);
	    } else { /* !IGNPAR */
		PUTSILO(line, (u_char)'\377');
		PUTSILO(line, c);
	    }
 ...
 */
static int
pcser_rcvex(pcser_unit_t *pcser, register u_char rcsr, register u_char c)
{
	register pcser_line_t *line = &pcser->line;
	register int i, rc;
	register tcflag_t iflag;
 ...
 */
static int
pcser_modem(pcser_unit_t *pcser, u_char msr, int call_state)
{
	register pcser_line_t *line = &pcser->line;
 ...
 */
static void
pcser_draintimeout(register pcser_line_t *line)
{
 ...
 */
static void
silocopy(register pcser_line_t *line, register u_char *buf, register count)
{
	register u_char *sink = line->pcser_sink;
	register cc = count;
...
#ifdef	PCSER_DEBUG
	if ((pcser_debug & PCSER_DEBUG_LOWMASK) > 1)
	    cmn_err(CE_CONT, "pcser_open: dev = 0x%x *dev = 0x%x\n",
						(int)dev, (int)*dev);
#endif
		 ...
		 */
			...
			if ((line->state & PCSER_FIRST_OPEN) ||
			    ...
			    line->ignore_cd_timeout_id = timeout(
					pcser_ignore_cd_timeout, (caddr_t)line,
 ...
 */
static void
pcser_ack(mblk_t *mp, mblk_t *dp, uint size)
{
	register struct iocblk  *iocp = (struct iocblk *)mp->b_rptr;
 ...
 */
static void
pcser_reioctl(register pcser_line_t *line)
{
	register queue_t *q;
	register mblk_t *mp;
		 ...
		 */
			...
			if (*(int *)mp->b_cont->b_rptr == 0) {
				...
				if (CARD_PRESENT(pcser)) {
				    ...
				    timeout(pcser_timeout, (caddr_t)line,
							BREAK1_TIMEOUT);
 ...
 */
static void
pcser_restart(register pcser_line_t *line)
{
	register pcser_unit_t *pcser = line->pcser;
...
pcser_param(register pcser_line_t *line)
{
	...
	if (pcser_debug & PCSER_DEBUG_PARAM) {
		...
		if (cflag & PARENB) {
		    strcat(dbuff, "USE_P ");
		    if (!(iflag & INPCK))
			strcat(dbuff, "IGNORE_P ");
		    if (cflag & PARODD)
			strcat(dbuff, "ODD_P ");
		    else
			strcat(dbuff, "EVEN_P ");
		} else {
		    strcat(dbuff, "NO_PARITY ");
		}

		if (cflag & CRTSCTS)
		    strcat(dbuff, "CRTSCTS ");
		else
		    strcat(dbuff, "NO_CRTSCTS ");

		if (cflag & CRTSXOFF)
		    strcat(dbuff, "CRTSXOFF ");
		else
		    strcat(dbuff, "NO_CRTSXOFF ");

		if (iflag & IXOFF)
		    strcat(dbuff, "IXOFF ");
		else
		    strcat(dbuff, "NO_IXOFF ");

		if (iflag & IXON)
		    strcat(dbuff, "IXON ");
		else
		    strcat(dbuff, "NO_IXON ");

		if (iflag & IXANY)
		    strcat(dbuff, "IXANY ");
		else
		    strcat(dbuff, "NO_IXANY ");
...
static void
pcser_timeout(register pcser_line_t *line)
{
	cv_broadcast(&line->cvp);
}
...
pcser_start(register pcser_line_t *line)
{
	 ...
	 */
	PCSER_HIMUTEX_ENTER(pcser);
	if ((line->state & (PCSER_BREAK | PCSER_BUSY |
				PCSER_DELAY | PCSER_FLUSH)) &&
					!(line->state & PCSER_DRAIN)) {
...
pcser_show_baud(tcflag_t ospeed, char *buf)
{
	...
	if (ospeed >= PCSER_MAX_SPEEDS)
	    sprintf(buf, "[invalid speed 0x%x] ", (int)ospeed);
	else
	    sprintf(buf, "%s ", pcser_text_speeds[ospeed]);
}
#endif
...
if (pcser_debug & PCSER_DEBUG_CISVARS) {
	...
	if (pcser_debug & PCSER_DEBUG_CIS)
		...
		mutex_init(pcser->pcser_mutex, "pcser->pcser_mutex",
...
pcser_event(event_t event, int priority, event_callback_args_t *eca)
{
	...
	switch (event) {
		...
		if (GET_CLIENT_INFO_SUBSVC(ci->Attributes) ==
		    ...
		    strcpy(ci->ClientName, PCSER_CLIENT_DESCRIPTION);
		    strcpy(ci->VendorName, PCSER_VENDOR_DESCRIPTION);
...
static void
pcser_card_ready_timeout(pcser_unit_t *pcser)
{
...
static void
pcser_readywait_timeout(pcser_unit_t *pcser)
{
#ifdef	PCSER_DEBUG
	if ((pcser_debug & PCSER_DEBUG_LOWMASK) > 16)
...
static void
pcser_ignore_cd_timeout(register struct pcser_line_t *line)
{
#ifdef	PCSER_DEBUG
	pcser_unit_t *pcser = line->pcser;
#endif
...
pcser_dtr_off(pcser_line_t *line)
{
	...
	if (line->flags & PCSER_DTRFORCE) {
	    ...
	    line->dtr_shadow = (u_char)DTR_OFF_SHADOW;
	}
}
...
pcser_dtr_on(pcser_line_t *line)
{
	...
	line->dtr_shadow = (u_char)DTR_ON_SHADOW;
}
...
CHECK_RTS_OFF(register pcser_line_t *line)
{
	...
	/*
	 * Don't do anything if the card hasn't been
	 *      configured yet.
	 */
	if (!(line->pcser->flags & PCSER_REQUESTCONFIG))
	    return;
