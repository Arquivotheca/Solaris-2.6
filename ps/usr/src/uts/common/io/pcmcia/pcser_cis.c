...
#ident "@(#)pcser_cis.c	1.26     96/05/20 SMI"
...
typedef struct pcser_cis_addr_tran_t {
	u_long		desireability;	/* how desireable this entry is */
	u_long		modem_base;	/* base offset of UART registers */
...
pcser_parse_cis(pcser_unit_t *pcser, pcser_cftable_t **cftable)
{
	...
	if ((ret = csx_GetFirstTuple(pcser->client_handle,
		...
		for (i = 0; i < cistpl_vers_1.ns; i++)
		    strcpy(cis_vars->prod_strings[i], cistpl_vers_1.pi[i]);
	...
	do {
		...
		*cftable = ocft;
		...
		if (cistpl_cftable_entry.flags & CISTPL_CFTABLE_TPCE_FS_IO) {
		    ...
		    if (io->ranges) {
			cft->p.modem_base = (u_long)io->range[0].addr;
			cft->p.length = (u_long)io->range[0].length;
			if (default_cftable) {
			    dcft->p.modem_base = (u_long)io->range[0].addr;
			    dcft->p.length = (u_long)io->range[0].length;
			}
		    } else {
			/*
					...
					int count, pcser_cftable_t *cftable)
{
	...
	for (j = 0; j < CISTPL_VERS_1_MAX_PROD_STRINGS; j++)
	    if (cis_vars->prod_strings[j][0] != NULL) {
		strcat(vers_1, (char *)cis_vars->prod_strings[j]);
		/*
						...
						pcser_manuspec_t *pcms)
{
	...
	while (k--) {
		...
		switch (pt->fmt) {
			...
			*(unsigned *)(((uchar_t *)pcms +
				(unsigned)pt->var)) = (unsigned)param;
