...
#pragma ident "@(#)pcgld.c	1.8	96/09/27 SMI"
...
gld_register(dev_info_t *devinfo, char *devname, gld_mac_info_t *macinfo)
{
	...
	if (!macinfo->gldm_options & GLDOPT_PCMCIA)
		ddi_get_iblock_cookie(devinfo, macinfo->gldm_irq_index,
					&macinfo->gldm_cookie);
	...
	if (macinfo->gldm_intr_hi != NULL && ddi_intr_hilevel(devinfo, 0)) {
		ddi_add_softintr(devinfo, DDI_SOFTINT_MED,
					&macinfo->gldm_softid,
					&macinfo->gldm_cookie,
					NULL, gld_intr,
					(caddr_t)macinfo);
	...
	if (!(macinfo->gldm_options & GLDOPT_PCMCIA)) {
		...
		sprintf(minordev, "%s%d", devname, (int) macinfo->gldm_ppa);
		ddi_create_minor_node(devinfo, minordev, S_IFCHR,
...
#ifdef GLD_DEBUG
	if (gld_debug & GLDTRACE)
		cmn_err(CE_NOTE, "gld_wsrv(%x)", (int) q);
#endif
...
#ifdef GLD_DEBUG
	if (gld_debug & GLDTRACE)
		cmn_err(CE_NOTE, "gld_ioctl(%x %x)", (int) q, (int) mp);
#endif
	gld = (gld_t *) q->q_ptr;
...
#ifdef GLD_DEBUG
	if (gld_debug & GLDTRACE)
		cmn_err(CE_NOTE, "gld_recv(%x, %x)", (int) mp, (int) macinfo);
#endif
...
#define	MLEN(mp) ((mp)->b_wptr - (mp)->b_rptr)
		if (gld_debug & GLDRECV) {
			cmn_err(CE_CONT, "gld_recv: mlen=%d machdr=<%s-",
				MLEN(mp),
 ...
 */
u_int
gld_intr_hi(gld_mac_info_t *macinfo)
{
	int claimed;
	ASSERT(macinfo != NULL);
	if (!(macinfo->gldm_GLD_flags & GLD_INTR_READY) ||
