...
#ident "@(#)pcram.c	1.88     96/05/20 SMI"
 ...
 */
#include <sys/pcramio.h>
#include <sys/pcramvar.h>
...
static int pcram_prop_op(dev_t dev, dev_info_t *dip,
				...
				int *lengthp);
static int pcram_ioctl(dev_t dev, int cmd, int arg, int mode,
...
static int pcram_identify(dev_info_t *dip);
...
static mem_region_t *pcram_get_firstnext_region(mem_region_t *, u_long,
						u_long, u_long *);
 ...
 *	pcram_identify()
 *	pcram_probe()	(optional)
 *	pcram_attach()
 *	pcram_detach()
 *	pcram_getinfo()
 */
static int
pcram_identify(dev_info_t *dip)
{
	char	*name;
	...
	if (pcram_debug & PCRAM_DEBUG_TRACE)
		cmn_err(CE_CONT,
			"pcram_identify: string [%s]\n", name);
#endif

	if (strcmp(name, pcram_name) == 0) {
...
pcram_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	mutex_init(&rs->event_hilock, "rs->event_hilock",
			MUTEX_DRIVER, *(client_reg.iblk_cookie));

	/* set up the mutex to protect the pcram_state_t */
	mutex_init(&rs->mutex, MUTEX_CONTROLER_NAME,
			MUTEX_DRIVER, &rs->soft_blk_cookie);

	/* strategy(): waiting for I/O to complete  */
	cv_init(&rs->condvar, CV_SOFT_NAME, CV_DRIVER, (void *)NULL);
	/* write(): waiting for I/O to complete  */
	cv_init(&rs->condvar_wr, CV_SOFT_NAME, CV_DRIVER, (void *)NULL);
	/* read(): waiting for I/O to complete  */
	cv_init(&rs->condvar_rd, CV_SOFT_NAME, CV_DRIVER, (void *)NULL);
	/* for DKIOCSTATE ioctl()  */
	cv_init(&rs->condvar_mediastate, CV_SOFT_NAME, CV_DRIVER,
						(void *)NULL);
	/* Init firstopenwait_cv */
	cv_init(&rs->firstopenwait_cv, "rs->firstopenwait_cv",
					CV_DRIVER, (void *)NULL);

	/* mutex to protect region lists */
	mutex_init(&rs->region_lock, "rs->region_lock", MUTEX_DRIVER,
						&rs->soft_blk_cookie);
	 ...
	 *	completed
	 */
	csx_GetStatus(rs->client_handle, &get_status);
...
pcram_open(dev_t *devp, int flag, int otyp, cred_t *cred)
{
	...
	/* get instance number */
	ret = pcram_getinfo(NULL, DDI_INFO_DEVT2INSTANCE,
				(void *)*devp, (void**)&instance);
	if (ret != DDI_SUCCESS) {
		cmn_err(CE_NOTE,
...
pcram_close(dev_t dev, int flag, int otyp, cred_t *cred)
{
	...
	if (ret != DDI_SUCCESS) {
		cmn_err(CE_NOTE, "pcram_close: pcram_getinfo failed\n");
		return (ENXIO);
		/* NOTREACHED */
	}
...
pcram_print(dev_t dev, char *str)
{
	...
	ret = pcram_getinfo(NULL, DDI_INFO_DEVT2INSTANCE,
					(void *)dev, (void**)&instance);
	if (ret != DDI_SUCCESS) {
		cmn_err(CE_NOTE, "pcram_print: pcram_getinfo failed\n");
		return (ENXIO);
		/* NOTREACHED */
	}
...
pcram_read(dev_t dev, struct uio *uiop, cred_t *credp)
{
	...
	ret = pcram_getinfo(NULL, DDI_INFO_DEVT2INSTANCE,
				(void *)dev, (void**)&instance);
	if (ret != DDI_SUCCESS) {
		cmn_err(CE_NOTE, "pcram_read: pcram_getinfo failed\n");
		return (ENXIO);
		/* NOTREACHED */
	}
	...
	if (pcram_debug & PCRAM_DEBUG_SIZE) {
		...
		if (PCRAM_CARD_PRESENT(rs)) {
			...
			/*
			 * Try to get the virtual address
			 *	from the handle
			 */
			if (csx_GetMappedAddr(rs->access_handle,
				(void **)&csbuf) == CS_SUCCESS) {
				error = uiomove((caddr_t)(csbuf +
					remainder_wsize),
				...
				/*
				 * If we can not get the virtual
				 *	address, just allocate pbuf
...
static int
pcram_ioctl(dev_t dev, int cmd, int arg, int mode, cred_t *credp,
							int *rvalp)
{
	int			i;
	int			instance;
	...
	struct	dk_allmap	dka;	    /* support SunVTS */
	struct	dk_map		*dkmap;
	pcram_state_t		*rs;
	...
	ret = pcram_getinfo(NULL, DDI_INFO_DEVT2INSTANCE,
				(void *)dev, (void**)&instance);
	if (ret != DDI_SUCCESS) {
		cmn_err(CE_NOTE, "pcram_ioctl: pcram_getinfo failed\n");
		return (ENXIO);
		/* NOTREACHED */
	}
	...
	case DKIOCGAPART:
		 ...
		 */
		for (i = 0; i < 1; i++) {
			dkmap->dkl_cylno = (daddr_t)0;
			dkmap->dkl_nblk = (daddr_t)nblks;
			dkmap++;
		}

		if (ddi_copyout((caddr_t)&dka,
		    (caddr_t)arg, sizeof (struct dk_allmap), mode)) {
			return (EFAULT);
		}
		return (0);
		/* NOTREACHED */
...
{
	register int 	i;
	uint_t 	cardoffset = offset;
	register uchar_t	*hostmempt;

	hostmempt = (u_char*)(rs->host_sp);
...
static int pcram_card_sizing(pcram_state_t *rs)
{
	...
	while (offset < MAX_CARD_SIZE) {
		...
		cm_next_addr = csx_Get8(rs->access_handle,
					(uint_t)next_offset);
		restore_data = cm_next_addr;

		/* Write this location with test_pattern */
		csx_Put8(rs->access_handle, (uint_t)next_offset,
						test_pattern);
...
pcram_card_insertion(pcram_state_t *rs)
{
	...
	if (pcram_debug & PCRAM_DEBUG_TRACE) {
	    cmn_err(CE_CONT, "pcram_card_insertion: socket %d \n"
		"\tBase 0x%x rs->access_handle 0x%x\n"
		"\tSize 0x%x rs->win_size 0x%x\n",
		rs->sn,
		win_req.Base.base, (int)rs->access_handle,
...
pcram_build_region_list(pcram_state_t *rs,
				mem_region_t **rlist, u_long flags)
{
	mem_region_t		*mr;
	convert_speed_t		convert_speed;
...
pcram_get_bpbfat_info(pcram_state_t *rs, mem_region_t *mr)
{
	...
	csx_RepGet8(rs->access_handle, (uchar_t *)&bootblk, (uint_t)0,
...
pcram_get_solaris_info(pcram_state_t *rs, mem_region_t *mr)
{
	...
	csx_RepGet8(rs->access_handle, (uchar_t *)&label, (uint_t)0,
 ...
 */
static mem_region_t *
pcram_get_firstnext_region(mem_region_t *mrp, u_long flags,
					u_long type, u_long *first)
{
