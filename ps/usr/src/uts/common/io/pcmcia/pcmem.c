...
#ident "@(#)pcmem.c	1.23     96/05/20 SMI"
 ...
 */
static int pcmem_identify(dev_info_t *dip);
...
static int
pcmem_identify(dev_info_t *dip)
{
	char	*name;

	name = ddi_get_name(dip);

	if (strcmp(name, "pcmem") == 0) {
		...
		ddi_ctl_enum_t ctlop, void *arg, void *result)
{
	...
	case DDI_CTLOPS_INITCHILD:
		...
		cissp = 0;
		sprintf(name, "%d,%d", techreg, cissp);
