 ...
 * All rights reserved.
 */
#pragma ident	"@(#)fdc.c	1.39	96/06/26 SMI"
...
static u_char rwretry = 4;
static u_char skretry = 3;
static u_char configurecmd[4] = {FO_CNFG, 0, 0x0F, 0};
static u_char recalcmd[2] = {FO_RECAL, 0};
static u_char senseintcmd = FO_SINT;
...
int fdrw(struct fcu_obj *, int, int, int, int, int, caddr_t, u_int);
 ...
 * Function prototypes
 */
int encode(xlate_tbl_t *tablep, int val, u_char *rcode);
...
static u_int fdc_intr(caddr_t arg);
static void fdwatch(caddr_t arg);
static void fdmotort(caddr_t arg);
static int fdrecover(struct fdcntlr *);
static int fdc_motorsm(struct fcu_obj *, int, int);
static int fdc_statemach(struct fdcntlr *);
int fdc_docmd(struct fdcntlr *, u_char *, u_char);
int fdc_result(struct fdcntlr *, u_char *, u_char);
    ...
    void *arg, void *result)
{
	...
	FCERRPRINT(FDEP_L0, FDEM_ATTA, ("fdc_bus_ctl: cmd= %x\n", ctlop));
	...
	case DDI_CTLOPS_INITCHILD:
	{
		...
		sprintf(name, "%d,%d", cntlr, FDUNIT(unit));
...
fdc_probe(register dev_info_t *dip)
{
	...
	FCERRPRINT(FDEP_L3, FDEM_ATTA, ("fdc_probe: dip %x\n", dip));
	...
	len = sizeof (cntrl);
	if (ddi_prop_op(DDI_DEV_T_ANY, dip, PROP_LEN_AND_VAL_BUF,
	    DDI_PROP_DONTPASS, "unit", (caddr_t)&cntrl, &len) !=
	    DDI_PROP_SUCCESS)
		return (DDI_PROBE_FAILURE);
...
fdc_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	FCERRPRINT(FDEP_L3, FDEM_ATTA, ("fdc_attach: dip %x\n", dip));

	switch (cmd) {
	case DDI_ATTACH:
		len = sizeof (cntlr_num);
		if (ddi_prop_op(DDI_DEV_T_ANY, dip, PROP_LEN_AND_VAL_BUF,
		    DDI_PROP_DONTPASS, "unit", (caddr_t)&cntlr_num, &len) !=
		    DDI_PROP_SUCCESS) {
			FCERRPRINT(FDEP_L3, FDEM_ATTA,
			    ("fdc_attach failed: dip %x\n", dip));
			return (DDI_FAILURE);
		}
		...
		if (ddi_get_iblock_cookie(dip, (u_int)0, &fcp->c_iblock) !=
		    DDI_SUCCESS) {
			cmn_err(CE_WARN,
			    "fdc_attach: cannot get iblock cookie");
			goto no_attach;
		}
		sprintf(name, "fdc%dlock", ctlr);
		mutex_init(&fcp->c_lock, name, MUTEX_DRIVER, fcp->c_iblock);
		intr_set = 1;

		/* setup interrupt handler */
		if (ddi_add_intr(dip, (u_int)0, NULL,
		...
		ddi_dmae_getlim(dip, &fdc_dma_lim);

		sprintf(name, "fdc%ddoreg", ctlr);
		mutex_init(&fcp->c_dorlock, name, MUTEX_DRIVER, fcp->c_iblock);
		sprintf(name, "fdc%dio", ctlr);
		cv_init(&fcp->c_iocv, name, CV_DRIVER, fcp->c_iblock);

		sprintf(name, "fdc%dsel", ctlr);
		sema_init(&fcp->c_selsem, 1, name, SEMA_DRIVER, NULL);

		sprintf(name, "fdc%d", ctlr);
...
fdc_propinit1(struct fdcntlr *fcp, int cntlr)
{
	...
	if (ddi_prop_op(DDI_DEV_T_ANY, dip, PROP_LEN_AND_VAL_BUF,
	    DDI_PROP_DONTPASS, "ioaddr", (caddr_t)&value, &len)
	    == DDI_PROP_SUCCESS)
		fcp->c_regbase = (ushort) value;
	else
		switch (cntlr) {
		case 0:
			fcp->c_regbase = FCR_BASE;
			break;
		case 1:
			fcp->c_regbase = 0x370;
			break;
	...
	if (ddi_prop_op(DDI_DEV_T_ANY, dip, PROP_LEN_AND_VAL_BUF,
	    DDI_PROP_DONTPASS, "dmachan", (caddr_t)&value, &len)
	    == DDI_PROP_SUCCESS)
		fcp->c_dmachan = (ushort) value;
	else
		switch (cntlr) {
		case 0:
			fcp->c_dmachan = 2;
...
fdc_propinit2(struct fdcntlr *fcp, int cntlr)
{
	...
	if (ddi_prop_op(DDI_DEV_T_ANY, dip, PROP_LEN_AND_VAL_BUF,
		...
		static u_char perpindcmd[2] = {FO_PERP, 0};
		static u_char versioncmd = FO_VRSN;
		u_char result;
...
fdc_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	FCERRPRINT(FDEP_L3, FDEM_ATTA, ("fdc_detach: dip %x\n", dip));
	...
	case DDI_DETACH:
		...
		ddi_dmae_release(fcp->c_dip, fcp->c_dmachan);
...
fdc_abort(struct fcu_obj *fjp)
{
	...
	FCERRPRINT(FDEP_L3, FDEM_RESE, ("fdc_abort\n"));
	if (fcp->c_curunit == unit) {
		...
		if (fcp->c_flags & FCFLG_WAITING) {
			...
			ddi_dmae_stop(fcp->c_dip, fcp->c_dmachan);
		}
		mutex_exit(&fcp->c_lock);
		drv_usecwait(500);
		return (DDI_SUCCESS);
...
fdc_dkinfo(struct fcu_obj *fjp, struct dk_cinfo *dcp)
{
	...
	strncpy((char *)&dcp->dki_cname, ddi_get_name(fcp->c_dip), DK_DEVLEN);
	...
	strncpy((char *)&dcp->dki_dname, ddi_get_name(fjp->fj_dip), DK_DEVLEN);
...
fdc_select(struct fcu_obj *fjp, int funit, int on)
{
	...
	if (on) {
		...
		fdc_motorsm(fjp, FMI_STARTCMD, fjp->fj_drive->fdd_motoron);

		if (fdcspdchange(fcp, fjp, fjp->fj_attr->fda_rotatespd))
			/* 3D drive requires 500 ms for speed change */
			fdc_motorsm(fjp, FMI_RSTARTCMD, 5);

		fcp->c_digout = (fcp->c_digout & ~FD_DRSEL) | (unit & FD_DRSEL);
		outb(fcp->c_regbase + FCR_DOR, fcp->c_digout);

		mutex_exit(&fcp->c_dorlock);
		fcp->c_csb.csb_drive = (u_char)unit;
	} else {
		FCERRPRINT(FDEP_L2, FDEM_DSEL,
		    ("fdc_select unit %d: off\n", funit));

		mutex_enter(&fcp->c_dorlock);

		fcp->c_digout |= FD_DRSEL;
		outb(fcp->c_regbase + FCR_DOR, fcp->c_digout);
		fdc_motorsm(fjp, FMI_IDLECMD, fjp->fj_drive->fdd_motoroff);
...
{
	fdcsense_drv(fjp->fj_fdc, funit & 3);
...
fdresetchng(struct fcu_obj *fjp, int funit)
{
	...
	FCERRPRINT(FDEP_L2, FDEM_CHEK, ("fdmediachng unit %d\n", funit));
...
fdrecalseek(struct fcu_obj *fjp, int funit, int arg, int execflg)
{
	...
	FCERRPRINT(FDEP_L2, FDEM_RECA, ("fdrecalseek unit %d to %d\n",
	    funit, arg));

	csb = &fcp->c_csb;
	csb->csb_cmd[1] = (u_char)unit;
	if (arg < 0) {			/* is recal... */
    ...
    int sector, struct buf *bp)
{
	...
	FCERRPRINT(FDEP_L2, FDEM_RW, ("fdrwbuf unit %d\n", funit));
	...
	csb->csb_cmd[1] = (u_char)(unit | ((head & 0x1) << 2));
	csb->csb_cmd[2] = (u_char)cyl;
	csb->csb_cmd[3] = (u_char)head;
	csb->csb_cmd[4] = (u_char)sector;
	encode(sector_size, fjp->fj_chars->fdc_sec_size, &csb->csb_cmd[5]);
...
out:
	if (csb->csb_dmahandle) {
		ddi_dma_free(csb->csb_dmahandle);
		csb->csb_dmahandle = NULL;
	}
	return (rval);
}
    ...
    int sector, caddr_t bufp, u_int len)
{
	...
	FCERRPRINT(FDEP_L1, FDEM_RW, ("fdrw unit %d\n", funit));
	...
	csb->csb_cmd[4] = (u_char)sector;
	encode(sector_size, fjp->fj_chars->fdc_sec_size, &csb->csb_cmd[5]);
	csb->csb_cmd[6] = (u_char)max(fjp->fj_chars->fdc_secptrack, sector);
	csb->csb_cmd[7] = fjp->fj_attr->fda_gapl;
	csb->csb_cmd[8] = 0xFF;
	...
	if (ddi_dma_addr_setup(fcp->c_dip, &kas, bufp, len, dmar_flags,
	    DDI_DMA_SLEEP, 0, &dmalimits, &csb->csb_dmahandle) != DDI_SUCCESS) {
		rval = EINVAL;
		goto out;
	}
...
fdtrkformat(struct fcu_obj *fjp, int funit, int cyl, int head, int filldata)
{
	...
	FCERRPRINT(FDEP_L2, FDEM_FORM, ("fdformattrk unit %d cyl=%d, hd=%d\n",
	    funit, cyl, head));
	...
	*csb->csb_cmd = FO_FRMT | FO_MFM;
	...
	encode(sector_size, fjp->fj_chars->fdc_sec_size, &csb->csb_cmd[2]);
	csb->csb_cmd[3] = numsctr = fjp->fj_chars->fdc_secptrack;
	csb->csb_cmd[4] = fjp->fj_attr->fda_gapf;
	csb->csb_cmd[5] = (u_char)filldata;

	csb->csb_npcyl = (u_char)(cyl * fjp->fj_chars->fdc_steps);
	 ...
	 */
	fmdatlen = 4 * numsctr;
	dp = fmdatp = (u_char *)kmem_alloc(fmdatlen, KM_SLEEP);
	...
	if (ddi_dma_addr_setup(fcp->c_dip, &kas, (caddr_t)fmdatp, fmdatlen,
...
fdrawioctl(struct fcu_obj *fjp, int funit, caddr_t arg)
{
	...
	csb->csb_ncmds = (u_char)fdrp->fdr_cnum;
	...
	case FO_FRMT:
		csb->csb_npcyl = *(u_char *)(fdrp->fdr_addr) *
	...
	if (csb->csb_opflags & (CSB_OFDMARD | CSB_OFDMAWT)) {
		...
		if (ddi_dma_addr_setup(fcp->c_dip, &kas, fdrp->fdr_addr,
		    (u_int)fdrp->fdr_nbytes, dmar_flags, DDI_DMA_SLEEP, 0,
...
int
encode(xlate_tbl_t *tablep, int val, u_char *rcode)
{
	do {
		if (tablep->value >= val) {
			*rcode = tablep->code;
...
fdcquiesce(register struct fdcntlr *fcp)
{
	...
	FCERRPRINT(FDEP_L2, FDEM_RESE, ("fdcquiesce fcp %x\n", fcp));
...
{
	static u_char readidcmd[2] = {FO_RDID | FO_MFM, 0};

	readidcmd[1] = csb->csb_cmd[1];
	return (fdc_docmd(fcp, readidcmd, 2));
}

int
fdcseek(struct fdcntlr *fcp, int unit, int cyl)
{
	static u_char seekabscmd[3] = {FO_SEEK, 0, 0};

	FCERRPRINT(FDEP_L0, FDEM_RECA, ("fdcseek unit %d to cyl %d\n",
	    unit, cyl));
	seekabscmd[1] = (u_char)unit;
	seekabscmd[2] = (u_char)cyl;
...
fdcsense_chng(struct fdcntlr *fcp, int unit)
{
	...
	FCERRPRINT(FDEP_L0, FDEM_SCHG, ("fdcsense_chng unit %d\n", unit));
...
{
	static u_char sensedrvcmd[2] = {FO_SDRV, 0};
	u_char senser;
	int rval;

	sensedrvcmd[1] = (u_char)unit;
...
{
	static u_char perpindcmd[2] = {FO_PERP, 0};
	static u_char specifycmd[3] = {FO_SPEC, 0, 0};
...
fdcspdchange(struct fdcntlr *fcp, struct fcu_obj *fjp, int rpm)
{
	...
	FCERRPRINT(FDEP_L1, FDEM_SCHG, ("fdcspdchange: %d rpm\n", rpm));
	...
	case PC87322:
		{
		u_char nscmodecmd[5] = {FO_MODE, 0x02, 0x00, 0xC8, 0x00};
...
fdc_motorsm(struct fcu_obj *fjp, int input, int timeval)
{
	...
	case FMI_TIMER:		/* timer expired */
		...
		case FMS_DELAY:
			...
			break;
		case FMS_KILLST:
			fcp->c_motort[unit] = timeout(fdmotort, (caddr_t)fjp,
			    HZ);
	...
	if (rval) {
		FCERRPRINT(FDEP_L4, FDEM_EXEC,
		    ("fdc_motorsm: unit %d  bad input %d or bad state %d\n",
		    fjp->fj_unit, input, old_mstate));
#if 0
		cmn_err(CE_WARN,
		    "fdc_motorsm: unit %d  bad input %d or bad state %d\n",
		    fjp->fj_unit, input, old_mstate);
		...
		FCERRPRINT(FDEP_L0, FDEM_EXEC,
		    ...
		    fjp->fj_unit, input, old_mstate, fcp->c_mtrstate[unit]));
	return (rval);
}
...
fdc_exec(struct fdcntlr *fcp, int sleep, int change)
{
	...
	mutex_enter(&fcp->c_lock);
	FCERRPRINT(FDEP_L0, FDEM_EXEC, ("fdc_exec: sleep %x change %x\n",
	    sleep, change));
	csb = &fcp->c_csb;
	unit = csb->csb_drive;
	fjp = fcp->c_unit[unit];
...
retry:
	...
	if (change && fdcsense_chng(fcp, unit)) {
		 ...
		 */
		mutex_enter(&fcp->c_dorlock);
		fdc_motorsm(fjp, FMI_RSTARTCMD, fjp->fj_drive->fdd_motoron);
		mutex_exit(&fcp->c_dorlock);
	}
	...
	if (csb->csb_dmahandle) {
		...
		ddi_dmae_prog(fcp->c_dip, &dmaereq, &csb->csb_dmacookie,
		    fcp->c_dmachan);
	}
...
fdcheckdisk(struct fdcntlr *fcp, int unit)
{
	...
	u_char save_cmd, save_cd1, save_npcyl;

	ASSERT(MUTEX_HELD(&fcp->c_lock));
	FCERRPRINT(FDEP_L1, FDEM_CHEK, ("fdcheckdisk unit %d\n", unit));
	...
	*csb->csb_cmd = FO_SEEK;
	csb->csb_cmd[1] = (u_char)unit;
	csb->csb_npcyl = (u_char)newcyl;
	fcp->c_flags |= FCFLG_WAITING;

	if (fcp->c_mtrstate[unit] != FMS_ON && fcp->c_motort[unit])
		/*
...
fdrecover(struct fdcntlr *fcp)
{
	...
	FCERRPRINT(FDEP_L2, FDEM_RECO, ("fdrecover unit %d\n", csb->csb_drive));
	...
	case FXS_RESET:		/* cntlr reset */
		FCERRPRINT(FDEP_L4, FDEM_RECO,
		    ("fd unit %d: %s error: st0=0x%x pcn=%d\n", csb->csb_drive,
		    fdcmds[*csb->csb_cmd & 0x1f].cmdname,
	...
	case FXS_DOWT:		/* waiting on operation */
		if (csb->csb_opflags & (CSB_OFDMARD | CSB_OFDMAWT)) {
			ddi_dmae_getcnt(fcp->c_dip, fcp->c_dmachan, &residual);
			FCERRPRINT(FDEP_L2, FDEM_RECO,
("fd unit %d: %s error: dma count=0x%x residual=0x%x\n",
			    csb->csb_drive,
			    fdcmds[*csb->csb_cmd & 0x1f].cmdname,
		...
		if ((fjp->fj_flags & (FUNIT_UNLABELED | FUNIT_LABELOK)) &&
			...
			if (csb->csb_retrys++ < csb->csb_maxretry &&
				...
				if (csb->csb_opflags &
					...
					FCERRPRINT(FDEP_L4, FDEM_RECO,
("fd unit %d: %s error: st0=0x%x st1=0x%x st2=0x%x\n",
					    csb->csb_drive,
fdcmds[*csb->csb_cmd & 0x1f].cmdname,
					    *csb->csb_rslt, csb->csb_rslt[1],
					    csb->csb_rslt[2]));
	...
	if ((pendstate == (MS_RQM | MS_DIO | MS_CB) || pendstate == MS_RQM) &&
		...
		if (newstate == FXS_END) {
			...
			if (csb->csb_dmahandle && !csb->csb_cmdstat &&
				 ...
				 */
				ddi_dma_segtocookie(csb->csb_dmasegment,
				    &off, &blklen, &csb->csb_dmacookie);

				ddi_dmae_prog(fcp->c_dip, NULL,
			...
			cmn_err(CE_PANIC, "fdc_statemach: unknown state");
		} else
			/* restore waiting flag */
			fcp->c_flags |= FCFLG_WAITING;
fi_exit:
...
fdc_statemach(struct fdcntlr *fcp)
{
	...
	case FXS_START:		/* start of operation */
		...
		time = (unsigned int)csb->csb_timer * HZ / 10;
		if (time == 0)
			time = 2 * HZ;
		fcp->c_timeid = timeout(fdwatch, (caddr_t)fcp, time);
	...
	case FXS_MTRON:		/* motor is at speed */
		if (fcp->c_mtrstate[unit] != FMS_ON) {
			/* how did we get here ?? */
			cmn_err(CE_PANIC, "fdc: selected but motor off");
		}
		if (fcp->c_curpcyl[unit] != -1 && *csb->csb_cmd != FO_RECAL)
			goto nxs_seek;
		recalcmd[1] = (u_char)unit;
...
nxs_cmpl:
	...
	case FXS_KILL:		/* quiesce cntlr by reset */
		(void) fdcquiesce(fcp);
		fcp->c_timeid = timeout(fdwatch, (caddr_t)fcp, 2 * HZ);
		csb->csb_xstate = FXS_RESET;
		break;
	...
	FCERRPRINT(FDEP_L1, FDEM_EXEC, ("fdc_statemach unit %d: %d -> %d\n",
 ...
 */
int
fdc_docmd(struct fdcntlr *fcp, u_char *oplistp, u_char count)
{
	register int ntries;
 ...
 */
int
fdc_result(struct fdcntlr *fcp, u_char *rsltp, u_char rcount)
{
	register int ntries;
	u_char *abresultp = rsltp;
	u_char stat;
	int laxative = 7;
