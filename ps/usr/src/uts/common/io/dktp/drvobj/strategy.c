...
#pragma ident	"@(#)strategy.c	1.23	96/05/27 SMI"
...
_init(void)
{
	objmgr_ins_entry("qfifo", (opaque_t)qfifo_create, "strategy");
	objmgr_ins_entry("qmerge", (opaque_t)qmerge_create, "strategy");
	objmgr_ins_entry("qsort", (opaque_t)qsort_create, "strategy");
	objmgr_ins_entry("qtag",  (opaque_t)qtag_create, "strategy");
	objmgr_ins_entry("dsngl", (opaque_t)dsngl_create, "strategy");
	objmgr_ins_entry("dmult", (opaque_t)dmult_create, "strategy");
	objmgr_ins_entry("duplx", (opaque_t)duplx_create, "strategy");
	objmgr_ins_entry("adapt", (opaque_t)adapt_create, "strategy");
...
fc_init(register struct fc_data *fcdp, opaque_t tgcom_objp, opaque_t que_objp,
	void *lkarg)
{
	mutex_init(&fcdp->ds_mutex, "ds_s", MUTEX_DRIVER, lkarg);
...
fc_start_kstat(struct fc_data *fcdp, char *devtype, int instance)
{
	...
	if (!fcdp->ds_kstat) {
		if (fcdp->ds_kstat = kstat_create("sd", instance, NULL, "disk",
...
{
	mutex_init(&qfp->q_mutex, "q_s", MUTEX_DRIVER, lkarg);
	return (DDI_SUCCESS);
}
