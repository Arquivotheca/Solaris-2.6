...
static char *gda_name(u_char cmd, char **cmdvec);
...
static char *
gda_name(u_char cmd, char **cmdvec)
{
	while (*cmdvec != NULL) {
		if (cmd == **cmdvec) {
...
void
gda_log(dev_info_t *dev, char *label, u_int level, const char *fmt, ...)
{
	auto char name[256];
	auto char buf [256];
	va_list ap;
    ...
    char **cmdvec, char **senvec)
{
	...
	(void) sprintf(buf, "Error for command '%s'\tError Level: %s",
		gda_name(*(u_char *)pktp->cp_cdbp, cmdvec),
	...
	(void) sprintf(buf, "Sense Key: %s\n",
		gda_name(*(u_char *)pktp->cp_scbp, senvec));
