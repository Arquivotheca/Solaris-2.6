...
#pragma	ident "@(#)dadk.c	1.33	96/05/27 SMI"
 ...
 */
static void dadk_restart(struct cmpkt *pktp);
...
_init(void)
{
	...
	if (dadk_debug & DENT)
		PRF("dadk_init: call\n");
#endif
	objmgr_ins_entry("dadk", (opaque_t)dadk_create, OBJ_MODGRP_SNGL);
...
dadk_probe(struct dadk *dadkp, int kmsflg)
{
	...
	dadkp->dad_extp->tg_rmb =
	dadkp->dad_rmb = devp->sd_inq->inq_rmb;
...
dadk_iob_alloc(struct dadk *dadkp, daddr_t blkno, long xfer, int kmsflg)
{
	...
	bp->b_un.b_addr = 0;
	if (ddi_iopb_alloc((dadkp->dad_sd)->sd_dev,
 ...
 */
		if (!pktp->cp_private) {
			...
			bbhckp = BBH_HTOC(dadkp->dad_bbhobjp, pktp->cp_private);
...
static void
dadk_restart(struct cmpkt *pktp)
{
...
/*	check error code table						*/
	...
	if (scb == DERR_BUSY) {
		(void) timeout(dadk_restart, (caddr_t)pktp, DADK_BSY_TIMEOUT);
	}
	...
	do {
		...
		if (state != dadkp->dad_iostate) {
			...
			if (state == DKIO_INSERTED) {
				 ...
				 */
				(void) timeout(cv_broadcast,
				    (caddr_t)&dadkp->dad_state_cv,
