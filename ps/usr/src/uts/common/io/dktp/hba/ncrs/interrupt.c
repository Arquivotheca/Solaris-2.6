...
#pragma	ident	"@(#)interrupt.c	1.9	96/06/03 SMI"
			...
			unchar	 istat )
{
	...
	NDBG16(("ncr_process_intr: instance=%d: istat=0x%x\n",
		ddi_get_instance(ncrp->n_dip), istat));
...
/***/
/***	/* if reading both registers need to delay between accesses */
/***	if ((istat & (NB_ISTAT_SIP | NB_ISTAT_DIP))
/***			== (NB_ISTAT_SIP | NB_ISTAT_DIP))
/***		/* manual says delay 12 BCLK, 1 usec. seems about right */
/***		drv_usecwait(1);
/***/
			...
			ulong	 action )
{
	...
	if (action & NACTION_MSG_REJECT) {
		...
		nptp->nt_errmsgbuf[1] = MSG_REJECT;
	...
	if (action & NACTION_MSG_PARITY) {
		...
		nptp->nt_errmsgbuf[1] = MSG_MSG_PARITY;
		NCR_START_SCRIPT(ncrp, NSS_ERR_MSG);
	...
	if (action & NACTION_INITIATOR_ERROR) {
		...
		nptp->nt_errmsgbuf[1] = MSG_INITIATOR_ERROR;
		NCR_START_SCRIPT(ncrp, NSS_ERR_MSG);
