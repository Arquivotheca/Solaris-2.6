...
#pragma	ident	"@(#)ncr_hba.c	1.6	96/08/12 SMI"
...
static 	ulong	gsc_options = HBA_CB_DISABLE;
...
scsi_create_cbthread(ddi_iblock_cookie_t lkarg, int sleep)
{
	...
	cv_init(&cbt->cb_cv, "cbt_callback_cv", CV_DRIVER, NULL);
	mutex_init(&cbt->cb_mutex, "cbt_callback_mutex", MUTEX_DRIVER, lkarg);
...
scsi_run_cbthread(opaque_t cbdhdl, struct scsi_cmd *cmd)
{
	...
	if (ddi_in_panic() || !cbt || (gsc_options & HBA_CB_DISABLE)) {
		(*cmd->cmd_pkt.pkt_comp)(cmd);
		return;
	}
