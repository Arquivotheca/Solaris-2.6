...
#pragma	ident	"@(#)init.c	1.6	96/02/12 SMI"
			...
			bool_t	 savethem )
{
	...
	if (savethem) {
		while (nregs-- > 0) {
			...
			*regbufp = ddi_io_getb(ncrp->n_handle,
					ncrp->n_reg+nrsp->nr_reg) &
					    nrsp->nr_bits;
			regbufp++;
			nrsp++;
		}
	...
	while (nregs-- > 0) {
		tmp = ddi_io_getb(ncrp->n_handle, ncrp->n_reg + nrsp->nr_reg) &
			nrsp->nr_bits;
		ddi_io_putb(ncrp->n_handle, ncrp->n_reg + nrsp->nr_reg,
			(tmp | *regbufp));
		regbufp++;
		nrsp++;
	}
		...
		int	 lun )
{
	...
	nptp->nt_lun = lun;
	nptp->nt_selectparm.nt_sdid = NCR_ENCODE_ID(ncrp, target);
	...
	nptp->nt_cmd.count	= sizeof(nptp->nt_cdb);
	...
	nptp->nt_dsa_physaddr	= NCR_KVTOP((unchar *)nptp);
}
...
ncr_target_init( ncr_t *ncrp )
{
	...
	pages = ((NTARGETS * NLUNS_PER_TARGET) + numperpage - 1) / numperpage;
