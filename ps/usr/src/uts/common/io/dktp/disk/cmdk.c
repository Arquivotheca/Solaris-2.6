...
#pragma ident	"@(#)cmdk.c	1.51	96/06/13 SMI"
...
static int cmdkioctl(dev_t, int, int, int, cred_t *, int *);
...
char _depends_on[] =
	"drv/objmgr misc/snlb misc/scdk misc/dadk misc/strategy";
...
_init(void)
{
	...
	mutex_init(&cmdk_attach_mutex, "cmdk_attach_mutex",
		MUTEX_DRIVER, NULL);
...
cmdkprobe(dev_info_t *devi)
{
	...
	sema_init(&dkp->dk_semoclose, 1, "cmdk_o", SEMA_DRIVER, NULL);
 ...
 */
static int
cmdkioctl(dev_t dev, int cmd, int arg, int flag, cred_t *cred_p, int *rval_p)
{
	int 		instance;
...
/* 		Unit Information 					*/
		...
		strcpy(info->dki_dname, ddi_get_name(dkp->dk_dip));
...
/*	search for proper disk label object				*/
	for (i = 0, dklbp = dkp->dk_lb; i < CMDK_LABEL_MAX; i++, dklbp++) {
		...
		if (DKLB_OPEN(dkp->dk_lbobjp) == DDI_SUCCESS)
			return (TRUE);
	}
...
cmdk_create_obj(dev_info_t *devi, struct cmdk *dkp)
{
	...
	if ((objmgr_load_obj(flc_keyvalp) != DDI_SUCCESS) ||
		...
		QUE_FREE(queobjp);
		objmgr_destroy_obj(que_keyvalp);
		cmn_err(CE_WARN,
	...
	if ((objmgr_load_obj(dsk_keyvalp) != DDI_SUCCESS) ||
		...
		objmgr_destroy_obj(que_keyvalp);
		FLC_FREE(flcobjp);
		objmgr_destroy_obj(flc_keyvalp);
		cmn_err(CE_WARN,
	...
	if (strcmp(dsk_keyvalp, "scdk") == 0) {
		TGDK_INIT_X(CMDK_TGOBJP(dkp), devp, flcobjp,
			queobjp, &cmdk_bbh_obj, NULL, cmdk_devstatus, dkp);
	} else {
...
cmdk_destroy_obj(dev_info_t *devi, struct cmdk *dkp, int unload)
{
	...
	objmgr_destroy_obj(que_keyvalp);
	objmgr_destroy_obj(flc_keyvalp);
	objmgr_destroy_obj(dsk_keyvalp);
...
cmdk_create_lbobj(dev_info_t *devi, struct cmdk *dkp)
{
	...
	for (i = 0, mystatus = DDI_FAILURE; i < CMDK_LABEL_MAX; i++, dklbp++) {
		...
		if ((objmgr_load_obj(dklbp->dkl_name) != DDI_SUCCESS) ||
			...
			DKLB_INIT(dklbp->dkl_objp, CMDK_TGOBJP(dkp),
					NULL);
...
cmdk_destroy_lbobj(dev_info_t *devi, struct cmdk *dkp, int unload)
{
	...
	for (i = 0; i < CMDK_LABEL_MAX; i++, dklbp++) {
		...
		objmgr_destroy_obj(dklbp->dkl_name);
		if (unload)
...
{
	mutex_init(&dkp->dk_pinfo_lock, "cmdk_pinfo_mutex",
		   MUTEX_DRIVER, NULL);
	cv_init(&dkp->dk_pinfo_cv, "cmdk_pinfo_cv", CV_DRIVER, NULL);
	dkp->dk_pinfo_state = CMDK_PARTINFO_INVALID;
	return;
}
