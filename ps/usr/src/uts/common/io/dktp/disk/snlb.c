...
#pragma ident	"@(#)snlb.c	1.44	96/09/20 SMI"
...
static int snlb_open(struct sn_label *snlbp);
...
_init(void)
{
	objmgr_ins_entry("snlb", (opaque_t)snlb_create, OBJ_MODGRP_SNGL);
	...
	register void *lkarg)
{
	...
	mutex_init(&snlbp->s_mutex, "snlb_s", MUTEX_DRIVER, lkarg);
	rw_init(&snlbp->s_rw_mutex, "snlb_s_rw", RW_DRIVER, lkarg);
	return (DDI_SUCCESS);
}
...
static int
snlb_open(struct sn_label *snlbp)
{
	int	rc;
...
snlb_preplb(struct sn_label *snlbp, int uidx)
{
	...
	snlb_chk(&sdp->s_dklb, &(sdp->s_dklb.dkl_cksum));
