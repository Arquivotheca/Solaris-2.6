...
#pragma ident	"@(#)pcser_manuspec.h	1.10	95/08/01 SMI"
...
typedef struct pcser_manuspec_t {
	u_long		flags;		/* matching flags */
	u_long		manufacturer;	/* manufacturer ID */
	u_long		card;		/* card ID */
	u_long		txbufsize;	/* Tx FIFO buffer size */
	u_long		rxbufsize;	/* Rx FIFO buffer size */
	u_long		fifo_enable;	/* Tx/Rx FIFO enable code */
	u_long		fifo_disable;	/* Tx/Rx FIFO disable code */
	u_long		auto_rts;	/* Auto RTS enable code */
	u_long		auto_cts;	/* Auto CTS enable code */
	u_long		ready_delay_1;	/* READY delay before config in mS */
	u_long		ready_delay_2;	/* READY delay after config in mS */
	u_long		config_index;	/* config index */
	u_long		config_address;	/* config regs address */
	u_long		present;	/* config regs present mask */
	u_long		addr_lines;	/* IO addr lines decoded */
	u_long		length;		/* length of IO range */
	u_long		modem_base;	/* base of UART registers */
	u_long		CD_ignore_time;	/* mS to ignore CD changes */
...
#define	PT_VAR_OFFSET(v)	((void *)((unsigned)&pcser_manuspec[0].v - \
						(unsigned)&pcser_manuspec[0]))
