...
#pragma ident	"@(#)fdc.h	1.20	96/05/27 SMI"
 ...
 */
struct fdattr {
	ushort fda_rotatespd;		/* rotational speed */
	ushort fda_intrlv;		/* interleave factor */

	u_char fda_gapl;		/* gap 3 length */
	u_char fda_gapf;		/* gap 3 length for format */
};
...
struct fdisk {
	 ...
	 */
	u_long	d_regopen[OTYPCNT - 1];
	u_long	d_lyropen[NDKMAP];	/* Layered open counters */
	 ...
	 */
	u_long		d_exclmask;	/* set to indicate exclusive open */
	...
	u_char		d_bsec;		/* encoded bytes_per_sector */
	u_char		d_drate;	/* encoded data_rate */
	u_char		d_motor;	/* motor-on bit */

	u_char		d_hutsrt;	/* encoded head unload & step_rate */
	u_char		d_hlt;		/* encoded head load time */
	u_char		d_dtl;		/* dtl code */

	int	d_media_timeout;	/* media detection timeout */
	int	d_media_timeout_id;	/* media detection timeout id */
	enum dkio_state d_media_state;	/* up-to-date media state */
	int		d_ejected;
	kcondvar_t	d_statecv;	/* condition var for media state */

	u_long		d_vtoc_bootinfo[3];	/* from label */
	u_long		d_vtoc_version;
...
struct fdcsb {
	...
	u_char	csb_npcyl;	/* new physical cylinder number */
	u_char	csb_drive;	/* floppy unit number */
	...
	u_char	csb_timer;	/* op timer, in 0.1 sec */
	u_char	csb_maxretry;	/* maximum retries this operation */
	u_char	csb_retrys;	/* how may retrys done so far */
	u_char	csb_ourtrys;	/* how may over/underrun retrys done so far */
	u_char	csb_status;	/* status returned from hwintr */
	u_char	csb_cmdstat;	/* if 0 then success, else failure */
	u_char	csb_cmd[10];	/* command to send to chip */
...
struct fdcntlr {
	...
	int		c_motort[NFDUN]; /* motor timer id */
	 ...
	 */
	u_char		c_digout;
	u_char		c_drate;	/* only 82072 and 82077AA controllers */
	u_char		c_config;	/* DSR on PC/AT with 8272A */
	u_char		c_mstat;
	u_char		c_data;
	u_char		c_digin;

	u_char		c_bsec;		/* encoded bytes_per_sector */
	u_char		c_hutsrt;	/* encoded head unload & step_rate */
	u_char		c_hlt;		/* encoded head load time */
};
...
struct fcu_obj {
	u_long		fj_flags;	/* state information */
	...
	dev_info_t	*fj_dip;
	ushort		fj_rotspd;	/* rotational speed */
	u_long		fj_unit;
