...
#pragma ident	"@(#)pcser_var.h	1.41	96/04/18 SMI"
...
typedef struct pcser_cftable_params_t {
	u_char		config_index;
	u_long		addr_lines;	/* IO addr lines decoded */
	u_long		length;		/* length of IO range */
	u_long		pin;		/* PRR bits valid mask */
	unsigned	modem_vcc;
	unsigned	modem_vpp1;
	unsigned	modem_vpp2;
	u_long		modem_base;	/* base of UART registers */
} pcser_cftable_params_t;

typedef struct pcser_cftable_t {
	u_long			desireability;	/* desireability factor */
...
typedef struct pcser_cis_vars_t {
	u_long		flags;		/* general capability flags */
	u_long		present;	/* config register present flags */
	char		prod_strings[CISTPL_VERS_1_MAX_PROD_STRINGS]
					    [CIS_MAX_TUPLE_DATA_LEN];
	u_long		major_revision;	/* card major revision level */
	u_long		minor_revision;	/* card minor revision level */
	u_long		manufacturer_id;	/* manufacturer ID */
	u_long		card_id;	/* card ID */
	u_long		config_base;	/* base offset of config registers */
	...
	u_long		modem_base;	/* base of UART registers */
	/* UART features */
	u_long		txbufsize;	/* Tx FIFO buffer size */
	u_long		rxbufsize;	/* Rx FIFO buffer size */
	...
	u_long		ready_delay_1;	/* READY delay before config in mS */
	u_long		ready_delay_2;	/* READY delay after config in mS */
...
typedef struct pcser_unit_t {
	...
	int			ready_timeout_id;
	int			readywait_timeout_id;
