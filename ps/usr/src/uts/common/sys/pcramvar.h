...
#pragma ident	"@(#)pcramvar.h	1.42	96/04/19 SMI"
 ...
 */
struct pcmm_info {
	u_short pcmm_type;	/* supported media type */
};
 ...
 */
#define	MUTEX_CONTROLER_NAME	"pcram_state mutex"
#define	CV_SOFT_NAME		"pcram_state condvar"
...
typedef struct mem_region_t {
	u_long	region_num;	/* region number */
	u_long	rflags;		/* region flags */
	u_long	flags;		/* device tuple flags */
	u_long	speed;		/* device speed in device */
				/* speed code format */
 ...
 */
struct	bootblock {
	u_char  sig[3];		/* Jump to boot code */
	u_char  oem[8];		/* OEM name & version */
	u_char  bps[2];		/* bytes per sector */
	u_char  alloc;		/* sectors per allocation unit */
	u_char  ressec[2];	/* reserved sectors */
	u_char  fats;		/* number of FATS */
	u_char  nrd[2];		/* number of root directory entries */
	u_char  tsec[2];	/* total number of sectors in */
				/* logical volume */
	u_char  media;		/* media descriptor byte */
	u_char  secfat[2];	/* sectors per FAT */
	u_char  sectrack[2];	/* sectors per track */
	u_char  heads[2];	/* number of heads */
	u_char  misc[484];
};
