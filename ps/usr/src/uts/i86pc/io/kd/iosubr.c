...
#pragma	ident "@(#)iosubr.c	1.4	96/06/02 SMI"
...
/*ARGSUSED*/
int
enableio(unsigned short *iolist)
{
#ifdef STUB_FOR_SUN
...
/* Grow tss_bitmap size, if needed */
	if (highest_io) {
		...
		if ((newsz = (int)((iobm + iobmcnt) - (char *)u.u_tss)) <=
		    u.u_sztss)
...
/*ARGSUSED*/
int
disableio(unsigned short *iolist)
{
#ifdef STUB_FOR_SUN
...
int
validproc(proc_t *procp, pid_t pid)
{
	proc_t *pp;
