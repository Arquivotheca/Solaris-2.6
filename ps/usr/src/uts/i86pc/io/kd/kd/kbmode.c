...
#pragma ident	"@(#)kbmode.c	1.7	96/06/19 SMI"
...
/*121*/ {{' ', ' ', K_NOP, K_NOP, K_NOP, K_NOP, K_NOP, K_NOP}, 0xff, L_O },
/*122*/ {{K_NOP, K_NOP, K_NOP, K_NOP, K_NOP, K_NOP, K_NOP, K_NOP}, 0xff, L_O },
/*123*/ {{' ', ' ', K_NOP, K_NOP, K_NOP, K_NOP, K_NOP, K_NOP}, 0xff, L_O },
/*124*/ {{K_NOP, K_NOP, K_NOP, K_NOP, K_NOP, K_NOP, K_NOP, K_NOP}, 0xff, L_O },
/*125*/ {{'\\', '|', K_NOP, K_NOP, K_NOP, K_NOP, K_NOP, K_NOP}, 0xff, L_O },
...
switch_kb_mode(int kbmode)
{
	 ...
	 */
	xmit8042(3, KBD_RCMD, &i);

	/*
	* Write a new command byte if and only if it is necessary.
	*/
	if ((kbmode == KBM_XT && (i & KCMD_PCC) == 0) ||
	    (kbmode == KBM_AT && (i & KCMD_PCC) != 0)) {
		xmit8042(3, KBD_WCMD, (unsigned char *) 0);
		if (kbmode == KBM_XT) {
			/* turn on PC compatibility */
			xmit8042(0, i | KCMD_PCC, (unsigned char *) 0);
		} else {
			/* turn off PC compatibility */
			xmit8042(0, i & ~KCMD_PCC, (unsigned char *) 0);
		}
	}
}
...
void
kbio_setmode(queue_t *qp, mblk_t *mp, struct iocblk *iocp)
{
...
#ifdef	DEBUG
	cmn_err(CE_NOTE, "mp->b_cont is 0x%x", mp->b_cont);
#endif	/* DEBUG */
...
#ifdef	DEBUG
	cmn_err(CE_NOTE, "getting kbmode");
#endif	/* DEBUG */

	kbmode = *(int *) mp->b_cont->b_rptr;

#ifdef	DEBUG
	cmn_err(CE_NOTE, "kbmode = %d", kbmode);
#endif	/* DEBUG */
	...
	case KBM_AT:
#ifdef	DEBUG
		cmn_err(CE_NOTE, "setting cr_kbmode to %d", kbmode);
#endif	/* DEBUG */
		chp->ch_charmap_p->cr_kbmode = kbmode;
#ifdef	DEBUG
		cmn_err(CE_NOTE, "calling switch_kb_mode(%d)", kbmode);
#endif	/* DEBUG */
		switch_kb_mode(kbmode);
#ifdef	DEBUG
		cmn_err(CE_NOTE, "called switch_kb_mode(%d)", kbmode);
#endif	/* DEBUG */
	...
	default:
#ifdef	DEBUG
		cmn_err(CE_NOTE, "invalid kbmode (%d)", kbmode);
#endif	/* DEBUG */
		ws_iocnack(qp, mp, iocp, EINVAL);
	}
}
