...
#pragma	ident "@(#)ws_tcl.c	1.6	96/07/30 SMI"
...
/* prototypes for ws_ functions used here */
extern channel_t	*ws_activechan(wstation_t *);
extern int		ws_activate(wstation_t *, channel_t *, int);
extern channel_t	*ws_getchan(wstation_t *, int);


/* function prototypes */
void	tcl_handler(wstation_t *, mblk_t *, termstate_t *, channel_t *);
void	tcl_bs(wstation_t *, channel_t *, termstate_t *);
void	tcl_ht(wstation_t *, channel_t *, termstate_t *);
void	tcl_reset(wstation_t *, channel_t *, termstate_t *);
void	tcl_bht(wstation_t *, channel_t *, termstate_t *);
void	tcl_norm(wstation_t *, channel_t *, termstate_t *, ushort);
void	tcl_cursor(wstation_t *, channel_t *);
void	tcl_scrlup(wstation_t *, channel_t *, termstate_t *);
void	tcl_scrldn(wstation_t *, channel_t *, termstate_t *);
void	tcl_dchar(wstation_t *, channel_t *, termstate_t *);
void	tcl_dline(wstation_t *, channel_t *, termstate_t *);
void	tcl_iline(wstation_t *, channel_t *, termstate_t *);
void	tcl_eline(wstation_t *, channel_t *, termstate_t *);
void	tcl_escr(wstation_t *, channel_t *, termstate_t *);
void	tcl_clrtabs(termstate_t *);
void	tcl_ichar(wstation_t *, channel_t *, termstate_t *);
void	tcl_sparam(termstate_t *, int, ushort);
void	tcl_sfont(wstation_t *, channel_t *, termstate_t *);
int	tcl_ckchar(wstation_t *, channel_t *, termstate_t *);
int	tcl_ckline(wstation_t *, channel_t *, termstate_t *);
void	tcl_settab(termstate_t *);
int	tcl_curattr(channel_t *);
void	tcl_scrolllock(wstation_t *, channel_t *, int);
void	tcl_sendscr(wstation_t *, channel_t *, termstate_t *);
...
void
tcl_handler(register wstation_t *wsp, mblk_t *mp, termstate_t *tsp,
	channel_t *chp)
{
...
/* Enhanced Application Compatibility Support */
	...
	case TCL_SWITCH_VT: {
		...
		if (newvt >= 0)
			(void) ws_activate(wsp, ws_getchan(wsp, newvt),
					VT_NOFORCE);
		break;
	   } /* TCL_SWITCH_VT */
	...
	case TCL_INTENSITY_BKGRD_COLOR:
...
/* End Enhanced Application Compatibility Support */
	...
	default:
		cmn_err(CE_WARN, "ws: do not understand TCL %d",
...
void
tcl_bs(wstation_t *wsp, channel_t *chp, termstate_t *tsp)
{
	if (!tsp->t_col)
		return;
	tsp->t_col--;
...
void
tcl_ht(wstation_t *wsp, channel_t *chp, termstate_t *tsp)
{
...
void
tcl_reset(wstation_t *wsp, channel_t *chp, termstate_t *tsp)
{
	tsp->t_origin = 0;
...
void
tcl_bht(wstation_t *wsp, channel_t *chp, termstate_t *tsp)
{
...
void
tcl_norm(wstation_t *wsp, channel_t *chp, termstate_t *tsp, ushort ch)
{
...
void
tcl_cursor(wstation_t *wsp, channel_t *chp)
{
...
void
tcl_scrlup(wstation_t *wsp, channel_t *chp, termstate_t *tsp)
{
...
void
tcl_scrldn(wstation_t *wsp, channel_t *chp, termstate_t *tsp)
{
	short	row, col;
	ushort	cursor;
...
void
tcl_dchar(wstation_t *wsp, channel_t *chp, termstate_t *tsp)
{
...
void
tcl_dline(wstation_t *wsp, channel_t *chp, termstate_t *tsp) 
{
...
void
tcl_iline(wstation_t *wsp, channel_t *chp, termstate_t *tsp) 
{
...
void
tcl_eline(wstation_t *wsp, channel_t *chp, termstate_t *tsp)
{
...
void
tcl_escr(wstation_t *wsp, channel_t *chp, termstate_t *tsp)
{
 ...
 */
void
tcl_clrtabs(register termstate_t *tsp)
{
...
void
tcl_ichar(wstation_t *wsp, channel_t *chp, termstate_t *tsp)
{
...
#ifdef notdef
tcl_sparam(register termstate_t *tsp, register int paramcnt, register ushort newparam)
{
	register int	cnt;

	for (cnt = 0; cnt < paramcnt; cnt++) {
		if (tsp->t_ppar[cnt] == (ushort) -1)
...
void
tcl_sfont(wstation_t *wsp, channel_t *chp, termstate_t *tsp) 
{
...
int
tcl_ckchar(wstation_t *wsp, channel_t *chp, termstate_t *tsp)
{
...
int
tcl_ckline(wstation_t *wsp, channel_t *chp, termstate_t *tsp)
{
	register ushort avail;
...
void
tcl_scrolllock(register wstation_t *wsp, channel_t *chp, int arg)
{
	kbstate_t *kbp;
	mblk_t *tmp;
	ch_proto_t *protop;
 ...
 */
void
tcl_sendscr(wstation_t *wsp, channel_t *chp, termstate_t *tsp)
{
	...
	/*
	 * Disable echo by telling LDTERM that we'll do it (we won't, of
	 * course, because we want ECHO disabled). Generate an M_CTL message
	 * for LDTERM that tells it we'll do the canonical processing for echo.
	 */
	 ...
	 */
	mp1 = allocb(tsp->t_rows * (tsp->t_cols + 1), BPRI_HI);
	if (mp1 == (mblk_t *) NULL) {
		(*wsp->w_bell)(wsp, chp);
		freemsg(bp);
		freemsg(cbp);
	...
	putnext(chp->ch_qp, mp);	/* put the CH_NOSCAN msg on the queue */
