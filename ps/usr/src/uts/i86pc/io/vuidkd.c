...
#pragma	ident "@(#)vuidkd.c	1.8	94/08/30 SMI"
...
_init(void)
{
	...
	mutex_init(&vuidkd_lock, "vuidkd lock", MUTEX_DEFAULT, NULL);
    ...
    const int oflag, const int sflag, const cred_t *const crp)
{
	 ...
	 */
	qp->q_ptr = (caddr_t) &vuidkd_state;
	WR(qp)->q_ptr = (caddr_t) &vuidkd_state;
	mutex_exit(&vuidkd_lock);
...
vuidkd_rput(queue_t *const qp, mblk_t *mp)
{
	...
	case M_DATA: {
	    mblk_t		*bp;
	    register vuidkd_state_t *sp = (vuidkd_state_t *) qp->q_ptr;
...
{
	register vuidkd_state_t *sp;

	ASSERT(qp != NULL);
	ASSERT(mp != NULL);

	sp = (vuidkd_state_t *) qp->q_ptr;
