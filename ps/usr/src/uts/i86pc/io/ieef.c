...
#pragma ident "@(#)ieef.c	1.21	96/10/01 SMI"
...
u_int	ieefintr(gld_mac_info_t *);
static void	ieef_wdog(caddr_t arg);
...
static int ieef_configure(struct ieefinstance *ieefp, int promisc);
static int ieef_configure_ee100(struct ieefinstance *ieefp, int promisc);
static int ieef_configure_flash32(struct ieefinstance *ieefp, int promisc);
...
static int ieef_pci_get_irq(dev_info_t *devinfo);
...
_init(void)
{
	...
	mutex_init(&ieef_probe_lock, "MP probe protection", MUTEX_DRIVER, NULL);
...
ee100_eisa_probe(dev_info_t *devinfo)
{
	...
	if (adap_type == IEEF_HW_UNISYS)
		...
		/* I guess this is the product ID for the Unisys EISA card */
...
ee100_pci_probe(dev_info_t *devinfo)
{
	...
	ushort	vendor_id, device_id, iline, cmdreg;
	int	regbuf[3], pci_probe_result;
	...
	if ((vendor_id == IEEF_EE100_PCI_VENDOR) &&
		...
		{	pci_probe_result = DDI_PROBE_SUCCESS;
			...
			regbuf[0] =
				pci_config_getl(handle, PLXP_BAR_1_REGISTER);
			regbuf[0] &= 0xFFFFFFFC;
			(void) ddi_prop_create(DDI_DEV_T_NONE, devinfo,
					DDI_PROP_CANSLEEP, "ioaddr",
					(caddr_t)regbuf, sizeof (int));
		}
	}
...
static int ieef_pci_get_irq(dev_info_t *devinfo)
{
	...
	parent_devinfo = ddi_get_parent(devinfo);

	if (ddi_ctlops(devinfo, parent_devinfo, DDI_CTLOPS_XLATE_INTRS,
	    (caddr_t) intarr, ddi_get_parent_data(devinfo)) != DDI_SUCCESS) {
		cmn_err(CE_CONT, "ieef_pci_get_irq(): ddi_ctlops failed.\n");
		return (-1);
	}

	return (0);	/* always index 0 for PCI */
}
...
ieefattach(dev_info_t *devinfo, ddi_attach_cmd_t cmd)
{
	...
	if (ddi_getprop(DDI_DEV_T_ANY, devinfo, 0,
		...
		else macinfo->gldm_irq_index = (long)ieef_pci_get_irq(devinfo);
	...
	ieefp->ieef_speed = ddi_getprop(DDI_DEV_T_NONE, devinfo,
				DDI_PROP_DONTPASS, "speed", 0);

	if (ieefp->ieef_speed == 0) {
		cmn_err(CE_WARN, "ieef: Could not find speed property.\n");
		cmn_err(CE_WARN, "ieef: Will not attach.\n");
		goto afail;
	}
	...
	if ((ieefp->ieef_type != IEEF_HW_EE100_EISA) &&
		...
		(ieefp->ieef_speed != 10)) {
		cmn_err(CE_WARN, "ieef: Speed has be set incorrectly.\n");
		cmn_err(CE_WARN, "ieef: Will not attach.\n");
		goto afail;
	}
	...
	/* reset the 82596/82556 controller, since booting over the net */
	/* might have left the network active */
	...
	ieefp->detaching = 0;
 	ieefp->wdog_lbolt = 0;
	ieefp->wdog_id = timeout(ieef_wdog, (caddr_t)macinfo, WDOGTICKS);
...
ieefdetach(dev_info_t *devinfo, ddi_detach_cmd_t cmd)
{
	gld_mac_info_t *macinfo;		/* GLD structure */
	struct ieefinstance *ieefp;		/* Our private device info */
	int i, more_cmds = 1, tbdnum;
	xmit_cmd_t *xcmd;
	volatile tbd_t  *tbd;
	volatile gen_cmd_t      *next;
...
int	ieef_burst = 0xc0, ieef_latched;
...
ieef_init_board(gld_mac_info_t *macinfo)
{
	...
	ddi_dmae_getlim(ieefp->ieef_dip, &limits);
...
#ifdef IEEFDEBUG
	if (ieefdebug & IEEFTRACE)
		cmn_err(CE_CONT, "ieef: shmem V: %x P: %x hg %x\n",
			(int) ieefp->kmem_map, (int) ieefp->pmem_map,
	...
	if ((ieefp->ieef_type == IEEF_HW_FLASH &&
		...
		outw(ieefp->ieef_ioaddr + ieefp->ieef_port,
		    (ushort)(((long)ieefp->pmem_map & 0xffff) | IEEF_NEWSCP));
		outw(ieefp->ieef_ioaddr + ieefp->ieef_port + 2,
			(ushort)((long)ieefp->pmem_map >> 16));
	}
...
ieef_prom(gld_mac_info_t *macinfo, int on)
{
	...
	ieefp->promiscuous = on;
	(void) ieef_configure(ieefp, on);
	return (0);
}
...
ieef_send(gld_mac_info_t *macinfo, mblk_t *mp)
{
	...
	if (ieefdebug & IEEFSEND)
		cmn_err(CE_CONT, "ieef_send(0x%x, 0x%x)",
			(int) macinfo, (int) mp);
#endif
	...
	do {
		...
		bcopy((caddr_t)mp->b_rptr, (caddr_t)
			ieefp->kmem_map->ieef_tbd[s].tbd_v_buffer + offset,
			length);
		mp->b_rptr = mp->b_wptr;
		offset += length;
		mp = mp->b_cont;
...
ieef_add_command(struct ieefinstance *ieefp, gen_cmd_t *cmd, int len)
{
	...
	if (ieefp->last_cb == 0xffff) {
		...
		/* lbolt value.						*/
 		drv_getparm(LBOLT, &ieefp->wdog_lbolt);
...
ieef_process_recv(gld_mac_info_t *macinfo, struct ieefinstance *ieefp)
{
	...
	while (rfd->rfd_status & CS_CMPLT) {
		...
		if (rfd->rfd_rbd == (rbd_t *)0xffffffff) {
			cmn_err(CE_CONT, "?ieef: rfd_rbd invalid status=0x%x\n",
						rfd->rfd_status);
			goto frame_done;
		}
		if ((ieefp->ieef_type == IEEF_HW_EE100_EISA) ||
			...
			if (status & (RFD_CRC_ERROR | RFD_ALIGNMENT_ERROR |
				...
				RFD_RECEIVE_COLLISION | RFD_LENGTH_ERROR)) {
...
ieef_configure_flash32(struct ieefinstance *ieefp, int promisc)
{
	...
	ccmd.conf_conf.cnf_hrdwr = (0x0 | ((ushort) promisc));
...
/*
long	ieef_program_burst = 210<<16 | 400;
*/
long	ieef_program_burst = 0;
long ieef_fifo_lim = 0xf;
...
ieef_configure_ee100(struct ieefinstance *ieefp, int promisc)
{
	...
	else ccmd.conf_conf.conf_bytes45 |= BIT_0;
...
ieef_configure(struct ieefinstance *ieefp, int promisc)
{
	if ((ieefp->ieef_type == IEEF_HW_EE100_EISA) ||
		(ieefp->ieef_type == IEEF_HW_EE100_PCI))
		return (ieef_configure_ee100(ieefp, promisc));
	else return (ieef_configure_flash32(ieefp, promisc));
}
...
ieef_wdog(caddr_t arg)
{
	...
	if ((val-ieefp->wdog_lbolt > TIMEOUT) && (ieefp->wdog_lbolt != 0)) {
		...
		if ((ieefp->ieef_type == IEEF_HW_FLASH &&
			...
			outw(ieefp->ieef_ioaddr + ieefp->ieef_port,
			    (ushort)(((long)ieefp->pmem_map & 0xffff) |
					IEEF_NEWSCP));
			outw(ieefp->ieef_ioaddr + ieefp->ieef_port + 2,
			    (ushort)((long)ieefp->pmem_map >> 16));
		}
		...
		ASSERT(ieefp->current_cb != 0xffff);
		...
		while (more_cmds) {
			...
			ieefp->cmdavail[i] = TRUE;

			/* Check for transmit command. */
			cmd = ieefp->kmem_map->ieef_cmds[i].cmd_status;
			if ((cmd & CS_CMD_MSK) == CS_CMD_XMIT) {
			    xcmd =
				(xmit_cmd_t *)&(ieefp->kmem_map->ieef_cmds[i]);
			    tbd = xcmd->xmit_v_tbd;
			    tbdnum = tbd - &ieefp->kmem_map->ieef_tbd[0];
			    ieefp->tbdavail[tbdnum] = TRUE;
			}
			next = ieefp->kmem_map->ieef_cmds[i].cmd_next;
			i = next - &ieefp->pmem_map->ieef_cmds[0];
		}
		...
		(void) ieef_configure(ieefp, ieefp->promiscuous);
		ieef_start_ru(ieefp);

		/* Reset the latch but don't clear pending interrupts. */
		if (ieefp->ieef_type != IEEF_HW_UNISYS) {
			tmp = inb(ieefp->ieef_ioaddr + 0x430);
			outb(ieefp->ieef_ioaddr + 0x430, tmp | 0x30);
			outb(ieefp->ieef_ioaddr + 0x430, tmp & ~0x30);
		}
		...
		for (s = 0; s < IEEF_NMCS; s++) {
			if (ieefp->mcs_addrs_valid[s] != 0) {
				bcopy((caddr_t)
				    (ieefp->mcs_addrs[s].ether_addr_octet),
				    (caddr_t)&(mcmd.mcs_addr[i * ETHERADDRL]),
				    ETHERADDRL);
				i++;
			}
