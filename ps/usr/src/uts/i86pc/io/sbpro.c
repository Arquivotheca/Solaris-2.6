...
#pragma ident   "@(#)sbpro.c	1.59    96/06/18 SMI"
...
_init(void)
{
	...
	for (i = 0; i < 4; i++) {
		mutex_init(&probe_mutex[i], "sbpro probe mutex", MUTEX_DRIVER,
			(void *) NULL);
		attached[i] = 0;
	}
...
sbpro_attach(dev_info_t *dip, ddi_attach_cmd_t cmd)
{
	...
	if (i != DDI_SUCCESS) {
		SBERRPRINT(SBEP_L4, SBEM_ATTA, (CE_WARN,
		    "sbpro attach: iopb_alloc(%d) failed (%d).",
					info->buflim, i));
		goto failed0;
	}
	...
	ddi_dmae_getlim(dip, &info->dma_limits);
	 ...
	 */
	mutex_init(&info->mutex, "sbpro mutex", MUTEX_DRIVER,
		(void *)info->iblock_cookie);

	cv_init(&info->cvopen, "sbpro cvopen", CV_DRIVER, NULL);
	cv_init(&info->cvclosewait, "sbpro cvclosewait", CV_DRIVER, NULL);
	...
	if (info->flags & SBPRO) {
		...
		ddi_dma_nextwin(info->stereo_byte_dmahand, 0, &dmawin);
		ddi_dma_nextseg(dmawin, 0, &dmaseg);
		ddi_dma_segtocookie(dmaseg, &offset, &length,
		    &info->sbpro_stereo_byte_cookie);
	}
...
failed2:
	if ((info->flags & SB16) && (info->dmachan8 != info->dmachan16))
		ddi_dmae_release(dip, info->dmachan16);

failed1:
	ddi_dmae_release(dip, info->dmachan8);
...
sbpro_detach(dev_info_t *dip, ddi_detach_cmd_t cmd)
{
	...
	case DDI_DETACH:
		...
		if (info->flags & SBPRO)
			ddi_dma_free(info->stereo_byte_dmahand);
...
sbpro_close(queue_t *q, int flag, cred_t *credp)
{
	...
	if (flags&(R_BUSY|W_BUSY))
		ddi_dmae_stop(info->dip, info->dmachan);
		...
		if (*mp->b_rptr & FLUSHW) {	/* flush the write queue */
			...
			*mp->b_rptr &= ~FLUSHW;
			...
			if (info->flags & W_BUSY) {
				 ...
				 */
				ddi_dmae_getcnt(info->dip, info->dmachan,
				    &count);
				if (info->bufnum == 1) {
...
sbpro_intr(caddr_t arg)
{
	...
	if (info->flags & READING) {
		...
		ddi_dma_sync(info->aidmahandle, 0, 0, DDI_DMA_SYNC_FORKERNEL);
		ASSERT(info->rdq);
...
load_write_buffer(SBInfo *info, int bufnum)
{
	...
	for (remain = maxcnt; ; ) {
		...
		ASSERT(info->aidmahandle);
		ddi_dma_sync(info->aidmahandle, 0, 0, DDI_DMA_SYNC_FORDEV);
...
sbpro_getdmahandle(SBInfo *info, int dmamode)
{
	...
	ddi_dma_nextwin(info->aidmahandle, (ddi_dma_win_t)(0), &dmawin);
	ddi_dma_nextseg(dmawin, (ddi_dma_seg_t)(0), &dmaseg);
	ddi_dma_segtocookie(dmaseg, &offset, &length, &info->dmacookie);
	...
	if (info->buflen * 2 != info->dmacookie.dmac_size) {
		cmn_err(CE_WARN, "sbpro: bad DMA cookie, size %d should be %d",
...
sbpro_progdmaeng(SBInfo *info, int der_command, int buf_flags)
{
	...
	ddi_dmae_prog(info->dip, &dmaereq, &info->dmacookie, info->dmachan);
}
...
dsp_dmahalt(SBInfo *info)
{
	...
	if (info->dsp_fastmode) {
		...
		info->dsp_fastmode = 0;
		ddi_dmae_stop(info->dip, info->dmachan);
		ddi_dmae_getcnt(info->dip, info->dmachan, &count);
		count &= ~1;
...
sbpro_fast_stereo_start(SBInfo *info)
{
	...
	ddi_dmae_prog(info->dip, &dmaereq, &info->sbpro_stereo_byte_cookie,
	    info->dmachan8);
...
probe_card(dev_info_t *dip, SBInfo *info)
{
	...
	if (i >= nregs) {
		cmn_err(CE_WARN,
		    "sbpro: reg property, I/O base address not specified");
	...
	if (index < 0 || index >= 4) {
		cmn_err(CE_WARN, "sbpro: Invalid I/O address specified in reg"
				" property (0x%x)", ioaddr);
		return (DDI_PROBE_FAILURE);
	}
...

	{
		...
		if (intr_val[i] != 2 && intr_val[i] != 5 && intr_val[i] != 7 &&
			intr_val[i] != 10) {
			cmn_err(CE_WARN, "sbpro: invalid interrupt value. Must"
					"be either 2,5,7 or 10");
			return (DDI_PROBE_FAILURE);
		}
	}
	...
	if (info != (SBInfo *)NULL) {
		sprintf(info->sbpro_devtype.version, "%d.%d", ver_hi, ver_low);
		SBERRPRINT(SBEP_L1, SBEM_PROB, (CE_NOTE,
...
check_sbpro_intr(dev_info_t *dip, SBInfo *info, int ioaddr, int dmachan8)
{
	...
	ddi_dma_nextwin(dmahand, 0, &dmawin);
	ddi_dma_nextseg(dmawin, 0, &dmaseg);
	ddi_dma_segtocookie(dmaseg, &offset, &length, &dmacookie);
	bzero((caddr_t)&dmaereq, sizeof (dmaereq));
	...
	for (iter = 0; iter < 3; iter++) {
		...
		ddi_dmae_prog(dip, &dmaereq, &dmacookie, dmachan8);
	...
	ddi_dma_free(dmahand);

failed1:
	ddi_remove_intr(dip, 0, iblock_cookie);
failed:
	ddi_dmae_release(dip, dmachan8);
	if (intr_count)
		return (DDI_PROBE_SUCCESS);
	else
		return (DDI_PROBE_FAILURE);
...
sbpro_pause(SBInfo *info, int record_info)
{
	...
	if (info->flags & (R_BUSY)) {
		...
		(void) dsp_reset(info->ioaddr, info);
		ddi_dmae_stop(info->dip, info->dmachan);
		ddi_dma_sync(info->aidmahandle, 0, 0,
