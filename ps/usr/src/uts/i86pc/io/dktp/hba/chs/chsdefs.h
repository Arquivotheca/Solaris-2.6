...
#pragma ident	"@(#)chsdefs.h	1.3	96/06/18 SMI"
...
int chs_dont_access(chs_t *const, const u_char, const u_char);

bool_t chs_in_any_sd_viper(chs_t *const,
    const u_char, const u_char, u_char *);
...
struct cmpkt *chs_dacioc_pktalloc(chs_unit_t *const, int,
    int, int, int *const);
ushort chs_dacioc_ubuf_len(chs_t *const, const u_char);
...
int chs_dacioc_valid_args(chs_t *const, chs_ccb_t *const,
    const ushort, chs_dacioc_t *const);
void chs_dacioc_callback(struct cmpkt *const);
int chs_dacioc_done(struct cmpkt *const,
    chs_ccb_t *const, chs_dacioc_t *const, int);
...
void chs_iosetup_viper(chs_ccb_t *, int, ulong, off_t, u_char, int);
...
bool_t		add_intr(dev_info_t *devi, u_int inumber,
			...
			caddr_t intr_arg);
bool_t		chs_find_irq(dev_info_t	*devi, unchar irq, int *intrp,
