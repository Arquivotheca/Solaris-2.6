...
#pragma	ident	"@(#)chs_interrupt.c	1.4	96/07/17 SMI"
...
chs_intr(caddr_t arg)
{
	...
	for (; CHS_IREADY(chs); ) {
		...
		while (CHS_GET_ISTAT(chs, hw_statp, 1) != 0) {
			...
			ccb = (chs_ccb_t *)chs_process_status(chs,
								hw_statp);
			if (ccb == NULL) {
	...
	while (ccb = (chs_ccb_t *)QueueRemove(doneqp)) {
		if (ccb->type == CHS_SCSI_CTYPE) {
			...
			if (!(pkt->pkt_flags & FLAG_NOINTR) &&
				    pkt->pkt_comp != NULL) {
				mutex_exit(&chs->mutex);
				scsi_run_cbthread(chs->scsi_cbthdl,
				    (struct scsi_cmd *)pkt);
				mutex_enter(&chs->mutex);
			}
		} else {
			...
			if (cmpkt->cp_passthru && cmpkt->cp_bp &&
			    cmpkt->cp_bp->b_back && (RWCMDP->flags &
								0x8000))
