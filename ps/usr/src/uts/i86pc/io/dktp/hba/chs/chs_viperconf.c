...
#pragma ident "@(#)chs_viperconf.c	1.4	96/07/15	SMI"
				...
				const u_char tgt)
{
	...
	/*
	 * For Viper the only state that a physical drive can be accessed
	 * directly  is STAND BY.
	 */
	int index;
	chs_dac_conf_viper_t *conf =
			(chs_dac_conf_viper_t *)(chs->conf);
...
void
chs_get_scsi_item_viper(chs_cdbt_t *cdbt,
