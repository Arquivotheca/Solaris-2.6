...
#pragma	ident	"@(#)chs_devops.c	1.3	96/06/18 SMI"
		...
		ddi_detach_cmd_t cmd)
{
	...
	MDBG1(("chs_detach: dip=%x, unit=%x", dip, unit));
	hba = unit->hba;
	ASSERT(hba != NULL);
...
chs_flush_cache(register dev_info_t *const dip, const ddi_reset_cmd_t cmd)
{
	...
	MDBG1(("chs_flush_cache: dip=%x, unit=%x", dip, unit));
...
/*	set up default customary properties 				*/
	 ...
	 * the chs parent devops for channel init and uninit.
	 */
		    chs_prop_default(dip, MSCSI_CALLPROP, (caddr_t)"y") ==
									0 ||
#endif
		...
		chs_prop_default(dip, "disk", (caddr_t)"scdk") == 0) {
			return (FALSE);
	}
		...
		chs_unit_t	*unit)
{
	...
	if ((tchs.ops = chs_hbatype(pdip, &regp, &reglen, FALSE)) ==
									NULL) {
		...
		chs_unit_t	*unitp)
{
	...
	chsp->scsi_cbthdl = scsi_create_cbthread(chsp->iblock_cookie,
		KM_NOSLEEP);
	if (chsp->scsi_cbthdl == NULL) {
		mutex_exit(&chsp->mutex);
		return (FALSE);
	}
	chsp->flags |= CHS_CBTHD_CREATED;
		...
		chs_unit_t	*unitp)
{
	...
	if (chs != NULL && !--chs->refcount &&
		...
		if (chs_cards == chs) {
			...
			for (chs_prev = chs_cards,
			    chs_tmp = chs_cards->next;
			    chs_tmp != NULL;
			    chs_prev = chs_tmp,
			    chs_tmp = chs_tmp->next)
				if (chs_tmp == chs)
					break;
			if (chs_tmp == NULL)
				cmn_err(CE_PANIC, "chs_unsetup: %x not in "
				    "cards list %x",
				    (int)chs,
				    (int)chs_cards);
		...
		if (chs->flags & CHS_INTR_SET) {
			ddi_remove_intr(chs->idip, chs->intr_idx,
					chs->iblock_cookie);
			cv_destroy(&chs->ccb_stk_cv);
			mutex_destroy(&chs->mutex);
			sema_destroy(&chs->scsi_ncdb_sema);
		}

		if (chs->flags & CHS_CBTHD_CREATED)
			scsi_destroy_cbthread(chs->scsi_cbthdl);
