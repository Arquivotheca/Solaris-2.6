...
#pragma ident	"@(#)him.c 1.5	94/07/12 SMI"
...
int scb_send (him_config *config_ptr, scb_struct *scb_pointer)
{
       ...
       (scb_pointer->SCB_Cmd != NO_OPERATION))
   {
    ...
    SET_STATE(config_ptr, sav_state);             /* ja 6/1  Ops !!*/
...
UBYTE int_handler (him_config *config_ptr)
{
   ...
   while ((int_status & ANYINT) || (ha_ptr->done_cmd))
   {
      ...
      if (int_status & (SEQINT | BRKINT))    /* Check for other interrupts */
      {
         ...
         {
            int_status = (int_status & ANYINT) | CDB_XFER_PROBLEM;
...
scb_struct *scb_ext_deque (him_struct *nha_ptr)
{
   ...
   if ((scb_index != 0xFF) &&
       (nha_ptr->actptr[scb_index] != NOT_DEFINED))
   {
...
void scb_LoadEntryTbl ( him_struct *ha_ptr, UWORD ScbMethod )
{
   if (ScbMethod == SMODE_NOSWAP)
   {
      ...
      ha_ptr->PreemptScb = scb_int_preempt;
   ...
   else
   {
      ...
      ha_ptr->PreemptScb = scb_ext_preempt;
...
void synnego (him_struct *ha_ptr,scb_struct *scb_ptr)
{
   UBYTE nego_reply,
         scratch_index = 0,
         current_rate = 0;   /* SunSoft */

   /* Handle no message response by explicitly deactivating
      synchronous transfers in scratch RAM and ScsiOption array. 
      This is similar to Message Reject handling, except we don't 
      transfer any messages. */
   if (extmsgo(ha_ptr,scb_ptr) != MIPHASE)
   {
      nego_reply = MSGNULL;	/* Need to deactivate synchrounous mode */
   }
   else				/* Get message in off bus */
   {
      nego_reply = INBYTE(ha_ptr->scsibusl);
   }
   ...
   switch (nego_reply)
   {
      ...
      /* SunSoft changes */
      /* Build index into transfer parameter array */
      scratch_index = (scb_ptr->SCB_Tarlun & TARGET_ID) >> 4;
      /* Disable synchronous mode and further negotiations */
      if (scb_ptr->SCB_Tarlun & CHANNEL)
         ha_ptr->BConfigPtr->Cfe_ScsiOption[scratch_index] &= WIDE_MODE;
      else
         ha_ptr->AConfigPtr->Cfe_ScsiOption[scratch_index] &= WIDE_MODE;
      if (scb_ptr->SCB_Tarlun & CHANNEL)    /* Array offset for channel */
         scratch_index += 8;
      current_rate = INBYTE((ha_ptr->xfer_option) + scratch_index) & WIDE_MODE;
      OUTBYTE((ha_ptr->xfer_option) + scratch_index, current_rate);
      OUTBYTE(ha_ptr->scsirate, current_rate);
      if (nego_reply == MSGNULL)       	    /* No message in, just exit */
         break;
      /* End of SunSoft changes */
