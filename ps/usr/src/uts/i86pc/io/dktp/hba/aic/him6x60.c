...
#include <sys/types.h>
#include <sys/dktp/aic/aic6x60.h>
#include <sys/dktp/aic/scsi6x60.h>
#include <sys/dktp/aic/scb6x60.h>
...
PRIVATE_FUNC VOID bitbucketAndABORT(HACB *hacb, UCHAR scbStatus);
PRIVATE_FUNC VOID dataInPIO(HACB *hacb);
PRIVATE_FUNC VOID dataOutPIO(HACB *hacb);
PRIVATE_FUNC VOID dataPhaseDMA(HACB *hacb);
PRIVATE_FUNC VOID debugTrace(HACB *hacb, UCHAR event, ULONG data0, ULONG data1);
PRIVATE_FUNC VOID deferredIsr(HACB *hacb);
PRIVATE_FUNC VOID initiateIO(HACB *hacb);
PRIVATE_FUNC VOID interpretMessageIn(HACB *hacb);
PRIVATE_FUNC VOID isr(HACB *hacb);
PRIVATE_FUNC VOID linkScb(SCB **queueHead, SCB *scb);
PRIVATE_FUNC VOID linkScbPreemptive(SCB **queueHead, SCB *scb);
PRIVATE_FUNC VOID negotiateSDTR(HACB *hacb);
PRIVATE_FUNC VOID prepareMessageOut(HACB *hacb, UCHAR message);
PRIVATE_FUNC VOID quiesceDmaAndSCSI(HACB *hacb);
PRIVATE_FUNC VOID reselection(HACB *hacb);
PRIVATE_FUNC VOID resetSDTR(HACB *hacb, USHORT busID);
PRIVATE_FUNC BOOLEAN samePhaseREQuest(HACB *hacb, BOOLEAN pioReady);
PRIVATE_FUNC VOID scsiBusFree(HACB *hacb);
PRIVATE_FUNC VOID scsiBusReset(HACB *hacb);
PRIVATE_FUNC VOID selection(HACB *hacb);
PRIVATE_FUNC VOID targetREQuest(HACB *hacb);
PRIVATE_FUNC BOOLEAN unlinkScb(SCB **queueHead, SCB *scb);
PRIVATE_FUNC VOID updateDataPointer(HACB *hacb);
PRIVATE_FUNC VOID updateSDTR(HACB *hacb, USHORT busID, UCHAR syncCycles,
			UCHAR syncOffset, BOOLEAN negotiationsCompleted);
PRIVATE_FUNC VOID watchdog(HACB *hacb);

extern INT memcmp(const VOID *ptr1, const VOID *ptr2, USHORT size) ;
extern VOID * memset(VOID *ptr,UCHAR val,USHORT size) ;
extern VOID * memcpy(VOID *dest,const VOID *src,USHORT size) ;
...
BOOLEAN
HIM6X60FindAdapter(AIC6X60_REG *baseAddress) 
{
   ...
   return( ((INPUT(scsiSeq) & (ENSELO | ENAUTOATNO | SCSIRSTO)) == 0)
	 		...
	 		&& ((INPUT(u2.sStat0) & (SELDO | SPIORDY)) == 0)
	  		&& (INPUT(sStat2) == SEMPTY)
	 		&& (INPUT(u4.sStat3) == 0)
...
USHORT
HIM6X60GetStackContents(HACB *hacb, VOID *stackContents,
			       USHORT maxStackSize) 
{
...
BOOLEAN
HIM6X60GetConfiguration(HACB *hacb) 
{
   ...
   if (config2 == 0xFF) {                       /* No jumpers available? */
      hacb->ac_u.ac_s.defaultConfiguration = TRUE;
      hacb->ownID = 7;
      hacb->IRQ = 11;
      hacb->dmaChannel = 0;
      hacb->ac_u.ac_s.checkParity = TRUE;
      hacb->ac_u.ac_s.fastSCSI = ((hacb->revision = INPUT(rev)) > 0);
      hacb->ac_u.ac_s.noDisconnect = FALSE;
      hacb->ac_u.ac_s.initiateSDTR = TRUE;
      hacb->ac_u.ac_s.useDma = FALSE;
   }
   else {                                     /* OK! Jumpered information */
      hacb->ownID = config1 & 0x07;
      hacb->IRQ = 9 + ((UCHAR)(config1 & 0x18)>> 3);
      if ((hacb->dmaChannel =((UCHAR)(config1 & 0x60) >> 5) != 0))
	 	hacb->dmaChannel += 4;
      hacb->ac_u.ac_s.checkParity = ((config1 & 0x80) == 0);

      if ((hacb->revision = INPUT(rev)) == 0)
	 	hacb->ac_u.ac_s.fastSCSI = FALSE;
      else if (hacb->signature == 0x52 || hacb->signature == 0)
	 	hacb->ac_u.ac_s.fastSCSI = ((config2 & 0x10) != 0);
      else
	 	hacb->ac_u.ac_s.fastSCSI = ((config2 & 0x01) != 0);

      hacb->ac_u.ac_s.noDisconnect = ((config2 & 0x04) == 0);
      hacb->ac_u.ac_s.initiateSDTR = ((config2 & 0x08) != 0);
      hacb->ac_u.ac_s.useDma = ((config2 & 0x80) != 0
		      					&& (hacb->revision > 0
								|| hacb->dmaChannel == 0));
   }
   ...
   hacb->ac_u.ac_s.synchronous = TRUE;
   hacb->clockPeriod = 50;
   hacb->negotiateSDTR = ((hacb->ac_u.ac_s.initiateSDTR) ? 0xFF : 0);
   ...
   hacb->sdtrMsg.transferPeriod = (hacb->ac_u.ac_s.fastSCSI)
				   					? (UCHAR)(2 * hacb->clockPeriod) >> 2
				   					:(UCHAR) (4 * hacb->clockPeriod) >> 2;
   ...
   if ( hacb->signature == 0x52
      ...
      for (i = 0; i <= 7; i++)
	 	if (i != hacb->ownID) {
	    	...
	    	if (scsiRate == 0x80)
	       		...
	       		hacb->negotiateSDTR &= ~(1 << i);
	       		hacb->syncCycles[i] = (((UCHAR)(scsiRate & SXFR)) >> 4 + 2);
     ...
     OUTPUT(dmaCntrl1, 0);
      ...
      hacb->ac_u.ac_s.initialReset = TRUE;            /* No BIOS has executed */
...
BOOLEAN
HIM6X60ResetBus(HACB *hacb, UCHAR scsiBus) 
{

   if (scsiBus == 0) {
      TRACE(hacb, RESET_BUS, (ULONG) hacb->ne_u.ne_s.activeScb, 0);
      DISABLE_TRACE;
...
HIM6X60Initialize(HACB *hacb) 
{
   ...
   hacb->ac_u.ac_s.fastSCSI &= ((hacb->revision = INPUT(rev)) > 0);
   if (!hacb->ac_u.ac_s.synchronous) {
      hacb->ac_u.ac_s.initiateSDTR = FALSE;
      hacb->ac_u.ac_s.initialReset |= (hacb->negotiateSDTR != 0xFF);
      hacb->negotiateSDTR = 0;
   } 
   else if (!hacb->ac_u.ac_s.initiateSDTR)
      hacb->negotiateSDTR = 0;

   hacb->ac_u.ac_s.useDma &= (hacb->revision > 0 || hacb->dmaChannel == 0);
   hacb->sdtrMsg.transferPeriod = (hacb->ac_u.ac_s.fastSCSI)
				   					? (UCHAR)((2 * hacb->clockPeriod)) >> 2
				   					: (UCHAR)((4 * hacb->clockPeriod)) >> 2;

   hacb->selectTimeLimit = ((hacb->selectTimeLimit + 31) / 32) * 32;
   ...
   hacb->dmaBusOnTime = MIN(15, hacb->dmaBusOnTime);
   hacb->dmaBusOffTime = MIN(15, hacb->dmaBusOffTime);
   ENABLE_TRACE;
   ...
   if (hacb->ac_u.ac_s.checkParity) {
	  hacb->sXfrCtl1Image |= ENSPCHK;
   ...
   OUTPUT(u1.scsiID, (UCHAR) (hacb->ownID << 4));  /* Our own SCSI ID */
   OUTPUT(u2.clrSInt0, 0xFF & ~SETSDONE);
   OUTPUT(u3.clrSInt1, 0xFF);
   OUTPUT(u5.clrSErr, CLRSYNCERR | CLRFWERR | CLRFRERR);
   OUTPUT(sIMode0, 0);                  /* Mask off all interrupt sources */
   OUTPUT(sIMode1, 0);
   OUTPUT(dmaCntrl0, RSTFIFO);          /* Clear data transfer FIFO */
   OUTPUT(u6.brstCntrl, (UCHAR)(hacb->dmaBusOnTime << 4 | hacb->dmaBusOffTime));
   ...
   if (hacb->ac_u.ac_s.initialReset
		|| INPUT(scsiSig) != 0) {     /* RST necessary? */
...
UCHAR
HIM6X60AbortSCB(HACB *hacb, SCB *scb, SCB *scbToAbort) 
{

   LUCB *lucb;
   SCB *nextScb;

   if (scb->length < sizeof(SCB))
      scb->scbStatus = SCB_INVALID_LENGTH;
   else {
      if ((lucb = HIM6X60GetLUCB(hacb, 0, scb->targetID, scb->lun)) == NULL)
	 scb->scbStatus = SCB_ABORT_FAILURE;
      else if (scbToAbort == hacb->ne_u.ne_s.activeScb) {         /* On the SCSI bus? */
	 scbToAbort->linkedScb = scb;                   /* Must notify later */
	 if (scbToAbort->scbStatus == SCB_PENDING)
	    scbToAbort->function = SCB_ABORT_REQUESTED; /* Ask for an ABORT */
	 else
	    HIM6X60ResetBus(hacb, 0);                   /* Too far gone! */
      } else if (scbToAbort == lucb->activeScb)         /* Already underway? */
	 if (scbToAbort->scbStatus == SCB_PENDING) {
	    scbToAbort->function = SCB_ABORT_REQUESTED;
	    scbToAbort->linkedScb = scb;
	 ...
	 lucb->activeScb = NULL;
	 if (lucb->queuedScb == NULL || hacb->cs_u.cs_s.queuesFrozen)
	    lucb->busy = FALSE;
	 else {
	    lucb->busy = TRUE;
...
UCHAR
HIM6X60QueueSCB(HACB *hacb, SCB *scb) 
{

   LUCB *lucb;
   UCHAR lun, targetID;
   SCB *nextScb, *queueFreezeScb;
   ...
   if (scb->length < sizeof(SCB))
	 ...
	 if (hacb->ac_u.ac_s.useDma && (scb->flags & SCB_DISABLE_DMA) == 0) {
	    scb->segmentAddress = HIM6X60GetPhysicalAddress(hacb, scb,
							  scb->dataPointer, 0,
							  &scb->segmentLength);
	    scb->segmentLength = MIN(scb->segmentLength, scb->dataLength);
	 ...
	 linkScb(&hacb->deferredScb, scb);
      else if (hacb->cs_u.cs_s.queuesFrozen || lucb->busy) {
	 ...
	 initiateIO(hacb);
      ...
      hacb->cs_u.cs_s.queuesFrozen = TRUE;
	 ...
	 HIM6X60CompleteSCB(hacb, queueFreezeScb);
      ...
      hacb->cs_u.cs_s.queuesFrozen = FALSE;
...
UCHAR
HIM6X60TerminateSCB(HACB *hacb, SCB *scb, SCB *scbToTerminate) 
{
   ...
   if (scb->length < sizeof(SCB))
	 ...
	 scb->scbStatus = SCB_TERMINATE_IO_FAILURE;
      else if (scbToTerminate == hacb->ne_u.ne_s.activeScb) {     /* On the SCSI bus? */
	 scbToTerminate->linkedScb = scb;               /* Must notify later */
	 if (scbToTerminate->scbStatus == SCB_PENDING)
	    scbToTerminate->function = SCB_TERMINATE_IO;
	 else
	    HIM6X60ResetBus(hacb, 0);                   /* Too late! */
      } else if (scbToTerminate == lucb->activeScb)     /* Already underway? */
	 if (scbToTerminate->scbStatus == SCB_PENDING) {
	    scbToTerminate->function = SCB_TERMINATE_IO;
	    scbToTerminate->linkedScb = scb;
...
initiateIO(HACB *hacb) 
{
   ...
   if (hacb->resetScb != NULL && hacb->cActiveScb == 0)
      ...
      hacb->ne_u.ne_s.activeScb = scb = hacb->eligibleScb;
	 ...
	 OUTPUT(dmaCntrl0, INTEN);              /* ...so make sure INTEN set */
      ...
      OUTPUT(u1.scsiID, (UCHAR) (hacb->ownID << 4 | hacb->busID));
...
HIM6X60IRQ(HACB *hacb) 
{
   if ((INPUT(dmaStat) & INTSTAT) != 0) {
	 ...
	 OUTPUT(dmaCntrl0, (UCHAR) (INPUT(dmaCntrl0) | INTEN));
      hacb->cs_u.cs_s.isrActive = FALSE;
      return(TRUE);
   } else
      return(FALSE);
...
BOOLEAN
HIM6X60ISR(HACB *hacb) 
{
   TRACE(hacb, ISR, (ULONG) hacb->ne_u.ne_s.activeScb, 0);
   if ((INPUT(dmaStat) & INTSTAT) != 0) {
      hacb->cs_u.cs_s.isrActive = TRUE;
      if (hacb->disableINT++ == 0)
	 OUTPUT(dmaCntrl0, (UCHAR) (INPUT(dmaCntrl0) & ~(INTEN | SWINT)));
...
VOID
deferredIsr(HACB *hacb) 
{

   hacb->cs_u.cs_s.deferredIsrActive = TRUE;
   ...
   while ((INPUT(dmaStat) & INTSTAT) != 0)
      isr(hacb);
   hacb->cs_u.cs_s.deferredIsrActive = FALSE;
...
watchdog(HACB *hacb) 
{
   ...
   } else if (   hacb->cs_u.cs_s.dmaActive
	      && (INPUT(dmaStat) & ATDONE) != 0
	      && (INPUT(u2.sStat0) & DMADONE) == 0) {
      HIM6X60LogError(hacb, hacb->ne_u.ne_s.activeScb, 0, hacb->busID, hacb->lun,
		      SCB_WARNING_DMA_HANG, hacb->scsiPhase);
      OUTPUT(dmaCntrl0, (UCHAR) (INPUT(dmaCntrl0) & ~ENDMA));
      OUTPUT(dmaCntrl0, (UCHAR) (INPUT(dmaCntrl0) | ENDMA));
   }
   if (hacb->cs_u.cs_s.dmaActive && hacb->revision == 1)
...
isr(HACB *hacb) 
{
   ...
   hacb->sStat0 = INPUT(u2.sStat0);
   hacb->maskedSStat0 = hacb->sStat0 & INPUT(sIMode0);
   hacb->sStat1 = INPUT(u3.sStat1);
   hacb->maskedSStat1 = hacb->sStat1 & INPUT(sIMode1);
   if (INPUT(u5.sStat4) != 0) {
      OUTPUT(u5.clrSErr, CLRSYNCERR | CLRFWERR | CLRFRERR);
      if (hacb->ne_u.ne_s.activeScb != NULL)
	 hacb->ne_u.ne_s.activeScb->scbStatus = SCB_PROTOCOL_ERROR;
      HIM6X60ResetBus(hacb, 0);
   } else if ((hacb->maskedSStat1 & (SELTO | SCSIRSTI | BUSFREE)) != 0)
      scsiBusFree(hacb);
   else if ((hacb->maskedSStat0 & SELINGO) != 0) {
      OUTPUT(u3.clrSInt1, CLRBUSFREE);     /* OK to watch for BUSFREE now */
      ...
      if (hacb->cs_u.cs_s.dmaActive)        /* If data phase DMA... */
	 quiesceDmaAndSCSI(hacb);               /* ...let it settle down */
      if ((INPUT(u3.sStat1) & REQINIT) == 0)
...
VOID
scsiBusFree(HACB *hacb) 
{

   LUCB *lucb;
   SCB *linkedScb, *nextScb, *queueFreezeScb, *scb = NULL;
   if (hacb->cs_u.cs_s.dmaActive) {  /* Do we need to quiesce any system DMA? */
      if (hacb->revision == 1)
	 HIM6X60Watchdog(hacb, NULL, 0);
      HIM6X60FlushDMA(hacb);
      hacb->cs_u.cs_s.dmaActive = FALSE;
   }
   OUTPUT(scsiSeq, 0);
   ...
   OUTPUT(u2.clrSInt0, 0xFF & ~(SETSDONE | CLRSELDI));
   OUTPUT(u3.clrSInt1, 0xFF);
   ...
   if ((hacb->maskedSStat1 & SCSIRSTI) != 0)
      scsiBusReset(hacb);
   else if (hacb->ne_u.ne_s.activeScb != NULL &&
			hacb->ne_u.ne_s.activeScb->length == sizeof(SCB) ) {
      scb = hacb->ne_u.ne_s.activeScb;
      if ((hacb->maskedSStat1 & SELTO) != 0) {
	 if (scb->function == SCB_EXECUTE)
	    scb->scbStatus = SCB_SELECTION_TIMEOUT;
	 else
	    HIM6X60ResetBus(hacb, 0);
      } else if (hacb->cs_u.cs_s.disconnectOK) {
	 hacb->cs_u.cs_s.disconnectOK = FALSE;
	 if (     hacb->scsiPhase == MESSAGE_OUT_PHASE
	       && SCB_STATUS(scb->scbStatus) == SCB_PENDING)
	    scb->scbStatus = (hacb->ne_u.ne_s.msgOut[0] == ABORT_MSG) ? SCB_ABORTED
							    : SCB_COMPLETED_OK;
      } else {
   ...
   hacb->busID = hacb->lun = DISCONNECTED;
   memset(&hacb->ne_u.nexus, 0, sizeof(hacb->ne_u.nexus));
   initiateIO(hacb);                            /* Check for more work */
   if ( scb != NULL &&							/* ADDED */
		scb->length == sizeof(SCB) ) {          /* Did an SCB just complete? */
      if ((linkedScb = scb->linkedScb) != NULL &&
			linkedScb->length == sizeof(SCB) )		/** ADDED **/
	 if (linkedScb->function == SCB_ABORT_REQUESTED)
	    if (scb->scbStatus == SCB_ABORTED)
	       linkedScb->scbStatus = SCB_COMPLETED_OK;
	    else
	       linkedScb->scbStatus = SCB_ABORT_FAILURE;
	 else if (linkedScb->function == SCB_TERMINATE_IO)
...
scsiBusReset(HACB *hacb) 
{
   ...
   OUTPUT(u3.clrSInt1, CLRSCSIRSTI);
	 ...
	 for (lun = 0; lun < 8; lun++)
	    if ((lucb = HIM6X60GetLUCB(hacb, 0, targetID, lun)) != NULL) {
	       ...
	       if (lucb->activeScb != NULL && 
				lucb->activeScb->length == sizeof(SCB)) {
   ...
   while ( hacb->eligibleScb != NULL  &&
			hacb->eligibleScb->length == sizeof(SCB) ) {
   /**
	**/
...
VOID
reselection(HACB *hacb) 
{

   UCHAR bitwiseID, busID;

   if (hacb->ne_u.ne_s.activeScb != NULL) {     /* A selection was underway? */
      hacb->ne_u.ne_s.activeScb = NULL;         /* If so, tidy up... */
      hacb->busID = hacb->lun = DISCONNECTED;
   }
   TRACE(hacb, RESELECTION, INPUT(u1.selID), 0);
   bitwiseID = INPUT(u1.selID) ^ 1 << hacb->ownID; /* What bits are present? */
   ...
   if (busID == DISCONNECTED) {         /* 6X60 is NOT supposed to interrupt */
      ...
      OUTPUT(u2.clrSInt0, CLRSELDI);       /* So that SELDI can clear later */
      OUTPUT(u3.clrSInt1, CLRBUSFREE);     /* OK to watch for BUSFREE now */
...
selection(HACB *hacb) 
{
   ...
   LUCB *lucb;
   SCB *scb = hacb->ne_u.ne_s.activeScb;


   hacb->ne_u.ne_s.msgOutLen = 1; /* There will ALWAYS be at least one message byte */
   if (scb == NULL)
      hacb->ne_u.ne_s.msgOut[0] = BUS_DEVICE_RESET_MSG;
   else {
      ...
      lucb->activeScb = scb;
      hacb->ne_u.ne_s.dataPointer = scb->dataPointer;     /* Current data pointer */
      hacb->ne_u.ne_s.dataLength = scb->dataLength;
      hacb->ne_u.ne_s.dataOffset = scb->dataOffset;
      hacb->ne_u.ne_s.segmentAddress = scb->segmentAddress;
      hacb->ne_u.ne_s.segmentLength = scb->segmentLength;
      if (scb->function == SCB_BUS_DEVICE_RESET)
	 hacb->ne_u.ne_s.msgOut[0] = BUS_DEVICE_RESET_MSG;
      else if (   hacb->ac_u.ac_s.noDisconnect
	       || (scb->flags & SCB_DISABLE_DISCONNECT) != 0)
	 hacb->ne_u.ne_s.msgOut[0] = IDENTIFY_MSG | scb->lun;
      else
	 hacb->ne_u.ne_s.msgOut[0] = IDENTIFY_MSG | PERMIT_DISCONNECT | scb->lun;
      if (scb->function == SCB_ABORT_REQUESTED)
	 hacb->ne_u.ne_s.msgOut[hacb->ne_u.ne_s.msgOutLen++] = ABORT_MSG;
      else if (scb->function == SCB_TERMINATE_IO)
	 hacb->ne_u.ne_s.msgOut[hacb->ne_u.ne_s.msgOutLen++] = TERMINATE_IO_PROCESS_MSG;
      else if (scb->function == SCB_RELEASE_RECOVERY)
	 hacb->ne_u.ne_s.msgOut[hacb->ne_u.ne_s.msgOutLen++] = RELEASE_RECOVERY_MSG;
      else if (   (hacb->negotiateSDTR & 1 << busID) != 0
	       && (scb->flags & SCB_DISABLE_NEGOTIATIONS) == 0) {
	 memcpy(&hacb->ne_u.ne_s.msgOut[hacb->ne_u.ne_s.msgOutLen], &hacb->sdtrMsg,
		sizeof(hacb->sdtrMsg));
	 hacb->ne_u.ne_s.msgOutLen += sizeof(hacb->sdtrMsg);
      }
   }
   TRACE(hacb, SELECTION, hacb->ne_u.ne_s.msgOut[0],
	 (hacb->ne_u.ne_s.msgOutLen == 1) ? 0 : hacb->ne_u.ne_s.msgOut[1]);
   OUTPUT(portA, LED_ON);               /* External indication of BSY */
   OUTPUT(u3.clrSInt1, CLRBUSFREE);        /* In case we missed SELINGO */
...
quiesceDmaAndSCSI(HACB *hacb) 
{
   ...
   if (hacb->revision == 1)
      HIM6X60Watchdog(hacb, NULL, 0);
   if (hacb->scsiPhase == DATA_OUT_PHASE)
      while (   (INPUT(u2.sStat0) & DMADONE) == 0
	     && (   (INPUT(u3.sStat1) & PHASEMIS) == 0
		 || (   (INPUT(dmaStat) & ATDONE) == 0
...
VOID
updateDataPointer(HACB *hacb) 
{

   ULONG fifoCount = 0, xferCount;
   SCB *scb = hacb->ne_u.ne_s.activeScb;

   if (hacb->cs_u.cs_s.bitBucket) {                       /* BITBUCKET mode... */
      hacb->cs_u.cs_s.bitBucket = FALSE;
      OUTPUT(sXfrCtl0, CHEN | CLRSTCNT | CLRCH);
      OUTPUT(sXfrCtl0, CHEN);
      OUTPUT(sXfrCtl1, hacb->sXfrCtl1Image);
      OUTPUT(dmaCntrl0, RSTFIFO);
      TRACE(hacb, UPDATE_DATA_POINTER, 0, INPUT(dmaCntrl0));
   } else if ( !hacb->ac_u.ac_s.useDma     /* ...or automatic PIO... */
	 ...
	 if (hacb->scsiPhase == DATA_IN_PHASE)
	    ...
	    if (fifoCount != 0) {
	       hacb->ne_u.ne_s.dataPointer -= fifoCount;
	       hacb->ne_u.ne_s.dataLength += fifoCount;
	       hacb->ne_u.ne_s.dataOffset -= fifoCount;
	       hacb->ne_u.ne_s.segmentLength += fifoCount;
	    }
	 }
	 OUTPUT(sXfrCtl0, CHEN | CLRSTCNT | CLRCH);
	 OUTPUT(sXfrCtl0, CHEN);
	 ...
	 xferCount = hacb->scsiCount;
	       ...
	       || hacb->scsiCount < hacb->ne_u.ne_s.segmentLength)
	 xferCount = hacb->scsiCount
		      - (INPUT(fifoStat) + (INPUT(sStat2) & SFCNT));
      else
	 xferCount = hacb->ne_u.ne_s.segmentLength;
      hacb->ne_u.ne_s.dataPointer += xferCount;
      hacb->ne_u.ne_s.dataLength -= xferCount;
      hacb->ne_u.ne_s.dataOffset += xferCount;
      UPDATE_PHYSICAL_ADDRESS(hacb->ne_u.ne_s.segmentAddress, xferCount);
      hacb->ne_u.ne_s.segmentLength -= xferCount;
      if ((INPUT(u3.sStat1) & PHASEMIS) == 0)
	 OUTPUT(dmaCntrl0, (UCHAR) (INPUT(dmaCntrl0) & ~ENDMA));
      else {
	 OUTPUT(sXfrCtl0, CHEN | CLRSTCNT | CLRCH);
	 OUTPUT(sXfrCtl0, CHEN);
...
targetREQuest(HACB *hacb) 
{
   ...
   OUTPUT(u3.clrSInt1, CLRPHASECHG);
   switch (hacb->scsiPhase) {
	 ...
	 else if (hacb->ne_u.ne_s.dataLength == 0)
	    bitbucketAndABORT(hacb, SCB_DATA_OVERRUN);
	 else if (hacb->ac_u.ac_s.useDma && (scb->flags & SCB_DISABLE_DMA) == 0)
	    dataPhaseDMA(hacb);
	 else if (hacb->cs_u.cs_s.deferredIsrActive)
	    dataOutPIO(hacb);
	 else {
	 ...
	 if ((INPUT(u3.sStat1) & SCSIPERR) != 0) {
	    if (hacb->revision == 0)
	       OUTPUT(sXfrCtl1, (UCHAR) (hacb->sXfrCtl1Image & ~ENSPCHK));
	    OUTPUT(u3.clrSInt1, CLRSCSIPERR);
	    hacb->cs_u.cs_s.parityError = TRUE;
	    prepareMessageOut(hacb, INITIATOR_DETECTED_ERROR_MSG);
	    HIM6X60LogError(hacb, hacb->ne_u.ne_s.activeScb, 0, hacb->busID, hacb->lun,
			    SCB_ERROR_PARITY, hacb->scsiPhase);
	 } else if (hacb->ne_u.ne_s.msgOutLen == 0)
	    prepareMessageOut(hacb, ABORT_MSG);
	 OUTPUT(dmaCntrl0, 0);
	 OUTPUT(sXfrCtl0, CHEN | SPIOEN);
	 msgBytes = hacb->ne_u.ne_s.msgOutLen;
	 while (hacb->ne_u.ne_s.msgOutLen != 0 && samePhaseREQuest(hacb, pioReady)) {
	    if (hacb->ne_u.ne_s.msgOutLen == 1)
	       OUTPUT(u3.clrSInt1, CLRATNO);               /* Last byte */
	    OUTPUT(scsiDat, hacb->ne_u.ne_s.msgOut[msgBytes - hacb->ne_u.ne_s.msgOutLen--]);
	    pioReady = FALSE;
	 }
	 if (msgBytes > 1 && (hacb->ne_u.ne_s.msgOut[0] & IDENTIFY_MSG) != 0)
	    memcpy(&hacb->ne_u.ne_s.msgOut[0], &hacb->ne_u.ne_s.msgOut[1], msgBytes - 1);
	 if ((hacb->ne_u.ne_s.msgOut[0] & IDENTIFY_MSG) == IDENTIFY_MSG)
	    ;
	 else if (hacb->ne_u.ne_s.msgOut[0] == BUS_DEVICE_RESET_MSG) {
	    resetSDTR(hacb, hacb->busID);
	    hacb->cs_u.cs_s.disconnectOK = TRUE;
	 } else if (   hacb->ne_u.ne_s.msgOut[0] == ABORT_MSG
		    || hacb->ne_u.ne_s.msgOut[0] == RELEASE_RECOVERY_MSG)
	    hacb->cs_u.cs_s.disconnectOK = TRUE;
	 else if (hacb->ne_u.ne_s.msgOut[0] == INITIATOR_DETECTED_ERROR_MSG)
	    hacb->cs_u.cs_s.parityError = FALSE;
	 else if (hacb->ne_u.ne_s.msgOut[0] == MESSAGE_PARITY_ERROR_MSG)
	    hacb->cs_u.cs_s.msgParityError = FALSE;
	 else if (memcmp(&hacb->ne_u.ne_s.msgOut, &hacb->sdtrMsg,
			 sizeof(hacb->sdtrMsg) - 2) == 0) {
	    if (hacb->ne_u.ne_s.msgOutLen != 0)
	       updateSDTR(hacb, hacb->busID, 0, 0, TRUE);
	    else if (memcmp(&hacb->ne_u.ne_s.msgIn, &hacb->sdtrMsg,
			    sizeof(hacb->sdtrMsg) - 2) == 0)
	       hacb->negotiateSDTR &= ~(1 << hacb->busID);
	 }
	 if (hacb->ne_u.ne_s.msgOutLen != 0) {            /* No response to ATN? */
	    OUTPUT(u3.clrSInt1, CLRATNO);          /* OK, don't insist */
	    hacb->ne_u.ne_s.msgOutLen = 0;
	 }
	 break;
	 ...
	 while (samePhaseREQuest(hacb, pioReady)) {
...
#ifdef DEBUG
		{
		  ...
		  OUTPUT(sXfrCtl1, (UCHAR) (hacb->sXfrCtl1Image & ~ENSPCHK));
	       ...
	       hacb->cs_u.cs_s.parityError = TRUE;
	       prepareMessageOut(hacb, INITIATOR_DETECTED_ERROR_MSG);
	       HIM6X60LogError(hacb, hacb->ne_u.ne_s.activeScb, 0, hacb->busID,
	 ...
	 while (samePhaseREQuest(hacb, pioReady)) {
	    if ((INPUT(u3.sStat1) & SCSIPERR) != 0) {
		  ...
		  OUTPUT(sXfrCtl1, (UCHAR) (hacb->sXfrCtl1Image & ~ENSPCHK));
	       OUTPUT(u3.clrSInt1, CLRSCSIPERR);
	       hacb->ne_u.ne_s.msgInLen = 0;      /* Prepare to receive message again */
	       hacb->cs_u.cs_s.msgParityError = TRUE;
	       prepareMessageOut(hacb, MESSAGE_PARITY_ERROR_MSG);
	       HIM6X60LogError(hacb, hacb->ne_u.ne_s.activeScb, 0, hacb->busID,
			       hacb->lun, SCB_ERROR_PARITY, hacb->scsiPhase);
	    }
	    if (hacb->cs_u.cs_s.msgParityError)
...
BOOLEAN
samePhaseREQuest(HACB *hacb, BOOLEAN pioReady) 
{

   USHORT watchdog = 0;

   while (   (INPUT(dmaStat) & INTSTAT) == 0
	  && (INPUT(u2.sStat0) & SPIORDY) == 0
	  && pioReady == FALSE) {
      if (++watchdog == 0xFFFF) {
	 HIM6X60LogError(hacb, hacb->ne_u.ne_s.activeScb, 0, hacb->busID, hacb->lun,
			 SCB_ERROR_BUS_TIMEOUT, hacb->scsiPhase);
	 if (hacb->ne_u.ne_s.activeScb != NULL)
	    hacb->ne_u.ne_s.activeScb->scbStatus = SCB_TIMEOUT;
	 HIM6X60ResetBus(hacb, 0);
...
VOID
bitbucketAndABORT(HACB *hacb, UCHAR scbStatus) 
{

   TRACE(hacb, BITBUCKET_AND_ABORT, scbStatus, 0);
   if (scbStatus != 0 && hacb->ne_u.ne_s.activeScb != NULL)
      hacb->ne_u.ne_s.activeScb->scbStatus = scbStatus;
   hacb->cs_u.cs_s.bitBucket = TRUE;
   prepareMessageOut(hacb, ABORT_MSG);
...
VOID
dataOutPIO(HACB *hacb) 
{

   UCHAR dmaStatus, xferMode;
   ULONG watchdog, xferCount;

   TRACE(hacb, DATA_PHASE_PIO, hacb->ne_u.ne_s.dataLength, (ULONG) hacb->ne_u.ne_s.dataPointer);
   xferMode = ((hacb->revision == 0)
			? ENDMA | FIFO_16 | FIFO_PIO | FIFO_WRITE
			: ENDMA | FIFO_16 | FIFO_PIO | DWORDPIO | FIFO_WRITE);
   OUTPUT(dmaCntrl0, xferMode );
   OUTPUT(sXfrCtl0, CHEN | SCSIEN | DMAEN);
   while (hacb->ne_u.ne_s.dataLength > 0) {
      if (hacb->ne_u.ne_s.segmentLength == 0) {
/*
	 hacb->ne_u.ne_s.dataPointer = HIM6X60GetVirtualAddress(hacb, hacb->ne_u.ne_s.activeScb,
						      hacb->ne_u.ne_s.dataPointer,
						      hacb->ne_u.ne_s.dataOffset,
						      &hacb->ne_u.ne_s.segmentLength);
*/
	 hacb->ne_u.ne_s.segmentLength = MIN(hacb->ne_u.ne_s.segmentLength, hacb->ne_u.ne_s.dataLength);
      } 
      while (hacb->ne_u.ne_s.segmentLength > 0) {
	 watchdog = 0;
	 ...
	 if ((dmaStatus & INTSTAT) != 0)        /* Possible phase change? */
	    return;
	 xferCount = (dmaStatus & DFIFOEMP) ? MIN(hacb->ne_u.ne_s.segmentLength, 128)
					    : MIN(hacb->ne_u.ne_s.segmentLength, 64);
	 if ((xferCount & 0x03) == 0 && hacb->revision > 0) {
	    if (xferMode != (  ENDMA | FIFO_16 | FIFO_PIO | DWORDPIO
			     | FIFO_WRITE)) {
	       xferMode = (ENDMA | FIFO_16 | FIFO_PIO | DWORDPIO | FIFO_WRITE);
	       OUTPUT(dmaCntrl0, xferMode );
		}

	   BLOCKOUTDWORD(u6.dmaData32, hacb->ne_u.ne_s.dataPointer, xferCount >> 2);
	    ...
	    BLOCKOUTPUT(dmaData, hacb->ne_u.ne_s.dataPointer, xferCount);
	 } else {
	    if (xferMode != (ENDMA | FIFO_16 | FIFO_PIO | FIFO_WRITE)) {
		   xferMode = (ENDMA | FIFO_16 | FIFO_PIO | FIFO_WRITE);
	       OUTPUT(dmaCntrl0, xferMode); 
		}
	    BLOCKOUTWORD(dmaData, hacb->ne_u.ne_s.dataPointer, xferCount >> 1);
	 }
	 hacb->ne_u.ne_s.dataPointer += xferCount;
	 hacb->ne_u.ne_s.dataLength -= xferCount;
	 hacb->ne_u.ne_s.dataOffset += xferCount;
...
dataInPIO(HACB *hacb) 
{
   ...
   while (hacb->ne_u.ne_s.dataLength > 0) {
	 ...
	 xferCount = MIN(xferCount, hacb->ne_u.ne_s.segmentLength);
	 if ((xferCount & 0x03) == 0 && hacb->revision > 0) {
	    if (xferMode != (  ENDMA | FIFO_16 | FIFO_PIO | DWORDPIO
			     | FIFO_READ)) {
	       xferMode = ENDMA | FIFO_16 | FIFO_PIO | DWORDPIO | FIFO_READ;
	       OUTPUT(dmaCntrl0, xferMode);
		}

	    BLOCKINDWORD(u6.dmaData32, hacb->ne_u.ne_s.dataPointer, xferCount >> 2);
	    ...
	    BLOCKINPUT(dmaData, hacb->ne_u.ne_s.dataPointer, xferCount);
	 } else {
	    if (xferMode != (ENDMA | FIFO_16 | FIFO_PIO | FIFO_READ)) {
		   xferMode = ENDMA | FIFO_16 | FIFO_PIO | FIFO_READ;
	       OUTPUT(dmaCntrl0,xferMode );
		}
	    BLOCKINWORD(dmaData, hacb->ne_u.ne_s.dataPointer, xferCount >> 1);
	 }
	 hacb->ne_u.ne_s.dataPointer += xferCount;
	 hacb->ne_u.ne_s.dataLength -= xferCount;
	 hacb->ne_u.ne_s.dataOffset += xferCount;
...
VOID
dataPhaseDMA(HACB *hacb) 
{
   if (hacb->ne_u.ne_s.segmentLength == 0) {
      hacb->ne_u.ne_s.segmentAddress = HIM6X60GetPhysicalAddress(hacb, hacb->ne_u.ne_s.activeScb,
						       hacb->ne_u.ne_s.dataPointer,
						       hacb->ne_u.ne_s.dataOffset,
						       &hacb->ne_u.ne_s.segmentLength);
      hacb->ne_u.ne_s.segmentLength = MIN(hacb->ne_u.ne_s.segmentLength, hacb->ne_u.ne_s.dataLength);
   }
   HIM6X60MapDMA(hacb, hacb->ne_u.ne_s.activeScb, hacb->ne_u.ne_s.dataPointer,
		 hacb->ne_u.ne_s.segmentAddress, hacb->ne_u.ne_s.segmentLength,
		 (BOOLEAN) (hacb->scsiPhase == DATA_IN_PHASE));
   hacb->cs_u.cs_s.dmaActive = TRUE;
...
VOID
HIM6X60DmaProgrammed(HACB *hacb) 
{

   UCHAR dmaControl, fifoCount;

   TRACE(hacb, DATA_PHASE_DMA, hacb->ne_u.ne_s.segmentLength,
	 PHYSICAL_TO_ULONG(hacb->ne_u.ne_s.segmentAddress));
   OUTPUT(sIMode0, ENDMADONE);
   if ((fifoCount = INPUT(fifoStat)) != 0) {
      fifoCount += (INPUT(sStat2) & SFCNT) - (INPUT(u4.sStat3) & OFFCNT);
      if (fifoCount != 0)
...
interpretMessageIn(HACB *hacb) 
{
   ...
   TRACE(hacb, MESSAGE_IN, ((USHORT) hacb->ne_u.ne_s.msgInLen) << 8 | INPUT(scsiBus),
	 hacb->targetStatus);
   hacb->ne_u.ne_s.msgIn[hacb->ne_u.ne_s.msgInLen++] = INPUT(scsiBus);
   if ((hacb->ne_u.ne_s.msgIn[0] & IDENTIFY_MSG) == IDENTIFY_MSG)
      if (hacb->lun != DISCONNECTED)
	 prepareMessageOut(hacb, ABORT_MSG);
      else {
	 lucb = HIM6X60GetLUCB(hacb, 0, hacb->busID,
			       hacb->lun = hacb->ne_u.ne_s.msgIn[0] & 0x07);
	 hacb->ne_u.ne_s.activeScb = scb = lucb->activeScb;
	 if (scb == NULL) {
	    prepareMessageOut(hacb, BUS_DEVICE_RESET_MSG);
	    HIM6X60LogError(hacb, NULL, 0, hacb->busID, 0xFF,
			    SCB_ERROR_INVALID_RESELECTION, hacb->ne_u.ne_s.msgIn[0]);
	    ...
	    hacb->ne_u.ne_s.dataPointer = scb->dataPointer;
	    hacb->ne_u.ne_s.dataLength = scb->dataLength;
	    hacb->ne_u.ne_s.dataOffset = scb->dataOffset;
	    hacb->ne_u.ne_s.segmentAddress = scb->segmentAddress;
	    hacb->ne_u.ne_s.segmentLength = scb->segmentLength;
   ...
   else if ((scb = hacb->ne_u.ne_s.activeScb) == NULL)
      prepareMessageOut(hacb, BUS_DEVICE_RESET_MSG);
   else switch (hacb->ne_u.ne_s.msgIn[0]) {
      case COMMAND_COMPLETE_MSG:
	 if ((scb->scbStatus & SCB_SENSE_DATA_VALID) == 0) {
	    scb->transferLength += scb->provisionalTransfer
				    + (scb->dataLength - hacb->ne_u.ne_s.dataLength);
	 ...
	 break;
      ...
      case DISCONNECT_MSG:
	 if ((scb->scbStatus & SCB_SENSE_DATA_VALID) == 0)
	    scb->provisionalTransfer = scb->dataLength - hacb->ne_u.ne_s.dataLength;
	 hacb->cs_u.cs_s.disconnectOK = TRUE;
	 break;

      case SAVE_DATA_POINTER_MSG:
	 if ((scb->scbStatus & SCB_SENSE_DATA_VALID) == 0) {
	    scb->transferLength += scb->dataLength - hacb->ne_u.ne_s.dataLength;
	    scb->transferResidual -= scb->dataLength - hacb->ne_u.ne_s.dataLength;
	 }
	 scb->dataPointer = hacb->ne_u.ne_s.dataPointer;
	 scb->dataLength = hacb->ne_u.ne_s.dataLength;
	 scb->dataOffset = hacb->ne_u.ne_s.dataOffset;
	 scb->segmentAddress = hacb->ne_u.ne_s.segmentAddress;
	 scb->segmentLength = hacb->ne_u.ne_s.segmentLength;
	 break;
	 ...
	 break;
      ...
      case EXTENDED_MSG:
	 if (     hacb->ne_u.ne_s.msgInLen == 1
	       || hacb->ne_u.ne_s.msgInLen < hacb->ne_u.ne_s.msgIn[1] + 2)
	    messageComplete = FALSE;
	 else if (   hacb->ne_u.ne_s.msgIn[2] == SYNCHRONOUS_DATA_TRANSFER_MSG
		  && hacb->ac_u.ac_s.synchronous)
	    negotiateSDTR(hacb);
	 else
	 ...
	 hacb->ne_u.ne_s.msgOutLen = 1;           /* Just in case ATN is still on */
	 hacb->ne_u.ne_s.msgOut[0] = NOP_MSG;
	 break;
   ...
   if (messageComplete)
      hacb->ne_u.ne_s.msgInLen = 0;               /* Reset for any new message(s) */
...
negotiateSDTR(HACB *hacb) 
{
   ...
   TRACE(hacb, NEGOTIATE_SDTR,
	 ((USHORT) hacb->ne_u.ne_s.msgOut[4]) << 8 | hacb->ne_u.ne_s.msgOut[3],
	 ((USHORT) hacb->ne_u.ne_s.msgIn[4]) << 8 | hacb->ne_u.ne_s.msgIn[3]);
   hacb->negotiateSDTR |= 1 << busID;   /* Mark negotiations in progress */
   if (hacb->ne_u.ne_s.msgIn[4] == 0)             /* Target refuses synchronous? */
      updateSDTR(hacb, busID, 0, 0, TRUE);
   else if (  (  ((USHORT) hacb->ne_u.ne_s.msgIn[3] << 2)
	       + hacb->clockPeriod - 1)
	    / hacb->clockPeriod > 9) {  /* Target too slow for 6X60? */
      prepareMessageOut(hacb, MESSAGE_REJECT_MSG);
      updateSDTR(hacb, busID, 0, 0, TRUE);
   } else {
      syncCycles = (hacb->ne_u.ne_s.msgIn[3] < hacb->sdtrMsg.transferPeriod)
		     ? (((USHORT) hacb->sdtrMsg.transferPeriod << 2)
			+ hacb->clockPeriod - 1) / hacb->clockPeriod
		     : (((USHORT) hacb->ne_u.ne_s.msgIn[3] << 2) + hacb->clockPeriod - 1)
			/ hacb->clockPeriod;
      syncOffset = MIN(hacb->ne_u.ne_s.msgIn[4], hacb->sdtrMsg.reqAckOffset);
      updateSDTR(hacb, busID, syncCycles, syncOffset, FALSE);
      if (memcmp(&hacb->ne_u.ne_s.msgOut, &hacb->sdtrMsg,
		 sizeof(hacb->sdtrMsg) - 2) == 0)
	 hacb->negotiateSDTR &= ~(1 << busID);
      else {
	 memcpy(&hacb->ne_u.ne_s.msgOut, &hacb->sdtrMsg,
		hacb->ne_u.ne_s.msgOutLen = sizeof(hacb->sdtrMsg));
	 hacb->ne_u.ne_s.msgOut[3] = MAX(hacb->sdtrMsg.transferPeriod, hacb->ne_u.ne_s.msgIn[3]);
	 hacb->ne_u.ne_s.msgOut[4] = MIN(hacb->sdtrMsg.reqAckOffset, hacb->ne_u.ne_s.msgIn[4]);
      }
   }
...
VOID
updateSDTR(HACB *hacb, USHORT busID, UCHAR syncCycles,
		UCHAR syncOffset, BOOLEAN negotiationsCompleted) 
{
   ...
   if (negotiationsCompleted)
      hacb->negotiateSDTR &= ~(1 << hacb->busID);
   if (hacb->signature == 0x53 || hacb->signature == 0x54)
...
VOID
resetSDTR(HACB *hacb, USHORT busID) 
{
   ...
   if (busID == hacb->ownID) {
      ...
      if (hacb->ac_u.ac_s.synchronous && hacb->ac_u.ac_s.initiateSDTR) {
	 ...
	 stackOffset = 0;
      ...
      if (stackOffset != 0) {
	 OUTPUT(dmaCntrl1, stackOffset);
	 OUTPUT(stack, (UCHAR) ((hacb->ac_u.ac_s.synchronous && hacb->ac_u.ac_s.initiateSDTR)
				? 0 : 0x80));
      }
   }
...
VOID
linkScbPreemptive(SCB **queueHead, SCB *scb) 
{
...
BOOLEAN
unlinkScb(SCB **queueHead, SCB *scb) 
{
...
VOID
prepareMessageOut(HACB *hacb, UCHAR message) 
{

   hacb->ne_u.ne_s.msgOutLen = 1;
   hacb->ne_u.ne_s.msgOut[0] = message;
   OUTPUT(scsiSig, (UCHAR) (INPUT(scsiSig) & (CD | IO | MSG) | ATN));
...
VOID debugTrace(HACB *hacb, UCHAR event, ULONG data0, ULONG data1) {
   ...
   traceLog->sStat0 = INPUT(u2.sStat0);
   traceLog->sStat1 = INPUT(u3.sStat1);
