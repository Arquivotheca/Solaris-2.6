...
#pragma ident	"@(#)him_init.c 1.10	95/01/12 SMI"
...
#include <sys/dktp/sequence.h>
#include <sys/dktp/seq_off.h>
#include <sys/dktp/seqmac.h>
#include <sys/dktp/him_rel.h>
#include <sys/dktp/vulture.h>
...
/*@VULTURE*/
   ...
   {
      /* New code for SunSoft, 7-8-94 */
      PAUSE_SEQ(hcntrl);			/* Pause sequencer */
      sblkctl_data = INBYTE(sblkctl);		/* Save channel state */
      OUTBYTE(sblkctl, (sblkctl_data & ~SELBUS1));  /* Switch to channel A */
      /* End of SunSoft */
...
/* XXX */
	if ((hcntrl_data & (PAUSE | CHIPRESET)) == 0) {
		hcntrl_data |= PAUSE;
	}
   ...
   else
   {
      ...
      */
      ...
      {                        
         ...
         /* For SunSoft, only set Wide negotiation if configured for Wide */
         if ((INBYTE(port_addr) & WIDEXFER) && 
             (config_ptr->Cfe_MaxTargets == 16))
         {
            config_ptr->Cfe_ScsiOption[i] |= WIDE_MODE;
         }
         /* End of SunSoft change */
         ++port_addr;
      }
      /*
...
UBYTE scb_initHA (him_config *config_ptr)
{
   ...
   PAUSE_SEQ(hcntrl);
   /* Save channel selection with AUTOFLUSH disabled */
   ha_ptr = config_ptr->Cfe_HaDataPtr;
   sblkctl_data = INBYTE(ha_ptr->sblkctl) & ~AUTOFLUSHDIS;
  ...
  /****** ja 3/17 MBRSTCTL bit required check ******/
      ...
      if((UBYTE) Mbrstctl_req( (config_ptr->Cfe_PortAddress),
                 (DWORD) (config_ptr->Cfe_ScbParam.Prm_HimDataPhysaddr),
                       (config_ptr->Cfe_HaDataPtr) ) != DMA_PASS)
      {
         OUTBYTE(config_ptr->Cfe_PortAddress + EISA_HOST + BUSTIME,
                (INBYTE(config_ptr->Cfe_PortAddress + EISA_HOST + BUSTIME) | 0x01));
  
         if (Mbrstctl_req( (config_ptr->Cfe_PortAddress),
                    (DWORD)(config_ptr->Cfe_ScbParam.Prm_HimDataPhysaddr),
                         (config_ptr->Cfe_HaDataPtr) ) == DMA_PASS)
         {
   ...
   {
      PAUSE_SEQ(hcntrl);
      temp = INBYTE(hcntrl) & ~CHIPRESET; 
      if ((config_ptr->Cfe_ConfigFlags & INTHIGH) == 0)
      {
         OUTBYTE(hcntrl, temp | IRQMS);
      }
      else
      {
         OUTBYTE(hcntrl, temp & ~IRQMS);
   ...
   /* Select proper HW channel to configure at this time */
   if (config_ptr->Cfe_MaxTargets == 16)	/* 16 bit configuration */
   {
      sblkctl_data = SELWIDE;
   }
   else						/* 8 bit configurations */
   {
	sblkctl_data = (config_ptr->Cfe_SCSIChannel & CHANNEL);
   }
   /* Need to "double pump" this register */
...
/*	took out enabling of interrupt			*/
...
void scb_init_hastruct( him_struct *ha_ptr)
{
   ...
   */
   ...
   ha_ptr->EnqueScb   = (void * ) NOT_DEFINED;  
   ha_ptr->ChekCond   = (void * ) NOT_DEFINED;
   ha_ptr->DequeScb   = (void * ) NOT_DEFINED;
...
int scb_init_extscb (him_config *config_ptr)
{
   ...
   for (i = 0; i < 256; i++)
   {
      READ_HINTSTAT( ha_ptr->hcntrl, ha_ptr->intstat, intval);
      if (intval == MACDONE)
         break;
   }
...
UBYTE Mbrstctl_req (UWORD base_addr,
                    ...
                    UBYTE test_arr[64])
{
   UBYTE temp , dma_b0 = 0, dma_b1= 0, dma_b2 = 0, dma_b3 = 0 ;
   int j, timeout = 10000;
   ...
   dma_b0 = (UBYTE) phys_addr ;
   dma_b1 = (UBYTE) (phys_addr >> 8) ;
   dma_b2 = (UBYTE) (phys_addr >> 16);
   dma_b3 = (UBYTE) (phys_addr >> 24) ;

   OUTBYTE((base_addr + EISA_HOST + BCTL), ENABLE);     /* ja 5/23 */


   OUTBYTE(base_addr + EISA_HOST + HADDR0, dma_b0);
   OUTBYTE(base_addr + EISA_HOST + HADDR1, dma_b1);
   OUTBYTE(base_addr + EISA_HOST + HADDR2, dma_b2);
   OUTBYTE(base_addr + EISA_HOST + HADDR3, dma_b3);
