...
#pragma ident	"@(#)chs_dac.h	1.3	96/07/17 SMI"
...
#define	CHS_DAC_TGT_DISK(t)		(((t) << 3) == 0x00)
#define	CHS_DAC_TGT_TAPE(t)		(((t) << 3) == 0x20)
...
typedef struct chs_dac_enquiry_viper {
	...
	u_char WrongAdrCnt;
	u_char UnidentCnt;
	u_char NVramDevChgCnt;
	u_char CodeBlkVersion[8];
	u_char BootBlkVersion[8];
	ulong sd_sz[CHS_DAC_MAX_SD]; 	/* sizes of system drives */
						/* in sectors */
	u_char ConCurrentCmdCount;
	u_char MaxPhysicalDevices;
	ushort usFlashRepgmCount;
	u_char DefuncDiskCount;
	u_char RebuildFlag;
	u_char	OffLineLogDrvCount;
	u_char CriticalDrvCOunt;
