...
#pragma ident	"@(#)chsops.h	1.2	96/06/18 SMI"
...
typedef	struct chsops {
	...
	unchar	(*chs_cready)(chs_t *chsp);
	bool_t	(*chs_csend)(chs_t *chsp, void *hw_ccbp);
	unchar	(*chs_iready)(chs_t *chsp);
	unchar	(*chs_get_istat)(chs_t *chsp, void *hw_statp,
					int clear);
	int	(*chs_geometry)(chs_t *chsp,
					struct scsi_address *ap, ulong bl);
	void	(*chs_iosetup) (chs_ccb_t *ccb, int num_segs,
					u_long bytes_xfer, off_t cofforsrtsec,
					u_char sdnum, int old);
	...
	bool_t 	(*chs_in_any_sd)(chs_t *const chs, const u_char chn,
					const u_char tgt, u_char *raidlevel);
	bool_t 	(*chs_get_logdrv_info)(chs_t *const chs, int tgt,
					chs_ld_t *ld_info);
	bool_t	(*chs_can_physdrv_access) (chs_t *const chs,
					const u_char chn, const u_char tgt);
