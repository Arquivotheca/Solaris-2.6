...
#pragma ident "@(#)chs_viper.c	1.6	96/07/17	SMI"
		...
		dev_info_t	*dip)
{
	ddi_acc_handle_t handle;
	int reset_counter = 0;
	u_char postmajor, postminor, bcs, ecs;
	caddr_t statusq;
...
step3:
	...
	while ((reset_counter < 2) && (reset_pass != TRUE)) {
		...
		ddi_io_putb(chsp->handle, chsp->reg + SCPR_REG,
							VIPER_RESET_ADAPTER);
		drv_usecwait(100);
		...
		/* step5: */
		postmajor = ddi_io_getb(chsp->handle,
					chsp->reg + ISPR_REG);

		/* step6: */
		ddi_io_putb(chsp->handle, chsp->reg + HIST_REG,
							VIPER_HIST_GHI);
		/* step7: */
		...
		/* step13: */
		bcs = ddi_io_getb(chsp->handle, chsp->reg + ISPR_REG);
		...
		/* step16: */
		ecs = ddi_io_getb(chsp->handle, chsp->reg + ISPR_REG);
	...
	if (reset_pass == FALSE) {
		...
		ddi_io_putb(chsp->handle,
			chsp->reg + HIST_REG, VIPER_HIST_GHI);
		viper_regs_free(chsp);
		return (FALSE);
	}
	...
	ddi_io_putl(chsp->handle, chsp->reg + CCCR_REG, 0x10);
	...
	statusq_phys = CHS_KVTOP((unchar *)statusq);

	/* setup status queue start, head, tail and end registers on the chip */

	ddi_io_putl(chsp->handle, chsp->reg + SQSR_REG,
					(ulong_t)(statusq_phys));

	ddi_io_putl(chsp->handle, chsp->reg + SQHR_REG,
		(ulong_t)(statusq_phys + sizeof (viper_statusq_element)));

	ddi_io_putl(chsp->handle, chsp->reg + SQTR_REG,
						(ulong_t)statusq_phys);

	ddi_io_putl(chsp->handle, chsp->reg + SQER_REG,
			(ulong_t)(statusq_phys +
				(sizeof (viper_statusq_element) *
						VIPER_STATUS_QUEUE_NUM)));


	MDBG8(("viper_init: statusq head is %x", statusq));
	...
	ddi_io_putb(chsp->handle, chsp->reg + HIST_REG, VIPER_HIST_GHI);
	...
	secondcmdblkp =
		(caddr_t)((unchar *) chsp->vipercmdblkp + sizeof (chs_cmd_t));

	secondcmdblkphys = CHS_KVTOP((unchar *)secondcmdblkp);

	*((unchar *) chsp->vipercmdblkp + 20) = VIPER_CMDBLK_BYTE20;
	*((unchar *) chsp->vipercmdblkp + 21) = VIPER_CMDBLK_BYTE21;
	*((unchar *) chsp->vipercmdblkp + 22) = VIPER_CMDBLK_BYTE22;
	*((unchar *) chsp->vipercmdblkp + 23) = VIPER_CMDBLK_BYTE23;
	...
	*((unchar *) secondcmdblkp) = CHS_DAC_NOOP;
	*((unchar *) secondcmdblkp + 1) = CHS_NEPTUNE_CMDID;

	*((unchar *) secondcmdblkp + 20) = VIPER_CMDBLK_BYTE20;
	*((unchar *) secondcmdblkp + 21) = VIPER_CMDBLK_BYTE21;
	*((unchar *) secondcmdblkp + 22) = VIPER_CMDBLK_BYTE22;
	*((unchar *) secondcmdblkp + 23) = VIPER_CMDBLK_BYTE23;

	*((ulong_t *)((unchar *) chsp->vipercmdblkp + 16)) =
					(ulong_t) secondcmdblkphys;
	*((ulong_t *)((unchar *) secondcmdblkp + 16)) =
					(ulong_t) secondcmdblkphys;
...
viper_enable(chs_t *chsp)
{
	...
	ddi_io_putb(chsp->handle, chsp->reg + HIST_REG, VIPER_HIST_EI);
...
{
	ddi_io_putb(chsp->handle, chsp->reg + HIST_REG, 0);
...
viper_cready(chs_t *chsp)
{
	 ...
	 *
	 */
	return ((ddi_io_getl(chsp->handle, chsp->reg + CCCR_REG)
							& VIPER_CCCR_SS) == 0);
}
		...
		void *ccbp)
{
	...
	register ulong *ip = (ulong *)ccbp;
	register unchar *op = (unchar *)chsp->vipercmdblkp;
	register int i;
	...
	ddi_io_putl(chsp->handle, chsp->reg + CCSAR_REG,
						chsp->vipercmdphys);

	ddi_io_putl(chsp->handle, chsp->reg + CCCR_REG,
			VIPER_CCCR_SS | VIPER_CCCR_ILE |
				(VIPER_CMD_BYTECNT << 8));
...
viper_iready(chs_t *chsp)
{
	...
	return ((ddi_io_getb(chsp->handle, chsp->reg + HIST_REG) &
		VIPER_HIST_SCE));
}
			...
			int clear)
{
	...
	register ushort	op = chsp->reg;
	unchar *vsqtr;
	unchar Isr;

	if (clear == 2) {
		Isr = (unchar)ddi_io_getb(chsp->handle,
					chsp->reg + HIST_REG);
	...
	if (nextelement(chsp->psq, chsp->sqtr) ==  chsp->sqhr) {
		if (viper_iready (chsp)) {
			chsp->sqhr =
				(paddr_t) ddi_io_getl(chsp->handle,
							op + SQHR_REG);
		} else {
			return (0);
		}
	...
	if (hw_stat) {
		...
		vsqtr = (unchar *)((ulong_t) chsp->vsq  +
			(ulong_t) chsp->sqtr -
			(ulong_t) chsp->psq);
		ip->reserved = 0;
		ip->stat_id =  *(unchar *) ((unchar *)vsqtr + 1);

		ip->status = *((u_char *) vsqtr + 3);
		ip->status = (ip->status << 8) | *((u_char *)vsqtr + 2);
	...
	if (nextelement(chsp->psq, chsp->sqtr) ==  chsp->sqhr) {
		/* queue empty */
		MDBG8(("empty"));
		ddi_io_putl(chsp->handle, op + SQTR_REG, (ulong_t) chsp->sqtr);
	} else {
			...
			ulong			blk)
{
	...
	if (ap)
cmn_err(CE_CONT, "?chs(%d,%d): reg=0x%x, sectors=%d heads=%ld sectors=%ld\n",
