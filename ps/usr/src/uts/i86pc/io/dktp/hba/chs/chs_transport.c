...
#pragma	ident	"@(#)chs_transport.c	1.4	96/07/17 SMI"
		...
		register chs_ccb_t *const ccb)
{
	...
	if (CHS_DAC_CHECK_STATUS(chs, NULL, ccb->ccb_status) &
							CHS_INV_OPCODE) {
		cmn_err(CE_WARN,
			"chs_init_cmd: failed opcode: %x "
			"cmdid: %d status %x",
			ccb->ccb_opcode, ccb->ccb_cmdid, ccb->ccb_status);
		return (DDI_FAILURE);
	}
		...
		int init)
{
	...
	if (init) {
		...
		if (chs->free_ccb->next == CHS_INVALID_CMDID)
			cv_wait_sig(&chs->ccb_stk_cv, &chs->mutex);
	...
	if (cntr == 0) {
		cmn_err(CE_WARN, "chs_sendcmd: not ready to accept "
    ...
    int init)
{
	...
	for (retry = CHS_MAX_RETRY; retry > 0; drv_usecwait(10), retry--) {
		for (cntr = CHS_MAX_RETRY; cntr > 0; drv_usecwait(10),
								cntr--) {
			if (CHS_IREADY(chs))
				break;
		}
    ...
    register struct scsi_device *const sd)
{
	...
	MDBG2(("chs_tran_tgt_init: S%xC%xt%x dip=0x%x sd=0x%x unit=0x%x",
    ...
    const caddr_t arg)
{
	...
	max_xfer = (ushort)CHS_MIN(CHS_MIN(pkt_totxfereq,
						CHS_SCSI_MAX_XFER),
	    dmacp->dmac_size);
	...
	if (pkt_totxfereq <= max_xfer || !fw_supports_sg) {
		...
		max_xfer = (ushort)CHS_MIN(pkt_totxfereq,
						CHS_SCSI_MAX_XFER);
    ...
    register struct scsi_pkt *const pkt)
{
	...
	if (pkt->pkt_flags & FLAG_NODISCON)
		CHS_SCSI_CDB_NO_DISCON(ccb->ccb_cdbt->cmd_ctrl);
	else
