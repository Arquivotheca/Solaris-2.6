...
#pragma	ident "@(#)ansi.c	1.11	96/06/02 SMI"
...
static int ansi_open(queue_t *qp, dev_t *devp, int oflag, int sflag,
	cred_t *crp);
static int ansi_close(register queue_t *qp, int flag, cred_t *crp);
static int ansi_read_queue_put(queue_t *qp, mblk_t *mp);
static int ansi_write_queue_put(queue_t *qp, register mblk_t *mp);
static int ansi_write_queue_serv(queue_t *qp);

static void ansi_equal_state(register ansistat_t *ap, u_char ch);
static void ansi_proc_w_data(register queue_t *qp, register mblk_t *mp,
	register ansistat_t *ap);
static void ansi_parse(register ansistat_t *ap, u_char ch);
static void ansi_control(register ansistat_t *ap, u_char ch);
static void ansi_setparam(register ansistat_t *ap, register int count,
	register u_short newparam);
static void ansi_send_1ctl(ansistat_t *ap, u_long cmd, u_long data,
	u_long type);
static void ansi_selgraph(register ansistat_t *ap);
static void ansi_chkparam(register ansistat_t *ap, u_char ch);
static void ansi_getparams(register ansistat_t *ap, u_char ch);
static void ansi_outch(ansistat_t *ap, u_char ch);
static void ansi_stat_init(queue_t *qp, register ansistat_t *ap);
...
_init(void)
{
	...
	mutex_init(&ansi_lock, "ansi lock", MUTEX_DEFAULT, NULL);
...
static void
ansi_stat_init(queue_t *qp, register ansistat_t *ap)
{
	register int j;
...
/*ARGSUSED1*/
static int
ansi_close(register queue_t *qp, int flag, cred_t *crp)
{
...
static void
ansi_proc_w_data(register queue_t *qp, register mblk_t *mp,
	register ansistat_t *ap)
{
...
static void
ansi_control(register ansistat_t *ap, u_char ch)
{
	ap->a_state = A_STATE_START;
	switch (ch) {
	case A_BEL:
...
static void
ansi_setparam(register ansistat_t *ap, register int count,
	register u_short newparam)
{
	register int i;
 ...
 */
static void
ansi_selgraph(register ansistat_t *ap)
{
 ...
 */
static void
ansi_chkparam(register ansistat_t *ap, u_char ch)
{
	int i;
 ...
 */
static void
ansi_getparams(register ansistat_t *ap, u_char ch)
{
...
static void
ansi_equal_state(register ansistat_t *ap, u_char ch)
{
	switch (ch) {
	case 'A':
...
static void
ansi_parse(register ansistat_t *ap, u_char ch)
{
	int i;
