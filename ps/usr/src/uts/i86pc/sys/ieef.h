...
#pragma ident "@(#)ieef.h	1.8	95/08/22 SMI"
...
typedef struct scb
{
    volatile ushort	scb_status;	/* Status of 82596 */
    volatile ushort	scb_command;    /* The operation 82596 should perform */
...
typedef struct dump_cmd
{
    ...
    volatile unchar	*dump_buffer_offset; /* Pointer to the dump area */
} dump_cmd_t;
...
#define	IEEF_NFRAMES		64	/* Max number of frames */
#define	IEEF_NXMIT		64	/* Max number of xmit buffers */
#define	IEEF_NCMDS		64	/* Max number of commands */
#define	IEEF_XFRAMESIZE		2048	/* Max xmit frame size */
#define	IEEF_NMCS		10	/* Max number of multicast addresses */
#define	IEEF_DUMP_BUFFER_SIZE	0x130	/* size of the dump buffer */
...
struct ieef_shmem
{
    ...
    unchar	ieef_dump_buffer[IEEF_DUMP_BUFFER_SIZE];
};
...
#define	IEEF_HW_FLASH 0
#define	IEEF_HW_UNISYS 1
#define	IEEF_HW_EE100_EISA 2
#define	IEEF_HW_EE100_PCI 3
...
#define	COMMAND_QUIESCE(ieefp) {\
	long	cnt = 1000000; \
    volatile ushort *tmp = &(ieefp->kmem_map->ieef_scb.scb_command); \
    while (*tmp != 0 && --cnt > 0) \
		drv_usecwait(10); \
	if (cnt <= 0) \
		cmn_err(CE_PANIC, "ieef(0x%x): can't quiesce 0x%x", \
