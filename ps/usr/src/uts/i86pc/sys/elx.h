...
#pragma ident	"@(#)elx.h	1.17	96/06/25 SMI"
...
#else
#define	DDI_INB(port)		ddi_io_getb(handle, port)
#define	DDI_INW(port)		ddi_io_getw(handle, port)
#define	DDI_INL(port)		ddi_io_getl(handle, port)
#define	DDI_OUTB(port, val)	ddi_io_putb(handle, port, val)
#define	DDI_OUTW(port, val)	ddi_io_putw(handle, port, val)
#define	DDI_OUTL(port, val)	ddi_io_putl(handle, port, val)
#endif
...
#define	DDI_REPOUTSD(port, buf, cnt)	repoutsd(port, buf, cnt)
#else
#define	DDI_REPINSB(port, buf, cnt)	ddi_io_rep_getb(handle, buf, port, cnt)
#define	DDI_REPINSD(port, buf, cnt)	ddi_io_rep_getl(handle, buf, port, cnt)
#define	DDI_REPOUTSB(port, buf, cnt)	ddi_io_rep_putb(handle, buf, port, cnt)
#define	DDI_REPOUTSD(port, buf, cnt)	ddi_io_rep_putl(handle, buf, port, cnt)
#endif
...
typedef struct elxinstance {
	...
	ushort	elx_rcvlen;	/* space allocated */
	ushort	elx_softinfo;	/* EEPROM software info word */
	ushort	elx_softinfo2;	/* EEPROM software info 2 word */
	ushort	elx_caps;	/* EEPROM capabilities word */
	ushort	elx_flags;
	ushort	elx_ver;	/* 0 for 3c5x9, 1 for 3c59x */
	ushort	elx_media;
	unchar	elx_speed;
	unchar	elx_latency;	/* interrupt latency */
	unchar	elx_pad;
