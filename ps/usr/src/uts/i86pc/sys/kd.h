...
#pragma ident	"@(#)kd.h	1.14	96/06/20 SMI"
...
/* KJS */
#define	EVGA	1		/* EVGA support is enabled */
...
#define	CTLKEY		0x2000	/* flag key as affected by control keys */
/*
...
#define	NOKEY		0x0f00	/* flag a key as nonexistent */
#define	TYPEMASK	0x0f00	/* select key type bits */


/*
 * Table selectors for KDSKBENT/KDGKBENT ioctls
 */
...
/* SHIFT conflicts with a name in VM.  It isn't used in KD anyway! */
...
#define	K_NEXT	130			/* select next virtual terminal */
#define	K_PREV	131			/* select previous virtual terminal */
#define	K_FRCNEXT 132			/* force switch to next vt */
#define	K_FRCPREV 133			/* force switch to previous vt */
#define	K_VTF	134			/* switch to vt number K_VTF + n */
#define	K_VTL	148			/* last valid vt selection value */
#define	K_MGRF	149
#define	K_MGRL	191
#define	K_PFXF	192
#define	K_PFXL	255
...
#define	IS_SPECIAL(s, i) \
		(kdkeymap.key[(s)].spcl & (0x80>>(i)))	/* Special? */
...
#define	IS_FUNKEY(c)	(((int)(c) >= K_FUNF) && \
				((int)(c) <= K_FUNL) || \
				((int)(c) >= K_PFXF) && \
...
#define	LED_WARN	0xED	/* Tell kbd that following byte is LED state */
...
#define	TYPE_WARN	0xF3	/* command--next byte is typematic value */
...
#define	DATA_REG	0x1
#define	MODE_REG	0x4
#define	MODE2_REG	0x0A	/* ATT rite-vu card only */
...
#define	KD_TEXT1	2	/* new text mode does not load char generator */
...
#define	OFFSET_80x43	(M_ENH_B80x43-DM_ENH_B80x43)	/* Offset for */
					/* converting 80x43 numbers */
#define	M_ENH_CGA	DM_ENH_CGA	/* AT&T 640x400 CGA hardware */
					/* mode - (Super-VU adapter) */

/* Defines for keyboard and display ioctls */
#define	KDIOC		('K'<<8)
#define	KDDISPTYPE	(KDIOC|1)	/* return display type to user */
#define	KDMAPDISP	(KDIOC|2)	/* map display into user space */
#define	KDUNMAPDISP	(KDIOC|3)	/* unmap display from user space */
#define	KDGKBENT	(KDIOC|4)	/* get keyboard table entry */
#define	KDSKBENT	(KDIOC|5)	/* set keyboard table entry */
#define	KDGKBMODE	(KDIOC|6)	/* get keyboard translation mode */
#define	KDSKBMODE	(KDIOC|7)	/* set keyboard translation mode */
#define	KDMKTONE	(KDIOC|8)	/* sound tone */
#define	KDGETMODE	(KDIOC|9)	/* get text/graphics mode */
#define	KDSETMODE	(KDIOC|10)	/* set text/graphics mode */
#define	KDADDIO		(KDIOC|11)	/* add I/O address to list */
#define	KDDELIO		(KDIOC|12)	/* delete I/O address from list */
#define	KDSBORDER	(KDIOC|13)	/* set ega color border */
#define	KDQUEMODE	(KDIOC|15)	/* enable/disable queue mode */
#define	KDVDCTYPE3_2	(KDIOC|16)	/* USL/SVR3.2 value of KDVDCTYPE */
#define	KIOCNONDOSMODE	(KDIOC | 17)	/* obsolete -- clear DOSMODE	    */
#define	KDDISPINFO	(KDIOC | 18)	/* get display start and size	    */
#define	KDGKBSTATE	(KDIOC | 19)	/* get state of keyboard shift keys */
#define	KDSETRAD	(KDIOC|20)	/* set keyboard typematic rate/delay */
#define	KDSCROLL	(KDIOC|21)	/* set hardware scrolling on/off */

/* VP/ix reserved ioctls */
#define	KDENABIO	(KDIOC|60)	/* enable direct I/O to ports */
#define	KDDISABIO	(KDIOC|61)	/* disable direct I/O to ports */
#define	KIOCINFO	(KDIOC|62)	/* tell user what device we are */
#define	KIOCSOUND	(KDIOC|63)	/* start sound generation */
#define	KDGKBTYPE	(KDIOC|64)	/* get keyboard type */
#define	KDGETLED	(KDIOC|65)	/* get keyboard LED status */
#define	KDSETLED	(KDIOC|66)	/* set keyboard LED status */
...
#define	KDDFLTSCRNMAP	(WSIOC|3)	/* set/get default screen map for */
					/* this workstation */
#define	KDDFLTSTRMAP	(WSIOC|4)	/* set/get default function key map */
...
#define	WS_SETSYSVCOMPAT	(WSIOC|12) /* SVR3/SVR4 compatible */

/* Defines for Xenix keyboard and display ioctls */
#define	MIOC		('k' << 8)	/* Upper byte of mapping ioctls */
...
/*
 * scancode mode ioctl commands.
 */
#define	TCGETSC		(TIOC|34)	/* ioctl for scancode support */
#define	TCSETSC		(TIOC|35)	/* ioctl for scancode support */

/* definitions for TCGETSC/TCSETSC ioctl arguments */
#define	KB_RESERVED1	0x01
#define	KB_RESERVED2	0x02
#define	KB_XSCANCODE	0x04		/* perform scancode/ascii translation */
#define	KB_ISSCANCODE	0x08		/* device is using scan codes */
#define	KB_NOPARSESC	0x10    	/* no shadow parsing of keyboard */
#define	KB_CS2SCANCODE	0x20		/* scan codes are codeset 2 (AT mode) */
 ...
 * that ioctl, subsequent SVR3.2 MODESWITCH/STREAMS ioctls will be resolved
...
#define	SCO_OLD_MODESWITCH	('S' << 8)	/* Old SCO modeswitch prefix */
#define	USL_OLD_MODESWITCH	('x' << 8)	/* Old USL modeswitch prefix */
#define	MODESWITCH	(('i'<<24)|('x'<<16))	/* Merge USL/SCO MODESWITCH */

/* End Enhanced Application Compatibility Support */

#define	KDMODEMASK	0xff		/* Lower byte of mode switch ioctls */
...
#define	O_MODESWITCH	('S' << 8)	/* Upper byte of mode switch ioctls */
...
#define	O_SW_BG320	(O_MODESWITCH | DM_BG320)	/* Select 320x200 b&w */
				/* Select 320x200 color */
#define	O_SW_CG320	(O_MODESWITCH | DM_CG320)
...
#define	O_SW_CG640x350	(O_MODESWITCH | DM_CG640x350)	/* EGA mode 10 */
					/* EGA mode F* */
#define	O_SW_ENH_MONOAPA2	(O_MODESWITCH | DM_ENHMONOAPA2)
...
#define	CGAIOC		('C' << 8)	/* Upper byte of CGA ioctls */
...
#define	MCAIOC		('M' << 8)	/* Upper byte of MCA ioctls */
...
#define	MAPADAPTER 	('m' << 8)	/* Upper byte of mapping ioctls */
...
#define	MAPSPECIAL	(MAPADAPTER | S_ADAP)	/* Map special adapter memory */

#define	SWAPCONS	('s' << 8)	/* Upper byte of switching ioctls */
...
#define	phystokv(x)	((((ulong)x) >= 0x1000) \
				? (caddr_t)((ulong)(kd_va) + (ulong)(x)) \
				: (caddr_t)((ulong)(p0_va) + (ulong)(x)))

#define	debug_outb(port, val) \
	(void) (prom_printf("%x %x ", port, val), \
	outb(port, val))

/*
 * This is needed because kd is unsafe but tries to do spls when being called
 * from the debugger.  These flags are passed to kdkb_cmd() to tell the lower
 * levels whether or not it is safe to do spls.
 */
...
extern void vcopy(caddr_t from, caddr_t to, int count, int dir);
extern void kdkb_toneoff(caddr_t);
extern int kdkb_mktone();
extern void kdkb_sound(int);
extern void kdkb_tone(void);
extern void kdkb_type();
extern void kdkb_init();
extern int kdclrscr();
extern void kdvt_switch(ushort);
extern int kdv_setxenixfont_bottom(queue_t *qp, mblk_t *mp, mblk_t *private);
extern void kdvm_xenixdoio_bottom(queue_t *qp, mblk_t *mp);
extern void evga_init_bottom(queue_t *, mblk_t *);
extern int evga_ext_init();
extern int evga_ext_rest();
extern void ws_pio_romfont_middle(queue_t *qp, mblk_t *mp);
extern void ws_pio_romfont_bottom(queue_t *qp, mblk_t *mp);
extern void kdvt_sendsig_bottom(queue_t *qp, mblk_t *mp);
extern void kdvt_setmode_bottom(queue_t *qp, mblk_t *mp);
extern int kdkb_locked(ushort ch, unchar kbrk);
extern int kd_xlate_at2xt(int c);
extern void kdvt_close();
extern int kdvt_open();
extern void kdv_setdisp();
extern int kdvm_ioctl(queue_t *qp, mblk_t *mp, int *rvalp, int *need_rval,
    int *hit, int *copyflag);
extern void kbio_setmode(queue_t *qp, mblk_t *mp, struct iocblk *iocp);
extern void kdv_init();
extern int kdv_modromfont(caddr_t buf, unsigned int numchar);
extern int kdv_getxenixfont();
extern int kdv_setxenixfont();
extern int kdv_release_fontmap(void);
extern void kdv_text1();
extern void kdv_textmode();
extern int kdsetcursor();
extern void kdv_scrxfer();
extern int kdv_disptype();
extern int kdv_undattrset();
extern int kdsetbase();
extern void kdv_mvuline();
extern void kdv_setuline();
extern void kdv_shiftset();
extern int kdv_mvword();
extern unchar vdc_rdmon(unchar mode);
extern void vdc_cas2extregs();
extern void vdc_lk750(int mode);
extern void vdc_scrambler(int dir);
extern void vdc_unlkcas2(void);
extern unchar vdc_unlk600(void);
extern int kdv_stchar();
extern void kdv_setcursor_type(int curtype);
extern void kdv_enable();
extern void kdv_rst();
extern void vdc_750cga();
extern void vdc_lktime(int dir);
extern void vdc_check(unchar adptype);
extern void vdc_info();
extern void kdv_setmode();
extern void kdv_adptinit(unchar atype);
extern int kdvt_acq_refuse();
extern int kdvt_rel_refuse();
extern void kdkb_cmd(unchar cmd, unchar whence);
extern void switch_kb_mode(int kbmode);
extern int char_xlate_at2xt(int c);
extern int validproc(proc_t *procp, pid_t pid);
extern int checkio(unsigned short *iolist);
extern int kdv_sborder();
extern int kdv_xenixmode();
extern int kdv_xenixctlr(void);
extern int kdv_colordisp(void);
extern int vdc_disptype();
extern int kdvt_ioctl();
extern int enableio(unsigned short *iolist);
extern int disableio(unsigned short *iolist);
