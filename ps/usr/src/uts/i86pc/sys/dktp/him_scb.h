 ...
 * (c) Copyright Sun Microsystems, Inc. 1993-94
 * All rights reserved.
 *
 * RESTRICTED RIGHTS
 *
...
#ifndef _SYS_DKTP_HIM_SCB_H
#define _SYS_DKTP_HIM_SCB_H

#pragma ident	"@(#)him_scb.h 1.14	94/07/09 SMI"

#ifdef	__cplusplus
extern "C" {
#endif

/* $Header:   V:/source/code/aic-7770/him/himd/him_scb.hv_   1.20   31 May 1994 17:35:58   AHMAD  $ */
/****************************************************************************
  ...
  Description:  This file is comprised of three sections:
                ...
                1. OS specific definitions that can be customized to allow
...
#define SWRD16    unsigned short       /* ja 5/31                          */
#define DWORD     unsigned long        /* Must be 32 bits                  */

#if defined(WINDOWS_NT)
#define UWORD unsigned long /*ULONG*/
#else
#define UWORD unsigned short /*ULONG*/
#endif
...
/* Solaris additions							*/
#define ESA_MAX_DMA_SEGS	32     	/* Max used Scatter/Gather seg	*/
...
/*	Solaris extensions to scb					   */
   struct  esa_scsi_cmd *SCB_cmdp;	/* ptr to the scsi_cmd	*/
   unsigned long SCB_paddr;			/* ccb physical address */
   struct	scsi_arq_status SCB_sense;	/* Auto Request sense	*/
   struct  esa_sg SCB_sg_list[ESA_MAX_DMA_SEGS]; /* scatter/gather */
   unsigned char SCB_CDB_save[MAX_CDB_LEN]; 	/* saved CDB 		*/ 
   UBYTE SCB_SegCnt_save;           /* Number of Scatter/Gather segments   */
   DWORD SCB_SegPtr_save;           /* Pointer to Scatter/Gather list      */
   UBYTE SCB_CDBLen_save;           /* Length of Command Descriptor Block  */
   UBYTE SCB_Cntrl_save;            /* Control register                    */
   struct sequencer_ctrl_block *scb_pforw;
   struct sequencer_ctrl_block *scb_pback;
#ifdef ESA_DEBUG
   struct sequencer_ctrl_block *scb_forw;
   struct sequencer_ctrl_block *scb_back;
#endif
...
typedef struct scbp
{
   SWRD16 Prm_AccessMode;         /* SCB handling mode to use:
                                    0 = Use default
   ...
   SWRD16 Prm_NumberScbs;         /* Number SCBs allowed in sequencer queue,
                                    valid settings are 1 - 255 */

   DWORD  Prm_HimDataPhysaddr;    /* Physical address of HIM data structure */
   SWRD16 Prm_HimDataSize;        /* Working size of HIM data structure in
...
typedef struct him_config_block {
   ...
   SWRD16 Cfe_AdapterID;             /* Host Adapter ID (ie. 0x7770)        */
   ...
   SWRD16 Cfe_StrobeOn;              /* DMA bus timing (ISA)                */
   SWRD16 Cfe_StrobeOff;             /* DMA bus timing (ISA)                */
   UBYTE Cfe_Threshold;             /* Data FIFO threshold                 */
   UBYTE Cfe_Reserved;
                                    /*          SCSI CONFIGURATION         */
   SWRD16 Cfe_ConfigFlags;           /* Flags                               */
   ...
   SWRD16 Cfe_AllowDscnt;            /* Bit map to allow disconnection      */
...
/*	Solaris additions to block			*/
   kmutex_t *eb_mutex;		/* ptr to card mutex      */
   struct esa_card *eb_cardp;	/* back ptr to card info  */ 
   dev_info_t *eb_dip;
   struct sequencer_ctrl_block *eb_scbp;
   struct sequencer_ctrl_block *eb_scbp_que;
   struct sequencer_ctrl_block *eb_last_scbp;
   ushort 	eb_child;
   ushort	eb_pkts_done;
   ushort	eb_pkts_out;
   unsigned char eb_hostid;
   unsigned char eb_flag;
   ddi_softintr_t 	 eb_softid;
#ifdef ESA_DEBUG
   struct sequencer_ctrl_block *eb_scboutp;
#endif

} him_config;

/* eb_flag defines for Solaris in esa_block */
#define ESA_POLLING		1
#define ESA_POLL_TRIGGER_ON	2
...
typedef struct him_data_block {
   ...
   UBYTE HaFlags;                   /* Flags                               */
   SWRD16 Hse_ConfigFlags;
   SWRD16 Hse_AccessMode;
   ...
   SWRD16 Hse_HimDataSize;           /* Working size of HIM data structure in */
...
#define  ESA_ERR        0xff        /* Error return value                  */
...
#define SaveAndDisable()		1
#define SaveAndEnable()		1
#define RestoreState(x)

#endif

#define  PAUSE_SEQ(hcntrl)                            \
         OUTBYTE(hcntrl, (UBYTE)(INBYTE(hcntrl) | PAUSE));  \
         while(!(INBYTE(hcntrl) & PAUSE))

#define  UNPAUSE_SEQ(hcntrl,intstat)                  \
         if ((INBYTE(hcntrl) & PAUSE))                   \
         {                                            \
            if (!(INBYTE(intstat) & ANYPAUSE))           \
               OUTBYTE(hcntrl, (UBYTE)(INBYTE(hcntrl) & ~PAUSE)); \
            INBYTE(hcntrl);                              \
         }                                            

#define  UPDATE_HCNTRL(hcntrl, value)                 \
         OUTBYTE(hcntrl, (UBYTE)(inp(hcntrl) | value))   

#define  WRITE_HCNTRL(hcntrl, value)                  \
         OUTBYTE(hcntrl, value)

#define  READ_HINTSTAT(hcntrl, intstat, intval)  if ((INBYTE(hcntrl) & PAUSE)) \
         {                                                        \
            intval = INBYTE(intstat);                                \
         }                                                        \
         else                                                     \
         {                                                        \
            OUTBYTE(hcntrl, (UBYTE)(INBYTE(hcntrl) | PAUSE));           \
