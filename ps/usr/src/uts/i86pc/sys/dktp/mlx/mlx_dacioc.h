...
#pragma ident	"@(#)mlx_dacioc.h	1.3	95/10/16 SMI"
 ...
 */
#define	MLX_DACIOC	(('M' << 24) | ('D' << 16) | (0x04 << 8))
...
#define	MLX_DACIOC_ENQUIRY	(MLX_DACIOC |13) /* Enquire system config */
#define	MLX_DACIOC_WRCONFIG	(MLX_DACIOC |14) /* Write DAC960 Config */
#define	MLX_DACIOC_RDCONFIG	(MLX_DACIOC |15) /* Read ROM Config */
...
typedef struct  mlx_dacioc_cardinfo {
	u_char slot;		/* where the card is physically installed */
	u_char nchn;		/* number of channels on the card */
	u_char max_tgt;		/* max number of targets per channel */
	u_char irq;
