...
#pragma ident	"@(#)sbpro.h	1.27	96/06/07 SMI"
 ...
 */
typedef struct {
	u_short		ioaddr;		/* card's base i/o address	*/
	...
	u_int		dsp_speed;	/* sampling rate set in DSP	*/
	u_char		dsp_fastmode;	/* currently executing in fast mode? */
	u_char		dsp_stereomode;	/* is DSP in stereo mode?	*/
	u_char		dsp_silent;	/* center amplitude value	*/
...
#define	CBA1847		0x8000	/* is this a Compaq AD1847 ? */
	 ...
	 */
	u_int		buflim;		/* actual length of allocated buffer */
	u_char		*buffers[2];	/* pointers to double buffers */
	u_int		buflen;		/* length of each buffer */
	int		length[2];	/* length of data in buffer */
	u_int		bufnum;		/* buffer currently being transferred */
